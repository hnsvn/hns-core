...
#define HNS_ENABLE_STATIC_PINS enable_static_pins_ = true;

#else

// Leave static pins disabled on Android and iOS, like upstream.
#define HNS_ENABLE_STATIC_PINS \
  {}
...
class HSTSPartitionHashHelper {
 public:
  static std::string GetPartitionDomain(const SchemefulSite& schemeful_site) {
    DCHECK(base::FeatureList::IsEnabled(features::kHnsPartitionHSTS));
...
GetPartitionHashForAddingHSTS(const IsolationInfo& isolation_info) {
  if (!base::FeatureList::IsEnabled(features::kHnsPartitionHSTS)) {
    return absl::nullopt;
  }
...
bool TransportSecurityState::DeleteDynamicDataForHost(const std::string& host) {
  ...
  if (hns_deleted)
    DirtyNotify();
  return chromium_deleted || hns_deleted;
}
