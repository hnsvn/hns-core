...
def is_gen_hns_dir(out_dir):
    GEN_HNS = 'gen/hns'
    dirs = out_dir.split('/')
    # Test gen/hns/... and <target_toolchain>/gen/hns/...
    return '/'.join(dirs[:2]) == GEN_HNS or '/'.join(dirs[1:3]) == GEN_HNS
...
def main(original_function, argv):
    ...
    if args.in_files is not None:
        for f in args.in_files:
            ...
            # input file."
            if args.root_dir != args.out_dir and is_gen_hns_dir(args.out_dir):
