...
def _ProcessEntry(original_function, entry, filtered_args, target_os):
    ...
    hns_chromium_src_file = re.sub(r'^(\.\./\.\./|gen/)',
                                     '../../hns/chromium_src/', entry_file)
    if hns_chromium_src_file != entry_file:
        abs_hns_chromium_src_file = os.path.join(entry['directory'],
                                                   hns_chromium_src_file)
        if os.path.exists(abs_hns_chromium_src_file):
            entry['command'] = entry['command'].replace(
                entry_file, hns_chromium_src_file)
            entry['file'] = hns_chromium_src_file
