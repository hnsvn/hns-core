...
constexpr SkColor kHnsBrandColor = SkColorSetRGB(0xff, 0x76, 0x54);
constexpr SkColor kHnsPrimaryColor = SkColorSetRGB(32, 74, 227);
  ...
  static base::NoDestructor<base::flat_map<MdTextButtonStyleKey, ButtonStyle>>
      button_themes(
          {{{Kind::kPrimary, ColorScheme::kLight, ButtonState::STATE_NORMAL},
            {.background_color = kHnsPrimaryColor,
           ...
           {{Kind::kSecondary, ColorScheme::kLight, ButtonState::STATE_HOVERED},
            {.background_color = absl::nullopt,
             .border_color = kHnsPrimaryColor,
             .text_color = kHnsPrimaryColor}},
...
class HnsTextButtonHighlightPathGenerator
    : public views::HighlightPathGenerator {
 public:
  HnsTextButtonHighlightPathGenerator() = default;
  HnsTextButtonHighlightPathGenerator(
      const HnsTextButtonHighlightPathGenerator&) = delete;
  HnsTextButtonHighlightPathGenerator& operator=(
      const HnsTextButtonHighlightPathGenerator&) = delete;
...
MdTextButton::MdTextButton(PressedCallback callback,
  ...
  views::HighlightPathGenerator::Install(
      this, std::make_unique<HnsTextButtonHighlightPathGenerator>());
...
void MdTextButton::UpdateTextColor() {
  ...
  // Once we update the buttons across Hns to use the new style, we can remove
  // this branch.
  if (kind_ == kOld) {
    if (GetProminent()) {
      return;
    ...
    if (theme->GetPlatformHighContrastColorScheme() !=
      ...
      SetTextColor(ButtonState::STATE_HOVERED, kHnsBrandColor);
      SetTextColor(ButtonState::STATE_PRESSED, kHnsBrandColor);
    }
    return;
  }
...
void MdTextButton::UpdateBackgroundColor() {
  ...
  if (kind_ == kOld) {
    ...
    if (GetState() == ButtonState::STATE_PRESSED ||
      ...
      // The only thing that differs for Hns is the stroke color
      SkColor stroke_color = kHnsBrandColor;
...
SkPath HnsTextButtonHighlightPathGenerator::GetHighlightPath(
