...
ScriptValue FarbleGLIntParameter(WebGL2RenderingContextBase* owner,
  ...
  if (value > 0) {
    hns::FarblingPRNG prng =
        hns::HnsSessionCache::From(*ExecutionContext::From(script_state))
...
#define HNS_WEBGL2_RENDERING_CONTEXT_BASE                              \
  if (!hns::AllowFingerprinting(ExecutionContext::From(script_state))) \
    return ScriptValue::CreateNull(script_state->GetIsolate());

#define HNS_WEBGL2_RENDERING_CONTEXT_BASE_GETPARAMETER                \
  switch (hns::GetHnsFarblingLevelFor(                              \
      ExecutionContext::From(script_state), HnsFarblingLevel::OFF)) { \
...
#undef HNS_WEBGL2_RENDERING_CONTEXT_BASE
#undef HNS_WEBGL2_RENDERING_CONTEXT_BASE_GETPARAMETER
