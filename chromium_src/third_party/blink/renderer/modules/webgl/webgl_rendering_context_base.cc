...
bool AllowFingerprintingForHost(blink::CanvasRenderingContextHost* host) {
  if (!host)
    return true;
  return hns::AllowFingerprinting(host->GetTopExecutionContext());
}

}  // namespace

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_RETURN \
  if (!AllowFingerprintingForHost(Host()))        \
    return;

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_NULLPTR \
  if (!AllowFingerprintingForHost(Host()))         \
    return nullptr;

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_NULLOPT \
  if (!AllowFingerprintingForHost(Host()))         \
    return absl::nullopt;

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_ZERO \
  if (!AllowFingerprintingForHost(Host()))      \
    return 0;

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_MINUS_ONE \
  if (!AllowFingerprintingForHost(Host()))           \
    return -1;

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_SCRIPT_VALUE \
  if (!AllowFingerprintingForHost(Host()))              \
    return ScriptValue::CreateNull(script_state->GetIsolate());

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_STRING \
  if (!AllowFingerprintingForHost(Host()))        \
    return String();

#define HNS_WEBGL_RENDERING_CONTEXT_BASE_GETSHADERINFOLOG \
  if (!AllowFingerprintingForHost(Host())) {                \
    range[0] = 0;                                           \
    range[1] = 0;                                           \
    precision = 0;                                          \
...
#define HNS_WEBGL_GET_PARAMETER_UNMASKED_RENDERER     \
  if (ExtensionEnabled(kWebGLDebugRendererInfoName) &&  \
      !AllowFingerprintingForHost(Host()))              \
    return WebGLAny(                                    \
        script_state,                                   \
        String(hns::HnsSessionCache::From(          \
                   *(Host()->GetTopExecutionContext())) \
                   .GenerateRandomString("UNMASKED_RENDERER_WEBGL", 8)));

#define HNS_WEBGL_GET_PARAMETER_UNMASKED_VENDOR       \
  if (ExtensionEnabled(kWebGLDebugRendererInfoName) &&  \
      !AllowFingerprintingForHost(Host()))              \
    return WebGLAny(                                    \
        script_state,                                   \
...
#undef HNS_WEBGL_GET_PARAMETER_UNMASKED_RENDERER
#undef HNS_WEBGL_GET_PARAMETER_UNMASKED_VENDOR
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_STRING
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_SCRIPT_VALUE
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_MINUS_ONE
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_ZERO
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_NULLOPT
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_NULLPTR
#undef HNS_WEBGL_RENDERING_CONTEXT_BASE_RETURN
