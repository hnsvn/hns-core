...
#define HNS_GET_IMAGE_DATA                                                  \
  if (ExecutionContext* context = ExecutionContext::From(script_state)) {     \
    SkPixmap image_data_pixmap = image_data->GetSkPixmap();                   \
    hns::HnsSessionCache::From(*context).PerturbPixels(                   \
        static_cast<const unsigned char*>(image_data_pixmap.writable_addr()), \
        image_data_pixmap.computeByteSize());                                 \
  }

#define HNS_BASE_RENDERING_CONTEXT_2D_MEASURE_TEXT         \
  if (!hns::AllowFingerprinting(GetTopExecutionContext())) \
    return MakeGarbageCollected<TextMetrics>();

#define HNS_GET_IMAGE_DATA_PARAMS ScriptState *script_state,
...
#undef getImageData
#undef HNS_GET_IMAGE_DATA_PARAMS
#undef HNS_GET_IMAGE_DATA
#undef HNS_BASE_RENDERING_CONTEXT_2D_MEASURE_TEXT

namespace {

bool AllowFingerprintingFromScriptState(blink::ScriptState* script_state) {
  return hns::AllowFingerprinting(
