...
#include "third_party/blink/renderer/modules/storage/hns_dom_window_storage.h"
...
// static
const char HnsDOMWindowStorage::kSupplementName[] = "HnsDOMWindowStorage";

// static
HnsDOMWindowStorage& HnsDOMWindowStorage::From(LocalDOMWindow& window) {
  HnsDOMWindowStorage* supplement =
      Supplement<LocalDOMWindow>::From<HnsDOMWindowStorage>(window);
  if (!supplement) {
    supplement = MakeGarbageCollected<HnsDOMWindowStorage>(window);
...
// static
StorageArea* HnsDOMWindowStorage::sessionStorage(
    LocalDOMWindow& window,
...
// static
StorageArea* HnsDOMWindowStorage::localStorage(
    LocalDOMWindow& window,
    ExceptionState& exception_state) {
  return From(window).localStorage(exception_state);
}

HnsDOMWindowStorage::HnsDOMWindowStorage(LocalDOMWindow& window)
...
StorageArea* HnsDOMWindowStorage::ephemeralSessionStorage() {
...
void HnsDOMWindowStorage::Trace(Visitor* visitor) const {
