...
String PluginReplacementName(FarblingPRNG* prng) {
  std::vector<String> chrome{"Chrome ", "Chromium ",   "Hns ",
...
void FarblePlugins(DOMPluginArray* owner,
  ...
  switch (hns::GetHnsFarblingLevelFor(owner->DomWindow(),
    ...
    case HnsFarblingLevel::BALANCED: {
      ...
      FarblingPRNG prng = HnsSessionCache::From(*(frame->DomWindow()))
      ...
      for (unsigned index = 0; index < dom_plugins->size(); index++) {
        ...
        if ((name == "Chrome PDF Plugin") || (name == "Chrome PDF Viewer")) {
          ...
          plugin->SetFilename(
              HnsSessionCache::From(*(frame->DomWindow()))
...
#define HNS_DOM_PLUGINS_UPDATE_PLUGIN_DATA__RESET_PLUGIN_DATA \
  if (PluginData* data = GetPluginData())                       \
    data->ResetPluginData();

#define HNS_DOM_PLUGINS_UPDATE_PLUGIN_DATA__FARBLE_PLUGIN_DATA \
  hns::FarblePlugins(this, data, &dom_plugins_);

#include "src/third_party/blink/renderer/modules/plugins/dom_plugin_array.cc"

#undef HNS_DOM_PLUGINS_UPDATE_PLUGIN_DATA__FARBLE_PLUGIN_DATA
#undef HNS_DOM_PLUGINS_UPDATE_PLUGIN_DATA__RESET_PLUGIN_DATA
