...
_IS_PG_ENABLED = override_utils.get_gn_arg("enable_hns_page_graph")
# Get gn arg to enable WebAPI probes.
_IS_PG_WEBAPI_PROBES_ENABLED = override_utils.get_gn_arg(
    "enable_hns_page_graph_webapi_probes")


def _add_page_graph_to_config(config):
    config["observers"]["PageGraph"] = {
        "include_path": "hns/third_party/blink/renderer/core/hns_page_graph",
        "probes": [
            "NodeCreated",
            "DidCommitLoad",
            "DidInsertDOMNode",
    ...
    if _IS_PG_WEBAPI_PROBES_ENABLED:
        config["settings"]["includes"].extend([
            "hns/third_party/blink/renderer/core/hns_page_graph/blink_converters.h",
            "hns/third_party/blink/renderer/core/hns_page_graph/blink_probe_types.h",
        ])
