...
const display::ScreenInfos& ChromeClientImpl::HnsGetScreenInfos(
  ...
  if (!hns::BlockScreenFingerprinting(context)) {
    return GetScreenInfos(frame);
  }
  // Don't tell window screen is smaller than 450x450.
  int min_width =
      FarbleInteger(context, hns::FarbleKey::kWindowInnerWidth, 450, 0, 8);
  int min_height =
      FarbleInteger(context, hns::FarbleKey::kWindowInnerHeight, 450, 0, 8);
