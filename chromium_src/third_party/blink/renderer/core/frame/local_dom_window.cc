...
using hns::BlockScreenFingerprinting;
using hns::FarbleInteger;
using hns::FarbleKey;
...
int LocalDOMWindow::outerWidth() const {
  ...
  return BlockScreenFingerprinting(context) && top_window
             ? FarbleInteger(context, hns::FarbleKey::kWindowInnerWidth,
...
int LocalDOMWindow::outerHeight() const {
  ...
  auto* top_window = DynamicTo<LocalDOMWindow>(top());
  return BlockScreenFingerprinting(context) && top_window
             ? FarbleInteger(context, hns::FarbleKey::kWindowInnerHeight,
...
int LocalDOMWindow::screenX() const {
  ...
  return BlockScreenFingerprinting(context)
             ? FarbleInteger(context, hns::FarbleKey::kWindowScreenX, 0, 0, 8)
...
int LocalDOMWindow::screenY() const {
  ...
  ExecutionContext* context = GetExecutionContext();
  return BlockScreenFingerprinting(context)
             ? FarbleInteger(context, hns::FarbleKey::kWindowScreenY, 0, 0, 8)
