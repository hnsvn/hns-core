...
float FarbleDeviceMemory(blink::ExecutionContext* context) {
  ...
  HnsFarblingLevel farbling_level =
      hns::GetHnsFarblingLevelFor(context, HnsFarblingLevel::OFF);
  // If Hns Shields are down or anti-fingerprinting is off for this site,
  // return the true value.
  if (farbling_level == HnsFarblingLevel::OFF)
    return true_value;

  std::vector<float> valid_values = {0.25, 0.5, 1.0, 2.0, 4.0, 8.0};
  size_t min_farbled_index;
  size_t max_farbled_index;
  if (farbling_level == HnsFarblingLevel::MAXIMUM) {
...
float NavigatorDeviceMemory::deviceMemory(ScriptState* script_state) const {
  ...
  return hns::FarbleDeviceMemory(context);
}
