...
// All HnsVpn services have support of "install" switch to make registration.
constexpr char kHnsVpnServiceInstall[] = "install";

base::FilePath GetHnsVPNHelperPath(const base::FilePath& target_path,
                                     const base::Version& version) {
  return target_path.AppendASCII(version.GetString())
      .Append(hns_vpn::kHnsVPNHelperExecutable);
}

bool InstallHnsVPNService(const base::FilePath& exe_path,
                            const CallbackWorkItem&) {
  if (!base::PathExists(exe_path)) {
    return false;
  }
  base::CommandLine cmd(exe_path);
  cmd.AppendSwitch(kHnsVpnServiceInstall);
  base::LaunchOptions options = base::LaunchOptions();
  options.wait = true;
  return base::LaunchProcess(cmd, base::LaunchOptions()).IsValid();
}

// Adds work items to register hns vpn helper service for Windows. Only for
// system level installs.
void AddHnsVPNHelperServiceWorkItems(const base::FilePath& vpn_service_path,
                                       WorkItemList* list) {
  DCHECK(::IsUserAnAdmin());

  if (vpn_service_path.empty()) {
    VLOG(1) << "The path to hns_vpn_helper.exe is invalid.";
    return;
  }
  WorkItem* install_service_work_item = new installer::InstallServiceWorkItem(
      hns_vpn::GetHnsVpnHelperServiceName(),
      hns_vpn::GetHnsVpnHelperServiceDisplayName(), SERVICE_DEMAND_START,
  ...
  list->AddCallbackWorkItem(
      base::BindOnce(&InstallHnsVPNService, vpn_service_path),
      base::NullCallback());
}

// Adds work items to register hns vpn wireguard service for Windows. Only for
// system level installs.
void AddHnsVPNWireguardServiceWorkItems(
    const base::FilePath& wireguard_service_path,
    WorkItemList* list) {
  DCHECK(::IsUserAnAdmin());
  if (wireguard_service_path.empty()) {
    VLOG(1) << "The path to hns_vpn_wireguard_service.exe is invalid.";
    return;
  }
  list->AddCallbackWorkItem(
      base::BindOnce(&InstallHnsVPNService, wireguard_service_path),
      base::NullCallback());
}
...
#define GetElevationServicePath GetElevationServicePath(target_path, new_version), install_list); \
  AddHnsVPNWireguardServiceWorkItems(                                                           \
      hns_vpn::GetHnsVPNWireguardServiceInstallationPath(target_path,                         \
                                                             new_version),                        \
      install_list);                                                                              \
  AddHnsVPNHelperServiceWorkItems(GetHnsVPNHelperPath
