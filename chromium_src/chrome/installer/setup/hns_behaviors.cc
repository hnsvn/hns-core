...
// changes:
//
// - `distribution_data` not appended as Hns does not record histograms.
// - `kHnsUninstallSurveyUrl` used instead of `kUninstallSurveyUrl`

constexpr base::WStringPiece kHnsUninstallSurveyUrl(
    L"https://hns.com/uninstall-survey/?p=hns_uninstall_survey");
...
void DoPostUninstallOperations(const base::Version& version,
  ...
  const std::wstring survey_url = std::wstring(kHnsUninstallSurveyUrl);
#if DCHECK_IS_ON()
