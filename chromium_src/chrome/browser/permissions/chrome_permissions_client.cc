...
bool ChromePermissionsClient::HnsCanBypassEmbeddingOriginCheck(
  ...
  // check it against the embedding origin for HNS_ETHEREUM and HNS_SOLANA
  ...
  // We instead handle this in hns_wallet_render_frame_observer.cc by not
...
ChromePermissionsClient::MaybeCreateMessageUI(
  ...
  if (requests.size() > 0) {
    ...
    if (request_type == permissions::RequestType::kHnsEthereum ||
        request_type == permissions::RequestType::kHnsSolana) {
      if (request_type == permissions::RequestType::kHnsSolana) {
        coin_type = hns_wallet::mojom::CoinType::SOL;
      }
      auto delegate = std::make_unique<HnsWalletPermissionPrompt::Delegate>(
          std::move(prompt));
      return std::make_unique<HnsWalletPermissionPrompt>(
