...
constexpr size_t kHnsDefaultAvatarIconsCount = 34;
#else
  constexpr size_t kHnsDefaultAvatarIconsCount = 0;
#endif

const IconResourceInfo* GetHnsDefaultAvatarIconResourceInfo(
      size_t chromium_index);

size_t GetHnsAvatarIconStartIndex();

#define HNS_GET_DEFAULT_AVATAR_ICON_RESOURCE_INFO              \
  size_t hns_start_index =                                     \
      kDefaultAvatarIconsCount - kHnsDefaultAvatarIconsCount;  \
  if (index >= hns_start_index) {                              \
    size_t hns_icon_index = index - hns_start_index;         \
    const IconResourceInfo* hns_icon =                         \
        GetHnsDefaultAvatarIconResourceInfo(hns_icon_index); \
    if (hns_icon)                                              \
      return hns_icon;                                         \
  }

#define HNS_GET_MODERN_AVATAR_ICON_START_INDEX  \
  return GetHnsAvatarIconStartIndex();

#define HNS_GET_ICONS_AND_LABELS_FOR_PROFILE_AVATAR_SELECTOR_NOT_SIGNED_IN \
  avatars.erase(avatars.begin());                                            \
  generic_avatar_info = GetDefaultProfileAvatarIconAndLabel_Hns(           \
...
#undef HNS_GET_DEFAULT_AVATAR_ICON_RESOURCE_INFO
#undef HNS_GET_MODERN_AVATAR_ICON_START_INDEX
#undef HNS_GET_ICONS_AND_LABELS_FOR_PROFILE_AVATAR_SELECTOR_NOT_SIGNED_IN
...
size_t GetHnsAvatarIconStartIndex() {
  return kDefaultAvatarIconsCount - kHnsDefaultAvatarIconsCount;
}
...
const IconResourceInfo* GetHnsDefaultAvatarIconResourceInfo(
  ...
  return nullptr;
#else
  CHECK_LT(index, kHnsDefaultAvatarIconsCount);
...
base::Value::Dict GetDefaultProfileAvatarIconAndLabel(SkColor fill_color,
  ...
  return GetAvatarIconAndLabelDict(webui::GetBitmapDataUrl(icon.AsBitmap()),
                                   hns_l10n::GetLocalizedResourceUTF16String(
                                       IDS_HNS_AVATAR_LABEL_PLACEHOLDER),
