...
#include "hns/browser/renderer_context_menu/hns_spelling_options_submenu_observer.h"
...
AutocompleteMatch GetAutocompleteMatchForText(Profile* profile,
  ...
  AutocompleteClassifier classifier(
      ...
      std::make_unique<HnsAutocompleteSchemeClassifier>(profile));
...
base::OnceCallback<void(HnsRenderViewContextMenu*)>*
HnsGetMenuShownCallback() {
...
void RenderViewContextMenu::RegisterMenuShownCallbackForTesting(
  ...
  *HnsGetMenuShownCallback() = std::move(cb);
}

#define HNS_APPEND_SEARCH_PROVIDER \
...
#define SpellingOptionsSubMenuObserver HnsSpellingOptionsSubMenuObserver
...
#undef RenderViewContextMenu
#undef HNS_APPEND_SEARCH_PROVIDER
...
void OnGetImageForTextCopy(base::WeakPtr<content::WebContents> web_contents,
  ...
  hns::ShowTextRecognitionDialog(web_contents.get(), image);
}
#endif

}  // namespace

HnsRenderViewContextMenu::HnsRenderViewContextMenu(
...
bool HnsRenderViewContextMenu::IsCommandIdEnabled(int id) const {
  switch (id) {
#if BUILDFLAG(ENABLE_TEXT_RECOGNITION)
...
#if BUILDFLAG(ENABLE_TOR)
      if (hns::IsTorDisabledForProfile(GetProfile()))
        return false;
...
#if BUILDFLAG(ENABLE_IPFS)
void HnsRenderViewContextMenu::ExecuteIPFSCommand(int id, int event_flags) {
...
void HnsRenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  switch (id) {
    case IDC_COPY_CLEAN_LINK: {
      ...
      hns::CopyLinkWithStrictCleaning(GetBrowser(), link_url);
    }; break;
    case IDC_CONTENT_CONTEXT_FORCE_PASTE: {
...
#if BUILDFLAG(ENABLE_TEXT_RECOGNITION)
void HnsRenderViewContextMenu::CopyTextFromImage() {
...
void HnsRenderViewContextMenu::AddSpellCheckServiceItem(bool is_checked) {
  // Call our implementation, not the one in the base class.
  // Assumption:
  // Use of spelling service is disabled in Hns profile preferences.
...
// static
void HnsRenderViewContextMenu::AddSpellCheckServiceItem(
    ui::SimpleMenuModel* menu,
    bool is_checked) {
  // Suppress adding "Spellcheck->Ask Hns for suggestions" item.
}

void HnsRenderViewContextMenu::AddAccessibilityLabelsServiceItem(
    bool is_checked) {
  // Suppress adding "Get image descriptions from Hns"
}

#if BUILDFLAG(ENABLE_IPFS)
bool HnsRenderViewContextMenu::IsIPFSCommandIdEnabled(int command) const {
...
void HnsRenderViewContextMenu::SeIpfsIconAt(int index) {
...
void HnsRenderViewContextMenu::BuildIPFSMenu() {
  if (!ipfs::IsIpfsMenuEnabled(GetProfile()->GetPrefs()))
    return;
...
void HnsRenderViewContextMenu::InitMenu() {
...
void HnsRenderViewContextMenu::NotifyMenuShown() {
  auto* cb = HnsGetMenuShownCallback();
  if (!cb->is_null()) {
