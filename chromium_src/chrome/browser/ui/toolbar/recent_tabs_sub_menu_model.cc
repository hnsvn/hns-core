...
const char kHnsStubSessionTag[] = "hns_stub_more_session_tag";
const char kHnsSyncedTabsUrl[] = "hns://history/syncedTabs";

}  //  namespace

// Patched because this inserting should be done before BuildLocalEntries() in
// ctor and only once.
#define HNS_RECENT_TABS_SUB_MENU_MODEL_BUILD \
  InsertItemWithStringIdAt(1, IDC_CLEAR_BROWSING_DATA, IDS_CLEAR_BROWSING_DATA);

#define HNS_RECENT_TABS_SUB_MENU_MODEL_BUILD_TABS_FROM_OTHER_DEVICES      \
  if (tabs_in_session.size() > kMaxTabsPerSessionToShow) {                  \
    ...
    if (!stub_tab_.get()) {                                                 \
      ...
      stub_nav_entry.set_title(hns_l10n::GetLocalizedResourceUTF16String( \
          IDS_OPEN_MORE_OTHER_DEVICES_SESSIONS));                           \
      stub_nav_entry.set_virtual_url(GURL(kHnsSyncedTabsUrl));            \
    ...
    BuildOtherDevicesTabItem(this, kHnsStubSessionTag,                    \
                             *tabs_in_session[kMaxTabsPerSessionToShow]);   \
  }

#include "src/chrome/browser/ui/toolbar/recent_tabs_sub_menu_model.cc"

#undef HNS_RECENT_TABS_SUB_MENU_MODEL_BUILD_TABS_FROM_OTHER_DEVICES
#undef HNS_RECENT_TABS_SUB_MENU_MODEL_BUILD

#include "hns/browser/ui/toolbar/hns_recent_tabs_sub_menu_model.h"

// Methods of HnsRecentTabsSubMenuModel are implemented below instead of
// hns_recent_tabs_sub_menu_model.cc to have the access to functions in
// anonymous namespace in recent_tabs_sub_menu_model.cc

HnsRecentTabsSubMenuModel::HnsRecentTabsSubMenuModel(
...
void HnsRecentTabsSubMenuModel::ExecuteCommand(int command_id,
                                                 int event_flags) {
  if (IsTabModelCommandId(command_id)) {
    ...
    if (item.session_tag == kHnsStubSessionTag) {
      ShowSingletonTabOverwritingNTP(browser_, GURL(kHnsSyncedTabsUrl));
      return;
    }
  }
