...
void AddHnsOmniboxColorMixer(ui::ColorProvider* provider,
                               const ui::ColorProviderKey& key) {
  // Apply hns theme when there is no custom theme.
  if (key.custom_theme)
    return;
#if !BUILDFLAG(IS_ANDROID)
  key.color_mode == ui::ColorProviderKey::ColorMode::kDark
      ? AddHnsOmniboxDarkThemeColorMixer(provider, key)
      : AddHnsOmniboxLightThemeColorMixer(provider, key);
...
void AddOmniboxColorMixer(ui::ColorProvider* provider,
  ...
  AddHnsOmniboxColorMixer(provider, key);
}
