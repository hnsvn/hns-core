...
class HnsStartupBrowserCreatorImpl final : public StartupBrowserCreatorImpl {
 public:
  HnsStartupBrowserCreatorImpl(const base::FilePath& cur_dir,
...
HnsStartupBrowserCreatorImpl::HnsStartupBrowserCreatorImpl(
...
// won't be launched.
void HnsStartupBrowserCreatorImpl::Launch(
    Profile* profile,
    chrome::startup::IsProcessStartup process_startup,
    std::unique_ptr<OldLaunchModeRecorder> launch_mode_recorder) {
#if BUILDFLAG(ENABLE_TOR)
  if (StartupBrowserCreatorImpl::command_line_->HasSwitch(switches::kTor)) {
    ...
    // one runs hns-browser --tor "? search query" the search query is not
...
#define StartupBrowserCreatorImpl HnsStartupBrowserCreatorImpl
