...
ui::SimpleMenuModel* DownloadShelfContextMenuView::GetMenuModel() {
  ...
  if (auto index = model->GetIndexOfCommandId(
          static_cast<int>(HnsDownloadCommands::REMOVE_FROM_LIST))) {
    return model;
  }
  ...
  if (auto index =
    ...
    model->InsertItemAt(
        *index + 1, static_cast<int>(HnsDownloadCommands::REMOVE_FROM_LIST),
...
bool DownloadShelfContextMenuView::IsCommandIdEnabled(int command_id) const {
  if (static_cast<HnsDownloadCommands>(command_id) ==
      HnsDownloadCommands::REMOVE_FROM_LIST) {
    return true;
  }
