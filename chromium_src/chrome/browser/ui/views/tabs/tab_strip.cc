...
#define AddTab(TAB, MODEL_INDEX, PINNED) \
  AddTab(std::make_unique<HnsTab>(this), MODEL_INDEX, PINNED)
#define CompoundTabContainer HnsCompoundTabContainer
...
#define HNS_TAB_DRAG_CONTEXT_IMPL_CALCULATE_INSERTION_INDEX                \
...
#define HNS_TAB_DRAG_CONTEXT_IMPL_CALCULATE_BOUNDS_FOR_DRAGGED_VIEWS      \
  if (base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs) &&   \
      tabs::utils::ShouldShowVerticalTabs(tab_strip_->GetBrowser())) {      \
    return tabs::CalculateBoundsForVerticalDraggedViews(views, tab_strip_); \
  }

#define HNS_TAB_DRAG_CONTEXT_IMPL_PAINT_CHILDREN                        \
  if (base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs) && \
      tabs::utils::ShouldShowVerticalTabs(tab_strip_->GetBrowser())) {    \
...
#undef HNS_TAB_DRAG_CONTEXT_IMPL_PAINT_CHILDREN
#undef HNS_TAB_DRAG_CONTEXT_IMPL_CALCULATE_BOUNDS_FOR_DRAGGED_VIEWS
#undef HNS_TAB_DRAG_CONTEXT_IMPL_CALCULATE_INSERTION_INDEX
