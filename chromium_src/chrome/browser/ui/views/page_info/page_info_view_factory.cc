...
std::unique_ptr<RichHoverButton> CreateButton(
  ...
  const std::u16string& title_text =
      hns_l10n::GetLocalizedResourceUTF16String(text_resource_id);
  const std::u16string& tooltip_text =
      hns_l10n::GetLocalizedResourceUTF16String(tooltip_resource_id);
...
void HnsAddIPFSButtons(views::View* container,
                         ChromePageInfoUiDelegate* delegate) {
  if (!container || !delegate)
    return;
  container->AddChildView(CreateButton(
      IDR_HNS_IPFS_LOGO, IDS_PAGE_INFO_IPFS_SETTINGS_BUTTON_TEXT,
      ...
      base::BindRepeating(
          ...
          delegate)));
  container->AddChildView(CreateButton(
      IDR_HNS_IPFS_LOGO, IDS_PAGE_INFO_IPFS_DIAGNOSTICS_BUTTON_TEXT,
      ...
      base::BindRepeating(
          ...
          delegate)));
  container->AddChildView(CreateButton(
      IDR_HNS_IPFS_LOGO, IDS_PAGE_INFO_IPFS_DOCS_BUTTON_TEXT,
...
#define HNS_PAGE_INFO_VIEW_FACTORY_GET_PERMISSION_ICON \
...
#undef HNS_PAGE_INFO_VIEW_FACTORY_GET_PERMISSION_ICON
...
std::unique_ptr<views::View> PageInfoViewFactory::CreateSecurityPageView() {
  ...
  if (!ipfs::IsIPFSScheme(presenter_->site_url()))
    return page_view;
  HnsAddIPFSButtons(page_view.get(), ui_delegate_);
  return page_view;
}
#endif  // BUILDFLAG(ENABLE_IPFS)
