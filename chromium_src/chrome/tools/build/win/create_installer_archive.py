...
def CopyAllFilesToStagingDir(original_function, config, distribution,
    ...
    hns_extension_locales_src_dir_path = os.path.realpath(
        os.path.join(get_src_dir(), 'hns', 'components', 'hns_extension',
                     'extension', 'hns_extension', '_locales'))
    CopyExtensionLocalization('hns_extension',
                              hns_extension_locales_src_dir_path, config,
...
def GetPrevVersion(_, build_dir, temp_dir, last_chrome_installer, output_name):
    ...
    # in (chrome.7z in) Hns's installation directory when the delta gets
...
def CopyExtensionLocalization(extension_name, locales_src_dir_path, config,
    ...
    locales_dest_path = os.path.join(
        locales_dest_path, config.get('GENERAL', 'hns_resources.pak'),
...
def SignAndCopyPreSignedBinaries(skip_signing, output_dir, staging_dir,
    ...
    if not skip_signing:
        ...
        shutil.copy(os.path.join(src_dir, 'hns.exe'), chrome_dir)
