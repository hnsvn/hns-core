...
class HnsChromeTestSetupHelper : public testing::EmptyTestEventListener {
 public:
  ...
  HnsChromeTestSetupHelper() = default;
  ~HnsChromeTestSetupHelper() override = default;

  // testing::EmptyTestEventListener:
  void OnTestStart(const testing::TestInfo& test_info) override {
    static const base::NoDestructor<std::vector<TestAdjustments>>
        kTestAdjustments({
            // We allow FileSystem API via hns://flags, so enable it here for
            // upstream tests.
            {
                .test_patterns =
                    {
...
void ChromeTestSuite::Initialize() {
  ...
  listeners.Append(new HnsChromeTestSetupHelper());
}
