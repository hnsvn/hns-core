...
def maybe_load_overrides():
    ...
    hns_root = os.path.join(get_root(), 'hns')
    overrides_file_path = f'{hns_root}/vector_icons/leo_overrides.json'
    ...
    for icon, leo_override in data.items():
        ...
        leo_overrides[icon] = \
          f'../../hns/node_modules/@hns/leo/icons-skia/{leo_override}.icon'
...
def get_icon_path(icon_path, alt_working_directory):
    ...
    2. The Hns equivalent of the the Chromium icon path.
    3. The Chromium Path (this is the default, if no override is specified).

    Args:
        icon_path: The path to the chromium icon
        alt_working_directory: The Hns overrides folder for this icon.
  """
