...
#include "hns/components/permissions/android/jni_headers/HnsPermissionDialogDelegate_jni.h"
...
void SetLifetimeOptions(const base::android::JavaRef<jobject>& j_delegate) {
  ...
  Java_HnsPermissionDialogDelegate_setLifetimeOptionsText(
      env, j_delegate,
      base::android::ConvertUTF16ToJavaString(
          env, hns_l10n::GetLocalizedResourceUTF16String(
  ...
  Java_HnsPermissionDialogDelegate_setLifetimeOptions(
      env, j_delegate,
...
void ApplyLifetimeToPermissionRequests(
  ...
  const int selected_lifetime_option =
      Java_HnsPermissionDialogDelegate_getSelectedLifetimeOption(env, obj);
...
void Java_PermissionDialogController_createDialog_HnsImpl(
    JNIEnv* env,
...
#define HNS_PERMISSION_DIALOG_DELEGATE_ACCEPT \
  ApplyLifetimeToPermissionRequests(env, obj, permission_prompt_);
#define HNS_PERMISSION_DIALOG_DELEGATE_CANCEL \
  ApplyLifetimeToPermissionRequests(env, obj, permission_prompt_);
#define Java_PermissionDialogController_createDialog \
  Java_PermissionDialogController_createDialog_HnsImpl
...
#undef HNS_PERMISSION_DIALOG_DELEGATE_CANCEL
#undef HNS_PERMISSION_DIALOG_DELEGATE_ACCEPT
