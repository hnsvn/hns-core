...
const char kHnsDefaultServiceName[] = "Hns Safe Storage";
const char kHnsDefaultAccountName[] = "Hns";

KeychainPassword::KeychainNameType& GetHnsServiceName();
KeychainPassword::KeychainNameType& GetHnsAccountName();

}  // namespace

#define HNS_GET_SERVICE_NAME return GetHnsServiceName();
#define HNS_GET_ACCOUNT_NAME return GetHnsAccountName();
#include "src/components/os_crypt/sync/keychain_password_mac.mm"
#undef HNS_GET_SERVICE_NAME
#undef HNS_GET_ACCOUNT_NAME
...
std::pair<std::string, std::string> GetServiceAndAccountName() {
  ...
  if (command_line->HasSwitch("import-edge")) {
    ...
    account_name = std::string("Vivaldi");
             ...
             command_line->HasSwitch("import-hns")) {
    ...
    service_name = std::string(kHnsDefaultServiceName);
    account_name = std::string(kHnsDefaultAccountName);
  }
