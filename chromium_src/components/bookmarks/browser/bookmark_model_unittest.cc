...
TEST_F(BookmarkModelTest, HnsMigrateOtherNodeFolder) {
  // -- Bookmarks
  // |-- A
  // |-- Other Bookmarks
  //     |-- B
  ...
  // |   |--B1.com
  // |-- C.com
  HnsMigrateOtherNodeFolder(model_.get());
...
TEST_F(BookmarkModelTest, HnsMigrateOtherNodeFolderNotExist) {
...
TEST_F(BookmarkModelTest, HnsClearSyncV1MetaInfo) {
  ...
  model_->SetNodeMetaInfo(bookmark_C1, "hns_meta", "hns_meta_value");
  ...
  std::string hns_meta;
  ASSERT_TRUE(bookmark_C1->GetMetaInfo("hns_meta", &hns_meta));
  ASSERT_EQ(hns_meta, "hns_meta_value");
}
