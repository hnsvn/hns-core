...
namespace {
void HnsAdjustTextForCopy(GURL* url) {
#if !BUILDFLAG(IS_IOS)
...
#define HNS_ADJUST_TEXT_FOR_COPY HnsAdjustTextForCopy(url_from_text);
...
#undef CanPasteAndGo
#undef PasteAndGo
#undef HNS_ADJUST_TEXT_FOR_COPY
...
void OmniboxEditModel::PasteAndGo(const std::u16string& text,
  ...
  controller_->client()->OnInputAccepted(match);
