...
#define HNS_SYNC_SCHEDULER_IMPL_HANDLE_FAILURE \
  HandleHnsConfigurationFailure(model_neutral_state);

#include "src/components/sync/engine/sync_scheduler_impl.cc"
#include "base/functional/callback_forward.h"

#undef HNS_SYNC_SCHEDULER_IMPL_HANDLE_FAILURE

#include "hns/components/sync/engine/hns_sync_server_commands.h"
...
void SyncSchedulerImpl::HandleHnsConfigurationFailure(
...
void SyncSchedulerImpl::PermanentlyDeleteAccountImpl(
  ...
  HnsSyncServerCommands::PermanentlyDeleteAccount(&cycle,
                                                    std::move(callback));
}
