...
base::Value::Dict ConstructAboutInformation(
  ...
  Section section_hns_sync("Hns Sync", /*is_sensitive=*/false);

  Stat<bool>* is_passphrase_set =
      section_hns_sync.AddBoolStat("Passphrase is set");
  HnsSyncServiceImpl* hns_sync_service =
      static_cast<HnsSyncServiceImpl*>(service);
  bool failed_to_decrypt = false;
  std::string seed = hns_sync_service->prefs().GetSeed(&failed_to_decrypt);
  ...
  if (failed_to_decrypt) {
    Stat<bool>* failed_to_decrypt_passphrase =
        section_hns_sync.AddBoolStat("Passphrase decryption failed");
    failed_to_decrypt_passphrase->Set(true);
  }

  Stat<bool>* is_os_encryption_available =
      section_hns_sync.AddBoolStat("OS encryption available");
  ...
  details->Append(section_hns_sync.ToValue());
  return about_info;
}
