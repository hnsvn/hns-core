...
#define HNS_MAKE_LOCAL_DEVICE_SPECIFICS \
  specifics->mutable_hns_fields()->set_is_self_delete_supported(true);
...
// breaks the ability to remove other device in sync chain for Hns
#define HNS_DEVICE_INFO_SYNC_BRIDGE_APPLY_SYNC_CHANGES_SKIP_NEXT_IF if (false)

#define HNS_SKIP_EXPIRE_OLD_ENTRIES return;

#define HNS_ON_READ_ALL_METADATA_CLEAR_PROGRESS_TOKEN         \
...
#undef HNS_ON_READ_ALL_METADATA_CLEAR_PROGRESS_TOKEN
#undef HNS_SKIP_EXPIRE_OLD_ENTRIES
#undef HNS_DEVICE_INFO_SYNC_BRIDGE_APPLY_SYNC_CHANGES_SKIP_NEXT_IF
#undef RefreshLocalDeviceInfoIfNeeded
#undef HNS_MAKE_LOCAL_DEVICE_SPECIFICS
...
std::unique_ptr<HnsDeviceInfo> HnsSpecificsToModel(
  ...
  // extra patch
  return std::make_unique<HnsDeviceInfo>(
      ...
      specifics.has_hns_fields() &&
          specifics.hns_fields().has_is_self_delete_supported() &&
          specifics.hns_fields().is_self_delete_supported());
}
...
DeviceInfoSyncBridge::GetAllHnsDeviceInfo() const {
  std::vector<std::unique_ptr<HnsDeviceInfo>> list;
  for (auto iter = all_data_.begin(); iter != all_data_.end(); ++iter) {
    list.push_back(HnsSpecificsToModel(*iter->second));
  }
  return list;
}
