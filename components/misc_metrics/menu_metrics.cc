...
#include "hns/components/misc_metrics/pref_names.h"
...
const char kHnsFeaturesPrefKey[] = "hns_features";
...
const char* GetMenuGroupPrefKey(MenuGroup group) {
  switch (group) {
    ...
    case MenuGroup::kHnsFeatures:
      return kHnsFeaturesPrefKey;
...
const char kMenuDismissRateHistogramName[] = "Hns.Toolbar.MenuDismissRate";
const char kMenuOpensHistogramName[] = "Hns.Toolbar.MenuOpens";
...
void MenuMetrics::RecordMenuGroupAction(MenuGroup group) {
  ...
  if (current_max_group_.has_value() && current_max_group_->first == group) {
    ...
    } else if (result->first == kHnsFeaturesPrefKey) {
      histogram_value = 1;
      current_max_group_ = std::make_pair(MenuGroup::kHnsFeatures, 1);
