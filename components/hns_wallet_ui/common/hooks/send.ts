...
export function useSend (isSendTab?: boolean) {
  ...
  const [selectedSendAsset, setSelectedSendAsset] = React.useState<HnsWallet.BlockchainToken | undefined>(undefined)
  ...
  const { data: fevmTranslatedAddresses } = useGetFVMAddressQuery({
    ...
    isMainNet: selectedSendAsset?.chainId === HnsWallet.FILECOIN_MAINNET})

  const selectSendAsset = (asset: HnsWallet.BlockchainToken | undefined) => {
  ...
  const validateETHAddress = React.useCallback(async (address: string) => {
    if (!isValidAddress(address, 20)) {
      setAddressWarning('')
      setAddressError(getLocale('hnsWalletInvalidRecipientAddress'))
      return false
    }
    ...
    const { checksumAddress } = (await getChecksumEthAddress(address))
      ...
      if ([address.toLowerCase(), address.toUpperCase()].includes(address)) {
        setAddressError('')
        setAddressWarning(getLocale('hnsWalletAddressMissingChecksumInfoWarning'))
        return false
      }
      setAddressWarning('')
      setAddressError(getLocale('hnsWalletNotValidChecksumAddressError'))
      return false      
  }, [setAddressWarning, setAddressError])

  const setNotRegisteredError = React.useCallback(() => {
    setAddressError(getLocale('hnsWalletNotDomain').replace('$1', CoinTypesMap[selectedNetwork?.coin ?? 0]))
  }, [selectedNetwork?.coin])

  const handleDomainLookupResponse = React.useCallback((addressOrUrl: string, error: HnsWallet.ProviderError, requireOffchainConsent: boolean) => {
    ...
    if (addressOrUrl && error === HnsWallet.ProviderError.kSuccess) {
      setAddressError('')
      setAddressWarning('')
      ...
      if (addressOrUrl.toLowerCase() === selectedAccount?.address?.toLowerCase()) {
        setAddressError(getLocale('hnsWalletSameAddressError'))
      }
      setSearchingForDomain(false)
      return
    }
  ...
  const processEthereumAddress = React.useCallback((addressOrUrl: string) => {
    ...
    if (selectedAccount && selectedSendAsset && 
      selectedAccount.accountId.coin === HnsWallet.CoinType.ETH &&
      (selectedSendAsset.chainId === HnsWallet.FILECOIN_ETHEREUM_MAINNET_CHAIN_ID ||
        selectedSendAsset.chainId === HnsWallet.FILECOIN_ETHEREUM_TESTNET_CHAIN_ID) &&
  ...
  const processAddressOrUrl = React.useCallback((addressOrUrl: string) => {
    ...
    if (selectedAccount.accountId.coin === HnsWallet.CoinType.ETH) {
      processEthereumAddress(addressOrUrl)
    } else if (selectedAccount.accountId.coin === HnsWallet.CoinType.FIL) {
      processFilecoinAddress(addressOrUrl)
    } else if (selectedAccount.accountId.coin === HnsWallet.CoinType.SOL) {
  ...
  const submitSend = React.useCallback(() => {
    ...
    if (
      selectedAccount.accountId.coin === HnsWallet.CoinType.SOL &&
    ...
    if (selectedAccount.accountId.coin === HnsWallet.CoinType.FIL) {
      dispatch(
