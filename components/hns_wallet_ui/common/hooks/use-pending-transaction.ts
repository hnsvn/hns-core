...
export const usePendingTransactions = () => {
  ...
  const { data: gasEstimates, isLoading: isLoadingGasEstimates } =
    useGetGasEstimation1559Query(
      transactionInfo && txCoinType !== HnsWallet.CoinType.SOL
  ...
  const isERC20Approve = transactionInfo?.txType === HnsWallet.TransactionType.ERC20Approve
  const isERC721SafeTransferFrom = transactionInfo?.txType === HnsWallet.TransactionType.ERC721SafeTransferFrom
  const isERC721TransferFrom = transactionInfo?.txType === HnsWallet.TransactionType.ERC721TransferFrom
  ...
  const isAssociatedTokenAccountCreation =
    transactionInfo?.txType === HnsWallet.TransactionType.SolanaSPLTokenTransferWithAssociatedTokenAccountCreation

  const isFilecoinTransaction =
    transactionInfo?.txType === HnsWallet.TransactionType.Other &&
  ...
  const unconfirmedGroupTransactionIds = React.useMemo(() =>
    groupTransactions
      .filter(txn => txn.txStatus !== HnsWallet.TransactionStatus.Confirmed)
      .map(txn => txn.id),
    [groupTransactions])
  ...
  React.useEffect(() => {
    let subscribed = true
    if (transactionInfo?.txType !== HnsWallet.TransactionType.ERC20Approve) {
      return
    }
  ...
  React.useEffect(() => {
    if (
      transactionDetails?.recipient &&
      transactionInfo?.txType === HnsWallet.TransactionType.ERC20Approve
    ) {
