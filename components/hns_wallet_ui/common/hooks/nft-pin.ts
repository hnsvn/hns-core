...
export function useNftPin () {
  ...
  const [pinnableNfts, setPinnableNfts] = React.useState<HnsWallet.BlockchainToken[]>([])
  ...
  const pinnedNftsCount = React.useMemo(() => {
    return Object.keys(nftsPinningStatus).reduce((accumulator, currentValue) => {
      ...
      if (status?.code === HnsWallet.TokenPinStatusCode.STATUS_PINNED) {
  ...
  const inProgressNftCount = React.useMemo(() => {
    return Object.keys(nftsPinningStatus).reduce((accumulator, currentValue) => {
      const status = nftsPinningStatus[currentValue]
      if (status?.code === HnsWallet.TokenPinStatusCode.STATUS_PINNING_IN_PROGRESS ||
          status?.code === HnsWallet.TokenPinStatusCode.STATUS_PINNING_PENDING) {
        return accumulator += 1
      }
  ...
  const pinningStatusSummary: HnsWallet.TokenPinStatusCode = React.useMemo(() => {
  ...
  const getNftPinningStatus = React.useCallback((token: HnsWallet.BlockchainToken): PinningStatusType | undefined => {
