...
export const useMultiChainSellAssets = () => {
  ...
  const [selectedSellAsset, setSelectedSellAsset] = React.useState<HnsWallet.BlockchainToken>()
  ...
  const selectedSellAssetNetwork = React.useMemo((): HnsWallet.NetworkInfo | undefined => {
  ...
  const openSellAssetLink = React.useCallback(
    ({
      ...
      sellAddress: string
      sellAsset: HnsWallet.BlockchainToken | undefined
    }) => {
      ...
      getSellAssetUrl({
        asset: asset,
        offRampProvider: HnsWallet.OffRampProvider.kRamp,
        chainId: sellAsset.chainId,
        address: sellAddress,
        amount:
          sellAsset.coin === HnsWallet.CoinType.SOL
  ...
  const checkIsAssetSellSupported = React.useCallback((token: HnsWallet.BlockchainToken) => {
