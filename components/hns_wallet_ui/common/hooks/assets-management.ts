...
const onlyInLeft = (left: HnsWallet.BlockchainToken[], right: HnsWallet.BlockchainToken[]) =>
...
const findTokensWithMismatchedVisibility = (left: HnsWallet.BlockchainToken[], right: HnsWallet.BlockchainToken[]) =>
  left.filter(leftValue =>
    right.some(rightValue =>
      leftValue.contractAddress.toLowerCase() === rightValue.contractAddress.toLowerCase() &&
...
export function useAssetManagement () {
  ...
  const onUpdateVisibleAssets = React.useCallback((updatedTokensList: HnsWallet.BlockchainToken[]) => {
  ...
  const makeTokenVisible = React.useCallback((token: HnsWallet.BlockchainToken) => {
