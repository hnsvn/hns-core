...
export const useProcessSignSolanaTransaction = (props: {
  ...
  const sign = React.useCallback(() => {
    ...
    const isHwAccount =
      props.account.accountId.kind === HnsWallet.AccountKind.kHardware
      ...
      if (isHwAccount) {
        dispatch(
          PanelActions.signTransactionHardware({
            ...
            request: props.request as HnsWallet.SignTransactionRequest
          })
        )
        return
      }
      ...
      if (isHwAccount) {
        dispatch(
          PanelActions.signAllTransactionsHardware({
            account: props.account,
            request: props.request as HnsWallet.SignAllTransactionsRequest,
          })
        )
        return
      }
