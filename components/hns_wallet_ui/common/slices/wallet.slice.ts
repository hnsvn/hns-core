...
const defaultState: WalletState = {
  ...
  selectedPortfolioTimeline:
    window.localStorage.getItem(
      ...
      : HnsWallet.AssetPriceTimeframe.OneDay,
  addUserAssetError: false,
  defaultEthereumWallet: HnsWallet.DefaultWallet.HnsWalletPreferExtension,
  defaultSolanaWallet: HnsWallet.DefaultWallet.HnsWalletPreferExtension,
  activeOrigin: {
    eTldPlusOne: '',
    originSpec: '',
  },
  ...
  onRampCurrencies: [] as HnsWallet.OnRampCurrency[],
  ...
  hidePortfolioNFTsTab:
    ...
    'true',
  removedNonFungibleTokens: [] as HnsWallet.BlockchainToken[],
...
export const WalletAsyncActions = {
  ...
  addFavoriteApp: createAction<HnsWallet.AppItem>('addFavoriteApp'), // should use ApiProxy.walletHandler + refreshWalletInfo
  removeFavoriteApp: createAction<HnsWallet.AppItem>('removeFavoriteApp'), // should use ApiProxy.walletHandler + refreshWalletInfo
  addUserAsset: createAction<HnsWallet.BlockchainToken>('addUserAsset'),
  updateUserAsset: createAction<UpdateUsetAssetType>('updateUserAsset'),
  removeUserAsset: createAction<HnsWallet.BlockchainToken>('removeUserAsset'),
  setUserAssetVisible: createAction<SetUserAssetVisiblePayloadType>(
    'setUserAssetVisible'
  ), // alias for ApiProxy.hnsWalletService.setUserAssetVisible
  selectAccount: createAction<HnsWallet.AccountId>('selectAccount'), // should use apiProxy - keyringService
  ...
  selectPortfolioTimeline: createAction<HnsWallet.AssetPriceTimeframe>(
  ...
  updateTokenPinStatus: createAction<HnsWallet.BlockchainToken>(
...
export const createWalletSlice = (initialState: WalletState = defaultState) => {
  return createSlice({
    ...
    reducers: {
      activeOriginChanged(
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.OriginInfo>
      ) {
      ...
      defaultEthereumWalletUpdated(
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.DefaultWallet>
      ) {
      ...
      portfolioTimelineUpdated(
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.AssetPriceTimeframe>
      ) {
      ...
      setAllTokensList: (
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.BlockchainToken[]>
      ) => {
      ...
      selectCurrency(
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.OnRampCurrency>
      ) {
      ...
      setGasEstimates(
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.GasEstimation1559>
      ) {
      ...
      refreshAccountInfo: (
        state: WalletState,
        { payload }: PayloadAction<HnsWallet.AllAccountsInfo>
      ) => {
