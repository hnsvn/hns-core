    ...
    >({
      queryFn: async (txArg, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          if (txArg.coinType !== HnsWallet.CoinType.ETH) {
            throw new Error(
    ...
    getSolanaTransactionSimulation: query<
      HnsWallet.SolanaSimulationResponse,
      {
        mode: keyof Parameters<
          HnsWallet.SimulationServiceRemote['scanSolanaTransaction']
        >[0]
        chainId: string
        id: Pick<
    ...
    >({
      queryFn: async (arg, { dispatch }, extraOptions, baseQuery) => {
        try {
          ...
          const api = baseQuery(undefined).data
          const { simulationService, hnsWalletService, txService } = api
          const { getPendingSignAllTransactionsRequests } = hnsWalletService
          ...
          switch (arg.mode) {
            ...
            case 'signTransactionRequest': {
              const { requests } =
                await hnsWalletService.getPendingSignTransactionRequests()
