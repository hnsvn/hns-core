...
export const tokenSuggestionsEndpoints = ({
  ...
  return {
    getPendingTokenSuggestionRequests: query<
      HnsWallet.AddSuggestTokenRequest[],
      void
    >({
      queryFn: async (_arg, { endpoint }, _extraOptions, baseQuery) => {
        try {
          ...
          const { requests } =
            await api.hnsWalletService.getPendingAddSuggestTokenRequests()
    ...
    >({
      queryFn: async (arg, { endpoint }, _extraOptions, baseQuery) => {
        ...
        try {
          apiProxy.hnsWalletService.notifyAddSuggestTokenRequestsProcessed(
            arg.approved,
            [arg.contractAddress]
          )
