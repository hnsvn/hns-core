...
export type BlockchainTokenEntityAdaptor = EntityAdapter<HnsWallet.BlockchainToken> & {
  selectId: (t: GetBlockchainTokenIdArg) => EntityId
}

export const blockchainTokenEntityAdaptor: BlockchainTokenEntityAdaptor = createEntityAdapter<HnsWallet.BlockchainToken>({
...
export type BlockchainTokenEntityAdaptorState = ReturnType<BlockchainTokenEntityAdaptor['getInitialState']> & {
  ...
  visibleTokenIdsByCoinType: Record<HnsWallet.CoinType, EntityId[]>
}
...
export const combineTokenRegistries = (
  ...
  const idsByCoinType: Record<HnsWallet.CoinType, EntityId[]> = {
...
export const selectCombinedTokensList = createDraftSafeSelector(
  // inputs
  [
    (knownTokens: HnsWallet.BlockchainToken[]) => knownTokens,
    (_, userTokens: HnsWallet.BlockchainToken[]) => userTokens
  ],
  // output
