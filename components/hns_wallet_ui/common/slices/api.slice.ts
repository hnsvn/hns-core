...
import { hnsRewardsApiEndpoints } from './endpoints/rewards.endpoints'
...
type GetCombinedTokenBalanceForAllAccountsArg =
  GetAccountTokenCurrentBalanceArg['token'] &
    Pick<HnsWallet.BlockchainToken, 'coin'>
...
interface GetFVMAddressArg {
  coin: HnsWallet.CoinType | undefined
  isMainNet: boolean
  addresses: string[]
}
...
interface GetTransactionsQueryArg {
  ...
  */
  coinType: HnsWallet.CoinType | null
  /**
   * will fetch for all coin-type addresses if null
  */
  accountId: HnsWallet.AccountId | null
  /**
...
export function createWalletApi () {
  ...
  return createWalletApiBase().injectEndpoints({
    endpoints: ({ mutation, query }) => { return {
      ...
      getSelectedAccountId: query<HnsWallet.AccountId | undefined, void>({
        queryFn: async (arg, { dispatch }, extraOptions, baseQuery) => {
          const { cache } = baseQuery(undefined)
      ...
      getDefaultFiatCurrency: query<string, void>({
        queryFn: async (arg, api, extraOptions, baseQuery) => {
          try {
            ...
            const { currency } =
              await hnsWalletService.getDefaultBaseCurrency()
      ...
      setDefaultFiatCurrency: mutation<string, string>({
        queryFn: async (currencyArg, api, extraOptions, baseQuery) => {
          try {
            const { hnsWalletService } = baseQuery(undefined).data
            hnsWalletService.setDefaultBaseCurrency(currencyArg)
            return {
              data: currencyArg
            }
          } catch (error) {
      ...
      getSelectedChain: query<HnsWallet.NetworkInfo | undefined, void>({
        queryFn: async (arg, api, extraOptions, baseQuery) => {
          try {
            return {
      ...
      setNetwork: mutation<
        {
          ...
          selectedAccountId?: HnsWallet.AccountId
        },
        Pick<HnsWallet.NetworkInfo, 'chainId' | 'coin'>
      >({
        queryFn: async (
          { chainId, coin },
          { dispatch },
          ...
          try {
            const {
              data: { hnsWalletService },
              cache
            } = baseQuery(undefined)

            cache.clearSelectedAccount()
            const { accountId: selectedAccountId } =
              await hnsWalletService.ensureSelectedAccountForChain(
                coin,
                chainId
              )
            ...
            const {
              success //
            } = await hnsWalletService //
              .setNetworkForSelectedAccountOnActiveOrigin(chainId)
            if (!success) {
              throw new Error(
                'hnsWalletService.SetNetworkForSelectedAccountOnActiveOrigin failed'
              )
            }
      ...
      getTokensRegistry: query<BlockchainTokenEntityAdaptorState, void>({
        queryFn: async (arg, { dispatch }, extraOptions, baseQuery) => {
          try {
            ...
            const networksRegistry = await cache.getNetworksRegistry()
            const networksList: HnsWallet.NetworkInfo[] =
            ...
            const tokenIdsByCoinType: Record<HnsWallet.CoinType, string[]> =
              {}
            ...
            const getTokensList = async () => {
              const tokenListsForNetworks = await mapLimit(
                networksList,
                10,
                async (network: HnsWallet.NetworkInfo) => {
                  ...
                  const fullTokensListForChain: HnsWallet.BlockchainToken[] =
                    await mapLimit(
                      tokens,
                      10,
      ...
      addUserToken: mutation<{ id: EntityId }, HnsWallet.BlockchainToken>({
        queryFn: async (tokenArg, { dispatch }, extraOptions, baseQuery) => {
          ...
          const result = await hnsWalletService.addUserAsset(tokenArg)
      ...
      removeUserToken: mutation<boolean, HnsWallet.BlockchainToken>({
        queryFn: async (tokenArg, api, extraOptions, baseQuery) => {
          ...
          try {
            const deleteResult = await hnsWalletService.removeUserAsset(
              tokenArg
            )
      ...
      updateUserToken: mutation<{ id: EntityId }, HnsWallet.BlockchainToken>({
        queryFn: async (tokenArg, { dispatch }, extraOptions, baseQuery) => {
          const { cache } = baseQuery(undefined)
          cache.clearUserTokensRegistry()
      ...
      updateUserAssetVisible: mutation<boolean, SetUserAssetVisiblePayloadType>(
        {
          queryFn: async (
            ...
            try {
              ...
              const { success } = await hnsWalletService.setUserAssetVisible(
                token,
                isVisible
              )
              return { data: success }
      ...
      >({
        queryFn: async (
          ...
          if (isNativeAsset(token)) {
            // LOCALHOST
            if (
              token.chainId === HnsWallet.LOCALHOST_CHAIN_ID &&
              accountId.coin !== HnsWallet.CoinType.SOL
            ) {
          ...
          try {
            const result = await mapLimit(
              ...
              async (arg: GetTokenBalancesForChainIdArg) => {
                ...
                const { jsonRpcService, hnsWalletService } =
                ...
                if (arg.coin === CoinTypes.ETH) {
                  ...
                  const { chainIds: supportedChainIds } =
                    await hnsWalletService.getBalanceScannerSupportedChains()
      ...
      getFVMAddress: query<GetFVMAddressResult, GetFVMAddressArg>({
        queryFn: async (arg, api, extraOptions, baseQuery) => {
          if (arg.coin !== HnsWallet.CoinType.FIL) {
            return { data: undefined }
          }
          try {
            const { hnsWalletService } = baseQuery(undefined).data
            const convertResult = (await hnsWalletService.convertFEVMToFVMAddress(arg.isMainNet, arg.addresses)).result
            return {
              data: convertResult
            }
          } catch(error) {
        ...
        queryFn: async (
          ...
          try {
            ...
            const nonRejectedTransactionInfos = txInfos
              // hide rejected txs
              .filter(
                (tx) => tx.txStatus !== HnsWallet.TransactionStatus.Rejected
              )
        ...
        invalidatesTags: (res, err, arg) =>
          err
            ...
            : [
                ...
                ...(arg.txStatus === HnsWallet.TransactionStatus.Confirmed
                  ? ([
      ...
      >({
        queryFn: async (txInfo, { dispatch }, extraOptions, baseQuery) => {
          try {
            ...
            const { txService, hnsWalletP3A } = api
            const result: {
              status: boolean
              errorUnion: HnsWallet.ProviderErrorUnion
              errorMessage: string
            ...
            if (error && error !== HnsWallet.ProviderError.kSuccess) {
              return {
            ...
            if (shouldReportTransactionP3A({ txInfo })) {
              hnsWalletP3A.reportTransactionSent(txInfo.coinType, true)
            }
      ...
      >({
        queryFn: async (txInfo, store, extraOptions, baseQuery) => {
          try {
            ...
            const hardwareAccount: HnsWallet.HardwareInfo =
            ...
            if (hardwareAccount.vendor === HnsWallet.LEDGER_HARDWARE_VENDOR) {
              ...
              hardwareAccount.vendor === HnsWallet.TREZOR_HARDWARE_VENDOR
            ) {
      ...
      rejectTransaction: mutation<
        { success: boolean },
        Pick<HnsWallet.TransactionInfo, 'id' | 'chainId'> & {
          coinType: HnsWallet.CoinType
        }
      >({
        ...
        queryFn: async (_arg, { dispatch }) => {
          try {
            const pendingTxs: SerializableTransactionInfo[] = (
              ...
              (tx) => tx.txStatus === HnsWallet.TransactionStatus.Unapproved
            )
    ...
    // hns rewards endpoints
    .injectEndpoints({ endpoints: hnsRewardsApiEndpoints })
    // tx simulation
...
async function getSelectedNetwork(
  api: WalletApiProxy
): Promise<HnsWallet.NetworkInfo | undefined> {
  const { hnsWalletService } = api
  return (await hnsWalletService.getNetworkForSelectedAccountOnActiveOrigin())
    .network ?? undefined
}
...
async function getEnabledCoinTypes(
  ...
  const enabledCoinTypes = SupportedCoinTypes.filter((coin) => {
    ...
    // we will not use their networks unless enabled by hns://flags
    return (
      (coin === HnsWallet.CoinType.FIL && isFilecoinEnabled) ||
      (coin === HnsWallet.CoinType.SOL && isSolanaEnabled) ||
      (coin === HnsWallet.CoinType.BTC && isBitcoinEnabled) ||
      coin === HnsWallet.CoinType.ETH
    )
  })
...
export async function getNetwork(
  api: WalletApiProxy,
  arg: Pick<HnsWallet.NetworkInfo, 'chainId' | 'coin'>
): Promise<HnsWallet.NetworkInfo | undefined> {
