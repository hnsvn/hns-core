...
export interface WalletApiDataOverrides {
  selectedCoin?: HnsWallet.CoinType
  selectedAccountId?: HnsWallet.AccountId
  chainIdsForCoins?: Record<HnsWallet.CoinType, string>
  networks?: HnsWallet.NetworkInfo[]
  defaultBaseCurrency?: string
  transactionInfos?: HnsWallet.TransactionInfo[]
  blockchainTokens?: HnsWallet.BlockchainToken[]
  userAssets?: HnsWallet.BlockchainToken[]
  accountInfos?: HnsWallet.AccountInfo[]
  ...
  evmSimulationResponse?: HnsWallet.EVMSimulationResponse
    | SafeBlowfishEvmResponse
    | null
  svmSimulationResponse?: HnsWallet.SolanaSimulationResponse
    | SafeBlowfishSolanaResponse
    | null
}
...
export class MockedWalletApiProxy {
  ...
  selectedAccountId: HnsWallet.AccountId = mockAccount.accountId
  accountInfos: HnsWallet.AccountInfo[] = [
    mockAccount,
    mockEthAccountInfo,
    mockSolanaAccountInfo,
  ...
  selectedNetwork: HnsWallet.NetworkInfo = mockEthMainnet

  chainIdsForCoins: Record<HnsWallet.CoinType, string> = {
    [HnsWallet.CoinType.ETH]: HnsWallet.MAINNET_CHAIN_ID,
    [HnsWallet.CoinType.SOL]: HnsWallet.SOLANA_MAINNET,
    [HnsWallet.CoinType.FIL]: HnsWallet.FILECOIN_MAINNET
  }

  chainsForCoins: Record<HnsWallet.CoinType, HnsWallet.NetworkInfo> = {
    [HnsWallet.CoinType.ETH]: mockEthMainnet,
    [HnsWallet.CoinType.SOL]: mockSolanaMainnetNetwork,
    [HnsWallet.CoinType.FIL]: mockFilecoinMainnetNetwork
  }

  networks: HnsWallet.NetworkInfo[] = mockNetworks
  ...
  userAssets: HnsWallet.BlockchainToken[] = mockAccountAssetOptions
  ...
  nativeBalanceRegistry: NativeAssetBalanceRegistry = {
    [mockAccount.address]: {
      [HnsWallet.MAINNET_CHAIN_ID]: '0' // 0 ETH
    }
  }
  ...
  transactionInfos: HnsWallet.TransactionInfo[] = [
    ...
    mockFilSendTransaction as HnsWallet.TransactionInfo,
  ...
  blockchainRegistry: Partial<
    InstanceType<typeof HnsWallet.BlockchainRegistryInterface>
  > = {
  ...
  hnsWalletService: Partial<
    InstanceType<typeof HnsWallet.HnsWalletServiceInterface>
  > = {
    getUserAssets: async (chainId: string, coin: HnsWallet.CoinType) => {
      return {
  ...
  swapService: Partial<InstanceType<typeof HnsWallet.SwapServiceInterface>> =
    {
      getTransactionPayload: async ({
        ...
        sellAmount,
        sellToken
      }: HnsWallet.SwapParams): Promise<{
        response: HnsWallet.SwapResponse
        errorResponse: HnsWallet.SwapErrorResponse
        errorString: string
      }> => ({
        errorResponse: {
          code: 0,
  ...
  keyringService: Partial<
    InstanceType<typeof HnsWallet.KeyringServiceInterface>
  > = {
    getAllAccounts: async (): Promise<{
      ...
      const allAccounts: HnsWallet.AllAccountsInfo = {
  ...
  ethTxManagerProxy: Partial<
    InstanceType<typeof HnsWallet.EthTxManagerProxyInterface>
  > = {
    getGasEstimation1559: async () => {
      return {
        estimation: {
          ...
          fastMaxFeePerGas: '0',
          baseFeePerGas: '0'
        } as HnsWallet.GasEstimation1559 | null
      }
    }
  }

  hnsWalletP3A: Partial<
    InstanceType<typeof HnsWallet.HnsWalletP3AInterface>
  > = {
  ...
  assetRatioService: Partial<
    InstanceType<typeof HnsWallet.AssetRatioServiceInterface>
  > = {
  ...
  jsonRpcService: Partial<
    InstanceType<typeof HnsWallet.JsonRpcServiceInterface>
  > = {
    getAllNetworks: async (coin: HnsWallet.CoinType) => {
  ...
  solanaTxManagerProxy: Partial<
    InstanceType<typeof HnsWallet.SolanaTxManagerProxyInterface>
  > = {
  ...
  walletHandler: Partial<
    InstanceType<typeof HnsWallet.WalletHandlerInterface>
  > = {
  ...
  txService: Partial<InstanceType<typeof HnsWallet.TxServiceInterface>> = {
  ...
  simulationService: Partial<
    InstanceType<typeof HnsWallet.SimulationServiceInterface>
  > = {
    ...
    scanEVMTransaction: async (txInfo, language) => {
      return {
        ...
        errorString: '',
        response: this
          .evmSimulationResponse as HnsWallet.EVMSimulationResponse
      }
    },
    scanSolanaTransaction: async (request, language) => {
      return {
        ...
        errorString: '',
        response: this
          .svmSimulationResponse as HnsWallet.SolanaSimulationResponse
      }
    }
  }

  hnsWalletIpfsService: Partial<
    InstanceType<typeof HnsWallet.IpfsServiceInterface>
  > = {
