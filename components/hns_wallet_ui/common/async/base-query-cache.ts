...
export class BaseQueryCache {
  ...
  private _walletInfo?: HnsWallet.WalletInfo
  private _allAccountsInfo?: HnsWallet.AllAccountsInfo
  ...
  getNetworksRegistry = async () => {
    if (!this._networksRegistry) {
      ...
      const filteredSupportedCoinTypes = SupportedCoinTypes.filter((coin) => {
        // FIL and SOL networks, unless enabled by hns://flags
        return (
      ...
      const networkLists: HnsWallet.NetworkInfo[][] = await mapLimit(
        filteredSupportedCoinTypes,
        10,
        async (coin: HnsWallet.CoinType) => {
  ...
  getUserTokensRegistry = async () => {
    if (!this._userTokensRegistry) {
      const { hnsWalletService } = apiProxyFetcher()
      ...
      const visibleTokenIdsByCoinType: Record<HnsWallet.CoinType, string[]> =
        {}

      const userTokenListsForNetworks = await mapLimit(
        Object.entries(networksRegistry.entities),
        10,
        async ([networkId, network]: [string, HnsWallet.NetworkInfo]) => {
          if (!network) {
            return []
          }

          const fullTokensListForNetwork: HnsWallet.BlockchainToken[] =
            await fetchUserAssetsForNetwork(hnsWalletService, network)
          ...
          const visibleTokensForNetwork: HnsWallet.BlockchainToken[] =
  ...
  getExtractedIPFSUrlFromGatewayLikeUrl = async (urlArg: string) => {
    ...
    if (!this._extractedIPFSUrlRegistry[trimmedURL]) {
      if (isIpfs(trimmedURL)) {
        ...
        const { ipfsUrl } =
          await api.hnsWalletIpfsService.extractIPFSUrlFromGatewayLikeUrl(
            trimmedURL
          )
  ...
  getIpfsGatewayTranslatedNftUrl = async (urlArg: string) => {
    ...
    if (!this._nftImageIpfsGateWayUrlRegistry[trimmedURL]) {
      const { hnsWalletIpfsService } = apiProxyFetcher()
...
async function fetchUserAssetsForNetwork(
  hnsWalletService: HnsWallet.HnsWalletServiceRemote,
  network: HnsWallet.NetworkInfo
) {
  // Get a list of user tokens for each coinType and network.
  const { tokens } = await hnsWalletService.getUserAssets(
    network.chainId,
    network.coin
  )

  // Adds a logo and chainId to each token object
  const tokenList: HnsWallet.BlockchainToken[] = await mapLimit(
    tokens,
    10,
