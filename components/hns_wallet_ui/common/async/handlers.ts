...
async function refreshWalletInfo (store: Store, payload: RefreshOpts = {}) {
  ...
  const defaultEthereumResult = await hnsWalletService.getDefaultEthereumWallet()
  store.dispatch(WalletActions.defaultEthereumWalletUpdated(defaultEthereumResult.defaultWallet))
  const defaultSolanaResult = await hnsWalletService.getDefaultSolanaWallet()
  store.dispatch(WalletActions.defaultSolanaWalletUpdated(defaultSolanaResult.defaultWallet))

  const mmResult =
    await hnsWalletService.isExternalWalletInstalled(
...
handler.on(
  ...
  const hnsWalletService = getAPIProxy().hnsWalletService
  const { originInfo } = await hnsWalletService.getActiveOrigin()
...
handler.on(WalletActions.addFavoriteApp.type, async (store: Store, appItem: HnsWallet.AppItem) => {
  await refreshWalletInfo(store)
})

handler.on(WalletActions.removeFavoriteApp.type, async (store: Store, appItem: HnsWallet.AppItem) => {
  await refreshWalletInfo(store)
})
...
handler.on(
  ...
  const hnsWalletService = getAPIProxy().hnsWalletService
  const defaultFiat = await hnsWalletService.getDefaultBaseCurrency()
  const defaultCrypto = await hnsWalletService.getDefaultBaseCryptocurrency()
  ...
  if (!state.isWalletLocked && state.isWalletCreated) {
    ...
    await store.dispatch(refreshVisibleTokenInfo())
    await store.dispatch(refreshPortfolioFilterOptions())
    await hnsWalletService.discoverAssetsOnAllSupportedChains()
  }
})
...
handler.on(WalletActions.addUserAsset.type, async (store: Store, payload: HnsWallet.BlockchainToken) => {
  ...
  const result = await hnsWalletService.addUserAsset(payload)
...
handler.on(WalletActions.updateUserAsset.type, async (store: Store, payload: UpdateUsetAssetType) => {
  ...
  const deleteResult = await hnsWalletService.removeUserAsset(existing)
  if (deleteResult.success) {
    const addResult = await hnsWalletService.addUserAsset(updated)
...
handler.on(WalletActions.removeUserAsset.type, async (store: Store, payload: HnsWallet.BlockchainToken) => {
  const { hnsWalletService } = getAPIProxy()
  await hnsWalletService.removeUserAsset(payload)
})

handler.on(WalletActions.setUserAssetVisible.type, async (store: Store, payload: SetUserAssetVisiblePayloadType) => {
  const { hnsWalletService } = getAPIProxy()
  await hnsWalletService.setUserAssetVisible(payload.token, payload.isVisible)
})
...
handler.on(WalletActions.selectPortfolioTimeline.type, async (store: Store, payload: HnsWallet.AssetPriceTimeframe) => {
...
handler.on(WalletActions.removeSitePermission.type, async (store: Store, payload: RemoveSitePermissionPayloadType) => {
  const hnsWalletService = getAPIProxy().hnsWalletService
  await hnsWalletService.resetPermission(payload.accountId)
  await refreshWalletInfo(store)
})

handler.on(WalletActions.addSitePermission.type, async (store: Store, payload: AddSitePermissionPayloadType) => {
  const hnsWalletService = getAPIProxy().hnsWalletService
  await hnsWalletService.addPermission(payload.accountId)
  await refreshWalletInfo(store)
})
...
handler.on(
  ...
  async (store: Store, payload: ImportAccountPayloadType) => {
    const { keyringService } = getAPIProxy()
    const result =
      payload.coin === HnsWallet.CoinType.FIL &&
        payload.network
