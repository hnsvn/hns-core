...
const getMockedLedgerEthKeyring = (expectedPath: string, expectedData: string | HnsWallet.TransactionInfo, signed?: SignHardwareOperationResult) => {
  return {
    type: (): HardwareVendor => {
      return HnsWallet.LEDGER_HARDWARE_VENDOR
    },
...
const getMockedLedgerFilKeyring = (expectedMessage: string | HnsWallet.TransactionInfo, signed?: SignHardwareOperationResult) => {
  return {
    type: (): HardwareVendor => {
      return HnsWallet.LEDGER_HARDWARE_VENDOR
    },
...
const getMockedTrezorKeyring = (expectedDevicePath: string, expectedData: string | HnsWallet.TransactionInfo, signed?: Success<EthereumSignedTx> | Unsuccessful) => {
  return {
    type: (): HardwareVendor => {
      return HnsWallet.TREZOR_HARDWARE_VENDOR
    },
...
const getMockedProxyServices = (
  ...
  return {
    ...
    txService: {
      getTransactionMessageToSign: (coinType: HnsWallet.CoinType, chainId: string, id: string): GetTransactionMessageToSignReturnInfo | undefined => {
...
const signEthTransactionWithLedger = (vrs?: SignatureVRS, signatureResponse?: boolean): Promise<SignHardwareTransactionType> => {
  ...
  const apiProxy = getMockedProxyServices(txInfo.chainId, txInfo.id, { nonce: '0x1' }, messageToSign,
    signed)
  return signLedgerEthereumTransaction(apiProxy as unknown as WalletApiProxy, expectedPath, txInfo, HnsWallet.CoinType.ETH, mockedKeyring as unknown as LedgerBridgeKeyring)
}
...
const signFilTransactionWithLedger = (expectedSignature: SignedLotusMessage, signatureResponse?: boolean): Promise<SignHardwareTransactionType> => {
  ...
  return signLedgerFilecoinTransaction(apiProxy as unknown as WalletApiProxy, txInfo, HnsWallet.CoinType.FIL, mockedKeyring as unknown as FilecoinLedgerKeyring)
}
...
test('Test sign raw Eth Ledger transaction', () => {
  ...
  return expect(signRawTransactionWithHardwareKeyring(mockedKeyring.type(), HnsWallet.CoinType.ETH)).resolves.toStrictEqual(hardwareTransactionErrorResponse('hnsWalletHardwareOperationUnsupportedError'))
})
...
test('Test sign Ledger transaction, approved, no message to sign', () => {
  ...
  return expect(signLedgerEthereumTransaction(apiProxy as unknown as WalletApiProxy,
    'path', txInfo, HnsWallet.CoinType.ETH)).resolves.toStrictEqual(hardwareTransactionErrorResponse('hnsWalletNoMessageToSignError'))
})

test('Test sign Ledger transaction, approved, device error', () => {
  return expect(signEthTransactionWithLedger()).resolves.toStrictEqual(
    hardwareTransactionErrorResponseWithCode('hnsWalletSignOnDeviceError'))
})
...
test('Test sign Ledger transaction, approved, processing error', () => {
  return expect(signEthTransactionWithLedger({ v: 1, r: 'R', s: 'S' })).resolves.toStrictEqual(
    hardwareTransactionErrorResponse('hnsWalletProcessTransactionError'))
})
...
test('Test sign Trezor transaction, approve failed', () => {
  ...
  return expect(signTrezorTransaction(apiProxy as unknown as WalletApiProxy,
    'path', txInfo)).resolves.toStrictEqual(
      hardwareTransactionErrorResponse('hnsWalletApproveTransactionError'))
})
...
test('Test sign Trezor transaction, approved, device error', () => {
  return expect(signTransactionWithTrezor({ success: false, payload: { error: 'error', code: '111' } }))
    .resolves.toStrictEqual(hardwareTransactionErrorResponse('hnsWalletSignOnDeviceError'))
})
