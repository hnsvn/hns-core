...
export function  makeHnsWalletServiceTokenObserver (store: Store) {
  const hnsWalletServiceTokenObserverReceiver = new HnsWallet.HnsWalletServiceTokenObserverReceiver({
    onTokenAdded(token) {
      store.dispatch(
  ...
  return hnsWalletServiceTokenObserverReceiver
}

export function makeJsonRpcServiceObserver (store: Store) {
    const jsonRpcServiceObserverReceiver = new HnsWallet.JsonRpcServiceObserverReceiver({
  ...
  export function  makeKeyringServiceObserver (store: Store) {
    const keyringServiceObserverReceiver = new HnsWallet.KeyringServiceObserverReceiver({
      ...
      selectedWalletAccountChanged: function (account: HnsWallet.AccountInfo) {
        store.dispatch(walletApi.endpoints.invalidateSelectedAccount.initiate())
      },
      selectedDappAccountChanged: function (coin: HnsWallet.CoinType, account: HnsWallet.AccountInfo | null) {
        // TODO: Handle this event.
      }
    })
  ...
  export function  makeTxServiceObserver (store: Store) {
    const txServiceManagerObserverReceiver = new HnsWallet.TxServiceObserverReceiver({
      ...
      onTransactionStatusChanged: (txInfo) => {
        ...
        if (
          [
            ...
            HnsWallet.TransactionStatus.Signed,
          ...
          if (
            ...
            (state.panel?.selectedPanel === 'approveTransaction' ||
              txInfo.txStatus === HnsWallet.TransactionStatus.Rejected)
          ) {
  ...
  export function  makeHnsWalletServiceObserver (store: Store) {
    const hnsWalletServiceObserverReceiver = new HnsWallet.HnsWalletServiceObserverReceiver({
    ...
    return hnsWalletServiceObserverReceiver
  }

  export function  makeHnsWalletPinServiceObserver (store: Store) {
    const hnsWalletServiceObserverReceiver = new HnsWallet.HnsWalletPinServiceObserverReceiver({
  ...
  export function  makeHnsWalletAutoPinServiceObserver (store: Store) {
    const hnsWalletAutoPinServiceObserverReceiver = new HnsWallet.WalletAutoPinServiceObserverReceiver({
    ...
    return hnsWalletAutoPinServiceObserverReceiver
  }
