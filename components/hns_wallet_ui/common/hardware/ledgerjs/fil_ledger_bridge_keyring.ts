...
import { HnsWallet, FilecoinNetwork } from '../../../constants/types'
...
export default class FilecoinLedgerBridgeKeyring extends LedgerBridgeKeyring implements LedgerFilecoinKeyring {
  ...
  coin = (): HnsWallet.CoinType => {
    return HnsWallet.CoinType.FIL
  }

  keyringId = (network: FilecoinNetwork): HnsWallet.KeyringId => {
    return network === HnsWallet.FILECOIN_MAINNET
  ...
  private readonly getPathForIndex = (index: number, type: FilecoinNetwork): string => {
    ...
    return type === HnsWallet.FILECOIN_MAINNET ? `m/44'/461'/0'/0/${index}` : `m/44'/1'/0'/0/${index}`
  }
}
