 ...
 * You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 import { LEDGER_HARDWARE_VENDOR } from 'gen/hns/components/hns_wallet/common/hns_wallet.mojom.m.js'
...
test('Extracting accounts from device MAIN', async () => {
  ...
  return expect(await keyring.getAccounts(-2, 1, HnsWallet.FILECOIN_MAINNET))
    .toEqual({
      payload: [{
        address: '0',
        ...
        keyringId: HnsWallet.KeyringId.kFilecoin
      }],
      success: true
    })
})
...
test('Extracting accounts from device TEST', async () => {
  ...
  return expect(await keyring.getAccounts(-2, 1, HnsWallet.FILECOIN_TESTNET))
    .toEqual({
      payload: [{
        address: '0',
        coin: HnsWallet.CoinType.FIL,
