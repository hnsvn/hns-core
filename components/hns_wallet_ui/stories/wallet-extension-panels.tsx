...
const mockEthAccountId = (
  address: string
): { fromAddress: string; fromAccountId: HnsWallet.AccountId } => {
  return {
    fromAddress: address,
    fromAccountId: {
...
export const _SignData = () => {
  ...
  const signMessageDataPayload: HnsWallet.SignMessageRequest[] = [{
    id: 0,
...
export const _ConnectedPanel = (args: { locked: boolean }) => {
  ...
  const [selectedAccount, setSelectedAccount] =
    React.useState<HnsWallet.AccountInfo>(mockAccounts[0])
  const [favoriteApps, setFavoriteApps] = React.useState<HnsWallet.AppItem[]>([
  ...
  const onSelectAccount = (account: HnsWallet.AccountInfo) => () => {
  ...
  const addToFavorites = (app: HnsWallet.AppItem) => {
    const newList = [...favoriteApps, app]
    setFavoriteApps(newList)
  }
  const removeFromFavorites = (app: HnsWallet.AppItem) => {
  ...
  const onNoAccountForNetwork = (network: HnsWallet.NetworkInfo) => {
    console.log('Will expand to Create Account panel')
  }

  const onAddAsset = () => {
    alert('Will redirect to hns://wallet/crypto/portfolio/add-asset')
  }
...
export const _ConnectHardwareWallet = () => {
  const onCancel = (account: HnsWallet.AccountInfo) => {
    // Doesn't do anything in storybook
  }

  const onClickInstructions = () => {
    // Open support link in new tab
    window.open('https://support.hns.com/hc/en-us/articles/4409309138701', '_blank', 'noreferrer')
  }
