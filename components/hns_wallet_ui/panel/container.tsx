...
function Container () {
  ...
  const [networkForCreateAccount, setNetworkForCreateAccount] = React.useState<
    HnsWallet.NetworkInfo | undefined
  >(undefined)
  ...
  const onSelectAccount = async (account: HnsWallet.AccountInfo) => {
  ...
  const onCancelConnectHardwareWallet = (account: HnsWallet.AccountInfo) => {
  ...
  const onClickInstructions = () => {
    const url = 'https://support.hns.com/hc/en-us/articles/4409309138701'
  ...
  if (selectedPendingTransaction?.txType === HnsWallet.TransactionType.ETHSwap) {
    return (
