...
import { HardwareVendor } from 'components/hns_wallet_ui/common/api/hardware_keyrings'
...
async function getPendingGetEncryptionPublicKeyRequest () {
  const hnsWalletService = getWalletPanelApiProxy().hnsWalletService
  const requests =
    (await hnsWalletService.getPendingGetEncryptionPublicKeyRequests()).requests
  if (requests && requests.length) {
    return requests[0]
  }
  return null
...
async function getPendingDecryptRequest () {
  const hnsWalletService = getWalletPanelApiProxy().hnsWalletService
  const requests =
    (await hnsWalletService.getPendingDecryptRequests()).requests
  if (requests && requests.length) {
    return requests[0]
  }
  return null
...
async function getPendingSignMessageRequests () {
  const hnsWalletService = getWalletPanelApiProxy().hnsWalletService
  const requests =
    (await hnsWalletService.getPendingSignMessageRequests()).requests
  if (requests && requests.length) {
    return requests
  }
  return null
...
async function getPendingSignTransactionRequests () {
  const { hnsWalletService } = getWalletPanelApiProxy()
  const { requests } = await hnsWalletService.getPendingSignTransactionRequests()
...
async function getPendingSignAllTransactionsRequests () {
  const { hnsWalletService } = getWalletPanelApiProxy()
  const { requests } = await hnsWalletService.getPendingSignAllTransactionsRequests()
...
handler.on(PanelActions.cancelConnectHardwareWallet.type, async (store: Store, payload: HnsWallet.AccountInfo) => {
  if (payload.hardware) {
...
handler.on(PanelActions.addEthereumChain.type, async (store: Store, request: HnsWallet.AddChainRequest) => {
...
handler.on(PanelActions.switchEthereumChain.type, async (store: Store, request: HnsWallet.SwitchChainRequest) => {
...
handler.on(PanelActions.getEncryptionPublicKeyProcessed.type, async (store: Store, payload: GetEncryptionPublicKeyProcessedPayload) => {
  const apiProxy = getWalletPanelApiProxy()
  const hnsWalletService = apiProxy.hnsWalletService
  hnsWalletService.notifyGetPublicKeyRequestProcessed(payload.requestId, payload.approved)
...
handler.on(PanelActions.decryptProcessed.type, async (store: Store, payload: DecryptProcessedPayload) => {
  const apiProxy = getWalletPanelApiProxy()
  const hnsWalletService = apiProxy.hnsWalletService
  hnsWalletService.notifyDecryptRequestProcessed(payload.requestId, payload.approved)
...
handler.on(PanelActions.signMessageProcessed.type, async (store: Store, payload: SignMessageProcessedPayload) => {
  const apiProxy = getWalletPanelApiProxy()
  const hnsWalletService = apiProxy.hnsWalletService
  hnsWalletService.notifySignMessageRequestProcessed(payload.approved, payload.id, null, null)
...
function toByteArrayStringUnion<D extends keyof HnsWallet.ByteArrayStringUnion>(
  unionItem: Pick<HnsWallet.ByteArrayStringUnion, D>
) {
  return Object.assign({}, unionItem) as HnsWallet.ByteArrayStringUnion
}
...
handler.on(PanelActions.signMessageHardware.type, async (store, messageData: SignMessageHardwarePayload) => {
  ...
  if (!isHardwareAccount(messageData.account.accountId)) {
    const hnsWalletService = apiProxy.hnsWalletService
    hnsWalletService.notifySignMessageRequestProcessed(false, messageData.request.id,
      null, getLocale('hnsWalletHardwareAccountNotFound'))
    const signMessageRequests = await getPendingSignMessageRequests()
    if (signMessageRequests) {
      store.dispatch(PanelActions.signMessage(signMessageRequests))
      return
  ...
  if (!signed.success && signed.code) {
    ...
    const deviceError = (info.vendor === HnsWallet.TREZOR_HARDWARE_VENDOR)
  ...
  let signature: HnsWallet.ByteArrayStringUnion | undefined
  if (signed.success) {
    signature =
...
handler.on(PanelActions.signMessageHardwareProcessed.type, async (store, payload: SignMessageProcessedPayload) => {
  const apiProxy = getWalletPanelApiProxy()
  const hnsWalletService = apiProxy.hnsWalletService
  hnsWalletService.notifySignMessageRequestProcessed(payload.approved, payload.id, payload.signature || null, payload.error || null)
  const signMessageRequests = await getPendingSignMessageRequests()
  if (signMessageRequests) {
    store.dispatch(PanelActions.signMessage(signMessageRequests))
    return
...
// Sign Transaction
handler.on(PanelActions.signTransaction.type, async (store: Store, payload: HnsWallet.SignTransactionRequest[]) => {
...
handler.on(PanelActions.signTransactionHardware.type, async (store, messageData: SignTransactionHardwarePayload) => {
  ...
  if (!isHardwareAccount(messageData.account.accountId)) {
    const hnsWalletService = apiProxy.hnsWalletService
    hnsWalletService.notifySignTransactionRequestProcessed(false, messageData.request.id,
      null, getLocale('hnsWalletHardwareAccountNotFound'))
...
handler.on(PanelActions.signTransactionProcessed.type, async (store: Store, payload: SignMessageProcessedPayload) => {
  const { hnsWalletService, panelHandler } = getWalletPanelApiProxy()
  hnsWalletService.notifySignTransactionRequestProcessed(payload.approved, payload.id, payload.signature || null, payload.error || null)
  const requests = await getPendingSignTransactionRequests()
  if (requests) {
    store.dispatch(PanelActions.signTransaction(requests))
    return
...
handler.on(PanelActions.signAllTransactions.type, async (store: Store, payload: HnsWallet.SignAllTransactionsRequest[]) => {
...
handler.on(PanelActions.signAllTransactionsHardware.type, async (store, messageData: SignAllTransactionsHardwarePayload) => {
  ...
  if (!isHardwareAccount(messageData.account.accountId)) {
    const hnsWalletService = apiProxy.hnsWalletService
    hnsWalletService.notifySignAllTransactionsRequestProcessed(false, messageData.request.id,
      null, getLocale('hnsWalletHardwareAccountNotFound'))
  ...
  for (const rawMessage of messageData.request.rawMessages) {
    ...
    const signature: HnsWallet.ByteArrayStringUnion =
      coin === HnsWallet.CoinType.SOL
        ? toByteArrayStringUnion({ bytes: [...(signed.payload as Buffer)] })
        : toByteArrayStringUnion({ str: signed.payload as string })
...
handler.on(PanelActions.signAllTransactionsProcessed.type, async (store: Store, payload: SignAllTransactionsProcessedPayload) => {
  const { hnsWalletService, panelHandler } = getWalletPanelApiProxy()
  hnsWalletService.notifySignAllTransactionsRequestProcessed(payload.approved, payload.id, payload.signatures || null, payload.error || null)
  const requests = await getPendingSignAllTransactionsRequests()
  if (requests) {
    store.dispatch(PanelActions.signAllTransactions(requests))
    return
...
handler.on(WalletActions.initialize.type, async (store) => {
  ...
  if (url.hash === '#connectWithSite') {
    ...
    const originInfo: HnsWallet.OriginInfo = {
