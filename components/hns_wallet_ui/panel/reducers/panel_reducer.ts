...
const defaultOriginInfo: HnsWallet.OriginInfo = {
  originSpec: '',
  eTldPlusOne: ''
}
...
const defaultState: PanelState = {
  ...
  addChainRequest: {
    ...
    networkInfo: {
      ...
      rpcEndpoints: [{ url: 'https://mainnet-infura.hns.com/' }],
      blockExplorerUrls: [],
      iconUrls: [],
      symbol: 'ETH',
      symbolName: 'Ethereum',
...
export const createPanelReducer = (initialState: PanelState) => {
  ...
  reducer.on(PanelActions.addEthereumChain.type, (state: any, request: HnsWallet.AddChainRequest) => {
    return {
      ...state,
  ...
  reducer.on(PanelActions.switchEthereumChain.type, (state: any, request: HnsWallet.SwitchChainRequest) => {
    return {
      ...state,
  ...
  reducer.on(PanelActions.getEncryptionPublicKey.type, (state: any, request: HnsWallet.GetEncryptionPublicKeyRequest) => {
    return {
      ...state,
  ...
  reducer.on(PanelActions.decrypt.type, (state: any, request: HnsWallet.DecryptRequest) => {
    return {
      ...state,
  ...
  reducer.on(PanelActions.signMessage.type, (state, payload: HnsWallet.SignMessageRequest[]) => {
    return {
      ...state,
  ...
  reducer.on(PanelActions.signTransaction.type, (state: PanelState, payload: HnsWallet.SignTransactionRequest[]): PanelState => {
    return {
      ...state,
  ...
  reducer.on(PanelActions.signAllTransactions.type, (state: PanelState, payload: HnsWallet.SignAllTransactionsRequest[]): PanelState => {
    return {
      ...state,
