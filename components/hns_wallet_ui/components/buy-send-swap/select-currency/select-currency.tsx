...
export interface Props {
  onSelectCurrency?: (currency: HnsWallet.OnRampCurrency) => void
  onBack: () => void
}
...
export const SelectCurrency = (props: Props) => {
  ...
  const [filteredCurrencies, setFilteredCurrencies] = React.useState<HnsWallet.OnRampCurrency[]>(currencies)
  ...
  const filterCurrencyList = React.useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (search === '') {
      ...
      const filteredList = fuse.search(search).map((result: Fuse.FuseResult<HnsWallet.OnRampCurrency>) => result.item)
      setFilteredCurrencies(filteredList)
    }
  }, [fuse, currencies])

  const onSelectedCurrency = React.useCallback((currency: HnsWallet.OnRampCurrency) => {
  ...
  return (
    <SelectCurrencyWrapper>
      ...
      />
      <SearchBar
        placeholder={getLocale('hnsWalletSearchCurrency')}
      ...
      <SelectScrollSearchContainer>
        {
          filteredCurrencies.map((currency: HnsWallet.OnRampCurrency) =>
            <SelectCurrencyItem
