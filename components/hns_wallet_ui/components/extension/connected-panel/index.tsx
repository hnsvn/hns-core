...
export const ConnectedPanel = (props: Props) => {
  ...
  // custom hooks
  const { hnsWalletService } = useApiProxy()
  ...
  const onShowSitePermissions = React.useCallback(() => {
    if (isPermissionDenied) {
      const contentPath = selectedCoin === HnsWallet.CoinType.SOL ? 'solana' : 'ethereum'
      chrome.tabs.create({
        url: `hns://settings/content/${contentPath}`
  ...
  React.useEffect(() => {
    ...
    if (selectedCoin) {
      (async () => {
        await hnsWalletService.isPermissionDenied(selectedCoin)
          .then(result => {
            if (subscribed) {
    ...
    // Keep dependency on originInfo. When hnsWalletService.isPermissionDenied
    // is moved to api slice it's cached value should be reset on activeOriginChanged event.
  }, [hnsWalletService, selectedCoin, originInfo])

  React.useEffect(() => {
    let subscribed = true

    if (selectedAccount?.address && selectedCoin === HnsWallet.CoinType.SOL) {
      (async () => {
  ...
  const connectedStatusText = React.useMemo((): string => {
    if (isPermissionDenied) {
      return getLocale('hnsWalletPanelBlocked')
    }
    if (selectedCoin === HnsWallet.CoinType.SOL) {
      return isConnected
        ? getLocale('hnsWalletPanelConnected')
        : getLocale('hnsWalletPanelDisconnected')
    }
    return isConnected
      ? getLocale('hnsWalletPanelConnected')
      : getLocale('hnsWalletPanelNotConnected')
  ...
  return (
    <StyledWrapper onClick={onHideMore} panelBackground={bg}>
      ...
      <CenterColumn>
        ...
        {showConnectButton ? (
          <StatusRow>
            <OvalButton onClick={onShowSitePermissions}>
              {selectedCoin === HnsWallet.CoinType.SOL ? (
        ...
        <MoreAssetsButton onClick={navigate('assets')}>{getLocale('hnsWalletPanelViewAccountAssets')}</MoreAssetsButton>
      </CenterColumn>
      <PanelBottomNav
