...
export const SitePermissions = () => {
  ...
  // memos
  const accountByCoinType = React.useMemo((): HnsWallet.AccountInfo[] => {
    ...
    <StyledWrapper>
      <HeaderRow>
        ...
        <HeaderColumn>
          ...
          <AccountsTitle>{getLocale('hnsWalletSitePermissionsAccounts').replace('$1', connectedAccounts.length.toString())}</AccountsTitle>
        </HeaderColumn>
      </HeaderRow>
      <AddressContainer>
        <NewAccountButton onClick={onAddAccount}>{getLocale('hnsWalletSitePermissionsNewAccount')}</NewAccountButton>
        <DividerLine />
        <AddressScrollContainer>
