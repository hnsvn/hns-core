...
export function ConfirmSwapTransaction () {
  ...
  return (
    <StyledWrapper>
      ...
      <HeaderTitle>{getLocale('hnsWalletSwapReviewHeader')}</HeaderTitle>
...
function SwapAsset (props: SwapAssetProps) {
  ...
  const networkDescription = network
    ? getLocale('hnsWalletPortfolioAssetNetworkDescription')
        .replace('$1', '')
  ...
  return (
    <SwapAssetContainer top={type === 'maker'}>
      <SwapAssetHeader>
        <SwapAssetTitle>
          {type === 'maker'
            ? getLocale('hnsWalletSwapReviewSpend')
            : getLocale('hnsWalletSwapReviewReceive')}
        </SwapAssetTitle>
        {expectAddress && (
          <SwapAssetAddress>
            {address && orb ? (
