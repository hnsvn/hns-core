...
export const ConnectHardwareWalletPanel = ({
  ...
  const title = React.useMemo(() => {
    if (hardwareWalletCode === 'deviceBusy') {
      return getLocale('hnsWalletConnectHardwarePanelConfirmation')
    }
    ...
    if (
      ...
      return getLocale('hnsWalletConnectHardwarePanelConnect').replace(
        '$1',
        account.name
      )
    }
    ...
    return getLocale('hnsWalletConnectHardwarePanelOpenApp')
      .replace('$1', network)
  ...
  React.useEffect(() => {
    ...
    // back to hns://wallet-panel.top-chrome/ without any
    ...
    <StyledWrapper>
      <ConnectionRow>
        ...
        <Description>
          {
            isConnected
              ? getLocale('hnsWalletConnectHardwarePanelConnected').replace('$1', account.name)
              : getLocale('hnsWalletConnectHardwarePanelDisconnected').replace('$1', account.name)
          }
        </Description>
      </ConnectionRow>
      <Title>{title}</Title>
      <InstructionsButton onClick={onClickInstructions}>{getLocale('hnsWalletConnectHardwarePanelInstructions')}</InstructionsButton>
      <PageIcon />
      {
