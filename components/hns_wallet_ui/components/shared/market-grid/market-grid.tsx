...
export type MarketGridProps = {
  ...
  onSelectCoinMarket: (coinMarket: HnsWallet.CoinMarket) => void
  isBuySupported: (coinMarket: HnsWallet.CoinMarket) => boolean
  isDepositSupported: (coinMarket: HnsWallet.CoinMarket) => boolean
  onClickBuy: (coinMarket: HnsWallet.CoinMarket) => void
  onClickDeposit: (coinMarket: HnsWallet.CoinMarket) => void
...
export const MarketGrid = ({
  ...
  const rows: MarketGridRow[] = React.useMemo(() => {
    return coinMarketData.map((coinMarketItem: HnsWallet.CoinMarket) => {
  ...
  return (
    <StyledWrapper onLoad={onContentLoad} ref={wrapperRef}>
        ...
        {showEmptyState ? (
          ...
          >
            <EmptyStateText
              ...
              textSize='14px'
            >
              {getLocale('hnsWalletMarketDataNoAssetsFound')}
            </EmptyStateText>
          </Row>
        ) : (
          <FixedSizeList
      ...
      <CoinGeckoText>
        {getLocale('hnsWalletPoweredByCoinGecko')}
      </CoinGeckoText>
    </StyledWrapper>
  )
}
