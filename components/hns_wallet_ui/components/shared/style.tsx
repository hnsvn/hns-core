...
// types
import { HnsWallet, StringWithAutocomplete } from '../../constants/types'
import IThemeProps from 'hns-ui/theme/theme-interface'

// utils
import { stripERC20TokenImageURL } from '../../utils/string-utils'

// components
import { LoaderIcon } from 'hns-ui/components/icons'
...
// graphics
import HnsWalletWithCoins from '../../assets/svg-icons/onboarding/hns-wallet-with-coins.svg'
...
export const StatusBubble = styled.div<{ status: HnsWallet.TransactionStatus }>`
  display: flex;
  align-items: center;
  justify-content: center;
  width: 10px;
  height: 10px;
  border-radius: 100%;
  opacity: ${(p) => p.status === HnsWallet.TransactionStatus.Submitted ||
    p.status === HnsWallet.TransactionStatus.Approved ||
    p.status === HnsWallet.TransactionStatus.Unapproved ||
    p.status === HnsWallet.TransactionStatus.Signed
    ? 0.4
    : 1
  };
  background-color: ${(p) => p.status === HnsWallet.TransactionStatus.Confirmed || p.status === HnsWallet.TransactionStatus.Approved
    ? p.theme.color.successBorder
    : p.status === HnsWallet.TransactionStatus.Rejected || p.status === HnsWallet.TransactionStatus.Error || p.status === HnsWallet.TransactionStatus.Dropped ? p.theme.color.errorBorder
      : p.status === HnsWallet.TransactionStatus.Unapproved ? p.theme.color.interactive08 : p.theme.color.warningIcon
  };
  margin-right: 6px;
`
...
export const WalletWelcomeGraphic = styled.div<{ scale?: CSSProperties['scale'] }>`
  width: 350px;
  height: 264px;
  background: url(${HnsWalletWithCoins});
