...
export const AddNftForm = (props: Props) => {
  ...
  const onAddCustomToken = React.useCallback(() => {
    ...
    if (foundTokenInfoByContractAddress) {
      ...
      const newToken: HnsWallet.BlockchainToken = {
        contractAddress: tokenContractAddress,
        decimals: 0,
        isErc20: customAssetsNetwork.coin !== HnsWallet.CoinType.SOL && !tokenID,
        isErc721: customAssetsNetwork.coin !== HnsWallet.CoinType.SOL && !!tokenID,
        isErc1155: false,
        isNft: true,
        isSpam: false,
        name: tokenName,
  ...
  const tokenContractAddressError =
    ...
    (customAssetsNetwork?.coin === HnsWallet.CoinType.ETH && tokenID === '')
      ...
      >
        ...
        <FullWidthFormColumn>
          {customAssetsNetwork?.coin !== HnsWallet.CoinType.SOL &&
            <>
              <InputLabel>
                {getLocale('hnsWalletWatchListTokenId')}
              </InputLabel>
              <Input
                value={tokenID}
        ...
        <>
          {showTokenIDRequired &&
            <ErrorText>
              {getLocale('hnsWalletWatchListTokenIdError')}
            </ErrorText>
          }
        </>
        {hasError &&
