...
export const AutoDiscoveryEmptyState = ({ isRefreshingTokens, onImportNft, onRefresh }: Props) => {
  const { duringTag: refreshBtnText, afterTag } = splitStringForTag(getLocale('hnsWalletAutoDiscoveryEmptyStateActions'))
    ...
    <StyledWrapper>
      {isRefreshingTokens ? (
        <>
          ...
          <RefreshText>{getLocale('hnsWalletAutoDiscoveryEmptyStateRefresh')}...</RefreshText>
        </>
      ) : (
        <>
          <Heading>
            {getLocale('hnsWalletAutoDiscoveryEmptyStateHeading')}
          </Heading>
          <Description>
            {getLocale('hnsWalletAutoDiscoveryEmptyStateSubHeading')}
          </Description>
          <Row margin='48px 0 8px 0' marginBottom={8}>
            <Description>
              {getLocale('hnsWalletAutoDiscoveryEmptyStateFooter')}
            </Description>
          </Row>
          <Description>
