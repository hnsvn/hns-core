...
interface Props {
  ...
  nftList: HnsWallet.BlockchainToken[],
  accounts: HnsWallet.AccountInfo[]
  onShowPortfolioSettings?: () => void
  tokenBalancesRegistry: TokenBalancesRegistry | undefined
}

const compareFn = (a: HnsWallet.BlockchainToken, b: HnsWallet.BlockchainToken) => a.name.localeCompare(b.name)
...
export const Nfts = (props: Props) => {
  ...
  const searchNfts = React.useCallback((item: HnsWallet.BlockchainToken) => {
  ...
  React.useEffect(() => {
    hnsWalletP3A.recordNFTGalleryView(nftList.length);
  }, [nftList]);

  // memos
  const [userNonSpamNfts, userMarkedSpamNfts] = React.useMemo(() => {
    return nftList.reduce(
      (result: [HnsWallet.BlockchainToken[], HnsWallet.BlockchainToken[]], nft: HnsWallet.BlockchainToken) => {
        if (!nft.isSpam) {
          result[0].push(nft)
        } else {
          result[1].push(nft)
  ...
  const dropDownOptions: NftDropdownOption[] = React.useMemo(() => {
    return [
      {
        id: 'collected',
        label: getLocale('hnsNftsTabCollected'),
        labelSummary: sortedNfts.length
      },
      {
        id: 'hidden',
        label: getLocale('hnsNftsTabHidden'),
