...
export const Account = () => {
  ...
  const accountsTokensList = React.useMemo(() => {
    ...
    const hasLocalHostNetwork = networkList.some(
      (network) =>
        network.chainId === HnsWallet.LOCALHOST_CHAIN_ID
        && network.coin === selectedAccount.accountId.coin
    )
    const coinName = CoinTypesMap[selectedAccount.accountId.coin]
    const localHostCoins = userVisibleTokensInfo.filter((token) => token.chainId === HnsWallet.LOCALHOST_CHAIN_ID)
    ...
    if (
      accountsLocalHost &&
      hasLocalHostNetwork &&
      selectedAccount.accountId.keyringId !== HnsWallet.KeyringId.kFilecoin
    ) {
      ...
      {selectedTab === AccountPageTabs.AccountNFTsSub && (
        ...
        >
          {nonFungibleTokens?.length !== 0 ? (
            <NftGrid>
              {nonFungibleTokens?.map((nft: HnsWallet.BlockchainToken) =>
                <NFTGridViewItem
          ...
          {transactionList.length !== 0 ? (
            ...
            >
              ...
              <Text
                ...
                isBold={true}
              >
                {getLocale('hnsWalletNoTransactionsYet')}
              </Text>
              ...
              <Text
                ...
                isBold={false}
              >
                {getLocale('hnsWalletNoTransactionsYetDescription')}
              </Text>
            </Column>
          )}
        </>
