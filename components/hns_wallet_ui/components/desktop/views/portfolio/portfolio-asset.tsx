...
export const PortfolioAsset = (props: Props) => {
  ...
  const selectedAssetFromParams = React.useMemo(() => {
    ...
    if (isShowingMarketData) {
      ...
      let token = undefined as HnsWallet.BlockchainToken | undefined
      if (coinMarket) {
        token = new HnsWallet.BlockchainToken()
  ...
  const fullAssetBalance = React.useMemo(() => {
    const amounts = candidateAccounts.map((account: HnsWallet.AccountInfo) =>
      getBalance(
        account.accountId,
        selectedAssetFromParams,
  ...
  const isSelectedAssetBridgeSupported = React.useMemo(() => {
    ...
    return (isBridgeAddress || isNativeAsset) && selectedAssetFromParams.chainId === HnsWallet.MAINNET_CHAIN_ID
  }, [selectedAssetFromParams])
  ...
  const onHideAsset = React.useCallback(() => {
    ...
    dispatch(WalletPageActions.selectAsset({
      asset: undefined,
      timeFrame: HnsWallet.AssetPriceTimeframe.OneDay
    }))
          ...
          <ButtonRow>
            ...
            {isSelectedAssetDepositSupported &&
              <BridgeToAuroraButton
                ...
                noBottomMargin={true}
              >
                {getLocale('hnsWalletAccountsDeposit')}
              </BridgeToAuroraButton>
            }
            {isSelectedAssetBridgeSupported &&
              <BridgeToAuroraButton
                ...
                noBottomMargin={true}
              >
                {getLocale('hnsWalletBridgeToAuroraButton')}
              </BridgeToAuroraButton>
            }
          </ButtonRow>
        </Row>
