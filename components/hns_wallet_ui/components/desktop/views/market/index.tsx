...
// Constants
import { HnsWallet, WalletRoutes, WalletState } from '../../../../constants/types'
...
export const MarketView = () => {
  ...
  const onSelectBuy = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
    history.push(WalletRoutes.FundWalletPage.replace(':tokenId?', coinMarket.symbol))
  }, [])

  const onSelectDeposit = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
  ...
  const onMessageEventListener = React.useCallback((event: MessageEvent<MarketCommandMessage>) => {
    ...
    if (event.origin !== hnsMarketUiOrigin) return
