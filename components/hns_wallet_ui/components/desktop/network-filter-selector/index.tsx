...
interface Props {
  networkListSubset?: HnsWallet.NetworkInfo[]
  selectedNetwork?: HnsWallet.NetworkInfo
  selectedAccount?: Pick<HnsWallet.AccountInfo,
    | 'accountId'
    | 'address'
    | 'name'
  >
  isV2?: boolean
  onSelectNetwork?: (network: HnsWallet.NetworkInfo) => void
}
...
export const NetworkFilterSelector = ({
  ...
  // memos
  const filteredNetworks: HnsWallet.NetworkInfo[] = React.useMemo(() => {
  ...
  const onSelectAndClose = React.useCallback((network: HnsWallet.NetworkInfo) => {
    if (onSelectNetwork) {
  ...
  return (
    <StyledWrapper>
      ...
      {showNetworkFilter &&
        <DropDown>
          {primaryNetworks.map((network: HnsWallet.NetworkInfo) =>
            <NetworkFilterItem
          ...
          {secondaryNetworks.length > 0 &&
            <>
              <SecondaryNetworkText>{getLocale('hnsWalletNetworkFilterSecondary')}</SecondaryNetworkText>
          ...
          {testNetworks.length > 0 &&
            <>
              <SecondaryNetworkText>{getLocale('hnsWalletNetworkFilterTestNetworks')}</SecondaryNetworkText>
