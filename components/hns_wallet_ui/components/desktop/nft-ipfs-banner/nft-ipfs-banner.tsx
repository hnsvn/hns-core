...
export const NftIpfsBanner = ({ onDismiss }: Props) => {
  ...
  const onLearnMore = React.useCallback(() => {
    if (isPanel) {
      chrome.tabs.create({ url: `hns://wallet${WalletRoutes.LocalIpfsNode}` })
    } else {
    ...
    <StyledWrapper status={bannerStatus}>
      <Row gap='12px' justifyContent='flex-start'>
        ...
        <Text status={bannerStatus}>
          {bannerStatus === 'start' ? (
            <>
              {getLocale('hnsWalletNftPinningBannerStart')}
            </>
          ) : bannerStatus === 'success' ? (
            `${getLocale('hnsWalletNftPinningBannerSuccess')
              .replace('$1', `${pinnedNftsCount}`)}`
          ) : (
            `${getLocale('hnsWalletNftPinningBannerUploading')}`
          )}
        </Text>
        {bannerStatus === 'start' &&
          <LearnMore onClick={onLearnMore}>
            {getLocale('hnsWalletNftPinningBannerLearnMore')}
          </LearnMore>
        }
      </Row>
      {(bannerStatus === 'success') && (
