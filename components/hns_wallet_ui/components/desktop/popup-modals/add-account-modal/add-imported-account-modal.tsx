...
// components
import { Select } from 'hns-ui/components'
...
export const ImportAccountModal = () => {
  ...
  const importAccount = React.useCallback((accountName: string, privateKey: string, coin: HnsWallet.CoinType) => {
  ...
  const onClickCreateAccount = React.useCallback(() => {
    if (importOption === 'key') {
      if (selectedAccountType?.coin === HnsWallet.CoinType.FIL) {
        ...
        importAccount(accountName, privateKey, selectedAccountType?.coin || HnsWallet.CoinType.ETH)
      }
      return
    }
  ...
  const modalTitle = selectedAccountType
    ? getLocale('hnsWalletCreateAccountImportAccount').replace('$1', selectedAccountType.name)
    : getLocale('hnsWalletAddAccountImport')

  const filPrivateKeyFormatDescriptionTextParts =
      getLocaleWithTag('hnsWalletFilImportPrivateKeyFormatDescription')
    ...
    <PopupModal title={modalTitle} onClose={onClickClose}>
      ...
      {!selectedAccountType &&
        <SelectAccountType
          ...
          buttonText={getLocale('hnsWalletAddAccountImport')}
      ...
      {selectedAccountType &&
        <StyledWrapper>
          <ImportDisclaimer>
            <DisclaimerText>{getLocale('hnsWalletImportAccountDisclaimer')}</DisclaimerText>
          </ImportDisclaimer>

          {selectedAccountType?.coin === HnsWallet.CoinType.FIL &&
            <>
              <WarningWrapper>
                <WarningText>
          ...
          {selectedAccountType?.coin === HnsWallet.CoinType.ETH &&
            <SelectWrapper>
              <Select
                value={importOption}
              ...
              >
                <div data-value='key'>
                  {getLocale('hnsWalletImportAccountKey')}
                </div>
                <div data-value='file'>
                  {getLocale('hnsWalletImportAccountFile')}
                </div>
              </Select>
            </SelectWrapper>
          }
          ...
          {hasImportError &&
            <ErrorText>{getLocale('hnsWalletImportAccountError')}</ErrorText>
          }

          {importOption === 'key' && (
            <Input
              placeholder={getLocale('hnsWalletImportAccountPlaceholder')}
          ...
          {importOption !== 'key' && (
            <>
              <ImportRow>
                <ImportButton htmlFor='recoverFile'>{getLocale('hnsWalletImportAccountUploadButton')}</ImportButton>
                <DisclaimerText>{file ? reduceFileName(file[0].name) : getLocale('hnsWalletImportAccountUploadPlaceholder')}</DisclaimerText>
              </ImportRow>
              <input
                type='file'
                id='recoverFile'
              ...
              />
              <Input
                placeholder={`Origin ${getLocale('hnsWalletCreatePasswordInput')}`}
          ...
          <NavButton
            ...
            disabled={isDisabled}
            text={getLocale('hnsWalletAddAccountImport')}
            buttonType='primary'
          />
