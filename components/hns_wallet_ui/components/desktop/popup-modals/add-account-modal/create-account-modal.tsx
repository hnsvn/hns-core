...
export const CreateAccountModal = () => {
  ...
  const [filecoinNetwork, setFilecoinNetwork] = React.useState<FilecoinNetwork>(HnsWallet.FILECOIN_MAINNET)
  const [bitcoinNetwork, setBitcoinNetwork] = React.useState<BitcoinNetwork>(HnsWallet.BITCOIN_TESTNET)
  ...
  const suggestedAccountName = React.useMemo(() => {
    ...
    return `${selectedAccountType?.name} ${getLocale('hnsWalletAccount')} ${accountTypeLength}`
  ...
  const targetKeyringId = React.useMemo(() => {
    ...
    const network =
      (selectedAccountType.coin === HnsWallet.CoinType.FIL &&
        filecoinNetwork) ||
      (selectedAccountType.coin === HnsWallet.CoinType.BTC &&
        bitcoinNetwork) ||
      undefined
  ...
  const modalTitle = selectedAccountType
    ? getLocale('hnsWalletCreateAccount').replace('$1', selectedAccountType.name)
    : getLocale('hnsWalletCreateAccountButton')
    ...
    <PopupModal title={modalTitle} onClose={onClickClose}>
      ...
      {selectedAccountType &&
        <StyledWrapper>
          ...
          <NavButton
            ...
            disabled={isDisabled}
            text={getLocale('hnsWalletCreateAccountButton')}
            buttonType='primary'
          />
        </StyledWrapper>
      }
      ...
      {!selectedAccountType &&
        <SelectAccountType
          ...
          buttonText={getLocale('hnsWalletAddAccountCreate')}
