...
const getErrorMessage = (error: undefined | string | LedgerError, accountTypeName: string) => {
  if (typeof error === 'undefined') {
    return { error: getLocale('hnsWalletUnknownInternalError'), userHint: '' }
  }
  ...
  if (error.statusCode && error.statusCode === 27404) { // Unknown Error
    return { error: getLocale('hnsWalletConnectHardwareInfo2').replace('$1', accountTypeName), userHint: '' }
  }
  ...
  if (error.statusCode && (error.statusCode === 27904 || error.statusCode === 26368)) { // INCORRECT_LENGTH or INS_NOT_SUPPORTED
    return { error: error.message, userHint: getLocale('hnsWalletConnectHardwareWrongApplicationUserHint') }
  }
...
export const HardwareWalletConnect = ({ onSuccess, selectedAccountType }: Props) => {
  ...
  // state
  const [selectedHardwareWallet, setSelectedHardwareWallet] = React.useState<HardwareVendor>(HnsWallet.LEDGER_HARDWARE_VENDOR)
  const [isConnecting, setIsConnecting] = React.useState<boolean>(false)
  const [accounts, setAccounts] = React.useState<HnsWallet.HardwareWalletAccount[]>([])
  ...
  const [selectedDerivationScheme, setSelectedDerivationScheme] = React.useState<HardwareDerivationScheme>(
    selectedAccountType.coin === HnsWallet.CoinType.SOL ? SolDerivationPaths.Default : LedgerDerivationPaths.LedgerLive
  )
  ...
  const onFilecoinNetworkChanged = React.useCallback((network: FilecoinNetwork) => {
    ...
    onConnectHardwareWallet({
      hardware: HnsWallet.LEDGER_HARDWARE_VENDOR,
      startIndex: 0,
  ...
  const onAddHardwareAccounts = React.useCallback((selected: HnsWallet.HardwareWalletAccount[]) => {
  ...
  const getDefaultAccountName = React.useCallback((account: HnsWallet.HardwareWalletAccount) => {
  ...
  const onSelectLedger = React.useCallback(() => {
    if (selectedHardwareWallet !== HnsWallet.LEDGER_HARDWARE_VENDOR) {
      setConnectionError(undefined)
    }

    selectVendor(HnsWallet.LEDGER_HARDWARE_VENDOR)
  }, [selectedHardwareWallet, selectVendor])

  const onSelectTrezor = React.useCallback(() => {
    if (selectedHardwareWallet !== HnsWallet.TREZOR_HARDWARE_VENDOR) {
      setConnectionError(undefined)
    }

    selectVendor(HnsWallet.TREZOR_HARDWARE_VENDOR)
  }, [selectedHardwareWallet, selectVendor])
    ...
    return (
      <>
        <HardwareInfoRow>
          ...
          <HardwareInfoColumn>
            <DisclaimerText>
              {getLocale('hnsWalletConnectHardwareAuthorizationNeeded')}
            </DisclaimerText>
          </HardwareInfoColumn>
        </HardwareInfoRow>
  ...
  return (
    <>
      {(selectedAccountType.coin !== HnsWallet.CoinType.FIL && selectedAccountType.coin !== HnsWallet.CoinType.SOL) &&
        <>
          <HardwareTitle>{getLocale('hnsWalletConnectHardwareTitle')}</HardwareTitle>
          <HardwareButtonRow>
            <HardwareButton
              onClick={onSelectLedger}
              isSelected={selectedHardwareWallet === HnsWallet.LEDGER_HARDWARE_VENDOR}
              disabled={isConnecting && selectedHardwareWallet !== HnsWallet.LEDGER_HARDWARE_VENDOR}
            >
              <LedgerIcon />
            </HardwareButton>
            ...
            <HardwareButton
              ...
              isSelected={selectedHardwareWallet === HnsWallet.TREZOR_HARDWARE_VENDOR}
              disabled={isConnecting && selectedHardwareWallet !== HnsWallet.TREZOR_HARDWARE_VENDOR}
            >
              <TrezorIcon />
            </HardwareButton>
          </HardwareButtonRow>
      ...
      <HardwareInfoRow>
        ...
        <HardwareInfoColumn>
          <DisclaimerText>
            {getLocale('hnsWalletConnectHardwareInfo1').replace('$1', selectedHardwareWallet)}
          </DisclaimerText>
          <DisclaimerText>{getLocale('hnsWalletConnectHardwareInfo2').replace('$1', selectedAccountType.name)}</DisclaimerText>
        </HardwareInfoColumn>
      </HardwareInfoRow>
      {connectionError &&
        <>
      ...
      {isConnecting ? (
        <ConnectingButton>
          <LoadIcon size='small' />
          <ConnectingButtonText>{getLocale('hnsWalletConnectingHardwareWallet')}</ConnectingButtonText>
        </ConnectingButton>
      ) : (
        <NavButton onSubmit={onSubmit} text={getLocale('hnsWalletAddAccountConnect')} buttonType='primary' />
      )}
    </>
  )
}
