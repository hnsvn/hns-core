...
import { Checkbox, Select } from 'hns-ui/components'
import {
  ButtonsContainer,
  DisclaimerWrapper,
...
interface Props {
  ...
  accounts: HnsWallet.HardwareWalletAccount[]
  preAddedHardwareWalletAccounts: HnsWallet.AccountInfo[]
  onLoadMore: () => void
...
export const HardwareWalletAccountsList = ({
  ...
  const [filteredAccountList, setFilteredAccountList] = React.useState<
    HnsWallet.HardwareWalletAccount[]
  >([])
  ...
  const networksSubset = React.useMemo(() => {
    ...
    return networksRegistry.idsByCoinType[coin].map(
      (id) => networksRegistry.entities[id] as HnsWallet.NetworkInfo
    )
  ...
  // methods
  const onSelectAccountCheckbox = (account: HnsWallet.HardwareWalletAccount) => () => {
  ...
  const isPreAddedAccount = React.useCallback((account: HnsWallet.HardwareWalletAccount) => {
    return preAddedHardwareWalletAccounts.some(e => e.address === account.address)
  }, [preAddedHardwareWalletAccounts])

  const onSelectNetwork = React.useCallback(
    (n: HnsWallet.NetworkInfo): void => {
  ...
  return (
    <>
      ...
      <DisclaimerWrapper>
        <DisclaimerText>{getLocale('hnsWalletSwitchHDPathTextHardwareWallet')}</DisclaimerText>
      </DisclaimerWrapper>
      <SearchBar
        placeholder={getLocale('hnsWalletSearchScannedAccounts')}
      ...
      <HardwareWalletAccountsListContainer>
        ...
        {accounts.length > 0 && filteredAccountList.length === 0 && (
          <NoSearchResultText>
            {getLocale('hnsWalletConnectHardwareSearchNothingFound')}
          </NoSearchResultText>
        )}
      ...
      <ButtonsContainer>
        <NavButton
          ...
          text={isLoadingMore ? getLocale('hnsWalletLoadingMoreAccountsHardwareWallet')
            : getLocale('hnsWalletLoadMoreAccountsHardwareWallet')}
          buttonType='primary'
          disabled={isLoadingMore || accounts.length === 0 || selectedDerivationScheme === SolDerivationPaths.Bip44Root}
        />
        <NavButton
          onSubmit={onAddAccounts}
          text={getLocale('hnsWalletAddCheckedAccountsHardwareWallet')}
          buttonType='primary'
...
interface AccountListItemProps {
  account: HnsWallet.HardwareWalletAccount
  onSelect: () => void
  selected: boolean
  disabled: boolean
  balanceAsset: Pick<
