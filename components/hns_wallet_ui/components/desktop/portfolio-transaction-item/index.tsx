...
export interface Props {
  transaction: HnsWallet.TransactionInfo | SerializableTransactionInfo
...
export const PortfolioTransactionItem = React.forwardRef<HTMLDivElement, Props>(({
  ...
  const { data: solFeeEstimates } = useGetSolanaEstimatedFeeQuery(
    txCoinType === HnsWallet.CoinType.SOL &&
      transaction.chainId &&
      transaction.id
      ? {
  ...
  const onSelectAccount = React.useCallback((account?: HnsWallet.AccountInfo) => {
    if (!account)
      return
  ...
  const onSelectAsset = React.useCallback((asset: HnsWallet.BlockchainToken) => {
  ...
  const transactionIntentDescription = React.useMemo(() => {
    switch (true) {
      case transaction.txType === HnsWallet.TransactionType.ERC20Approve: {
      ...
      case transaction.txType === HnsWallet.TransactionType.ETHSwap: {
        return (
          <DetailRow>
            <DetailTextDark>
              {sellAmount?.format(6)}{' '}
      ...
      case transaction.txType === HnsWallet.TransactionType.ETHSend:
      case transaction.txType === HnsWallet.TransactionType.ERC20Transfer:
  ...
  const transactionActionLocale = React.useMemo(() => {
    if (isSwap) {
      return <strong>
        {getLocale('hnsWalletSwap').toLocaleUpperCase()}
      </strong>
    }
    ...
    return (
      <>
        <strong>
          {getLocale('hnsWalletTransactionSent').toLocaleUpperCase()}{' '}
        </strong>
        <AddressOrAsset
          // Disabled for ERC721 tokens until we have NFT meta data
          disabled={
            transaction.txType === HnsWallet.TransactionType.ERC721TransferFrom ||
            transaction.txType === HnsWallet.TransactionType.ERC721SafeTransferFrom
          }
          onClick={onAssetClick(txSymbol, txToken?.contractAddress)}
        >
          {txSymbol}
  ...
  const wasTxRejected =
    transaction.txStatus !== HnsWallet.TransactionStatus.Rejected &&
    transaction.txStatus !== HnsWallet.TransactionStatus.Unapproved
    ...
    >
      ...
      <StatusBalanceAndMoreContainer>
        ...
        <BalanceAndMoreRow>
          <BalanceColumn>
            {transaction.txType !==
              HnsWallet.TransactionType.ERC20Approve && (
              <>
                <DetailTextDark>
                  {/*
          ...
          {showTransactionPopup && (
            <TransactionPopup>
              {[
                ...
                HnsWallet.TransactionStatus.Confirmed,
                HnsWallet.TransactionStatus.Dropped
                ...
                <>
                  <TransactionPopupItem
                    ...
                    text={getLocale('hnsWalletTransactionExplorer')}
                  />
                  <TransactionPopupItem
                    onClick={onClickCopyTransactionHash}
                    text={getLocale('hnsWalletTransactionCopyHash')}
                  />
                </>
              )}
                ...
                !isFilecoinTx && (
                  <>
                    <TransactionPopupItem
                      ...
                      text={getLocale('hnsWalletTransactionSpeedup')}
                    />
                    <TransactionPopupItem
                      onClick={onClickCancelTransaction}
                      text={getLocale('hnsWalletTransactionCancel')}
                    />
                  </>
                )}

              {HnsWallet.TransactionStatus.Error === transaction.txStatus &&
                !isSolanaTx &&
                !isFilecoinTx && (
                  <TransactionPopupItem
                    onClick={onClickRetryTransaction}
                    text={getLocale('hnsWalletTransactionRetry')}
                  />
                )}
            </TransactionPopup>
          )}
