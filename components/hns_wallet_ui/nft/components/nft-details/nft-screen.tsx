...
interface Props {
  ...
  tokenNetwork?: HnsWallet.NetworkInfo
  ownerAccount?: HnsWallet.AccountInfo
}
...
export const NftScreen = (props: Props) => {
  ...
  const onClickViewOnBlockExplorer = useExplorer(tokenNetwork || new HnsWallet.NetworkInfo())

  // memos
  const isNftPinned = React.useMemo(() => {
    return currentNftPinningStatus?.code === HnsWallet.TokenPinStatusCode.STATUS_PINNED
  ...
  const pinningStatusText = React.useMemo(() => {
    switch (currentNftPinningStatus?.code) {
      case HnsWallet.TokenPinStatusCode.STATUS_PINNING_IN_PROGRESS:
        return getLocale('hnsWalletNFTDetailsPinningInProgress')

      case HnsWallet.TokenPinStatusCode.STATUS_PINNING_PENDING:
        return getLocale('hnsWalletNFTDetailsPinningInProgress')

      case HnsWallet.TokenPinStatusCode.STATUS_NOT_PINNED:
        return getLocale('hnsWalletNFTDetailsNotAvailable')

        case HnsWallet.TokenPinStatusCode.STATUS_PINNED:
        return getLocale('hnsWalletNFTDetailsPinningSuccessful')

        case HnsWallet.TokenPinStatusCode.STATUS_PINNING_FAILED:
        return getLocale('hnsWalletNFTDetailsPinningFailed')
  ...
  const onMessageEventListener = React.useCallback((event: MessageEvent<CommandMessage>) => {
    ...
    if (
      event.origin === hnsNftDisplayOrigin ||
      event.origin === hnsWalletPanelOrigin
    ) {
  ...
  const onClickContractAddress = React.useCallback(() => {
    onClickViewOnBlockExplorer(
      selectedAsset.coin === HnsWallet.CoinType.ETH
        ? 'nft'
        : 'token',
    ...
    return (
      <StyledWrapper>
        <Row alignItems='center' justifyContent='center'>
          <ErrorMessage>{getLocale('hnsWalletNftFetchingError')}</ErrorMessage>
        </Row>
      </StyledWrapper>
    )
  ...
  return (
    <StyledWrapper>
      ...
      <SectionTitle>{getLocale('hnsWalletNFTDetailsOverview')}</SectionTitle>
      {ownerAccount && (
        <SectionWrapper>
          <InfoBox>
            <InfoTitle>{getLocale('hnsWalletNFTDetailsOwnedBy')}</InfoTitle>
            ...
            <ViewAccount onClick={() => onClickViewAccount(ownerAccount)}>
              {getLocale('hnsWalletNFTDetailsViewAccount')}
            </ViewAccount>
          </InfoBox>
        </SectionWrapper>
      )}
      <SectionWrapper>
        {selectedAsset.tokenId !== '' && (
          <InfoBox>
            <InfoTitle>{getLocale('hnsWalletNFTDetailTokenID')}</InfoTitle>
            <InfoText>{tokenId || ''}</InfoText>
          </InfoBox>
        )}
        {tokenNetwork && (
          <InfoBox>
            <InfoTitle>{getLocale('hnsWalletNFTDetailBlockchain')}</InfoTitle>
      ...
      <SectionWrapper>
        <InfoBox>
          <InfoTitle>
            {getLocale('hnsWalletNFTDetailContractAddress')}
          </InfoTitle>
        ...
        <InfoBox>
          <InfoTitle>
            {getLocale('hnsWalletNFTDetailTokenStandard')}
          </InfoTitle>
      ...
      <SectionWrapper>
        {isFetchingNFTMetadata ? (
          ...
          <InfoBox>
            <InfoTitle>
              {getLocale('hnsWalletNFTDetailDescription')}
            </InfoTitle>
            <Description>
        ...
        <>
          ...
          <SectionTitle>
            {isFetchingNFTMetadata ? (
              ...
              <>
                {getLocale('hnsWalletNFTDetailsPinningStatusLabel')}:&nbsp;
                {currentNftPinningStatus?.code ? (
                  <PinningStatus pinningStatus={currentNftPinningStatus?.code}>
                    {pinningStatusText}
                  </PinningStatus>
                ) : (
                  getLocale('hnsWalletNFTDetailsNotAvailable')
                )}
                {currentNftPinningStatus?.code ===
                  HnsWallet.TokenPinStatusCode.STATUS_PINNING_FAILED && (
                  <ErrorWrapper
          ...
          <SectionWrapper>
            {isFetchingNFTMetadata ? (
              ...
              <InfoBox>
                <InfoTitle>{getLocale('hnsWalletNFTDetailsCid')}</InfoTitle>
                <InfoText>
                  {isNftPinned
            ...
            {isFetchingNFTMetadata ? (
              ...
              <InfoBox>
                <InfoTitle>
                  {getLocale('hnsWalletNFTDetailImageAddress')}
                </InfoTitle>
      ...
      {isFetchingNFTMetadata ? (
        ...
        <>
          {nftMetadata?.attributes?.length !== 0 && (
            <SectionTitle>
              {getLocale('hnsWalletNFTDetailsProperties')}
            </SectionTitle>
          )}
        </>
      )}
