...
export const PinningStatus = styled.span<{
  pinningStatus: HnsWallet.TokenPinStatusCode
}>`
  color: ${(p) =>
    (p.pinningStatus === HnsWallet.TokenPinStatusCode.STATUS_PINNING_IN_PROGRESS) ||
    (p.pinningStatus === HnsWallet.TokenPinStatusCode.STATUS_PINNING_PENDING) ||
    (p.pinningStatus === HnsWallet.TokenPinStatusCode.STATUS_NOT_PINNED)
      ? `${leo.color.systemfeedback.warningIcon}`
      : p.pinningStatus === HnsWallet.TokenPinStatusCode.STATUS_PINNED
