...
const nftDisplayOrigin = loadTimeData.getString('hnsWalletNftBridgeUrl') || ''
// remove trailing /
export const hnsNftDisplayOrigin = nftDisplayOrigin.endsWith('/') ? nftDisplayOrigin.slice(0, -1) : nftDisplayOrigin
...
export const sendMessageToNftUiFrame = (targetWindow: Window | null, message: CommandMessage) => {
  if (targetWindow && !isComponentInStorybook()) {
    targetWindow.postMessage(message, hnsNftDisplayOrigin)
  }
}

export const sendMessageToWalletUi = (targetWindow: Window | null, message: CommandMessage) => {
  if (targetWindow) {
    targetWindow.postMessage(message, hnsWalletOrigin)
  }
}
