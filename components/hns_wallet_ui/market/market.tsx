...
const App = () => {
  ...
  const [coinMarkets, setCoinMarkets] = React.useState<HnsWallet.CoinMarket[]>([])
  const [tradableAssets, setTradableAssets] = React.useState<HnsWallet.BlockchainToken[]>([])
  const [buyableAssets, setBuyableAssets] = React.useState<HnsWallet.BlockchainToken[]>([])
  const [depositableAssets, setDepositableAssets] = React.useState<HnsWallet.BlockchainToken[]>([])
  const [defaultCurrencies, setDefaultCurrencies] = React.useState<DefaultCurrencies>()

  // Constants
  const origin =
    window.location.ancestorOrigins[0] === hnsWalletPanelOrigin
      ? hnsWalletPanelOrigin
      : hnsWalletOrigin
  ...
  // Methods
  const isBuySupported = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
    return buyableAssets.some((asset) => asset.symbol.toLowerCase() === coinMarket.symbol.toLowerCase())
  }, [buyableAssets])

  const isDepositSupported = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
    return depositableAssets.some((asset) => asset.symbol.toLowerCase() === coinMarket.symbol.toLowerCase())
  }, [depositableAssets])

  const onClickBuy = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
  ...
  const onClickDeposit = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
  ...
  const onMessageEventListener = React.useCallback((event: MessageEvent<MarketCommandMessage>) => {
    ...
    if (event.origin === hnsWalletOrigin || event.origin === hnsWalletPanelOrigin) {
  ...
  const onSelectCoinMarket = React.useCallback((coinMarket: HnsWallet.CoinMarket) => {
