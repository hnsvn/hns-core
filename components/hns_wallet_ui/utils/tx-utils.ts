...
export interface ParsedTransaction
  extends ParsedTransactionFees,
    Pick<HnsWallet.TransactionInfo, 'txType'> {
  // Common fields
  id: string
  ...
  status: HnsWallet.TransactionStatus
  senderLabel: string
  recipient: string
  ...
  intent: string
  chainId: string
  originInfo?: HnsWallet.OriginInfo | undefined
  ...
  erc721BlockchainToken?: HnsWallet.BlockchainToken
  ...
  // Swap
  sellToken?: HnsWallet.BlockchainToken
  sellAmount?: Amount
  sellAmountWei?: Amount
  buyToken?: HnsWallet.BlockchainToken
  minBuyAmount?: Amount
...
export const getLocaleKeyForTxStatus = (
  ...
  switch (status) {
    case HnsWallet.TransactionStatus.Unapproved:
      return 'hnsWalletTransactionStatusUnapproved'
    case HnsWallet.TransactionStatus.Approved:
      return 'hnsWalletTransactionStatusApproved'
    case HnsWallet.TransactionStatus.Rejected:
      return 'hnsWalletTransactionStatusRejected'
    case HnsWallet.TransactionStatus.Submitted:
      return 'hnsWalletTransactionStatusSubmitted'
    case HnsWallet.TransactionStatus.Confirmed:
      return 'hnsWalletTransactionStatusConfirmed'
    case HnsWallet.TransactionStatus.Error:
      return 'hnsWalletTransactionStatusError'
    case HnsWallet.TransactionStatus.Dropped:
      return 'hnsWalletTransactionStatusDropped'
    case HnsWallet.TransactionStatus.Signed:
      return 'hnsWalletTransactionStatusSigned'
    default:
      return ''
  }
}
...
export function isSolanaTransaction(
  ...
  return (
    ...
    (txType === HnsWallet.TransactionType.Other && solanaTxData !== undefined)
  )
}
...
export function shouldReportTransactionP3A({
  ...
  txInfo: Pick<
    HnsWallet.TransactionInfo | SerializableTransactionInfo,
    'txType' | 'chainId'
  > & { coinType: HnsWallet.CoinType }
}) {
  if (
    P3ASendTransactionTypes.includes(txInfo.txType) ||
    (txInfo.coinType === HnsWallet.CoinType.FIL &&
      txInfo.txType === HnsWallet.TransactionType.Other)
  ) {
    const countTestNetworks = loadTimeData.getBoolean(
      HnsWallet.P3A_COUNT_TEST_NETWORKS_LOAD_TIME_KEY
    )
...
export function isSolanaDappTransaction (tx: TransactionInfo): tx is SolanaTransactionInfo {
  return (
    ...
    [
      ...
      HnsWallet.TransactionType.Other
    ].includes(tx.txType)
  )
}
...
export function isSolanaSplTransaction (tx: TransactionInfo): tx is SolanaTransactionInfo {
  return (
    tx.txType === HnsWallet.TransactionType.SolanaSPLTokenTransfer ||
    tx.txType === HnsWallet.TransactionType.SolanaSPLTokenTransferWithAssociatedTokenAccountCreation
  )
}

export const findTransactionToken = <
  T extends Pick<HnsWallet.BlockchainToken, 'contractAddress'>
>(
...
export const getETHSwapTransactionBuyAndSellTokens = ({
  ...
  tx: TransactionInfo | undefined
  nativeAsset?: HnsWallet.BlockchainToken
  tokensList: HnsWallet.BlockchainToken[]
}): {
  buyToken?: HnsWallet.BlockchainToken
  sellToken?: HnsWallet.BlockchainToken
  buyAmount: Amount
  ...
  if (
    !tx ||
    tx.txType !== HnsWallet.TransactionType.ETHSwap
  ) {
    return {
      buyToken: undefined,
      sellToken: undefined,
  ...
  const fillTokens: HnsWallet.BlockchainToken[] = (fillContracts || [])
    .map(path => '0x' + path)
    .map(address =>
...
interface GetTransactionTransferredValueArgs {
  ...
  token?: HnsWallet.BlockchainToken
  sellToken?: HnsWallet.BlockchainToken
  txAccount: HnsWallet.AccountInfo | undefined
  txNetwork: HnsWallet.NetworkInfo | undefined
}
...
export function getTransactionTransferredValue (args: GetTransactionTransferredValueArgs): {
  ...
  // ERC20 Transfers
  if (
    tx.txType === HnsWallet.TransactionType.ERC20Approve ||
    tx.txType === HnsWallet.TransactionType.ERC20Transfer
  ) {
  ...
  // ERC721
  if (
    tx.txType === HnsWallet.TransactionType.ERC721TransferFrom ||
    tx.txType === HnsWallet.TransactionType.ERC721SafeTransferFrom
  ) {
  ...
  // ETH Swap
  if (tx.txType === HnsWallet.TransactionType.ETHSwap) {
    // (bytes fillPath, uint256 sellAmount, uint256 minBuyAmount)
...
export const getTransactionApprovalTargetAddress = (tx: TransactionInfo): string => {
  if (tx.txType === HnsWallet.TransactionType.ERC20Approve) {
...
export function getTransactionDecimals ({
  ...
  sellToken?: HnsWallet.BlockchainToken
  erc721Token?: HnsWallet.BlockchainToken
  token?: HnsWallet.BlockchainToken
}) {
  if (sellToken) {
  ...
  switch (getCoinFromTxDataUnion(tx.txDataUnion)) {
    case HnsWallet.CoinType.SOL: return 9
    case HnsWallet.CoinType.ETH: return 18
    case HnsWallet.CoinType.FIL: return 18
    default: return 18
  }
}
...
export const getTransactionErc721TokenId = (
  ...
  if ([
    HnsWallet.TransactionType.ERC721TransferFrom,
    HnsWallet.TransactionType.ERC721SafeTransferFrom
  ].includes(tx.txType)) {
...
export const isSendingToKnownTokenContractAddress = (
  ...
  // ERC20Transfer
  if (tx.txType === HnsWallet.TransactionType.ERC20Transfer) {
...
export const transactionHasSameAddressError = (
  ...
  if (txType === HnsWallet.TransactionType.ERC20Transfer) {
  ...
  if (txType === HnsWallet.TransactionType.ERC20Approve) {
  ...
  if (
    ...
    HnsWallet.TransactionType.ETHSwap ||
    HnsWallet.TransactionType.Other
  ) {
    return false
  }
...
export function getGasFeeFiatValue ({
  ...
  txNetwork?: Pick<HnsWallet.NetworkInfo, 'decimals'>
}) {
...
export const accountHasInsufficientFundsForTransaction = ({
  ...
  // Eth Swap
  if (txType === HnsWallet.TransactionType.ETHSwap) {
...
export function getTransactionTransferredToken ({
  ...
  tx: TransactionInfo
  txNetwork?: HnsWallet.NetworkInfo
  token?: HnsWallet.BlockchainToken
  sellToken?: HnsWallet.BlockchainToken
}): HnsWallet.BlockchainToken | undefined {
  if (tx.txType === HnsWallet.TransactionType.ETHSwap) {
    return sellToken
  }
...
export function getTransactionTokenSymbol ({
  ...
  tx: TransactionInfo
  txNetwork?: Pick<HnsWallet.NetworkInfo, 'symbol'>
  token?: HnsWallet.BlockchainToken
  sellToken?: HnsWallet.BlockchainToken
}): string {
  if (isSolanaDappTransaction(tx)) {
...
export const getTransactionIntent = ({
  ...
  sellToken?: HnsWallet.BlockchainToken
  token?: HnsWallet.BlockchainToken
  transactionNetwork?: HnsWallet.NetworkInfo
  tx: TransactionInfo
}): string => {
  // ERC20 Approve
  if (tx.txType === HnsWallet.TransactionType.ERC20Approve) {
    return toProperCase(getLocale('hnsWalletApprovalTransactionIntent')) + ' ' + token?.symbol ?? ''
  }

  // ERC20 Transfer
  if (tx.txType === HnsWallet.TransactionType.ERC20Transfer) {
    return getLocale('hnsWalletTransactionIntentSend').replace(
      '$1',
  ...
  // Solana Dapps
  if (isSolanaDappTransaction(tx)) {
    return tx.txType === HnsWallet.TransactionType.SolanaSwap
      ? getLocale('hnsWalletSwap')
      : getLocale('hnsWalletTransactionIntentDappInteraction')
  }

  // SPL
  if (isSolanaSplTransaction(tx)) {
    return getLocale('hnsWalletTransactionIntentSend')
      .replace(
        '$1',
        new Amount(normalizedTransferredValue).formatAsAsset(6, token?.symbol)
      )
  ...
  // ETHSwap
  if (tx.txType === HnsWallet.TransactionType.ETHSwap) {
    return getLocale('hnsWalletTransactionIntentSwap')
...
export const parseTransactionWithoutPrices = ({
  ...
  tx: TransactionInfo
  transactionAccount: HnsWallet.AccountInfo
  transactionNetwork: HnsWallet.NetworkInfo
  tokensList: HnsWallet.BlockchainToken[]
  ...
  const missingGasLimitError = isMissingGasLimit
    ? getLocale('hnsWalletMissingGasLimitError')
    : undefined
  ...
  const contractAddressError = isSendingToKnownTokenContractAddress(
    ...
    ? getLocale('hnsWalletContractAddressError')
    : undefined
  ...
  const sameAddressError = transactionHasSameAddressError(tx)
    ? getLocale('hnsWalletSameAddressError')
    : undefined
...
export function toTxDataUnion<D extends keyof HnsWallet.TxDataUnion>(
  unionItem: Pick<HnsWallet.TxDataUnion, D>
) {
  return Object.assign({}, unionItem) as HnsWallet.TxDataUnion
}


export function isSolanaSignTransactionRequest(
  request?:
    | HnsWallet.SignTransactionRequest
    | HnsWallet.SignAllTransactionsRequest
): request is HnsWallet.SignTransactionRequest {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
  return !!(request as HnsWallet.SignTransactionRequest | undefined)?.txData
    ?.solanaTxData
}
...
export function isSolanaSignAllTransactionsRequest(
  request?:
    | HnsWallet.SignTransactionRequest
    | HnsWallet.SignAllTransactionsRequest
): request is HnsWallet.SignAllTransactionsRequest {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
  return !!(request as HnsWallet.SignAllTransactionsRequest | undefined)
    ?.txDatas
}

export function getTxDataFromSolSignTxRequest(
  selectedQueueData: HnsWallet.SignTransactionRequest
): HnsWallet.SolanaTxData | undefined {
  return selectedQueueData.txData?.solanaTxData
}

export function getTxDatasFromSolSignAllTxsRequest(
  selectedQueueData: HnsWallet.SignAllTransactionsRequest
): HnsWallet.SolanaTxData[] {
  return selectedQueueData.txDatas
    .map(({ solanaTxData }) => solanaTxData)
    .filter((data): data is HnsWallet.SolanaTxData => !!data)
}
