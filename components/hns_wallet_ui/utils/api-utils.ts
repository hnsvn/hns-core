...
import { HnsWallet, SupportedTestNetworks, SupportedCoinTypes } from '../constants/types'
...
export const getPriceIdForToken = (
  ...
  if (SupportedTestNetworks.includes(token.chainId)) {
    ...
    if (
      token.chainId === HnsWallet.GOERLI_CHAIN_ID &&
  ...
  const isEthereumNetwork = token.chainId === HnsWallet.MAINNET_CHAIN_ID
