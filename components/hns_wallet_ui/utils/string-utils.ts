...
import { HnsWallet, WalletRoutes, TokenStandards } from '../constants/types'
...
export const getRampNetworkPrefix = (chainId: string, isOfframp?: boolean) => {
  switch (chainId) {
    ...
    case HnsWallet.MAINNET_CHAIN_ID: return isOfframp ? 'ETH' : ''
    case HnsWallet.AVALANCHE_MAINNET_CHAIN_ID: return 'AVAXC'
    case HnsWallet.BINANCE_SMART_CHAIN_MAINNET_CHAIN_ID: return 'BSC'
    case HnsWallet.POLYGON_MAINNET_CHAIN_ID: return 'MATIC'
    case HnsWallet.SOLANA_MAINNET: return 'SOLANA'
    case HnsWallet.OPTIMISM_MAINNET_CHAIN_ID: return 'OPTIMISM'
    // Offramp uses CELO prefix
    case HnsWallet.CELO_MAINNET_CHAIN_ID: return isOfframp ? 'CELO' : ''
    case HnsWallet.FANTOM_MAINNET_CHAIN_ID: return 'FANTOM'
    case HnsWallet.FILECOIN_MAINNET: return 'FILECOIN'
    default: return ''
  }
}
...
export const getWalletLocationTitle = (location: string) => {
  ...
  if (location.includes(WalletRoutes.FundWalletPageStart)) {
    return getLocale('hnsWalletBuyCryptoButton')
  }
  /** Deposit crypto */
  if (location.includes(WalletRoutes.DepositFundsPageStart)) {
    return getLocale('hnsWalletDepositFundsTitle')
  }
  /** Swap */
  ...
  if (location === WalletRoutes.SendPageStart) {
    return getLocale('hnsWalletSend')
  }
  /** Wallet */
  return getLocale('hnsWalletTitle')
}
...
export const getNFTTokenStandard = (token: HnsWallet.BlockchainToken) => {
  if (token.isNft && token.coin === HnsWallet.CoinType.SOL) {
    return TokenStandards.SPL
  }
  if (token.isErc721 && token.coin === HnsWallet.CoinType.ETH) {
...
export const isComponentInStorybook = () => {
  const nftDisplayOrigin =
    loadTimeData.getString('hnsWalletNftBridgeUrl') || ''
  return (
    nftDisplayOrigin === 'hnsWalletNftBridgeUrl' || nftDisplayOrigin === ''
  )
}
