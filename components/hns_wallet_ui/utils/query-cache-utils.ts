...
export const TX_CACHE_TAGS = {
  ID: (txId: HnsWallet.TransactionInfo['id']) => ({
    type: 'Transactions' as const,
    id: txId
  }),
  IDS: (txIds: Array<HnsWallet.TransactionInfo['id']>) =>
    txIds.map((id) => TX_CACHE_TAGS.ID(id)),
  FOR_COIN_TYPE: (coinType: HnsWallet.CoinType) =>
    ({
      type: 'Transactions',
  ...
  FROM_ACCOUNT_ID: (fromAccountId: HnsWallet.AccountId) =>
    ({
      type: 'Transactions',
  ...
  LISTS: ({
    ...
    chainId: string | null
    coin: HnsWallet.CoinType | null
    fromAccountId: HnsWallet.AccountId | null
  }) =>
    [
