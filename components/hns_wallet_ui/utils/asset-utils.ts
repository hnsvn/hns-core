...
export const getUniqueAssets = (assets: HnsWallet.BlockchainToken[]) => {
...
export const isSelectedAssetInAssetOptions = (
  ...
  assetOptions: HnsWallet.BlockchainToken[]
) => {
...
export const getRampAssetSymbol = (asset: HnsWallet.BlockchainToken, isOfframp?: boolean) => {
  if (asset.symbol.toUpperCase() === 'BAT' && asset.chainId === HnsWallet.MAINNET_CHAIN_ID) {
    // BAT is the only token on Ethereum Mainnet with a prefix on Ramp.Network
    return 'ETH_BAT'
  }

  if (asset.chainId === HnsWallet.AVALANCHE_MAINNET_CHAIN_ID && asset.contractAddress === '') {
...
export const getNativeTokensFromList = (tokenList: HnsWallet.BlockchainToken[]) => {
  // separate Native (gas) assets from other tokens
  const { nativeAssets, tokens } = tokenList.reduce((acc, t) => {
    if (
      t.symbol.toLowerCase() === 'eth' && t.chainId === HnsWallet.MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'eth' && t.chainId === HnsWallet.OPTIMISM_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'eth' && t.chainId === HnsWallet.AURORA_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'matic' && t.chainId === HnsWallet.POLYGON_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'ftm' && t.chainId === HnsWallet.FANTOM_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'celo' && t.chainId === HnsWallet.CELO_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'bnb' && t.chainId === HnsWallet.BINANCE_SMART_CHAIN_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'sol' && t.chainId === HnsWallet.SOLANA_MAINNET ||
      t.symbol.toLowerCase() === 'fil' && t.chainId === HnsWallet.FILECOIN_MAINNET ||
      t.symbol.toLowerCase() === 'avax' && t.chainId === HnsWallet.AVALANCHE_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'avaxc' && t.chainId === HnsWallet.AVALANCHE_MAINNET_CHAIN_ID ||
      t.symbol.toLowerCase() === 'neon' && t.chainId === HnsWallet.NEON_EVM_MAINNET_CHAIN_ID
    ) {
    ...
    nativeAssets: [] as HnsWallet.BlockchainToken[],
    tokens: [] as HnsWallet.BlockchainToken[]
  })
...
export const getBatTokensFromList = (tokenList: HnsWallet.BlockchainToken[]) => {
  ...
  const { bat, nonBat } = tokenList.reduce((acc, t) => {
    ...
    bat: [] as HnsWallet.BlockchainToken[],
    nonBat: [] as HnsWallet.BlockchainToken[]
  })
...
export const isNativeAsset = (
  token: Pick<HnsWallet.BlockchainToken, 'contractAddress'>
) => token.contractAddress === ''

export const formatTokenBalance = (
  tokenBalanceString: string | undefined,
  selectedAsset:
    | Pick<HnsWallet.BlockchainToken, 'decimals' | 'symbol'>
    | undefined,
...
export const checkIfTokensMatch = (
  tokenOne: HnsWallet.BlockchainToken,
  tokenTwo: HnsWallet.BlockchainToken
): boolean => {
...
export function filterTokensByNetworks (assets: HnsWallet.BlockchainToken[], networks: HnsWallet.NetworkInfo[]) {
...
export const checkIfTokenNeedsNetworkIcon = (
  network: Pick<HnsWallet.NetworkInfo, 'chainId' | 'symbol'>,
  contractAddress: string
) => {
