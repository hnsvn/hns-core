...
export const VerifyRecoveryPhrase = () => {
  ...
  const onNextStep = React.useCallback(() => {
    if (isOnboarding) {
      hnsWalletP3A.reportOnboardingAction(HnsWallet.OnboardingAction.Complete);
      dispatch(WalletPageActions.walletSetupComplete(true))
    }
    ...
    <CenteredPageLayout>
      <MainWrapper>
        <StyledWrapper>
          ...
          <TitleAndDescriptionContainer>
            <Title>{getLocale('hnsWalletVerifyRecoveryPhraseTitle')}</Title>
            <Description>
              <span>
                {getLocaleWithTags('hnsWalletVerifyRecoveryPhraseInstructions', 3).map((text, i) => {
          ...
          <ErrorTextRow hasError={!nextStepEnabled && hasSelectedWords}>
            {!nextStepEnabled && hasSelectedWords &&
              <>
                <ErrorXIcon />
                <ErrorText>
                  {getLocale('hnsWalletVerifyPhraseError')}
                </ErrorText>
              </>
            }
          </ErrorTextRow>
          ...
          <NextButtonRow>
            <NavButton
              buttonType='primary'
              text={getLocale('hnsWalletButtonNext')}
