  ...
  return (
    <>
      ...
      <StyledWrapper>
        ...
        <Title>{getLocale('hnsWalletRestoreTite')}</Title>
        <Description>{getLocale('hnsWalletRestoreDescription')}</Description>

        <FormWrapper>
          <RecoveryPhraseInput
            autoFocus={true}
            placeholder={getLocale('hnsWalletRestorePlaceholder')}
          ...
          {invalidMnemonic && <ErrorText>{getLocale('hnsWalletRestoreError')}</ErrorText>}

          {recoveryPhrase.split(' ').length === 24 &&
            <LegacyCheckboxRow>
              <Checkbox value={{ isLegacy: isLegacyWallet }} onChange={onSetIsLegacyWallet}>
                <div data-key='isLegacy'>{getLocale('hnsWalletRestoreLegacyCheckBox')}</div>
              </Checkbox>
            </LegacyCheckboxRow>
          }
          ...
          <CheckboxRow>
            <Checkbox value={{ showPhrase: showRecoveryPhrase }} onChange={onShowRecoveryPhrase}>
              <div data-key='showPhrase'>{getLocale('hnsWalletRestoreShowPhrase')}</div>
            </Checkbox>
          </CheckboxRow>

          <FormText>{getLocale('hnsWalletRestoreFormText')}</FormText>
          <Description textAlign='left'>{getLocale('hnsWalletCreatePasswordDescription')}</Description>
          <InputColumn>
            <PasswordInput
              placeholder={getLocale('hnsWalletCreatePasswordInput')}
              onChange={handlePasswordChanged}
              hasError={hasPasswordError}
              error={getLocale('hnsWalletCreatePasswordError')}
              onKeyDown={handleKeyDown}
            />
            <PasswordInput
              placeholder={getLocale('hnsWalletConfirmPasswordInput')}
              onChange={handleConfirmPasswordChanged}
              hasError={hasConfirmedPasswordError}
              error={getLocale('hnsWalletConfirmPasswordError')}
              onKeyDown={handleKeyDown}
            />
          </InputColumn>
        ...
        <NavButton
          disabled={isDisabled}
          buttonType='primary'
          text={getLocale('hnsWalletWelcomeRestoreButton')}
