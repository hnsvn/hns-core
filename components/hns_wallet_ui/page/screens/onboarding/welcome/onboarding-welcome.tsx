...
// routes
import { HnsWallet, PageState, WalletRoutes } from '../../../../constants/types'
...
export const OnboardingWelcome = () => {
  ...
  // custom hooks
  const { hnsWalletP3A } = useApiProxy()
  ...
  React.useEffect(() => {
    let action = HnsWallet.OnboardingAction.Shown;
    switch (nextStep) {
      case WalletRoutes.OnboardingImportOrRestore:
        action = HnsWallet.OnboardingAction.StartRestore
        break
      case WalletRoutes.OnboardingCreatePassword:
        action = HnsWallet.OnboardingAction.LegalAndPassword
        break
    }
    hnsWalletP3A.reportOnboardingAction(action)
  }, [nextStep, hnsWalletP3A])
  ...
  return <WalletPageLayout>
    <OnboardingWrapper>
      ...
      <Title maxWidth='467px'>
        {getLocale('hnsWalletWelcomeTitle')}
      </Title>
      ...
      <BlockQuote>
        ...
        <BlockQuoteTextContainer>
          <span>
            {getLocale('hnsWalletPerksTokens')}
          </span>
          <span>
            {getLocale('hnsWalletMultiChain')}
          </span>
          <span>
            {getLocale('hnsWalletPerksBrowserNative')}
          </span>
        </BlockQuoteTextContainer>
      </BlockQuote>
      ...
      <ButtonContainer>
        <NavButton
          buttonType='primary'
          text={getLocale('hnsWalletWelcomeButton')}
        ...
        <NavButton
          ...
          text={getLocale('hnsWalletImportExistingWallet')}
      ...
      <Row>
        <SubDivider />
        <SubDividerText>{getLocale('hnsWalletWelcomeDividerText')}</SubDividerText>
        <SubDivider />
      </Row>

      <NavButton
        buttonType='primary'
        text={getLocale('hnsWalletConnectHardwareWallet')}
      ...
      <LearnMoreLink
        href='https://support.hns.com/hc/en-us/categories/360001059151-Hns-Wallet'
        target='_blank'
        rel='noreferrer'
      >
        {getLocale('hnsWalletLearnMoreAboutHnsWallet')}
      </LearnMoreLink>
