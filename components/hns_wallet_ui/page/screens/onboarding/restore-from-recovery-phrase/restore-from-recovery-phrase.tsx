    ...
    switch (currentStep) {
      case RestoreFromOtherWalletSteps.currentPassword: return {
        title: restoreFrom === 'metamask'
          ? getLocale('hnsWalletMetaMaskExtensionDetected')
          : getLocale('hnsWalletCryptoWalletsDetected'),
        description: restoreFrom === 'metamask'
          ? getLocale('hnsWalletMetaMaskExtensionImportDescription')
          : getLocale('hnsWalletImportHnsLegacyDescription')
      }

      case RestoreFromOtherWalletSteps.newPassword: return {
        title: getLocale('hnsWalletCreatePasswordTitle'),
        description: getLocale('hnsWalletCreatePasswordDescription')
      }
      ...
      case RestoreFromOtherWalletSteps.phrase: return {
        ...
        description: restoreFrom.includes('metamask')
          ? getLocale('hnsWalletImportFromMetaMaskSeedInstructions')
          : getLocale('hnsWalletRestoreMyHnsWalletInstructions')
      }
        ...
        <StyledWrapper>
          ...
          <TitleAndDescriptionContainer>
            {
              isCheckingExtensions
                ? <>
                  <Title>
                    {getLocale('hnsWalletCheckingInstalledExtensions')}
                  </Title>
                </>
                : <>
              ...
              <Row alignItems='center' style={{ minHeight: 53 }}>
                {phraseWordsLength === 24 &&
                  <Checkbox isChecked={isImportingFromLegacySeed} onChange={setIsImportingFromLegacySeed} disabled={false}>
                    <Description>
                      {getLocale('hnsWalletRestoreLegacyCheckBox')}
                    </Description>
                  </Checkbox>
                }
                ...
                {phraseInput && phraseWordsLength > 12 && !isCorrectPhraseLength &&
                  <ErrorText>
                    {getLocale('hnsWalletRecoveryPhraseLengthError')}
                  </ErrorText>
                }
              </Row>
            </>
          ...
          {!isCheckingExtensions && currentStep === RestoreFromOtherWalletSteps.currentPassword &&
            <>
              <PasswordInput
                ...
                placeholder={
                  restoreFrom === 'metamask'
                    ? getLocale('hnsWalletMetaMaskPasswordInputPlaceholder')
                    : getLocale('hnsWalletImportHnsLegacyInput')
                }
                name='extensionPassword'
                label={getLocale('hnsWalletInputLabelPassword')}
              />
          ...
          {currentStep === RestoreFromOtherWalletSteps.newPassword &&
            <>
              ...
              {!!invalidMnemonic &&
                <ErrorText>
                  {getLocale('hnsWalletInvalidMnemonicError')}
                </ErrorText>
              }
            </>
          }
