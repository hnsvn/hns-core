...
export const DepositFundsScreen = (props: Props) => {
  ...
  const [selectedAccount, setSelectedAccount] =
    React.useState<HnsWallet.AccountInfo | undefined>()
  ...
  const nftCollectionAssets: HnsWallet.BlockchainToken[] =
    React.useMemo(() => {
      const nftContractTokens: HnsWallet.BlockchainToken[] = []
  ...
  const fullAssetsList: HnsWallet.BlockchainToken[] = React.useMemo(() => {
  ...
  const depositTitleText: string = React.useMemo(() => {
    ...
    const isFil = selectedAsset?.coin === HnsWallet.CoinType.FIL
    const isSolOrSpl = selectedAsset?.coin === HnsWallet.CoinType.SOL
    const isErc = selectedAsset?.isErc20 || selectedAsset?.isErc721

    // EVM native network (gas) assets & Filecoin
    if (isNativeAsset || isFil) {
      return getLocale('hnsWalletDepositX').replace('$1', selectedAsset.symbol)
    }

    // ERC-based tokens
    if (isErc) {
      return getLocale('hnsWalletDepositErc')
    }

    // Solana assets
    if (isSolOrSpl) {
      return getLocale('hnsWalletDepositSolSplTokens')
    }
  ...
  const checkIsDepositAssetSelected = React.useCallback((asset: HnsWallet.BlockchainToken) => {
    if (selectedAsset) {
  ...
  const handleScrollIntoView = React.useCallback((asset: HnsWallet.BlockchainToken, ref: HTMLButtonElement | null) => {
        ...
        <>
          ...
          <NextButtonRow>
            <NavButton
              ...
              text={
                selectedAsset
                  ? getLocale('hnsWalletButtonContinue')
                  : getLocale('hnsWalletBuySelectAsset')
              }
              onSubmit={nextStep}
      ...
      {!needsAccount && showDepositAddress &&
        <>
            ...
            <Column gap={'16px'}>
              ...
              <Column alignItems='flex-start'>
                ...
                {selectedAssetNetwork &&
                  <Description>
                    {
                      getLocale('hnsWalletDepositOnlySendOnXNetwork')
          ...
          {showAccountSearch &&
            <SearchWrapper>
              <SelectHeader
                title={getLocale('hnsWalletSelectAccount')}
                onBack={closeAccountSearch}
                hasAddButton={false}
              />
              <SearchBar
                placeholder={getLocale('hnsWalletSearchAccount')}
