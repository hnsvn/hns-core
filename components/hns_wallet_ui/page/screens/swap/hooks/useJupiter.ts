...
export function useJupiter (params: SwapParams) {
  ...
  // State
  const [quote, setQuote] = useState<HnsWallet.JupiterQuote | undefined>(undefined)
  const [error, setError] = useState<HnsWallet.JupiterErrorResponse | undefined>(undefined)
  const [loading, setLoading] = useState<boolean>(false)
  const [selectedRoute, setSelectedRoute] = useState<HnsWallet.JupiterRoute | undefined>(
    undefined
  )
  const [hnsFee, setHnsFee] =
    useState<HnsWallet.HnsSwapFeeResponse | undefined>(undefined)
  ...
  const refresh = useCallback(
    async function (
    ...
    ): Promise<HnsWallet.JupiterQuote | undefined> {
      ...
      if (
        !selectedNetwork ||
        selectedNetwork.coin !== HnsWallet.CoinType.SOL
      ) {
        return
      }
      ...
      try {
        const { response: hnsFeeResponse } = await swapService.getHnsFee({
        ...
        setHnsFee(hnsFeeResponse || undefined)
      } catch (e) {
        console.log(
          `Error getting Hns fee (Jupiter): ${
            //
    ...
    async function (callback?: () => Promise<void>) {
      ...
      try {
        const { success, errorMessage } = await sendSolanaSerializedTransaction({
          ...
          txType: HnsWallet.TransactionType.SolanaSwap,
          sendOptions: {
            skipPreflight: {
              skipPreflight: true
            },
