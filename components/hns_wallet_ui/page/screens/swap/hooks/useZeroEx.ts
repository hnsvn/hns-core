...
export function useZeroEx (params: SwapParams) {
  ...
  // State
  const [quote, setQuote] = useState<HnsWallet.SwapResponse | undefined>(undefined)
  const [error, setError] = useState<HnsWallet.SwapErrorResponse | undefined>(undefined)
  ...
  const refresh = useCallback(
    async function (
    ...
    ): Promise<HnsWallet.SwapResponse | undefined> {
      ...
      try {
        ...
        if (priceQuoteResponse?.response && hnsFeeResponse) {
          setHnsFee({
            ...hnsFeeResponse,
            protocolFeePct: priceQuoteResponse.response.fees.zeroExFee
              ? hnsFeeResponse.protocolFeePct
              : '0'
          })
        }
      } catch (e) {
