...
const hasDecimalsOverflow = (amount: string, asset?: HnsWallet.BlockchainToken) => {
  if (!asset) {
    return false
  }
...
export const useSwap = () => {
  ...
  // State
  const [fromToken, setFromToken] = useState<HnsWallet.BlockchainToken | undefined>(undefined)
  const [toToken, setToToken] = useState<HnsWallet.BlockchainToken | undefined>(undefined)
  ...
  const tokensForBalances = useMemo(() => {
    ...
    if (selectedNetwork.coin === HnsWallet.CoinType.SOL) {
      return undefined
    }
  ...
  const tokenPriceIds = useMemo(
    () =>
      [nativeAsset, fromToken, toToken]
        .filter(
          (token): token is HnsWallet.BlockchainToken => token !== undefined
        )
  ...
  const onSelectQuoteOption = useCallback(
    (index: number) => {
      ...
      } else if (selectedNetwork?.coin === HnsWallet.CoinType.ETH) {
  ...
  const getNetworkAssetsList = useCallback(
    (networkInfo: HnsWallet.NetworkInfo) => {
      return assetsList.filter(
        asset =>
  ...
  const getAssetBalance = useCallback(
    (token: HnsWallet.BlockchainToken): Amount => {
      if (!selectedAccount) {
        return Amount.zero()
      }
  ...
  const feesWrapped = useMemo(() => {
    if (selectedNetwork?.coin === HnsWallet.CoinType.ETH) {
      if (zeroEx.networkFee.isUndefined()) {
        return Amount.zero()
      } else {
        return zeroEx.networkFee
      }
    } else if (selectedNetwork?.coin === HnsWallet.CoinType.SOL) {
  ...
  const submitButtonText = useMemo(() => {
    if (!selectedAccount) {
      return getLocale('hnsSwapConnectWallet')
    }

    if (!fromToken) {
      return getLocale('hnsSwapReviewOrder')
    }

    if (swapValidationError === 'insufficientBalance') {
      return getLocale('hnsSwapInsufficientBalance').replace('$1', fromToken.symbol)
    }

    if (swapValidationError === 'insufficientFundsForGas') {
      return getLocale('hnsSwapInsufficientBalance').replace('$1', selectedNetwork?.symbol || '')
    }

    if (selectedNetwork?.coin === HnsWallet.CoinType.ETH) {
      if (swapValidationError === 'insufficientAllowance') {
        return getLocale('hnsSwapApproveToken').replace('$1', fromToken.symbol)
      }
    }

    if (swapValidationError === 'insufficientLiquidity') {
      return getLocale('hnsSwapInsufficientLiquidity')
    }
  ...
  const isSubmitButtonDisabled = useMemo(() => {
    return (
      ...
      (selectedNetwork?.coin === HnsWallet.CoinType.ETH && zeroEx.quote === undefined) ||
      // If Jupiter quote is empty, there's nothing to create the swap
      // transaction with, so Swap button must be disabled.
      (selectedNetwork?.coin === HnsWallet.CoinType.SOL && jupiter.quote === undefined) ||
      ...
      (swapValidationError && selectedNetwork?.coin === HnsWallet.CoinType.SOL)
    )
  }, [
    zeroEx.loading,
    jupiter.loading,
