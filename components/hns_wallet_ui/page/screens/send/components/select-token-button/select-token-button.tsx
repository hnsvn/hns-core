...
interface Props {
  onClick: () => void
  token: HnsWallet.BlockchainToken | undefined
...
export const SelectTokenButton = (props: Props) => {
  ...
  const buttonText = React.useMemo(() => {
    if (selectedSendOption === SendPageTabHashes.nft) {
      ...
      return token !== undefined ? `${token.name} ${id}` : getLocale('hnsWalletSelectNFT')
    }
    return token !== undefined ? token.symbol : getLocale('hnsWalletSelectToken')
