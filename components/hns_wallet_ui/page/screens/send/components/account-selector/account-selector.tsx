...
export const AccountSelector = (props: Props) => {
  ...
  const handleOnSelectAccount = React.useCallback((account: HnsWallet.AccountInfo) => {
  ...
  const isFVMAccount = React.useCallback(
    (account) =>
      (selectedNetwork?.chainId === HnsWallet.FILECOIN_ETHEREUM_MAINNET_CHAIN_ID &&
        account.accountId.keyringId === HnsWallet.KeyringId.kFilecoin) ||
      (selectedNetwork?.chainId === HnsWallet.FILECOIN_ETHEREUM_TESTNET_CHAIN_ID &&
        account.accountId.keyringId === HnsWallet.KeyringId.kFilecoinTestnet),
    [selectedNetwork]);
  ...
  const accountsByNetwork = React.useMemo(() => {
    ...
    if (selectedAccountId.coin === HnsWallet.CoinType.FIL) {
      ...
      const fevmAccounts = accounts.filter((account) =>
        (account.accountId.coin === HnsWallet.CoinType.ETH))
    ...
    // https://github.com/hnsvn/hns-browser/issues/29262
  ...
  accountsByNetwork
    .filter(account => account.accountId.coin === HnsWallet.CoinType.ETH)
  ...
  const { data: fvmTranslatedAddresses } = useGetFVMAddressQuery(
    ...
    ? {
        ...
        isMainNet: selectedNetwork.chainId === HnsWallet.FILECOIN_MAINNET
      }
    : skipToken
  )
