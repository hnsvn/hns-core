...
export const Send = (props: Props) => {
  ...
  const reviewButtonText = React.useMemo(() => {
    return showEnsOffchainWarning
      ? getLocale('hnsWalletEnsOffChainButton')
      : searchingForDomain
        ? getLocale('hnsWalletSearchingForDomain')
        : sendAmountValidationError
          ? getLocale('hnsWalletDecimalPlacesError')
          : insufficientFundsError
            ? getLocale('hnsWalletNotEnoughFunds')
            : (
              ...
              : (
                ...
                addressWarning !== '' &&
                addressWarning !== getLocale('hnsWalletAddressMissingChecksumInfoWarning')
              )
                ? addressWarning
                : getLocale('hnsWalletReviewSend')
  ...
  const addressMessageInformation: AddressMessageInfo | undefined = React.useMemo(() => {
    ...
    if (addressError === getLocale('hnsWalletNotValidChecksumAddressError')) {
      return { ...FailedChecksumMessage, type: 'error' }
    }
    if (addressWarning === getLocale('hnsWalletAddressMissingChecksumInfoWarning')) {
  ...
  const showResolvedDomain = React.useMemo(() => {
    return (addressError === undefined ||
      addressError === '' ||
      addressError === getLocale('hnsWalletSameAddressError')) &&
      toAddress &&
  ...
  return (
    <>
      <WalletPageWrapper
        ...
        cardHeader={
          <PageTitleHeader title={getLocale('hnsWalletSend')}/>
        }
      >
        <SendContainer>
                  ...
                  <Row>
                      ...
                      selectedSendOption={selectedSendOption} />
                    {
                      ...
                      <>
                        ...
                        />
                        <PresetButton buttonText={getLocale('hnsWalletSendHalf')} onClick={() => setPresetAmountValue(0.5)} />
                        <PresetButton buttonText={getLocale('hnsWalletSendMax')} onClick={() => setPresetAmountValue(1)} />
                      </>
                    }
                  </Row>
                  {selectedSendOption === SendPageTabHashes.token &&
          ...
          >
            ...
            >
              ...
              <AddressInput
                placeholder={getLocale('hnsWalletEnterRecipientAddress')}
                hasError={hasAddressError}
