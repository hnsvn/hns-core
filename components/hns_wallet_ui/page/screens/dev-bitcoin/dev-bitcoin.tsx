...
const defaultNetworkId = HnsWallet.BITCOIN_TESTNET
const defaultKeyringId = HnsWallet.KeyringId.kBitcoin84Testnet

interface CreateAccountSectionProps {
  setAccountId: (accountId: HnsWallet.AccountId | undefined) => void
}
...
const CreateAccountSection = (props: CreateAccountSectionProps) => {
  const createBtcAccount = async () => {
    const {accountInfo} = await getAPIProxy().keyringService.addAccount(
      ...
      HnsWallet.KeyringId.kBitcoin84Testnet,
      'BTC Account'
    )
...
const AccountSection = (props: AccountSectionProps) => {
  ...
  const [accountInfo, setAccountInfo] =
    useState<HnsWallet.BitcoinAccountInfo | undefined>()
