...
// 1. change the domain to translate.hns.com;
// 2. adjust static paths to use hnsTranslateStaticPath.
const rewriteUrl = (url) => {
  try {
    ...
    if (new_url.pathname === '/translate_a/t') {
      ...
      // hnsTranslateStaticPath is predefined by translate_script.cc.
      new_url.pathname = new_url.pathname.replace('/translate_static/',
        hnsTranslateStaticPath);
    }
...
// in future versions.
const hnsExtraStyles = `.goog-te-spinner-pos, #goog-gt-tt {display: none;}`

// An overridden version of onLoadCSS from translate.js.
// The differences:
// 1. change url via rewriteUrl();
// 2. process the loaded styles via processCSS().
// 3. Add hnsExtraStyles in the end.
cr.googleTranslate.onLoadCSS = function (url) {
  ...
  xhr.onreadystatechange = function () {
    ...
    element.innerText = processCSS(this.responseText) + hnsExtraStyles;
