...
#include "hns/components/tor/tor_switches.h"
...
#if BUILDFLAG(IS_WIN)
const char kTorClientComponentName[] = "Hns Tor Client Updater (Windows)";
...
const char kTorClientComponentName[] = "Hns Tor Client Updater (Mac)";
...
const char kTorClientComponentName[] = "Hns Tor Client Updater (Linux)";
...
HnsTorClientUpdater::HnsTorClientUpdater(
    HnsComponent::Delegate* component_delegate,
    PrefService* local_state,
    const base::FilePath& user_data_dir)
    : HnsComponent(component_delegate),
      task_runner_(
...
HnsTorClientUpdater::~HnsTorClientUpdater() = default;

void HnsTorClientUpdater::Register() {
  ...
  HnsComponent::Register(kTorClientComponentName, kTorClientComponentId,
                           kTorClientComponentBase64PublicKey);
  registered_ = true;
}

void HnsTorClientUpdater::Unregister() {
  // We don't call HnsComponent::Unregister here in order to prevent tor
  // executable component from getting deleted when last tor window closed
  registered_ = false;
}

void HnsTorClientUpdater::Cleanup() {
...
void HnsTorClientUpdater::RemoveObsoleteFiles() {
  // tor log
...
void HnsTorClientUpdater::SetTorPath(
...
base::FilePath HnsTorClientUpdater::GetExecutablePath() const {
  return executable_path_;
}

base::FilePath HnsTorClientUpdater::GetTorrcPath() const {
  return torrc_path_;
}

base::FilePath HnsTorClientUpdater::GetTorDataPath() const {
  DCHECK(!user_data_dir_.empty());
  return user_data_dir_.Append(FILE_PATH_LITERAL("tor"))
      .Append(FILE_PATH_LITERAL("data"));
}

base::FilePath HnsTorClientUpdater::GetTorWatchPath() const {
  DCHECK(!user_data_dir_.empty());
  return user_data_dir_.Append(FILE_PATH_LITERAL("tor"))
      .Append(FILE_PATH_LITERAL("watch"));
}

void HnsTorClientUpdater::OnComponentReady(const std::string& component_id,
                                             const base::FilePath& install_dir,
                                             const std::string& manifest) {
  GetTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&InitTorPath, install_dir),
      base::BindOnce(&HnsTorClientUpdater::SetTorPath,
                     weak_ptr_factory_.GetWeakPtr()));
}

bool HnsTorClientUpdater::IsTorDisabled() {
  if (local_state_)
    return local_state_->GetBoolean(tor::prefs::kTorDisabled);
  return false;
}

void HnsTorClientUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void HnsTorClientUpdater::RemoveObserver(Observer* observer) {
