...
#if BUILDFLAG(IS_WIN)
constexpr const char kComponentName[] = "Hns Pluggable Transports (Windows)";
...
constexpr const char kComponentName[] = "Hns Pluggable Transports (Mac)";
constexpr const char kTorPluggableTransportComponentId[] =
...
constexpr const char kComponentName[] = "Hns Pluggable Transports (Linux)";
constexpr const char kTorPluggableTransportComponentId[] =
...
constexpr const char kSnowflakeExecutableName[] = "tor-snowflake-hns";
constexpr const char kObfs4ExecutableName[] = "tor-obfs4-hns";
...
HnsTorPluggableTransportUpdater::HnsTorPluggableTransportUpdater(
...
void HnsTorPluggableTransportUpdater::Register() {
  if (registered_)
    return;

  HnsComponent::Register(kComponentName, kTorPluggableTransportComponentId,
                           kComponentBase64PublicKey);
  registered_ = true;
  is_ready_ = false;
}

void HnsTorPluggableTransportUpdater::Unregister() {
  registered_ = false;
  is_ready_ = false;
}

void HnsTorPluggableTransportUpdater::Cleanup() {
...
bool HnsTorPluggableTransportUpdater::IsReady() const {
  return is_ready_;
}

const base::FilePath&
HnsTorPluggableTransportUpdater::GetSnowflakeExecutable() const {
  return snowflake_path_;
}

const base::FilePath& HnsTorPluggableTransportUpdater::GetObfs4Executable()
    const {
  return obfs4_path_;
}

void HnsTorPluggableTransportUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void HnsTorPluggableTransportUpdater::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

void HnsTorPluggableTransportUpdater::OnComponentReady(
    const std::string& component_id,
    const base::FilePath& install_dir,
    const std::string& manifest) {
  GetTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&Initialize, install_dir),
      base::BindOnce(&HnsTorPluggableTransportUpdater::OnInitialized,
                     weak_ptr_factory_.GetWeakPtr(), install_dir));
}

void HnsTorPluggableTransportUpdater::OnInitialized(
    const base::FilePath& install_dir,
    bool success) {
  if (success) {
