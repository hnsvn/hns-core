...
#include "hns/components/ai_chat/common/mojom/page_content_extractor.mojom-shared.h"
...
const char16_t kYoutubeTranscriptUrlExtractionScript[] =
    // TODO(petemill): Consider user's language
    uR"JS(
      (function() {
        const tracks = ytplayer?.config?.args?.raw_player_response?.captions?.playerCaptionsTracklistRenderer?.captionTracks
        if (!tracks.length) {
          return null
        }
        const langTracks = tracks.filter(track => track.languageCode === "en")
        if (langTracks.length) {
          const nonAutoGeneratedTrack = langTracks.find(track => track.kind !== 'asr')
          if (nonAutoGeneratedTrack) {
            return nonAutoGeneratedTrack.baseUrl
          }
          return langTracks[0].baseUrl
        }
        const nonAutoGeneratedTrack = tracks.find(track => track.kind !== 'asr')
        if (nonAutoGeneratedTrack) {
          return nonAutoGeneratedTrack.baseUrl
        }
