...
const GURL GetEndpointBaseUrl() {
  auto* endpoint = BUILDFLAG(HNS_AI_CHAT_ENDPOINT);
  ...
  if (strlen(endpoint) != 0) {
    ...
    LOG(ERROR) << "HNS_AI_CHAT_ENDPOINT was empty. Must supply an AI Chat"
...
void AIChatAPI::QueryPrompt(
  ...
  headers.emplace("x-hns-key", BUILDFLAG(HNS_SERVICES_KEY));
