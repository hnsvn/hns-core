...
#include "base/uuid.h"
#include "hns/components/hns_wallet/common/eth_request_helper.h"
...
constexpr char kHnsEthereum[] = "hnsEthereum";
constexpr char kEthereum[] = "ethereum";
constexpr char kEmit[] = "emit";
constexpr char kIsHnsWallet[] = "isHnsWallet";
...
void JSEthereumProvider::Install(bool install_ethereum_provider,
  ...
  // Check window.hnsEthereum existence.
  v8::Local<v8::Object> global = context->Global();
  v8::Local<v8::Value> hns_ethereum_value =
      global->Get(context, gin::StringToV8(isolate, kHnsEthereum))
          .ToLocalChecked();
  if (!hns_ethereum_value->IsUndefined()) {
    return;
  }
  ...
  // Set window.hnsEthereum
  SetProviderNonWritable(context, global, ethereum_proxy,
                         gin::StringToV8(isolate, kHnsEthereum), true);
  ...
  // https://github.com/hnsvn/hns-browser/issues/22213;
  SetOwnPropertyWritable(context, provider_object,
...
bool JSEthereumProvider::GetIsHnsWallet() {
  return true;
}
...
gin::ObjectTemplateBuilder JSEthereumProvider::GetObjectTemplateBuilder(
  ...
  return gin::Wrappable<JSEthereumProvider>::GetObjectTemplateBuilder(isolate)
      .SetProperty(kIsHnsWallet, &JSEthereumProvider::GetIsHnsWallet)
...
void JSEthereumProvider::FireEvent(const std::string& event,
  ...
  CallMethodOfObject(render_frame()->GetWebFrame(), kHnsEthereum, kEmit,
                     std::move(args));
}
...
void JSEthereumProvider::AnnounceProvider() {
  ...
  auto provider =
      GetProperty(context, context->Global(), kHnsEthereum).ToLocalChecked();
...
const std::string& JSEthereumProvider::GetHnsWalletImage() {
  if (!hns_wallet_image_) {
    hns_wallet_image_ =
        LoadImageResourceAsDataUrl(IDR_HNS_WALLET_PROVIDER_ICON);
  }
  return hns_wallet_image_.value();
}
