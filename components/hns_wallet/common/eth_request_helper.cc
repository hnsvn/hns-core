...
#include "hns/components/hns_wallet/common/json_rpc_requests.h"
...
absl::optional<base::Value::List> GetParamsList(const std::string& json) {
  ...
  auto* params = value.FindListByDottedPath(hns_wallet::kParams);
  if (!params) {
    return absl::nullopt;
  }
...
absl::optional<base::Value::Dict> GetParamsDict(const std::string& json) {
  ...
  auto* value = json_value->GetDict().FindDict(hns_wallet::kParams);
  if (!value) {
    return absl::nullopt;
  }
...
hns_wallet::mojom::TxDataPtr ValueToTxData(
    const hns_wallet::json_rpc_requests::Transaction& tx,
    std::string* from_out) {
  CHECK(from_out);
  ...
  *from_out = tx.from;
  ...
  if (tx.data && !tx.data->empty() &&
      !hns_wallet::PrefixedHexStringToBytes(*tx.data, &bytes)) {
    return nullptr;
  }
  tx_data->data = bytes;
...
mojom::TxDataPtr ParseEthTransactionParams(const std::string& json,
  ...
  auto tx = hns_wallet::json_rpc_requests::Transaction::FromValue(*param_obj);
  if (!tx) {
    return nullptr;
  }
...
bool ShouldCreate1559Tx(hns_wallet::mojom::TxData1559Ptr tx_data_1559,
