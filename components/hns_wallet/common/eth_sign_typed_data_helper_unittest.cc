...
TEST(EthSignedTypedDataHelperUnitTest, EncodedData) {
  ...
  EXPECT_FALSE(helper->EncodeData("Hns", data_dict));
...
TEST(EthSignedTypedDataHelperUnitTest, EncodeField) {
  ...
  // string
  {
    auto encoded_field = helper->EncodeField("string", base::Value("hns"));
    ASSERT_TRUE(encoded_field);
    EXPECT_EQ(
        base::ToLowerASCII(base::HexEncode(*encoded_field)),
  ...
  {  // custom type but not dictionary
    EXPECT_FALSE(helper->EncodeField("Hns123", base::Value(1)));
    EXPECT_FALSE(helper->EncodeField("Hns123", base::Value("123")));
    EXPECT_FALSE(helper->EncodeField("Hns123", base::Value(true)));
    EXPECT_FALSE(helper->EncodeField("Hns123", list));
  }
}
