...
#include "hns/components/hns_wallet/browser/eth_allowance_manager.h"
...
#include "hns/components/hns_wallet/common/hns_wallet_response_helpers.h"
#include "hns/components/hns_wallet/common/hns_wallet_types.h"
#include "hns/components/hns_wallet/common/common_utils.h"
#include "hns/components/hns_wallet/common/eth_address.h"
#include "hns/components/hns_wallet/common/fil_address.h"
#include "hns/components/hns_wallet/common/hex_utils.h"
#include "hns/components/hns_wallet/common/solana_utils.h"
...
base::Value::Dict GetEthNativeAssetFromChain(
    const hns_wallet::mojom::NetworkInfoPtr& chain) {
...
bool ShouldCheckTokenId(const hns_wallet::mojom::BlockchainTokenPtr& token) {
...
HnsWalletService::HnsWalletService(
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory,
    std::unique_ptr<HnsWalletServiceDelegate> delegate,
    ...
    : delegate_(std::move(delegate)),
      ...
      hns_wallet_p3a_(this, keyring_service, profile_prefs, local_state),
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&HnsWalletService::OnWalletUnlockPreferenceChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultEthereumWallet,
      base::BindRepeating(&HnsWalletService::OnDefaultEthereumWalletChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultSolanaWallet,
      base::BindRepeating(&HnsWalletService::OnDefaultSolanaWalletChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultBaseCurrency,
      base::BindRepeating(&HnsWalletService::OnDefaultBaseCurrencyChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultBaseCryptocurrency,
      base::BindRepeating(
          &HnsWalletService::OnDefaultBaseCryptocurrencyChanged,
          base::Unretained(this)));
  pref_change_registrar_.Add(
      kHnsWalletCustomNetworks,
      base::BindRepeating(&HnsWalletService::OnNetworkListChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kHnsWalletHiddenNetworks,
      base::BindRepeating(&HnsWalletService::OnNetworkListChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kHnsWalletNftDiscoveryEnabled,
      base::BindRepeating(&HnsWalletService::OnHnsWalletNftDiscoveryEnabled,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kHnsWalletSelectedNetworks,
      base::BindRepeating(&HnsWalletService::OnNetworkChanged,
                          weak_ptr_factory_.GetWeakPtr()));
}

HnsWalletService::HnsWalletService() : weak_ptr_factory_(this) {}

HnsWalletService::~HnsWalletService() = default;

mojo::PendingRemote<mojom::HnsWalletService>
HnsWalletService::MakeRemote() {
  mojo::PendingRemote<mojom::HnsWalletService> remote;
  receivers_.Add(this, remote.InitWithNewPipeAndPassReceiver());
  return remote;
}

// For unit tests
void HnsWalletService::RemovePrefListenersForTests() {
  pref_change_registrar_.RemoveAll();
}

void HnsWalletService::Bind(
...
// static
absl::optional<std::string> HnsWalletService::GetUserAssetAddress(
    const std::string& address,
    mojom::CoinType coin,
    const std::string& chain_id) {
  ...
  if (coin == mojom::CoinType::SOL) {
    ...
    if (!::hns_wallet::IsBase58EncodedSolanaPubkey(address)) {
      return absl::nullopt;
    }
    return address;
  }
...
// static
absl::optional<std::string> HnsWalletService::GetChecksumAddress(
...
// static
std::vector<mojom::BlockchainTokenPtr> HnsWalletService::GetUserAssets(
    PrefService* profile_prefs) {
  std::vector<mojom::BlockchainTokenPtr> result;
  const auto& user_assets_dict = profile_prefs->GetDict(kHnsWalletUserAssets);
...
// static
bool HnsWalletService::AddUserAsset(mojom::BlockchainTokenPtr token,
                                      bool visible,
                                      PrefService* profile_prefs) {
  ...
  ScopedDictPrefUpdate update(profile_prefs, kHnsWalletUserAssets);
...
void HnsWalletService::GetUserAssets(const std::string& chain_id,
                                       mojom::CoinType coin,
...
void HnsWalletService::GetAllUserAssets(GetUserAssetsCallback callback) {
...
bool HnsWalletService::AddUserAsset(mojom::BlockchainTokenPtr token,
  ...
  bool result = HnsWalletService::AddUserAsset(std::move(token), visible,
                                                 profile_prefs_);
...
void HnsWalletService::AddUserAsset(mojom::BlockchainTokenPtr token,
  ...
  if (token->is_nft && token->coin == mojom::CoinType::ETH) {
    ...
    json_rpc_service_->GetEthNftStandard(
        ...
        base::BindOnce(&HnsWalletService::OnGetEthNftStandard,
...
void HnsWalletService::OnGetEthNftStandard(
...
void HnsWalletService::RemoveUserAsset(mojom::BlockchainTokenPtr token,
                                         RemoveUserAssetCallback callback) {
  std::move(callback).Run(RemoveUserAsset(std::move(token)));
}

bool HnsWalletService::RemoveUserAsset(mojom::BlockchainTokenPtr token) {
  absl::optional<std::string> address = GetUserAssetAddress(
      token->contract_address, token->coin, token->chain_id);
  if (!address) {
    return false;
  ...
  ScopedDictPrefUpdate update(profile_prefs_, kHnsWalletUserAssets);
...
void HnsWalletService::SetUserAssetVisible(
    mojom::BlockchainTokenPtr token,
    bool visible,
...
bool HnsWalletService::SetUserAssetVisible(mojom::BlockchainTokenPtr token,
                                             bool visible) {
  DCHECK(token);
...
void HnsWalletService::SetAssetSpamStatus(
    mojom::BlockchainTokenPtr token,
    bool is_spam,
...
bool HnsWalletService::SetAssetSpamStatus(mojom::BlockchainTokenPtr token,
                                            bool is_spam) {
  DCHECK(token);
...
mojom::BlockchainTokenPtr HnsWalletService::GetUserAsset(
  ...
  const auto& user_assets_dict =
      profile_prefs_->GetDict(kHnsWalletUserAssets);
...
void HnsWalletService::IsExternalWalletInstalled(
...
void HnsWalletService::ImportFromExternalWallet(
  ...
  delegate_->GetImportInfoFromExternalWallet(
      type, password,
      base::BindOnce(&HnsWalletService::OnGetImportInfo,
                     weak_ptr_factory_.GetWeakPtr(), new_password,
                     std::move(callback)));
}

void HnsWalletService::GetDefaultEthereumWallet(
    GetDefaultEthereumWalletCallback callback) {
  std::move(callback).Run(
      ::hns_wallet::GetDefaultEthereumWallet(profile_prefs_));
}

void HnsWalletService::GetDefaultSolanaWallet(
    GetDefaultSolanaWalletCallback callback) {
  std::move(callback).Run(
      ::hns_wallet::GetDefaultSolanaWallet(profile_prefs_));
}

void HnsWalletService::SetDefaultEthereumWallet(
    mojom::DefaultWallet default_wallet) {
  auto old_default_wallet =
      ::hns_wallet::GetDefaultEthereumWallet(profile_prefs_);
  if (old_default_wallet != default_wallet) {
    ::hns_wallet::SetDefaultEthereumWallet(profile_prefs_, default_wallet);
  }
}

void HnsWalletService::SetDefaultSolanaWallet(
    mojom::DefaultWallet default_wallet) {
  auto old_default_wallet =
      ::hns_wallet::GetDefaultSolanaWallet(profile_prefs_);
  if (old_default_wallet != default_wallet) {
    ::hns_wallet::SetDefaultSolanaWallet(profile_prefs_, default_wallet);
  }
}

void HnsWalletService::GetDefaultBaseCurrency(
    GetDefaultBaseCurrencyCallback callback) {
  std::move(callback).Run(
      ::hns_wallet::GetDefaultBaseCurrency(profile_prefs_));
}

void HnsWalletService::SetDefaultBaseCurrency(const std::string& currency) {
  auto old_default_currency =
      ::hns_wallet::GetDefaultBaseCurrency(profile_prefs_);
  if (old_default_currency != currency) {
    ::hns_wallet::SetDefaultBaseCurrency(profile_prefs_, currency);
  }
}

void HnsWalletService::GetDefaultBaseCryptocurrency(
    GetDefaultBaseCryptocurrencyCallback callback) {
  std::move(callback).Run(
      ::hns_wallet::GetDefaultBaseCryptocurrency(profile_prefs_));
}

void HnsWalletService::SetDefaultBaseCryptocurrency(
    const std::string& cryptocurrency) {
  auto old_default_cryptocurrency =
      ::hns_wallet::GetDefaultBaseCryptocurrency(profile_prefs_);
  if (old_default_cryptocurrency != cryptocurrency) {
    ::hns_wallet::SetDefaultBaseCryptocurrency(profile_prefs_,
                                                 cryptocurrency);
  }
}

void HnsWalletService::EnsureSelectedAccountForChain(
    mojom::CoinType coin,
    const std::string& chain_id,
...
mojom::AccountIdPtr HnsWalletService::EnsureSelectedAccountForChainSync(
    mojom::CoinType coin,
    const std::string& chain_id) {
...
void HnsWalletService::GetNetworkForSelectedAccountOnActiveOrigin(
...
HnsWalletService::GetNetworkForSelectedAccountOnActiveOriginSync() {
...
void HnsWalletService::SetNetworkForSelectedAccountOnActiveOrigin(
    const std::string& chain_id,
...
bool HnsWalletService::HasPendingDecryptRequestForOrigin(
...
bool HnsWalletService::HasPendingGetEncryptionPublicKeyRequestForOrigin(
    const url::Origin& origin) const {
...
void HnsWalletService::OnDefaultEthereumWalletChanged() {
  auto default_wallet =
      ::hns_wallet::GetDefaultEthereumWallet(profile_prefs_);
...
void HnsWalletService::OnDefaultSolanaWalletChanged() {
  auto default_wallet = ::hns_wallet::GetDefaultSolanaWallet(profile_prefs_);
  for (const auto& observer : observers_) {
    observer->OnDefaultSolanaWalletChanged(default_wallet);
  }
}

void HnsWalletService::OnDefaultBaseCurrencyChanged() {
  auto value = ::hns_wallet::GetDefaultBaseCurrency(profile_prefs_);
  for (const auto& observer : observers_) {
    observer->OnDefaultBaseCurrencyChanged(value);
  }
}

void HnsWalletService::OnDefaultBaseCryptocurrencyChanged() {
  auto value = ::hns_wallet::GetDefaultBaseCryptocurrency(profile_prefs_);
  for (const auto& observer : observers_) {
    observer->OnDefaultBaseCryptocurrencyChanged(value);
  }
}

void HnsWalletService::OnNetworkListChanged() {
  for (const auto& observer : observers_) {
    observer->OnNetworkListChanged();
  }
}

void HnsWalletService::OnHnsWalletNftDiscoveryEnabled() {
  if (profile_prefs_->GetBoolean(kHnsWalletNftDiscoveryEnabled)) {
    DiscoverAssetsOnAllSupportedChains(true);
  }
}

void HnsWalletService::AddPermission(mojom::AccountIdPtr account_id,
...
void HnsWalletService::HasPermission(
...
void HnsWalletService::ResetPermission(mojom::AccountIdPtr account_id,
...
void HnsWalletService::IsPermissionDenied(
    mojom::CoinType coin,
...
// static
void HnsWalletService::MigrateUserAssetEthContractAddress(
    PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletUserAssetEthContractAddressMigrated)) {
    return;
  }

  if (!prefs->HasPrefPath(kHnsWalletUserAssetsDeprecated)) {
    prefs->SetBoolean(kHnsWalletUserAssetEthContractAddressMigrated, true);
    return;
  }

  ScopedDictPrefUpdate update(prefs, kHnsWalletUserAssetsDeprecated);
...
// static
void HnsWalletService::MigrateMultichainUserAssets(PrefService* prefs) {
  if (!prefs->HasPrefPath(kHnsWalletUserAssetsDeprecated)) {
    return;
  }

  auto eth_user_assets =
      prefs->GetDict(kHnsWalletUserAssetsDeprecated).Clone();
  ...
  prefs->Set(kHnsWalletUserAssets, base::Value(std::move(new_user_assets)));
  prefs->ClearPref(kHnsWalletUserAssetsDeprecated);
}

// static
void HnsWalletService::MigrateUserAssetsAddPreloadingNetworks(
    PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletUserAssetsAddPreloadingNetworksMigrated)) {
    return;
  }

  if (!prefs->HasPrefPath(kHnsWalletUserAssets)) {
    prefs->SetBoolean(kHnsWalletUserAssetsAddPreloadingNetworksMigrated,
                      true);
    return;
  }

  ScopedDictPrefUpdate update(prefs, kHnsWalletUserAssets);
  auto& user_assets_pref = update.Get();
  ...
  prefs->SetBoolean(kHnsWalletUserAssetsAddPreloadingNetworksMigrated, true);
...
// static
void HnsWalletService::MigrateUserAssetsAddIsNFT(PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletUserAssetsAddIsNFTMigrated)) {
    return;
  }

  if (!prefs->HasPrefPath(kHnsWalletUserAssets)) {
    prefs->SetBoolean(kHnsWalletUserAssetsAddIsNFTMigrated, true);
    return;
  }
...
// static
void HnsWalletService::MigrateHiddenNetworks(PrefService* prefs) {
  auto previous_version_code =
      prefs->GetInteger(kHnsWalletDefaultHiddenNetworksVersion);
  if (previous_version_code >= 1) {
    return;
  }
  {
    // Default hidden networks
    ScopedDictPrefUpdate update(prefs, kHnsWalletHiddenNetworks);
  ...
  prefs->SetInteger(kHnsWalletDefaultHiddenNetworksVersion, 1);
}

void HnsWalletService::MigrateUserAssetsAddIsERC1155(PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletUserAssetsAddIsERC1155Migrated)) {
    return;
  }

  if (!prefs->HasPrefPath(kHnsWalletUserAssets)) {
    prefs->SetBoolean(kHnsWalletUserAssetsAddIsERC1155Migrated, true);
    return;
  }
...
void HnsWalletService::MigrateUserAssetsAddIsSpam(PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletUserAssetsAddIsSpamMigrated)) {
    return;
  }

  if (!prefs->HasPrefPath(kHnsWalletUserAssets)) {
    prefs->SetBoolean(kHnsWalletUserAssetsAddIsSpamMigrated, true);
    return;
  }
...
bool ShouldMigrateRemovedPreloadedNetwork(PrefService* prefs,
  ...
  const auto& selected_networks =
      prefs->GetDict(kHnsWalletSelectedNetworksPerOrigin);
...
void HnsWalletService::MigrateFantomMainnetAsCustomNetwork(
    PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletCustomNetworksFantomMainnetMigrated)) {
    return;
  }
  ...
  prefs->SetBoolean(kHnsWalletCustomNetworksFantomMainnetMigrated, true);
}

// static
base::Value::Dict HnsWalletService::GetDefaultEthereumAssets() {
...
// static
base::Value::Dict HnsWalletService::GetDefaultSolanaAssets() {
  base::Value::Dict user_assets;
...
// static
base::Value::Dict HnsWalletService::GetDefaultFilecoinAssets() {
  base::Value::Dict user_assets;
...
// static
base::Value::Dict HnsWalletService::GetDefaultBitcoinAssets() {
  base::Value::Dict user_assets;
...
void HnsWalletService::OnWalletUnlockPreferenceChanged(
    const std::string& pref_name) {
  hns_wallet_p3a_.ReportUsage(true);
}

HnsWalletP3A* HnsWalletService::GetHnsWalletP3A() {
  return &hns_wallet_p3a_;
}

void HnsWalletService::GetActiveOrigin(GetActiveOriginCallback callback) {
  std::move(callback).Run(GetActiveOriginSync());
}

mojom::OriginInfoPtr HnsWalletService::GetActiveOriginSync() {
  return MakeOriginInfo(delegate_->GetActiveOrigin().value_or(url::Origin()));
}

void HnsWalletService::GetPendingSignMessageRequests(
...
void HnsWalletService::NotifySignMessageRequestProcessed(
    bool approved,
    int id,
...
void HnsWalletService::GetPendingSignTransactionRequests(
...
void HnsWalletService::NotifySignTransactionRequestProcessed(
    bool approved,
    int id,
    mojom::ByteArrayStringUnionPtr signature,
    const absl::optional<std::string>& error) {
...
void HnsWalletService::GetPendingSignAllTransactionsRequests(
...
void HnsWalletService::NotifySignAllTransactionsRequestProcessed(
    bool approved,
    int id,
...
void HnsWalletService::AddObserver(
    ::mojo::PendingRemote<mojom::HnsWalletServiceObserver> observer) {
  observers_.Add(std::move(observer));
}

void HnsWalletService::AddTokenObserver(
    ::mojo::PendingRemote<mojom::HnsWalletServiceTokenObserver> observer) {
  token_observers_.Add(std::move(observer));
}

void HnsWalletService::OnActiveOriginChanged(
...
void HnsWalletService::OnDiscoverAssetsStarted() {
  for (const auto& observer : observers_) {
    observer->OnDiscoverAssetsStarted();
  }
}

void HnsWalletService::OnDiscoverAssetsCompleted(
...
void HnsWalletService::OnGetImportInfo(
    const std::string& new_password,
    ...
    switch (error) {
      case ImportError::kJsonError:
        std::move(callback).Run(false, l10n_util::GetStringUTF8(
                                           IDS_HNS_WALLET_IMPORT_JSON_ERROR));
        break;
      case ImportError::kPasswordError:
        std::move(callback).Run(
            false,
            l10n_util::GetStringUTF8(IDS_HNS_WALLET_IMPORT_PASSWORD_ERROR));
        break;
      case ImportError::kInternalError:
        std::move(callback).Run(
            false,
            l10n_util::GetStringUTF8(IDS_HNS_WALLET_IMPORT_INTERNAL_ERROR));
        break;
...
void HnsWalletService::AddSignMessageRequest(
...
void HnsWalletService::AddSignTransactionRequest(
...
void HnsWalletService::AddSignAllTransactionsRequest(
...
void HnsWalletService::AddSuggestTokenRequest(
...
void HnsWalletService::AddGetPublicKeyRequest(
...
void HnsWalletService::AddDecryptRequest(
    const mojom::AccountIdPtr& account_id,
    const url::Origin& origin,
...
void HnsWalletService::GetPendingGetEncryptionPublicKeyRequests(
...
void HnsWalletService::GetPendingAddSuggestTokenRequests(
...
void HnsWalletService::GetPendingDecryptRequests(
...
void HnsWalletService::NotifyAddSuggestTokenRequestsProcessed(
    bool approved,
...
void HnsWalletService::NotifyGetPublicKeyRequestProcessed(
...
void HnsWalletService::NotifyDecryptRequestProcessed(
    const std::string& request_id,
    bool approved) {
...
void HnsWalletService::IsBase58EncodedSolanaPubkey(
    const std::string& key,
    IsBase58EncodedSolanaPubkeyCallback callback) {
  std::move(callback).Run(::hns_wallet::IsBase58EncodedSolanaPubkey(key));
}

void HnsWalletService::Base58Encode(
  ...
  for (const auto& address : addresses) {
    encoded_addresses.push_back(hns_wallet::Base58Encode(address));
  }
  std::move(callback).Run(std::move(encoded_addresses));
}

void HnsWalletService::DiscoverAssetsOnAllSupportedChains() {
...
void HnsWalletService::DiscoverAssetsOnAllSupportedChains(
  ...
  keyring_info = keyring_service_->GetKeyringInfoSync(
      hns_wallet::mojom::kSolanaKeyringId);
...
void HnsWalletService::GetNftDiscoveryEnabled(
    GetNftDiscoveryEnabledCallback callback) {
  std::move(callback).Run(
      profile_prefs_->GetBoolean(kHnsWalletNftDiscoveryEnabled));
}

void HnsWalletService::SetNftDiscoveryEnabled(bool enabled) {
  profile_prefs_->SetBoolean(kHnsWalletNftDiscoveryEnabled, enabled);
}

void HnsWalletService::GetBalanceScannerSupportedChains(
...
void HnsWalletService::ConvertFEVMToFVMAddress(
    bool is_mainnet,
...
void HnsWalletService::GetSimpleHashSpamNFTs(
...
void HnsWalletService::CancelAllSuggestedTokenCallbacks() {
...
void HnsWalletService::CancelAllSignMessageCallbacks() {
...
void HnsWalletService::CancelAllSignTransactionCallbacks() {
...
void HnsWalletService::CancelAllSignAllTransactionsCallbacks() {
...
void HnsWalletService::CancelAllGetEncryptionPublicKeyCallbacks() {
  base::Value formed_response = GetProviderErrorDictionary(
      mojom::ProviderError::kUserRejectedRequest,
      l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
...
void HnsWalletService::CancelAllDecryptCallbacks() {
  base::Value formed_response = GetProviderErrorDictionary(
      mojom::ProviderError::kUserRejectedRequest,
      l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
...
void HnsWalletService::OnNetworkChanged() {
  CancelAllSuggestedTokenCallbacks();
}

void HnsWalletService::Reset() {
  ...
  // Clear HnsWalletService
  ClearHnsWalletServicePrefs(profile_prefs_);
  CancelAllSuggestedTokenCallbacks();
...
void HnsWalletService::DiscoverEthAllowances(
