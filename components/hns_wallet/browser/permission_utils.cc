...
void ExtractAddresses(permissions::RequestType type,
  ...
  std::string match;
  re2::RE2* regex;
  if (type == permissions::RequestType::kHnsEthereum) {
...
bool ParseRequestingOriginInternal(permissions::RequestType type,
                                   ...
                                   std::string* account,
                                   std::queue<std::string>* address_queue) {
  if (origin.opaque() || (type != permissions::RequestType::kHnsEthereum &&
                          type != permissions::RequestType::kHnsSolana)) {
    return false;
  }
...
absl::optional<blink::PermissionType> CoinTypeToPermissionType(
  ...
  switch (coin_type) {
    case mojom::CoinType::ETH:
      return blink::PermissionType::HNS_ETHEREUM;
    case mojom::CoinType::SOL:
      return blink::PermissionType::HNS_SOLANA;
    default:
      return absl::nullopt;
  }
}
