...
bool Transfer(const std::string& to_address,
  ...
  if (!hns_wallet::PadHexEncodedParameter(to_address, &padded_address)) {
    return false;
  }
...
bool BalanceOf(const std::string& address, std::string* data) {
  ...
  std::string params;
  if (!hns_wallet::PadHexEncodedParameter(address, &params)) {
    return false;
  }
  return hns_wallet::ConcatHexStrings(function_hash, params, data);
}
...
bool Approve(const std::string& spender_address,
  ...
  std::string padded_address;
  if (!hns_wallet::PadHexEncodedParameter(spender_address, &padded_address)) {
    return false;
  }
  std::string padded_amount;
  if (!PadHexEncodedParameter(Uint256ValueToHex(amount), &padded_amount)) {
...
bool Allowance(const std::string& owner_address,
  ...
  if (!hns_wallet::PadHexEncodedParameter(owner_address,
                                            &padded_owner_address)) {
    return false;
  }
  std::string padded_spender_address;
  if (!hns_wallet::PadHexEncodedParameter(spender_address,
...
bool TransferFromOrSafeTransferFrom(bool is_safe_transfer_from,
  ...
  if (!hns_wallet::PadHexEncodedParameter(from, &padded_from)) {
    return false;
  }

  std::string padded_to;
  if (!hns_wallet::PadHexEncodedParameter(to, &padded_to)) {
    return false;
  }
...
bool OwnerOf(uint256_t token_id, std::string* data) {
  ...
  return hns_wallet::ConcatHexStrings(function_hash, padded_token_id, data);
}
...
bool BalanceOf(const std::string& owner_address,
  ...
  std::string padded_address;
  if (!hns_wallet::PadHexEncodedParameter(owner_address, &padded_address)) {
    return false;
  }
  std::string padded_token_id;
  if (!PadHexEncodedParameter(Uint256ValueToHex(token_id), &padded_token_id)) {
...
absl::optional<std::string> TokensBalance(
  ...
  for (const auto& contract_address : contract_addresses) {
    ...
    if (!hns_wallet::PadHexEncodedParameter(contract_address,
