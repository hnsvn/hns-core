...
TEST_F(TxStateManagerUnitTest, GetTransactionsByStatus) {
  prefs_.ClearPref(kHnsWalletTransactions);
...
TEST_F(TxStateManagerUnitTest, MultiChainId) {
  ...
  auto localhost_url_spec =
      hns_wallet::GetNetworkURL(&prefs_, mojom::kLocalhostChainId,
                                  mojom::CoinType::ETH)
          .spec();
...
TEST_F(TxStateManagerUnitTest,
  ...
  ASSERT_FALSE(
      prefs_.GetBoolean(kHnsWalletSolanaTransactionsV0SupportMigrated));
  ...
  prefs_.Set(kHnsWalletTransactions, txs_value);
  ...
  EXPECT_EQ(*prefs_.GetDict(kHnsWalletTransactions)
