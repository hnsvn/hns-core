...
#include "hns/components/hns_wallet/browser/bitcoin/bitcoin_database_synchronizer.h"
#include "hns/components/hns_wallet/browser/bitcoin/bitcoin_transaction_database.h"
#include "hns/components/hns_wallet/browser/keyring_service.h"
#include "hns/components/hns_wallet/common/bitcoin_utils.h"
...
std::vector<hns_wallet::mojom::KeyringId> BitcoinKeyringsForNetwork(
    const std::string& network_id) {
  DCHECK(hns_wallet::IsBitcoinNetwork(network_id));
  if (network_id == hns_wallet::mojom::kBitcoinMainnet) {
    return {hns_wallet::mojom::KeyringId::kBitcoin84};
  } else if (network_id == hns_wallet::mojom::kBitcoinTestnet) {
    return {hns_wallet::mojom::KeyringId::kBitcoin84Testnet};
  } else {
    NOTREACHED();
  }
...
void PushOutpoint(const hns_wallet::bitcoin::Outpoint& outpoint,
                  std::vector<uint8_t>& to) {
