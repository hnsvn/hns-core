...
TEST(FilResponseParserUnitTest, ParseFilEstimateGas) {
  ...
  int64_t gas_limit = 0;
  EXPECT_TRUE(hns_wallet::ParseFilEstimateGas(ParseJson(json), &gas_premium,
  ...
  gas_limit = 0;
  json.clear();
  EXPECT_FALSE(hns_wallet::ParseFilEstimateGas(base::Value(), &gas_premium,
                                                 &gas_fee_cap, &gas_limit));
  ...
  json = "[]";
  EXPECT_FALSE(hns_wallet::ParseFilEstimateGas(ParseJson(json), &gas_premium,
                                                 &gas_fee_cap, &gas_limit));
...
TEST(FilResponseParserUnitTest, ParseFilGetChainHead) {
  uint64_t height = 0;
  EXPECT_TRUE(hns_wallet::ParseFilGetChainHead(GetResponse(R"({
        "Blocks":[],
        "Cids": [{
  ...
  EXPECT_FALSE(hns_wallet::ParseFilGetChainHead(base::Value(), &height));
  EXPECT_FALSE(
      hns_wallet::ParseFilGetChainHead(GetResponse(R"({})"), &height));
  EXPECT_FALSE(hns_wallet::ParseFilGetChainHead(
...
TEST(FilResponseParserUnitTest, ParseFilStateSearchMsgLimited) {
  int64_t exit_code = -1;
  EXPECT_TRUE(hns_wallet::ParseFilStateSearchMsgLimited(
      GetResponse(
  ...
  EXPECT_FALSE(hns_wallet::ParseFilStateSearchMsgLimited(
      GetResponse(
          "{\"Message\": {\"/\":\"cid\"},\"Receipt\": {\"ExitCode\":\"" +
          std::to_string(INT64_MIN) + "\"}}"),
      "anothercid", &exit_code));

  EXPECT_FALSE(hns_wallet::ParseFilStateSearchMsgLimited(base::Value(), "cid",
                                                           &exit_code));
  EXPECT_FALSE(hns_wallet::ParseFilStateSearchMsgLimited(ParseJson("{}"),
                                                           "cid", &exit_code));
  EXPECT_FALSE(hns_wallet::ParseFilStateSearchMsgLimited(
  ...
  EXPECT_FALSE(hns_wallet::ParseFilStateSearchMsgLimited(
      GetResponse(R"({"Message": {"/":"cid"},"Receipt": {"ExitCode": {}}})"),
      "cid", &exit_code));
  EXPECT_FALSE(hns_wallet::ParseFilStateSearchMsgLimited(GetResponse("null"),
                                                           "cid", &exit_code));
}

TEST(FilResponseParserUnitTest, ParseSendFilecoinTransaction) {
  std::string cid;
  EXPECT_TRUE(hns_wallet::ParseSendFilecoinTransaction(GetResponse(R"({
  ...
  cid.clear();
  EXPECT_FALSE(
      hns_wallet::ParseSendFilecoinTransaction(GetResponse(R"({})"), &cid));
  EXPECT_TRUE(cid.empty());

  EXPECT_FALSE(
      hns_wallet::ParseSendFilecoinTransaction(GetResponse("1"), &cid));
  EXPECT_TRUE(cid.empty());
  EXPECT_FALSE(
      hns_wallet::ParseSendFilecoinTransaction(GetResponse("[]"), &cid));
  EXPECT_TRUE(cid.empty());
}
