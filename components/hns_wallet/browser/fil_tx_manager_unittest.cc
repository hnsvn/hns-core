...
#include "hns/components/hns_wallet/browser/fil_tx_manager.h"
#include <memory>
...
#include "hns/components/hns_wallet/browser/fil_tx_meta.h"
...
class FilTxManagerUnitTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    keyring_service_->CreateWallet(kMnemonicDivideCruise, "hns",
                                   base::DoNothing());
  ...
  fil_tx_manager()->ProcessFilHardwareSignature(
      ...
      base::BindLambdaForTesting([&](bool success,
        ...
        EXPECT_EQ(err_message, l10n_util::GetStringUTF8(
                                   IDS_HNS_WALLET_TRANSACTION_NOT_FOUND));
        run_loop.Quit();
      }));
  run_loop.Run();
}
