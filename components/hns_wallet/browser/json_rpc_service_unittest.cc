...
#include "hns/components/hns_wallet/browser/ens_resolver_task.h"
...
#include "hns/components/hns_wallet/browser/sns_resolver_task.h"
#include "hns/components/hns_wallet/browser/unstoppable_domains_dns_resolve.h"
...
void OnRequestResponse(bool* callback_called,
  ...
  *callback_called = true;
  ...
  bool success = error == hns_wallet::mojom::ProviderError::kSuccess;
...
void OnStringResponse(bool* callback_called,
                      hns_wallet::mojom::ProviderError expected_error,
...
void OnFilUint256Response(
    ...
    hns_wallet::mojom::FilecoinProviderError expected_error,
    const std::string& expected_error_message,
    uint256_t expected_response,
    uint256_t response,
    hns_wallet::mojom::FilecoinProviderError error,
    const std::string& error_message) {
  *callback_called = true;
  EXPECT_EQ(expected_response, response);
  EXPECT_EQ(expected_error, error);
...
class JsonRpcServiceUnitTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    url_loader_factory_.SetInterceptor(base::BindLambdaForTesting(
        [&](const network::ResourceRequest& request) {
          ...
          url_loader_factory_.AddResponse(
              hns_wallet::GetNetworkURL(prefs(), mojom::kLocalhostChainId,
                                          mojom::CoinType::ETH)
                  .spec(),
    ...
    hns_wallet::RegisterProfilePrefs(prefs_.registry());
    hns_wallet::RegisterProfilePrefsForMigration(prefs_.registry());
  ...
  GURL GetNetwork(const std::string& chain_id, mojom::CoinType coin) {
    return hns_wallet::GetNetworkURL(prefs(), chain_id, coin);
  }
  ...
  bool GetIsEip1559FromPrefs(const std::string& chain_id) {
    ...
    const base::Value* custom_networks =
        prefs()->GetDict(kHnsWalletCustomNetworks).Find(kEthereumPrefKey);
    if (!custom_networks) {
      return false;
    }
    ...
    url_loader_factory_.SetInterceptor(base::BindLambdaForTesting(
        [&, expected_url, expected_method, expected_cache_header,
         content](const network::ResourceRequest& request) {
          ...
          EXPECT_TRUE(request.headers.GetHeader("x-hns-key", &header_value));
          EXPECT_EQ(BUILDFLAG(HNS_SERVICES_KEY), header_value);
          url_loader_factory_.ClearResponses();
...
TEST_F(JsonRpcServiceUnitTest, SetNetwork) {
  ...
  for (const auto& network :
       hns_wallet::GetAllKnownChains(prefs(), mojom::CoinType::ETH)) {
    ...
    EXPECT_EQ(url::Origin::Create(
              ...
              url::Origin::Create(GURL("https://goerli-infura.hns.com")));
    EXPECT_EQ(url::Origin::Create(
  ...
  EXPECT_EQ(url::Origin::Create(
            ...
            url::Origin::Create(GURL("https://mainnet-beta-solana.hns.com")));
  EXPECT_EQ(
...
TEST_F(JsonRpcServiceUnitTest, EnsGetContentHash) {
  ...
  feature_list.InitAndDisableFeature(features::kHnsWalletENSL2Feature);
...
TEST_F(JsonRpcServiceUnitTest, AddEthereumChainApproved) {
  ...
  ASSERT_FALSE(
      hns_wallet::GetNetworkURL(prefs(), "0x111", mojom::CoinType::ETH)
          .is_valid());
...
TEST_F(JsonRpcServiceUnitTest, AddEthereumChainApprovedForOrigin) {
  ...
  EXPECT_EQ("",
            json_rpc_service_->AddEthereumChainForOrigin(
                chain.Clone(), url::Origin::Create(GURL("https://hns.com"))));
  json_rpc_service_->AddEthereumChainRequestCompleted("0x111", true);
  loop.Run();
...
TEST_F(JsonRpcServiceUnitTest, AddEthereumChainError) {
  ...
  mojom::ProviderError expected = mojom::ProviderError::kSuccess;
  ASSERT_FALSE(
      hns_wallet::GetNetworkURL(prefs(), chain.chain_id, mojom::CoinType::ETH)
          .is_valid());
  SetEthChainIdInterceptor(GetActiveEndpointUrl(chain), chain.chain_id);
  json_rpc_service_->AddChain(
      chain.Clone(),
...
TEST_F(JsonRpcServiceUnitTest, AddEthereumChainForOriginError) {
  ...
  auto origin = url::Origin::Create(GURL("https://hns.com"));
  ...
  // New chain, not valid rpc url.
  {
    ...
    std::unique_ptr<TestJsonRpcServiceObserver> observer(
        new TestJsonRpcServiceObserver(
            ...
            l10n_util::GetStringFUTF8(IDS_HNS_WALLET_ETH_CHAIN_ID_FAILED,
                                      base::ASCIIToUTF16(network_url.spec()))));
 ...
 public:
  ...
  std::string DnsIpfsResponse() const {
    return MakeJsonRpcStringArrayResponse(
        {"ipfs_hash", "", "", "", "", "https://hns.com"});
  }

  std::string DnsHnsResponse() const {
    return MakeJsonRpcStringArrayResponse(
        {"", "", "", "", "", "https://hns.com"});
  }
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_PolygonNetworkError) {
  ...
  SetEthTimeoutResponse();
  SetPolygonTimeoutResponse();
  json_rpc_service_->UnstoppableDomainsResolveDns("hns.crypto",
                                                  callback.Get());
  base::RunLoop().RunUntilIdle();
  testing::Mock::VerifyAndClearExpectations(&callback);
  ...
  EXPECT_CALL(callback,
              Run(absl::optional<GURL>(), mojom::ProviderError::kInternalError,
                  l10n_util::GetStringUTF8(IDS_WALLET_INTERNAL_ERROR)));
  SetEthRawResponse(DnsHnsResponse());
  SetPolygonTimeoutResponse();
  json_rpc_service_->UnstoppableDomainsResolveDns("hns.crypto",
                                                  callback.Get());
  base::RunLoop().RunUntilIdle();
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_PolygonResult) {
  base::MockCallback<ResolveDnsCallback> callback;
  EXPECT_CALL(callback, Run(absl::optional<GURL>("https://hns.com"),
                            mojom::ProviderError::kSuccess, ""));
  SetEthTimeoutResponse();
  SetPolygonRawResponse(DnsHnsResponse());
  json_rpc_service_->UnstoppableDomainsResolveDns("hns.crypto",
                                                  callback.Get());
  base::RunLoop().RunUntilIdle();
  testing::Mock::VerifyAndClearExpectations(&callback);
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_InvalidDomain) {
  ...
  EXPECT_CALL(callback,
              Run(absl::optional<GURL>(), mojom::ProviderError::kInvalidParams,
                  l10n_util::GetStringUTF8(IDS_WALLET_INVALID_PARAMETERS)));
  json_rpc_service_->UnstoppableDomainsResolveDns("hns.test", callback.Get());
  EXPECT_EQ(0, url_loader_factory_.NumPending());
  base::RunLoop().RunUntilIdle();
}
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_ManyCalls) {
  ...
  EXPECT_CALL(callback1, Run(absl::optional<GURL>("https://hns.com"),
                             mojom::ProviderError::kSuccess, ""));
  base::MockCallback<ResolveDnsCallback> callback2;
  EXPECT_CALL(callback2, Run(absl::optional<GURL>("https://hns.com"),
                             mojom::ProviderError::kSuccess, ""));
  ...
  // This will resolve hns.crypto requests.
  eth_mainnet_getmany_call_handler_->AddItem("hns.crypto", keys[0],
                                             "ipfs_hash");
  eth_mainnet_getmany_call_handler_->AddItem("hns.crypto", keys[5],
                                             "https://hns.com");
  polygon_getmany_call_handler_->AddItem("hns.crypto", keys[5],
                                         "https://hns.com");

  // This will resolve hns.x requests.
  polygon_getmany_call_handler_->AddItem("hns.x", keys[0], "ipfs_hash");
  polygon_getmany_call_handler_->AddItem("hns.x", keys[5],
                                         "https://hns.com");
  eth_mainnet_getmany_call_handler_->AddItem("hns.x", keys[5],
                                             "https://hns.com");
  ...
  json_rpc_service_->UnstoppableDomainsResolveDns("hns.x", callback3.Get());
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(2, eth_mainnet_getmany_call_handler_->calls_number());
  EXPECT_EQ(2, polygon_getmany_call_handler_->calls_number());
  testing::Mock::VerifyAndClearExpectations(&callback3);
...
TEST_F(JsonRpcServiceUnitTest, GetWalletAddrInvalidDomain) {
  const std::vector<std::string> invalid_domains = {"", ".eth", "-hns.eth",
                                                    "hns-.eth", "b.eth"};
...
TEST_F(JsonRpcServiceUnitTest, IsValidDomain) {
  std::vector<std::string> valid_domains = {"hns.eth", "test.hns.eth",
                                            "hns-test.test-dev.eth"};
  ...
  std::vector<std::string> invalid_domains = {
      "",      ".eth",    "-hns.eth",      "hns-.eth",     "hns.e-th",
      "b.eth", "hns.e", "-hns.test.eth", "hns-.test.eth"};
  for (const auto& domain : invalid_domains) {
...
TEST_F(JsonRpcServiceUnitTest, Reset) {
  std::vector<base::Value::Dict> values;
  mojom::NetworkInfo chain = GetTestNetworkInfo1("0x1");
  values.push_back(hns_wallet::NetworkInfoToValue(chain));
  UpdateCustomNetworks(prefs(), &values);
  ...
  EXPECT_TRUE(prefs()->HasPrefPath(kHnsWalletCustomNetworks));
  ...
  EXPECT_FALSE(prefs()->HasPrefPath(kHnsWalletCustomNetworks));
  EXPECT_EQ(GetCurrentChainId(prefs(), mojom::CoinType::ETH, absl::nullopt),
            mojom::kMainnetChainId);
...
TEST_F(JsonRpcServiceUnitTest, MigrateDeprecatedEthereumTestnets) {
  ...
  for (const std::string& deprecated_chain_id : deprecated_chain_ids) {
    ...
    prefs()->SetBoolean(kHnsWalletDeprecateEthereumTestNetworksMigrated,
                        false);
    ASSERT_FALSE(
        prefs()->GetBoolean(kHnsWalletDeprecateEthereumTestNetworksMigrated));

    // Set selected network to deprecated network and validate
    ScopedDictPrefUpdate update(prefs(), kHnsWalletSelectedNetworks);
    update->Set(kEthereumPrefKey, deprecated_chain_id);
    const auto& selected_networks =
        prefs()->GetDict(kHnsWalletSelectedNetworks);
  ...
  prefs()->SetBoolean(kHnsWalletDeprecateEthereumTestNetworksMigrated, false);
  ASSERT_FALSE(
      prefs()->GetBoolean(kHnsWalletDeprecateEthereumTestNetworksMigrated));
...
TEST_F(JsonRpcServiceUnitTest, MigrateMultichainNetworks) {
  ...
  prefs()->Set(kHnsWalletCustomNetworksDeprecated, *old_custom_networks);
  prefs()->SetString(kHnsWalletCurrentChainId, "0x3");

  JsonRpcService::MigrateMultichainNetworks(prefs());

  const auto& new_custom_networks =
      prefs()->GetDict(kHnsWalletCustomNetworks);
  ...
  EXPECT_FALSE(prefs()->HasPrefPath(kHnsWalletCustomNetworksDeprecated));
  EXPECT_FALSE(prefs()->HasPrefPath(kHnsWalletCurrentChainId));
}
...
class ENSL2JsonRpcServiceUnitTest : public JsonRpcServiceUnitTest {
 public:
  ...
  GURL gateway_url() { return GURL("https://gateway.hns.com/"); }
 ...
 private:
  ...
  base::test::ScopedFeatureList feature_list_{
      features::kHnsWalletENSL2Feature};
#endif
};
...
class SnsJsonRpcServiceUnitTest : public JsonRpcServiceUnitTest {
 public:
  ...
  SolanaAddress GetMintAddress() const {
    return *hns_wallet::GetMintAddress(GetDomainKeyAddress());
  }
  ...
  GURL url_value() const { return GURL("https://hns.com"); }
 ...
 private:
  base::test::ScopedFeatureList feature_list_{features::kHnsWalletSnsFeature};
};
