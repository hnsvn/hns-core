...
#include "hns/components/hns_wallet/browser/bitcoin/bitcoin_keyring.h"
#include "hns/components/hns_wallet/browser/hns_wallet_prefs.h"
#include "hns/components/hns_wallet/browser/hns_wallet_utils.h"
#include "hns/components/hns_wallet/browser/ethereum_keyring.h"
...
const char kLegacyHnsWallet[] = "legacy_hns_wallet";
...
std::string GetAccountName(size_t number) {
  return l10n_util::GetStringFUTF8(IDS_HNS_WALLET_NUMBERED_ACCOUNT_NAME,
...
void AddImportedAccountForKeyring(PrefService* profile_prefs,
  ...
  ScopedDictPrefUpdate update(profile_prefs, kHnsWalletKeyrings);
...
void AddDerivedAccountInfoForKeyring(PrefService* profile_prefs,
                                     ...
                                     mojom::KeyringId keyring_id) {
  ScopedDictPrefUpdate keyrings_update(profile_prefs, kHnsWalletKeyrings);
...
bool SetSelectedWalletAccountInPrefs(PrefService* profile_prefs,
  ...
  if (unique_key ==
      profile_prefs->GetString(kHnsWalletSelectedWalletAccount)) {
    return false;
  }

  profile_prefs->SetString(kHnsWalletSelectedWalletAccount, unique_key);
  return true;
}

std::string GetSelectedWalletAccountFromPrefs(PrefService* profile_prefs) {
  return profile_prefs->GetString(kHnsWalletSelectedWalletAccount);
}
...
bool SetSelectedDappAccountInPrefs(PrefService* profile_prefs,
  ...
  const char* pref_name = dapp_coin == mojom::CoinType::ETH
                              ? kHnsWalletSelectedEthDappAccount
                              : kHnsWalletSelectedSolDappAccount;
...
void KeyringService::MaybeMigrateSelectedAccountPrefs() {
  if (!profile_prefs_->HasPrefPath(kHnsWalletSelectedCoinDeprecated)) {
    return;
  }
  ...
  auto find_account = [&](mojom::KeyringId keyring_id) -> mojom::AccountIdPtr {
    if (!profile_prefs_->GetDict(kHnsWalletKeyrings)
    ...
    std::string address;
    {
      ScopedDictPrefUpdate update(profile_prefs_, kHnsWalletKeyrings);
  ...
  auto coin = static_cast<mojom::CoinType>(
      profile_prefs_->GetInteger(kHnsWalletSelectedCoinDeprecated));
  switch (coin) {
  ...
  profile_prefs_->ClearPref(kHnsWalletSelectedCoinDeprecated);
}
...
const base::Value* KeyringService::GetPrefForKeyring(
    ...
    const std::string& key,
    mojom::KeyringId keyring_id) {
  const auto& keyrings_pref = profile_prefs.GetDict(kHnsWalletKeyrings);
...
HDKeyring* KeyringService::ResumeKeyring(mojom::KeyringId keyring_id,
  ...
  bool is_legacy_hns_wallet = false;
  if (auto* value =
          GetPrefForKeyring(*profile_prefs_, kLegacyHnsWallet, keyring_id)) {
    is_legacy_hns_wallet = value->GetBool();
  }
  auto* keyring =
      CreateKeyringInternal(keyring_id, mnemonic, is_legacy_hns_wallet);
  if (!keyring) {
    return nullptr;
  }
...
HDKeyring* KeyringService::RestoreKeyring(mojom::KeyringId keyring_id,
  ...
  if (CreateEncryptorForKeyring(password, keyring_id)) {
    ...
    // Also need to make sure is_legacy_hns_wallet are the same, users might
    // choose the option wrongly and then want to start over with same mnemonic
    // but different is_legacy_hns_wallet value
    const base::Value* value =
        GetPrefForKeyring(*profile_prefs_, kLegacyHnsWallet, keyring_id);
    if (!current_mnemonic.empty() && current_mnemonic == mnemonic && value &&
        value->GetBool() == is_legacy_hns_wallet) {
      ...
      profile_prefs_->SetBoolean(kHnsWalletKeyringEncryptionKeysMigrated,
                                 true);
    }
  }
...
void KeyringService::CreateWallet(const std::string& mnemonic,
                                  const std::string& password,
                                  CreateWalletCallback callback) {
  profile_prefs_->SetBoolean(kHnsWalletKeyringEncryptionKeysMigrated, true);
...
std::vector<mojom::AccountInfoPtr> KeyringService::AddHardwareAccountsSync(
  ...
  ScopedDictPrefUpdate keyrings_update(profile_prefs_, kHnsWalletKeyrings);
...
void KeyringService::MaybeMigratePBKDF2Iterations(const std::string& password) {
  if (profile_prefs_->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated)) {
    return;
  }

  // Pref is supposed to be set only as true.
  DCHECK(
      !profile_prefs_->HasPrefPath(kHnsWalletKeyringEncryptionKeysMigrated));
...
void KeyringService::ResetAutoLockTimer() {
  if (auto_lock_timer_->IsRunning()) {
    ...
    size_t auto_lock_minutes =
        (size_t)profile_prefs_->GetInteger(kHnsWalletAutoLockMinutes);
...
HDKeyring* KeyringService::CreateKeyringInternal(mojom::KeyringId keyring_id,
  ...
  auto seed = is_legacy_hns_wallet ? MnemonicToEntropy(mnemonic)
                                     : MnemonicToSeed(mnemonic, "");
  if (!seed) {
    return nullptr;
  }
  if (is_legacy_hns_wallet && seed->size() != 32) {
    VLOG(1) << __func__
            << "mnemonic for legacy hns wallet must be 24 words which will "
  ...
  SetPrefForKeyring(profile_prefs_, kLegacyHnsWallet,
                    base::Value(is_legacy_hns_wallet), keyring_id);
...
void KeyringService::GetAutoLockMinutes(GetAutoLockMinutesCallback callback) {
  std::move(callback).Run(
      profile_prefs_->GetInteger(kHnsWalletAutoLockMinutes));
}
...
void KeyringService::SetAutoLockMinutes(int32_t minutes,
  ...
  if (minutes != old_auto_lock_minutes) {
    profile_prefs_->SetInteger(kHnsWalletAutoLockMinutes, minutes);
  }
  std::move(callback).Run(true);
}
...
bool KeyringService::ValidatePasswordInternal(const std::string& password) {
  ...
  auto iterations =
      profile_prefs_->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated)
          ? GetPbkdf2Iterations()
...
mojom::AccountInfoPtr KeyringService::GetSelectedWalletAccount() {
  ...
  auto unique_key =
      profile_prefs_->GetString(kHnsWalletSelectedWalletAccount);
