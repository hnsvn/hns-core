...
#include "hns/components/hns_wallet/browser/account_resolver_delegate_impl.h"
...
#include "hns/components/hns_wallet/common/eth_sign_typed_data_helper.h"
...
std::vector<std::string> FilterAccounts(
    ...
    const hns_wallet::mojom::AccountInfoPtr& selected_account) {
...
EthereumProviderImpl::EthereumProviderImpl(
    ...
    std::unique_ptr<HnsWalletProviderDelegate> delegate,
    PrefService* prefs)
...
void EthereumProviderImpl::AddEthereumChain(const std::string& json_payload,
  ...
  const auto* params = root.FindList(hns_wallet::kParams);
  ...
  // https://github.com/hnsvn/hns-browser/issues/17637
...
void EthereumProviderImpl::GetEncryptionPublicKey(const std::string& address,
  ...
  // Only show bubble when there is no immediate error
  hns_wallet_service_->AddGetPublicKeyRequest(
...
void EthereumProviderImpl::ContinueDecryptWithSanitizedJson(
  ...
  hns_wallet_service_->AddDecryptRequest(account_id, origin, unsafe_message,
                                           std::move(callback), std::move(id));
  delegate_->ShowPanel();
}
...
void EthereumProviderImpl::SignMessageInternal(
  ...
  hns_wallet_service_->AddSignMessageRequest(
      std::move(request),
...
void EthereumProviderImpl::RequestEthereumPermissions(
  ...
  DCHECK_EQ(keyring_info->id, hns_wallet::mojom::kDefaultKeyringId);
...
void EthereumProviderImpl::OnContentSettingChanged(
  ...
  if (content_type == ContentSettingsType::HNS_ETHEREUM) {
    UpdateKnownAccounts();
  }
}
...
void EthereumProviderImpl::AddSuggestToken(mojom::BlockchainTokenPtr token,
  ...
  hns_wallet_service_->AddSuggestTokenRequest(
