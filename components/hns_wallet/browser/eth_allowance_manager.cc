...
const std::vector<std::string>& GetChainIdsForAlowanceDiscovering() {
  static base::NoDestructor<std::vector<std::string>> supported_chain_ids(
      {hns_wallet::mojom::kMainnetChainId,
       ...
       hns_wallet::mojom::kCeloMainnetChainId,
       hns_wallet::mojom::kArbitrumMainnetChainId,
...
std::string GetBlocknumberFilterFromCache(
  ...
  const auto& allowance_cache_dict =
      prefs->GetDict(kHnsWalletEthAllowancesCache);
  ...
  hns_wallet::uint256_t block_number{0};
  if (last_block_number_ptr && !last_block_number_ptr->empty() &&
      hns_wallet::HexValueToUint256(*last_block_number_ptr, &block_number)) {
    result = hns_wallet::Uint256ValueToHex(++block_number);
  }
  return result;
}
...
void EthAllowanceManager::LoadCachedAllowances(
  ...
  const auto& allowance_cache_dict =
      prefs_->GetDict(kHnsWalletEthAllowancesCache);
