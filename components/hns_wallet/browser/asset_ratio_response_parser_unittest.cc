...
#include "hns/components/hns_wallet/browser/asset_ratio_response_parser.h"
...
TEST(AssetRatioResponseParserUnitTest, ParseAssetPriceHistory) {
  // https://ratios.bsg.hnssoftware.com/v2/history/coingecko/basic-attention-token/usd/2021-06-03T15%3A00%3A00.000Z/2021-06-03T18%3A00%3A00.000Z
  std::string json(R"(
    {
      "payload": {
...
TEST(AssetRatioResponseParserUnitTest, ParseCoinMarkets) {
  // https://ratios.rewards.hns.software/v2/market/provider/coingecko\?vsCurrency\=usd\&limit\=2
  std::string json(R"(
    {
      "payload": [
        {
  ...
  std::vector<hns_wallet::mojom::CoinMarketPtr>& values = *parsed_values;
