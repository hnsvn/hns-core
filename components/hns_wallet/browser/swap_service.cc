...
#include "hns/components/hns_wallet/browser/swap_request_helper.h"
#include "hns/components/hns_wallet/browser/swap_response_parser.h"
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("swap_service", R"(
      semantics {
        ...
        trigger:
          "Triggered by uses of the native Hns wallet."
        data:
...
bool IsEVMNetworkSupported(const std::string& chain_id) {
  return (chain_id == hns_wallet::mojom::kGoerliChainId ||
          chain_id == hns_wallet::mojom::kMainnetChainId ||
          chain_id == hns_wallet::mojom::kPolygonMainnetChainId ||
          chain_id == hns_wallet::mojom::kBinanceSmartChainMainnetChainId ||
          chain_id == hns_wallet::mojom::kAvalancheMainnetChainId ||
          chain_id == hns_wallet::mojom::kFantomMainnetChainId ||
          chain_id == hns_wallet::mojom::kCeloMainnetChainId ||
          chain_id == hns_wallet::mojom::kOptimismMainnetChainId ||
          chain_id == hns_wallet::mojom::kArbitrumMainnetChainId ||
          chain_id == hns_wallet::mojom::kBaseMainnetChainId);
}

bool IsSolanaNetworkSupported(const std::string& chain_id) {
  return chain_id == hns_wallet::mojom::kSolanaMainnet;
}

bool HasRFQTLiquidity(const std::string& chain_id) {
  return (chain_id == hns_wallet::mojom::kMainnetChainId ||
          chain_id == hns_wallet::mojom::kPolygonMainnetChainId);
}

absl::optional<double> GetFeePercentage(const std::string& chain_id) {
  if (IsEVMNetworkSupported(chain_id)) {
    return hns_wallet::k0xBuyTokenFeePercentage;
  }

  if (IsSolanaNetworkSupported(chain_id)) {
    return hns_wallet::kSolanaBuyTokenFeePercentage;
  }
...
std::string GetAffiliateAddress(const std::string& chain_id) {
  if (IsEVMNetworkSupported(chain_id)) {
    return hns_wallet::kAffiliateAddress;
  }

  return "";
}

hns_wallet::mojom::HnsSwapFeeResponsePtr GetHnsFeeInternal(
    const hns_wallet::mojom::HnsSwapFeeParamsPtr& params) {
  ...
  if (IsSolanaNetworkSupported(params->chain_id)) {
    hns_wallet::mojom::HnsSwapFeeResponsePtr response =
        hns_wallet::mojom::HnsSwapFeeResponse::New();

    bool has_fees =
        hns_wallet::HasJupiterFeesForTokenMint(params->output_token);
    ...
    auto hns_fee_pct = percentage_fee.value() * 1.0;
    auto discount_on_hns_fee_pct = has_fees ? 0.0 : 100.0;
    auto discount_on_prototcol_fee_pct = has_fees ? 0.0 : 100.0;

    response->hns_fee_pct = base::NumberToString(hns_fee_pct);

    response->discount_on_hns_fee_pct =
        base::NumberToString(discount_on_hns_fee_pct);
    ...
    response->effective_fee_pct = base::NumberToString(
        (100.0 - discount_on_hns_fee_pct) / 100.0 * hns_fee_pct);
    ...
    response->discount_code =
        has_fees ? hns_wallet::mojom::DiscountCode::kNone
                 : hns_wallet::mojom::DiscountCode::kUnknownJupiterOutputMint;

    response->has_hns_fee = has_fees;
  ...
  if (IsEVMNetworkSupported(params->chain_id)) {
    ...
    // We currently do not offer discounts on 0x Hns fees.
    auto discount_on_hns_fee_pct = 0.0;
    ...
    response->protocol_fee_pct =
        base::NumberToString(hns_wallet::k0xProtocolFeePercentage);
    response->hns_fee_pct = base::NumberToString(percentage_fee.value());
    response->discount_on_hns_fee_pct =
        base::NumberToString(discount_on_hns_fee_pct);

    auto effective_fee_pct =
        (100.0 - discount_on_hns_fee_pct) / 100.0 * percentage_fee.value();
    ...
    response->discount_code = hns_wallet::mojom::DiscountCode::kNone;
    response->has_hns_fee = effective_fee_pct > 0.0;
...
GURL Append0xSwapParams(const GURL& swap_url,
                        const hns_wallet::mojom::SwapParams& params,
  ...
  auto hns_swap_fee_params = hns_wallet::mojom::HnsSwapFeeParams::New();
  hns_swap_fee_params->chain_id = chain_id;
  hns_swap_fee_params->taker = params.taker_address;
  hns_swap_fee_params->input_token = params.sell_token;
  hns_swap_fee_params->output_token = params.buy_token;
  const auto& hns_fee = GetHnsFeeInternal(std::move(hns_swap_fee_params));
  if (hns_fee && hns_fee->has_hns_fee) {
...
GURL AppendJupiterQuoteParams(
    const GURL& swap_url,
    const hns_wallet::mojom::JupiterQuoteParams& params,
    const std::string& chain_id) {
  GURL url = swap_url;
  ...
  auto hns_swap_fee_params = hns_wallet::mojom::HnsSwapFeeParams::New();
  hns_swap_fee_params->chain_id = chain_id;
  hns_swap_fee_params->taker = params.user_public_key;
  hns_swap_fee_params->input_token = params.input_mint;
  hns_swap_fee_params->output_token = params.output_mint;
  const auto& hns_fee = GetHnsFeeInternal(std::move(hns_swap_fee_params));
  if (hns_fee && hns_fee->has_hns_fee) {
    url = net::AppendQueryParameter(url, "feeBps", hns_fee->fee_param);
  }
...
base::flat_map<std::string, std::string> Get0xAPIHeaders() {
  std::string hns_zero_ex_api_key(BUILDFLAG(HNS_ZERO_EX_API_KEY));
  if (hns_zero_ex_api_key.empty()) {
    return {};
  }
  return {{hns_wallet::k0xAPIKeyHeader, std::move(hns_zero_ex_api_key)}};
}
...
std::string SwapService::GetBaseSwapURL(const std::string& chain_id) {
  ...
  if (chain_id == hns_wallet::mojom::kGoerliChainId) {
    return hns_wallet::kGoerliSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kMainnetChainId) {
    return hns_wallet::kSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kPolygonMainnetChainId) {
    return hns_wallet::kPolygonSwapBaseAPIURL;
  } else if (chain_id ==
             hns_wallet::mojom::kBinanceSmartChainMainnetChainId) {
    return hns_wallet::kBinanceSmartChainSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kAvalancheMainnetChainId) {
    return hns_wallet::kAvalancheSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kFantomMainnetChainId) {
    return hns_wallet::kFantomSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kCeloMainnetChainId) {
    return hns_wallet::kCeloSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kOptimismMainnetChainId) {
    return hns_wallet::kOptimismSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kArbitrumMainnetChainId) {
    return hns_wallet::kArbitrumSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kSolanaMainnet) {
    return hns_wallet::kSolanaSwapBaseAPIURL;
  } else if (chain_id == hns_wallet::mojom::kBaseMainnetChainId) {
    return hns_wallet::kBaseSwapBaseAPIURL;
  }
...
void SwapService::GetPriceQuote(mojom::SwapParamsPtr swap_params,
  ...
  if (!IsEVMNetworkSupported(json_rpc_service_->GetChainIdSync(
    ...
    std::move(callback).Run(
        nullptr, nullptr,
        l10n_util::GetStringUTF8(IDS_HNS_WALLET_UNSUPPORTED_NETWORK));
    return;
  }
...
void SwapService::GetJupiterSwapTransactions(
  ...
  hns_swap_fee_params->output_token = params->output_mint;
  const auto& hns_fee = GetHnsFeeInternal(std::move(hns_swap_fee_params));
  bool has_fee = hns_fee && hns_fee->has_hns_fee;
...
void SwapService::GetHnsFee(mojom::HnsSwapFeeParamsPtr params,
                              GetHnsFeeCallback callback) {
  if (auto response = GetHnsFeeInternal(std::move(params))) {
