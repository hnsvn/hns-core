...
class HnsWalletPinServiceTest : public testing::Test {
 public:
  HnsWalletPinServiceTest() = default;

  HnsWalletPinService* service() { return hns_wallet_pin_service_.get(); }
 ...
 protected:
  void SetUp() override {
    ...
    hns_wallet_pin_service_ = std::make_unique<HnsWalletPinService>(
  ...
  std::unique_ptr<HnsWalletPinService> hns_wallet_pin_service_;
  TestingPrefServiceSimple pref_service_;
  content::BrowserTaskEnvironment task_environment_;
};

TEST_F(HnsWalletPinServiceTest, AddSolPin) {
  {
  ...
  }
  {
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kSolMonkey1Path);
    token->is_nft = true;
    ...
    EXPECT_EQ(HnsWalletPinService::StatusToString(
...

  {
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kSolMonkey2Path);
    token->is_nft = true;
    absl::optional<bool> call_status;
    service()->AddPin(
        std::move(token), absl::nullopt,
...
TEST_F(HnsWalletPinServiceTest, AddPin) {
  ...
  }
  {
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kMonkey1Path);
...

  {
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kMonkey2Path);
    token->is_erc721 = true;
    absl::optional<bool> call_status;
    service()->AddPin(
        std::move(token), absl::nullopt,
...
TEST_F(HnsWalletPinServiceTest, AddPin_GatewayUrl) {
  {
    ON_CALL(*GetContentTypeChecker(), CheckContentTypeSupported(_, _))
        .WillByDefault(::testing::Invoke(
            [](const std::string& cid,
  ...
  // Right gateway
  {
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kMonkey5Path);
    token->is_erc721 = true;
    absl::optional<bool> call_status;
    service()->AddPin(
        std::move(token), absl::nullopt,
  ...
  // Wrong gateway
  {
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kMonkey6Path);
    token->is_erc721 = true;
    absl::optional<bool> call_status;
    service()->AddPin(
        std::move(token), absl::nullopt,
...
TEST_F(HnsWalletPinServiceTest, AddPin_ContentVerification) {
  {
    ON_CALL(*GetContentTypeChecker(), CheckContentTypeSupported(_, _))
...
TEST_F(HnsWalletPinServiceTest, AddPin_NonIpfsImage) {
  {
    ON_CALL(*GetJsonRpcService(), GetERC721Metadata(_, _, _, _))
        .WillByDefault(::testing::Invoke(
            [](const std::string& contract_address, const std::string& token_id,
    ...
    mojom::BlockchainTokenPtr token =
        HnsWalletPinService::TokenFromPrefPath(kMonkey4Path);
    token->is_erc721 = true;
    absl::optional<bool> call_status;
    service()->AddPin(
        std::move(token), absl::nullopt,
...
TEST_F(HnsWalletPinServiceTest, RemoveSolPin) {
  {
...
TEST_F(HnsWalletPinServiceTest, RemovePin) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
...
TEST_F(HnsWalletPinServiceTest, ValidatePin) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
...
TEST_F(HnsWalletPinServiceTest, GetTokenStatus) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
...

  {
    ...
    base::Value::Dict item;
    item.Set("status", HnsWalletPinService::StatusToString(
                           mojom::TokenPinStatusCode::STATUS_PINNING_FAILED));
    base::Value::Dict error;
    error.Set("error_code",
              HnsWalletPinService::ErrorCodeToString(
                  mojom::WalletPinServiceErrorCode::ERR_FETCH_METADATA_FAILED));
...
TEST_F(HnsWalletPinServiceTest, GetLastValidateTime) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
...
TEST_F(HnsWalletPinServiceTest, SolTokenFromPrefPath) {
  mojom::BlockchainTokenPtr token =
      HnsWalletPinService::TokenFromPrefPath(kSolMonkey1Path);
...
TEST_F(HnsWalletPinServiceTest, ServiceFromSolPath) {
  EXPECT_FALSE(
      HnsWalletPinService::ServiceFromPrefPath(kMonkey1Path).has_value());

  EXPECT_EQ("nftstorage", HnsWalletPinService::ServiceFromPrefPath(
                              "nft.nftstorage.501.0x65."
                              "GdGXUTd9ZctZQdNCsV85ULryafVgMR4Jyiq3zkqwR8kY.*")
                              .value());
}

TEST_F(HnsWalletPinServiceTest, TokenFromPrefPath) {
  mojom::BlockchainTokenPtr token =
      HnsWalletPinService::TokenFromPrefPath(kMonkey1Path);
...
TEST_F(HnsWalletPinServiceTest, ServiceFromPath) {
  EXPECT_FALSE(
      HnsWalletPinService::ServiceFromPrefPath(kMonkey1Path).has_value());
...
TEST_F(HnsWalletPinServiceTest, GetPath) {
  {
    ...
    auto path = HnsWalletPinService::GetTokenPrefPath(absl::nullopt, token);
...

  {
    ...
    token->token_id = "0x2";
    token->chain_id = "mainnet";
    auto path = HnsWalletPinService::GetTokenPrefPath("nftstorage", token);
...
TEST_F(HnsWalletPinServiceTest, GetTokens) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
...
TEST_F(HnsWalletPinServiceTest, Reset) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
...
TEST_F(HnsWalletPinServiceTest, Reset_Failed) {
  {
    ScopedDictPrefUpdate update(GetPrefs(), kPinnedNFTAssets);
    base::Value::Dict& update_dict = update.Get();
