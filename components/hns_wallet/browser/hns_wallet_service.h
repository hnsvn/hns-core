...
#ifndef HNS_COMPONENTS_HNS_WALLET_BROWSER_HNS_WALLET_SERVICE_H_
#define HNS_COMPONENTS_HNS_WALLET_BROWSER_HNS_WALLET_SERVICE_H_
...
#include "hns/components/hns_wallet/browser/asset_discovery_manager.h"
...
extern const char kHnsWalletLastUsageTimeHistogramName[];
...
class HnsWalletService : public KeyedService,
                           public mojom::HnsWalletService,
                           public HnsWalletServiceDelegate::Observer {
 public:
  ...
  ~HnsWalletService() override;

  HnsWalletService(const HnsWalletService&) = delete;
  HnsWalletService& operator=(const HnsWalletService&) = delete;

  mojo::PendingRemote<mojom::HnsWalletService> MakeRemote();
  void Bind(mojo::PendingReceiver<mojom::HnsWalletService> receiver);
  ...
  // mojom::HnsWalletService:
  void AddObserver(::mojo::PendingRemote<mojom::HnsWalletServiceObserver>
                       observer) override;
  ...
  // HnsWalletServiceDelegate::Observer:
  ...
  // Resets things back to the original state of HnsWalletService.
 ...
 private:
  ...
  friend class HnsWalletServiceUnitTest;

  FRIEND_TEST_ALL_PREFIXES(HnsWalletServiceUnitTest, GetChecksumAddress);
  FRIEND_TEST_ALL_PREFIXES(HnsWalletServiceUnitTest, AddSuggestToken);
  FRIEND_TEST_ALL_PREFIXES(HnsWalletServiceUnitTest, GetUserAsset);
  FRIEND_TEST_ALL_PREFIXES(HnsWalletServiceUnitTest, ImportFromMetaMask);
  FRIEND_TEST_ALL_PREFIXES(HnsWalletServiceUnitTest, Reset);
  FRIEND_TEST_ALL_PREFIXES(HnsWalletServiceUnitTest, GetUserAssetAddress);
  ...
  void OnHnsWalletNftDiscoveryEnabled();
  ...
  mojo::RemoteSet<mojom::HnsWalletServiceObserver> observers_;
  mojo::RemoteSet<mojom::HnsWalletServiceTokenObserver> token_observers_;
  std::unique_ptr<HnsWalletServiceDelegate> delegate_;
  raw_ptr<KeyringService> keyring_service_ = nullptr;
  raw_ptr<JsonRpcService> json_rpc_service_ = nullptr;
  raw_ptr<TxService> tx_service_ = nullptr;
  raw_ptr<PrefService> profile_prefs_ = nullptr;
  HnsWalletP3A hns_wallet_p3a_;
  ...
  mojo::ReceiverSet<mojom::HnsWalletService> receivers_;
  PrefChangeRegistrar pref_change_registrar_;
  base::WeakPtrFactory<HnsWalletService> weak_ptr_factory_;
};

}  // namespace hns_wallet

#endif  // HNS_COMPONENTS_HNS_WALLET_BROWSER_HNS_WALLET_SERVICE_H_
