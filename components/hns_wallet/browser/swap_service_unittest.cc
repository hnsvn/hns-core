...
hns_wallet::mojom::SwapParamsPtr GetCannedSwapParams() {
  auto params = hns_wallet::mojom::SwapParams::New();
...
hns_wallet::mojom::JupiterQuoteParamsPtr GetCannedJupiterQuoteParams() {
  auto params = hns_wallet::mojom::JupiterQuoteParams::New();
...
hns_wallet::mojom::JupiterSwapParamsPtr GetCannedJupiterSwapParams(
    const std::string& output_mint) {
  auto params = hns_wallet::mojom::JupiterSwapParams::New();

  auto route = hns_wallet::mojom::JupiterRoute::New();
  ...
  auto market_info = hns_wallet::mojom::JupiterMarketInfo::New();
  ...
  auto lp_fee = hns_wallet::mojom::JupiterFee::New();
  lp_fee->amount = 30ULL;
  lp_fee->mint = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v";
  lp_fee->pct = 0.003;

  auto platform_fee = hns_wallet::mojom::JupiterFee::New();
...
class SwapServiceUnitTest : public testing::Test {
 public:
  ...
  void TestGetJupiterSwapTransactions(
      const bool expected_success,
      hns_wallet::mojom::JupiterSwapTransactionsPtr expected_response,
...
TEST_F(SwapServiceUnitTest, GetPriceQuote) {
  ...
  auto source = hns_wallet::mojom::ZeroExSource::New();
  source->name = "Uniswap_V2";
  source->proportion = "1";
  expected_swap_response->sources.push_back(source.Clone());

  auto fees = hns_wallet::mojom::ZeroExFees::New();
  auto zero_ex_fee = hns_wallet::mojom::ZeroExFee::New();
...
TEST_F(SwapServiceUnitTest, GetPriceQuoteError) {
  ...
  swap_service_->GetPriceQuote(hns_wallet::mojom::SwapParams::New(),
                               callback.Get());
  base::RunLoop().RunUntilIdle();
}
...
TEST_F(SwapServiceUnitTest, GetHnsFee) {
  auto params = mojom::HnsSwapFeeParams::New();
  auto expected_response = mojom::HnsSwapFeeResponse::New();
  ...
  expected_response->hns_fee_pct = "0.875";
  expected_response->discount_on_hns_fee_pct = "0";
  expected_response->effective_fee_pct = "0.875";
  expected_response->discount_code = mojom::DiscountCode::kNone;
  expected_response->has_hns_fee = true;

  base::MockCallback<mojom::SwapService::GetHnsFeeCallback> callback;
  EXPECT_CALL(callback, Run(EqualsMojo(expected_response), ""));
  swap_service_->GetHnsFee(params->Clone(), callback.Get());
  base::RunLoop().RunUntilIdle();
  testing::Mock::VerifyAndClearExpectations(&callback);
  ...
  expected_response->hns_fee_pct = "0.85";
  expected_response->discount_on_hns_fee_pct = "100";
  expected_response->effective_fee_pct = "0";
  expected_response->discount_code =
      mojom::DiscountCode::kUnknownJupiterOutputMint;
  expected_response->has_hns_fee = false;
  ...
  EXPECT_CALL(callback,
              Run(EqualsMojo(mojom::HnsSwapFeeResponsePtr()),
                  l10n_util::GetStringUTF8(IDS_WALLET_INTERNAL_ERROR)));
  swap_service_->GetHnsFee(params->Clone(), callback.Get());
  base::RunLoop().RunUntilIdle();
  testing::Mock::VerifyAndClearExpectations(&callback);
