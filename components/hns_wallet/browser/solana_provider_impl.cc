...
void SolanaProviderImpl::Connect(absl::optional<base::Value::Dict> arg,
  ...
  if (allowed) {
    ...
    const auto keyring_info = keyring_service_->GetKeyringInfoSync(
        hns_wallet::mojom::KeyringId::kSolana);
...
SolanaProviderImpl::GetDeserializedMessage(
  ...
  // implementation. See https://github.com/hnsvn/hns-browser/issues/23542
  // for more details.
...
void SolanaProviderImpl::ContinueSignTransaction(
  ...
  hns_wallet_service_->AddSignTransactionRequest(
      std::move(request),
...
void SolanaProviderImpl::ContinueSignAllTransactions(
  ...
  hns_wallet_service_->AddSignAllTransactionsRequest(
      std::move(request),
...
void SolanaProviderImpl::OnContentSettingChanged(
  ...
  if (content_type != ContentSettingsType::HNS_SOLANA) {
    return;
  }
  auto account = keyring_service_->GetSelectedSolanaDappAccount();
  if (!account) {
