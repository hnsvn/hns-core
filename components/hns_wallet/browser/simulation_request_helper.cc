...
#include "hns/components/hns_wallet/browser/simulation_request_helper.h"
...
base::Value::Dict GetMetadata(const mojom::OriginInfoPtr& origin_info) {
  ...
  if (origin_info && origin_info->origin_spec != "chrome://wallet" &&
      origin_info->origin_spec != "hns://wallet") {
    metadata_object.Set("origin", origin_info->origin_spec);
  } else {
    // TODO(onyb): We use "https://hns.com" as the default origin for now.
    //  This is because Blowfish doesn't support "chrome://wallet" and
    //  "hns://wallet" as origins yet. We'll update this once Blowfish
    //  supports them.
    metadata_object.Set("origin", "https://hns.com");
  }
