...
#include "hns/components/hns_wallet/browser/eth_data_builder.h"
#include "hns/components/hns_wallet/browser/eth_requests.h"
#include "hns/components/hns_wallet/browser/eth_response_parser.h"
#include "hns/components/hns_wallet/browser/fil_requests.h"
#include "hns/components/hns_wallet/browser/fil_response_parser.h"
#include "hns/components/hns_wallet/browser/json_rpc_requests_helper.h"
#include "hns/components/hns_wallet/browser/json_rpc_response_parser.h"
#include "hns/components/hns_wallet/browser/nft_metadata_fetcher.h"
#include "hns/components/hns_wallet/browser/pref_names.h"
#include "hns/components/hns_wallet/browser/solana_keyring.h"
#include "hns/components/hns_wallet/browser/solana_requests.h"
#include "hns/components/hns_wallet/browser/solana_response_parser.h"
#include "hns/components/hns_wallet/browser/unstoppable_domains_dns_resolve.h"
#include "hns/components/hns_wallet/browser/unstoppable_domains_multichain_calls.h"
...
#include "hns/components/json/rs/src/lib.rs.h"
...
using hns_wallet::mojom::ResolveMethod;
  ...
  return net::DefineNetworkTrafficAnnotation("json_rpc_service", R"(
      semantics {
        ...
        description:
          ...
          "on behalf of the user interacting with the native Hns wallet."
        trigger:
...
net::NetworkTrafficAnnotationTag GetENSOffchainNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("json_rpc_service", R"(
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable ENS on hns://settings/extensions page."
        policy_exception_justification:
          "Not implemented."
      }
    )");
...
hns_wallet::mojom::ResolveMethod ToMojomResolveMethod(
...
decentralized_dns::ResolveMethodTypes FromMojomResolveMethod(
    hns_wallet::mojom::ResolveMethod method) {
  switch (method) {
...
hns_wallet::mojom::ResolveMethod ToMojomEnsOffchainResolveMethod(
...
T* GetCustomEVMNetworkFromPrefsDict(const std::string& chain_id,
  ...
  auto* custom_evm_networks =
      custom_networks.FindList(hns_wallet::kEthereumPrefKey);
...
void JsonRpcService::MigrateMultichainNetworks(PrefService* prefs) {
  // custom networks
  if (prefs->HasPrefPath(kHnsWalletCustomNetworksDeprecated)) {
    const auto& custom_networks =
        prefs->GetList(kHnsWalletCustomNetworksDeprecated);

    base::Value::Dict new_custom_networks;
    new_custom_networks.Set(kEthereumPrefKey, custom_networks.Clone());

    prefs->SetDict(kHnsWalletCustomNetworks, std::move(new_custom_networks));

    prefs->ClearPref(kHnsWalletCustomNetworksDeprecated);
  }
  // selected networks
  if (prefs->HasPrefPath(kHnsWalletCurrentChainId)) {
    const std::string chain_id = prefs->GetString(kHnsWalletCurrentChainId);
    ScopedDictPrefUpdate update(prefs, kHnsWalletSelectedNetworks);
    update->Set(kEthereumPrefKey, chain_id);
    prefs->ClearPref(kHnsWalletCurrentChainId);
  }
}

// static
void JsonRpcService::MigrateDeprecatedEthereumTestnets(PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletDeprecateEthereumTestNetworksMigrated)) {
    return;
  }

  if (prefs->HasPrefPath(kHnsWalletSelectedNetworks)) {
    ScopedDictPrefUpdate update(prefs, kHnsWalletSelectedNetworks);
  ...
  prefs->SetBoolean(kHnsWalletDeprecateEthereumTestNetworksMigrated, true);
}
...
void JsonRpcService::MaybeUpdateIsEip1559(const std::string& chain_id) {
  ...
  if (chain_id != hns_wallet::mojom::kLocalhostChainId &&
      !CustomChainExists(prefs_, chain_id, mojom::CoinType::ETH)) {
    return;
  }
...
void JsonRpcService::UpdateIsEip1559(const std::string& chain_id,
  ...
  if (chain_id == hns_wallet::mojom::kLocalhostChainId) {
    ...
    // TODO(apaymyshev): move all work with kHnsWalletCustomNetworks into
    // one file.
    ...
    // details.
    auto& custom_networks = prefs_->GetDict(kHnsWalletCustomNetworks);
    auto* custom_network =
    ...
    if (changed) {
      ScopedDictPrefUpdate update(prefs_, kHnsWalletCustomNetworks);
...
void JsonRpcService::GetCustomNetworks(mojom::CoinType coin,
  ...
  for (const auto& it : hns_wallet::GetAllCustomChains(prefs_, coin)) {
...
void JsonRpcService::GetKnownNetworks(mojom::CoinType coin,
  ...
  std::vector<std::string> chain_ids;
  for (const auto& it : hns_wallet::GetAllKnownChains(prefs_, coin)) {
    chain_ids.push_back(it->chain_id);
  }
  std::move(callback).Run(std::move(chain_ids));
}
...
void JsonRpcService::GetHiddenNetworks(mojom::CoinType coin,
  ...
  auto hidden_networks = hns_wallet::GetHiddenNetworks(prefs_, coin);
...
void JsonRpcService::AddHiddenNetwork(mojom::CoinType coin,
  ...
  hns_wallet::AddHiddenNetwork(prefs_, coin, chain_id);
...
void JsonRpcService::RemoveHiddenNetwork(mojom::CoinType coin,
  ...
  hns_wallet::RemoveHiddenNetwork(prefs_, coin, chain_id);
...
void JsonRpcService::EnsRegistryGetResolver(const std::string& domain,
  ...
  const std::string contract_address =
      GetEnsRegistryContractAddress(hns_wallet::mojom::kMainnetChainId);
...
void JsonRpcService::SnsGetSolAddr(const std::string& domain,
  ...
  if (!base::FeatureList::IsEnabled(features::kHnsWalletSnsFeature)) {
    std::move(callback).Run(
