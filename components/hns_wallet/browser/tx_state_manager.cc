...
void TxStateManager::MigrateAddChainIdToTransactionInfo(PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletTransactionsChainIdMigrated)) {
    return;
  }
  if (!prefs->HasPrefPath(kHnsWalletTransactions)) {
    prefs->SetBoolean(kHnsWalletTransactionsChainIdMigrated, true);
    return;
  }

  ScopedDictPrefUpdate txs_update(prefs, kHnsWalletTransactions);
...
void TxStateManager::MigrateSolanaTransactionsForV0TransactionsSupport(
    PrefService* prefs) {
  if (prefs->GetBoolean(kHnsWalletSolanaTransactionsV0SupportMigrated)) {
    return;
  }

  if (!prefs->HasPrefPath(kHnsWalletTransactions)) {
    prefs->SetBoolean(kHnsWalletSolanaTransactionsV0SupportMigrated, true);
    return;
  }
