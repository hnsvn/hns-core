...
#include "hns/components/hns_wallet/common/hns_wallet.mojom-shared.h"
...
std::vector<hns_wallet::mojom::NetworkInfoPtr> GetAllEthCustomChains(
    PrefService* prefs) {
  return GetAllCustomChains(prefs, hns_wallet::mojom::CoinType::ETH);
}

}  // namespace

TEST(HnsWalletUtilsUnitTest, Mnemonic) {
  const struct {
    const char* entropy;
    const char* mnemonic;
    const char* seed;
...
TEST(HnsWalletUtilsUnitTest, MnemonicToSeedAndEntropy) {
...
TEST(HnsWalletUtilsUnitTest, IsValidMnemonic) {
  EXPECT_TRUE(
...
TEST(HnsWalletUtilsUnitTest, EncodeString) {
  std::string output;
...
TEST(HnsWalletUtilsUnitTest, EncodeStringArray) {
...
TEST(HnsWalletUtilsUnitTest, DecodeString) {
  std::string output;
...
TEST(HnsWalletUtilsUnitTest, TransactionReceiptAndValue) {
...
TEST(HnsWalletUtilsUnitTest, GetAllCustomChainsTest) {
  for (auto coin : kAllCoins) {
    SCOPED_TRACE(coin);
    TestingPrefServiceSimple prefs;
    prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
...
TEST(HnsWalletUtilsUnitTest, KnownChainExists) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
...
TEST(HnsWalletUtilsUnitTest, CustomChainExists) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
...
TEST(HnsWalletUtilsUnitTest, CustomChainsExist) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
...
TEST(HnsWalletUtilsUnitTest, GetAllChainsTest) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
...
TEST(HnsWalletUtilsUnitTest, GetNetworkURLTest) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
  ...
  for (const auto& chain : GetAllKnownChains(&prefs, mojom::CoinType::ETH)) {
    // Hns proxies should have infura key added to path.
    GURL rpc_url(chain->rpc_endpoints.front());
    if (base::EndsWith(rpc_url.host(), "hns.com")) {
  ...
  EXPECT_EQ(GURL("https://mainnet-beta-solana.hns.com/rpc"),
...
TEST(HnsWalletUtilsUnitTest, GetNetworkURLForKnownChains) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);

  // GetNetworkURL for these known chains should resolve to hns subdomain.
  base::flat_set<std::string> infura_chains = {
      hns_wallet::mojom::kMainnetChainId,
      hns_wallet::mojom::kPolygonMainnetChainId,
      hns_wallet::mojom::kOptimismMainnetChainId,
      hns_wallet::mojom::kAuroraMainnetChainId,
      hns_wallet::mojom::kAvalancheMainnetChainId,
      hns_wallet::mojom::kGoerliChainId,
      hns_wallet::mojom::kSepoliaChainId};

  for (const auto& chain : GetAllKnownChains(&prefs, mojom::CoinType::ETH)) {
    auto network_url =
        GetNetworkURL(&prefs, chain->chain_id, mojom::CoinType::ETH);
    EXPECT_EQ(base::EndsWith(network_url.host(), ".hns.com"),
              infura_chains.contains(chain->chain_id));
  }
}

TEST(HnsWalletUtilsUnitTest, GetSolanaSubdomainForKnownChainId) {
  for (const auto& chain : GetAllKnownChains(nullptr, mojom::CoinType::SOL)) {
    ...
    bool expected = (chain->chain_id == hns_wallet::mojom::kLocalhostChainId);
    ASSERT_EQ(subdomain.empty(), expected);
  }
}

TEST(HnsWalletUtilsUnitTest, GetFilecoinSubdomainForKnownChainId) {
...
TEST(HnsWalletUtilsUnitTest, GetBitcoinSubdomainForKnownChainId) {
...
TEST(HnsWalletUtilsUnitTest, GetKnownChain) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);

  const base::flat_set<std::string> non_eip1559_networks = {
      hns_wallet::mojom::kLocalhostChainId,
      hns_wallet::mojom::kBinanceSmartChainMainnetChainId,
      hns_wallet::mojom::kAuroraMainnetChainId,
      hns_wallet::mojom::kNeonEVMMainnetChainId};
  ...
  auto network = GetKnownChain(&prefs, hns_wallet::mojom::kLocalhostChainId,
                               mojom::CoinType::ETH);
  EXPECT_EQ(network->is_eip1559, true);
}

TEST(HnsWalletUtilsUnitTest, GetCustomChain) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
...
TEST(HnsWalletUtilsUnitTest, GetChain) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
  ...
  mojom::NetworkInfo sol_mainnet(
      hns_wallet::mojom::kSolanaMainnet, "Solana Mainnet Beta",
      {"https://explorer.solana.com/"}, {}, 0,
      {GURL("https://mainnet-beta-solana.hns.com/rpc")}, "SOL", "Solana", 9,
      hns_wallet::mojom::CoinType::SOL, {mojom::KeyringId::kSolana}, false);
  ...
  mojom::NetworkInfo fil_mainnet(
      hns_wallet::mojom::kFilecoinMainnet, "Filecoin Mainnet",
      {"https://filscan.io/tipset/message-detail"}, {}, 0,
      {GURL("https://api.node.glif.io/rpc/v0")}, "FIL", "Filecoin", 18,
      hns_wallet::mojom::CoinType::FIL, {mojom::KeyringId::kFilecoin}, false);
...
TEST(HnsWalletUtilsUnitTest, GetAllKnownEthNetworkIds) {
...
TEST(HnsWalletUtilsUnitTest, GetKnownEthNetworkId) {
...
TEST(HnsWalletUtilsUnitTest, GetKnownSolNetworkId) {
...
TEST(HnsWalletUtilsUnitTest, GetKnownFilNetworkId) {
...
TEST(HnsWalletUtilsUnitTest, GetNetworkId) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
...
TEST(HnsWalletUtilsUnitTest, AddCustomNetwork) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
  prefs.registry()->RegisterDictionaryPref(kHnsWalletUserAssets);
  ...
  // kHnsWalletCustomNetworks should be updated with new chains.
  ...
  const auto& assets_pref = prefs.GetDict(kHnsWalletUserAssets);
...
TEST(HnsWalletUtilsUnitTest, CustomNetworkMatchesKnownNetwork) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
  prefs.registry()->RegisterDictionaryPref(kHnsWalletUserAssets);
  ...
  EXPECT_EQ(
      GetNetworkURL(&prefs, mojom::kPolygonMainnetChainId, mojom::CoinType::ETH)
          .GetWithoutFilename(),
      GURL("https://mainnet-polygon.hns.com/"));
...
TEST(HnsWalletUtilsUnitTest, RemoveCustomNetwork) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
  prefs.registry()->RegisterDictionaryPref(kHnsWalletUserAssets);
...
TEST(HnsWalletUtilsUnitTest, HiddenNetworks) {
...
TEST(HnsWalletUtilsUnitTest, GetPrefKeyForCoinType) {
...
TEST(HnsWalletUtilsUnitTest, GetAndSetCurrentChainId) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(HnsWalletUtilsUnitTest, eTLDPlusOne) {
  EXPECT_EQ("", eTLDPlusOne(url::Origin()));
  EXPECT_EQ("hns.com",
            eTLDPlusOne(url::Origin::Create(GURL("https://blog.hns.com"))));
  EXPECT_EQ("hns.com",
            eTLDPlusOne(url::Origin::Create(GURL("https://...hns.com"))));
  EXPECT_EQ(
      "hns.com",
      eTLDPlusOne(url::Origin::Create(GURL("https://a.b.c.d.hns.com/1"))));
  EXPECT_EQ("hns.github.io", eTLDPlusOne(url::Origin::Create(GURL(
                                   "https://a.b.hns.github.io/example"))));
  EXPECT_EQ("", eTLDPlusOne(url::Origin::Create(GURL("https://github.io"))));
}

TEST(HnsWalletUtilsUnitTest, MakeOriginInfo) {
  auto origin_info =
      MakeOriginInfo(url::Origin::Create(GURL("https://blog.hns.com:443")));
  EXPECT_EQ("https://blog.hns.com", origin_info->origin_spec);
  EXPECT_EQ("hns.com", origin_info->e_tld_plus_one);
...
TEST(HnsWalletUtilsUnitTest, GenerateRandomHexString) {
...
TEST(HnsWalletUtilsUnitTest, GetUnstoppableDomainsRpcUrl) {
  EXPECT_EQ(AddInfuraProjectId(GURL("https://mainnet-infura.hns.com")),
            GetUnstoppableDomainsRpcUrl(mojom::kMainnetChainId));
  EXPECT_EQ(AddInfuraProjectId(GURL("https://mainnet-polygon.hns.com")),
            GetUnstoppableDomainsRpcUrl(mojom::kPolygonMainnetChainId));
}

TEST(HnsWalletUtilsUnitTest, GetEnsRpcUrl) {
  EXPECT_EQ(AddInfuraProjectId(GURL("https://mainnet-infura.hns.com")),
            GetEnsRpcUrl());
}

TEST(HnsWalletUtilsUnitTest, GetSnsRpcUrl) {
  EXPECT_EQ(GURL("https://mainnet-beta-solana.hns.com/rpc"), GetSnsRpcUrl());
}

TEST(HnsWalletUtilsUnitTest, GetChainIdByNetworkId) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterDictionaryPref(kHnsWalletCustomNetworks);
  prefs.registry()->RegisterBooleanPref(kSupportEip1559OnLocalhostChain, false);
