...
#include "hns/third_party/bip39wally-core-native/include/wally_bip39.h"
...
const mojom::NetworkInfo* GetSolMainnet() {
  ...
  static base::NoDestructor<mojom::NetworkInfo> network_info(
      {chain_id,
       ...
       {},
       0,
       {GURL("https://mainnet-beta-solana.hns.com/rpc")},
       "SOL",
       "Solana",
       9,
       coin,
...
const base::flat_map<std::string, std::string> kInfuraSubdomains = {
    {hns_wallet::mojom::kMainnetChainId, "mainnet"},
    {hns_wallet::mojom::kGoerliChainId, "goerli"},
    {hns_wallet::mojom::kSepoliaChainId, "sepolia"}};
...
const base::flat_map<std::string, std::string> kSolanaSubdomains = {
    {hns_wallet::mojom::kSolanaMainnet, "mainnet"},
    {hns_wallet::mojom::kSolanaTestnet, "testnet"},
    {hns_wallet::mojom::kSolanaDevnet, "devnet"}};

const base::flat_map<std::string, std::string> kFilecoinSubdomains = {
    {hns_wallet::mojom::kFilecoinMainnet, "mainnet"},
    {hns_wallet::mojom::kFilecoinTestnet, "testnet"}};
...
const base::Value::List* GetCustomNetworksList(PrefService* prefs,
                                               mojom::CoinType coin) {
  const auto& custom_networks = prefs->GetDict(kHnsWalletCustomNetworks);
...
std::string GetCurrentChainId(PrefService* prefs, mojom::CoinType coin) {
  const auto& selected_networks = prefs->GetDict(kHnsWalletSelectedNetworks);
  ...
  if (coin == mojom::CoinType::ETH) {
    for (const auto* network : GetKnownEthNetworks()) {
      ...
      if (prefs && base::CompareCaseInsensitiveASCII(
                       chain_id, hns_wallet::mojom::kLocalhostChainId) == 0) {
...
mojom::NetworkInfoPtr GetCustomChain(PrefService* prefs,
  ...
  for (const auto& it : *custom_list) {
    if (auto opt_chain_id =
            hns_wallet::ExtractChainIdFromValue(it.GetIfDict())) {
      if (base::CompareCaseInsensitiveASCII(chain_id, *opt_chain_id) == 0) {
        return hns_wallet::ValueToNetworkInfo(it);
      }
    }
  }
  return nullptr;
...
GURL GetInfuraURLForKnownChainId(const std::string& chain_id) {
  auto endpoint = hns_wallet::GetInfuraEndpointForKnownChainId(chain_id);
  if (!endpoint.empty()) {
    return GURL(endpoint);
  }

  auto subdomain = hns_wallet::GetInfuraSubdomainForKnownChainId(chain_id);
  if (subdomain.empty()) {
    return GURL();
  }
  return GURL(
      base::StringPrintf("https://%s-infura.hns.com/", subdomain.c_str()));
}
...
void UpdateLastUnlockPref(PrefService* prefs) {
  prefs->SetTime(kHnsWalletLastUnlockTime, base::Time::Now());
}
...
std::vector<std::string> GetAllKnownBtcNetworkIds() {
  std::vector<std::string> network_ids;
  for (const auto* network : GetKnownBitcoinNetworks()) {
    std::string network_id = GetKnownBtcNetworkId(network->chain_id);
    if (!network_id.empty()) {
      network_ids.push_back(network_id);
    }
  }
...
mojom::DefaultWallet GetDefaultEthereumWallet(PrefService* prefs) {
  return static_cast<hns_wallet::mojom::DefaultWallet>(
...
std::vector<std::string> GetHiddenNetworks(PrefService* prefs,
  ...
  const auto& hidden_networks = prefs->GetDict(kHnsWalletHiddenNetworks);
...
bool SetCurrentChainId(PrefService* prefs,
  ...
  if (!origin) {
    ...
    if (origin->scheme() == url::kHttpScheme ||
      ...
      ScopedDictPrefUpdate update(prefs, kHnsWalletSelectedNetworksPerOrigin);
