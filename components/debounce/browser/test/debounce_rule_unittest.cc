...
#include "hns/components/debounce/browser/debounce_rule.h"
...
TEST(DebounceRuleUnitTest, CheckBaseCase) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(),
                     GURL("https://test.com/https://hns.com/test/abc.jpg"),
                     "https://hns.com/test/abc.jpg", false);
  }
}
...
TEST(DebounceRuleUnitTest, MalformedParam) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/https://hns.com"), "",
                     true);
  }
}
...
TEST(DebounceRuleUnitTest, ParamCapturesNonURLNoPrependScheme) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/hns.com"), "", true);
  }
}
...
TEST(DebounceRuleUnitTest, ParamCapturesNonURLWithPrependScheme) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/hns.com"),
                     "http://hns.com/", false);
  }
}
...
TEST(DebounceRuleUnitTest, TwoCaptureGroups) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/hns.com/xyz/abc.jpg"),
                     "https://hns.com/abc.jpg", false);
  }
}
...
TEST(DebounceRuleUnitTest, ThreeCaptureGroups) {
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(
        rule.get(),
        GURL("https://test.com/turbo/hns.com/xyz/2022/xyzzy/abc.jpg"),
        "https://hns.com/2022/abc.jpg", false);
  }
}
...
TEST(DebounceRuleUnitTest, PrefToggle) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterBooleanPref("hns.de_amp.enabled", false);
  const std::string contents = R"json(
  ...
  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/hns.com"), "", &prefs,
                     true);
  }
  prefs.SetBoolean("hns.de_amp.enabled", true);

  for (const std::unique_ptr<DebounceRule>& rule : rules) {
    CheckApplyResult(rule.get(), GURL("https://test.com/http://hns.com"),
                     "http://hns.com/", &prefs, false);
  }
}
...
TEST(DebounceRuleUnitTest, ArbitrarySubdomainInInclude) {
  TestingPrefServiceSimple prefs;
  prefs.registry()->RegisterBooleanPref("hns.de_amp.enabled", true);
...
TEST(DebounceRuleUnitTest, UrlEncodedDestinationUrl) {
  ...
  // From https://github.com/hnsvn/hns-browser/issues/22429
  auto* tracker_url =
