...
HnsReferralsService::ReferralInitializedCallback*
...
base::Value::Dict CreateReferralHeader(
  ...
  headers_sub_dict.Set(kHnsPartnerHeader, partner_name);
...
std::string BuildReferralEndpoint(const std::string& path) {
  ...
  env->GetVar("HNS_REFERRALS_SERVER", &referral_server);
  if (referral_server.empty())
    referral_server = kHnsReferralsServer;
  if (env->HasVar("HNS_REFERRALS_LOCAL"))
    proto = "http";
...
HnsReferralsHeaders::HnsReferralsHeaders() {
  // NOTE TO READER: This header is for partners to determine the browser is
  // Hns without creating webcompat issues by creating a new user-agent.
  // All Hns users send the exact same header and so this cannot be used for
  // tracking individual users. See
  // https://github.com/hnsvn/hns-browser/wiki/Custom-Headers for more
  // information. Custom headers are deprecated and new partners use the
  // navigator.hns.isHns() JavaScript API.
...
bool HnsReferralsHeaders::GetMatchingReferralHeaders(
...
HnsReferralsService::HnsReferralsService(PrefService* pref_service,
...
HnsReferralsService::~HnsReferralsService() = default;

void HnsReferralsService::Start() {
  if (initialized_)
    return;
  ...
  finalization_checks_timer_->Start(
      FROM_HERE,
      base::Seconds(
          hns_base::random::Geometric(kFinalizationChecksFrequency)),
      this, &HnsReferralsService::OnFinalizationChecksTimerFired);
  ...
  if (!checked_for_promo_code_file && !has_initialized && download_id.empty()) {
    ...
    task_runner_->PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&HnsReferralsService::OnReadPromoCodeComplete,
...
// static
void HnsReferralsService::SetReferralInitializedCallbackForTesting(
...
// static
bool HnsReferralsService::IsDefaultReferralCode(const std::string& code) {
  return code == kDefaultPromoCode;
}

void HnsReferralsService::OnFinalizationChecksTimerFired() {
  PerformFinalizationChecks();
}

void HnsReferralsService::OnReferralInitLoadComplete(
  ...
  if (referral_init_loader_->NetError() != net::OK || response_code < 200 ||
    ...
    initialization_timer_->Start(FROM_HERE,
                                 base::Seconds(hns_base::random::Geometric(
                                     kReportInitializationFrequency)),
                                 this, &HnsReferralsService::InitReferral);
...
void HnsReferralsService::OnReferralFinalizationCheckLoadComplete(
    std::unique_ptr<std::string> response_body) {
  int response_code = -1;
...
void HnsReferralsService::OnReadPromoCodeComplete(
...
void HnsReferralsService::GetFirstRunTime() {
#if BUILDFLAG(IS_ANDROID)
  ...
  task_runner_->PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&HnsReferralsService::SetFirstRunTime,
                     weak_factory_.GetWeakPtr()));
#endif
}

void HnsReferralsService::SetFirstRunTime(
...
void HnsReferralsService::PerformFinalizationChecks() {
  // Delete the promo code preference, if appropriate.
  content::GetUIThreadTaskRunner({})->PostTask(
      FROM_HERE,
      base::BindOnce(&HnsReferralsService::MaybeDeletePromoCodePref,
                     weak_factory_.GetWeakPtr()));

  // Check for referral finalization, if appropriate.
  content::GetUIThreadTaskRunner({})->PostTask(
      FROM_HERE,
      base::BindOnce(&HnsReferralsService::MaybeCheckForReferralFinalization,
                     weak_factory_.GetWeakPtr()));
}

// static
void HnsReferralsService::SetPromoFilePathForTesting(
    const base::FilePath& path) {
  g_promo_file_path = path;
}

base::FilePath HnsReferralsService::GetPromoCodeFileName() const {
...
void HnsReferralsService::MaybeCheckForReferralFinalization() {
  ...
  env->GetVar("HNS_REFERRALS_CHECK_TIME", &check_time_str);
...
void HnsReferralsService::MaybeDeletePromoCodePref() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  uint64_t delete_time = 90 * 24 * 60 * 60;
  std::unique_ptr<base::Environment> env(base::Environment::Create());
  std::string delete_time_str;
  env->GetVar("HNS_REFERRALS_DELETE_TIME", &delete_time_str);
...
std::string HnsReferralsService::BuildReferralInitPayload() const {
...
std::string HnsReferralsService::BuildReferralFinalizationCheckPayload()
    const {
  base::Value::Dict root;
  root.Set("api_key", api_key_);
...
void HnsReferralsService::InitReferral() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("hns_referral_initializer", R"(
        semantics {
          sender:
            "Hns Referrals Service"
          description:
            "Validates the current referral offer with Hns, potentially "
            "unlocking special features and/or services."
          trigger:
            "On startup, sends the current referral code to Hns."
          data: "Hns referral metadata."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url = GURL(BuildReferralEndpoint(kHnsReferralsInitPath));
  ...
  referral_init_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(&HnsReferralsService::OnReferralInitLoadComplete,
                     base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

#if BUILDFLAG(IS_ANDROID)
void HnsReferralsService::GetSafetynetStatusResult(
...
void HnsReferralsService::CheckForReferralFinalization() {
#if BUILDFLAG(IS_ANDROID)
  if (pref_service_->GetString(kSafetynetStatus).empty()) {
    // Get safetynet status before finalization
    safetynet_check::ClientAttestationCallback attest_callback =
        base::BindOnce(&HnsReferralsService::GetSafetynetStatusResult,
                       weak_factory_.GetWeakPtr());
...
#endif
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("hns_referral_finalization_checker",
                                          R"(
        semantics {
          sender:
            "Hns Referrals Service"
          description:
            "Fetches referral finalization data from Hns."
          trigger:
            ""
          data: "Hns referral finalization status."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url =
      GURL(BuildReferralEndpoint(kHnsReferralsActivityPath));
  resource_request->credentials_mode = network::mojom::CredentialsMode::kOmit;
  resource_request->load_flags = net::LOAD_DO_NOT_SAVE_COOKIES |
                                 net::LOAD_BYPASS_CACHE |
                                 net::LOAD_DISABLE_CACHE;
  ...
  referral_finalization_check_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(
          &HnsReferralsService::OnReferralFinalizationCheckLoadComplete,
          base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

#if BUILDFLAG(IS_ANDROID)
void HnsReferralsService::InitAndroidReferrer() {
  android_hns_referrer::InitReferrerCallback init_referrer_callback =
      base::BindOnce(&HnsReferralsService::OnAndroidHnsReferrerReady,
                     weak_factory_.GetWeakPtr());
  android_hns_referrer_.InitReferrer(std::move(init_referrer_callback));
}

void HnsReferralsService::OnAndroidHnsReferrerReady() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  task_runner_->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&ReadPromoCode, GetPromoCodeFileName()),
      base::BindOnce(&HnsReferralsService::OnReadPromoCodeComplete,
