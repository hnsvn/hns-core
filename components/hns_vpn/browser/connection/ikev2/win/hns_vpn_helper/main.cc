...
#include "hns/components/hns_vpn/browser/connection/ikev2/win/hns_vpn_helper/service_main.h"
...
int main(int argc, char* argv[]) {
  ...
  HnsVPNHelperCrashReporterClient::InitializeCrashReportingForProcess(
      process_type);
  ...
  if (command_line->HasSwitch(hns_vpn::kHnsVpnHelperInstall)) {
    auto success = hns_vpn::ConfigureServiceAutoRestart(
        hns_vpn::GetHnsVpnHelperServiceName(),
        hns_vpn::GetHnsVPNConnectionName());
    return success ? 0 : 1;
  }

  // Run the service.
  hns_vpn::ServiceMain* service = hns_vpn::ServiceMain::GetInstance();
