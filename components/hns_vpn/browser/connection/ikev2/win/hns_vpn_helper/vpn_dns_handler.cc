...
VpnDnsHandler::VpnDnsHandler(HnsVpnDnsDelegate* delegate)
    : delegate_(delegate) {
  DCHECK(delegate_);
}
...
ras::CheckConnectionResult VpnDnsHandler::GetVpnEntryStatus() {
  ...
  return ras::CheckConnection(GetHnsVPNConnectionName());
}
...
void VpnDnsHandler::DisconnectVPN() {
  ...
  auto result = ras::DisconnectEntry(GetHnsVPNConnectionName());
  if (!result.success) {
...
void VpnDnsHandler::UpdateFiltersState() {
  ...
  switch (GetVpnEntryStatus()) {
    case ras::CheckConnectionResult::CONNECTED:
      VLOG(1) << "HnsVPN connected, set filters";
      if (IsActive()) {
        VLOG(1) << "Filters are already installed";
        return;
      }
      if (!SetFilters(GetHnsVPNConnectionName())) {
    ...
    case ras::CheckConnectionResult::DISCONNECTED:
      VLOG(1) << "HnsVPN Disconnected, remove filters";
      if (!RemoveFilters(GetHnsVPNConnectionName())) {
        VLOG(1) << "Failed to remove DNS filters";
        Exit();
        break;
      }
      ...
      break;
    default:
      VLOG(1) << "HnsVPN is connecting, try later after "
