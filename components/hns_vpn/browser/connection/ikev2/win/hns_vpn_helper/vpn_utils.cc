...
#include "hns/components/hns_vpn/browser/connection/ikev2/win/hns_vpn_helper/vpn_utils.h"
...
bool SetServiceTriggerForVPNConnection(SC_HANDLE hService,
  ...
  std::wstring hns_vpn_entry_with_null(hns_vpn_entry);
  hns_vpn_entry_with_null += L'\0';
  ...
  // Exclude EOL
  deviceData.cbData = hns_vpn_entry_with_null.size() *
                      sizeof(hns_vpn_entry_with_null.front());
  deviceData.pData = (PBYTE)hns_vpn_entry_with_null.c_str();
...
DWORD AddSublayer(GUID uuid) {
  ...
  if (result == ERROR_SUCCESS) {
    std::wstring name(kHnsVPNServiceFilter);
...
DWORD RegisterSublayer(HANDLE engine_handle, GUID uuid) {
  ...
  if (result != (DWORD)FWP_E_ALREADY_EXISTS && result != ERROR_SUCCESS) {
    VLOG(1) << "Failed to add a persistent sublayer with "
               "HNSVPN_DNS_SUBLAYER UUID";
    return result;
  }
  VLOG(1) << "Added a persistent sublayer with HNSVPN_DNS_SUBLAYER UUID";
  return ERROR_SUCCESS;
}
...
bool ConfigureServiceAutoRestart(const std::wstring& service_name,
  ...
  if (!hns_vpn::SetServiceFailureActions(service.Get())) {
    LOG(ERROR) << "SetServiceFailureActions failed:" << std::hex
               << HRESULTFromLastError();
    return false;
  }
  if (!SetServiceTriggerForVPNConnection(service.Get(), hns_vpn_entry)) {
...
void SetFiltersInstalledFlag() {
  base::win::RegKey key(HKEY_LOCAL_MACHINE, kHnsVpnHelperRegistryStoragePath,
                        KEY_ALL_ACCESS);
  if (!key.Valid()) {
  ...
  DWORD launch = 1;
  key.WriteValue(kHnsVpnHelperFiltersInstalledValue, launch);
}
...
void ResetFiltersInstalledFlag() {
  ...
  key.DeleteValue(kHnsVpnHelperFiltersInstalledValue);
}
