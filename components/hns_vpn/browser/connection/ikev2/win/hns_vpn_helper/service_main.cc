...
int ServiceMain::RunAsService() {
  const std::wstring& service_name(hns_vpn::GetHnsVpnHelperServiceName());
...
void ServiceMain::ServiceMainImpl() {
  VLOG(1) << __func__ << " HnsVPN Service started";
  service_status_handle_ = ::RegisterServiceCtrlHandler(
      hns_vpn::GetHnsVpnHelperServiceName().c_str(),
...
HRESULT ServiceMain::Run() {
  ...
  if (command_line && command_line->HasSwitch(kHnsVpnHelperCrashMe)) {
    CHECK(!command_line->HasSwitch(kHnsVpnHelperCrashMe))
        << "--crash-me was used.";
  }
