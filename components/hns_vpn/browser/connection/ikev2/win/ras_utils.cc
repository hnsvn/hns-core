...
RasOperationResult DisconnectEntry(const std::wstring& entry_name) {
  ...
  if (connection_result == CheckConnectionResult::DISCONNECTING) {
    VLOG(2) << __func__
            << " Don't try to disconnect while hns vpn entry is already in "
               "disconnecting state";
...
RasOperationResult SetConnectionParamsUsingPowerShell(
  ...
  auto result = hns::ProcessLauncher::ReadAppOutput(power_shell, options, 10);
  if (!result.has_value()) {
...
RasOperationResult CreateEntry(const HnsVPNConnectionInfo& info) {
  ...
  if (connection_result == CheckConnectionResult::CONNECTING ||
      connection_result == CheckConnectionResult::CONNECTED) {
    VLOG(2) << __func__
            << " Don't try to create entry when hns vpn entry is in "
  ...
  // `eventvwr`:
  // >> The user DESKTOP - DRCJVG6\brian dialed a connection named HNSVPN
