...
const char kProfileCredentialData[] = R"(
        {
          "eap-username": "hns-user",
          "eap-password": "hns-pwd"
        }
      )";
const char kHostNamesTestData[] = R"([
        {
          "hostname": "host-1.hns.com",
          "display-name": "host-1",
          "offline": false,
          "capacity-score": 0
        },
        {
          "hostname": "host-2.hns.com",
          "display-name": "host-2",
          "offline": false,
          "capacity-score": 1
        },
        {
          "hostname": "host-3.hns.com",
          "display-name": "Singapore",
          "offline": false,
          "capacity-score": 0
        },
        {
          "hostname": "host-4.hns.com",
          "display-name": "host-4",
          "offline": false,
          "capacity-score": 0
        },
        {
          "hostname": "host-5.hns.com",
          "display-name": "host-5",
          "offline": false,
          "capacity-score": 1
        }
...
class HnsVPNOSConnectionAPIUnitTest : public testing::Test {
 public:
  HnsVPNOSConnectionAPIUnitTest()
  ...
  void OnFetchRegionList(const std::string& region_list, bool success) {
    GetHnsVPNConnectionAPIBase()->GetRegionDataManager().OnFetchRegionList(
        region_list, success);
  }

  void OnFetchTimezones(const std::string& timezones_list, bool success) {
    GetHnsVPNConnectionAPIBase()->GetRegionDataManager().OnFetchTimezones(
  ...
  HnsVPNOSConnectionAPIBase* GetHnsVPNConnectionAPIBase() const {
    return static_cast<HnsVPNOSConnectionAPIBase*>(connection_api_.get());
  }

  void SetFallbackDeviceRegion() {
    GetHnsVPNConnectionAPIBase()
        ->GetRegionDataManager()
        .SetFallbackDeviceRegion();
  }

  void SetTestTimezone(const std::string& timezone) {
    GetHnsVPNConnectionAPIBase()->GetRegionDataManager().test_timezone_ =
        timezone;
  }
  ...
  void ClearRegions() {
    GetHnsVPNConnectionAPIBase()->GetRegionDataManager().regions_.clear();
  }

  bool NeedToUpdateRegionData() {
    return GetHnsVPNConnectionAPIBase()
        ->GetRegionDataManager()
  ...
  mojom::Region device_region() {
    if (auto region_ptr =
            GetRegionPtrWithNameFromRegionList(GetHnsVPNConnectionAPIBase()
                                                   ->GetRegionDataManager()
                                                   .GetDeviceRegion(),
                                               regions())) {
      return *region_ptr;
  ...
  const std::vector<mojom::Region>& regions() {
    return GetHnsVPNConnectionAPIBase()->GetRegionDataManager().GetRegions();
  }
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, LoadRegionDataFromPrefsTest) {
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, RegionDataTest) {
  // Initially, prefs doesn't have region data.
  EXPECT_EQ(mojom::Region(), device_region());
  EXPECT_TRUE(regions().empty());
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, NeedToUpdateRegionDataTest) {
...
// connection info.
TEST_F(HnsVPNOSConnectionAPIUnitTest,
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, CreateOSVPNEntryWithInvalidInfoTest) {
  // Prepare region data before asking connect.
  OnFetchRegionList(GetRegionsData(), true);

  GetConnectionAPI()->CheckConnection();
  local_state()->SetString(prefs::kHnsVPNSelectedRegion, "region-a");
  // Prepare valid connection info.
  auto* test_api = static_cast<HnsVPNOSConnectionAPISim*>(GetConnectionAPI());
  test_api->OnFetchHostnames("region-a", kHostNamesTestData, true);
  test_api->SetPreventCreationForTesting(true);
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, NeedsConnectTest) {
  // Prepare region data before asking connect.
  OnFetchRegionList(GetRegionsData(), true);

  auto* test_api =
      static_cast<HnsVPNOSConnectionAPIBase*>(GetConnectionAPI());

  GetConnectionAPI()->CheckConnection();

  // Check ignore Connect() request while connecting or disconnecting is
  // in-progress.
  local_state()->SetString(prefs::kHnsVPNSelectedRegion, "eu-es");
  ...
  EXPECT_EQ(mojom::ConnectionState::DISCONNECTING,
            test_api->GetConnectionState());
  static_cast<HnsVPNOSConnectionAPISim*>(test_api)
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, HostnamesTest) {
  auto* test_api =
      static_cast<HnsVPNOSConnectionAPIBase*>(GetConnectionAPI());
  ...
  EXPECT_EQ("host-2.hns.com", test_api->hostname_->hostname);
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, ConnectionInfoTest) {
  auto* test_api =
      static_cast<HnsVPNOSConnectionAPIBase*>(GetConnectionAPI());
...
TEST_F(HnsVPNOSConnectionAPIUnitTest, CancelConnectingTest) {
  auto* test_api =
      static_cast<HnsVPNOSConnectionAPIBase*>(GetConnectionAPI());
  ...
  // See the comment of HnsVPNOSConnectionAPIBase::api_request_.
  test_api->cancel_connecting_ = false;
  test_api->connection_state_ = mojom::ConnectionState::CONNECTING;
...
// HnsVPNOSConnectionAPI::UpdateAndNotifyConnectionStateChange().
TEST_F(HnsVPNOSConnectionAPIUnitTest,
