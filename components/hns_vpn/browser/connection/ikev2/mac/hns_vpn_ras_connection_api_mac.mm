...
#include "hns/components/hns_vpn/browser/connection/ikev2/mac/hns_vpn_ras_connection_api_mac.h"
...
const NSString* kHnsVPNKey = @"HnsVPNKey";
...
NSData* GetPasswordRefForAccount() {
  ...
  NSDictionary* query = @{
    ...
    (__bridge id)kSecAttrAccount : kHnsVPNKey,
...
NEVPNProtocolIKEv2* CreateProtocolConfig(const HnsVPNConnectionInfo& info) {
...
std::unique_ptr<HnsVPNOSConnectionAPI> CreateHnsVPNIKEv2ConnectionAPI(
  ...
  return std::make_unique<HnsVPNOSConnectionAPIMac>(url_loader_factory,
                                                      local_prefs, channel);
}

HnsVPNOSConnectionAPIMac::HnsVPNOSConnectionAPIMac(
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory,
    PrefService* local_prefs,
    version_info::Channel channel)
...
void HnsVPNOSConnectionAPIMac::CreateVPNConnectionImpl(
...
void HnsVPNOSConnectionAPIMac::ConnectImpl(const std::string& name) {
  NEVPNManager* vpn_manager = [NEVPNManager sharedManager];
  [vpn_manager loadFromPreferencesWithCompletionHandler:^(NSError* error) {
    if (error) {
      SetLastConnectionError(
...
void HnsVPNOSConnectionAPIMac::DisconnectImpl(const std::string& name) {
  NEVPNManager* vpn_manager = [NEVPNManager sharedManager];
  [vpn_manager loadFromPreferencesWithCompletionHandler:^(NSError* error) {
    if (error) {
...
void HnsVPNOSConnectionAPIMac::CheckConnectionImpl(const std::string& name) {
  NEVPNManager* vpn_manager = [NEVPNManager sharedManager];
  [vpn_manager loadFromPreferencesWithCompletionHandler:^(NSError* error) {
    if (error) {
      LOG(ERROR) << "Connect - loadFromPrefs error: "
...
void HnsVPNOSConnectionAPIMac::ObserveVPNConnectionChange() {
...
bool HnsVPNOSConnectionAPIMac::IsPlatformNetworkAvailable() {
