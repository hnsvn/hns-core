...
mojom::Region GetRegionFromValue(const base::Value::Dict& value) {
  mojom::Region region;
  if (auto* continent = value.FindString(hns_vpn::kRegionContinentKey)) {
    region.continent = *continent;
  }
  if (auto* name = value.FindString(hns_vpn::kRegionNameKey)) {
    region.name = *name;
  }
  if (auto* name_pretty = value.FindString(hns_vpn::kRegionNamePrettyKey)) {
    region.name_pretty = *name_pretty;
  }
  if (auto* country_iso_code =
          value.FindString(hns_vpn::kRegionCountryIsoCodeKey)) {
...
HnsVPNRegionDataManager::HnsVPNRegionDataManager(
...
HnsVPNRegionDataManager::~HnsVPNRegionDataManager() = default;

const std::vector<mojom::Region>& HnsVPNRegionDataManager::GetRegions()
    const {
  return regions_;
}

bool HnsVPNRegionDataManager::IsRegionDataReady() const {
  return !regions_.empty();
}

void HnsVPNRegionDataManager::SetSelectedRegion(const std::string& name) {
  local_prefs_->SetString(prefs::kHnsVPNSelectedRegion, name);
...
std::string HnsVPNRegionDataManager::GetSelectedRegion() const {
  DCHECK(!regions_.empty())
      << "regions data must be prepared before panel asks.";

  auto region_name = local_prefs_->GetString(prefs::kHnsVPNSelectedRegion);
...
std::string HnsVPNRegionDataManager::GetDeviceRegion() const {
  return local_prefs_->GetString(prefs::kHnsVPNDeviceRegion);
}

void HnsVPNRegionDataManager::SetDeviceRegion(const std::string& name) {
  local_prefs_->SetString(prefs::kHnsVPNDeviceRegion, name);
}

void HnsVPNRegionDataManager::SetFallbackDeviceRegion() {
  // Set first item in the region list as a |device_region_| as a fallback.
  DCHECK(!regions_.empty());
  SetDeviceRegion(regions_[0].name);
}

void HnsVPNRegionDataManager::SetDeviceRegionWithTimezone(
  ...
  for (const auto& timezones : timezones_value) {
    ...
    for (const auto& timezone : *timezone_list_value) {
      ...
      if (current_time_zone == timezone.GetString()) {
        ...
        if (local_prefs_->GetString(prefs::kHnsVPNSelectedRegion).empty()) {
...
void HnsVPNRegionDataManager::LoadCachedRegionData() {
  ...
  auto* preference = local_prefs_->FindPreference(prefs::kHnsVPNRegionList);
  DCHECK(preference);
...
bool HnsVPNRegionDataManager::NeedToUpdateRegionData() const {
  ...
  const auto last_fetched_date =
      local_prefs_->GetTime(prefs::kHnsVPNRegionListFetchedDate);
...
void HnsVPNRegionDataManager::NotifyRegionDataReady() const {
  if (region_data_ready_callback_) {
    region_data_ready_callback_.Run(!regions_.empty());
  }
}

void HnsVPNRegionDataManager::FetchRegionDataIfNeeded() {
  if (api_request_) {
  ...
  api_request_->GetAllServerRegions(base::BindOnce(
      &HnsVPNRegionDataManager::OnFetchRegionList, base::Unretained(this)));
}

void HnsVPNRegionDataManager::OnFetchRegionList(
  ...
  if (value && value->is_list() &&
    ...
    api_request_->GetTimezonesForRegions(base::BindOnce(
        &HnsVPNRegionDataManager::OnFetchTimezones, base::Unretained(this)));
    return;
  }
...
bool HnsVPNRegionDataManager::ParseAndCacheRegionList(
...
void HnsVPNRegionDataManager::OnFetchTimezones(
...
void HnsVPNRegionDataManager::SetRegionListToPrefs() {
  DCHECK(!regions_.empty());
  ...
  local_prefs_->Set(prefs::kHnsVPNRegionList,
                    base::Value(std::move(regions_list)));
  local_prefs_->SetTime(prefs::kHnsVPNRegionListFetchedDate,
                        base::Time::Now());
}

std::string HnsVPNRegionDataManager::GetCurrentTimeZone() {
