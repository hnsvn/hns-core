...
TEST(VpnResponseParserUnitTest, ParseSubscriberCredentialFromJson) {
  ...
  EXPECT_TRUE(hns_vpn::ParseSubscriberCredentialFromJson(
                  R"({
    "error-message":
  ...
  EXPECT_EQ(hns_vpn::ParseSubscriberCredentialFromJson(
                R"({"subscriber-credential":"test"})", &error_value),
            "test");
  EXPECT_TRUE(error_value.empty());
  EXPECT_TRUE(
      hns_vpn::ParseSubscriberCredentialFromJson(R"([])", &error_value)
          .empty());
  EXPECT_TRUE(error_value.empty());
  EXPECT_TRUE(
      hns_vpn::ParseSubscriberCredentialFromJson(R"({,})", &error_value)
  ...
  EXPECT_TRUE(error_value.empty());
  EXPECT_TRUE(
      hns_vpn::ParseSubscriberCredentialFromJson("", &error_value).empty());
  EXPECT_TRUE(error_value.empty());
}
