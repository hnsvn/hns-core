...
#include "hns/components/skus/browser/skus_context_impl.h"
#include "hns/components/skus/browser/skus_service_impl.h"
...
class TestHnsVPNServiceObserver : public mojom::ServiceObserver {
 public:
  TestHnsVPNServiceObserver() = default;
...
class HnsVPNServiceTest : public testing::Test {
 public:
  HnsVPNServiceTest()
      : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME) {
    scoped_feature_list_.InitWithFeatures(
        {skus::features::kSkusFeature, features::kHnsVPN}, {});
  }
  ...
  void SetUp() override {
    ...
    hns_vpn::RegisterProfilePrefs(profile_pref_service_.registry());
    shared_url_loader_factory_ =
        base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(
            &url_loader_factory_);
    url_loader_factory_.SetInterceptor(base::BindRepeating(
        &HnsVPNServiceTest::Interceptor, base::Unretained(this)));
...
#if !BUILDFLAG(IS_ANDROID)
    connection_api_ = std::make_unique<HnsVPNOSConnectionAPISim>(
  ...
  void ResetVpnService() {
    ...
    service_ = std::make_unique<HnsVpnService>(
#if !BUILDFLAG(IS_ANDROID)
        connection_api_.get(),
#else
        nullptr,
        ...
        base::BindRepeating(&HnsVPNServiceTest::GetSkusService,
  ...
  void BlockVPNByPolicy(bool value) {
    profile_pref_service_.SetManagedPref(prefs::kManagedHnsVPNDisabled,
                                         base::Value(value));
    EXPECT_EQ(hns_vpn::IsHnsVPNDisabledByPolicy(&profile_pref_service_),
              value);
  }
  ...
  mojom::Region device_region() const {
    if (auto region_ptr =
            GetRegionPtrWithNameFromRegionList(GetHnsVPNConnectionAPI()
  ...
  const std::vector<mojom::Region>& regions() const {
    return GetHnsVPNConnectionAPI()->GetRegionDataManager().GetRegions();
  }

  void SetSelectedRegion(const std::string& region) {
    GetHnsVPNConnectionAPI()->SetSelectedRegion(region);
  }

  void OnFetchRegionList(const std::string& region_list, bool success) {
    GetHnsVPNConnectionAPI()->GetRegionDataManager().OnFetchRegionList(
        region_list, success);
  }

  void OnFetchTimezones(const std::string& timezones_list, bool success) {
    GetHnsVPNConnectionAPI()->GetRegionDataManager().OnFetchTimezones(
        timezones_list, success);
  }

  HnsVPNOSConnectionAPIBase* GetHnsVPNConnectionAPI() const {
    return static_cast<HnsVPNOSConnectionAPIBase*>(service_->connection_api_);
  }
  ...
  void SetDeviceRegion(const std::string& name) {
    GetHnsVPNConnectionAPI()->GetRegionDataManager().SetDeviceRegion(name);
  }

  void SetFallbackDeviceRegion() {
    GetHnsVPNConnectionAPI()
        ->GetRegionDataManager()
        .SetFallbackDeviceRegion();
  }

  void SetTestTimezone(const std::string& timezone) {
    GetHnsVPNConnectionAPI()->GetRegionDataManager().test_timezone_ =
        timezone;
  }
  ...
  void SetAndExpectPurchasedStateChange(TestHnsVPNServiceObserver* observer,
...
#if !BUILDFLAG(IS_ANDROID)
  std::unique_ptr<HnsVPNOSConnectionAPISim> connection_api_;
#endif
  ...
  std::unique_ptr<HnsVpnService> service_;
...
TEST_F(HnsVPNServiceTest, ResponseSanitizingTest) {
...
#if !BUILDFLAG(IS_ANDROID)
TEST_F(HnsVPNServiceTest, SkusCredentialCacheTest) {
...
TEST_F(HnsVPNServiceTest, LoadPurchasedStateSessionExpiredTest) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);
  ...
  auto session_expired_time =
      local_pref_service_.GetTime(prefs::kHnsVPNSessionExpiredDate);
  ...
  // starts.
  SetPurchasedState(env, PurchasedState::LOADING);
  local_pref_service_.SetTime(prefs::kHnsVPNSessionExpiredDate,
...
TEST_F(HnsVPNServiceTest, LoadPurchasedStateTest) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);
  // Service try loading
  SetPurchasedState(env, PurchasedState::LOADING);
...
TEST_F(HnsVPNServiceTest, ResetConnectionStateTest) {
  // Prepare valid connection info.
  auto* test_api = static_cast<HnsVPNOSConnectionAPISim*>(GetConnectionAPI());

  // Set failed state before setting observer.
  test_api->SetConnectionStateForTesting(ConnectionState::CONNECT_FAILED);

  TestHnsVPNServiceObserver observer;
...
TEST_F(HnsVPNServiceTest, ConnectionStateUpdateWithPurchasedStateTest) {
  // Prepare valid connection info.
  auto* test_api = static_cast<HnsVPNOSConnectionAPISim*>(GetConnectionAPI());
...
TEST_F(HnsVPNServiceTest, IsConnectedWithPurchasedStateTest) {
  auto* test_api = static_cast<HnsVPNOSConnectionAPISim*>(GetConnectionAPI());
  ...
  // Check HnsVpnService gives false for IsConnected() when
...
TEST_F(HnsVPNServiceTest, DisconnectedIfDisabledByPolicy) {
  // Prepare valid connection info.
  auto* test_api = static_cast<HnsVPNOSConnectionAPISim*>(GetConnectionAPI());
...
TEST_F(HnsVPNServiceTest, SelectedRegionChangedUpdateTest) {
  TestHnsVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(HnsVPNServiceTest, SelectedRegionChangedUpdateWithDeviceRegionTest) {
  TestHnsVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(HnsVPNServiceTest, LoadPurchasedStateForAnotherEnvFailed) {
...
TEST_F(HnsVPNServiceTest, CheckInitialPurchasedStateTest) {
...
TEST_F(HnsVPNServiceTest, SubscribedCredentialsWithTokenNoLongerValid) {
  std::string env = skus::GetDefaultEnvironment();
...
TEST_F(HnsVPNServiceTest, CheckConnectionStateAfterPurchased) {
  std::string env = skus::GetDefaultEnvironment();
  auto* test_api = static_cast<HnsVPNOSConnectionAPISim*>(GetConnectionAPI());
...
TEST_F(HnsVPNServiceTest, GetPurchasedInfoSync) {
  std::string env = skus::GetDefaultEnvironment();
...
TEST_F(HnsVPNServiceTest, SetPurchasedState) {
  std::string env = skus::GetDefaultEnvironment();
  TestHnsVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
  EXPECT_EQ(PurchasedState::NOT_PURCHASED, GetPurchasedInfoSync());
...
TEST_F(HnsVPNServiceTest, LoadPurchasedStateNotifications) {
  std::string env = skus::GetDefaultEnvironment();
  std::string domain = skus::GetDomain("vpn", env);
  TestHnsVPNServiceObserver observer;
  AddObserver(observer.GetReceiver());
...
TEST_F(HnsVPNServiceTest, LoadPurchasedStateForAnotherEnv) {
  auto development = SetupTestingStoreForEnv(skus::GetDefaultEnvironment());
  EXPECT_EQ(skus::GetEnvironmentForDomain(development),
            skus::GetDefaultEnvironment());
  TestHnsVPNServiceObserver observer;
...
TEST_F(HnsVPNServiceTest, NewUserReturningMetric) {
  RecordP3A(false);
...
TEST_F(HnsVPNServiceTest, DaysInMonthUsedMetric) {
  RecordP3A(false);
...
TEST_F(HnsVPNServiceTest, LastUsageTimeMetric) {
