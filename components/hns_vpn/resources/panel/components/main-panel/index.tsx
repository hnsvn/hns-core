...
import { SettingsAdvancedIcon, CaratStrongRightIcon } from 'hns-ui/components/icons'
...
function SessionExpiredContent () {
  ...
  const message = getLocale('hnsVpnSessionExpiredContent')
      ...
      <S.PanelContent>
        <S.PanelHeader>
          <S.SettingsButton
            ...
            title={getLocale('hnsVpnSettingsTooltip')}
          >
            <SettingsAdvancedIcon />
          </S.SettingsButton>
        </S.PanelHeader>
        <S.PanelTitle>{getLocale('hnsVpn')}</S.PanelTitle>
        <Toggle disabled={expired} />
        {connectionStatus === ConnectionState.CONNECT_NOT_ALLOWED && (
          <S.ConnectNotAllowedNote>
          <div>{getLocale('hnsVpnConnectNotAllowed')}</div>
          </S.ConnectNotAllowedNote>
        )}
        {expired && (
          <S.SessionExpiredNote>
            <S.SessionExpiredNoteTitle>
              {getLocale('hnsVpnSessionExpiredTitle')}
            </S.SessionExpiredNoteTitle>
