...
void VpnRenderFrameObserver::DidCreateScriptContext(
  ...
  if (!connected) {
    LOG(ERROR) << "Failed to establish connection to HnsVpnService";
    return;
  }
  ...
  if (frame) {
    if (IsValueAllowed(purchase_token)) {
      std::u16string set_local_storage =
          base::StrCat({u"window.localStorage.setItem(\"hnsVpn.receipt\", \"",
...
bool VpnRenderFrameObserver::IsValueAllowed(
  ...
  if (purchase_token.length() > 0) {
    ...
    // See https://github.com/hnsvn/hns-browser/issues/27524
...
bool VpnRenderFrameObserver::IsAllowed() {
  DCHECK(hns_vpn::IsHnsVPNFeatureEnabled());
