...
#include "hns/components/webcompat_reporter/buildflags/buildflags.h"
...
void WebcompatReportUploader::SubmitReport(
    ...
    const bool hns_vpn_connected,
    const base::Value& details,
    const base::Value& contact) {
  std::string api_key = hns_stats::GetAPIKey();
  ...
  post_data_obj.Set(kHnsVPNEnabledField, hns_vpn_connected);
...
void WebcompatReportUploader::CreateAndStartURLLoader(
  ...
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("background_performance_tracer", R"(
        semantics {
          sender: "Hns Web Compatibility Reporting"
          description:
            "A user-initiated report of a website that appears broken as a"
            "result of having Hns Shields enabled."
          trigger:
            "Though the 'Report a Broken Site' option of the help menu or"
            "the Hns Shields panel."
          data: "Broken URL, IP address, Shields settings, language settings,"
                "Hns VPN connection status; user provided additional details"
                "and contact information."
          destination: OTHER
          destination_other: "Hns developers"
        }
        policy {
          cookies_allowed: NO
        })");
