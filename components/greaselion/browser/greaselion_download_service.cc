...
#include "hns/components/greaselion/browser/switches.h"
...
// Note(petemill): "hns" instead of "browser" version in order
// to preserve some sense of cross-browser targetting of the scripts.
const char kMinimumHnsVersion[] = "minimum_hns_version";
// precondition keys
...
const char kSupportsMinimumHnsVersion[] =
    "supports-minimum-hns-version";
...
void GreaselionRule::Parse(base::Value::Dict* preconditions_value,
                           ...
                           const std::string& minimum_hns_version_value,
  ...
  if (preconditions_value) {
    for (const auto kv : *preconditions_value) {
      ...
      } else if (kv.first == kSupportsMinimumHnsVersion) {
        preconditions_.supports_minimum_hns_version = condition;
      } else {
  ...
  minimum_hns_version_ = minimum_hns_version_value;
...
bool GreaselionRule::Matches(
  ...
  if (!PreconditionFulfilled(preconditions_.auto_contribution_enabled,
    ...
    return false;
  if (!PreconditionFulfilled(preconditions_.supports_minimum_hns_version,
                          state[greaselion::SUPPORTS_MINIMUM_HNS_VERSION]))
    return false;
  if (!PreconditionFulfilled(preconditions_.ads_enabled,
                             state[greaselion::ADS]))
    return false;
  // Validate against browser version.
  if (base::Version::IsValidWildcardString(minimum_hns_version_)) {
    bool rule_version_is_higher_than_browser =
        (browser_version.CompareToWildcardString(minimum_hns_version_) < 0);
...
void GreaselionDownloadService::LoadDirectlyFromResourcePath() {
  ...
  GetTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE,
      base::BindOnce(&hns_component_updater::GetDATFileAsString,
                     dat_file_path),
...
void GreaselionDownloadService::OnDATFileDataReady(
  ...
  for (base::Value& rule_it : root_list) {
    ...
    const std::string* minimum_hns_version_ptr =
        rule_dict.FindString(kMinimumHnsVersion);
    const std::string minimum_hns_version_value =
        minimum_hns_version_ptr ? *minimum_hns_version_ptr : "";
    ...
    rule->Parse(preconditions_value, urls_value, scripts_value, run_at_value,
        minimum_hns_version_value, messages_path, resource_dir_);
