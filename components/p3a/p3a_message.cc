...
#include "hns/components/p3a/p3a_message.h"
...
base::Value::Dict GenerateP3AMessageDict(base::StringPiece metric_name,
  ...
  base::Time date_of_install_monday =
      hns_stats::GetLastMondayTime(meta.date_of_install);
  ...
  if (log_type != MetricLogType::kSlow) {
    ...
    date_of_survey = hns_stats::GetLastMondayTime(date_of_survey);
  }
  ...
  if (log_type == MetricLogType::kSlow) {
    ...
    result.Set("wos", hns_stats::GetIsoWeekNumber(date_of_survey));
  }
...
void MessageMetainfo::Init(PrefService* local_state,
  ...
  platform = hns_stats::GetPlatformIdentifier();
  channel = hns_channel;
  InitVersion();

  if (!week_of_install.empty()) {
    date_of_install = hns_stats::GetYMDAsDate(week_of_install);
  } else {
    date_of_install = base::Time::Now();
  }
  woi = hns_stats::GetIsoWeekNumber(date_of_install);
