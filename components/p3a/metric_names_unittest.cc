...
// npm run test -- hns_unit_tests --filter=P3AMetrics*
...
TEST(P3AMetrics, Searchable) {
  ...
  EXPECT_TRUE(kCollectedTypicalHistograms.contains("Hns.Core.IsDefault"));
  EXPECT_TRUE(
      kCollectedTypicalHistograms.contains("Hns.P3A.SentAnswersCount"));
  EXPECT_TRUE(
      kCollectedTypicalHistograms.contains("Hns.P2A.TotalAdOpportunities"));
  // Verify set membership testing isn't returning true unconditionally.
  EXPECT_FALSE(kCollectedTypicalHistograms.contains("Not.A.Hns.Metric"));
