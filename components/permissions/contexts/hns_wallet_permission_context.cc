...
#include "hns/components/permissions/permission_lifetime_utils.h"
...
HnsWalletPermissionContext::HnsWalletPermissionContext(
...
HnsWalletPermissionContext::~HnsWalletPermissionContext() = default;

bool HnsWalletPermissionContext::IsRestrictedToSecureOrigins() const {
...
void HnsWalletPermissionContext::RequestPermission(
  ...
  if (!hns_wallet::ParseRequestingOrigin(
  ...
  hns_wallet::GetSubRequestOrigin(type, origin, addr_queue.front(),
                                    &sub_request_origin);
  addr_queue.pop();
  if (addr_queue.empty()) {
...
// static
void HnsWalletPermissionContext::AcceptOrCancel(
    const std::vector<std::string>& accounts,
    hns_wallet::mojom::PermissionLifetimeOption option,
    content::WebContents* web_contents) {
...
// static
void HnsWalletPermissionContext::Cancel(content::WebContents* web_contents) {
  PermissionRequestManager* manager =
      PermissionRequestManager::FromWebContents(web_contents);
  DCHECK(manager);
...
// static
bool HnsWalletPermissionContext::HasRequestsInProgress(
...
// static
void HnsWalletPermissionContext::RequestPermissions(
  ...
  url::Origin origin;
  if (!hns_wallet::GetConcatOriginFromWalletAddresses(
...
HnsWalletPermissionContext::GetAllowedAccounts(
    blink::PermissionType permission,
    content::RenderFrameHost* rfh,
    const std::vector<std::string>& addresses) {
  if (!rfh) {
  ...
  for (const auto& address : addresses) {
    url::Origin sub_request_origin;
    bool success = hns_wallet::GetSubRequestOrigin(
...
// static
bool HnsWalletPermissionContext::IsPermissionDenied(
    blink::PermissionType permission,
...
// static
bool HnsWalletPermissionContext::AddPermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
    const url::Origin& origin,
  ...
  if (!hns_wallet::GetSubRequestOrigin(
          ContentSettingsTypeToRequestType(content_settings_type), origin,
...
// static
bool HnsWalletPermissionContext::HasPermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
    const url::Origin& origin,
    const std::string& account,
...
// static
bool HnsWalletPermissionContext::ResetPermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
    const url::Origin& origin,
    const std::string& account) {
...
HnsWalletPermissionContext::GetWebSitesWithPermission(
    blink::PermissionType permission,
    content::BrowserContext* context) {
  const ContentSettingsType content_settings_type =
      PermissionUtil::PermissionTypeToContentSettingTypeSafe(permission);
...
// static
bool HnsWalletPermissionContext::ResetWebSitePermission(
    blink::PermissionType permission,
    content::BrowserContext* context,
