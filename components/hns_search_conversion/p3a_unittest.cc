...
class HnsSearchConversionP3ATest : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(HnsSearchConversionP3ATest, TestOmniboxTriggerAndDefaultEngine) {
  for (auto& type_info : type_infos) {
    GetPrefs()->ClearPrefsWithPrefixSilently("hns.hns_search_conversion");
...
TEST_F(HnsSearchConversionP3ATest, TestOmniboxShownAndDefaultEngine) {
  for (auto& type_info : type_infos) {
    GetPrefs()->ClearPrefsWithPrefixSilently("hns.hns_search_conversion");
