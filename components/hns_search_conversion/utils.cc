...
#include "hns/components/hns_search_conversion/constants.h"
...
bool IsNTPPromotionEnabled(PrefService* prefs, TemplateURLService* service) {
  ...
  // Don't need to prompt for conversion if user uses hns as a default
  // provider.
  ...
  if (id == TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_HNS ||
      id == TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_HNS_TOR) {
    return false;
  }
...
ConversionType GetConversionType(PrefService* prefs,
  ...
  // Don't need to ask conversion if user uses hns as a default provider.
...
GURL GetPromoURL(const std::string& search_term) {
  std::string promo_url(kHnsSearchConversionPromotionURL);
  base::ReplaceSubstringsAfterOffset(&promo_url, 0, kSearchTermsParameter,
                                     search_term);
  return GURL(promo_url);
}

bool IsHnsSearchConversionFeatureEnabled() {
