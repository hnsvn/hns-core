...
class HnsSearchConversionTest : public testing::Test {
 public:
  HnsSearchConversionTest()
  ...
  void ConfigureHnsSearchAsDefaultProvider(bool tor) {
    template_url_service_.SetUserSelectedDefaultSearchProvider(
        tor ? hns_search_tor_template_url_.get()
            : hns_search_template_url_.get());
  }

  content::BrowserTaskEnvironment task_environment_;
  std::unique_ptr<TemplateURL> hns_search_template_url_;
  std::unique_ptr<TemplateURL> hns_search_tor_template_url_;
  std::unique_ptr<TemplateURL> bing_template_url_;
  TestingPrefServiceSimple pref_service_;
  TemplateURLService template_url_service_;
};

TEST_F(HnsSearchConversionTest, DefaultValueTest) {
  ...
  EXPECT_EQ(GURL("https://search.hns.com/search?q=hns&action=makeDefault"),
            GetPromoURL(u"hns"));
}

TEST_F(HnsSearchConversionTest, ConversionTypeTest) {
  ...
  // Check do not conversion when hns search(tor) is set as a default
  // provider.
  ConfigureHnsSearchAsDefaultProvider(false);
  EXPECT_EQ(ConversionType::kNone,
            GetConversionType(&pref_service_, &template_url_service_));
  ConfigureHnsSearchAsDefaultProvider(true);
  EXPECT_EQ(ConversionType::kNone,
            GetConversionType(&pref_service_, &template_url_service_));
