...
static SPEEDREADER_CSS: &str = "https://raw.githubusercontent.com/hns-experiments/SpeedReader/master/data/content-stylesheet.css";
...
fn main() {
    ...
    let sheet = format!(
        "<style id=\"hns_speedreader_style\">{}</style>",
    ...
    let slideshow_path = fs::canonicalize("out/slideshow.html")
        ...
        .into_string()
        .unwrap();
    Command::new("hns").arg(slideshow_path).spawn().unwrap();
}
