...
#include "hns/components/hns_rewards/browser/test/common/rewards_browsertest_network_util.h"
#include "hns/components/hns_rewards/browser/test/common/rewards_browsertest_response.h"
...
// npm run test -- hns_browser_tests --filter=RewardsStateBrowserTest*
...
class RewardsStateBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void GetTestFile(const std::string& file_name, base::FilePath* path) const {
    ...
    test_path = test_path.Append(FILE_PATH_LITERAL("hns"));
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, State_1) {
  profile_->GetPrefs()->SetInteger("hns.rewards.version", -1);
  test_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetInteger("hns.rewards.ac.min_visit_time"),
      5);
  EXPECT_EQ(profile_->GetPrefs()->GetInteger("hns.rewards.ac.min_visits"), 5);
  EXPECT_EQ(profile_->GetPrefs()->GetDouble("hns.rewards.ac.score.a"),
      14500.0);
  EXPECT_EQ(profile_->GetPrefs()->GetDouble("hns.rewards.ac.score.b"),
      -14000.0);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, State_2) {
  ...
  const std::string wallet_json =
      profile_->GetPrefs()->GetString("hns.rewards.wallets.hns");
  EXPECT_EQ(
      wallet_json,
      R"({"payment_id":"eea767c4-cd27-4411-afd4-78a9c6b54dbc","recovery_seed":"PgFfhazUJuf8dX+8ckTjrtK1KMLyrfXmKJFDiS1Ad3I="})");  // NOLINT
  EXPECT_EQ(
      profile_->GetPrefs()->GetUint64("hns.rewards.creation_stamp"),
      1590484778ul);
  EXPECT_EQ(
      profile_->GetPrefs()->GetUint64("hns.rewards.ac.next_reconcile_stamp"),
      2593076778ul);
  EXPECT_EQ(
      profile_->GetPrefs()->GetDouble("hns.rewards.ac.amount"),
      20.0);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.ac.enabled"),
      true);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.inline_tip.reddit"),
      true);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.inline_tip.twitter"),
      false);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.inline_tip.github"),
      false);
}

IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V8RewardsEnabledACEnabled) {
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 7);
  profile_->GetPrefs()->SetBoolean("hns.rewards.enabled", true);
  profile_->GetPrefs()->SetBoolean("hns.rewards.ac.enabled", true);
  test_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.ac.enabled"),
      true);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V8RewardsEnabledACDisabled) {
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 7);
  profile_->GetPrefs()->SetBoolean("hns.rewards.enabled", true);
  profile_->GetPrefs()->SetBoolean("hns.rewards.ac.enabled", false);
  test_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.ac.enabled"),
      false);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V8RewardsDisabledACEnabled) {
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 1);
  profile_->GetPrefs()->SetBoolean("hns.rewards.enabled", false);
  profile_->GetPrefs()->SetBoolean("hns.rewards.ac.enabled", true);
  test_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("hns.rewards.ac.enabled"),
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V11ValidWallet) {
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 10);

  const std::string wallet = "wallet";

  const auto encrypted = test_util::EncryptPrefString(rewards_service_, wallet);
  ASSERT_TRUE(encrypted);
  profile_->GetPrefs()->SetString("hns.rewards.wallets.hns", *encrypted);

  test_util::StartProcess(rewards_service_);

  const auto hns_wallet =
      profile_->GetPrefs()->GetString("hns.rewards.wallets.hns");

  EXPECT_EQ(hns_wallet, wallet);
}
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V11CorruptedWallet) {
  ...
  profile_->GetPrefs()->SetString("hns.rewards.wallets.hns", base64_wallet);
  ...
  const auto decrypted =
      test_util::DecryptPrefString(rewards_service_, hns_wallet);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V11InvalidWallet) {
  ...
  profile_->GetPrefs()->SetString("hns.rewards.wallets.hns", "foobar");
...
IN_PROC_BROWSER_TEST_P_(V10, Paths) {
  ...
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 9);
  ...
  profile_->GetPrefs()->SetString("hns.rewards.wallets.uphold",
                                  *encrypted_from_wallet);

  test_util::StartProcess(rewards_service_);

  const auto encrypted_to_wallet =
      profile_->GetPrefs()->GetString("hns.rewards.wallets.uphold");
...
IN_PROC_BROWSER_TEST_P_(V12, Paths) {
  ...
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 11);
  rewards_service_->SetEngineStateTargetVersionForTesting(12);

  const auto encrypted_from_wallet =
      test_util::EncryptPrefString(rewards_service_, std::get<1>(GetParam()));
  ASSERT_TRUE(encrypted_from_wallet);
  profile_->GetPrefs()->SetString("hns.rewards.wallets.bitflyer",
                                  *encrypted_from_wallet);

  test_util::StartProcess(rewards_service_);

  const auto encrypted_to_wallet =
      profile_->GetPrefs()->GetString("hns.rewards.wallets.bitflyer");
  const auto decrypted_to_wallet =
      test_util::DecryptPrefString(rewards_service_, encrypted_to_wallet);
  ASSERT_TRUE(decrypted_to_wallet);
...
IN_PROC_BROWSER_TEST_P_(V13, Paths) {
  ...
  profile_->GetPrefs()->SetInteger("hns.rewards.version", 12);
  ...
  profile_->GetPrefs()->SetString(
      "hns.rewards.wallets." + std::get<0>(GetParam()), *encrypted_wallet);

  test_util::StartProcess(rewards_service_);

  if (wallet_status == mojom::WalletStatus::kConnected) {
    ASSERT_TRUE(profile_->GetPrefs()->HasPrefPath(
        hns_ads::prefs::kShouldMigrateVerifiedRewardsUser));
