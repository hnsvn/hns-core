...
#include "hns/components/hns_rewards/browser/diagnostic_log.h"
#include "hns/components/hns_rewards/browser/logging.h"
...
#include "hns/components/hns_rewards/browser/rewards_notification_service_impl.h"
...
#include "hns/components/hns_rewards/browser/service_sandbox_type.h"
...
constexpr char pref_prefix[] = "hns.rewards";
...
GetNetworkTrafficAnnotationTagForFaviconFetch() {
  return net::DefineNetworkTrafficAnnotation(
      "hns_rewards_favicon_fetcher", R"(
      semantics {
        sender:
          "Hns Rewards Media Fetcher"
        description:
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTagForURLLoad() {
  return net::DefineNetworkTrafficAnnotation("rewards_service_impl", R"(
      semantics {
        ...
        trigger:
          "User-initiated for direct tipping or on a set interval while Hns "
      ...
      policy {
        ...
        setting:
          ...
          "bar or by visiting hns://rewards/."
        policy_exception_justification:
          "Not implemented."
      })");
}
...
RewardsServiceImpl::RewardsServiceImpl(Profile* profile)
  ...
  content::URLDataSource::Add(profile_,
                              std::make_unique<HnsRewardsSource>(profile_));
...
void RewardsServiceImpl::OnPreferenceChanged(const std::string& key) {
  ...
  if (key == ntp_background_images::prefs::
                 kNewTabPageShowSponsoredImagesBackgroundImage ||
      key == hns_ads::prefs::kOptedInToNotificationAds) {
    p3a::RecordAdTypesEnabled(profile_->GetPrefs());
  }

#if BUILDFLAG(ENABLE_GREASELION)
  if (key == hns_ads::prefs::kOptedInToNotificationAds) {
    if (greaselion_service_) {
  ...
  auto on_start = [](base::WeakPtr<RewardsServiceImpl> self,
    ...
    auto on_created = [](base::WeakPtr<RewardsServiceImpl> self,
      ...
      if (!prefs->GetBoolean(prefs::kEnabled)) {
        ...
        prefs->SetBoolean(hns_ads::prefs::kOptedInToNotificationAds, true);
...
mojom::RewardsEngineOptionsPtr RewardsServiceImpl::HandleFlags(
  ...
  // in a future version. Use --enable-features=HnsRewardsVerboseLogging
  // instead.
...
void RewardsServiceImpl::ConnectExternalWallet(
  ...
  const auto url = GURL("hns:/" + path + query);
  for (net::QueryIterator it(url); !it.IsAtEnd(); it.Advance()) {
