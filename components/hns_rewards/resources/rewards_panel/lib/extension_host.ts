...
import * as mojom from 'gen/hns/components/hns_rewards/common/mojom/rewards_panel.mojom.m.js'
...
export function createHost (): Host {
  ...
  function loadGrantCaptcha (grantId: string, status: GrantCaptchaStatus) {
    ...
    chrome.hnsRewards.claimPromotion(grantId, (properties) => {
      stateManager.update({
        grantCaptchaInfo: {
  ...
  function loadAdaptiveCaptcha () {
    chrome.hnsRewards.getScheduledCaptchaInfo((scheduledCaptchaInfo) => {
  ...
  function setLoadingTimer () {
    ...
    // the `hnsRewards` extension API can block on network requests. If the
  ...
  function addListeners () {
    ...
    chrome.hnsRewards.onReconcileComplete.addListener(updateBalance)
    chrome.hnsRewards.onUnblindedTokensReady.addListener(updateBalance)
    chrome.hnsRewards.onExternalWalletLoggedOut.addListener(updateBalance)

    // Update user type when the user's wallet is disconnected.
    chrome.hnsRewards.onExternalWalletDisconnected.addListener(updateUserType)
  ...
  return {
    ...
    refreshPublisherStatus () {
      ...
      chrome.hnsRewards.refreshPublisher(publisherInfo.id, () => {
    ...
    setIncludeInAutoContribute (include) {
      ...
      chrome.hnsRewards.includeInAutoContribution(publisherInfo.id, !include)
    ...
    sendTip () {
      getCurrentTabInfo().then((tabInfo) => {
        ...
        const { publisherInfo } = stateManager.getState()
        if (publisherInfo) {
          chrome.hnsRewards.tipSite(tabInfo.id, publisherInfo.id, 'one-time')
          closePanel()
        }
      }).catch(console.error)
    },
    ...
    solveGrantCaptcha (solution) {
      ...
      chrome.hnsRewards.attestPromotion(grantId, json, (result) => {
        const { grantCaptchaInfo } = stateManager.getState()
    ...
    handleAdaptiveCaptchaResult (result) {
      ...
      switch (result) {
        case 'success':
          chrome.hnsRewards.updateScheduledCaptchaResult(true)
          stateManager.update({
        ...
        case 'failure':
        case 'error':
          chrome.hnsRewards.updateScheduledCaptchaResult(false)
          loadAdaptiveCaptcha()
          break
      }
    },
