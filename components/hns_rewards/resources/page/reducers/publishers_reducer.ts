...
const publishersReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    ...
    case types.ON_EXCLUDE_PUBLISHER: {
      ...
      chrome.send('hns_rewards.excludePublisher', [publisherKey])
      break
    }
    case types.ON_RESTORE_PUBLISHER: {
      ...
      chrome.send('hns_rewards.restorePublisher', [publisherKey])
      break
    }
    case types.ON_RESTORE_PUBLISHERS:
      state = { ...state }
      chrome.send('hns_rewards.restorePublishers', [])
      break
    ...
    case types.REMOVE_RECURRING_TIP:
      ...
      chrome.send('hns_rewards.removeRecurringTip', [action.payload.publisherKey])
      break
    ...
    case types.GET_EXCLUDED_SITES:
      chrome.send('hns_rewards.getExcludedSites')
      break
