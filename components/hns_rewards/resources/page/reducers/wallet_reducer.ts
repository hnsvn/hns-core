...
const walletReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    case types.GET_REWARDS_PARAMETERS:
      chrome.send('hns_rewards.getRewardsParameters')
      break
    ...
    case types.GET_BALANCE_REPORT: {
      chrome.send('hns_rewards.getBalanceReport', [
    ...
    case types.GET_EXTERNAL_WALLET_PROVIDERS: {
      chrome.send('hns_rewards.getExternalWalletProviders')
      break
    }
    case types.SET_EXTERNAL_WALLET_TYPE: {
      chrome.send('hns_rewards.setExternalWalletType', [action.payload.provider])
      break
    }
    ...
    case types.GET_MONTHLY_REPORT: {
      ...
      chrome.send('hns_rewards.getMonthlyReport', [month, year])
      break
    }
    ...
    case types.GET_MONTHLY_REPORT_IDS: {
      chrome.send('hns_rewards.getMonthlyReportIds')
      break
    }
