...
const rewardsReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    case types.IS_INITIALIZED: {
      chrome.send('hns_rewards.isInitialized')
      break
    }
    case types.GET_IS_GRANDFATHERED_USER: {
      chrome.send('hns_rewards.isGrandfatheredUser')
      break
    }
    ...
    case types.GET_USER_TYPE: {
      chrome.send('hns_rewards.getUserType')
      break
    }
    ...
    case types.GET_IS_AUTO_CONTRIBUTE_SUPPORTED: {
      chrome.send('hns_rewards.isAutoContributeSupported')
      break
    }
    ...
    case types.GET_AUTO_CONTRIBUTE_PROPERTIES: {
      chrome.send('hns_rewards.getAutoContributeProperties')
      break
    }
    ...
    case types.ON_SETTING_SAVE: {
      ...
      if (persist) {
        chrome.send('hns_rewards.saveSetting', [key, value.toString()])
      }
    ...
    case types.GET_TIP_TABLE: {
      chrome.send('hns_rewards.getRecurringTips')
      chrome.send('hns_rewards.getOneTimeTips')
      break
    }
    case types.GET_CONTRIBUTE_LIST: {
      chrome.send('hns_rewards.getContributionList')
      break
    }
    case types.GET_ADS_DATA: {
      chrome.send('hns_rewards.getAdsData')
      break
    }
    ...
    case types.GET_ADS_HISTORY: {
      chrome.send('hns_rewards.getAdsHistory')
      break
    }
    ...
    case types.TOGGLE_AD_THUMB_UP: {
      chrome.send('hns_rewards.toggleAdThumbUp', [action.payload.adContent])
      break
    }
    ...
    case types.TOGGLE_AD_THUMB_DOWN: {
      chrome.send('hns_rewards.toggleAdThumbDown', [action.payload.adContent])
      break
    }
    ...
    case types.TOGGLE_AD_OPT_IN: {
      chrome.send('hns_rewards.toggleAdOptIn', [action.payload.categoryContent])
      break
    }
    ...
    case types.TOGGLE_AD_OPT_OUT: {
      chrome.send('hns_rewards.toggleAdOptOut', [action.payload.categoryContent])
      break
    }
    ...
    case types.TOGGLE_SAVED_AD: {
      chrome.send('hns_rewards.toggleSavedAd', [action.payload.adContent])
      break
    }
    ...
    case types.TOGGLE_FLAGGED_AD: {
      chrome.send('hns_rewards.toggleFlaggedAd', [action.payload.adContent])
      break
    }
    ...
    case types.ON_ADS_SETTING_SAVE: {
      ...
      const value = action.payload.value
      if (key) {
        chrome.send('hns_rewards.saveAdsSetting', [key, value.toString()])
    ...
    case types.ON_STATEMENT_CHANGED: {
      chrome.send('hns_rewards.getStatement', [])
      break
    }
    ...
    case types.GET_ENABLED_INLINE_TIPPING_PLATFORMS: {
      chrome.send('hns_rewards.getEnabledInlineTippingPlatforms')
      break
    }
    ...
    case types.ON_INLINE_TIP_SETTINGS_CHANGE: {
      ...
      inlineTip[key] = value
      chrome.send('hns_rewards.setInlineTippingPlatformEnabled', [key, value.toString()])
    ...
    case types.ON_INLINE_TIPS_ENABLED_CHANGE: {
      ...
      chrome.send('hns_rewards.setInlineTipsEnabled', [inlineTipsEnabled])
      state = {
        ...state,
        inlineTipsEnabled
      }
    ...
    case types.CONNECT_EXTERNAL_WALLET: {
      ...
      chrome.send('hns_rewards.connectExternalWallet', [path, query])
    ...
    case types.ON_CONNECT_EXTERNAL_WALLET: {
      chrome.send('hns_rewards.getExternalWallet')
      ...
      if (value) {
        chrome.send('hns_rewards.getUserType')
        chrome.send('hns_rewards.fetchBalance')
    ...
    case types.GET_COUNTRY_CODE: {
      chrome.send('hns_rewards.getCountryCode')
      break
    }
    ...
    case types.ON_INITIALIZED: {
      chrome.send('hns_rewards.getReconcileStamp')
      state = {
        ...state,
        initializing: false
      }
    ...
    case types.COMPLETE_RESET: {
      chrome.send('hns_rewards.completeReset')
      break
    }
    case types.ON_COMPLETE_RESET: {
      if (action.payload.success) {
        chrome.send('hns_rewards.getOnboardingStatus')
        return undefined
      }
      break
    }
    ...
    case types.ENABLE_REWARDS: {
      chrome.send('hns_rewards.enableRewards')
      break
    }
    case types.RESTART_BROWSER: {
      chrome.send('hns_rewards.restartBrowser')
      break
    }
    case types.ON_PREF_CHANGED: {
      chrome.send('hns_rewards.getEnabledInlineTippingPlatforms')
      chrome.send('hns_rewards.getContributionAmount')
      chrome.send('hns_rewards.getAutoContributeProperties')
      chrome.send('hns_rewards.getAdsData')
      break
    }
    case types.GET_IS_UNSUPPORTED_REGION: {
      chrome.send('hns_rewards.getIsUnsupportedRegion')
      break
    }
