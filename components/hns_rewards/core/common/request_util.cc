...
std::string SignatureHeaderValue(const std::string& data,
  ...
  auto digest_header_value = util::Security::DigestValue(body);
  ...
  return util::Security::Sign(headers, key_id, private_key);
}
...
std::map<std::string, std::string> GetSignHeaders(
    ...
    const std::vector<uint8_t>& private_key,
    const bool idempotency_key) {
  const std::string digest_header = util::Security::DigestValue(body).c_str();
