...
#include "hns/components/hns_rewards/core/common/security_util.h"
#include "hns/components/hns_rewards/core/constants.h"
#include "hns/components/hns_rewards/core/legacy/bat_helper.h"
...
std::string Security::Sign(
  ...
  bool success = GetPublicKeyFromSeed(secret_key, &public_key, &new_secret_key);
  if (!success) {
    return "";
  }
...
bool Security::GetPublicKeyFromSeed(const std::vector<uint8_t>& seed,
                                    std::vector<uint8_t>* public_key,
...
std::vector<uint8_t> Security::GetHKDF(const std::vector<uint8_t>& seed) {
  ...
  int hkdf_res =
      HKDF(&out.front(), kSeedLength, EVP_sha512(), &seed.front(), seed.size(),
           kHkdfSalt, kSaltLength, info, sizeof(info) / sizeof(info[0]));
