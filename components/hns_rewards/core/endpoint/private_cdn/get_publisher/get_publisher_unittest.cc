...
// npm run test -- hns_unit_tests --filter=GetPublisherTest.*
...
TEST_F(GetPublisherTest, ServerError404) {
  ...
  result = Request("hns.com", "ce55", &info);
  EXPECT_EQ(result, mojom::Result::OK);
  ASSERT_TRUE(info);
  EXPECT_EQ(info->publisher_key, "hns.com");
...
TEST_F(GetPublisherTest, UpholdVerified) {
  ...
  channel->set_channel_identifier("hns.com");
