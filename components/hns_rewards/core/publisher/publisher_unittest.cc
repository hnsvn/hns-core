...
// npm run test -- hns_unit_tests --filter=PublisherTest.*
...
TEST_F(PublisherTest, GetShareURL) {
  ...
  args.insert({"name", "hns"});
  args.insert({"comment", "I just tipped someone using the Hns browser."});
  args.insert({"hashtag", "TipWithHns"});
  EXPECT_EQ(Publisher::GetShareURL(args),
            "https://twitter.com/intent/tweet?text=I just tipped someone using "
            "the Hns browser.%20%23TipWithHns");
  ...
  EXPECT_EQ(Publisher::GetShareURL(args),
            "https://twitter.com/intent/tweet?text=I just tipped someone using "
            "the Hns browser.%20%23TipWithHns"
            "&url=https://twitter.com/hns/status/794221010484502528");
}
