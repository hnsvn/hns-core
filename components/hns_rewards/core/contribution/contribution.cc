...
#include "hns/components/hns_rewards/core/contribution/contribution_util.h"
...
#include "hns/components/hns_rewards/core/publisher/publisher_status_helper.h"
...
bool IsRevivedAC(const mojom::ContributionInfo& contribution) {
  ...
  if (contribution.processor == ContributionProcessor::HNS_TOKENS) {
    return false;
  }
...
void Contribution::OnResult(mojom::ContributionInfoPtr contribution,
  ...
  if (result == mojom::Result::RETRY_LONG) {
    if (contribution->processor == mojom::ContributionProcessor::HNS_TOKENS) {
...
void Contribution::Retry(
  ...
  switch ((*shared_contribution)->processor) {
    case mojom::ContributionProcessor::HNS_TOKENS: {
