...
// npm run test -- hns_unit_tests --filter=MediaYouTubeTest.*
...
TEST(MediaYouTubeTest, GetUserFromUrl) {
  ...
  // single element in the url
  path = "https://www.youtube.com/user/hns";
  ...
  path =
      "https://www.youtube.com/user/"
      "hns/playlist?view_as=subscriber";
...
TEST(MediaYouTubeTest, GetBasicPath) {
  ...
  path = "/hnssoftware";
  realPath = YouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/hnssoftware");

  path = "/hnssoftware/videos";
  realPath = YouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/hnssoftware");
  ...
  path = "/hnssoftware/playlists";
  realPath = YouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/hnssoftware");

  path = "/hnssoftware/community";
  realPath = YouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/hnssoftware");

  path = "/hnssoftware/channels";
  realPath = YouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/hnssoftware");

  path = "/hnssoftware/about";
  realPath = YouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/hnssoftware");
...
TEST(MediaYouTubeTest, GetChannelId) {
  ...
  data =
      "<div id=\"microformat\"><title>Hns</title><link rel=\"canonical\" h"
      ...
      "Zg\"><meta property=\"og:title\" content=\"Hns\"><meta property=\"o"
