...
#include "hns/components/hns_stats/browser/buildflags.h"
...
std::string GetAPIKey() {
  std::string api_key = BUILDFLAG(HNS_STATS_API_KEY);
  std::unique_ptr<base::Environment> env(base::Environment::Create());
  if (env->HasVar("HNS_STATS_API_KEY"))
    env->GetVar("HNS_STATS_API_KEY", &api_key);
