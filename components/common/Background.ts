...
const hnsExtensionId = 'mnojpmjdmbbfmejpflffifhffcmidifd'
...
export function send<U= void, T= void> (messageType: string, payload?: T): Promise<U> {
  ...
  console.debug(`Sending data to hns extension for ${messageType}`, { messageType, payload })
  return new Promise(function (resolve) {
    chrome.runtime.sendMessage(hnsExtensionId, { messageType, payload }, function (responseData: U) {
      console.debug(`got response from hns extension for "${messageType}"`, responseData)
