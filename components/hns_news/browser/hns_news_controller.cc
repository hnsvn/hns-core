...
#include "hns/components/hns_news/browser/feed_v2_builder.h"
#include "hns/components/hns_news/browser/locales_helper.h"
...
#include "hns/components/hns_news/browser/unsupported_publisher_migrator.h"
...
#include "hns/components/hns_private_cdn/private_cdn_helper.h"
#include "hns/components/hns_private_cdn/private_cdn_request_helper.h"
...
bool GetIsEnabled(PrefService* prefs) {
  ...
  bool opted_in = prefs->GetBoolean(prefs::kHnsNewsOptedIn);
  bool is_enabled = (should_show && opted_in);
  return is_enabled;
}

// static
void HnsNewsController::RegisterProfilePrefs(PrefRegistrySimple* registry) {
  ...
  registry->RegisterBooleanPref(prefs::kHnsNewsOptedIn, false);
  registry->RegisterDictionaryPref(prefs::kHnsNewsSources);
  registry->RegisterDictionaryPref(prefs::kHnsNewsChannels);
  registry->RegisterDictionaryPref(prefs::kHnsNewsDirectFeeds);
...
HnsNewsController::HnsNewsController(
    ...
    hns_ads::AdsService* ads_service,
  ...
  pref_change_registrar_.Add(
      prefs::kHnsNewsOptedIn,
      base::BindRepeating(&HnsNewsController::OnOptInChange,
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&HnsNewsController::OnOptInChange,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      prefs::kHnsNewsChannels,
      base::BindRepeating(&HnsNewsController::HandleSubscriptionsChanged,
                          base::Unretained(this)));

  publishers_observation_.Observe(&publishers_controller_);

  p3a::RecordAtInit(prefs_);
  // Monitor kHnsNewsSources and update feed / publisher cache
  // Start timer of updating feeds, if applicable
  ConditionallyStartOrStopTimer();
}

HnsNewsController::~HnsNewsController() = default;

void HnsNewsController::Bind(
    mojo::PendingReceiver<mojom::HnsNewsController> receiver) {
  receivers_.Add(this, std::move(receiver));
}

void HnsNewsController::ClearHistory() {
  // TODO(petemill): Clear history once/if we actually store
  // feed cache somewhere.
}

mojo::PendingRemote<mojom::HnsNewsController>
HnsNewsController::MakeRemote() {
  mojo::PendingRemote<mojom::HnsNewsController> remote;
  receivers_.Add(this, remote.InitWithNewPipeAndPassReceiver());
  return remote;
}

void HnsNewsController::GetLocale(GetLocaleCallback callback) {
...
void HnsNewsController::GetFeed(GetFeedCallback callback) {
  if (!GetIsEnabled(prefs_)) {
    std::move(callback).Run(hns_news::mojom::Feed::New());
    return;
  }
  feed_controller_.GetOrFetchFeed(std::move(callback));
}

void HnsNewsController::GetFeedV2(GetFeedV2Callback callback) {
  if (!GetIsEnabled(prefs_) ||
...
void HnsNewsController::GetSignals(GetSignalsCallback callback) {
  if (!GetIsEnabled(prefs_) ||
      !base::FeatureList::IsEnabled(
          hns_news::features::kHnsNewsFeedUpdate)) {
    std::move(callback).Run({});
...
void HnsNewsController::GetPublishers(GetPublishersCallback callback) {
  if (!GetIsEnabled(prefs_)) {
    std::move(callback).Run({});
    return;
  }
...
void HnsNewsController::AddPublishersListener(
...
void HnsNewsController::SubscribeToNewDirectFeed(
    const GURL& feed_url,
  ...
  direct_feed_controller_.VerifyFeedUrl(
      ...
      base::BindOnce(
          [](const GURL& feed_url, SubscribeToNewDirectFeedCallback callback,
             HnsNewsController* controller, const bool is_valid,
            ...
            ParseDirectPublisherList(
                controller->prefs_->GetDict(prefs::kHnsNewsDirectFeeds),
                &direct_feeds);
            auto event = mojom::PublishersEvent::New();
...
void HnsNewsController::RemoveDirectFeed(const std::string& publisher_id) {
...
void HnsNewsController::GetImageData(const GURL& padded_image_url,
  ...
  // will contain (Hns's PrivateCDN) padding or
  // be a direct image
      ...
      base::BindOnce(
          [](GetImageDataCallback callback, const bool is_padded,
            ...
            if (response_code < 200 || response_code >= 300 ||
                (is_padded &&
                 !hns::PrivateCdnHelper::GetInstance()->RemovePadding(
                     &body_payload))) {
...
void HnsNewsController::GetFavIconData(const std::string& publisher_id,
                                         GetFavIconDataCallback callback) {
  GetPublishers(base::BindOnce(
      [](HnsNewsController* controller, const std::string& publisher_id,
...
void HnsNewsController::SetPublisherPref(const std::string& publisher_id,
  ...
  GetPublishers(base::BindOnce(
      [](const std::string& publisher_id, mojom::UserEnabled new_status,
         HnsNewsController* controller, Publishers publishers) {
...
void HnsNewsController::ClearPrefs() {
  ScopedDictPrefUpdate update(prefs_, prefs::kHnsNewsSources);
  update->clear();
...
void HnsNewsController::IsFeedUpdateAvailable(
...
void HnsNewsController::AddFeedListener(
    mojo::PendingRemote<mojom::FeedListener> listener) {
  feed_controller_.AddListener(std::move(listener));
}

void HnsNewsController::GetDisplayAd(GetDisplayAdCallback callback) {
  ...
  auto on_ad_received = base::BindOnce(
      [](GetDisplayAdCallback callback, const std::string& dimensions,
        ...
        // TODO(petemill): hns_ads seems to use mojom, perhaps we can receive
        // and send to callback the actual typed mojom struct from hns_ads?
...
void HnsNewsController::OnInteractionSessionStarted() {
  p3a::RecordAtSessionStart(prefs_);
}

void HnsNewsController::OnSessionCardVisitsCountChanged(
    uint16_t total_count) {
  // TODO(djandries): Add new News card visit metrics
}

void HnsNewsController::OnPromotedItemView(
  ...
  if (ads_service_ && !item_id.empty() && !creative_instance_id.empty()) {
    ads_service_->TriggerPromotedContentAdEvent(
        ...
        hns_ads::mojom::PromotedContentAdEventType::kViewed,
        /*intentional*/ base::DoNothing());
  }
}

void HnsNewsController::OnPromotedItemVisit(
    const std::string& item_id,
    const std::string& creative_instance_id) {
  if (ads_service_ && !item_id.empty() && !creative_instance_id.empty()) {
    ads_service_->TriggerPromotedContentAdEvent(
        item_id, creative_instance_id,
        hns_ads::mojom::PromotedContentAdEventType::kClicked,
        /*intentional*/ base::DoNothing());
  }
}

void HnsNewsController::OnSessionCardViewsCountChanged(uint16_t total_count,
...
void HnsNewsController::OnDisplayAdVisit(
    const std::string& item_id,
    const std::string& creative_instance_id) {
  // Validate
  if (item_id.empty()) {
  ...
  ads_service_->TriggerInlineContentAdEvent(
      item_id, creative_instance_id,
      hns_ads::mojom::InlineContentAdEventType::kClicked,
      /*intentional*/ base::DoNothing());
}

void HnsNewsController::OnDisplayAdView(
    const std::string& item_id,
    const std::string& creative_instance_id) {
  // Validate
  if (item_id.empty()) {
  ...
  ads_service_->TriggerInlineContentAdEvent(
      item_id, creative_instance_id,
      hns_ads::mojom::InlineContentAdEventType::kViewed,
      /*intentional*/ base::DoNothing());

  p3a::RecordWeeklyDisplayAdsViewedCount(prefs_, true);
}

void HnsNewsController::OnDisplayAdPurgeOrphanedEvents() {
  if (!ads_service_) {
  ...
  ads_service_->PurgeOrphanedAdEventsForType(
      hns_ads::mojom::AdType::kInlineContentAd,
      /*intentional*/ base::DoNothing());
}

void HnsNewsController::CheckForPublishersUpdate() {
  if (!GetIsEnabled(prefs_)) {
    return;
  }
  publishers_controller_.EnsurePublishersIsUpdating();
...
void HnsNewsController::CheckForFeedsUpdate() {
  if (!GetIsEnabled(prefs_)) {
    return;
  }
...
void HnsNewsController::HandleSubscriptionsChanged() {
  if (GetIsEnabled(prefs_)) {
...
void HnsNewsController::MaybeInitPrefs() {
  if (GetIsEnabled(prefs_)) {
    // We had a bug where you could be subscribed to a channel in the empty
    // locale in earlier versions of Hns News. If so, we should remove it.
    // After this has been out for a bit we can remove it.
    // https://github.com/hnsvn/hns-browser/issues/26596
    if (prefs_->GetDict(prefs::kHnsNewsChannels).contains("")) {
      ScopedDictPrefUpdate update(prefs_, prefs::kHnsNewsChannels);
      update->Remove("");
    }

    const auto& channels = prefs_->GetDict(prefs::kHnsNewsChannels);
    if (channels.empty()) {
...
void HnsNewsController::OnPublishersUpdated(
