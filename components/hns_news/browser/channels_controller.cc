...
void ChannelsController::SetChannelSubscribedPref(PrefService* prefs,
                                                  ...
                                                  const std::string& channel_id,
                                                  bool subscribed) {
  ScopedDictPrefUpdate update(prefs, prefs::kHnsNewsChannels);
...
Channels ChannelsController::GetChannelsFromPublishers(
    ...
    PrefService* prefs) {
  Channels channels;
  const auto& channel_subscriptions = prefs->GetDict(prefs::kHnsNewsChannels);
...
std::vector<std::string> ChannelsController::GetChannelLocales() const {
  ...
  const auto& pref = prefs_->GetDict(prefs::kHnsNewsChannels);
...
bool ChannelsController::GetChannelSubscribed(const std::string& locale,
                                              const std::string& channel_id) {
  const auto& subscriptions = prefs_->GetDict(prefs::kHnsNewsChannels);
