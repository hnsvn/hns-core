...
double GetArticleWeight(const mojom::FeedItemMetadataPtr& article,
                        const Signal& signal) {
  const double source_visits_min = features::kHnsNewsSourceVisitsMin.Get();
...
std::vector<mojom::FeedItemV2Ptr> GenerateBlock(ArticleInfos& articles) {
  ...
  const int block_min_inline = features::kHnsNewsMinBlockCards.Get();
  const int block_max_inline = features::kHnsNewsMaxBlockCards.Get();
  ...
  for (auto i = 0; i < follow_count; ++i) {
    ...
    const double inline_discovery_ratio =
        features::kHnsNewsInlineDiscoveryRatio.Get();
