...
TEST(HnsNewsDirectFeed, ParseFeed) {
  FeedData data;
  // If this errors, probably our xml was not valid
  auto json = GetFeedJson();
  bool parse_success = hns_news::parse_feed_bytes(
...
TEST(HnsNewsDirectFeed, ParseWindows1251Feed) {
  FeedData data;
...
TEST(HnsNewsDirectFeed, ParseEUCJPFeed) {
  FeedData data;
...
TEST(HnsNewsDirectFeed, ParseFeedRegression) {
  FeedData data;
...
TEST(HnsNewsDirectFeed, ParseToArticle) {
  // Create a feed item which should be valid as a Hns News Article
  FeedItem item;
  item.id = "1";
...
TEST(HnsNewsDirectFeed, ParseOnlyAllowsHTTPLinks) {
  // Create a feed item which should be invalid as a Hns News Article
  FeedItem item;
  item.id = "1";
  item.published_timestamp = 1672793966;
  item.title = "Title";
