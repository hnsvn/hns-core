...
class PublishersControllerTest : public testing::Test {
 public:
  PublishersControllerTest()
      : api_request_helper_(TRAFFIC_ANNOTATION_FOR_TESTS,
        ...
        publishers_controller_(profile_.GetPrefs(),
                               ...
                               &api_request_helper_) {
    profile_.GetPrefs()->SetBoolean(hns_news::prefs::kHnsNewsOptedIn, true);
    profile_.GetPrefs()->SetBoolean(hns_news::prefs::kNewTabPageShowToday,
                                    true);
  }

  std::string GetV1SourcesUrl() {
    std::string locale =
        hns_l10n::GetDefaultISOLanguageCodeString() == "ja" ? "ja" : "";
    return "https://" + hns_news::GetHostname() + "/sources." + locale +
           "json";
  }

  std::string GetSourcesUrl() {
    return "https://" + hns_news::GetHostname() + "/sources." +
           hns_news::kRegionUrlPart + "json";
  }

  void SetSubscribedSources(const std::vector<std::string>& publisher_ids) {
    ScopedDictPrefUpdate update(profile_.GetPrefs(), prefs::kHnsNewsSources);
  ...
  bool CombinedSourceExists(const std::string& publisher_id) {
    const auto& value = profile_.GetPrefs()->GetDict(prefs::kHnsNewsSources);
...
TEST_F(PublishersControllerTest,
  ...
  WaitForUpdate();
  LOG(ERROR) << "Direct: "
             << profile_.GetPrefs()->GetDict(prefs::kHnsNewsDirectFeeds);
...
TEST_F(PublishersControllerTest, DoesntFetchPublishersWhenNotOptedIn) {
  ...
  profile_.GetPrefs()->SetBoolean(hns_news::prefs::kHnsNewsOptedIn, false);
  auto result = GetPublishers();
