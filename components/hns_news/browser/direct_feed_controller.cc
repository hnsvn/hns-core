...
#include "base/uuid.h"
#include "hns/components/hns_news/browser/direct_feed_fetcher.h"
...
DirectFeedController::FindFeedRequest::FindFeedRequest(
    ...
    mojom::HnsNewsController::FindFeedsCallback callback)
...
bool DirectFeedController::AddDirectFeedPref(
  ...
  const auto& existing_feeds = prefs_->GetDict(prefs::kHnsNewsDirectFeeds);
  for (const auto&& [key, value] : existing_feeds) {
    ...
    const auto* existing_url =
        value.GetDict().FindString(prefs::kHnsNewsDirectFeedsKeySource);
  ...
  ScopedDictPrefUpdate update(prefs_, prefs::kHnsNewsDirectFeeds);
  base::Value::Dict value;
  value.Set(prefs::kHnsNewsDirectFeedsKeySource, feed_url.spec());
  value.Set(prefs::kHnsNewsDirectFeedsKeyTitle, entry_title);
...
std::vector<mojom::PublisherPtr> DirectFeedController::ParseDirectFeedsPref() {
  ...
  const auto& pref = prefs_->GetDict(prefs::kHnsNewsDirectFeeds);
...
void DirectFeedController::VerifyFeedUrl(const GURL& feed_url,
  ...
  // and that the content has the correct fields for Hns News.
