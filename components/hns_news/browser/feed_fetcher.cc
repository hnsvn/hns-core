...
GURL GetFeedUrl(const std::string& locale) {
  GURL feed_url("https://" + hns_news::GetHostname() + "/hns-today/feed." +
                locale + "json");
  return feed_url;
}
...
void FeedFetcher::OnFetchFeedFetchedPublishers(FetchFeedCallback callback,
  ...
  if (publishers.empty()) {
    LOG(ERROR) << "Hns News Publisher list was empty";
...
void FeedFetcher::OnFetchFeedFetchedFeed(
  ...
  if (result.response_code() != 200 || result.value_body().is_none()) {
    LOG(ERROR) << "Bad response from hns news feed.json. Status: "
...
void FeedFetcher::OnIsUpdateAvailableFetchedHead(
  ...
  if (etag.empty()) {
    LOG(ERROR) << "Hns News did not get correct etag, "
