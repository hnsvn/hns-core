...
#include "hns/components/hns_private_cdn/headers.h"
...
void PublishersController::GetLocale(
    mojom::HnsNewsController::GetLocaleCallback callback) {
  GetOrFetchPublishers(base::BindOnce(
      [](PublishersController* controller,
         mojom::HnsNewsController::GetLocaleCallback callback, Publishers _) {
...
void PublishersController::EnsurePublishersIsUpdating() {
  ...
  GURL sources_url(
      base::StrCat({"https://", hns_news::GetHostname(), "/sources.",
                    hns_news::kRegionUrlPart, "json"}));
  auto on_request = base::BindOnce(
      [](PublishersController* controller,
        ...
        const auto& publisher_prefs =
            controller->prefs_->GetDict(prefs::kHnsNewsSources);
  ...
  api_request_helper_->Request(
      ...
      hns::private_cdn_headers, {.auto_retry_on_network_change = true});
}

void PublishersController::UpdateDefaultLocale() {
  auto available_locales = GetPublisherLocales(publishers_);

  // Locale can be "language_Script_COUNTRY.charset@variant" but Hns News
  // wants the format to be "language_COUNTRY".
  const std::string hns_news_locale =
      base::StrCat({hns_l10n::GetDefaultISOLanguageCodeString(), "_",
  ...
  default_locale_ = base::Contains(available_locales, hns_news_locale)
                        ? hns_news_locale
                        : "en_US";
}
