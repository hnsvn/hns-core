...
class NTPBackgroundImagesSourceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    hns::RegisterPrefsForHnsReferralsService(registry);
    service_ =
...
TEST_F(NTPBackgroundImagesSourceTest, SponsoredImagesTest) {
  const std::string test_json_string_referral = R"(
      {
        ...
        "logo": {
          ...
          "destinationUrl": "https://www.hns.com/?from-super-referreer-demo"
        },
        "wallpapers": [
          {
  ...
  EXPECT_FALSE(source_->IsValidPath("super-duper/hns.png"));
  EXPECT_FALSE(source_->IsValidPath("sponsored-images/abcd.png"));

  EXPECT_EQ("image/jpeg",
            source_->GetMimeType(GURL("hns://test/wallpaper-0.jpg")));
  EXPECT_EQ("image/jpeg",
            source_->GetMimeType(GURL("hns://test/wallpaper-0.jpeg")));
  EXPECT_EQ("image/webp",
            source_->GetMimeType(GURL("hns://test/wallpaper-0.webp")));
  EXPECT_EQ("image/png",
            source_->GetMimeType(GURL("hns://test/wallpaper-0.png")));
  EXPECT_EQ("image/avif",
            source_->GetMimeType(GURL("hns://test/wallpaper-0.avif")));
}

TEST_F(NTPBackgroundImagesSourceTest, BackgroundImagesFormatTest) {
  EXPECT_EQ("image/jpeg",
            bg_source_->GetMimeType(GURL("hns://test/wallpaper-0.jpg")));
  EXPECT_EQ("image/webp",
            bg_source_->GetMimeType(GURL("hns://test/wallpaper-0.webp")));
  EXPECT_EQ("image/png",
            bg_source_->GetMimeType(GURL("hns://test/wallpaper-0.png")));
  EXPECT_EQ("image/avif",
            bg_source_->GetMimeType(GURL("hns://test/wallpaper-0.avif")));
}
...
TEST_F(NTPBackgroundImagesSourceTest, BackgroundImagesTest) {
  ...
  EXPECT_EQ(0, bg_source_->GetWallpaperIndexFromPath("hns-bg-1.webp"));
  EXPECT_EQ(1, bg_source_->GetWallpaperIndexFromPath("hns-bg-2.webp"));
  EXPECT_EQ(2, bg_source_->GetWallpaperIndexFromPath("hns-bg-3.webp"));
  EXPECT_EQ(3, bg_source_->GetWallpaperIndexFromPath("hns-bg-4.webp"));
...
TEST_F(NTPBackgroundImagesSourceTest, BasicSuperReferralDataTest) {
  ...
  EXPECT_TRUE(source_->IsValidPath("super-referral/hns.png"));
