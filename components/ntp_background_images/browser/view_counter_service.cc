...
constexpr char kNewTabsCreated[] = "hns.new_tab_page.p3a_new_tabs_created";
constexpr char kSponsoredNewTabsCreated[] =
    "hns.new_tab_page.p3a_sponsored_new_tabs_created";

constexpr char kNewTabsCreatedHistogramName[] = "Hns.NTP.NewTabsCreated.2";
...
ViewCounterService::ViewCounterService(
    ...
    HnsNTPCustomBackgroundService* custom_service,
...
void ViewCounterService::BrandedWallpaperWillBeDisplayed(
  ...
  if (ads_service_) {
    ads_service_->TriggerNewTabPageAdEvent(
        ...
        hns_ads::mojom::NewTabPageAdEventType::kViewed,
...
ViewCounterService::GetCurrentBrandedWallpaper() const {
  ...
  const bool should_frequency_cap_ads =
      prefs_->GetBoolean(hns_rewards::prefs::kEnabled);
...
void ViewCounterService::OnPreferenceChanged(const std::string& pref_name) {
  if (pref_name == hns_rewards::prefs::kEnabled) {
...
void ViewCounterService::BrandedWallpaperLogoClicked(
  ...
  ads_service_->TriggerNewTabPageAdEvent(
      wallpaper_id, creative_instance_id,
      hns_ads::mojom::NewTabPageAdEventType::kClicked,
      /*intentional*/ base::DoNothing());
...
void ViewCounterService::MaybePrefetchNewTabPageAd() {
  ...
  if (!ads_service_ || !IsBrandedWallpaperActive() ||
      !prefs_->GetBoolean(hns_rewards::prefs::kEnabled) || !images_data ||
