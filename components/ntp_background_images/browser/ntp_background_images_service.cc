...
#include "hns/components/ntp_background_images/browser/ntp_background_images_component_installer.h"
...
#include "hns/components/ntp_background_images/browser/sponsored_images_component_data.h"
#include "hns/components/ntp_background_images/browser/switches.h"
...
void NTPBackgroundImagesService::Init() {
  ...
  if (base::FeatureList::IsEnabled(features::kHnsNTPSuperReferralWallpaper)) {
    // Flag override for testing or demo purposes
...
void NTPBackgroundImagesService::RegisterSponsoredImagesComponent() {
  ...
  if (!data) {
    DVLOG(2) << __func__ << ": Not support NTP SI component for "
             << hns_l10n::GetDefaultLocaleString();
    return;
  }
...
void NTPBackgroundImagesService::CheckSuperReferralComponent() {
  ...
  if (local_pref_->FindPreference(
    ...
    if (hns::HnsReferralsService::IsDefaultReferralCode(code)) {
      MarkThisInstallIsNotSuperReferralForever();
    } else {
...
void NTPBackgroundImagesService::OnPreferenceChanged(
  ...
  if (hns::HnsReferralsService::IsDefaultReferralCode(new_referral_code)) {
...
NTPSponsoredImagesData* NTPBackgroundImagesService::GetBrandedImagesData(
  ...
  const bool is_sr_enabled =
      base::FeatureList::IsEnabled(features::kHnsNTPSuperReferralWallpaper);
  if (is_sr_enabled) {
    if (super_referral) {
...
bool NTPBackgroundImagesService::IsSuperReferral() const {
  ...
  return base::FeatureList::IsEnabled(
             features::kHnsNTPSuperReferralWallpaper) &&
