...
#include "hns/components/hns_ads/browser/ads_service_mock.h"
...
#include "hns/components/ntp_background_images/browser/ntp_p3a_helper.h"
#include "hns/components/ntp_background_images/browser/ntp_sponsored_images_data.h"
#include "hns/components/ntp_background_images/browser/url_constants.h"
#include "hns/components/ntp_background_images/browser/view_counter_model.h"
...
constexpr char kDestinationUrl[] = "https://hns.com";
...
std::unique_ptr<NTPSponsoredImagesData> GetDemoBrandedWallpaper(
  ...
  if (super_referral) {
    ...
    demo->top_sites = {
      { "Hns", "https://hns.com", "hns.png",
        base::FilePath(FILE_PATH_LITERAL("hns.png")) },
...
std::unique_ptr<NTPBackgroundImagesData> GetDemoBackgroundWallpaper() {
  ...
  demo->backgrounds = {
      {base::FilePath(FILE_PATH_LITERAL("wallpaper1.jpg")), "Hns",
       "https://hns.com/"},
  };
...
#if BUILDFLAG(ENABLE_CUSTOM_BACKGROUND)
class TestDelegate : public HnsNTPCustomBackgroundService::Delegate {
 public:
  ...
  bool HasPreferredHnsBackground() const override { return false; }
  base::Value::Dict GetPreferredHnsBackground() const override { return {}; }
...
class NTPBackgroundImagesViewCounterTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    hns_rewards::RewardsService::RegisterProfilePrefs(registry);
    auto* local_registry = local_pref_.registry();
    hns::RegisterPrefsForHnsReferralsService(local_registry);
    ...
    custom_bi_service_ =
        std::make_unique<HnsNTPCustomBackgroundService>(std::move(delegate));
  ...
  hns_ads::NewTabPageAdInfo CreateNewTabPageAdInfo() {
    hns_ads::NewTabPageAdInfo ad_info;
    ...
    hns_ads::NewTabPageAdWallpaperInfo wallpaper_info;
  ...
  bool AdInfoMatchesSponsoredImage(const hns_ads::NewTabPageAdInfo& ad_info,
...
#if BUILDFLAG(ENABLE_CUSTOM_BACKGROUND)
  std::unique_ptr<HnsNTPCustomBackgroundService> custom_bi_service_;
  raw_ptr<TestDelegate> delegate_ = nullptr;
#endif

  std::unique_ptr<ViewCounterService> view_counter_;
  hns_ads::AdsServiceMock ads_service_mock_;
};
...
TEST_F(NTPBackgroundImagesViewCounterTest, SponsoredImageAdServed) {
  ...
  hns_ads::NewTabPageAdInfo ad_info = CreateNewTabPageAdInfo();
