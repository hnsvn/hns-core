...
void RecordShieldsToggled(PrefService* local_state) {
  ::hns_shields::MaybeRecordShieldsUsageP3A(::hns_shields::kShutOffShields,
                                              local_state);
}

void RecordShieldsSettingChanged(PrefService* local_state) {
  ::hns_shields::MaybeRecordShieldsUsageP3A(
      ::hns_shields::kChangedPerSiteShields, local_state);
}
...
class HnsCookieRules {
 public:
  HnsCookieRules(ContentSetting general_setting,
  ...
  static HnsCookieRules GetDefault(
  ...
  void Merge(const HnsCookieRules& other) {
...
void SetHnsShieldsEnabled(HostContentSettingsMap* map,
                            bool enable,
                            const GURL& url,
  ...
  map->SetContentSettingCustomScope(
      ...
      // this is 'allow_hns_shields' so 'enable' == 'allow'
      enable ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);

  RecordShieldsToggled(local_state);
}

void ResetHnsShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (url.is_valid() && !url.SchemeIsHTTPOrHTTPS())
    return;
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::HNS_SHIELDS, CONTENT_SETTING_DEFAULT);
}

bool GetHnsShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::HNS_SHIELDS);

  // see EnableHnsShields - allow and default == true
...
void SetAdControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::HNS_ADS, GetDefaultBlockFromControlType(type));
...
ControlType GetAdControlType(HostContentSettingsMap* map, const GURL& url) {
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::HNS_ADS);
...
void SetCosmeticFilteringControlType(HostContentSettingsMap* map,
  ...
  base::Value web_setting = map->GetWebsiteSetting(
      url, GURL(), ContentSettingsType::HNS_COSMETIC_FILTERING,
      &setting_info);
  bool was_default =
...
bool IsReduceLanguageEnabledForProfile(PrefService* pref_service) {
  ...
  if (!base::FeatureList::IsEnabled(features::kHnsReduceLanguage))
    return false;

  // Don't reduce language if user preference is unchecked
  if (!pref_service->GetBoolean(hns_shields::prefs::kReduceLanguageEnabled))
    return false;
...
bool ShouldDoReduceLanguage(HostContentSettingsMap* map,
  ...
  // Don't reduce language if Hns Shields is down (this also handles cases
  // where the URL is not HTTP(S))
  if (!hns_shields::GetHnsShieldsEnabled(map, url))
    return false;

  // Don't reduce language if fingerprinting is off
  if (hns_shields::GetFingerprintingControlType(map, url) ==
      ControlType::ALLOW)
    return false;
...
DomainBlockingType GetDomainBlockingType(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(hns_shields::features::kHnsDomainBlock))
    return DomainBlockingType::kNone;

  // Don't block if Hns Shields is down (this also handles cases where
  // the URL is not HTTP(S))
  if (!hns_shields::GetHnsShieldsEnabled(map, url))
    return DomainBlockingType::kNone;

  // Don't block if ad blocking is off.
  if (hns_shields::GetAdControlType(map, url) != ControlType::BLOCK)
    return DomainBlockingType::kNone;

  const ControlType cosmetic_control_type =
      hns_shields::GetCosmeticFilteringControlType(map, url);
  ...
  if (cosmetic_control_type == BLOCK_THIRD_PARTY &&
      ...
      base::FeatureList::IsEnabled(
          hns_shields::features::kHnsDomainBlock1PES)) {
  ...
  switch (type) {
    case ControlType::BLOCK_THIRD_PARTY:
      ...
      map->SetContentSettingCustomScope(
          ...
          ContentSettingsType::HNS_COOKIES, CONTENT_SETTING_BLOCK);
      // first-party rule:
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::HNS_COOKIES, CONTENT_SETTING_ALLOW);
      break;
    case ControlType::ALLOW:
    case ControlType::BLOCK:
      ...
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::HNS_COOKIES, CONTENT_SETTING_DEFAULT);
      // general-rule:
      map->SetContentSettingCustomScope(
          ContentSettingsPattern::Wildcard(), patterns.host_pattern,
...
ControlType GetCookieControlType(
  ...
  auto result = HnsCookieRules::Get(map, url);
  if (result.HasDefault()) {
    const auto default_rules = HnsCookieRules::GetDefault(cookie_settings);
...
bool AreReferrersAllowed(HostContentSettingsMap* map, const GURL& url) {
  const ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::HNS_REFERRERS);
...
void SetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  content_settings::SettingInfo setting_info;
  base::Value web_setting = map->GetWebsiteSetting(
      url, GURL(), ContentSettingsType::HNS_FINGERPRINTING_V2, &setting_info);
  bool was_default =
      web_setting.is_none() || setting_info.primary_pattern.MatchesAllHosts();
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::HNS_FINGERPRINTING_V2, content_setting);
  if (!map->IsOffTheRecord()) {
    // Only report to P3A if not a guest/incognito profile
    RecordShieldsSettingChanged(local_state);
    if (url.is_empty()) {
...
ControlType GetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  ContentSettingsForOneType fingerprinting_rules =
      map->GetSettingsForOneType(ContentSettingsType::HNS_FINGERPRINTING_V2);

  ContentSetting fp_setting =
      GetHnsFPContentSettingFromRules(fingerprinting_rules, url);
...
bool IsHnsShieldsManaged(PrefService* prefs,
                           HostContentSettingsMap* map,
                           GURL url) {
  DCHECK(prefs);
  DCHECK(map);
  content_settings::SettingInfo info;
  map->GetWebsiteSetting(url, url, ContentSettingsType::HNS_SHIELDS, &info);
...
bool GetHTTPSEverywhereEnabled(HostContentSettingsMap* map, const GURL& url) {
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::HNS_HTTP_UPGRADABLE_RESOURCES);

  return setting == CONTENT_SETTING_ALLOW ? false : true;
}

bool IsHttpsByDefaultFeatureEnabled() {
  return base::FeatureList::IsEnabled(net::features::kHnsHttpsByDefault);
}
...
void SetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::HNS_HTTPS_UPGRADE, setting);
...
ControlType GetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::HNS_HTTPS_UPGRADE);
  if (setting == CONTENT_SETTING_ALLOW) {
    // Disabled (allow http)
    return ControlType::ALLOW;
...
bool ShouldUpgradeToHttps(
  ...
  if (!GetHnsShieldsEnabled(map, url)) {
    return false;
  }
...
bool ShouldForceHttps(HostContentSettingsMap* map, const GURL& url) {
  return GetHnsShieldsEnabled(map, url) &&
...
bool GetForgetFirstPartyStorageEnabled(HostContentSettingsMap* map,
                                       const GURL& url) {
  ContentSetting setting = map->GetContentSetting(
      url, url, ContentSettingsType::HNS_REMEMBER_1P_STORAGE);
...
bool MaybeChangeReferrer(bool allow_referrers,
  ...
  // See https://github.com/hnsvn/hns-browser/issues/13464
...
ShieldsSettingCounts GetAdsSettingCount(HostContentSettingsMap* map) {
  ContentSettingsForOneType cosmetic_rules =
      map->GetSettingsForOneType(ContentSettingsType::HNS_COSMETIC_FILTERING);
