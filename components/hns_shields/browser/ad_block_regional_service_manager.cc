...
using hns_shields::features::kHnsAdblockCookieListDefault;
using hns_shields::features::kHnsAdblockMobileNotificationsListDefault;
...
const ListDefaultOverrideConstants kCookieListConstants{
    .feature = raw_ref<const base::Feature>(kHnsAdblockCookieListDefault),
...
void AdBlockRegionalServiceManager::StartRegionalServices() {
  ...
  if (!checked_all_default_regions) {
    ...
    auto locale_lists = hns_shields::FindAdBlockFilterListsByLocale(
  ...
  for (const auto kv : regional_filters_dict_with_overrides) {
    ...
    if (enabled) {
      auto catalog_entry = hns_shields::FindAdBlockFilterListByUUID(
...
bool AdBlockRegionalServiceManager::IsFilterListAvailable(
  ...
  DCHECK(!uuid.empty());
  auto catalog_entry =
      hns_shields::FindAdBlockFilterListByUUID(filter_list_catalog_, uuid);
...
base::Value::List AdBlockRegionalServiceManager::GetRegionalLists() {
  ...
  for (const auto& region_list : filter_list_catalog_) {
    ...
    // https://github.com/hnsvn/adblock-resources
