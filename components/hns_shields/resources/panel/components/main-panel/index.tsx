...
function MainPanel () {
  ...
  const hnsShieldsStatusText = splitStringForTag(siteBlockInfo?.isHnsShieldsEnabled ? getLocale('hnsShieldsUp') : getLocale('hnsShieldsDown'))
  const hnsShieldsBrokenText = splitStringForTag(getLocale('hnsShieldsBroken'))
  const hnsShieldsNote = splitStringForTag(siteBlockInfo?.isHnsShieldsEnabled
    ? getLocale('hnsShieldsBlockedNote')
    : getLocale('hnsShieldsNOTBlockedNote'))

  const handleToggleChange = async (isOn: boolean) => {
    await getPanelBrowserAPI().dataHandler.setHnsShieldsEnabled(isOn)
    if (isOn) {
  ...
  const handleLearnMoreClick = () => {
    chrome.tabs.create({ url: 'https://hns.com/privacy-features/', active: true })
  }
  ...
  let reportSiteOrFootnoteElement = (
    <S.Footnote>
      {hnsShieldsBrokenText.beforeTag}
      <span>{hnsShieldsBrokenText.duringTag}</span>
      {hnsShieldsBrokenText.afterTag}
    </S.Footnote>
  )
  let managedFootnoteElement = (
    <S.Footnote>
      <S.ControlBox>
        <S.ManagedIcon />
        <S.ManagedText>{getLocale('hnsShieldsManaged')}</S.ManagedText>
      </S.ControlBox>
    </S.Footnote>
  )
  let advancedControlButtonElement = (isExpanded != null) && (
    ...
    >
      ...
      <span>{getLocale('hnsShieldsAdvancedCtrls')}</span>
  ...
  if (!siteBlockInfo?.isHnsShieldsEnabled) {
    ...
    advancedControlButtonElement = (
      ...
      >
        ...
        <span>{getLocale('hnsShieldsChangeDefaults')}</span>
      </S.GlobalDefaultsButton>
    )
    ...
    reportSiteOrFootnoteElement = (
      <S.ReportSiteBox>
        <S.ReportSiteAction>
          <span>{getLocale('hnsShieldsReportSiteDesc')}</span>
          <Button
            isPrimary
            onClick={handleReportSite}
          >
            {getLocale('hnsShieldsReportSite')}
          </Button>
        </S.ReportSiteAction>
      </S.ReportSiteBox>
    )
  ...
  return (
    <S.Box>
      ...
      <S.CountBox>
        <S.BlockNote>
          {hnsShieldsNote.beforeTag}
          <a href="#" onClick={handleLearnMoreClick}>{hnsShieldsNote.duringTag}</a>
          {hnsShieldsNote.afterTag}
        </S.BlockNote>
        {totalCountElement}
      </S.CountBox>
      </S.HeaderBox>
      <S.StatusBox>
        <S.ControlBox>
          <S.ShieldsIcon isActive={siteBlockInfo?.isHnsShieldsEnabled ?? false}>
          ...
          <S.StatusText>
            <span>{hnsShieldsStatusText.beforeTag}</span>
            {hnsShieldsStatusText.duringTag}
            {hnsShieldsStatusText.afterTag}
            {' '}
            {siteBlockInfo?.host}
          </S.StatusText>
          <S.StatusToggle>
            <Toggle
              brand="shields"
              isOn={siteBlockInfo?.isHnsShieldsEnabled}
              onChange={handleToggleChange}
              accessibleLabel={getLocale('hnsShieldsEnable')}
              disabled={siteBlockInfo?.isHnsShieldsManaged}
            />
          </S.StatusToggle>
        </S.ControlBox>
        {!siteBlockInfo?.isHnsShieldsManaged &&
        <S.StatusFootnoteBox>
          {reportSiteOrFootnoteElement}
        </S.StatusFootnoteBox>
        }
      ...
      {advancedControlButtonElement}
      { isExpanded &&
        siteBlockInfo?.isHnsShieldsEnabled &&
