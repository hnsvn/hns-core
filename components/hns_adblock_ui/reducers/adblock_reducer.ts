...
const updateCustomFilters = debounce((customFilters: string) => {
  chrome.send('hns_adblock.updateCustomFilters', [customFilters])
}, 1500)
...
const adblockReducer: Reducer<AdBlock.State | undefined> = (state: AdBlock.State | undefined, action) => {
  ...
  switch (action.type) {
    case types.ADBLOCK_ENABLE_FILTER_LIST:
      chrome.send('hns_adblock.enableFilterList', [action.payload.uuid, action.payload.enabled])
      state = {
        ...state,
        settings: {
          ...state.settings,
    ...
    case types.ADBLOCK_GET_CUSTOM_FILTERS:
      chrome.send('hns_adblock.getCustomFilters')
      break
    case types.ADBLOCK_GET_REGIONAL_LISTS:
      chrome.send('hns_adblock.getRegionalLists')
      break
    case types.ADBLOCK_GET_LIST_SUBSCRIPTIONS:
      chrome.send('hns_adblock.getListSubscriptions')
      break
    case types.ADBLOCK_SUBMIT_NEW_SUBSCRIPTION:
      chrome.send('hns_adblock.submitNewSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_SET_SUBSCRIPTION_ENABLED:
      chrome.send('hns_adblock.setSubscriptionEnabled', [action.payload.listUrl, action.payload.enabled])
      break
    case types.ADBLOCK_DELETE_SUBSCRIPTION:
      chrome.send('hns_adblock.deleteSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_REFRESH_SUBSCRIPTION:
      chrome.send('hns_adblock.refreshSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_VIEW_SUBSCRIPTION_SOURCE:
      chrome.send('hns_adblock.viewSubscriptionSource', [action.payload.listUrl])
      break
