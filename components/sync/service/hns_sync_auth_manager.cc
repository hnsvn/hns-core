...
namespace {
std::string AppendHnsServiceKeyHeaderString() {
  return base::StrCat(
      {"\r\n", kHnsServicesKeyHeader, ": ", BUILDFLAG(HNS_SERVICES_KEY)});
}
}  // namespace

HnsSyncAuthManager::HnsSyncAuthManager(
...
HnsSyncAuthManager::~HnsSyncAuthManager() = default;

void HnsSyncAuthManager::DeriveSigningKeys(const std::string& seed) {
  VLOG(1) << __func__;
  if (seed.empty()) {
    return;
  }
  ...
  if (!hns_sync::crypto::PassphraseToBytes32(seed, &seed_bytes)) {
    return;
  }
  const std::string info_str = "sync-auth-key";
  std::vector<uint8_t> info(info_str.begin(), info_str.end());
  hns_sync::crypto::DeriveSigningKeysFromSeed(seed_bytes, &HKDF_SALT, &info,
...
void HnsSyncAuthManager::ResetKeys() {
  VLOG(1) << __func__;
  public_key_.clear();
  private_key_.clear();
  if (registered_for_auth_notifications_) {
...
void HnsSyncAuthManager::RequestAccessToken() {
  VLOG(1) << __func__;
  hns_sync::NetworkTimeHelper::GetInstance()->GetNetworkTime(
      base::BindOnce(&HnsSyncAuthManager::OnNetworkTimeFetched,
                     weak_ptr_factory_.GetWeakPtr()));
}

SyncAccountInfo HnsSyncAuthManager::DetermineAccountToUse() const {
  if (!public_key_.empty()) {
    ...
    account_info.email =
        ...
        " @hns.com";
    VLOG(1) << "hns client id=" << client_id;
...
std::string HnsSyncAuthManager::GenerateAccessToken(
  ...
  hns_sync::crypto::Sign(timestamp_bytes, private_key_, &signature);
  DCHECK(hns_sync::crypto::Verify(timestamp_bytes, signature, public_key_));
  ...
  return encoded_access_token + AppendHnsServiceKeyHeaderString();
}

void HnsSyncAuthManager::OnNetworkTimeFetched(const base::Time& time) {
