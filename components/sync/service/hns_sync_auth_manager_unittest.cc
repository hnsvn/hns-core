...
const char kAccountEmail[] =
    ...
    "9886900AB36F2FFF655635DBE516765E @hns.com";

class HnsSyncAuthManagerTest : public testing::Test {
 protected:
  ...
  HnsSyncAuthManagerTest() : identity_env_(&test_url_loader_factory_) {}

  ~HnsSyncAuthManagerTest() override = default;

  std::unique_ptr<HnsSyncAuthManager> CreateAuthManager(
      const AccountStateChangedCallback& account_state_changed,
      const CredentialsChangedCallback& credentials_changed) {
    return std::make_unique<HnsSyncAuthManager>(
        identity_env_.identity_manager(), account_state_changed,
        credentials_changed);
  }

  void SetNetworkTime() {
    hns_sync::NetworkTimeHelper::GetInstance()->SetNetworkTimeForTest(
...
TEST_F(HnsSyncAuthManagerTest, IgnoresEventsIfNotRegistered) {
...
TEST_F(HnsSyncAuthManagerTest, GetAccessToken) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
...
TEST_F(HnsSyncAuthManagerTest, Reset) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
...
TEST_F(HnsSyncAuthManagerTest, MalformedSyncCode) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
  EXPECT_CALL(account_state_changed, Run()).Times(0);
  EXPECT_CALL(credentials_changed, Run()).Times(0);
  ...
  auth_manager->DeriveSigningKeys("hns5566");
  EXPECT_TRUE(
      auth_manager->GetActiveAccountInfo().account_info.account_id.empty());
}
