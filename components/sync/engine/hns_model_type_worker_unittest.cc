...
class HnsModelTypeWorkerTest : public ::testing::Test {
 protected:
  explicit HnsModelTypeWorkerTest(ModelType model_type = PREFERENCES)
      : model_type_(model_type) {}

  ~HnsModelTypeWorkerTest() override = default;
  ...
  void InitializeWithState(const ModelType type, const ModelTypeState& state) {
    ...
    processor->SetDisconnectCallback(
        base::BindOnce(&HnsModelTypeWorkerTest::DisconnectProcessor,
                       base::Unretained(this)));

    worker_ = std::make_unique<HnsModelTypeWorker>(
  ...
  HnsModelTypeWorker* worker() { return worker_.get(); }
 ...
 private:
  ...
  std::unique_ptr<HnsModelTypeWorker> worker_;
...
TEST_F(HnsModelTypeWorkerTest, ResetProgressMarker) {
  g_minimal_time_between_reset_marker =
      HnsModelTypeWorker::MinimalTimeBetweenResetForTests();
  ...
  for (const auto& err : err_codes) {
    ...
    for (size_t i = 0;
         i < HnsModelTypeWorker::GetFailuresToResetMarkerForTests() - 1;
         ++i) {
      auto local_time_override = AdvanceTimeToAllowResetMarker();
      worker()->OnCommitResponse(CommitResponseDataList(),
...
TEST_F(HnsModelTypeWorkerTest, ResetProgressMarkerMaxPeriod) {
  NormalInitialize();
  auto error_response_list =
      MakeErrorResponseList(CommitResponse_ResponseType_CONFLICT);

  for (size_t i = 0;
       i < HnsModelTypeWorker::GetFailuresToResetMarkerForTests() - 1; ++i) {
...
TEST_F(HnsModelTypeWorkerTest, ResetProgressMarkerDisabledFeature) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndDisableFeature(features::kHnsSyncResetProgressMarker);
