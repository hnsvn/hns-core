...
class HnsContentSettingsAgentImplBrowserTest : public InProcessBrowserTest {
 public:
  HnsContentSettingsAgentImplBrowserTest()
      : https_server_(net::EmbeddedTestServer::TYPE_HTTPS) {
    feature_list_.InitAndEnableFeature(net::features::kHnsEphemeralStorage);
  }
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestMonitor(base::BindRepeating(
        &HnsContentSettingsAgentImplBrowserTest::SaveReferrer,
  ...
  void BlockReferrers() {
    content_settings()->SetContentSettingCustomScope(
        ...
        ContentSettingsType::HNS_REFERRERS, CONTENT_SETTING_BLOCK);
  ...
  void AllowReferrers() {
    content_settings()->SetContentSettingCustomScope(
        top_level_page_pattern(), ContentSettingsPattern::Wildcard(),
        ContentSettingsType::HNS_REFERRERS, CONTENT_SETTING_ALLOW);
    ContentSettingsForOneType settings =
        content_settings()->GetSettingsForOneType(
            ContentSettingsType::HNS_REFERRERS);
    // default plus new setting
  ...
  void ShieldsDown() {
    hns_shields::SetHnsShieldsEnabled(content_settings(), false,
                                          top_level_page_url());
  }

  void ShieldsUp() {
    hns_shields::SetHnsShieldsEnabled(content_settings(), true,
                                          top_level_page_url());
  }
  ...
  void BlockThirdPartyFingerprinting() {
    hns_shields::SetFingerprintingControlType(content_settings(),
...
class HnsContentSettingsAgentImplNoEphemeralStorageBrowserTest
    : public HnsContentSettingsAgentImplBrowserTest {
 public:
  HnsContentSettingsAgentImplNoEphemeralStorageBrowserTest() {
    feature_list_.Reset();
    feature_list_.InitAndDisableFeature(net::features::kHnsEphemeralStorage);
  }
};

IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplBrowserTest,
                       FarbleGetImageData) {
...
class HnsContentSettingsAgentImplV2BrowserTest
    : public HnsContentSettingsAgentImplBrowserTest {
 public:
  void SetUp() override { HnsContentSettingsAgentImplBrowserTest::SetUp(); }
...
#endif
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplV2BrowserTest,
...
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplV2BrowserTest,
                       FarbleGetImageData) {
  // Farbling should be default when kHnsFingerprintingV2 is enabled
  ...
  // settings and kHnsFingerprintingV2 is enabled
  BlockFingerprinting();
  NavigateToPageWithIframe();
  ...
  // content settings and kHnsFingerprintingV2 is enabled
  SetFingerprintingDefault();
  NavigateToPageWithIframe();
  EXPECT_EQ(kExpectedImageDataHashFarblingBalanced,
            content::EvalJs(contents(), kGetImageDataScript));
...
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplBrowserTest, BlockCookies) {
  BlockCookies();
...
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplBrowserTest, AllowCookies) {
  AllowCookies();
...
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplBrowserTest,
                       ChromiumCookieBlockOverridesHnsAllowCookiesTopLevel) {
  AllowCookies();
  HostContentSettingsMap* content_settings =
...
IN_PROC_BROWSER_TEST_F(
    HnsContentSettingsAgentImplNoEphemeralStorageBrowserTest,
    ChromiumCookieBlockOverridesHnsAllowCookiesIframe) {
  AllowCookies();
  HostContentSettingsMap* content_settings =
      HostContentSettingsMapFactory::GetForProfile(browser()->profile());
  content_settings->SetContentSettingCustomScope(
...
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplBrowserTest, BlockScripts) {
  BlockScripts();
...
IN_PROC_BROWSER_TEST_F(HnsContentSettingsAgentImplBrowserTest, AllowScripts) {
  AllowScripts();
