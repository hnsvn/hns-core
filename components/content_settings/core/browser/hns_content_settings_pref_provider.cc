...
#include "hns/components/content_settings/core/common/content_settings_util.h"
...
constexpr char kHnsShieldsFPSettingsMigration[] =
    "hns.shields_fp_settings_migration";
...
// static
void HnsPrefProvider::CopyPluginSettingsForMigration(PrefService* prefs) {
  ...
  prefs->SetDict("hns.migrate.content_settings.exceptions.plugins",
                 plugins.Clone());
...
HnsPrefProvider::HnsPrefProvider(PrefService* prefs,
                                     bool off_the_record,
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&HnsPrefProvider::OnCookiePrefsChanged,
  ...
  OnCookieSettingsChanged(ContentSettingsType::HNS_COOKIES);
...
HnsPrefProvider::~HnsPrefProvider() = default;

void HnsPrefProvider::ShutdownOnUIThread() {
  RemoveObserver(this);
...
// static
void HnsPrefProvider::RegisterProfilePrefs(
    user_prefs::PrefRegistrySyncable* registry) {
  PrefProvider::RegisterProfilePrefs(registry);
  // Register shields settings migration pref.
  registry->RegisterIntegerPref(kHnsShieldsSettingsVersion, 1);

  // migration of obsolete plugin prefs
  registry->RegisterDictionaryPref(
      "hns.migrate.content_settings.exceptions.plugins");
  ...
  registry->RegisterBooleanPref(kHnsShieldsFPSettingsMigration, false);
  registry->RegisterDictionaryPref(kObsoleteShieldCookies);
}

void HnsPrefProvider::MigrateShieldsSettings(bool incognito) {
  ...
  const int version = prefs_->GetInteger(kHnsShieldsSettingsVersion);
  ...
  // https://github.com/hnsvn/hns-browser/issues/23113
    ...
    if (!pref_change_registrar_.IsObserved(
      ...
      pref_change_registrar_.Add(
          ...
          base::BindPostTask(
              ...
              base::BindRepeating(&HnsPrefProvider::EnsureNoWildcardEntries,
...
void HnsPrefProvider::EnsureNoWildcardEntries(
    ContentSettingsType content_type) {
  // ContentSettingsType::HNS_SHIELDS should not have wildcard entries, i.e.
...
void HnsPrefProvider::MigrateShieldsSettingsFromResourceIds() {
  HnsPrefProvider::CopyPluginSettingsForMigration(prefs_);

  const auto& plugins_dict =
      prefs_->GetDict("hns.migrate.content_settings.exceptions.plugins");
  ...
  for (const auto [key, value] : plugins_dict) {
    ...
    if (resource_dict) {
      ...
      for (const auto&& [resource_key, resource_value] : *resource_dict) {
        ...
        if (resource_identifier == hns_shields::kObsoleteAds)
          shields_preference_name = hns_shields::kAds;
        else if (resource_identifier == hns_shields::kObsoleteCookies)
          shields_preference_name = hns_shields::kObsoleteShieldsCookies;
        else
        ...
        // Drop a "global" value of hns shields, that actually shouldn't exist
        // at all since we don't have any global toggle for this.
        if (shields_preference_name == hns_shields::kHnsShields &&
            patterns_string == "*,*") {
          continue;
        }
  ...
  prefs_->ClearPref("hns.migrate.content_settings.exceptions.plugins");
}

void HnsPrefProvider::MigrateShieldsSettingsFromResourceIdsForOneType(
...
void HnsPrefProvider::MigrateShieldsSettingsV1ToV2() {
  // Check if migration is needed.
  if (prefs_->GetInteger(kHnsShieldsSettingsVersion) != 1)
    return;

  // All sources in Hns-specific ContentSettingsType(s) we want to migrate.
  ...
  prefs_->SetInteger(kHnsShieldsSettingsVersion, 2);
}

void HnsPrefProvider::MigrateShieldsSettingsV2ToV3() {
  // Check if migration is needed.
  if (prefs_->GetInteger(kHnsShieldsSettingsVersion) != 2)
    return;
  ...
  auto rule_iterator =
      PrefProvider::GetRuleIterator(ContentSettingsType::HNS_COOKIES,
  ...
  while (rule_iterator && rule_iterator->HasNext()) {
    ...
    if (old_rule->primary_pattern == wildcard &&
      ...
      // Remove default rules from HNS_COKIES because it's already mapped to
      // the Chromium prefs.
      continue;
    }
  ...
  ClearAllContentSettingsRules(ContentSettingsType::HNS_COOKIES);
  ...
  // Mark migration as done.
  prefs_->SetInteger(kHnsShieldsSettingsVersion, 3);
}

void HnsPrefProvider::MigrateShieldsSettingsV3ToV4(int start_version) {
  if (prefs_->GetInteger(kHnsShieldsSettingsVersion) != 3)
    return;

  if (start_version == 3) {
    // Because of
    // https://github.com/hnsvn/hns-browser/issues/24119 the cookies
    // overwritten by the obsolete cookies. Repeat v2 to v3 migration.
    prefs_->SetInteger(kHnsShieldsSettingsVersion, 2);
    MigrateShieldsSettingsV2ToV3();
  }
  prefs_->SetInteger(kHnsShieldsSettingsVersion, 4);
}

void HnsPrefProvider::MigrateShieldsSettingsV1ToV2ForOneType(
    ContentSettingsType content_type) {
  using OldRule = std::pair<ContentSettingsPattern, ContentSettingsPattern>;
  // Find rules that can be migrated and create replacement rules for them.
  std::vector<OldRule> old_rules;
...
void HnsPrefProvider::MigrateFingerprintingSettings() {
  if (prefs_->GetBoolean(kHnsShieldsFPSettingsMigration) || off_the_record_)
    return;
  ...
  prefs_->SetBoolean(kHnsShieldsFPSettingsMigration, true);
}

void HnsPrefProvider::MigrateFingerprintingSetingsToOriginScoped() {
  if (off_the_record_)
    return;
...
bool HnsPrefProvider::SetWebsiteSetting(
  ...
  if (content_type == ContentSettingsType::COOKIES) {
    if (cookie_is_found_in(hns_shield_down_rules_[off_the_record_])) {
      // Don't do anything with the generated Shiels-down rules. Unremovable
      // rule.
      return true;
    }
    if (cookie_is_found_in(hns_cookie_rules_[off_the_record_])) {
      // change to type ContentSettingsType::HNS_COOKIES
...
bool HnsPrefProvider::SetWebsiteSettingForTest(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
    base::Value&& value,
...
bool HnsPrefProvider::SetWebsiteSettingInternal(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
    base::Value&& in_value,
  ...
  if (content_settings::IsShieldsContentSettingsType(content_type) &&
    ...
    if (content_type == ContentSettingsType::HNS_COOKIES) {
      // Default value for HNS_COOKIES handled in chromium code. This value
      // based on default COOKIES value (which provided by DefaultPrefProvider)
      // and kCookieControlsMode pref (default value in
      // hns::SetDefaultThirdPartyCookieBlockValue).
      return false;
    }
  ...
  if (content_type == ContentSettingsType::HNS_FINGERPRINTING_V2 &&
...
std::unique_ptr<RuleIterator> HnsPrefProvider::GetRuleIterator(
    ContentSettingsType content_type,
...
void HnsPrefProvider::UpdateCookieRules(ContentSettingsType content_type,
                                          bool incognito) {
  std::vector<std::unique_ptr<OwnedRule>> rules;
  auto old_rules = std::move(hns_cookie_rules_[incognito]);
  auto old_shields_down_rules = std::move(hns_shield_down_rules_[incognito]);
  hns_cookie_rules_[incognito].clear();
  ...
  if (!google_sign_in_flag_enabled &&
    ...
    hns_cookie_rules_[incognito].emplace_back(
    ...
    hns_cookie_rules_[incognito].emplace_back(CloneRule(firebase_rule.get()));
  } else if (google_sign_in_flag_enabled) {
    // Google Sign-In feature:
    // Add per-site cookie exception for Google/Firebase auth domains.
    // Get all sites that have HNS_GOOGLE_SIGN_IN turned on, and add exception
    // for them
    auto google_sign_in_content_setting_it = PrefProvider::GetRuleIterator(
        ContentSettingsType::HNS_GOOGLE_SIGN_IN, incognito);
    while (google_sign_in_content_setting_it &&
      ...
      // for the HNS_GOOGLE_SIGN_IN permission.
  ...
  {
    auto hns_shields_iterator = PrefProvider::GetRuleIterator(
        ContentSettingsType::HNS_SHIELDS, incognito);
    while (hns_shields_iterator && hns_shields_iterator->HasNext()) {
      shield_rules.emplace_back(
          CloneRule(hns_shields_iterator->Next().get()));
    }
  }

  // Add hns cookies after checking shield status.
  {
    auto hns_cookies_iterator = PrefProvider::GetRuleIterator(
        ContentSettingsType::HNS_COOKIES, incognito);
    // Matching cookie rules against shield rules.
    while (hns_cookies_iterator && hns_cookies_iterator->HasNext()) {
      auto rule = hns_cookies_iterator->Next();
      if (IsActive(rule.get(), shield_rules)) {
        rules.emplace_back(CloneRule(rule.get()));
        hns_cookie_rules_[incognito].emplace_back(CloneRule(rule.get()));
      }
    }
  }
  ...
  for (const auto& shield_rule : shield_rules) {
    ...
    if (ValueToContentSetting(shield_rule->value()) == CONTENT_SETTING_BLOCK) {
      ...
      rules.emplace_back(std::make_unique<OwnedRule>(
          ...
          ContentSettingToValue(CONTENT_SETTING_ALLOW), metadata));
      hns_shield_down_rules_[incognito].emplace_back(
          std::make_unique<OwnedRule>(
              ContentSettingsPattern::Wildcard(), shield_rule->primary_pattern,
              ContentSettingToValue(CONTENT_SETTING_ALLOW), metadata));
      hns_cookie_rules_[incognito].emplace_back(std::make_unique<OwnedRule>(
          ContentSettingsPattern::Wildcard(), shield_rule->primary_pattern,
          ContentSettingToValue(CONTENT_SETTING_ALLOW), metadata));
    }
  }
  ...
  std::vector<std::unique_ptr<OwnedRule>> hns_cookie_updates;
  for (const auto& new_rule : hns_cookie_rules_[incognito]) {
    auto match =
    ...
    if (match == old_rules.end()) {
      hns_cookie_updates.emplace_back(CloneRule(new_rule.get()));
    }
  }
  ...
  for (const auto& old_rule : old_rules) {
    auto match = base::ranges::find_if(
        hns_cookie_rules_[incognito], [&old_rule](const auto& new_rule) {
          // We only care about the patterns here because we're looking for
          // deleted rules, not changed rules.
          return new_rule->primary_pattern == old_rule->primary_pattern &&
                 new_rule->secondary_pattern == old_rule->secondary_pattern;
        });
    if (match == hns_cookie_rules_[incognito].end()) {
      hns_cookie_updates.emplace_back(std::make_unique<OwnedRule>(
  ...
  // Notify hns cookie changes as ContentSettingsType::COOKIES.
  if (initialized_ &&
      (content_type == ContentSettingsType::HNS_COOKIES ||
       content_type == ContentSettingsType::HNS_SHIELDS ||
       content_type == ContentSettingsType::HNS_GOOGLE_SIGN_IN)) {
    NotifyChanges(hns_cookie_updates, incognito);
  }
}

void HnsPrefProvider::NotifyChanges(
    const std::vector<std::unique_ptr<OwnedRule>>& rules,
    bool incognito) {
  for (const auto& rule : rules) {
...
void HnsPrefProvider::OnCookiePrefsChanged(const std::string& pref) {
  OnCookieSettingsChanged(ContentSettingsType::HNS_COOKIES);
}

void HnsPrefProvider::OnCookieSettingsChanged(
    ContentSettingsType content_type) {
  UpdateCookieRules(content_type, true);
  UpdateCookieRules(content_type, false);
}

void HnsPrefProvider::OnContentSettingChanged(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type) {
  if (content_type == ContentSettingsType::COOKIES ||
