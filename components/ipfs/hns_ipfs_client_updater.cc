...
std::string HnsIpfsClientUpdater::g_ipfs_client_component_id_(
    kIpfsClientComponentId);
std::string HnsIpfsClientUpdater::g_ipfs_client_component_base64_public_key_(
    kIpfsClientComponentBase64PublicKey);

HnsIpfsClientUpdater::HnsIpfsClientUpdater(
    HnsComponent::Delegate* delegate,
    const base::FilePath& user_data_dir)
    : HnsComponent(delegate),
      task_runner_(
...
HnsIpfsClientUpdater::~HnsIpfsClientUpdater() = default;

void HnsIpfsClientUpdater::Register() {
  if (registered_)
    return;

  HnsComponent::Register(kIpfsClientComponentName,
...
void HnsIpfsClientUpdater::SetExecutablePath(const base::FilePath& path) {
  executable_path_ = path;
  for (Observer& observer : observers_)
    observer.OnExecutableReady(path);
}

base::FilePath HnsIpfsClientUpdater::GetExecutablePath() const {
  return executable_path_;
}

void HnsIpfsClientUpdater::OnEvent(Events event, const std::string& id) {
...
void HnsIpfsClientUpdater::OnComponentReady(const std::string& component_id,
                                              const base::FilePath& install_dir,
                                              const std::string& manifest) {
  GetTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&InitExecutablePath, install_dir),
      base::BindOnce(&HnsIpfsClientUpdater::SetExecutablePath,
                     weak_ptr_factory_.GetWeakPtr()));
}

void HnsIpfsClientUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void HnsIpfsClientUpdater::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

void HnsIpfsClientUpdater::Cleanup() {
...
// static
void HnsIpfsClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
...
// The Hns Ipfs client extension factory.
std::unique_ptr<HnsIpfsClientUpdater> HnsIpfsClientUpdaterFactory(
    HnsComponent::Delegate* delegate,
    const base::FilePath& user_data_dir) {
  return std::make_unique<HnsIpfsClientUpdater>(delegate, user_data_dir);
}
