...
#ifndef HNS_COMPONENTS_IPFS_HNS_IPFS_CLIENT_UPDATER_H_
#define HNS_COMPONENTS_IPFS_HNS_IPFS_CLIENT_UPDATER_H_
...
class HnsIpfsClientUpdaterTest;
...
static const char kIpfsClientComponentName[] =
    "Hns IPFS Client Updater (Windows)";
...
class HnsIpfsClientUpdater : public HnsComponent,
                               public HnsComponent::ComponentObserver {
 public:
  class Observer : public base::CheckedObserver {
   public:
    virtual void OnExecutableReady(const base::FilePath& path) = 0;
    virtual void OnInstallationEvent(
        HnsComponent::ComponentObserver::Events event) = 0;

   protected:
    ~Observer() override = default;
  };

  explicit HnsIpfsClientUpdater(HnsComponent::Delegate* delegate,
                                  const base::FilePath& user_data_dir);
  HnsIpfsClientUpdater(const HnsIpfsClientUpdater&) = delete;
  HnsIpfsClientUpdater& operator=(const HnsIpfsClientUpdater&) = delete;
  ~HnsIpfsClientUpdater() override;
 ...
 protected:
  ...
  // HnsComponent::ComponentObserver
  void OnEvent(Events event, const std::string& id) override;

 private:
  friend class ::HnsIpfsClientUpdaterTest;
  ...
  base::ScopedObservation<HnsComponent, HnsComponent::ComponentObserver>
      updater_observer_{this};

  base::WeakPtrFactory<HnsIpfsClientUpdater> weak_ptr_factory_;
};

// Creates the HnsIpfsClientUpdater
...
#endif  // HNS_COMPONENTS_IPFS_HNS_IPFS_CLIENT_UPDATER_H_
