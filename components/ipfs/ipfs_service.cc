...
#include "hns/components/ipfs/blob_context_getter_factory.h"
#include "hns/components/ipfs/buildflags/buildflags.h"
#include "hns/components/ipfs/ipfs_constants.h"
#include "hns/components/ipfs/ipfs_json_parser.h"
#include "hns/components/ipfs/ipfs_network_utils.h"
#include "hns/components/ipfs/ipfs_ports.h"
...
#include "hns/components/ipfs/service_sandbox_type.h"
...
#include "hns/components/ipfs/import/ipfs_import_worker_base.h"
#include "hns/components/ipfs/import/ipfs_link_import_worker.h"
...
IpfsService::IpfsService(
    ...
    ipfs::HnsIpfsClientUpdater* ipfs_client_updater,
  ...
  // Return early since g_hns_browser_process and ipfs_client_updater are not
...
base::FilePath IpfsService::GetDataPath() const {
  return user_data_dir_.Append(FILE_PATH_LITERAL("hns_ipfs"));
}
...
#endif
  return hns::ProcessLauncher::ReadAppOutput(command_line, options, 10);
}
...
void IpfsService::PreWarmShareableLink(const GURL& url) {
  ...
  auto iter =
      requests_list_.insert(requests_list_.begin(), std::move(url_loader));
  iter->get()->Request("HEAD", url, std::string(), std::string(),
