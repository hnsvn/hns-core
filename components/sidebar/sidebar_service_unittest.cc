...
#include "hns/components/sidebar/sidebar_p3a.h"
...
TEST_F(SidebarServiceTest, UpdateItem) {
  ...
  SidebarItem hns_item;
  const GURL hns_url("https://hns.com/");
  const std::u16string hns_title(u"Hns software");
  hns_item.url = hns_url;
  hns_item.title = hns_title;
  hns_item.type = SidebarItem::Type::kTypeWeb;
  hns_item.built_in_item_type = SidebarItem::BuiltInItemType::kNone;
  service_->AddItem(hns_item);
  last_item_index++;
  EXPECT_EQ(hns_url, service_->items()[last_item_index].url);
  ...
  service_->UpdateItem(hns_url, hns_url, hns_title, u"Hns");
  testing::Mock::VerifyAndClearExpectations(&observer_);
  EXPECT_EQ(u"Hns", service_->items()[last_item_index].title);

  // Update title & url.
  update.title_updated = true;
  update.url_updated = true;
  EXPECT_CALL(observer_, OnItemUpdated(testing::_, update)).Times(1);
  service_->UpdateItem(hns_url, GURL("https://a.com/"), u"Hns", u"a");
  testing::Mock::VerifyAndClearExpectations(&observer_);
  ...
  // check item is not updated and it still has hns url.
  EXPECT_CALL(observer_, OnItemUpdated(testing::_, testing::_)).Times(0);
  service_->UpdateItem(hns_url, GURL("https://a.com/"), u"a", u"ab");
  testing::Mock::VerifyAndClearExpectations(&observer_);
  EXPECT_EQ(hns_url, service_->items()[last_item_index].url);
}
...
TEST_F(SidebarServiceTest, HideBuiltInItem) {
  ...
  }
  {
    ...
    dict.Set(sidebar::kSidebarItemURLKey, "https://custom1.hns.com/");
...
TEST_F(SidebarServiceTest, NewDefaultItemAdded) {
  ...
  std::vector<SidebarItem::BuiltInItemType> remaining_default_items{
      SidebarItem::BuiltInItemType::kHnsTalk,
...
TEST_F(SidebarServiceTest, MigratePrefSidebarBuiltInItemsSomeHidden) {
  ...
  {
    base::Value::Dict dict;
    dict.Set(sidebar::kSidebarItemURLKey, "https://anything.hns.com/");
    ...
    dict.Set(sidebar::kSidebarItemBuiltInItemTypeKey,
             static_cast<int>(SidebarItem::BuiltInItemType::kHnsTalk));
  ...
  // Verify service has migrated the "include" of kHnsTalk to be the exclude
  ...
  auto items = service_->items();
  auto talk_iter =
      base::ranges::find(items, SidebarItem::BuiltInItemType::kHnsTalk,
                         &SidebarItem::built_in_item_type);
  ...
  EXPECT_EQ(talk_item.url, kHnsTalkURL);
}
...
TEST_F(SidebarServiceTest, BuiltInItemUpdateTestWithBuiltInItemTypeKey) {
  ...
  {
    base::Value::Dict dict;
    dict.Set(sidebar::kSidebarItemURLKey, "https://deprecated.hns.com/");
    dict.Set(sidebar::kSidebarItemTitleKey, "Hns together");
    dict.Set(sidebar::kSidebarItemTypeKey,
             static_cast<int>(SidebarItem::Type::kTypeBuiltIn));
    dict.Set(sidebar::kSidebarItemBuiltInItemTypeKey,
             static_cast<int>(SidebarItem::BuiltInItemType::kHnsTalk));
  ...
  EXPECT_EQ(2UL, service_->items().size());
  EXPECT_EQ(GURL(kHnsTalkURL), service_->items()[0].url);
