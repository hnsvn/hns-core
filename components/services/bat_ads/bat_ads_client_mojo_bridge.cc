...
#include "hns/components/hns_federated/public/interfaces/hns_federated.mojom.h"  // IWYU pragma: keep
...
void BatAdsClientMojoBridge::AddObserver(
    hns_ads::AdsClientNotifierObserver* observer) {
...
void BatAdsClientMojoBridge::ShowNotificationAd(
    const hns_ads::NotificationAdInfo& ad) {
  if (bat_ads_client_.is_bound()) {
    bat_ads_client_->ShowNotificationAd(hns_ads::NotificationAdToValue(ad));
  }
}

void BatAdsClientMojoBridge::ShowReminder(
    const hns_ads::mojom::ReminderType type) {
  if (bat_ads_client_.is_bound()) {
...
void OnUrlRequest(hns_ads::UrlRequestCallback callback,
                  const hns_ads::mojom::UrlResponseInfoPtr url_response_ptr) {
...
void BatAdsClientMojoBridge::UrlRequest(
    hns_ads::mojom::UrlRequestInfoPtr url_request,
    hns_ads::UrlRequestCallback callback) {
  if (!bat_ads_client_.is_bound()) {
...
void BatAdsClientMojoBridge::LoadFileResource(
    const std::string& id,
    const int version,
    hns_ads::LoadFileCallback callback) {
  if (!bat_ads_client_.is_bound()) {
...
void BatAdsClientMojoBridge::GetBrowsingHistory(
    const int max_count,
    const int days_ago,
    hns_ads::GetBrowsingHistoryCallback callback) {
  if (!bat_ads_client_.is_bound()) {
...
void BatAdsClientMojoBridge::RunDBTransaction(
  ...
  if (!bat_ads_client_.is_bound()) {
    std::move(callback).Run(hns_ads::mojom::DBCommandResponseInfoPtr());
    return;
  }
...
void BatAdsClientMojoBridge::GetScheduledCaptcha(
    ...
    hns_ads::GetScheduledCaptchaCallback callback) {
  if (!bat_ads_client_.is_bound()) {
