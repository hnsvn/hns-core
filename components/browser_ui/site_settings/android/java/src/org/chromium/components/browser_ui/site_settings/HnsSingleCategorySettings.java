...
@UsedByReflection("hns_site_settings_preferences.xml")
public class HnsSingleCategorySettings
    ...
    public String getAddExceptionDialogMessage() {
        ...
        int resource = 0;
        SiteSettingsCategory mCategory = (SiteSettingsCategory) HnsReflectionUtil.getField(
        ...
        } else if (mCategory.getType() == SiteSettingsCategory.Type.HNS_GOOGLE_SIGN_IN) {
            resource = R.string.website_settings_google_sign_in_allow_exceptions;
        } else if (mCategory.getType() == SiteSettingsCategory.Type.HNS_LOCALHOST_ACCESS) {
    ...
    public void resetList() {
        HnsReflectionUtil.InvokeMethod(SingleCategorySettings.class, this, "resetList");
        BrowserContextHandle browserContextHandle =
                getSiteSettingsDelegate().getBrowserContextHandle();
        ...
        if (mCategory.getType() == SiteSettingsCategory.Type.AUTOPLAY
                || mCategory.getType() == SiteSettingsCategory.Type.HNS_GOOGLE_SIGN_IN
                || mCategory.getType() == SiteSettingsCategory.Type.HNS_LOCALHOST_ACCESS) {
            exception = true;
        }
        if (exception) {
