...
TEST(UserAgentUtilsTest, UserAgentMetadata) {
  ...
  const blink::UserAgentBrandVersion product_brand_version = {"Hns",
                                                              major_version};
  EXPECT_TRUE(
...
TEST(UserAgentUtilsTest, DoNotClampPlatformVersion) {
  ...
  auto hns_metadata = GetUserAgentMetadata(nullptr);
  EXPECT_EQ(metadata, hns_metadata);
}
...
TEST(UserAgentUtilsTest, ClampPlatformVersion) {
  ...
  base::Version hns_platform_version(hns_metadata.platform_version);

  EXPECT_EQ(3u, platform_version.components().size());
  EXPECT_EQ(3u, hns_platform_version.components().size());
  EXPECT_EQ(platform_version.components()[0],
            hns_platform_version.components()[0]);
  EXPECT_EQ(platform_version.components()[1],
            hns_platform_version.components()[1]);
  EXPECT_NE(platform_version.components()[2],
            hns_platform_version.components()[2]);
  EXPECT_EQ(kClampedValue,
            base::NumberToString(hns_platform_version.components()[2]));
}
...
TEST(UserAgentUtilsTest, UserAgentFromCommandLine) {
  ...
  const auto hns_metadata = GetUserAgentMetadata(nullptr);
  const blink::UserAgentMetadata empty_metadata;

  EXPECT_EQ(GetUserAgent(), kCmdUserAgentValue);
  EXPECT_EQ(hns_metadata, empty_metadata);
}
