...
const StyledPage = styled('div')<PageProps>`
  ...
  /* Fix the main NTP content so, when Hns News is in-view,
  ...
  .${CLASSNAME_PAGE_STUCK} & {
    ...
    position: fixed;
    bottom: 0;
    /* Blur out the content when Hns News is interacted
...
export const Page: React.FunctionComponent<PageProps> = (props) => {
  ...
  // extra scroll area (Hns News) then we "sticky" the Page at
...
export const GridItemNavigationHnsNews = styled('div')<{}>`
  position: absolute;
  bottom: 20px;
  left: 50%;
...
function getPageBackground (p: HasImageProps) {
  ...
  // and Page is the element with blur (when Hns News is active)
...
export const IconButton = styled('button')<IconButtonProps>`
  ...
  &:focus-visible {
    outline: 2px solid ${p => p.theme.color.brandHnsInteracting};
  }
`
