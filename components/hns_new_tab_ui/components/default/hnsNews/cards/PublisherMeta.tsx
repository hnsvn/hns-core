...
import { Publisher } from '../../../../api/hns_news'
...
export default function PublisherMetaComponent (props: Props) {
  ...
  const commandText = React.useMemo<string>(() => {
    const raw = getLocale('hnsNewsDisableSourceCommand')
    const publisherIndex = raw.indexOf('$1')
    if (publisherIndex === -1) {
      console.warn('Locale string for hnsNewsDisableSourceCommand did not have a $1 replacement area for publisher name.', raw)
