...
import * as HnsNewsElement from './default'
...
export type GetDisplayAdContent = HnsNews.HnsNewsControllerRemote['getDisplayAd']
...
export type Props = {
  ...
  articleToScrollTo?: HnsNews.FeedItemMetadata
...
export default function HnsNewsSection (props: Props) {
  const dispatch = useDispatch()
  const [optedIn, setOptedIn] = useNewTabPref('isHnsNewsOptedIn')
  ...
  const loadDataObserver = React.useMemo<PossibleInteractionObserver>(() => {
    ...
    const handleHits: IntersectionObserverCallback = (entries) => {
      ...
      if (isIntersecting && !hasRequestedLoad.current) {
        ...
        console.debug('Hns News: section is in position that requires requesting data load')
