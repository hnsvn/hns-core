...
export default function SourcesList () {
  const { subscribedPublisherIds } = useHnsNews()
  const channels = useChannels({ subscribedOnly: true })

  const { result: sourcesCount } = usePromise(async () => PluralStringProxyImpl.getInstance().getPluralString('hnsNewsSourceCount', subscribedPublisherIds.length + channels.length), [subscribedPublisherIds.length, channels.length])

  return <div>
    <Flex direction="row" justify="space-between" align="center">
      <Title>{getLocale('hnsNewsFeedsHeading')}</Title>
