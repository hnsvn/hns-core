...
const ToggleButton = styled.button`
  ...
  flex: 0 0 auto;
  cursor: pointer;
  color: var(--hns-color-text02);
  &:hover {
    text-decoration: underline;
  }
  &:active {
    color: var(--hns-color-interactive08);
  }
  &:focus-visible {
    outline: 1px solid var(--hns-color-focusBorder);
    outline-offset: 4px;
  }
`
...
function FavIcon (props: { publisherId: string }) {
  const { url, setElementRef } = useLazyFavicon(props.publisherId, {
    rootElement: document.getElementById('hns-news-configure'),
  ...
  return (
    <Container direction="row" justify="space-between" align='center' gap={8}>
      ...
      <ToggleButton onClick={() => setFollowed(false)}>
        {getLocale('hnsNewsFollowButtonFollowing')}
      </ToggleButton>
    </Container>
  )
}
