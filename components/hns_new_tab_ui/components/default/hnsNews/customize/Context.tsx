...
import getHnsNewsController, { Channels, Publisher, Publishers, PublisherType, isPublisherEnabled } from '../../../../api/hns_news'
import Modal from './Modal'
import { PublishersCachingWrapper } from '../../../../api/hns_news/publishersCache'
import { ChannelsCachingWrapper } from '../../../../api/hns_news/channelsCache'
...
interface HnsNewsContext {
  ...
  toggleHnsNewsOnNTP: (enabled: boolean) => void
}

export const HnsNewsContext = React.createContext<HnsNewsContext>({
  locale: '',
  customizePage: null,
  setCustomizePage: () => { },
  publishers: {},
  ...
  toggleHnsNewsOnNTP: (enabled: boolean) => {}
})

const publishersCache = new PublishersCachingWrapper()
const channelsCache = new ChannelsCachingWrapper()

export function HnsNewsContextProvider (props: { children: React.ReactNode }) {
  ...
  // not use useNewTabPref here so that we can move Hns News to a shared component.
  // But for now we're tied to NTP.
  const [isOptInPrefEnabled, setOptInPrefEnabled] = useNewTabPref('isHnsNewsOptedIn')
  const [isShowOnNTPPrefEnabled, setShowOnNTPPrefEnabled] = useNewTabPref('showToday')

  // Get the default locale on load.
  useEffect(() => {
    getHnsNewsController().getLocale().then(({ locale }) => setLocale(locale))
  ...
  const updateSuggestedPublisherIds = useCallback(async () => {
    ...
    const { suggestedPublisherIds } = await getHnsNewsController().getSuggestedPublisherIds()
  ...
  const toggleHnsNewsOnNTP = (shouldEnable: boolean) => {
    if (shouldEnable) {
  ...
  const context = useMemo<HnsNewsContext>(() => ({
    locale,
    customizePage,
    setCustomizePage,
    channels,
    ...
    toggleHnsNewsOnNTP
  }), [customizePage, channels, publishers, suggestedPublisherIds, updateSuggestedPublisherIds, isOptInPrefEnabled, isShowOnNTPPrefEnabled, toggleHnsNewsOnNTP])

  return <HnsNewsContext.Provider value={context}>
    {props.children}
    <Modal />
  </HnsNewsContext.Provider>
}

export const useHnsNews = () => {
  return React.useContext(HnsNewsContext)
}

export const useChannels = (options: { subscribedOnly: boolean } = { subscribedOnly: false }) => {
  const { channels } = useHnsNews()
...
export const useChannelSubscribed = (channelName: string) => {
  const { channels, locale } = useHnsNews()
