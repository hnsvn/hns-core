...
function useFavicon (publisherId: string | undefined) {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false
    getHnsNewsController().getFavIconData(publisherId).then(({ imageData }) => {
      if (cancelled) return
      if (!imageData) return
...
export function useUnpaddedImageUrl (paddedUrl: string | undefined, onLoaded?: () => any, useCache?: boolean) {
  ...
  React.useEffect(() => {
    ...
    // Storybook method
    // @ts-expect-error
    if (window.hnsStorybookUnpadUrl) {
      // @ts-expect-error
      window.hnsStorybookUnpadUrl(paddedUrl)
    ...
    let blobUrl: string
    getHnsNewsController().getImageData({ url: paddedUrl })
      .then(async (result) => {
        if (!result.imageData) {
          return
        }
