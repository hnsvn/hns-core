...
import { useHnsNews } from './customize/Context'
...
export default function HnsNewsContent (props: Props) {
  const { feed, publishers } = props

  const dispatch = useDispatch()
  const { setCustomizePage } = useHnsNews()
  ...
  const setScrollTriggerRef = React.useCallback((element) => {
    ...
    const endOfCurrentArticlesListObserver = new IntersectionObserver((entries) => {
      console.debug('Hns News content Intersection Observer triggered', entries)
      const hasReachedPaginationPoint = entries.some(
        entry => entry.intersectionRatio > 0 ||
                  entry.boundingClientRect.top < 0)
      if (hasReachedPaginationPoint) {
        console.debug('Hns News content Intersection Observer determined need new page.')
  ...
  const interactionObserver = React.useMemo<IntersectionObserver>(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      console.debug('Hns News: Intersection Observer trigger show options, changing', isInteracting)
  ...
  const intersectionObserver = React.useRef(new IntersectionObserver(entries => {
    ...
    console.debug(`Hns News: viewed ${count} cards.`)
  ...
  if (!feed || !publishers) {
    console.error('Hns News: should have shown error or loading state, but ran in to an unintended code path.')
    return null
  }
  ...
  return (
    <>
      ...
      <CardNoContent onCustomize={props.onCustomizeHnsNews} />}
      {/* featured item */}
      ...
      {!isOnlyDisplayingPeekingCard &&
        <>
          ...
          <Customize onCustomizeHnsNews={() => setCustomizePage('news')} show={showOptions} />
