...
function getWidgetStackOrder (firstWidget: string): NewTab.StackWidget[] {
  switch (firstWidget) {
    case 'hnsTalk':
      return ['rewards', 'hnsTalk']
    default:
      return ['hnsTalk', 'rewards']
  }
}
...
export const useNewTabData = (state: NewTab.State = defaultState) => {
  const result: NewTab.State = {
    ...
    featureFlagHnsNTPSponsoredImagesWallpaper: true,
    featureCustomBackgroundEnabled: true,
    featureFlagHnsNewsPromptEnabled: true,
    ...
    showToday: boolean('Show Hns News?', true),
    showClock: boolean('Show clock?', true),
    clockFormat: select('Clock format?', ['', '12', '24'], ''),
    showTopSites: boolean('Show top sites?', true),
    hnsRewardsSupported: boolean('Hns Rewards supported?', true),
    showRewards: boolean('Show rewards?', true),
    showHnsTalk: boolean('Show Hns Talk?', true),
    hnsTalkSupported: boolean('Hns Talk supported?', true),
    hideAllWidgets: boolean('Hide all widgets?', false),
    isHnsNewsOptedIn: boolean('Hns News opted-in?', false),
    ...
    widgetStackOrder: getWidgetStackOrder(select('First widget', ['hnsTalk', 'rewards'], 'rewards'))
  }
