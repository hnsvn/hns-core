...
export function wireApiEventsToStore () {
  ...
  .then((initialData) => {
    ...
    backgroundData.updateImages(initialData.hnsBackgrounds)
...
export function rewardsInitData () {
  getRewardsPreInitialData().then((preInitialRewardsData) => {
    ...
    chrome.hnsRewards.isInitialized((isInitialized) => {
      if (isInitialized) {
...
chrome.hnsRewards.initialized.addListener(fetchRewardsData)

chrome.hnsRewards.onRewardsWalletCreated.addListener(fetchRewardsData)

chrome.hnsRewards.onPromotions.addListener((result: number, promotions: NewTab.Promotion[]) => {
  getActions().onPromotions(result, promotions)
})

chrome.hnsRewards.onPromotionFinish.addListener((result: number, promotion: NewTab.Promotion) => {
  getActions().onPromotionFinish(result, promotion)
})

chrome.hnsRewards.onCompleteReset.addListener((properties: { success: boolean }) => {
  getActions().onCompleteReset(properties.success)
})

chrome.hnsRewards.onExternalWalletLoggedOut.addListener(fetchRewardsData)

chrome.hnsRewards.onExternalWalletDisconnected.addListener(fetchRewardsData)
