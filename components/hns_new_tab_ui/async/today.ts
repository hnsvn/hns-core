...
import getHnsNewsController, * as HnsNews from '../api/hns_news'
import store from '../store'
import { addFeedListener } from '../api/hns_news/feedListener'
...
handler.on(Actions.interactionBegin.getType(), async () => {
  console.debug('Hns News: Marking actual interaction begin')
  getHnsNewsController().onInteractionSessionStarted()
})
...
handler.on<Actions.RefreshPayload>(
  ...
  async (store, payload) => {
    ...
    try {
      console.debug('Hns News: Getting data...')
      const [{ feed }, { publishers }] = await Promise.all([
        getHnsNewsController().getFeed(),
        getHnsNewsController().getPublishers()
      ])
      console.debug('Hns News: ...data received.')
...
handler.on(Actions.ensureSettingsData.getType(), async (store) => {
  ...
  const { publishers } = await getHnsNewsController().getPublishers()
...
handler.on<Actions.ReadFeedItemPayload>(
  ...
  async (store, payload) => {
    const state = store.getState() as ApplicationState
    getHnsNewsController().onSessionCardVisitsCountChanged(
    ...
    if (payload.isPromoted) {
      ...
      if (!promotedArticle) {
        console.error(
          'Hns News: readFeedItem payload with invalid promoted article',
          payload
        )
        return
      }
      if (!payload.promotedUUID) {
        console.error(
          'Hns News: invalid promotedUUID for readFeedItem',
          payload
        )
        return
      }
      getHnsNewsController().onPromotedItemVisit(
    ...
    if (!data) {
      console.error(
        'Hns News: readFeedItem payload item not present',
        payload
      )
      return
    }
...
handler.on<Actions.PromotedItemViewedPayload>(
  ...
  async (store, payload) => {
    if (!payload.item.promotedArticle) {
      console.error(
        'Hns News: promotedItemViewed invalid promoted article',
        payload
      )
      return
    }
    getHnsNewsController().onPromotedItemView(
      payload.uuid,
...
handler.on<number>(
  ...
  async (store, payload) => {
    const state = store.getState() as ApplicationState
    getHnsNewsController().onSessionCardViewsCountChanged(
...
handler.on<Actions.RemoveDirectFeedPayload>(
  ...
  async (store, payload) => {
    getHnsNewsController().removeDirectFeed(payload.directFeed.publisherId)
    window.setTimeout(() => {
...
handler.on<Actions.SetPublisherPrefPayload>(
  ...
  async (store, payload) => {
    ...
    let userStatus =
      enabled === null
        ? HnsNews.UserEnabled.NOT_MODIFIED
        : enabled
        ? HnsNews.UserEnabled.ENABLED
        : HnsNews.UserEnabled.DISABLED
    getHnsNewsController().setPublisherPref(publisherId, userStatus)
...
handler.on(Actions.checkForUpdate.getType(), async function (store) {
  ...
  const isUpdateAvailable: { isUpdateAvailable: boolean } =
    await getHnsNewsController().isFeedUpdateAvailable(hash)
...
handler.on(Actions.resetTodayPrefsToDefault.getType(), async function (store) {
  getHnsNewsController().clearPrefs()
  const { publishers } = await getHnsNewsController().getPublishers()
  store.dispatch(Actions.dataReceived({ publishers }))
  store.dispatch(Actions.checkForUpdate())
})
...
handler.on<Actions.VisitDisplayAdPayload>(
  ...
  async function (store, payload) {
    ...
    getHnsNewsController().onDisplayAdVisit(
      payload.ad.uuid,
...
handler.on<Actions.DisplayAdViewedPayload>(
  ...
  async (store, item) => {
    getHnsNewsController().onDisplayAdView(
      item.ad.uuid,
