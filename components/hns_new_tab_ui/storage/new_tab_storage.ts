...
export const defaultState: NewTab.State = {
  ...
  featureFlagHnsNTPSponsoredImagesWallpaper: loadTimeData.getBoolean('featureFlagHnsNTPSponsoredImagesWallpaper'),
  featureFlagHnsNewsPromptEnabled: loadTimeData.getBoolean('featureFlagHnsNewsPromptEnabled'),
  ...
  isHnsNewsOptedIn: false,
  showEmptyPage: false,
  hnsRewardsSupported: false,
  hnsTalkSupported: false,
...
// as a result of https://github.com/hnsvn/hns-browser/issues/10067
...
export const debouncedSave = debounce<NewTab.State>((data: NewTab.State) => {
  if (data) {
    ...
    // (or are obsolete).
    const dataToSave = {
      hnsRewardsSupported: data.hnsRewardsSupported,
      hnsTalkSupported: data.hnsTalkSupported,
