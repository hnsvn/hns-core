...
import {
  HnsTalkWidget as HnsTalk, Clock, EditCards, EditTopSite, OverrideReadabilityColor, RewardsWidget as Rewards, SearchPromotion
} from '../../components/default'
import BrandedWallpaperLogo from '../../components/default/brandedWallpaper/logo'
import HnsNews, { GetDisplayAdContent } from '../../components/default/hnsNews'
...
import { HnsNewsContextProvider } from '../../components/default/hnsNews/customize/Context'
import HnsNewsHint from '../../components/default/hnsNews/hint'
import GridWidget from './gridWidget'
import SponsoredImageClickArea from '../../components/default/sponsoredImage/sponsoredImageClickArea'

import Icon, { setIconBasePath } from '@hns/leo/react/icon'
...
interface Props {
  ...
  todayData: HnsNewsState
  actions: NewTabActions
  getHnsNewsDisplayAd: GetDisplayAdContent
  saveShowBackgroundImage: (value: boolean) => void
  saveShowRewards: (value: boolean) => void
  saveShowHnsTalk: (value: boolean) => void
...
function GetBackgroundImageSrc (props: Props) {
  ...
  if (props.newTabData.backgroundWallpaper?.type === 'image' ||
      props.newTabData.backgroundWallpaper?.type === 'hns') {
...
class NewTabPage extends React.Component<Props, State> {
  state: State = {
    ...
    isPromptingHnsNews: false,
    showSearchPromotion: false,
    forceToHideWidget: false
  }

  imgCache: HTMLImageElement
  hnsNewsPromptTimerId: number
  hasInitHnsNews: boolean = false
  ...
  componentWillUnmount () {
    if (this.hnsNewsPromptTimerId) {
      window.clearTimeout(this.hnsNewsPromptTimerId)
    }
    window.removeEventListener('resize', this.handleResize.bind(this))
  }

  componentDidUpdate (prevProps: Props) {
    this.maybePeekHnsNews()
  ...
  maybePeekHnsNews () {
    const hasPromptedHnsNews = !!this.hnsNewsPromptTimerId
    const shouldPromptHnsNews =
      !hasPromptedHnsNews && // Don't start a prompt if we already did
      window.scrollY === 0 && // Don't start a prompt if we are scrolled
      this.props.newTabData.featureFlagHnsNewsPromptEnabled &&
    ...
    if (shouldPromptHnsNews) {
      this.hnsNewsPromptTimerId = window.setTimeout(() => {
        if (window.scrollY > 0) {
        ...
        this.setState({ isPromptingHnsNews: true })
      }, 1700)
    }
  }
  ...
  toggleShowHnsTalk = () => {
    this.props.saveShowHnsTalk(!this.props.newTabData.showHnsTalk)
  }
  ...
  learnMoreRewards = () => {
    window.open('https://hns.com/hns-rewards/', '_blank', 'noopener')
  }
  ...
  getCryptoContent () {
    ...
    const lookup = {
      'rewards': {
        display: hnsRewardsSupported && showRewards,
        render: this.renderRewardsWidget.bind(this)
      },
      'hnsTalk': {
        display: hnsTalkSupported && showHnsTalk,
        render: this.renderHnsTalkWidget.bind(this)
      }
    }
  ...
  allWidgetsHidden = () => {
    ...
    return hideAllWidgets || [
      hnsRewardsSupported && showRewards,
      hnsTalkSupported && showHnsTalk
  ...
  renderSearchPromotion () {
    ...
    const onDismiss = () => { getNTPBrowserAPI().pageHandler.dismissHnsSearchPromotion() }
    const onTryHnsSearch = (input: string, openNewTab: boolean) => { getNTPBrowserAPI().pageHandler.tryHnsSearchPromotion(input, openNewTab) }

    return (
      <SearchPromotion textDirection={this.props.newTabData.textDirection} onTryHnsSearch={onTryHnsSearch} onClose={onClose} onDismiss={onDismiss} />
    )
  }
  ...
  renderRewardsWidget (showContent: boolean, position: number) {
    const { rewardsState, showRewards, textDirection, hnsRewardsSupported } = this.props.newTabData
    if (!hnsRewardsSupported || !showRewards) {
      return null
    }
    ...
    const customMenuItems = [
      {
        ...
        onClick: () => { chrome.hnsRewards.openRewardsPanel() }
      }
    ]
    ...
    return (
      <Rewards
        {...rewardsState}
        widgetTitle={getLocale('rewardsWidgetHnsRewards')}
  ...
  renderHnsTalkWidget (showContent: boolean, position: number) {
    const { newTabData } = this.props
    const { showHnsTalk, textDirection, hnsTalkSupported } = newTabData

    if (!showHnsTalk || !hnsTalkSupported) {
      return null
    }
    ...
    return (
      <HnsTalk
        ...
        paddingType={'none'}
        menuPosition={'left'}
        widgetTitle={getLocale('hnsTalkWidgetTitle')}
        isForeground={showContent}
        stackPosition={position}
        textDirection={textDirection}
        hideWidget={this.toggleShowHnsTalk}
        showContent={showContent}
        onShowContent={this.setForegroundStackWidget.bind(this, 'hnsTalk')}
      />
    )
  }
  ...
  render () {
    ...
    const hasWallpaperInfo = newTabData.backgroundWallpaper?.type === 'hns'
    ...
    return (
      <Page.App
        ...
        data-show-news-prompt={((this.state.backgroundHasLoaded || colorForBackground) && this.state.isPromptingHnsNews) ? true : undefined}>
        <OverrideReadabilityColor override={ this.shouldOverrideReadabilityColor(this.props.newTabData) } />
        <HnsNewsContextProvider>
        <Page.Page
            hasImage={hasImage}
            imageSrc={this.imageSource}
            imageHasLoaded={this.state.backgroundHasLoaded}
          ...
          </GridWidget>
          {
            ...
            <Page.Footer>
              <Page.FooterContent>
                ...
                <FooterInfo
                  textDirection={newTabData.textDirection}
                  supportsHnsTalk={newTabData.hnsTalkSupported}
            ...
            {newTabData.showToday &&
              <Page.GridItemNavigationHnsNews>
                <HnsNewsHint />
              </Page.GridItemNavigationHnsNews>
            }
          </Page.Page>
        ...
        <HnsNews
          ...
          isPrompting={this.state.isPromptingHnsNews}
          ...
          onCustomizeHnsNews={() => { this.openSettings(SettingsTabType.HnsNews) }}
          ...
          getDisplayAd={this.props.getHnsNewsDisplayAd}
        />
        }
        <Settings
          actions={actions}
