...
export type HnsNewsState = {
  ...
  articleScrollTo?: HnsNews.FeedItemMetadata
...
const defaultState: HnsNewsState = {
  isFetching: true,
...
// if it gets removed and rendered again (e.g. if Hns News is toggled off and on).
// Reset to defaultState when Today is turned off or refreshed.

const reducer = createReducer<HnsNewsState>({}, defaultState)
...
reducer.on(Actions.removeDirectFeed, (state, { directFeed }) => {
  ...
  if (!hasMatch) {
    console.warn('Hns News: asked to remove direct feed which did not exist', directFeed)
    return state
  }
