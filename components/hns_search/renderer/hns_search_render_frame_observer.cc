...
HnsSearchRenderFrameObserver::HnsSearchRenderFrameObserver(
    content::RenderFrame* render_frame,
    int32_t world_id)
    : RenderFrameObserver(render_frame), world_id_(world_id) {}

HnsSearchRenderFrameObserver::~HnsSearchRenderFrameObserver() = default;

void HnsSearchRenderFrameObserver::DidCreateScriptContext(
  ...
  if (!native_javascript_handle_) {
    native_javascript_handle_ = std::make_unique<HnsSearchDefaultJSHandler>(
...
void HnsSearchRenderFrameObserver::DidStartNavigation(
    const GURL& url,
    absl::optional<blink::WebNavigationType> navigation_type) {
  url_ = url;
}

void HnsSearchRenderFrameObserver::OnDestruct() {
  delete this;
}
