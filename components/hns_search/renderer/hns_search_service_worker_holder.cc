...
// HnsSearchFallbackJSHandler instance in local thread storage, and providing
...
class [[maybe_unused, nodiscard]] JsHandlersForCurrentThread
 ...
 public:
  ...
  void AddJsHandler(std::unique_ptr<HnsSearchFallbackJSHandler> js_handler);
 ...
 private:
  ...
  std::vector<std::unique_ptr<HnsSearchFallbackJSHandler>> js_handlers_;
...
void JsHandlersForCurrentThread::AddJsHandler(
    std::unique_ptr<HnsSearchFallbackJSHandler> js_handler) {
...
void JsHandlersForCurrentThread::RemoveContext(
  ...
  js_handlers_.erase(
      base::ranges::remove_if(
          js_handlers_,
          [&v8_context](
              const std::unique_ptr<HnsSearchFallbackJSHandler>& js_handler) {
...
HnsSearchServiceWorkerHolder::HnsSearchServiceWorkerHolder()
    : broker_(nullptr) {}

HnsSearchServiceWorkerHolder::~HnsSearchServiceWorkerHolder() = default;

void HnsSearchServiceWorkerHolder::SetBrowserInterfaceBrokerProxy(
    blink::ThreadSafeBrowserInterfaceBrokerProxy* broker) {
  broker_ = broker;
}

void HnsSearchServiceWorkerHolder::WillEvaluateServiceWorkerOnWorkerThread(
