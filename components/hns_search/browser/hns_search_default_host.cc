...
#include "hns/components/time_period_storage/daily_storage.h"
...
using hns_search_conversion::IsNTPPromotionEnabled;
...
// static
void HnsSearchDefaultHost::RegisterProfilePrefs(
    PrefRegistrySimple* registry) {
  registry->RegisterListPref(hns_search::prefs::kDailyAsked);
  registry->RegisterIntegerPref(hns_search::prefs::kTotalAsked, 0);
#if BUILDFLAG(IS_ANDROID)
  registry->RegisterBooleanPref(hns_search::prefs::kFetchFromNative, false);
#endif
}

HnsSearchDefaultHost::HnsSearchDefaultHost(
    const std::string& host,
    TemplateURLService* template_url_service,
    PrefService* prefs)
    : host_(host), template_url_service_(template_url_service), prefs_(prefs) {}

HnsSearchDefaultHost::~HnsSearchDefaultHost() = default;

bool HnsSearchDefaultHost::CanSetDefaultSearchProvider(TemplateURL* provider,
                                                         bool is_historic) {
  // Return false if:
...
uint64_t HnsSearchDefaultHost::GetMaxDailyCanAskCount() {
  return static_cast<uint64_t>(
      features::kHnsSearchDefaultAPIDailyLimit.Get());
}

uint64_t HnsSearchDefaultHost::GetMaxTotalCanAskCount() {
  return static_cast<uint64_t>(
      features::kHnsSearchDefaultAPITotalLimit.Get());
}

void HnsSearchDefaultHost::SetCanAlwaysSetDefault() {
...
void HnsSearchDefaultHost::GetCanSetDefaultSearchProvider(
...
void HnsSearchDefaultHost::SetIsDefaultSearchProvider() {
