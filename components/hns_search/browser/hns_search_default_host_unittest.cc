...
class HnsSearchDefaultHostTest : public ::testing::Test {
 protected:
  using MockGetCanSetCallback = base::MockCallback<
      HnsSearchDefaultHost::GetCanSetDefaultSearchProviderCallback>;

  HnsSearchDefaultHostTest() : template_url_service_(nullptr, 0) {
  ...
  void PrepareFieldTrialParamsForBannerTypeC() {
    constexpr char kPromotionTrial[] = "HnsSearchPromotionBannerStudy";
  ...
  std::unique_ptr<HnsSearchDefaultHost> GetAPIHost(const std::string& host) {
    return std::make_unique<HnsSearchDefaultHost>(
...
TEST_F(HnsSearchDefaultHostTest, AllowsIfPresent) {
...
TEST_F(HnsSearchDefaultHostTest, DisallowsIfNotPresent) {
  auto host = GetAPIHost("search.test.com");
...
TEST_F(HnsSearchDefaultHostTest, DisallowsIfDefault) {
  auto host = GetAPIHost("search.test.com");
...
TEST_F(HnsSearchDefaultHostTest, AllowsIfNotDefault) {
  auto host = GetAPIHost("search.test.com");
  // Add a search provider for the host
  AddSearchProviderForHost("search.test.com");
...
TEST_F(HnsSearchDefaultHostTest, DisallowsAfterMaxTimesAsked) {
  auto host = GetAPIHost("search.test.com");
  // Add a search provider for the host
  AddSearchProviderForHost("search.test.com");
...
TEST_F(HnsSearchDefaultHostTest, CanSetDefaultAlwaysTestWithSearchPromotion) {
  base::test::ScopedFeatureList feature_list;

  hns_l10n::test::ScopedDefaultLocale scoped_default_locale{"en_US"};
  ...
  feature_list.InitAndEnableFeature(
      hns_search_conversion::features::kOmniboxButton);
  host->SetCanAlwaysSetDefault();
  ...
  feature_list.Reset();
  feature_list.InitAndEnableFeature(hns_search_conversion::features::kNTP);
  host->SetCanAlwaysSetDefault();
