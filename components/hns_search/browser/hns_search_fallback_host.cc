...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("hns_search_host", R"(
      semantics {
        sender: "Hns Search Host Controller"
        description:
          "This controller is used as a backup search "
          "provider for users that have opted into this feature."
        trigger:
          "Triggered by Hns search if a user has opted in."
        data:
...
void HnsSearchFallbackHost::SetBackupProviderForTest(
    const GURL& backup_provider) {
  backup_provider_for_test = backup_provider;
}

HnsSearchFallbackHost::HnsSearchFallbackHost(
    scoped_refptr<network::SharedURLLoaderFactory> factory)
    : shared_url_loader_factory_(std::move(factory)), weak_factory_(this) {}

HnsSearchFallbackHost::~HnsSearchFallbackHost() = default;

// [static]
GURL HnsSearchFallbackHost::GetBackupResultURL(const GURL& baseURL,
...
void HnsSearchFallbackHost::FetchBackupResults(
    const std::string& query,
    const std::string& lang,
    const std::string& country,
    const std::string& geo,
  ...
  iter->get()->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      ...
      base::BindOnce(&HnsSearchFallbackHost::OnURLLoaderComplete,
                     weak_factory_.GetWeakPtr(), iter, std::move(callback)));
}

void HnsSearchFallbackHost::OnURLLoaderComplete(
