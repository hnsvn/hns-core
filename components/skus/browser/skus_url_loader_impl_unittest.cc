...
#include "hns/components/skus/browser/skus_url_loader_impl.h"
...
#include "hns/components/skus/browser/rs/cxx/src/lib.rs.h"
...
TEST_F(SkusUrlLoaderImplUnitTest, SanitizedResponse) {
  SetResponseText("{}");
  EXPECT_TRUE(GetRequestResponse("GET", "https://hns.com").is_dict());
  SetResponseText("{,}");
  EXPECT_TRUE(GetRequestResponse("GET", "https://hns.com").is_none());
}
...
TEST_F(SkusUrlLoaderImplUnitTest, BeginFetch) {
  ...
  FetchResponse(
      ...
      "https://hns.com", skus::SkusResult::Ok, "{}");
  ...
  FetchResponse(
      "GET",
      rust::cxxbridge1::Box<skus::HttpRoundtripContext>::from_raw(nullptr),
      "https://hns.com", skus::SkusResult::Ok, std::string());
}
