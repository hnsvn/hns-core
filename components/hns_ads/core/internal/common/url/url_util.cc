...
GURL ReplaceUrlHnsHostWithChromeHost(const GURL& url) {
  CHECK(url.is_valid());

  if (!url.SchemeIs(kHnsScheme)) {
    return url;
  }
...
bool DoesSupportUrl(const GURL& url) {
  ...
  // We must replace the hns:// scheme with chrome:// due to GURL not parsing
  // hns:// schemes.
  const GURL modified_url = ReplaceUrlHnsHostWithChromeHost(url);
