...
class HnsAdsLinearTest : public UnitTestBase {};

TEST_F(HnsAdsLinearTest, ThreeClassesPredictionTest) {
  // Arrange
...
TEST_F(HnsAdsLinearTest, BiasesPredictionTest) {
  // Arrange
  const std::map<std::string, VectorData> weights = {
      {"class_1", VectorData({1.0, 0.0, 0.0})},
      {"class_2", VectorData({0.0, 1.0, 0.0})},
...
TEST_F(HnsAdsLinearTest, BinaryClassifierPredictionTest) {
  // Arrange
...
TEST_F(HnsAdsLinearTest, TopPredictionsTest) {
  // Arrange
