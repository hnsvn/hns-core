...
class HnsAdsConversionQueueTest : public ConversionQueueDelegate,
                                    public UnitTestBase {
 protected:
  void SetUp() override {
    UnitTestBase::SetUp();
...
TEST_F(HnsAdsConversionQueueTest, AddConversionToQueue) {
  // Arrange
...
TEST_F(HnsAdsConversionQueueTest, ProcessSingleConversionInQueue) {
  // Arrange
  const AdInfo ad = BuildAdForTesting(AdType::kNotificationAd,
                                      /*should_use_random_uuids*/ false);
...
TEST_F(HnsAdsConversionQueueTest, ProcessMultipleConversionsInQueue) {
  // Arrange
  AdvanceClockTo(
