 ...
 protected:
  ...
  void ProcessPurchaseIntent() {
    const std::vector<GURL> urls = {
        GURL("https://www.hns.com/test?foo=bar"),
...
TEST_P(HnsAdsTopSegmentsTest, GetSegments) {
  // Arrange
...
INSTANTIATE_TEST_SUITE_P(,
                         HnsAdsTopSegmentsTest,
                         ::testing::ValuesIn(kTests),
                         TestParamToString);

TEST_F(HnsAdsTopSegmentsTest, GetSegmentsForAllModelsIfPreviouslyProcessed) {
  // Arrange
  SetEpsilonGreedyBanditEligibleSegments(GetSegmentList());
...
TEST_F(HnsAdsTopSegmentsTest, GetSegmentsForFieldTrialParticipationPath) {
  // Arrange
  SetEpsilonGreedyBanditEligibleSegments(GetSegmentList());
