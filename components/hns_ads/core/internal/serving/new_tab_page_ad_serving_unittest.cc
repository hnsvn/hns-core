...
#include "hns/components/hns_ads/core/internal/serving/new_tab_page_ad_serving_feature_unittest_util.h"
...
class HnsAdsNewTabPageAdServingDelegate : public NewTabPageAdServingDelegate {
 public:
...
class HnsAdsNewTabPageAdServingTest : public UnitTestBase {
 protected:
  void SetUp() override {
    UnitTestBase::SetUp();
  ...
  HnsAdsNewTabPageAdServingDelegate serving_delegate_;
};

TEST_F(HnsAdsNewTabPageAdServingTest, DoNotServeAdForUnsupportedVersion) {
  // Arrange
  ForceNewTabPageAdServingVersionForTesting(0);

  // Act
  serving_->MaybeServeAd(base::BindOnce(
      [](HnsAdsNewTabPageAdServingDelegate* serving_delegate,
...
TEST_F(HnsAdsNewTabPageAdServingTest, ServeAd) {
  // Arrange
...
TEST_F(HnsAdsNewTabPageAdServingTest, DoNotServeAdIfMissingWallpapers) {
  // Arrange
  ForcePermissionRulesForTesting();
...
TEST_F(HnsAdsNewTabPageAdServingTest, DoNotServeAdIfNoEligibleAdsFound) {
  // Arrange
  ForcePermissionRulesForTesting();
...
TEST_F(HnsAdsNewTabPageAdServingTest,
