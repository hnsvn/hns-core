...
#include "hns/components/hns_ads/browser/ads_service_impl.h"
...
#include "hns/browser/hns_ads/notification_helper/notification_helper.h"
#include "hns/browser/hns_ads/notifications/notification_ad_platform_bridge.h"
...
#include "hns/components/hns_ads/browser/device_id.h"
...
#include "hns/components/hns_ads/core/public/ads_constants.h"
...
#include "hns/components/hns_ads/resources/grit/bat_ads_resources.h"
#include "hns/components/hns_federated/data_stores/async_data_store.h"
...
#if BUILDFLAG(IS_ANDROID)
#include "hns/browser/notifications/hns_notification_platform_bridge_helper_android.h"
...
constexpr char kNotificationAdUrlPrefix[] = "https://www.hns.com/ads/?";
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("ads_service_impl", R"(
      semantics {
        sender: "Hns Ads Service"
        description:
          "This service is used to communicate with Hns servers "
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable this feature by visiting hns://rewards."
...
void RegisterResourceComponentsForDefaultCountryCode() {
  const std::string& locale = hns_l10n::GetDefaultLocaleString();
  const std::string country_code = hns_l10n::GetISOCountryCode(locale);
  g_hns_browser_process->resource_component()
      ->RegisterComponentForCountryCode(country_code);
}

void RegisterResourceComponentsForDefaultLanguageCode() {
  const std::string& locale = hns_l10n::GetDefaultLocaleString();
  const std::string language_code = hns_l10n::GetISOLanguageCode(locale);
  g_hns_browser_process->resource_component()
...
AdsServiceImpl::AdsServiceImpl(
    Profile* profile,
    hns_adaptive_captcha::HnsAdaptiveCaptchaService*
    ...
    hns_federated::AsyncDataStore* notification_ad_timing_data_store)
    : profile_(profile),
  ...
  CHECK(hns::IsRegularProfile(profile));
  ...
  g_hns_browser_process->resource_component()->AddObserver(this);

  rewards_service_->AddObserver(this);
}

AdsServiceImpl::~AdsServiceImpl() {
  g_hns_browser_process->resource_component()->RemoveObserver(this);
...
bool AdsServiceImpl::UserHasOptedInToHnsRewards() const {
  return profile_->GetPrefs()->GetBoolean(hns_rewards::prefs::kEnabled);
}

bool AdsServiceImpl::UserHasOptedInToHnsNewsAds() const {
...
bool AdsServiceImpl::CanStartBatAdsService() const {
  return ShouldAlwaysRunService() || UserHasOptedInToHnsNewsAds() ||
         (UserHasOptedInToHnsRewards() &&
...
void AdsServiceImpl::InitializeRewardsWalletCallback(
    const size_t current_start_number,
    hns_rewards::mojom::RewardsWalletPtr wallet) {
  if (!ShouldProceedInitialization(current_start_number)) {
    return;
  }
  ...
  if (!wallet && UserHasOptedInToNotificationAds()) {
    VLOG(1) << "Failed to initialize Hns Rewards wallet";
    return Shutdown();
  }
...
void AdsServiceImpl::InitializeBatAds(
    hns_rewards::mojom::RewardsWalletPtr rewards_wallet) {
  if (!bat_ads_.is_bound()) {
    return;
  }
...
void AdsServiceImpl::ShutdownAndResetState() {
  ...
  profile_->GetPrefs()->ClearPrefsWithPrefixSilently("hns.hns_ads");
...
void AdsServiceImpl::SetBuildChannel() {
  ...
  build_channel->name = hns::GetChannelName();
...
void AdsServiceImpl::SetFlags() {
  ...
  if (profile_->GetPrefs()->GetBoolean(
          hns_rewards::prefs::kUseRewardsStagingServer)) {
...
void AdsServiceImpl::InitializePrefChangeRegistrar() {
  ...
  InitializeHnsRewardsPrefChangeRegistrar();
  InitializeSubdivisionTargetingPrefChangeRegistrar();
  InitializeHnsNewsAdsPrefChangeRegistrar();
  InitializeNewTabPageAdsPrefChangeRegistrar();
  InitializeNotificationAdsPrefChangeRegistrar();
}

void AdsServiceImpl::InitializeHnsRewardsPrefChangeRegistrar() {
...
void AdsServiceImpl::InitializeHnsNewsAdsPrefChangeRegistrar() {
  pref_change_registrar_.Add(
      hns_news::prefs::kHnsNewsOptedIn,
...
void AdsServiceImpl::CloseAllNotificationAds() {
  // TODO(https://github.com/hnsvn/hns-browser/issues/25410): Temporary
...
void AdsServiceImpl::ShowNotificationAd(base::Value::Dict dict) {
  ...
  if (ShouldShowCustomNotificationAds()) {
    ...
    display_service_->Display(NotificationHandler::Type::HNS_ADS,
...
#if BUILDFLAG(IS_ANDROID)
    const std::string hns_ads_url_prefix = kNotificationAdUrlPrefix;
    const GURL url =
        GURL(hns_ads_url_prefix.substr(0, hns_ads_url_prefix.size() - 1));
    HnsNotificationPlatformBridgeHelperAndroid::MaybeRegenerateNotification(
        placement_id, url);
#endif
    display_service_->Close(NotificationHandler::Type::HNS_ADS, placement_id);
  }
}
...
void AdsServiceImpl::ShowReminder(const mojom::ReminderType type) {
  ...
  if (UserHasOptedInToNotificationAds() && CheckIfCanShowNotificationAds()) {
    // TODO(https://github.com/hnsvn/hns-browser/issues/29587): Decouple Hns
...
void AdsServiceImpl::LoadFileResource(const std::string& id,
  ...
  absl::optional<base::FilePath> file_path =
      g_hns_browser_process->resource_component()->GetPath(id, version);
  if (!file_path) {
...
void AdsServiceImpl::ShowScheduledCaptchaNotification(
  ...
  if (profile_->GetPrefs()->GetBoolean(
          hns_adaptive_captcha::prefs::kScheduledCaptchaPaused)) {
    return VLOG(1) << "Ads paused; support intervention required";
  }

  const int snooze_count = profile_->GetPrefs()->GetInteger(
      hns_adaptive_captcha::prefs::kScheduledCaptchaSnoozeCount);
