...
class HnsBookmarkProviderTest : public testing::Test {
 public:
  HnsBookmarkProviderTest()
  ...
  void SetUp() override {
    ...
    provider_ = base::MakeRefCounted<HnsBookmarkProvider>(&client_);
  }
 ...
 protected:
  ...
  scoped_refptr<HnsBookmarkProvider> provider_;
};

TEST_F(HnsBookmarkProviderTest, SuggestionsDisabledNoResults) {
  prefs()->SetBoolean(omnibox::kBookmarkSuggestionsEnabled, false);
  provider_->Start(CreateAutocompleteInput("Hello"), true);
  EXPECT_TRUE(provider_->matches().empty());
}

TEST_F(HnsBookmarkProviderTest, SuggestionsEnabledHasResults) {
