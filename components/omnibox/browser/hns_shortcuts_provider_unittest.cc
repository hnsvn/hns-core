...
class HnsShortcutsProviderTest : public testing::Test {
 public:
  HnsShortcutsProviderTest() = default;
  ...
  void SetUp() override {
    ...
    provider_ = base::MakeRefCounted<HnsShortcutsProvider>(&client_);
  }
 ...
 protected:
  ...
  HnsFakeAutocompleteProviderClient client_;
  scoped_refptr<HnsShortcutsProvider> provider_;
  scoped_refptr<ShortcutsBackend> backend_;
};

TEST_F(HnsShortcutsProviderTest, SuggestionsDisabledNoResults) {
  prefs()->SetBoolean(omnibox::kHistorySuggestionsEnabled, false);
  provider_->Start(CreateAutocompleteInput("hel"), true);
  EXPECT_TRUE(provider_->matches().empty());
}

TEST_F(HnsShortcutsProviderTest, SuggestionsEnabledHasResults) {
