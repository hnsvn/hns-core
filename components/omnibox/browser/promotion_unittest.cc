...
using hns_search_conversion::RegisterPrefs;
using ::testing::Eq;
 ...
 public:
  ...
  void SetUp() override {
    ...
    scoped_default_locale_ =
        std::make_unique<hns_l10n::test::ScopedDefaultLocale>("en_US");
  ...
  void CreateController(bool incognito) {
    // Set non hns search provider to get promotion match.
...
TEST_F(OmniboxPromotionTest, ProfileTest) {
  ...
  AutocompleteInput input(u"hns", metrics::OmniboxEventProto::OTHER,
                          classifier_);
...
TEST_F(OmniboxPromotionTest, PromotionEntrySortTest) {
  ...
  auto hns_search_conversion_match =
      base::ranges::find_if(controller_->result(), IsHnsSearchPromotionMatch);
  EXPECT_NE(hns_search_conversion_match, controller_->result().end());

  // Located as second entry for button type.
  EXPECT_EQ(1, std::distance(controller_->result().begin(),
                             hns_search_conversion_match));
...
TEST_F(OmniboxPromotionTest, AutocompleteResultTest) {
  ...
  SortHnsSearchPromotionMatch(&result);
  EXPECT_TRUE(IsHnsSearchPromotionMatch(*result.match_at(1)));
  ...
  result.AppendMatches(matches);
  SortHnsSearchPromotionMatch(&result);
  EXPECT_TRUE(IsHnsSearchPromotionMatch(*result.match_at(3)));
  ...
  for (const auto& match : result) {
    EXPECT_FALSE(IsHnsSearchPromotionMatch(match));
  }
}
