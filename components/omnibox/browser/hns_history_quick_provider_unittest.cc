...
class HnsHistoryQuickProviderTest : public testing::Test {
 public:
  HnsHistoryQuickProviderTest() = default;
  HnsHistoryQuickProviderTest(const HnsHistoryQuickProviderTest&) = delete;
  HnsHistoryQuickProviderTest& operator=(
      const HnsHistoryQuickProviderTest&) = delete;
 ...
 protected:
  ...
  void SetUp() override {
    ...
    provider_ = base::MakeRefCounted<HnsHistoryQuickProvider>(client_.get());
  }
 ...
 private:
  ...
  scoped_refptr<HnsHistoryQuickProvider> provider_;
};

HnsHistoryQuickProviderTest::SetShouldContain::SetShouldContain(
    const ACMatches& matched_urls) {
  for (const auto& iter : matched_urls)
    matches_.insert(iter.destination_url.spec());
}

void HnsHistoryQuickProviderTest::SetShouldContain::operator()(
    const std::string& expected) {
  EXPECT_EQ(1U, matches_.erase(expected))
      << "Results did not contain '" << expected << "' but should have.";
}

TEST_F(HnsHistoryQuickProviderTest, HasResultsWhenHistoryEnabled) {
...
TEST_F(HnsHistoryQuickProviderTest, HasNoResultsWhenHistoryDisabled) {
