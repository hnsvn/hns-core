...
#include "hns/components/hns_wayback_machine/hns_wayback_machine_utils.h"
...
HnsWaybackMachineTabHelper::HnsWaybackMachineTabHelper(
    content::WebContents* contents)
    : WebContentsObserver(contents),
      content::WebContentsUserData<HnsWaybackMachineTabHelper>(*contents),
      pref_service_(*user_prefs::UserPrefs::Get(contents->GetBrowserContext())),
      weak_factory_(this) {}

HnsWaybackMachineTabHelper::~HnsWaybackMachineTabHelper() = default;

void HnsWaybackMachineTabHelper::set_delegate(
    std::unique_ptr<HnsWaybackMachineDelegate> delegate) {
  DCHECK(delegate);
  delegate_ = std::move(delegate);
}

void HnsWaybackMachineTabHelper::DidFinishNavigation(
  ...
  if (const net::HttpResponseHeaders* header =
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&HnsWaybackMachineTabHelper::CreateInfoBar,
                                  weak_factory_.GetWeakPtr()));
  }
}

void HnsWaybackMachineTabHelper::CreateInfoBar() {
  DCHECK(delegate_);
  delegate_->CreateInfoBar(web_contents());
}

bool HnsWaybackMachineTabHelper::IsWaybackMachineEnabled() const {
  return pref_service_->GetBoolean(kHnsWaybackMachineEnabled);
}

bool HnsWaybackMachineTabHelper::ShouldAttachWaybackMachineInfoBar(
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(HnsWaybackMachineTabHelper);
