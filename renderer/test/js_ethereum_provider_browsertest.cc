      ...
      try {
        let promise = new Promise((resolve) => {
          const listener = (event) => {
            ...
            let is_hns_wallet = event.detail.info.name === "Hns Wallet" &&
                                  event.detail.provider.isHnsWallet === true;
            resolve(is_hns_wallet);
          }
...
std::string NonWriteableScriptProperty(const std::string& property) {
  return base::StringPrintf(
      R"(window.ethereum.%s = "hns";
         !(window.ethereum.%s === "hns");)",
...
// TODO(darkdh): Move this browser test to //hns/browser/hns_wallet/ because
// it has layer violation (//chrome/browser,
// //hns/components/hns_wallet/browser and //hns/browser)
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, AttachOnReload) {
  ...
  histogram_tester_->ExpectUniqueSample("Hns.Wallet.EthProvider.4", 0, 1);
  ...
  histogram_tester_->ExpectBucketCount("Hns.Wallet.EthProvider.4", 0, 2);
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  {
    std::string command = "window.hnsEthereum.isMetaMask";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), command,
                                content::EXECUTE_SCRIPT_DEFAULT_OPTIONS,
                                ISOLATED_WORLD_ID_TRANSLATE)
                    .error.find("Cannot read properties of undefined") !=
  ...
  histogram_tester_->ExpectTotalCount("Hns.Wallet.EthProvider.3", 0);
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  {
    std::string command = "window.ethereum.isHnsWallet";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), command)
                    .error.find("Cannot read properties of undefined") !=
                std::string::npos);
  }
  ...
  {
    std::string command = "window.hnsEthereum.isHnsWallet";
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, AttachIfWalletCreated) {
  ...
  {
    constexpr char kEvalIsHnsWallet[] = "window.ethereum.isHnsWallet";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), kEvalIsHnsWallet)
                    .ExtractBool());
  }

  {
    constexpr char kEvalIsHnsWallet[] = "window.hnsEthereum.isHnsWallet";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), kEvalIsHnsWallet)
                    .ExtractBool());
  }
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  constexpr char kEvalIsHnsWallet[] = "window.hnsEthereum.isHnsWallet";
  EXPECT_TRUE(
      content::EvalJs(primary_main_frame(), kEvalIsHnsWallet).ExtractBool());
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
  ...
  // Check whether window.hnsEthereum is installed
  {
    std::string command = "window.hnsEthereum.isHnsWallet";
    EXPECT_EQ(base::Value(true),
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, NonWritable) {
  ...
  for (const std::string& property : {"isHnsWallet", "_metamask", "chainId",
  ...
  // https://github.com/hnsvn/hns-browser/issues/25078
  for (const std::string& method :
...

    {
      auto result = EvalJs(web_contents(),
                           NonWriteableScriptMethod("hnsEthereum", method));
      EXPECT_EQ(base::Value(true), result.value) << result.error;
    }
  }
  {
  ...
  {
    auto result = EvalJs(web_contents(),
                         NonWriteableScriptMethod("hnsEthereum", "send"));
    EXPECT_EQ(base::Value(false), result.value) << result.error;
  }
...
// See https://github.com/hnsvn/hns-browser/issues/22213 for details
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest,
                       HnsEthereum_NonConfigurable) {
  hns_wallet::mojom::DefaultWallet non_configurable_states[] = {
      hns_wallet::mojom::DefaultWallet::HnsWallet,
      hns_wallet::mojom::DefaultWallet::HnsWalletPreferExtension};
  for (const auto& default_wallet : non_configurable_states) {
    hns_wallet::SetDefaultEthereumWallet(browser()->profile()->GetPrefs(),
                                           default_wallet);
    const GURL url = https_server_.GetURL("/simple.html");
    ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
    std::string overwrite =
        R"(try {
           Object.defineProperty(window, 'hnsEthereum', {
             writable: true,
           });
         } catch (e) {}
         window.hnsEthereum = 42;
         typeof window.hnsEthereum === 'object'
      )";
    EXPECT_TRUE(content::EvalJs(primary_main_frame(), overwrite).ExtractBool());
  }
}
...
IN_PROC_BROWSER_TEST_F(JSEthereumProviderBrowserTest, OnlyWriteOwnProperty) {
  ...
  GetJsonRpcService()->SetNetwork("0x5", hns_wallet::mojom::CoinType::ETH,
                                  absl::nullopt);
  ...
  GetJsonRpcService()->SetNetwork("0x4", hns_wallet::mojom::CoinType::ETH,
                                  absl::nullopt);
  // Needed so ChainChangedEvent observers run
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(content::EvalJs(primary_main_frame(), get_chain_id).ExtractString(),
