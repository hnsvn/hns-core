...
void CheckUserAgentMetadataVersionsList(
  ...
  // Expect 3 items in the list: Hns, Chromium, and greased.
  EXPECT_EQ(3UL, versions_list.size());

  bool has_hns_brand = false;
  ...
  EXPECT_TRUE(has_hns_brand);
  EXPECT_TRUE(has_chromium_brand);
}

}  // namespace

class HnsNavigatorUserAgentFarblingBrowserTest : public InProcessBrowserTest {
 public:
  void SetUpOnMainThread() override {
    ...
    https_server_->RegisterRequestMonitor(base::BindRepeating(
        &HnsNavigatorUserAgentFarblingBrowserTest::MonitorHTTPRequest,
...
IN_PROC_BROWSER_TEST_F(HnsNavigatorUserAgentFarblingBrowserTest,
...
IN_PROC_BROWSER_TEST_F(HnsNavigatorUserAgentFarblingBrowserTest,
  ...
  // Expected major version for Hns and Chromium.
  ...
  // Expected version string for Hns and Chromium.
