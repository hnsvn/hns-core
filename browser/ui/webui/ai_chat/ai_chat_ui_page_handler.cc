...
void AIChatUIPageHandler::SetAutoGenerateQuestions(bool value) {
  profile_->GetPrefs()->SetBoolean(
      ai_chat::prefs::kHnsChatAutoGenerateQuestions, value);
}
...
void AIChatUIPageHandler::OpenHnsLeoSettings() {
  if (active_chat_tab_helper_) {
    active_chat_tab_helper_->web_contents()->OpenURL(
        {GURL("hns://settings/leo-assistant"), content::Referrer(),
         WindowOpenDisposition::NEW_FOREGROUND_TAB, ui::PAGE_TRANSITION_LINK,
         false});
  }
}

void AIChatUIPageHandler::OpenHnsLeoWiki() {
  if (active_chat_tab_helper_) {
    active_chat_tab_helper_->web_contents()->OpenURL(
        {GURL("https://github.com/hnsvn/hns-browser/wiki/Hns-Leo"),
