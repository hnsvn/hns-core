...
#include "hns/components/hns_adblock/adblock_internals/resources/grit/hns_adblock_internals_generated_map.h"
...
// Class acting as a controller of the hns://adblock-internals WebUI.
class HnsAdblockInternalsMessageHandler
    : public content::WebUIMessageHandler {
 public:
  HnsAdblockInternalsMessageHandler() = default;
  ~HnsAdblockInternalsMessageHandler() override = default;

 private:
  // WebUIMessageHandler implementation.
  void RegisterMessages() override {
    web_ui()->RegisterMessageCallback(
        "hns_adblock_internals.getDebugInfo",
        base::BindRepeating(&HnsAdblockInternalsMessageHandler::GetDebugInfo,
                            base::Unretained(this)));
    web_ui()->RegisterMessageCallback(
        "hns_adblock_internals.discardRegex",
        base::BindRepeating(&HnsAdblockInternalsMessageHandler::DiscardRegex,
                            base::Unretained(this)));
  }
  ...
  void GetDebugInfo(const base::Value::List& args) {
    ...
    instrumentation->RequestGlobalDumpForPid(
        ...
        base::BindOnce(&HnsAdblockInternalsMessageHandler::OnGetMemoryDump,
  ...
  void OnGetMemoryDump(
    ...
    g_hns_browser_process->ad_block_service()->GetDebugInfoAsync(
        base::BindOnce(&HnsAdblockInternalsMessageHandler::OnGetDebugInfo,
                       weak_ptr_factory_.GetWeakPtr(), callback_id,
                       std::move(mem_info)));
  }
  ...
  void DiscardRegex(const base::Value::List& args) {
    ...
    if (!base::StringToUint64(args[0].GetString(), &regex_id))
      return;
    g_hns_browser_process->ad_block_service()->DiscardRegex(regex_id);
  }
  ...
  base::WeakPtrFactory<HnsAdblockInternalsMessageHandler> weak_ptr_factory_{
      this};
};

}  // namespace

HnsAdblockInternalsUI::HnsAdblockInternalsUI(content::WebUI* web_ui,
                                                 const std::string& name)
    : content::WebUIController(web_ui) {
  CreateAndAddWebUIDataSource(web_ui, name, kHnsAdblockInternalsGenerated,
                              kHnsAdblockInternalsGeneratedSize,
                              IDR_HNS_ADBLOCK_INTERNALS_HTML);

  web_ui->AddMessageHandler(
      std::make_unique<HnsAdblockInternalsMessageHandler>());
}

HnsAdblockInternalsUI::~HnsAdblockInternalsUI() = default;
