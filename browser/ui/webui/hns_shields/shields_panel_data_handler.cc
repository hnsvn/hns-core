...
#include "hns/browser/ui/webui/hns_shields/shields_panel_data_handler.h"
...
using hns_shields::mojom::SiteSettings;
...
ShieldsPanelDataHandler::ShieldsPanelDataHandler(
  ...
  active_shields_data_controller_ =
      HnsShieldsDataController::FromWebContents(web_contents);
...
void ShieldsPanelDataHandler::RegisterUIHandler(
    mojo::PendingRemote<hns_shields::mojom::UIHandler> ui_handler_receiver) {
...
void ShieldsPanelDataHandler::SetHnsShieldsEnabled(bool is_enabled) {
  if (!active_shields_data_controller_)
    return;

  active_shields_data_controller_->SetHnsShieldsEnabled(is_enabled);
}
...
void ShieldsPanelDataHandler::UpdateSiteBlockInfo() {
  ...
  site_block_info_.is_hns_shields_enabled =
      active_shields_data_controller_->GetHnsShieldsEnabled();
  site_block_info_.is_hns_shields_managed =
      active_shields_data_controller_->IsHnsShieldsManaged();
...
void ShieldsPanelDataHandler::OnTabStripModelChanged(
  ...
  if (selection.active_tab_changed()) {
    ...
    if (selection.new_contents) {
      active_shields_data_controller_ =
          HnsShieldsDataController::FromWebContents(selection.new_contents);
      active_shields_data_controller_->AddObserver(this);
