...
GURL WebcompatReporterDialogDelegate::GetDialogContentURL() const {
  return GURL(kHnsUIWebcompatReporterURL);
}
...
void OpenReporterDialog(content::WebContents* initiator) {
  ...
  hns_shields::mojom::FingerprintMode fp_block_mode =
      hns_shields::mojom::FingerprintMode::STANDARD;
  hns_shields::mojom::AdBlockMode ad_block_mode =
      hns_shields::mojom::AdBlockMode::STANDARD;
  hns_shields::HnsShieldsDataController* shields_data_controller =
      hns_shields::HnsShieldsDataController::FromWebContents(initiator);
  if (shields_data_controller != nullptr) {
    shields_enabled = shields_data_controller->GetHnsShieldsEnabled();
