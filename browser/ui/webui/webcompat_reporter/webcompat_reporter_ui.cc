...
#include "hns/components/webcompat_reporter/browser/webcompat_report_uploader.h"
#include "hns/components/webcompat_reporter/resources/grit/webcompat_reporter_generated_map.h"
...
void WebcompatReporterDOMHandler::InitAdditionalParameters(Profile* profile) {
  ...
  if (ad_block_service != nullptr) {
    hns_shields::AdBlockRegionalServiceManager* regional_service_manager =
        ad_block_service->regional_service_manager();
    CHECK(regional_service_manager);
    for (const hns_shields::FilterListCatalogEntry& entry :
  ...
  hns_vpn::HnsVpnService* vpn_service =
      hns_vpn::HnsVpnServiceFactory::GetForProfile(profile);
  if (vpn_service != nullptr) {
    hns_vpn_connected_ = vpn_service->IsConnected();
  }
#endif
  ...
  language_farbling_enabled_ =
      profile_prefs->GetBoolean(hns_shields::prefs::kReduceLanguageEnabled);
}
...
void WebcompatReporterDOMHandler::HandleSubmitReport(
  ...
  uploader_->SubmitReport(GURL(url), shields_enabled, ad_block_setting,
                          ...
                          language_farbling_enabled_, hns_vpn_connected_,
                          details, contact);
}
