...
HnsImportBulkDataHandler::HnsImportBulkDataHandler() = default;
HnsImportBulkDataHandler::~HnsImportBulkDataHandler() = default;

void HnsImportBulkDataHandler::RegisterMessages() {
  HnsImportDataHandler::RegisterMessages();
  web_ui()->RegisterMessageCallback(
      "importDataBulk",
      base::BindRepeating(&HnsImportBulkDataHandler::HandleImportDataBulk,
                          base::Unretained(this)));
}

void HnsImportBulkDataHandler::PrepareProfile(
...
void HnsImportBulkDataHandler::HandleImportDataBulk(
  ...
  for (const auto& it : list) {
    ...
    HnsImportDataHandler::HandleImportData(single_profile_args);
  }
}

absl::optional<int> HnsImportBulkDataHandler::GetProfileIndex(
...
void HnsImportBulkDataHandler::StartImport(
    const importer::SourceProfile& source_profile,
  ...
  if (!GetProfileIndex(source_profile).has_value()) {
    HnsImportDataHandler::StartImport(source_profile, imported_items);
    return;
  }
  ...
  auto import_callback = base::BindOnce(
      &HnsImportBulkDataHandler::StartImportImpl, weak_factory_.GetWeakPtr(),
      source_profile, imported_items);
#if BUILDFLAG(IS_MAC)
  CheckDiskAccess(imported_items, source_profile.source_path,
                  source_profile.importer_type,
                  base::BindOnce(&HnsImportBulkDataHandler::PrepareProfile,
...
void HnsImportBulkDataHandler::NotifyImportProgress(
    const importer::SourceProfile& source_profile,
    const base::Value::Dict& info) {
  FireWebUIListener("hns-import-data-status-changed", info);
}

void HnsImportBulkDataHandler::OnImportEnded(
    const importer::SourceProfile& source_profile) {
  ...
  HnsImportDataHandler::OnImportEnded(source_profile);
}
