...
class HnsImporterObserverUnitTest : public testing::Test {
 public:
  HnsImporterObserverUnitTest() {}
 ...
 private:
  ...
  raw_ptr<HnsExternalProcessImporterHost> _ = nullptr;
};

TEST_F(HnsImporterObserverUnitTest, ImportEvents) {
  ...
  std::unique_ptr<HnsImporterObserver> observer =
      std::make_unique<HnsImporterObserver>(
          importer_host, source_profile, imported_items,
          base::BindRepeating(
              &HnsImporterObserverUnitTest::NotifyImportProgress,
...
TEST_F(HnsImporterObserverUnitTest, DestroyObserverEarly) {
  auto* importer_host = new HnsExternalProcessImporterHost();
  importer::SourceProfile source_profile;
  source_profile.importer_name = u"importer_name";
  source_profile.importer_type = importer::TYPE_CHROME;
