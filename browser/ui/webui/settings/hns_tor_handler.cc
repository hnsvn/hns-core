...
constexpr net::NetworkTrafficAnnotationTag kTorBridgesMoatAnnotation =
    net::DefineNetworkTrafficAnnotation("hns_tor_bridges", R"(
    semantics {
      sender:
        "Hns Tor Handler"
      description:
...
HnsTorHandler::HnsTorHandler() = default;
HnsTorHandler::~HnsTorHandler() = default;

void HnsTorHandler::RegisterMessages() {
  web_ui()->RegisterMessageCallback(
      "hns_tor.getBridgesConfig",
      base::BindRepeating(&HnsTorHandler::GetBridgesConfig,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "hns_tor.setBridgesConfig",
      base::BindRepeating(&HnsTorHandler::SetBridgesConfig,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "hns_tor.requestBridgesCaptcha",
      base::BindRepeating(&HnsTorHandler::RequestBridgesCaptcha,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "hns_tor.resolveBridgesCaptcha",
      base::BindRepeating(&HnsTorHandler::ResolveBridgesCaptcha,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "hns_tor.setTorEnabled",
      base::BindRepeating(&HnsTorHandler::SetTorEnabled,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "hns_tor.isTorEnabled",
      base::BindRepeating(&HnsTorHandler::IsTorEnabled,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "hns_tor.isTorManaged",
      base::BindRepeating(&HnsTorHandler::IsTorManaged,
                          base::Unretained(this)));

  local_state_change_registrar_.Init(g_browser_process->local_state());
  local_state_change_registrar_.Add(
      tor::prefs::kTorDisabled,
      base::BindRepeating(&HnsTorHandler::OnTorEnabledChanged,
                          base::Unretained(this)));
}

void HnsTorHandler::GetBridgesConfig(const base::Value::List& args) {
  AllowJavascript();
...
void HnsTorHandler::SetBridgesConfig(const base::Value::List& args) {
  CHECK_EQ(1u, args.size());
...
void HnsTorHandler::RequestBridgesCaptcha(const base::Value::List& args) {
  CHECK_EQ(1u, args.size());

  auto captcha_callback = base::BindOnce(
      &HnsTorHandler::SendResultToJavascript, base::Unretained(this),
...
void HnsTorHandler::ResolveBridgesCaptcha(const base::Value::List& args) {
...
void HnsTorHandler::SendResultToJavascript(bool reset_request,
...
void HnsTorHandler::SetTorEnabled(const base::Value::List& args) {
...
void HnsTorHandler::IsTorEnabled(const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
  AllowJavascript();
...
void HnsTorHandler::OnTorEnabledChanged() {
...
void HnsTorHandler::IsTorManaged(const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
