...
#include "hns/browser/ui/webui/settings/hns_full_disk_access_confirm_dialog_delegate.h"
...
HnsImportDataHandler::HnsImportDataHandler() = default;
HnsImportDataHandler::~HnsImportDataHandler() = default;

void HnsImportDataHandler::StartImport(
  ...
  CheckDiskAccess(imported_items, source_profile.source_path,
                  ...
                  base::BindOnce(&HnsImportDataHandler::StartImportImpl,
...
void HnsImportDataHandler::StartImportImpl(
    const importer::SourceProfile& source_profile,
    uint16_t imported_items,
    Profile* profile) {
  ...
  auto* importer_host = new HnsExternalProcessImporterHost();
  import_observers_[source_profile.source_path] =
      std::make_unique<HnsImporterObserver>(
          importer_host, source_profile, imported_items,
          base::BindRepeating(&HnsImportDataHandler::NotifyImportProgress,
...
void HnsImportDataHandler::NotifyImportProgress(
    const importer::SourceProfile& source_profile,
  ...
  if (*event == "ImportItemEnded") {
    ...
    content::GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&HnsImportDataHandler::OnImportEnded,
                                  weak_factory_.GetWeakPtr(), source_profile));
  }
}

void HnsImportDataHandler::HandleImportData(const base::Value::List& args) {
  ImportDataHandler::HandleImportData(args);
}

void HnsImportDataHandler::OnImportEnded(
    const importer::SourceProfile& source_profile) {
  import_observers_.erase(source_profile.source_path);
...
const importer::SourceProfile& HnsImportDataHandler::GetSourceProfileAt(
    int browser_index) {
  return importer_list_->GetSourceProfileAt(browser_index);
}

#if BUILDFLAG(IS_MAC)
void HnsImportDataHandler::CheckDiskAccess(
    uint16_t imported_items,
  ...
  if (importer_type == importer::TYPE_SAFARI) {
    // Start import if Hns has full disk access permission.
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&HnsImportDataHandler::OnGetDiskAccessPermission,
...
void HnsImportDataHandler::OnGetDiskAccessPermission(
    ContinueImportCallback callback,
    base::FilePath source_path,
    bool allowed) {
  if (!allowed) {
...
void HnsImportDataHandler::DidStopLoading() {
  Observe(nullptr);
