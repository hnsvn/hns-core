...
HnsPrivateNewTabPageHandler::HnsPrivateNewTabPageHandler(
    Profile* profile,
...
void HnsPrivateNewTabPageHandler::SetClientPage(
    mojo::PendingRemote<hns_private_new_tab::mojom::PrivateTabPage> page) {
  page_.Bind(std::move(page));
}

void HnsPrivateNewTabPageHandler::SetDisclaimerDismissed(bool dismissed) {
  DCHECK(profile_);

  profile_->GetOriginalProfile()->GetPrefs()->SetBoolean(
      profile_->IsTor()
          ? hns_private_new_tab::prefs::kHnsTorWindowDisclaimerDismissed
          : hns_private_new_tab::prefs::
                kHnsPrivateWindowDisclaimerDismissed,
      dismissed);
}

void HnsPrivateNewTabPageHandler::GetDisclaimerDismissed(
...
void HnsPrivateNewTabPageHandler::GetIsTorConnected(
...
using ConnectionStatus = hns_private_new_tab::mojom::ConnectionStatus;

void HnsPrivateNewTabPageHandler::GoToHnsSearch(const std::string& input,
                                                    bool open_new_tab) {
  DCHECK(profile_);

  auto provider_data = TemplateURLDataFromPrepopulatedEngine(
      profile_->IsTor() ? TemplateURLPrepopulateData::hns_search_tor
                        : TemplateURLPrepopulateData::hns_search);
...
void HnsPrivateNewTabPageHandler::GoToHnsSupport() {
  ...
  web_contents->OpenURL(content::OpenURLParams(
      GURL("https://support.hns.com/"), content::Referrer(),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui::PageTransition::PAGE_TRANSITION_LINK, false));
}

void HnsPrivateNewTabPageHandler::OnTorCircuitEstablished(bool result) {
  stuck_timer_.Stop();
  if (page_) {
...
void HnsPrivateNewTabPageHandler::OnTorInitializing(
    const std::string& percentage,
    const std::string& message) {
  stuck_timer_.Start(
      FROM_HERE, kStuckPeriod,
      base::BindOnce(&HnsPrivateNewTabPageHandler::OnTorCircuitTimer,
...
void HnsPrivateNewTabPageHandler::OnTorCircuitTimer(ConnectionStatus status) {
  if (!page_)
    return;
