...
#include "hns/components/skus/browser/resources/grit/skus_internals_generated_map.h"
...
base::Value::Dict SkusInternalsUI::GetVPNOrderInfo() const {
  ...
  if (!hns_vpn::IsHnsVPNEnabled(profile->GetPrefs())) {
    return dict;
  }

  dict.Set("env",
           local_state_->GetString(hns_vpn::prefs::kHnsVPNEnvironment));
...
void SkusInternalsUI::ResetSkusState() {
  ...
  auto* profile = Profile::FromWebUI(web_ui());
  if (hns_vpn::IsHnsVPNEnabled(profile->GetPrefs())) {
    hns_vpn::ClearSubscriberCredential(local_state_);
  }
#endif
...
std::string SkusInternalsUI::GetLastVPNConnectionError() const {
  std::string error;
#if BUILDFLAG(ENABLE_HNS_VPN)
  auto* api = g_hns_browser_process->hns_vpn_os_connection_api();
  CHECK(api);
