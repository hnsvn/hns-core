...
class HnsNewTabUIBrowserTest : public extensions::ExtensionFunctionalTest {
 public:
...
IN_PROC_BROWSER_TEST_F(HnsNewTabUIBrowserTest, StartupURLTest) {
...
// This test simply checks that by default the Hns new tab page is used.
// It does this by loading the newtab page and then checking if
// window.hns_new_tab exists.
IN_PROC_BROWSER_TEST_F(HnsNewTabUIBrowserTest, HnsNewTabIsDefault) {
  auto* contents = browser()->tab_strip_model()->GetActiveWebContents();
  GURL new_tab_url(chrome::kChromeUINewTabURL);
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), new_tab_url));
  WaitForLoadStop(contents);
...
IN_PROC_BROWSER_TEST_F(HnsNewTabUIBrowserTest, NewTabPageLocationOverride) {
