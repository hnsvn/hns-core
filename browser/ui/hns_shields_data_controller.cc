...
HnsShieldsDataController::~HnsShieldsDataController() = default;

HnsShieldsDataController::HnsShieldsDataController(
    content::WebContents* web_contents)
    : content::WebContentsObserver(web_contents),
      content::WebContentsUserData<HnsShieldsDataController>(*web_contents) {
  favicon::ContentFaviconDriver::FromWebContents(web_contents)
      ->AddObserver(this);
  observation_.Observe(GetHostContentSettingsMap(web_contents));
}

void HnsShieldsDataController::DidFinishNavigation(
...
void HnsShieldsDataController::WebContentsDestroyed() {
  favicon::ContentFaviconDriver::FromWebContents(web_contents())
      ->RemoveObserver(this);
  observation_.Reset();
}

void HnsShieldsDataController::OnContentSettingChanged(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsTypeSet content_type_set) {
  if ((content_type_set.ContainsAllTypes() ||
       content_type_set.GetType() == ContentSettingsType::HNS_SHIELDS) &&
...
void HnsShieldsDataController::OnFaviconUpdated(
...
void HnsShieldsDataController::ReloadWebContents() {
  web_contents()->GetController().Reload(content::ReloadType::NORMAL, true);
}

void HnsShieldsDataController::ClearAllResourcesList() {
...
void HnsShieldsDataController::AddObserver(Observer* obs) {
  observer_list_.AddObserver(obs);
}

void HnsShieldsDataController::RemoveObserver(Observer* obs) {
  observer_list_.RemoveObserver(obs);
}

bool HnsShieldsDataController::HasObserver(Observer* observer) {
  return observer_list_.HasObserver(observer);
}

int HnsShieldsDataController::GetTotalBlockedCount() {
...
std::vector<GURL> HnsShieldsDataController::GetBlockedAdsList() {
...
std::vector<GURL> HnsShieldsDataController::GetHttpRedirectsList() {
...
bool HnsShieldsDataController::GetHnsShieldsEnabled() {
  return hns_shields::GetHnsShieldsEnabled(
      GetHostContentSettingsMap(web_contents()), GetCurrentSiteURL());
}

void HnsShieldsDataController::SetHnsShieldsEnabled(bool is_enabled) {
  auto* map = GetHostContentSettingsMap(web_contents());
  if (map->GetDefaultContentSetting(ContentSettingsType::HNS_SHIELDS,
                                    nullptr) == is_enabled) {
    hns_shields::ResetHnsShieldsEnabled(map, GetCurrentSiteURL());
  } else {
    hns_shields::SetHnsShieldsEnabled(map, is_enabled, GetCurrentSiteURL());
  }
  ReloadWebContents();
}

GURL HnsShieldsDataController::GetCurrentSiteURL() {
  return web_contents()->GetLastCommittedURL();
}

GURL HnsShieldsDataController::GetFaviconURL(bool refresh) {
...
AdBlockMode HnsShieldsDataController::GetAdBlockMode() {
  auto* map = GetHostContentSettingsMap(web_contents());

  ControlType control_type_ad =
      hns_shields::GetAdControlType(map, GetCurrentSiteURL());

  ControlType control_type_cosmetic =
      hns_shields::GetCosmeticFilteringControlType(map, GetCurrentSiteURL());
...
FingerprintMode HnsShieldsDataController::GetFingerprintMode() {
  ControlType control_type = hns_shields::GetFingerprintingControlType(
      GetHostContentSettingsMap(web_contents()), GetCurrentSiteURL());
...
CookieBlockMode HnsShieldsDataController::GetCookieBlockMode() {
  auto cookie_settings = CookieSettingsFactory::GetForProfile(
      Profile::FromBrowserContext(web_contents()->GetBrowserContext()));

  const ControlType control_type = hns_shields::GetCookieControlType(
...
bool HnsShieldsDataController::GetHTTPSEverywhereEnabled() {
  return hns_shields::GetHTTPSEverywhereEnabled(
      GetHostContentSettingsMap(web_contents()), GetCurrentSiteURL());
}

HttpsUpgradeMode HnsShieldsDataController::GetHttpsUpgradeMode() {
  ControlType control_type = hns_shields::GetHttpsUpgradeControlType(
      GetHostContentSettingsMap(web_contents()), GetCurrentSiteURL());
  if (control_type == ControlType::ALLOW) {
...
bool HnsShieldsDataController::GetNoScriptEnabled() {
  ControlType control_type = hns_shields::GetNoScriptControlType(
      GetHostContentSettingsMap(web_contents()), GetCurrentSiteURL());
...
bool HnsShieldsDataController::GetForgetFirstPartyStorageEnabled() {
  return hns_shields::GetForgetFirstPartyStorageEnabled(
      GetHostContentSettingsMap(web_contents()), GetCurrentSiteURL());
}

void HnsShieldsDataController::SetAdBlockMode(AdBlockMode mode) {
  auto* map = GetHostContentSettingsMap(web_contents());
  ...
  hns_shields::SetAdControlType(map, control_type_ad, GetCurrentSiteURL(),
...
void HnsShieldsDataController::SetFingerprintMode(FingerprintMode mode) {
...
void HnsShieldsDataController::SetCookieBlockMode(CookieBlockMode mode) {
  ...
  hns_shields::SetCookieControlType(GetHostContentSettingsMap(web_contents()),
...
void HnsShieldsDataController::SetHttpsUpgradeMode(HttpsUpgradeMode mode) {
  ControlType control_type;
...
void HnsShieldsDataController::SetIsNoScriptEnabled(bool is_enabled) {
  ControlType control_type;
...
void HnsShieldsDataController::SetIsHTTPSEverywhereEnabled(bool is_enabled) {
...
void HnsShieldsDataController::SetForgetFirstPartyStorageEnabled(
    bool is_enabled) {
...
void HnsShieldsDataController::BlockAllowedScripts(
...
void HnsShieldsDataController::AllowScriptsOnce(
    const std::vector<std::string>& origins) {
  HnsShieldsWebContentsObserver* observer =
      HnsShieldsWebContentsObserver::FromWebContents(web_contents());
  if (!observer) {
...
bool HnsShieldsDataController::IsHnsShieldsManaged() {
  PrefService* profile_prefs =
      Profile::FromBrowserContext(web_contents()->GetBrowserContext())
          ->GetPrefs();

  return hns_shields::IsHnsShieldsManaged(
      profile_prefs, GetHostContentSettingsMap(web_contents()),
      GetCurrentSiteURL());
}

bool HnsShieldsDataController::IsForgetFirstPartyStorageFeatureEnabled()
    const {
  return base::FeatureList::IsEnabled(
      net::features::kHnsForgetFirstPartyStorage);
}

void HnsShieldsDataController::HandleItemBlocked(
...
void HnsShieldsDataController::HandleItemAllowedOnce(
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(HnsShieldsDataController);
