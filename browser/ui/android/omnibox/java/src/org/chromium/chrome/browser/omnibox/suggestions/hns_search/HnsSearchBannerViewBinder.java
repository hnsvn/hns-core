...
public class HnsSearchBannerViewBinder {
    public static void bind(PropertyModel model, View contentView, PropertyKey propertyKey) {
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            textViewParams.topMargin = contentView.getResources().getDimensionPixelSize(
                    R.dimen.omnibox_hns_search_banner_text_desc_top_margin);
            textViewParams.rightMargin = contentView.getResources().getDimensionPixelSize(
                    R.dimen.omnibox_hns_search_banner_text_desc_right_margin);
        ...
        boolean isMaybeLaterPressed =
                OmniboxPrefManager.getInstance().isHnsSearchPromoBannerMaybeLater();
        ...
        if (propertyKey == HnsSearchBannerProperties.DELEGATE) {
            HnsSearchBannerProperties.Delegate delegate =
                    model.get(HnsSearchBannerProperties.DELEGATE);
            if (delegate != null) {
                btnPositive.setOnClickListener(view -> { delegate.onPositiveClicked(); });

                btnNegative.setOnClickListener(view -> {
                    if (!isMaybeLaterPressed) {
                        OmniboxPrefManager.getInstance().setHnsSearchPromoBannerMaybeLater();
                    } else {
                        OmniboxPrefManager.getInstance().setHnsSearchPromoBannerDismissed();
                    }
