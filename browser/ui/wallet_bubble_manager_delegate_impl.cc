...
#include "hns/browser/ui/wallet_bubble_manager_delegate_impl.h"
...
#include "hns/browser/ui/views/wallet_bubble_focus_observer.h"
...
class WalletWebUIBubbleManager : public HnsWebUIBubbleManager<WalletPanelUI>,
 ...
 public:
  WalletWebUIBubbleManager(views::View* anchor_view,
      ...
      : HnsWebUIBubbleManager(anchor_view,
                                browser->profile(),
                                webui_url,
  ...
  base::WeakPtr<WebUIBubbleDialogView> CreateWebUIBubbleDialog(
    ...
    auto bubble_view_to_close =
        HnsWebUIBubbleManager::CreateWebUIBubbleDialog(anchor, arrow);
    ...
    HnsWebUIBubbleManager::CustomizeBubbleDialogView(*bubble_view);
  ...
  void CloseOpenedPopups() {
    ...
    for (auto tab_id : contents_wrapper->popup_ids()) {
      ...
      content::WebContents* popup_contents =
          hns_wallet::GetWebContentsFromTabId(&popup_browser, tab_id);
  ...
  void OnWidgetDestroying(views::Widget* widget) override {
    CloseOpenedPopups();
    HnsWebUIBubbleManager::OnWidgetDestroying(widget);
  }
  ...
  void SetCloseOnDeactivate(bool close) {
    if (bubble_view_) {
      ...
      if (hns_observer_)
        hns_observer_->UpdateBubbleDeactivationState(close);
    }
  }
 ...
 private:
  ...
  std::unique_ptr<WalletBubbleFocusObserver> hns_observer_;
...
WalletBubbleManagerDelegateImpl::WalletBubbleManagerDelegateImpl(
  ...
  if (browser->is_type_normal()) {
    anchor_view = static_cast<HnsBrowserView*>(browser->window())
  ...
  webui_bubble_manager_ = std::make_unique<WalletWebUIBubbleManager>(
      anchor_view, browser, webui_url_, IDS_ACCNAME_HNS_WALLET_BUTTON);
}
