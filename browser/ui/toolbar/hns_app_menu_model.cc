...
#include "hns/browser/ui/toolbar/app_menu_icons.h"
...
#include "hns/browser/ipfs/import/ipfs_import_controller.h"
...
class HnsHelpMenuModel : public ui::SimpleMenuModel {
 public:
  explicit HnsHelpMenuModel(ui::SimpleMenuModel::Delegate* delegate)
      : SimpleMenuModel(delegate) {
    Build();
  }
  HnsHelpMenuModel(const HnsHelpMenuModel&) = delete;
  HnsHelpMenuModel& operator=(const HnsHelpMenuModel&) = delete;
  ~HnsHelpMenuModel() override = default;

 private:
  void Build() {
    AddItemWithStringId(IDC_ABOUT, IDS_ABOUT);
    AddItemWithStringId(IDC_HELP_PAGE_VIA_MENU, IDS_HELP_PAGE);
    AddItemWithStringId(IDC_SHOW_HNS_WEBCOMPAT_REPORTER,
                        IDS_SHOW_HNS_WEBCOMPAT_REPORTER);
  }
};
...
HnsAppMenuModel::HnsAppMenuModel(
...
HnsAppMenuModel::~HnsAppMenuModel() = default;

void HnsAppMenuModel::Build() {
  // Customize items after build chromium items.
  // Insert & reorder hns menus based on corresponding commands enable status.
  // If we you want to add/remove from app menu, adjust commands enable status
  // at HnsBrowserCommandController.
  AppMenuModel::Build();

  RemoveUpstreamMenus();
  BuildTabsAndWindowsSection();
  BuildHnsProductsSection();
  BuildBrowserSection();
...
void HnsAppMenuModel::BuildTabsAndWindowsSection() {
...
void HnsAppMenuModel::BuildHnsProductsSection() {
  // Needs to add separator as this section is hns specific section.
  bool need_separator = false;
  if (IsCommandIdEnabled(IDC_SHOW_HNS_WALLET)) {
    InsertItemWithStringIdAt(GetNextIndexOfHnsProductsSection(),
                             IDC_SHOW_HNS_WALLET, IDS_SHOW_HNS_WALLET);
    need_separator = true;
  }

#if BUILDFLAG(ENABLE_HNS_VPN)
  if (IsCommandIdEnabled(IDC_HNS_VPN_MENU)) {
    sub_menus().push_back(std::make_unique<HnsVPNMenuModel>(
        browser(), browser()->profile()->GetPrefs()));
    InsertSubMenuWithStringIdAt(GetNextIndexOfHnsProductsSection(),
                                IDC_HNS_VPN_MENU, IDS_HNS_VPN_MENU,
                                sub_menus().back().get());
    need_separator = true;
  } else if (IsCommandIdEnabled(IDC_SHOW_HNS_VPN_PANEL)) {
    InsertItemWithStringIdAt(GetNextIndexOfHnsProductsSection(),
                             IDC_SHOW_HNS_VPN_PANEL, IDS_HNS_VPN_MENU);
    need_separator = true;
  }
#endif
  ...
  if (IsCommandIdEnabled(IDC_APP_MENU_IPFS)) {
    ...
    const int index = GetNextIndexOfHnsProductsSection();
  ...
  if (need_separator) {
    InsertSeparatorAt(GetNextIndexOfHnsProductsSection(),
                      ui::NORMAL_SEPARATOR);
  }
}

void HnsAppMenuModel::BuildBrowserSection() {
...
void HnsAppMenuModel::BuildMoreToolsSubMenu() {
  ...
  // Insert sync menu
  if (IsCommandIdEnabled(IDC_SHOW_HNS_SYNC)) {
    more_tools_menu_model->InsertItemWithStringIdAt(
        next_target_index++, IDC_SHOW_HNS_SYNC, IDS_SHOW_HNS_SYNC);
    need_separator = true;
  }
...
void HnsAppMenuModel::ExecuteCommand(int id, int event_flags) {
#if BUILDFLAG(ENABLE_IPFS_LOCAL_NODE)
...
bool HnsAppMenuModel::IsCommandIdEnabled(int id) const {
#if BUILDFLAG(ENABLE_IPFS_LOCAL_NODE)
  content::BrowserContext* browser_context =
...
#if BUILDFLAG(ENABLE_IPFS_LOCAL_NODE)
int HnsAppMenuModel::AddIpnsKeysToSubMenu(ui::SimpleMenuModel* submenu,
...
void HnsAppMenuModel::ExecuteIPFSCommand(int id, const std::string& key) {
...
int HnsAppMenuModel::GetSelectedIPFSCommandId(int id) const {
...
int HnsAppMenuModel::AddIpfsImportMenuItem(int action_command_id,
                                             int string_id,
                                             int keys_command_id) {
  content::BrowserContext* browser_context =
      static_cast<content::BrowserContext*>(browser()->profile());
...
size_t HnsAppMenuModel::GetNextIndexOfHnsProductsSection() const {
  std::vector<int> commands_to_check = {IDC_APP_MENU_IPFS,
                                        IDC_SHOW_HNS_VPN_PANEL,
                                        IDC_HNS_VPN_MENU,
                                        IDC_SHOW_HNS_WALLET,
...
absl::optional<size_t> HnsAppMenuModel::GetProperItemIndex(
