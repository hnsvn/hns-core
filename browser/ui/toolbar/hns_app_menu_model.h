...
#ifndef HNS_BROWSER_UI_TOOLBAR_HNS_APP_MENU_MODEL_H_
#define HNS_BROWSER_UI_TOOLBAR_HNS_APP_MENU_MODEL_H_
...
class HnsAppMenuModel : public AppMenuModel {
 public:
  HnsAppMenuModel(ui::AcceleratorProvider* provider,
                    Browser* browser,
                    AppMenuIconController* app_menu_icon_controller = nullptr,
                    AlertMenuItem alert_item = AlertMenuItem::kNone);
  ~HnsAppMenuModel() override;

  HnsAppMenuModel(const HnsAppMenuModel&) = delete;
  HnsAppMenuModel& operator=(const HnsAppMenuModel&) = delete;

 private:
  FRIEND_TEST_ALL_PREFIXES(HnsAppMenuModelBrowserTest, HnsIpfsMenuTest);
  ...
  void BuildHnsProductsSection();
  size_t GetNextIndexOfHnsProductsSection() const;
  ...
  // About hns, help center and report broken site items.
...
#endif  // HNS_BROWSER_UI_TOOLBAR_HNS_APP_MENU_MODEL_H_
