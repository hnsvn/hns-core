...
class HnsAppMenuModelBrowserTest : public InProcessBrowserTest {
 public:
  HnsAppMenuModelBrowserTest() {
#if BUILDFLAG(ENABLE_HNS_VPN)
...
void CheckCommandsAreDisabledInMenuModel(
  ...
  HnsAppMenuModel model(browser_view->toolbar(), browser);
  model.Init();
...
// Test hns menu order test.
// Hns menu is inserted based on corresponding commands enable status.
// So, this doesn't test for each profiles(normal, private, tor and guest).
// Instead, HnsBrowserCommandControllerTest will do that.
IN_PROC_BROWSER_TEST_F(HnsAppMenuModelBrowserTest, MenuOrderTest) {
  ...
  std::vector<int> help_commands_in_order = {
      ...
      IDC_SHOW_HNS_WEBCOMPAT_REPORTER,
  };
  ...
  std::vector<int> more_tools_in_order = {
      ...
      IDC_SHOW_HNS_SYNC, IDC_DEV_TOOLS,          IDC_TASK_MANAGER,
  };
  ...
  if (!syncer::IsSyncAllowedByFlag()) {
    ...
    more_tools_in_order.push_back(IDC_SHOW_HNS_SYNC);
  }
...
IN_PROC_BROWSER_TEST_F(HnsAppMenuModelBrowserTest, HnsVPNMenuTest) {
...
#if BUILDFLAG(ENABLE_IPFS_LOCAL_NODE)
IN_PROC_BROWSER_TEST_F(HnsAppMenuModelBrowserTest, HnsIpfsMenuTest) {
...
IN_PROC_BROWSER_TEST_F(HnsAppMenuModelBrowserTest, MenuItemsHaveIcons) {
  auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
  HnsAppMenuModel model(browser_view->toolbar(), browser());
  model.Init();
