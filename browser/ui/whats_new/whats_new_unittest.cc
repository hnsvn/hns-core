...
#include "hns/browser/ui/whats_new/pref_names.h"
...
class HnsWhatsNewTest : public testing::Test {
 public:
  HnsWhatsNewTest()
      : testing_profile_manager_(TestingBrowserProcess::GetGlobal()) {}
  ~HnsWhatsNewTest() override = default;
...
TEST_F(HnsWhatsNewTest, SupportedLangTest) {
  // Prepare all other factors to show hns whats-new except lang.
  ...
  {
    const hns_l10n::test::ScopedDefaultLocale scoped_default_locale("it_IT");
    EXPECT_FALSE(ShouldShowHnsWhatsNewForState(&local_state_));
  }

  // South korea is supported.
  {
    const hns_l10n::test::ScopedDefaultLocale scoped_default_locale("ko_KR");
    EXPECT_TRUE(ShouldShowHnsWhatsNewForState(&local_state_));
  }
}
...
// called.
TEST_F(HnsWhatsNewTest, FieldTrialNotAvailableTest) {
  // Set supported lang.
  const hns_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
...
TEST_F(HnsWhatsNewTest, NotUpdatedUserTest) {
...
TEST_F(HnsWhatsNewTest, MatchedCurrentVersionTest) {
  // Set supported lang.
  const hns_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
...
TEST_F(HnsWhatsNewTest, NotMatchedCurrentVersionTest) {
  // Set supported lang.
  const hns_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
...
TEST_F(HnsWhatsNewTest, NotWhatsNewIsAlreadyShown) {
  // Set supported lang.
  const hns_l10n::test::ScopedDefaultLocale scoped_default_locale("en_US");
