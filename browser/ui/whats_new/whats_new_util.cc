...
// Hns's major version is second and third component like 1.51.
// Ignored fourth number as it's build number.
absl::optional<double> GetHnsMajorVersionAsDouble(
    const base::Version& version) {
  double hns_major_version;
...
absl::optional<double> GetCurrentBrowserVersion() {
  ...
  return GetHnsMajorVersionAsDouble(version);
}
...
bool DoesUserGetMajorUpdateSinceInstall() {
  ...
  const auto profile_created_version = GetHnsMajorVersionAsDouble(
...
bool ShouldShowHnsWhatsNewForState(PrefService* local_state) {
  ...
  const std::string default_lang_code =
      hns_l10n::GetDefaultISOLanguageCodeString();
...
void StartHnsWhatsNew(Browser* browser) {
  constexpr char kHnsWhatsNewURL[] = "https://hns.com/whats-new/";
  // Load whats-new url in the first foreground tab.
  chrome::AddTabAt(browser, GURL(kHnsWhatsNewURL), 0, true);
