...
#include "hns/browser/ui/views/hns_news/hns_news_feed_item_view.h"
#include "hns/browser/ui/views/hns_news/hns_news_feeds_container_view.h"
...
// static
base::WeakPtr<views::Widget> HnsNewsBubbleView::Show(
    views::View* anchor,
    content::WebContents* contents) {
  auto* widget = views::BubbleDialogDelegateView::CreateBubble(
      std::make_unique<HnsNewsBubbleView>(anchor, contents));
  widget->Show();
  return widget->GetWeakPtr();
}

HnsNewsBubbleView::HnsNewsBubbleView(views::View* action_view,
                                         content::WebContents* contents)
  ...
  title_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_HNS_NEWS_BUBBLE_TITLE), custom_font));
  ...
  subtitle_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_HNS_NEWS_BUBBLE_SUBTITLE),
  ...
  feeds_container_ =
      AddChildView(std::make_unique<HnsNewsFeedsContainerView>(contents));
  ...
  auto* manage_feeds_button =
      AddChildView(std::make_unique<views::MdTextButton>(
          base::BindRepeating(&HnsNewsBubbleView::OpenManageFeeds,
                              base::Unretained(this)),
          l10n_util::GetStringUTF16(IDS_HNS_NEWS_BUBBLE_MANAGE_FEEDS)));
...
HnsNewsBubbleView::~HnsNewsBubbleView() = default;

void HnsNewsBubbleView::OpenManageFeeds() {
  auto* browser = chrome::FindBrowserWithWebContents(contents_);
  browser->OpenURL({GURL("hns://newtab/?openSettings=HnsNews"),
                    content::Referrer(),
                    WindowOpenDisposition::NEW_FOREGROUND_TAB,
                    ui::PAGE_TRANSITION_LINK, false});
}

void HnsNewsBubbleView::OnThemeChanged() {
  views::BubbleDialogDelegateView::OnThemeChanged();

  auto is_dark = dark_mode::GetActiveHnsDarkModeType() ==
...
BEGIN_METADATA(HnsNewsBubbleView, views::BubbleDialogDelegateView)
END_METADATA
