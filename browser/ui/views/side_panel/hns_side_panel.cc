...
HnsSidePanel::HnsSidePanel(BrowserView* browser_view,
                               HorizontalAlignment horizontal_alignment)
    : browser_view_(browser_view) {
  SetVisible(false);
  side_panel_width_.Init(
      sidebar::kSidePanelWidth, browser_view_->GetProfile()->GetPrefs(),
      base::BindRepeating(&HnsSidePanel::OnSidePanelWidthChanged,
...
HnsSidePanel::~HnsSidePanel() {
  RemoveObserver(this);
}

void HnsSidePanel::SetHorizontalAlignment(HorizontalAlignment alignment) {
  horizontal_alignment_ = alignment;
  UpdateBorder();
}

HnsSidePanel::HorizontalAlignment HnsSidePanel::GetHorizontalAlignment() {
  return horizontal_alignment_;
}

bool HnsSidePanel::IsRightAligned() {
  return horizontal_alignment_ == kHorizontalAlignRight;
}

void HnsSidePanel::UpdateBorder() {
...
void HnsSidePanel::OnSidePanelWidthChanged() {
  SetPanelWidth(side_panel_width_.GetValue());
}

void HnsSidePanel::OnThemeChanged() {
  View::OnThemeChanged();
  UpdateBorder();
}

gfx::Size HnsSidePanel::GetMinimumSize() const {
  // Use default width as a minimum width.
  return gfx::Size(sidebar::kDefaultSidePanelWidth, 0);
}

void HnsSidePanel::AddedToWidget() {
  resize_widget_ = std::make_unique<SidePanelResizeWidget>(
      this, static_cast<HnsBrowserView*>(browser_view_), this);
}

void HnsSidePanel::Layout() {
  if (children().empty()) {
    return;
  }
...
void HnsSidePanel::SetPanelWidth(int width) {
  // Only the width is used by BrowserViewLayout.
  SetPreferredSize(gfx::Size(width, 0));
}

void HnsSidePanel::OnResize(int resize_amount, bool done_resizing) {
...
void HnsSidePanel::AddHeaderView(std::unique_ptr<views::View> view) {
  // Do nothing.
}

BEGIN_METADATA(HnsSidePanel, views::View)
END_METADATA
