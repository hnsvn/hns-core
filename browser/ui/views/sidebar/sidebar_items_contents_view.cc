...
#include "hns/browser/ui/views/sidebar/sidebar_edit_item_bubble_delegate_view.h"
#include "hns/browser/ui/views/sidebar/sidebar_item_added_feedback_bubble.h"
#include "hns/browser/ui/views/sidebar/sidebar_item_view.h"
...
void SidebarItemsContentsView::UpdateAllBuiltInItemsViewState() {
  ...
  for (size_t item_index = 0; item_index < items_num; ++item_index) {
    ...
    // If browser window has tab that loads hns talk, hns talk panel icon
    // will use colored one for normal state also.
    if (item.built_in_item_type ==
        sidebar::SidebarItem::BuiltInItemType::kHnsTalk) {
...
gfx::ImageSkia SidebarItemsContentsView::GetImageForBuiltInItems(
  ...
  switch (type) {
    case sidebar::SidebarItem::BuiltInItemType::kWallet:
      ...
      break;
    case sidebar::SidebarItem::BuiltInItemType::kHnsTalk:
      focused_image_resource = IDR_SIDEBAR_HNS_TALK_FOCUSED;
      normal_image_icon = &kSidebarHnsTalkIcon;
      break;
