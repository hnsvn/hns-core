...
sidebar::SidebarService* GetSidebarService(HnsBrowser* browser) {
...
void SidebarContainerView::SetSidebarOnLeft(bool sidebar_on_left) {
  ...
  side_panel_->SetHorizontalAlignment(
      sidebar_on_left ? HnsSidePanel::kHorizontalAlignLeft
                      : HnsSidePanel::kHorizontalAlignRight);
...
void SidebarContainerView::ShowSidebar(bool show_side_panel) {
  ...
  if (ShouldUseAnimation()) {
    ...
    if (show_side_panel) {
      ...
      // width and HnsContentsLayoutManager only knows it because side panel's
      ...
      // preferred width, HnsContentsLayoutManager allocates more smaller
      // width to it.
      auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser_);
      const int target_sidebar_width =
          static_cast<HnsContentsLayoutManager*>(
