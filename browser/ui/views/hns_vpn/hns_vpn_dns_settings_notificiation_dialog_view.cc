...
constexpr char kHnsVPNLearnMoreURL[] =
    "https://support.hns.com/hc/en-us/articles/10864482160141";
...
// static
void HnsVpnDnsSettingsNotificiationDialogView::Show(Browser* browser) {
  auto* prefs = browser->profile()->GetPrefs();
  if (!prefs->GetBoolean(prefs::kHnsVPNShowNotificationDialog))
    return;
  ...
  constrained_window::CreateBrowserModalDialogViews(
      new HnsVpnDnsSettingsNotificiationDialogView(browser),
      browser->window()->GetNativeWindow())
      ->Show();
}

HnsVpnDnsSettingsNotificiationDialogView::
    HnsVpnDnsSettingsNotificiationDialogView(Browser* browser)
  ...
  SetButtonLabel(ui::DIALOG_BUTTON_OK,
                 l10n_util::GetStringUTF16(
                     IDS_HNS_VPN_DNS_SETTINGS_NOTIFICATION_DIALOG_OK_TEXT));

  RegisterWindowClosingCallback(
      base::BindOnce(&HnsVpnDnsSettingsNotificiationDialogView::OnClosing,
                     base::Unretained(this)));
  SetAcceptCallback(
      base::BindOnce(&HnsVpnDnsSettingsNotificiationDialogView::OnAccept,
                     base::Unretained(this)));
  auto* header_label =
      AddChildView(std::make_unique<views::Label>(l10n_util::GetStringUTF16(
          IDS_HNS_VPN_DNS_SETTINGS_NOTIFICATION_DIALOG_TITLE)));
  header_label->SetHorizontalAlignment(gfx::ALIGN_LEFT);

  const std::u16string contents_text = l10n_util::GetStringUTF16(
      IDS_HNS_VPN_DNS_SETTINGS_NOTIFICATION_DIALOG_TEXT);

  std::u16string learn_more_link_text = l10n_util::GetStringUTF16(
      IDS_HNS_VPN_DNS_SETTINGS_NOTIFICATION_DIALOG_LEARN_MORE_TEXT);
  ...
  views::StyledLabel::RangeStyleInfo link_style =
      views::StyledLabel::RangeStyleInfo::CreateForLink(base::BindRepeating(
          &HnsVpnDnsSettingsNotificiationDialogView::OnLearnMoreLinkClicked,
          base::Unretained(this)));
...
HnsVpnDnsSettingsNotificiationDialogView::
    ~HnsVpnDnsSettingsNotificiationDialogView() = default;

void HnsVpnDnsSettingsNotificiationDialogView::OnLearnMoreLinkClicked() {
  chrome::AddSelectedTabWithURL(browser_, GURL(kHnsVPNLearnMoreURL),
                                ui::PAGE_TRANSITION_AUTO_TOPLEVEL);
  AcceptDialog();
}

ui::ModalType HnsVpnDnsSettingsNotificiationDialogView::GetModalType() const {
  return ui::MODAL_TYPE_WINDOW;
}

bool HnsVpnDnsSettingsNotificiationDialogView::ShouldShowCloseButton() const {
  return false;
}

bool HnsVpnDnsSettingsNotificiationDialogView::ShouldShowWindowTitle() const {
  return false;
}

void HnsVpnDnsSettingsNotificiationDialogView::OnAccept() {
  close_window_ = true;
}

void HnsVpnDnsSettingsNotificiationDialogView::OnClosing() {
  prefs_->SetBoolean(prefs::kHnsVPNShowNotificationDialog,
                     !dont_ask_again_checkbox_->GetChecked());
}

BEGIN_METADATA(HnsVpnDnsSettingsNotificiationDialogView,
