...
#include "hns/browser/ui/views/infobars/web_discovery_infobar_content_view.h"
...
#include "hns/browser/ui/views/infobars/custom_styled_label.h"
#include "hns/browser/web_discovery/web_discovery_infobar_delegate.h"
    ...
    canvas->DrawImageInt(
        gfx::ImageSkiaOperations::CreateResizedImage(
            *rb.GetImageSkiaNamed(
                ui::NativeTheme::GetInstanceForNativeUi()->ShouldUseDarkColors()
                    ? IDR_HNS_WEB_DISCOVERY_INFOBAR_BG_GRAPHIC_DARK
                    : IDR_HNS_WEB_DISCOVERY_INFOBAR_BG_GRAPHIC),
...
void WebDiscoveryInfoBarContentView::AddedToWidget() {
  ...
  // And it's added to widget again when hns search tab is activated.
...
std::unique_ptr<views::View> WebDiscoveryInfoBarContentView::GetMessage(
    int order,
    int line_height) {
  const std::u16string hns_search_text =
      l10n_util::GetStringUTF16(IDS_WEB_DISCOVERY_INFOBAR_MESSAGE_HNS_SEARCH);
  ...
  const std::u16string message_text =
      l10n_util::GetStringFUTF16(IDS_WEB_DISCOVERY_INFOBAR_MESSAGE,
                                 hns_search_text, more_info_text, &offsets);
  ...
  views::StyledLabel::RangeStyleInfo hns_search_style;
  hns_search_style.custom_font = gfx::FontList("Poppins, Semi-Bold 14px");
  hns_search_style.override_color =
      GetColorProvider()->GetColor(kColorWebDiscoveryInfoBarMessage);
  message_label->AddStyleRange(
      gfx::Range(offsets[0], offsets[0] + hns_search_text.length()),
      hns_search_style);
  ...
  message_label->AddStyleRange(
      gfx::Range(offsets[0] + hns_search_text.length(),
...
std::unique_ptr<views::View> WebDiscoveryInfoBarContentView::GetIcon(
  ...
  auto icon = std::make_unique<views::ImageView>(ui::ImageModel::FromImageSkia(
      *rb.GetImageSkiaNamed(IDR_HNS_WEB_DISCOVERY_INFOBAR_ICON)));
  icon->SetProperty(
      views::kFlexBehaviorKey,
      views::FlexSpecification(views::MinimumFlexSizeRule::kPreferred,
                               views::MaximumFlexSizeRule::kPreferred)
