...
std::unique_ptr<infobars::InfoBar> CreateHnsConfirmInfoBar(
  ...
  return std::make_unique<HnsConfirmInfoBar>(std::move(delegate));
}

HnsConfirmInfoBar::HnsConfirmInfoBar(
    std::unique_ptr<HnsConfirmInfoBarDelegate> delegate)
  ...
  const auto create_button =
      [this](ConfirmInfoBarDelegate::InfoBarButton type,
             void (HnsConfirmInfoBar::*click_function)()) {
  ...
  if (buttons & ConfirmInfoBarDelegate::BUTTON_OK) {
    ok_button_ = create_button(ConfirmInfoBarDelegate::BUTTON_OK,
                               &HnsConfirmInfoBar::OkButtonPressed);
  ...
  if (buttons & ConfirmInfoBarDelegate::BUTTON_CANCEL) {
    cancel_button_ = create_button(ConfirmInfoBarDelegate::BUTTON_CANCEL,
                                   &HnsConfirmInfoBar::CancelButtonPressed);
    if (buttons == ConfirmInfoBarDelegate::BUTTON_CANCEL ||
  ...
  if (buttons & ConfirmInfoBarDelegate::BUTTON_EXTRA) {
    extra_button_ = create_button(ConfirmInfoBarDelegate::BUTTON_EXTRA,
                                  &HnsConfirmInfoBar::ExtraButtonPressed);
    if (buttons == ConfirmInfoBarDelegate::BUTTON_EXTRA ||
  ...
  if (delegate_ptr->HasCheckbox()) {
    checkbox_ = AddChildView(std::make_unique<views::Checkbox>(
        ...
        base::BindRepeating(&HnsConfirmInfoBar::CheckboxPressed,
                            weak_ptr_factory_.GetWeakPtr())));
  }
}

HnsConfirmInfoBar::~HnsConfirmInfoBar() = default;

views::MdTextButton* HnsConfirmInfoBar::GetButtonById(int id) {
  switch (id) {
...
void HnsConfirmInfoBar::CheckboxPressed() {
  GetDelegate()->SetCheckboxChecked(checkbox_->GetChecked());
}

void HnsConfirmInfoBar::CloseButtonPressed() {
...
void HnsConfirmInfoBar::OkButtonPressed() {
  if (!owner()) {
...
void HnsConfirmInfoBar::CancelButtonPressed() {
  if (!owner()) {
    return;  // We're closing; don't call anything, it might access the owner.
  }
...
void HnsConfirmInfoBar::ExtraButtonPressed() {
  if (!owner()) {
    return;  // We're closing; don't call anything, it might access the owner.
  }
...
HnsConfirmInfoBarDelegate* HnsConfirmInfoBar::GetDelegate() const {
  return reinterpret_cast<HnsConfirmInfoBarDelegate*>(delegate());
}

int HnsConfirmInfoBar::GetContentMinimumWidth() const {
  return label_->GetMinimumSize().width() + link_->GetMinimumSize().width() +
         NonLabelWidth();
}

int HnsConfirmInfoBar::NonLabelWidth() const {
  ChromeLayoutProvider* layout_provider = ChromeLayoutProvider::Get();
...
BEGIN_METADATA(HnsConfirmInfoBar, InfoBarView)
END_METADATA
