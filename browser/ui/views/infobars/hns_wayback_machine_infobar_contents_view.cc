...
HnsWaybackMachineInfoBarContentsView::HnsWaybackMachineInfoBarContentsView(
...
HnsWaybackMachineInfoBarContentsView::
    ~HnsWaybackMachineInfoBarContentsView() = default;

void HnsWaybackMachineInfoBarContentsView::OnThemeChanged() {
  ...
  wayback_spot_graphic_->SetImage(
      rb.GetImageSkiaNamed(ui::NativeTheme::GetInstanceForNativeUi()->
          ShouldUseDarkColors() ? IDR_HNS_WAYBACK_INFOBAR_DARK
                                : IDR_HNS_WAYBACK_INFOBAR));
}

void HnsWaybackMachineInfoBarContentsView::OnWaybackURLFetched(
...
void HnsWaybackMachineInfoBarContentsView::HideInfobar() {
  ...
  for (size_t i = 0; i < infobar_manager->infobar_count(); ++i) {
    ...
    if (infobar->delegate()->GetIdentifier() ==
        HnsWaybackMachineInfoBarDelegate::WAYBACK_MACHINE_INFOBAR_DELEGATE) {
...
void HnsWaybackMachineInfoBarContentsView::FetchURLButtonPressed() {
...
void HnsWaybackMachineInfoBarContentsView::OnCheckboxUpdated() {
  pref_service_->SetBoolean(kHnsWaybackMachineEnabled,
                            !dont_ask_again_checkbox_->GetChecked());
}

void HnsWaybackMachineInfoBarContentsView::InitializeChildren() {
  ...
  auto* label = CreateLabel(hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_WAYBACK_MACHINE_INFOBAR_PAGE_MISSING_TEXT));
  label->SetFontList(
  ...
  label = CreateLabel(hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_WAYBACK_MACHINE_INFOBAR_ASK_ABOUT_CHECK_TEXT));
  views_visible_before_checking_.push_back(label);
  label->SetProperty(
      views::kMarginsKey,
      gfx::Insets::VH(ChromeLayoutProvider::Get()->GetDistanceMetric(
  ...
  dont_ask_again_checkbox_ =
      AddChildView(std::make_unique<DontAskAgainCheckbox>(
          hns_l10n::GetLocalizedResourceUTF16String(
              IDS_HNS_WAYBACK_MACHINE_DONT_ASK_AGAIN_TEXT),
          base::BindRepeating(
              &HnsWaybackMachineInfoBarContentsView::OnCheckboxUpdated,
  ...
  label = CreateLabel(hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_WAYBACK_MACHINE_INFOBAR_NOT_AVAILABLE_TEXT));
  ...
  fetch_url_button_ =
      AddChildView(std::make_unique<HnsWaybackMachineInfoBarButtonContainer>(
          base::BindRepeating(
              &HnsWaybackMachineInfoBarContentsView::FetchURLButtonPressed,
              base::Unretained(this))));
...
views::Label* HnsWaybackMachineInfoBarContentsView::CreateLabel(
...
void HnsWaybackMachineInfoBarContentsView::UpdateChildrenVisibility(
...
SkColor HnsWaybackMachineInfoBarContentsView::GetColor(int id) const {
  const auto* color_provider = GetColorProvider();
  return color_provider ? color_provider->GetColor(id) : gfx::kPlaceholderColor;
}

void HnsWaybackMachineInfoBarContentsView::FetchWaybackURL() {
  fetch_url_button_->StartThrobber();
  wayback_machine_url_fetcher_.Fetch(contents_->GetVisibleURL());
  Layout();
}

void HnsWaybackMachineInfoBarContentsView::LoadURL(const GURL& url) {
...
BEGIN_METADATA(HnsWaybackMachineInfoBarContentsView, views::View)
END_METADATA
