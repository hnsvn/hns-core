...
#include "hns/browser/ui/views/download/hns_download_item_view.h"
...
HnsDownloadItemView::~HnsDownloadItemView() = default;

// View overrides.

void HnsDownloadItemView::Layout() {
  DownloadItemView::Layout();
}

gfx::Size HnsDownloadItemView::CalculatePreferredSize() const {
  ...
  int child_height = file_name_label_->GetLineHeight() +
                     kHnsVerticalTextPadding +
                     origin_url_font_list_.GetHeight() +
                     kHnsVerticalTextPadding + status_label_->GetLineHeight();
...
void HnsDownloadItemView::OnPaint(gfx::Canvas* canvas) {
...
void HnsDownloadItemView::OnDownloadUpdated() {
  ...
  if (IsShowingWarningDialog() != model()->IsDangerous()) {
    ...
    std::u16string new_origin_url =
        hns_model_.GetOriginURLText(&new_is_secure);
  ...
  // Update tooltip.
  std::u16string new_tip = hns_model_.GetTooltipText();
...
int HnsDownloadItemView::GetYForFilenameText() const {
  int text_height = file_name_label_->GetLineHeight();
  if (!origin_url_text_.empty())
    text_height +=
        kHnsVerticalTextPadding + origin_url_font_list_.GetHeight();
  if (status_label_ && !status_label_->GetText().empty())
    text_height += kHnsVerticalTextPadding + status_label_->GetLineHeight();
  return (height() - text_height) / 2;
}

int HnsDownloadItemView::GetYForOriginURLText() const {
  return file_name_label_->y() + kHnsVerticalTextPadding;
}

int HnsDownloadItemView::GetYForStatusText() const {
  int y = GetYForOriginURLText();
  if (!origin_url_text_.empty())
    y += (origin_url_font_list_.GetHeight() + kHnsVerticalTextPadding);
  return y;
}

// Drawing routines.

void HnsDownloadItemView::DrawOriginURL(gfx::Canvas* canvas) {
...
void HnsDownloadItemView::DrawLockIcon(gfx::Canvas* canvas) {
  if (origin_url_text_.empty() || IsShowingWarningDialog())
    return;
...
gfx::ImageSkia HnsDownloadItemView::GetLockIcon(int height) {
  return gfx::CreateVectorIcon(kDownloadUnlockIcon, height,
    kDownloadUnlockIconColor);
}

// Update accessible name with origin URL.
void HnsDownloadItemView::SetMode(download::DownloadItemMode mode) {
  ...
  if (!origin_url_text_.empty()) {
    std::u16string extra;
    if (!is_origin_url_secure_)
      extra += char16_t(' ') + hns_l10n::GetLocalizedResourceUTF16String(
...
void HnsDownloadItemView::UpdateLabels() {
...
void HnsDownloadItemView::SetOriginUrlVisible(bool visible) {
  is_origin_url_visible_ = visible;
  UpdateLabels();
}

void HnsDownloadItemView::OnMouseEntered(const ui::MouseEvent& event) {
  SetOriginUrlVisible(true);
}

void HnsDownloadItemView::OnMouseExited(const ui::MouseEvent& event) {
  SetOriginUrlVisible(false);
}

void HnsDownloadItemView::OnViewFocused(View* observed_view) {
  SetOriginUrlVisible(true);
}

void HnsDownloadItemView::OnViewBlurred(View* observed_view) {
  SetOriginUrlVisible(false);
}
