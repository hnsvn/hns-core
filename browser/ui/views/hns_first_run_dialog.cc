...
#include "hns/browser/ui/views/hns_first_run_dialog.h"
...
void ShowHnsFirstRunDialogViews() {
  base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
  HnsFirstRunDialog::Show(run_loop.QuitClosure());
  run_loop.Run();
}
...
// static
void HnsFirstRunDialog::Show(base::RepeatingClosure quit_runloop) {
  HnsFirstRunDialog* dialog =
      new HnsFirstRunDialog(std::move(quit_runloop));
  views::DialogDelegate::CreateDialogWidget(dialog, nullptr, nullptr)->Show();
}

HnsFirstRunDialog::HnsFirstRunDialog(base::RepeatingClosure quit_runloop)
  ...
  auto* header_label = AddChildView(std::make_unique<views::Label>(
      hns_l10n::GetLocalizedResourceUTF16String(IDS_FIRSTRUN_DLG_HEADER_TEXT),
      header_font));
...
HnsFirstRunDialog::~HnsFirstRunDialog() = default;

void HnsFirstRunDialog::Done() {
  CHECK(!quit_runloop_.is_null());
  quit_runloop_.Run();
}

bool HnsFirstRunDialog::Accept() {
  GetWidget()->Hide();

#if BUILDFLAG(ENABLE_PIN_SHORTCUT)
  base::MakeRefCounted<shell_integration::HnsDefaultBrowserWorker>()
      ->StartSetAsDefault(base::BindOnce(
          [](bool pin_to_shortcut,
             shell_integration::DefaultWebClientState state) {
            if (pin_to_shortcut &&
                state == shell_integration::DefaultWebClientState::IS_DEFAULT) {
              // Try to pin to taskbar when Hns is set as a default browser.
...
void HnsFirstRunDialog::WindowClosing() {
  Done();
}

BEGIN_METADATA(HnsFirstRunDialog, views::DialogDelegateView)
END_METADATA
