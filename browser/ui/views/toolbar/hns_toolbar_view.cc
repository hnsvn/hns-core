...
HnsToolbarView::HnsToolbarView(Browser* browser, BrowserView* browser_view)
    : ToolbarView(browser, browser_view) {}

HnsToolbarView::~HnsToolbarView() = default;

void HnsToolbarView::Init() {
  ToolbarView::Init();

  // This will allow us to move this window by dragging toolbar.
  // See hns_non_client_hit_test_helper.h
  ...
  edit_bookmarks_enabled_.Init(
      ...
      base::BindRepeating(&HnsToolbarView::OnEditBookmarksEnabledChanged,
                          base::Unretained(this)));
  show_bookmarks_button_.Init(
      kShowBookmarksButton, browser_->profile()->GetPrefs(),
      base::BindRepeating(&HnsToolbarView::OnShowBookmarksButtonChanged,
                          base::Unretained(this)));
  // track changes in wide locationbar setting
  location_bar_is_wide_.Init(
      kLocationBarIsWide, profile->GetPrefs(),
      base::BindRepeating(&HnsToolbarView::OnLocationBarIsWideChanged,
                          base::Unretained(this)));
  ...
  if (base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs) &&
    ...
    show_vertical_tabs_.Init(
        ...
        base::BindRepeating(&HnsToolbarView::UpdateHorizontalPadding,
                            base::Unretained(this)));
...
#if BUILDFLAG(ENABLE_HNS_VPN)
  if (hns_vpn::IsAllowedForContext(profile)) {
    hns_vpn_ = container_view->AddChildViewAt(
        std::make_unique<HnsVPNButton>(browser()),
        *container_view->GetIndexOf(GetAppMenuButton()) - 1);
    show_hns_vpn_button_.Init(
        hns_vpn::prefs::kHnsVPNShowButton, profile->GetPrefs(),
        base::BindRepeating(&HnsToolbarView::OnVPNButtonVisibilityChanged,
                            base::Unretained(this)));
    hide_hns_vpn_button_by_policy_.Init(
        hns_vpn::prefs::kManagedHnsVPNDisabled, profile->GetPrefs(),
        base::BindRepeating(&HnsToolbarView::OnVPNButtonVisibilityChanged,
                            base::Unretained(this)));
    hns_vpn_->SetVisible(IsHnsVPNButtonVisible());
  }
#endif
...
#if BUILDFLAG(ENABLE_HNS_VPN)
bool HnsToolbarView::IsHnsVPNButtonVisible() const {
  return show_hns_vpn_button_.GetValue() &&
         !hide_hns_vpn_button_by_policy_.GetValue();
}
void HnsToolbarView::OnVPNButtonVisibilityChanged() {
  DCHECK(hns_vpn_);
  hns_vpn_->SetVisible(IsHnsVPNButtonVisible());
}
#endif

void HnsToolbarView::OnEditBookmarksEnabledChanged() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
  Update(nullptr);
}

void HnsToolbarView::OnShowBookmarksButtonChanged() {
  if (!bookmark_)
    return;
...
void HnsToolbarView::OnLocationBarIsWideChanged() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
...
void HnsToolbarView::OnThemeChanged() {
...
void HnsToolbarView::OnProfileAdded(const base::FilePath& profile_path) {
  Update(nullptr);
}

void HnsToolbarView::OnProfileWasRemoved(const base::FilePath& profile_path,
                                           const std::u16string& profile_name) {
  Update(nullptr);
}

void HnsToolbarView::LoadImages() {
...
void HnsToolbarView::Update(content::WebContents* tab) {
...
void HnsToolbarView::UpdateBookmarkVisibility() {
  if (!bookmark_)
    return;
...
void HnsToolbarView::UpdateHorizontalPadding() {
...
void HnsToolbarView::ShowBookmarkBubble(const GURL& url,
                                          bool already_bookmarked) {
  // Show BookmarkBubble attached to Hns's bookmark button
...
void HnsToolbarView::ViewHierarchyChanged(
  ...
  if (details.is_add && details.parent == children()[0]) {
    ...
    // perceived as caption area. See hns_non_client_hit_test_helper.h
    views::SetHitTestComponent(details.child, HTCLIENT);
  }
}

void HnsToolbarView::Layout() {
  ToolbarView::Layout();
...
void HnsToolbarView::ResetLocationBarBounds() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
...
void HnsToolbarView::ResetButtonBounds() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
