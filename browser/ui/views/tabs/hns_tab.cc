...
HnsTab::HnsTab(TabSlotController* controller)
    : Tab(controller),
...
HnsTab::~HnsTab() = default;

std::u16string HnsTab::GetTooltipText(const gfx::Point& p) const {
  auto* browser = controller_->GetBrowser();
  if (browser &&
      hns_tabs::AreTooltipsEnabled(browser->profile()->GetPrefs())) {
...
int HnsTab::GetWidthOfLargestSelectableRegion() const {
...
void HnsTab::ActiveStateChanged() {
  ...
  // https://github.com/hnsvn/hns-browser/issues/23476/
...
absl::optional<SkColor> HnsTab::GetGroupColor() const {
  if (!base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs)) {
...
void HnsTab::UpdateIconVisibility() {
...
void HnsTab::ViewHierarchyChanged(
...
void HnsTab::OnLayerBoundsChanged(const gfx::Rect& old_bounds,
...
gfx::Insets HnsTab::GetInsets() const {
...
void HnsTab::ReorderChildLayers(ui::Layer* parent_layer) {
...
void HnsTab::MaybeAdjustLeftForPinnedTab(gfx::Rect* bounds,
...
bool HnsTab::ShouldRenderAsNormalTab() const {
  if (IsAtMinWidthForVerticalTabStrip()) {
...
bool HnsTab::IsAtMinWidthForVerticalTabStrip() const {
  if (!base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs)) {
    return false;
  }
...
void HnsTab::UpdateShadowForActiveTab() {
...
std::unique_ptr<ui::Layer> HnsTab::CreateShadowLayer() {
  auto layer = std::make_unique<ShadowLayer>(this);
  layer->SetFillsBoundsOpaquely(false);
  return layer;
}

void HnsTab::LayoutShadowLayer() {
  CHECK(shadow_layer_);
...
void HnsTab::AddLayerToBelowThis() {
  if (!layer()) {
    SetPaintToLayer();
    layer()->SetFillsBoundsOpaquely(false);
  }
