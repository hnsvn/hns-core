...
// HnsGM2TabStyle
//
class HnsGM2TabStyle : public GM2TabStyleViews {
 public:
  explicit HnsGM2TabStyle(Tab* tab);
  HnsGM2TabStyle(const HnsGM2TabStyle&) = delete;
  HnsGM2TabStyle& operator=(const HnsGM2TabStyle&) = delete;
  ~HnsGM2TabStyle() override = default;
...
HnsGM2TabStyle::HnsGM2TabStyle(Tab* tab)
    : GM2TabStyleViews(tab), tab_(tab) {}

TabStyle::TabColors HnsGM2TabStyle::CalculateTargetColors() const {
...
// enabled.
//
class HnsVerticalTabStyle : public HnsGM2TabStyle {
 public:
  explicit HnsVerticalTabStyle(Tab* tab);
  HnsVerticalTabStyle(const HnsVerticalTabStyle&) = delete;
  HnsVerticalTabStyle& operator=(const HnsVerticalTabStyle&) = delete;
  ~HnsVerticalTabStyle() override = default;
...
TabStyle::SeparatorBounds HnsVerticalTabStyle::GetSeparatorBounds(
    float scale) const {
  if (ShouldShowVerticalTabs()) {
    return {};
  }

  return HnsGM2TabStyle::GetSeparatorBounds(scale);
}

void HnsVerticalTabStyle::PaintTab(gfx::Canvas* canvas) const {
  if (ShouldShowVerticalTabs()) {
    ...
    HnsGM2TabStyle::PaintTab(canvas);
  }
  ...
  if (tab()->data().pinned) {
    ...
    const SkColor tab_stroke_color =
        widget->GetColorProvider()->GetColor(kColorHnsVerticalTabSeparator);
    PaintBackgroundStroke(canvas, TabStyle::TabSelectionState::kActive,
                          tab_stroke_color);
  }
}

SkColor HnsVerticalTabStyle::GetTargetTabBackgroundColor(
    TabStyle::TabSelectionState selection_state,
    bool hovered) const {
  if (!ShouldShowVerticalTabs()) {
    return HnsGM2TabStyle::GetTargetTabBackgroundColor(selection_state,
                                                         hovered);
  }
...
bool HnsVerticalTabStyle::ShouldShowVerticalTabs() const {
...
std::unique_ptr<TabStyleViews> TabStyleViews::CreateForTab(Tab* tab) {
  if (base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs)) {
    return std::make_unique<HnsVerticalTabStyle>(tab);
  }

  return std::make_unique<HnsGM2TabStyle>(tab);
}
