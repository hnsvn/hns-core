...
#include "hns/browser/ui/views/tabs/hns_compound_tab_container.h"
...
class ContentsView : public views::View {
 public:
  explicit ContentsView(HnsCompoundTabContainer* container)
  ...
  void ChildPreferredSizeChanged(views::View* child) override {
    // Bypass ScrollView and notify the HnsCompoundTabContainer directly.
    container_->ChildPreferredSizeChanged(child);
  }

  base::raw_ptr<HnsCompoundTabContainer> container_;
};
...
HnsCompoundTabContainer::HnsCompoundTabContainer(
...
void HnsCompoundTabContainer::SetAvailableWidthCallback(
...
void HnsCompoundTabContainer::SetScrollEnabled(bool enabled) {
  DCHECK(base::FeatureList::IsEnabled(
      tabs::features::kHnsVerticalTabsStickyPinnedTabs));
...
void HnsCompoundTabContainer::TransferTabBetweenContainers(
...
void HnsCompoundTabContainer::Layout() {
  if (!ShouldShowVerticalTabs()) {
...
gfx::Size HnsCompoundTabContainer::CalculatePreferredSize() const {
  if (!ShouldShowVerticalTabs()) {
...
gfx::Size HnsCompoundTabContainer::GetMinimumSize() const {
  if (!ShouldShowVerticalTabs()) {
...
views::SizeBounds HnsCompoundTabContainer::GetAvailableSize(
...
Tab* HnsCompoundTabContainer::AddTab(std::unique_ptr<Tab> tab,
                                       int model_index,
                                       TabPinned pinned) {
  auto* new_tab =
...
int HnsCompoundTabContainer::GetUnpinnedContainerIdealLeadingX() const {
  if (!ShouldShowVerticalTabs()) {
...
BrowserRootView::DropTarget* HnsCompoundTabContainer::GetDropTarget(
...
void HnsCompoundTabContainer::OnThemeChanged() {
  CompoundTabContainer::OnThemeChanged();

  if (ShouldShowVerticalTabs()) {
    pinned_tab_container_->SetBorder(views::CreateSolidSidedBorder(
        gfx::Insets().set_bottom(1),
        GetColorProvider()->GetColor(kColorHnsVerticalTabSeparator)));
  } else {
    pinned_tab_container_->SetBorder(nullptr);
  }
}

void HnsCompoundTabContainer::PaintChildren(const views::PaintInfo& info) {
  if (ShouldShowVerticalTabs() &&
...
void HnsCompoundTabContainer::ChildPreferredSizeChanged(views::View* child) {
...
void HnsCompoundTabContainer::SetActiveTab(
...
TabContainer* HnsCompoundTabContainer::GetTabContainerAt(
...
gfx::Rect HnsCompoundTabContainer::ConvertUnpinnedContainerIdealBoundsToLocal(
...
bool HnsCompoundTabContainer::ShouldShowVerticalTabs() const {
  return base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs) &&
         tabs::utils::ShouldShowVerticalTabs(
             tab_slot_controller_->GetBrowser());
}

void HnsCompoundTabContainer::UpdateUnpinnedContainerSize() {
  DCHECK(scroll_view_);
...
void HnsCompoundTabContainer::ScrollTabToBeVisible(int model_index) {
  if (!scroll_view_) {
    return;
  }
...
BEGIN_METADATA(HnsCompoundTabContainer, CompoundTabContainer)
END_METADATA
