...
HnsTabStrip::HnsTabStrip(std::unique_ptr<TabStripController> controller)
    : TabStrip(std::move(controller)) {}

HnsTabStrip::~HnsTabStrip() = default;

bool HnsTabStrip::IsVerticalTabsFloating() const {
...
bool HnsTabStrip::ShouldDrawStrokes() const {
  if (ShouldShowVerticalTabs()) {
  ...
  return contrast_ratio < kHnsMinimumContrastRatioForOutlines;
}

int HnsTabStrip::GetStrokeThickness() const {
  if (ShouldShowVerticalTabs()) {
...
void HnsTabStrip::UpdateHoverCard(Tab* tab, HoverCardUpdateType update_type) {
  if (hns_tabs::AreTooltipsEnabled(controller_->GetProfile()->GetPrefs())) {
    return;
  }
  TabStrip::UpdateHoverCard(tab, update_type);
}

void HnsTabStrip::MaybeStartDrag(
    TabSlotView* source,
...
void HnsTabStrip::AddedToWidget() {
  ...
  if (BrowserView::GetBrowserViewForBrowser(GetBrowser())) {
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&HnsTabStrip::UpdateTabContainer,
                                  base::Unretained(this)));
  }
}

SkColor HnsTabStrip::GetTabSeparatorColor() const {
  if (ShouldShowVerticalTabs()) {
  ...
  bool dark_mode = dark_mode::GetActiveHnsDarkModeType() ==
...
absl::optional<int> HnsTabStrip::GetCustomBackgroundId(
...
void HnsTabStrip::UpdateTabContainer() {
  ...
  const bool is_using_compound_tab_container =
      ...
      HnsCompoundTabContainer::kViewClassName;
  ...
  if (should_use_compound_tab_container != is_using_compound_tab_container) {
    ...
    if (should_use_compound_tab_container) {
      ...
      auto* hns_tab_container = AddChildViewAt(
          std::make_unique<HnsCompoundTabContainer>(
              *this, hover_card_controller_.get(), drag_context, *this, this),
          0);
      tab_container_ = *hns_tab_container;
      layout_lock =
          base::ScopedClosureRunner(hns_tab_container->LockLayout());

      if (using_sticky_pinned_tabs) {
        hns_tab_container->SetScrollEnabled(using_vertical_tabs);
      ...
      // Container should be attached before TabDragContext so that dragged
      // views can be atop container.
      auto* hns_tab_container =
          AddChildViewAt(std::make_unique<HnsTabContainer>(
...
bool HnsTabStrip::ShouldShowVerticalTabs() const {
  if (!base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs)) {
    return false;
  }
...
BEGIN_METADATA(HnsTabStrip, TabStrip)
END_METADATA
