...
class HnsTabHoverTest : public InProcessBrowserTest {
 public:
  HnsTabHoverTest()
  ...
  HnsTabHoverTest(const HnsTabHoverTest&) = delete;
  HnsTabHoverTest& operator=(const HnsTabHoverTest&) = delete;
...
IN_PROC_BROWSER_TEST_F(HnsTabHoverTest,
  ...
  browser()->profile()->GetPrefs()->SetInteger(hns_tabs::kTabHoverMode,
                                               hns_tabs::TabHoverMode::CARD);
  EXPECT_EQ(u"", active_tab()->GetTooltipText(gfx::Point()));

  browser()->profile()->GetPrefs()->SetInteger(
      hns_tabs::kTabHoverMode, hns_tabs::TabHoverMode::CARD_WITH_PREVIEW);
  EXPECT_EQ(u"", active_tab()->GetTooltipText(gfx::Point()));

  browser()->profile()->GetPrefs()->SetInteger(
      hns_tabs::kTabHoverMode, hns_tabs::TabHoverMode::TOOLTIP);
...
IN_PROC_BROWSER_TEST_F(HnsTabHoverTest, ThumbnailHelperIsAlwaysAttached) {
  browser()->profile()->GetPrefs()->SetInteger(hns_tabs::kTabHoverMode,
                                               hns_tabs::TabHoverMode::CARD);
  TabRendererData data;
...
IN_PROC_BROWSER_TEST_F(HnsTabHoverTest, ChromeFeatureDisabledByDefault) {
  EXPECT_FALSE(base::FeatureList::IsEnabled(features::kTabHoverCardImages));
}

class HnsTabHoverTestWithChromeFlag : public HnsTabHoverTest {
 public:
  HnsTabHoverTestWithChromeFlag() {
...
#endif
IN_PROC_BROWSER_TEST_F(HnsTabHoverTestWithChromeFlag,
