...
HnsTooltipPopup::HnsTooltipPopup(Profile* profile,
...
HnsTooltipPopup::~HnsTooltipPopup() {
  display::Screen* screen = display::Screen::GetScreen();
  if (screen) {
...
void HnsTooltipPopup::Show() {
  HnsTooltipDelegate* delegate = tooltip_->delegate();
  if (delegate) {
    delegate->OnTooltipShow(tooltip_->id());
  }
}

void HnsTooltipPopup::Close(const bool by_user) {
  HnsTooltipDelegate* delegate = tooltip_->delegate();
  if (delegate) {
...
void HnsTooltipPopup::CloseWidget() {
  CloseWidgetView();
}

void HnsTooltipPopup::OnOkButtonPressed() {
...
void HnsTooltipPopup::OnCancelButtonPressed() {
...
void HnsTooltipPopup::set_normalized_display_coordinates(double x, double y) {
  normalized_display_coordinate_x_ = x;
  normalized_display_coordinate_y_ = y;
}

void HnsTooltipPopup::set_display_work_area_insets(int x, int y) {
  display_work_area_inset_x_ = x;
  display_work_area_inset_y_ = y;
}

void HnsTooltipPopup::OnDisplayRemoved(const display::Display& old_display) {
  // Called when |old_display| has been removed
  RecomputeAlignment();
}

void HnsTooltipPopup::OnDisplayMetricsChanged(const display::Display& display,
                                                uint32_t changed_metrics) {
  // Called when the metrics of a display change
  RecomputeAlignment();
}

void HnsTooltipPopup::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kAlertDialog;
  node_data->SetName(l10n_util::GetStringUTF8(
      IDS_HNS_TOOLTIPS_HNS_TOOLTIP_ACCESSIBLE_NAME));
}

void HnsTooltipPopup::OnDisplayChanged() {
  // Called with the display changes (color depth or resolution)
  RecomputeAlignment();
}

void HnsTooltipPopup::OnWorkAreaChanged() {
  // Called when the work area (the desktop area minus task bars, menu bars,
  // etc.) changes in size
  RecomputeAlignment();
}

void HnsTooltipPopup::OnPaintBackground(gfx::Canvas* canvas) {
  DCHECK(canvas);
...
void HnsTooltipPopup::OnThemeChanged() {
  views::View::OnThemeChanged();

  SchedulePaint();
}

void HnsTooltipPopup::OnWidgetCreated(views::Widget* widget) {
  DCHECK(widget);
...
void HnsTooltipPopup::OnWidgetDestroyed(views::Widget* widget) {
  DCHECK(widget);
...
void HnsTooltipPopup::OnWidgetBoundsChanged(views::Widget* widget,
                                              const gfx::Rect& new_bounds) {
  DCHECK(widget);
  widget_origin_ = new_bounds.origin();
}

void HnsTooltipPopup::AnimationEnded(const gfx::Animation* animation) {
  UpdateAnimation();
...
void HnsTooltipPopup::AnimationProgressed(const gfx::Animation* animation) {
  UpdateAnimation();
}

void HnsTooltipPopup::AnimationCanceled(const gfx::Animation* animation) {
  UpdateAnimation();
}

///////////////////////////////////////////////////////////////////////////////

void HnsTooltipPopup::CreatePopup() {
  ...
  tooltip_view_ = container_view->AddChildView(
      new HnsTooltipView(this, tooltip_->attributes()));
...
gfx::Point HnsTooltipPopup::GetDefaultOriginForSize(const gfx::Size& size) {
...
gfx::Rect HnsTooltipPopup::CalculateBounds(bool use_default_origin) {
  DCHECK(tooltip_view_);
...
void HnsTooltipPopup::RecomputeAlignment() {
  if (!IsWidgetValid()) {
    return;
  }
...
const gfx::ShadowDetails& HnsTooltipPopup::GetShadowDetails() const {
  return gfx::ShadowDetails::Get(kShadowElevation, kCornerRadius);
}

gfx::Insets HnsTooltipPopup::GetShadowMargin() const {
  const gfx::ShadowDetails& shadow_details = GetShadowDetails();
...
void HnsTooltipPopup::CreateWidgetView() {
...
void HnsTooltipPopup::CloseWidgetView() {
  if (!GetWidget()) {
    DeleteDelegate();
    return;
  }
...
void HnsTooltipPopup::StartAnimation() {
  animation_->Start();
...
void HnsTooltipPopup::UpdateAnimation() {
...
bool HnsTooltipPopup::IsWidgetValid() const {
  return GetWidget() && !GetWidget()->IsClosed();
}

BEGIN_METADATA(HnsTooltipPopup, views::WidgetDelegateView)
END_METADATA
