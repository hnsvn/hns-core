...
HnsBrowserFrameViewWin::HnsBrowserFrameViewWin(BrowserFrame* frame,
                                                   BrowserView* browser_view)
    : BrowserFrameViewWin(frame, browser_view) {
  frame_graphic_.reset(
      new HnsWindowFrameGraphic(browser_view->browser()->profile()));
  ...
  if (base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs)) {
    ...
    using_vertical_tabs_.Init(
        ...
        base::BindRepeating(
            &HnsBrowserFrameViewWin::OnVerticalTabsPrefsChanged,
...
HnsBrowserFrameViewWin::~HnsBrowserFrameViewWin() = default;

void HnsBrowserFrameViewWin::OnVerticalTabsPrefsChanged() {
  if (auto* widget = GetWidget();
    ...
    // https://github.com/hnsvn/hns-browser/issues/31971
...
void HnsBrowserFrameViewWin::OnPaint(gfx::Canvas* canvas) {
...
int HnsBrowserFrameViewWin::GetTopInset(bool restored) const {
  if (!base::FeatureList::IsEnabled(tabs::features::kHnsVerticalTabs)) {
...
int HnsBrowserFrameViewWin::NonClientHitTest(const gfx::Point& point) {
  ...
  if (auto overridden_result = hns::NonClientHitTest(browser_view(), point);
