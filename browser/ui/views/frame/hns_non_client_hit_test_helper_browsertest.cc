...
using HnsNonClientHitTestHelperBrowserTest = InProcessBrowserTest;

// TODO(sko) It might be good to have resizable area tests. But testing it
// is pretty flaky depending on platforms.
IN_PROC_BROWSER_TEST_F(HnsNonClientHitTestHelperBrowserTest, Toolbar) {
  ...
  for (auto* view : toolbar_container->GetChildrenInZOrder()) {
    ...
    // HnsToolbarView::ViewHierarchyChanged where we set children that way).
