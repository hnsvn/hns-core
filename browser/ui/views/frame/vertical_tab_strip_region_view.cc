...
// the search button. Unfortunately, we can't inherit HnsTabSearchButton
// as NotifyClick() is marked as 'final'.
class ToggleButton : public HnsNewTabButton {
 public:
  METADATA_HEADER(ToggleButton);

  ToggleButton(Button::PressedCallback callback,
               VerticalTabStripRegionView* region_view)
      : HnsNewTabButton(region_view->tab_strip(),
  ...
  // views::HnsNewTabButton:
  void OnThemeChanged() override {
    HnsNewTabButton::OnThemeChanged();
    ...
    auto color = cp->GetColor(kColorHnsVerticalTabHeaderButtonColor);
  ...
  gfx::Insets GetInsets() const override {
    // By pass HnsNewTabButton::GetInsets().
  ...
  void StateChanged(ButtonState old_state) override {
    HnsNewTabButton::StateChanged(old_state);
    ...
    if (GetState() == views::Button::STATE_NORMAL) {
      ...
      // https://github.com/hnsvn/hns-browser/issues/31421
...
// https://github.com/hnsvn/hns-browser/issues/29935
...
class VerticalTabSearchButton : public HnsTabSearchButton {
 public:
  ...
  void UpdateColors() override {
    HnsTabSearchButton::UpdateColors();

    // Override images set from UpdateIcon().
    SetImageModel(views::Button::STATE_NORMAL,
                  ui::ImageModel::FromVectorIcon(
                      kLeoSearchIcon, kColorHnsVerticalTabHeaderButtonColor,
                      /* icon_size= */ 16));
...
BEGIN_METADATA(VerticalTabSearchButton, HnsTabSearchButton)
END_METADATA

class VerticalTabNewTabButton : public HnsNewTabButton {
 public:
  ...
  VerticalTabNewTabButton(TabStrip* tab_strip,
                          ...
                          VerticalTabStripRegionView* region_view)
      : HnsNewTabButton(tab_strip, std::move(callback)),
        region_view_(region_view) {
    ...
    plus_icon_->SetImage(ui::ImageModel::FromVectorIcon(
        kLeoPlusAddIcon, kColorHnsVerticalTabNTBIconColor,
        /* icon_size= */ 16));
  ...
  void OnPaintFill(gfx::Canvas* canvas) const override {
    ...
    flags.setColor(cp->GetColor(kColorHnsVerticalTabSeparator));
  ...
  void OnThemeChanged() override {
    ...
    text_->SetEnabledColor(cp->GetColor(kColorHnsVerticalTabNTBTextColor));
    shortcut_text_->SetEnabledColor(
        cp->GetColor(kColorHnsVerticalTabNTBShortcutTextColor));
  }

  void Layout() override {
    HnsNewTabButton::Layout();
  ...
  gfx::Size CalculatePreferredSize() const override {
    auto size = HnsNewTabButton::CalculatePreferredSize();
...
BEGIN_METADATA(VerticalTabNewTabButton, HnsNewTabButton)
END_METADATA
...
class VerticalTabStripRegionView::HeaderView : public views::View {
 public:
  ...
  HnsTabSearchButton* tab_search_button() { return tab_search_button_; }
 ...
 private:
  ...
  raw_ptr<HnsTabSearchButton> tab_search_button_ = nullptr;
};
...
VerticalTabStripRegionView::VerticalTabStripRegionView(
  ...
  expanded_width_.Init(
      hns_tabs::kVerticalTabsExpandedWidth, prefs,
...
void VerticalTabStripRegionView::OnThemeChanged() {
  ...
  SetBorder(views::CreateSolidSidedBorder(
      ...
      cp->GetColor(kColorHnsVerticalTabSeparator)));
}
...
void VerticalTabStripRegionView::OnBoundsChanged(
  ...
  if (previous_bounds.size() != size()) {
    if (GetAvailableWidthForTabContainer() != tab_strip()->width()) {
      ...
      // force it to layout.
      // https://github.com/hnsvn/hns-browser/issues/29941
      tab_strip()->tab_container_->InvalidateIdealBounds();
      tab_strip()->tab_container_->CompleteAnimationAndLayout();
    }

    // TODO(sko) Remove this once the "sticky pinned tabs" is enabled by
    // default. https://github.com/hnsvn/hns-browser/issues/29935
    if (scroll_view_) {
...
void VerticalTabStripRegionView::ResetExpandedWidth() {
  auto* prefs = browser_->profile()->GetPrefs();
  prefs->ClearPref(hns_tabs::kVerticalTabsExpandedWidth);
...
void VerticalTabStripRegionView::ScrollActiveTabToBeVisible() {
  ...
  if (!contents_view_->Contains(active_tab)) {
    ...
    // https://github.com/hnsvn/hns-browser/issues/32183
    return;
  }
