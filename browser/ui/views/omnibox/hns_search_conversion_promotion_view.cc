...
using hns_search_conversion::SetDismissed;
using hns_search_conversion::SetMaybeLater;
  ...
  void Paint(gfx::Canvas* canvas, views::View* view) const override {
    ...
    const auto* graphic = bundle.GetImageSkiaNamed(
        ui::NativeTheme::GetInstanceForNativeUi()->ShouldUseDarkColors()
            ? IDR_HNS_SEARCH_CONVERSION_BANNER_GRAPHIC_DARK
            : IDR_HNS_SEARCH_CONVERSION_BANNER_GRAPHIC);
...
class CloseButton : public views::ImageButton {
 public:
  ...
  void GetAccessibleNodeData(ui::AXNodeData* node_data) override {
    ...
    node_data->SetName(hns_l10n::GetLocalizedResourceUTF16String(
        IDS_ACC_HNS_SEARCH_CONVERSION_DISMISS_BUTTON));
  }
};
...
class HnsOmniboxResultSelectionIndicator : public views::View {
 public:
  METADATA_HEADER(HnsOmniboxResultSelectionIndicator);

  static constexpr int kStrokeThickness = 3;

  explicit HnsOmniboxResultSelectionIndicator(
      HnsSearchConversionPromotionView* parent_view)
 ...
 private:
  ...
  const raw_ptr<HnsSearchConversionPromotionView> parent_view_;
...
BEGIN_METADATA(HnsOmniboxResultSelectionIndicator, views::View)
END_METADATA

}  // namespace

HnsSearchConversionPromotionView::HnsSearchConversionPromotionView(
    HnsOmniboxResultView* result_view,
    PrefService* local_state,
    PrefService* profile_prefs)
    : result_view_(result_view),
      mouse_enter_exit_handler_(base::BindRepeating(
          &HnsSearchConversionPromotionView::UpdateHoverState,
...
void HnsSearchConversionPromotionView::ResetChildrenVisibility() {
...
void HnsSearchConversionPromotionView::SetTypeAndInput(
    hns_search_conversion::ConversionType type,
  ...
  hns_search_conversion::p3a::RecordPromoShown(local_state_, type);
}

void HnsSearchConversionPromotionView::OnSelectionStateChanged(
    bool selected) {
  selected_ = selected;
  UpdateState();
}

void HnsSearchConversionPromotionView::UpdateState() {
  if (type_ == ConversionType::kButton) {
...
void HnsSearchConversionPromotionView::OpenMatch() {
  hns_search_conversion::p3a::RecordPromoTrigger(local_state_, type_);
  result_view_->OpenMatch();
}

void HnsSearchConversionPromotionView::Dismiss() {
  SetDismissed(profile_prefs_);
  result_view_->RefreshOmniboxResult();
}

void HnsSearchConversionPromotionView::MaybeLater() {
  SetMaybeLater(profile_prefs_);
  result_view_->RefreshOmniboxResult();
}

void HnsSearchConversionPromotionView::UpdateButtonTypeState() {
  if (!button_type_container_)
    return;
...
void HnsSearchConversionPromotionView::UpdateBannerTypeState() {
  if (!banner_type_container_)
    return;
...
void HnsSearchConversionPromotionView::ConfigureForButtonType() {
  if (button_type_container_)
    return;
  ...
  append_for_input_ =
      input_container->AddChildView(std::make_unique<views::Label>(
          hns_l10n::GetLocalizedResourceUTF16String(
              IDS_HNS_SEARCH_CONVERSION_INPUT_APPEND_LABEL),
          normal_font));
  append_for_input_->SetAutoColorReadabilityEnabled(false);
  append_for_input_->SetHorizontalAlignment(gfx::ALIGN_LEFT);

  // Configure description
  const std::u16string first_part = hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_SEARCH_CONVERSION_PROMOTION_DESC_LABEL_FIRST_PART);
  const std::u16string desc_label = hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_SEARCH_CONVERSION_PROMOTION_DESC_LABEL);
  ...
  auto* try_button = button_type_container_->AddChildView(
      std::make_unique<CustomMdTextButton>(views::Button::PressedCallback(
          base::BindRepeating(&HnsSearchConversionPromotionView::OpenMatch,
                              base::Unretained(this)))));
  try_button->SetKind(views::MdTextButton::Kind::kPrimary);
  try_button->SetProperty(views::kMarginsKey, gfx::Insets::VH(17, 0));
  try_button->SetText(hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_SEARCH_CONVERSION_TRY_BUTTON_LABEL));
  ...
  auto* close_button = button_type_container_->AddChildView(
      std::make_unique<CloseButton>(views::Button::PressedCallback(
          base::BindRepeating(&HnsSearchConversionPromotionView::Dismiss,
                              base::Unretained(this)))));
  ...
  close_button->SetTooltipText(hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_SEARCH_CONVERSION_CLOSE_BUTTON_TOOLTIP));
}

void HnsSearchConversionPromotionView::ConfigureForBannerType() {
  if (banner_type_container_)
    return;
  ...
  auto* maybe_later_button = button_row->AddChildView(
      std::make_unique<CustomMdTextButton>(views::Button::PressedCallback(
          base::BindRepeating(&HnsSearchConversionPromotionView::MaybeLater,
                              base::Unretained(this)))));
  ...
  maybe_later_button->SetProperty(
      ...
      views::FlexSpecification(views::MinimumFlexSizeRule::kScaleToZero,
                               views::MaximumFlexSizeRule::kPreferred));
  maybe_later_button->SetText(hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_SEARCH_CONVERSION_MAYBE_LATER_BUTTON_LABEL));
  ...
  maybe_later_button->SetTooltipText(
      hns_l10n::GetLocalizedResourceUTF16String(
          IDS_HNS_SEARCH_CONVERSION_MAYBE_LATER_BUTTON_TOOLTIP));
...
SkColor HnsSearchConversionPromotionView::GetCloseButtonColor() const {
...
HnsSearchConversionPromotionView::GetButtonTypeBackground() {
...
gfx::Size HnsSearchConversionPromotionView::CalculatePreferredSize() const {
...
void HnsSearchConversionPromotionView::OnThemeChanged() {
  View::OnThemeChanged();

  UpdateState();
}

void HnsSearchConversionPromotionView::UpdateHoverState() {
  UpdateState();
}

int HnsSearchConversionPromotionView::GetBannerTypeTitleStringResourceId() {
  switch (type_) {
    case ConversionType::kBannerTypeA:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_A_TITLE;
    case ConversionType::kBannerTypeB:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_B_TITLE;
    case ConversionType::kBannerTypeC:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_C_TITLE;
    case ConversionType::kBannerTypeD:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_D_TITLE;
    default:
      break;
  }
...
int HnsSearchConversionPromotionView::GetBannerTypeDescStringResourceId() {
  switch (type_) {
    case ConversionType::kBannerTypeA:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_A_DESC;
    case ConversionType::kBannerTypeB:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_B_DESC;
    case ConversionType::kBannerTypeC:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_C_DESC;
    case ConversionType::kBannerTypeD:
      return IDS_HNS_SEARCH_CONVERSION_PROMOTION_BANNER_TYPE_D_DESC;
    default:
      break;
  }
...
BEGIN_METADATA(HnsSearchConversionPromotionView, views::View)
END_METADATA
