...
#include "hns/browser/ui/sidebar/sidebar.h"
...
// static
void HnsBrowser::SuppressBrowserWindowClosingDialogForTesting(bool suppress) {
  g_suppress_dialog_for_testing = suppress;
}

HnsBrowser::HnsBrowser(const CreateParams& params) : Browser(params) {
#if defined(TOOLKIT_VIEWS)
  if (!sidebar::CanUseSidebar(this))
    return;
  // Below call order is important.
  // When reaches here, Sidebar UI is setup in HnsBrowserView but
  // not initialized. It's just empty because sidebar controller/model is not
  // ready yet. HnsBrowserView is instantiated by the ctor of Browser.
  ...
  sidebar_controller_->SetSidebar(hns_window()->InitSidebar());
#endif
}

HnsBrowser::~HnsBrowser() = default;

void HnsBrowser::ScheduleUIUpdate(content::WebContents* source,
...
bool HnsBrowser::ShouldDisplayFavicon(
...
void HnsBrowser::OnTabStripModelChanged(
...
void HnsBrowser::FinishWarnBeforeClosing(WarnBeforeClosingResult result) {
...
void HnsBrowser::BeforeUnloadFired(content::WebContents* source,
                                     bool proceed,
...
bool HnsBrowser::TryToCloseWindow(
...
void HnsBrowser::ResetTryToCloseWindow() {
  confirmed_to_close_ = false;
  Browser::ResetTryToCloseWindow();
}

void HnsBrowser::UpdateTargetURL(content::WebContents* source,
                                   const GURL& url) {
  GURL target_url = url;
  if (url.SchemeIs(content::kChromeUIScheme)) {
    GURL::Replacements replacements;
    replacements.SetSchemeStr(content::kHnsUIScheme);
    target_url = target_url.ReplaceComponents(replacements);
  }
  Browser::UpdateTargetURL(source, target_url);
}

bool HnsBrowser::ShouldAskForBrowserClosingBeforeHandlers() {
...
HnsBrowserWindow* HnsBrowser::hns_window() {
  return static_cast<HnsBrowserWindow*>(window_);
}
