...
#include "hns/browser/ui/views/side_panel/hns_side_panel_resize_widget.h"
#include "hns/browser/ui/views/sidebar/sidebar_container_view.h"
#include "hns/browser/ui/views/sidebar/sidebar_control_view.h"
#include "hns/browser/ui/views/sidebar/sidebar_items_contents_view.h"
#include "hns/browser/ui/views/sidebar/sidebar_items_scroll_view.h"
...
class SidebarBrowserTest : public InProcessBrowserTest {
 public:
  ...
  HnsBrowser* hns_browser() const {
    return static_cast<HnsBrowser*>(browser());
  }
  ...
  SidebarController* controller() const {
    return hns_browser()->sidebar_controller();
  }

  views::View* GetVerticalTabsContainer() const {
    auto* view = BrowserView::GetBrowserViewForBrowser(browser());
    return static_cast<HnsBrowserView*>(view)->vertical_tab_strip_host_view_;
  }
  ...
  HnsSidePanel* GetSidePanel() const {
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTest, WebTypePanelTest) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURLWithDisposition(
      browser(), GURL("hns://newtab/"),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP));
  current_tab_index = tab_model()->active_index();
  EXPECT_EQ(1, current_tab_index);

  // Activate sidebar item(hns://settings) and check existing first tab is
  // activated.
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTest, IterateBuiltInWebTypeTest) {
  ...
  static_cast<HnsBrowser*>(browser2)->sidebar_controller()->ActivateItemAt(1);
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTestWithPlaylist, Incognito) {
  ...
  auto sidebar_items_contents_view = GetSidebarItemsContentsView(
      static_cast<HnsBrowser*>(private_browser)->sidebar_controller());
...
class SidebarBrowserTestWithVerticalTabs : public SidebarBrowserTest {
 public:
  SidebarBrowserTestWithVerticalTabs() {
    feature_list_.InitAndEnableFeature(tabs::features::kHnsVerticalTabs);
  }
...
IN_PROC_BROWSER_TEST_F(SidebarBrowserTestWithVerticalTabs,
  ...
  hns::ToggleVerticalTabStrip(browser());
