...
class WindowClosingConfirmBrowserTest : public InProcessBrowserTest,
 ...
 public:
  void SetUpOnMainThread() override {
    HnsBrowser::SuppressBrowserWindowClosingDialogForTesting(false);
  ...
  void SetDownloadConfirmReturn(bool allow) {
    HnsBrowserView::SetDownloadConfirmReturnForTesting(allow);
  }
...
IN_PROC_BROWSER_TEST_F(WindowClosingConfirmBrowserTest, TestWithTwoNTPTabs) {
  ...
  EXPECT_FALSE(hns_browser->ShouldAskForBrowserClosingBeforeHandlers());

  // Two tabs. Need to ask browser closing.
  ui_test_utils::NavigateToURLWithDisposition(
      hns_browser, GURL(url::kAboutBlankURL),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP);
  EXPECT_TRUE(hns_browser->ShouldAskForBrowserClosingBeforeHandlers());
  ...
  chrome::CloseWindow(hns_browser);
  chrome::CloseWindow(hns_browser);
  ui_test_utils::WaitForBrowserToClose(hns_browser);
...
IN_PROC_BROWSER_TEST_F(WindowClosingConfirmBrowserTest,
  ...
  EXPECT_TRUE(static_cast<HnsBrowser*>(browser())
...
IN_PROC_BROWSER_TEST_F(WindowClosingConfirmBrowserTest,
  ...
  ASSERT_NO_FATAL_FAILURE(ASSERT_TRUE(ui_test_utils::NavigateToURL(
      hns_browser, embedded_test_server()->GetURL("/beforeunload.html"))));
  ui_test_utils::NavigateToURLWithDisposition(
      hns_browser, GURL(url::kAboutBlankURL),
      WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP);

  PrepareForBeforeUnloadDialog(hns_browser);
...
// So, test with private profile window on macOS.
#if BUILDFLAG(IS_MAC)
  auto* hns_browser = static_cast<HnsBrowser*>(CreateIncognitoBrowser());
#else
  auto* hns_browser = static_cast<HnsBrowser*>(browser());
#endif
  hns_browser->profile()->GetPrefs()->SetBoolean(prefs::kPromptForDownload,
                                                   false);
  ...
  // Ensure that we have enough disk space to download the large file.
  {
    ...
    int64_t free_space = base::SysInfo::AmountOfFreeDiskSpace(
        GetDownloadDirectory(hns_browser));
  ...
  std::unique_ptr<content::DownloadTestObserver> progress_waiter(
      CreateInProgressWaiter(hns_browser, 1));

  // Start downloading a file, wait for it to be created.
  ui_test_utils::NavigateToURLWithDisposition(
      hns_browser, url, WindowOpenDisposition::CURRENT_TAB,
