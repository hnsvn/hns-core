...
#ifndef HNS_BROWSER_NET_URL_CONTEXT_H_
#define HNS_BROWSER_NET_URL_CONTEXT_H_
...
int OnBeforeURLRequest(const hns::ResponseCallback& next_callback,
                       std::shared_ptr<hns::HnsRequestInfo> ctx);
}  // namespace hns_rewards

namespace hns {

enum HnsNetworkDelegateEventType {
  kOnBeforeRequest,
...
struct HnsRequestInfo {
  HnsRequestInfo();
  HnsRequestInfo(const HnsRequestInfo&) = delete;
  HnsRequestInfo& operator=(const HnsRequestInfo&) = delete;

  // For tests, should not be used directly.
  explicit HnsRequestInfo(const GURL& url);
  ...
  HnsNetworkDelegateEventType event_type = kUnknownEventType;
  ...
  static std::shared_ptr<hns::HnsRequestInfo> MakeCTX(
 ...
 private:
  ...
  friend class ::HnsRequestHandler;
...
#endif  // HNS_BROWSER_NET_URL_CONTEXT_H_
