...
// Test to check if key is added for hns apis
class SystemNetworkContextManagerBrowsertest : public PlatformBrowserTest {
 ...
 private:
  void MonitorResourceRequest(const net::test_server::HttpRequest& request) {
    service_key_present_ = request.headers.count(kHnsServicesKeyHeader) > 0;
  }
...
IN_PROC_BROWSER_TEST_F(SystemNetworkContextManagerBrowsertest,
                       CheckForHnsServiceKey) {
  EXPECT_TRUE(LoadURL("demo.hns.com"));
  EXPECT_TRUE(LoadURL("demo.hnssoftware.com"));
  EXPECT_FALSE(LoadURL("hns.demo.com"));
