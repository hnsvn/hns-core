...
absl::optional<std::string> GetCspDirectivesOnTaskRunner(
  ...
  absl::optional<std::string> csp_directives =
      g_hns_browser_process->ad_block_service()->GetCspDirectives(
          ctx->request_url, ctx->resource_type, source_host);

  hns_shields::MergeCspDirectiveInto(original_csp, &csp_directives);
...
int OnHeadersReceived_AdBlockCspWork(
  ...
  if (!response_headers || !ctx->allow_hns_shields || ctx->allow_ads) {
    return net::OK;
  }
