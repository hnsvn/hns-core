...
#include "hns/browser/net/hns_ad_block_csp_network_delegate_helper.h"
...
#include "hns/browser/net/hns_localhost_permission_network_delegate_helper.h"
#include "hns/browser/net/hns_reduce_language_network_delegate_helper.h"
...
#include "hns/browser/net/hns_stp_util.h"
...
#include "hns/browser/net/hns_torrent_redirect_network_delegate_helper.h"
#endif

#if BUILDFLAG(ENABLE_IPFS)
#include "hns/browser/net/ipfs_redirect_network_delegate_helper.h"
#include "hns/components/ipfs/features.h"
#endif

static bool IsInternalScheme(std::shared_ptr<hns::HnsRequestInfo> ctx) {
  DCHECK(ctx);
...
HnsRequestHandler::HnsRequestHandler() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  SetupCallbacks();
}

HnsRequestHandler::~HnsRequestHandler() = default;

void HnsRequestHandler::SetupCallbacks() {
  hns::OnBeforeURLRequestCallback callback =
      base::BindRepeating(hns::OnBeforeURLRequest_SiteHacksWork);
  before_url_request_callbacks_.push_back(callback);

  callback = base::BindRepeating(hns::OnBeforeURLRequest_AdBlockTPPreWork);
  before_url_request_callbacks_.push_back(callback);

  if (!base::FeatureList::IsEnabled(net::features::kHnsHttpsByDefault)) {
    callback = base::BindRepeating(hns::OnBeforeURLRequest_HttpsePreFileWork);
    before_url_request_callbacks_.push_back(callback);
  }

  callback =
      base::BindRepeating(hns::OnBeforeURLRequest_CommonStaticRedirectWork);
  before_url_request_callbacks_.push_back(callback);
  ...
  if (base::FeatureList::IsEnabled(
    ...
    callback =
        base::BindRepeating(hns::OnBeforeURLRequest_LocalhostPermissionWork);
    before_url_request_callbacks_.push_back(callback);
  }
  ...
  hns::OnBeforeStartTransactionCallback start_transaction_callback =
      base::BindRepeating(hns::OnBeforeStartTransaction_SiteHacksWork);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

  if (base::FeatureList::IsEnabled(
          blink::features::kHnsGlobalPrivacyControl)) {
    start_transaction_callback = base::BindRepeating(
        hns::OnBeforeStartTransaction_GlobalPrivacyControlWork);
    before_start_transaction_callbacks_.push_back(start_transaction_callback);
  }

  start_transaction_callback =
      base::BindRepeating(hns::OnBeforeStartTransaction_HnsServiceKey);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

  start_transaction_callback =
      base::BindRepeating(hns::OnBeforeStartTransaction_ReferralsWork);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

  if (base::FeatureList::IsEnabled(
          hns_shields::features::kHnsReduceLanguage)) {
    start_transaction_callback =
        base::BindRepeating(hns::OnBeforeStartTransaction_ReduceLanguageWork);
    before_start_transaction_callbacks_.push_back(start_transaction_callback);
  }

  start_transaction_callback =
      base::BindRepeating(hns::OnBeforeStartTransaction_AdsStatusHeader);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

#if BUILDFLAG(ENABLE_HNS_WEBTORRENT)
  hns::OnHeadersReceivedCallback headers_received_callback =
  ...
  if (base::FeatureList::IsEnabled(
          ::hns_shields::features::kHnsAdblockCspRules)) {
    hns::OnHeadersReceivedCallback headers_received_callback2 =
        base::BindRepeating(hns::OnHeadersReceived_AdBlockCspWork);
    headers_received_callbacks_.push_back(headers_received_callback2);
  }
}

bool HnsRequestHandler::IsRequestIdentifierValid(
    uint64_t request_identifier) {
  return base::Contains(callbacks_, request_identifier);
}

int HnsRequestHandler::OnBeforeURLRequest(
  ...
  ctx->new_url = new_url;
  ctx->event_type = hns::kOnBeforeRequest;
  callbacks_[ctx->request_identifier] = std::move(callback);
  RunNextCallback(ctx);
  return net::ERR_IO_PENDING;
}

int HnsRequestHandler::OnBeforeStartTransaction(
    std::shared_ptr<hns::HnsRequestInfo> ctx,
    net::CompletionOnceCallback callback,
  ...
  ctx->event_type = hns::kOnBeforeStartTransaction;
  ctx->headers = headers;
  callbacks_[ctx->request_identifier] = std::move(callback);
  RunNextCallback(ctx);
  return net::ERR_IO_PENDING;
...
int HnsRequestHandler::OnHeadersReceived(
    std::shared_ptr<hns::HnsRequestInfo> ctx,
    net::CompletionOnceCallback callback,
  ...
  if (!ctx->tab_origin.is_empty()) {
    hns::RemoveTrackableSecurityHeadersForThirdParty(
  ...
  if (headers_received_callbacks_.empty() &&
    ...
    // Extension scheme not excluded since hns_webtorrent needs it.
    return net::OK;
  }

  callbacks_[ctx->request_identifier] = std::move(callback);
  ctx->event_type = hns::kOnHeadersReceived;
...
void HnsRequestHandler::OnURLRequestDestroyed(
    std::shared_ptr<hns::HnsRequestInfo> ctx) {
...
void HnsRequestHandler::RunCallbackForRequestIdentifier(
    uint64_t request_identifier,
    int rv) {
...
void HnsRequestHandler::RunNextCallback(
    std::shared_ptr<hns::HnsRequestInfo> ctx) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  ...
  if (ctx->event_type == hns::kOnBeforeRequest) {
    while (before_url_request_callbacks_.size() !=
      ...
      hns::ResponseCallback next_callback =
          base::BindRepeating(&HnsRequestHandler::RunNextCallback,
  ...
  } else if (ctx->event_type == hns::kOnBeforeStartTransaction) {
    while (before_start_transaction_callbacks_.size() !=
           ctx->next_url_request_index) {
      hns::OnBeforeStartTransactionCallback callback =
  ...
  } else if (ctx->event_type == hns::kOnHeadersReceived) {
    while (headers_received_callbacks_.size() != ctx->next_url_request_index) {
      hns::OnHeadersReceivedCallback callback =
  ...
  if (ctx->event_type == hns::kOnBeforeRequest) {
    ...
    if (ctx->blocked_by == hns::kAdBlocked ||
        ctx->blocked_by == hns::kOtherBlocked) {
