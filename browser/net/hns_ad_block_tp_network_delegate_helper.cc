...
class AdblockCnameResolveHostClient : public network::mojom::ResolveHostClient {
 ...
 public:
  ...
  void OnComplete(int32_t result,
    ...
    UMA_HISTOGRAM_TIMES("Hns.ShieldsCNAMEBlocking.TotalResolutionTime",
...
EngineFlags ShouldBlockRequestOnTaskRunner(
  ...
  SCOPED_UMA_HISTOGRAM_TIMER("Hns.Adblock.ShouldBlockRequest");
  auto adblock_result =
      g_hns_browser_process->ad_block_service()->ShouldStartRequest(
...
void OnShouldBlockRequestResult(
  ...
  if (ctx->blocked_by == kAdBlocked) {
    hns_shields::HnsShieldsWebContentsObserver::DispatchBlockedEvent(
        ctx->request_url, ctx->frame_tree_node_id, hns_shields::kAds);
...
bool ProxySettingsAllowUncloaking(content::BrowserContext* browser_context,
  ...
  auto* ad_block_pref_service =
      hns_shields::AdBlockPrefServiceFactory::GetForBrowserContext(
          browser_context);
...
int OnBeforeURLRequest_AdBlockTPPreWork(const ResponseCallback& next_callback,
  ...
  if (ctx->request_url.is_empty() ||
      ...
      !ctx->allow_hns_shields || ctx->allow_ads ||
      ctx->resource_type == HnsRequestInfo::kInvalidResourceType) {
    return net::OK;
  }
  ...
  // https://github.com/hnsvn/hns-browser/issues/26302).
