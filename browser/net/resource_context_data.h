...
#ifndef HNS_BROWSER_NET_RESOURCE_CONTEXT_DATA_H_
#define HNS_BROWSER_NET_RESOURCE_CONTEXT_DATA_H_
...
class ResourceContextData : public base::SupportsUserData::Data {
 public:
  ...
  static HnsProxyingWebSocket* StartProxyingWebSocket(
  ...
  void RemoveProxy(HnsProxyingURLLoaderFactory* proxy);
  void RemoveProxyWebSocket(HnsProxyingWebSocket* proxy);

 private:
  ResourceContextData();

  std::unique_ptr<HnsRequestHandler> request_handler_;
  scoped_refptr<RequestIDGenerator> request_id_generator_;

  std::set<std::unique_ptr<HnsProxyingURLLoaderFactory>,
           base::UniquePtrComparator>
      proxies_;

  std::set<std::unique_ptr<HnsProxyingWebSocket>,
           base::UniquePtrComparator>
      websocket_proxies_;
...
#endif  // HNS_BROWSER_NET_RESOURCE_CONTEXT_DATA_H_
