...
static constexpr auto kFarbleAcceptLanguageExceptions =
    base::MakeFixedFlatSet<base::StringPiece>(
        {// https://github.com/hnsvn/hns-browser/issues/25309
         "ulta.com", "www.ulta.com",
         // https://github.com/hnsvn/hns-browser/issues/26325
         "aeroplan.rewardops.com",
         // https://github.com/hnsvn/hns-browser/issues/31196
         "login.live.com"});
}  // namespace
...
std::string FarbleAcceptLanguageHeader(
  ...
  if (g_hns_browser_process->hns_farbling_service()
...
int OnBeforeStartTransaction_ReduceLanguageWork(
  ...
  if (!hns_shields::ShouldDoReduceLanguage(content_settings, origin_url,
                                             profile->GetPrefs())) {
    return net::OK;
  }
  ...
  if (headers->HasHeader(net::HttpRequestHeaders::kAcceptLanguage)) {
    ...
    // https://github.com/hnsvn/hns-browser/issues/28945
    return net::OK;
  }

  std::string accept_language_string;
  switch (hns_shields::GetFingerprintingControlType(content_settings,
                                                      origin_url)) {
