...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIHTTPScheme) {
  ...
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(hns::ResponseCallback(),
                                                     hns_request_info);
  EXPECT_EQ(rc, net::OK);
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIIPFSSchemeLocal) {
  ...
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->browser_context = profile();
  hns_request_info->ipfs_gateway_url = GetLocalGateway();
  hns_request_info->initiator_url = ipfs::GetIPFSGatewayURL(
      initiator_cid, "",
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest,
  ...
  GURL url("ipfs://QmfM2r8seH2GiRaC4esTjeraXEachRt8ZsSeGaWTPLyMoG");
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->resource_type = blink::mojom::ResourceType::kMainFrame;
  hns_request_info->browser_context = profile();
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(hns::ResponseCallback(),
                                                     hns_request_info);
  EXPECT_EQ(rc, net::ERR_IPFS_DISABLED);
  EXPECT_EQ(hns_request_info->blocked_by, hns::kOtherBlocked);
}
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest,
  ...
  GURL url("ipfs://QmfM2r8seH2GiRaC4esTjeraXEachRt8ZsSeGaWTPLyMoG");
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->resource_type = blink::mojom::ResourceType::kSubFrame;
  hns_request_info->browser_context = profile();
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(hns::ResponseCallback(),
                                                     hns_request_info);
  EXPECT_EQ(rc, net::OK);
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest,
  ...
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->resource_type = blink::mojom::ResourceType::kMainFrame;
  hns_request_info->browser_context = profile()->GetOffTheRecordProfile(
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest,
  ...
  // IPFS Subframe
  {
    ...
    int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(
        hns::ResponseCallback(), hns_request_info);
    EXPECT_EQ(rc, net::OK);
    ASSERT_EQ(hns_request_info->blocked_by, hns::kOtherBlocked);
  }
  ...
  // IPFS Mainframe
  {
    ...
    ASSERT_EQ(hns_request_info->blocked_by, hns::kNotBlocked);
  }
}
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest,
  ...
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->resource_type = blink::mojom::ResourceType::kSubFrame;
  hns_request_info->browser_context = nullptr;
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(hns::ResponseCallback(),
                                                     hns_request_info);
  EXPECT_EQ(rc, net::OK);
  EXPECT_EQ(hns_request_info->blocked_by, hns::kOtherBlocked);
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIIPFSScheme) {
  ...
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->browser_context = profile();
  hns_request_info->ipfs_gateway_url = GetPublicGateway();
  hns_request_info->initiator_url = ipfs::GetIPFSGatewayURL(
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, PrivateProfile) {
  GURL url("ipfs://QmfM2r8seH2GiRaC4esTjeraXEachRt8ZsSeGaWTPLyMoG");
  auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
  hns_request_info->browser_context = profile()->GetPrimaryOTRProfile(true);
  hns_request_info->ipfs_gateway_url = GetPublicGateway();
  hns_request_info->resource_type = blink::mojom::ResourceType::kMainFrame;
  hns_request_info->initiator_url = ipfs::GetIPFSGatewayURL(
      initiator_cid, "", ipfs::GetDefaultIPFSGateway(profile()->GetPrefs()));
