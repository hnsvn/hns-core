...
#include "hns/browser/safebrowsing/buildflags.h"
...
int OnBeforeURLRequest_StaticRedirectWorkForGURL(
  ...
  // https://github.com/hnsvn/hns-browser/issues/10314
  ...
  if (!safebrowsing_endpoint.empty() &&
    ...
    replacements.SetHostStr(kHnsSafeBrowsingSslProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (!safebrowsing_endpoint.empty() &&
    ...
    replacements.SetHostStr(kHnsSafeBrowsing2Proxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (crxDownload_pattern.MatchesURL(request_url)) {
    ...
    replacements.SetHostStr("crxdownload.hns.com");
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (autofill_pattern.MatchesURL(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kHnsStaticProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (crlSet_pattern1.MatchesURL(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr("redirector.hns.com");
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (gvt1_pattern.MatchesURL(request_url) &&
    ...
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kHnsRedirectorProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
