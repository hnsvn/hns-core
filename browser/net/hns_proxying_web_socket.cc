...
HnsProxyingWebSocket::HnsProxyingWebSocket(
...
// static
HnsProxyingWebSocket* HnsProxyingWebSocket::ProxyWebSocket(
...
void HnsProxyingWebSocket::Start() {
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &HnsProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &HnsProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }

  ctx_ = hns::HnsRequestInfo::MakeCTX(request_, process_id_,
...
HnsProxyingWebSocket::web_socket_factory() {
  return base::BindOnce(&HnsProxyingWebSocket::WebSocketFactoryRun,
                        base::Unretained(this));
}

mojo::Remote<network::mojom::WebSocketHandshakeClient>
HnsProxyingWebSocket::handshake_client() {
  return std::move(forwarding_handshake_client_);
}

bool HnsProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void HnsProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void HnsProxyingWebSocket::OnOpeningHandshakeStarted(
...
void HnsProxyingWebSocket::ContinueToHeadersReceived() {
  auto continuation = base::BindRepeating(
      &HnsProxyingWebSocket::OnHeadersReceivedComplete,
      weak_factory_.GetWeakPtr());
  ctx_ = hns::HnsRequestInfo::MakeCTX(request_, process_id_,
                                          frame_tree_node_id_, request_id_,
                                          browser_context_, ctx_);
...
void HnsProxyingWebSocket::OnFailure(const std::string& message,
                                       int32_t net_error,
                                       int32_t response_code) {}

void HnsProxyingWebSocket::OnConnectionEstablished(
...
void HnsProxyingWebSocket::OnAuthRequired(
...
void HnsProxyingWebSocket::OnBeforeSendHeaders(
...
void HnsProxyingWebSocket::OnHeadersReceived(
...
void HnsProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &HnsProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void HnsProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
  ...
  auto continuation = base::BindRepeating(
      &HnsProxyingWebSocket::OnBeforeSendHeadersComplete,
      weak_factory_.GetWeakPtr());
...
void HnsProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void HnsProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  receiver_as_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&HnsProxyingWebSocket::OnMojoConnectionError,
                     base::Unretained(this)));
}

void HnsProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void HnsProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &HnsProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void HnsProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void HnsProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void HnsProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void HnsProxyingWebSocket::OnMojoConnectionError(
