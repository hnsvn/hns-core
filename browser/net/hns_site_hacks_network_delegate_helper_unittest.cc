...
TEST(HnsSiteHacksNetworkDelegateHelperTest, UANotAllowedTest) {
  const std::vector<const GURL> urls(
      {GURL("https://brianbondy.com"), GURL("https://hnscombo.com"),
       GURL("https://hns.example.com"),
  ...
  for (const auto& url : urls) {
    ...
    int rc = hns::OnBeforeStartTransaction_SiteHacksWork(
        &headers, ResponseCallback(), hns_request_info);
...
TEST(HnsSiteHacksNetworkDelegateHelperTest, ReferrerPreserved) {
  const std::vector<const GURL> urls(
  ...
  for (const auto& url : urls) {
    ...
    auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(url);
    hns_request_info->referrer = original_referrer;
    int rc = hns::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     hns_request_info);
    EXPECT_EQ(rc, net::OK);
    // new_url should not be set.
    EXPECT_TRUE(hns_request_info->new_url_spec.empty());
    EXPECT_EQ(hns_request_info->referrer, original_referrer);
  }
}

TEST(HnsSiteHacksNetworkDelegateHelperTest, ReferrerTruncated) {
  ...
  for (const auto& url : urls) {
    ...
    // new_url should not be set.
    EXPECT_TRUE(hns_request_info->new_url_spec.empty());
    EXPECT_TRUE(hns_request_info->new_referrer.has_value());
    EXPECT_EQ(hns_request_info->new_referrer.value(),
              url::Origin::Create(original_referrer).GetURL());
  }
}

TEST(HnsSiteHacksNetworkDelegateHelperTest,
...
TEST(HnsSiteHacksNetworkDelegateHelperTest, OnionReferrerStripped) {
  const GURL original_referrer(
      "https://"
      "hns4u7jddbv7cyviptqjc7jusxh72uik7zt6adtckl5f4nwy2v72qd.onion/");
  const GURL destination("https://hns.com");
...
TEST(HnsSiteHacksNetworkDelegateHelperTest, QueryStringUntouched) {
  ...
  for (const auto& url : urls) {
    auto hns_request_info =
        std::make_shared<hns::HnsRequestInfo>(GURL(url));
    hns_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    hns_request_info->method = "GET";
    int rc = hns::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     hns_request_info);
    EXPECT_EQ(rc, net::OK);
    // new_url should not be set
...
TEST(HnsSiteHacksNetworkDelegateHelperTest, QueryStringExempted) {
  ...
  for (const auto& initiator : initiators) {
    auto hns_request_info =
        std::make_shared<hns::HnsRequestInfo>(tracking_url);
    hns_request_info->initiator_url = GURL(initiator);
    hns_request_info->method = "GET";
    int rc = hns::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     hns_request_info);
    EXPECT_EQ(rc, net::OK);
  ...
  // Internal redirect
  {
    ...
    hns_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    hns_request_info->method = "GET";
    hns_request_info->internal_redirect = true;
    hns_request_info->redirect_source =
...
TEST(HnsSiteHacksNetworkDelegateHelperTest, QueryStringFiltered) {
  ...
  for (const auto& pair : urls) {
    auto hns_request_info =
        std::make_shared<hns::HnsRequestInfo>(GURL(pair.first));
    hns_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    hns_request_info->method = "GET";
    int rc = hns::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     hns_request_info);
    EXPECT_EQ(rc, net::OK);
    EXPECT_EQ(hns_request_info->new_url_spec, pair.second);
  }

  // Cross-site redirect
  {
    auto hns_request_info = std::make_shared<hns::HnsRequestInfo>(
    ...
    int rc = hns::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     hns_request_info);
    EXPECT_EQ(rc, net::OK);
    EXPECT_EQ(hns_request_info->new_url_spec, "https://example.com/");
  }
  ...
  // Direct navigation
  {
    ...
    hns_request_info->initiator_url = GURL();
    hns_request_info->method = "GET";
    int rc = hns::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     hns_request_info);
    EXPECT_EQ(rc, net::OK);
