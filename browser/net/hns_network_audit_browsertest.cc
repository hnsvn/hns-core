...
#include "hns/browser/net/hns_network_audit_allowed_lists.h"
...
// that the value passed to --ui-test-action-timeout in //hns/package.json, as
...
bool PerformNetworkAuditProcess(base::Value::List* events) {
  ...
  events->EraseIf([&failed](base::Value& event_value) {
    ...
    if (!url.is_valid()) {
      ...
      // dev-only environments (e.g. building with hns_stats_updater_url="").
      return true;
    }
...
class HnsNetworkAuditTest : public InProcessBrowserTest {
 public:
  HnsNetworkAuditTest() {
#if BUILDFLAG(ENABLE_PLAYLIST_WEBUI)
  ...
  HnsNetworkAuditTest(const HnsNetworkAuditTest&) = delete;
  HnsNetworkAuditTest& operator=(const HnsNetworkAuditTest&) = delete;
  ...
  void SetUpOnMainThread() override {
    ...
    rewards_service_ = static_cast<hns_rewards::RewardsServiceImpl*>(
  ...
  bool EnableHnsRewards() {
    PrefService* pref_service = profile()->GetPrefs();
    pref_service->SetInteger("hns.rewards.version", 7);
    pref_service->SetBoolean("hns.rewards.enabled", true);
    return pref_service->GetBoolean("hns.rewards.enabled");
  }
 ...
 private:
  ...
  raw_ptr<hns_rewards::RewardsServiceImpl> rewards_service_ = nullptr;
...
// Loads hns://welcome first to simulate a first run and then loads another
// URL, and finally enables hns rewards, waiting some time after each load to
// allow gathering network requests.
IN_PROC_BROWSER_TEST_F(HnsNetworkAuditTest, BasicTests) {
  // Load the Welcome page.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("hns://welcome")));
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("hns://newtab")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
  ...
  // Finally, load hns://rewards and enable Hns Rewards.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("hns://rewards")));
  ASSERT_TRUE(EnableHnsRewards());
  WaitForTimeout(kMaxTimeoutPerLoadedURL);

  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("hns://wallet")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
