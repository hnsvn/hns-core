...
HnsRequestInfo::HnsRequestInfo() = default;

HnsRequestInfo::HnsRequestInfo(const GURL& url) : request_url(url) {}

HnsRequestInfo::~HnsRequestInfo() = default;

// static
std::shared_ptr<hns::HnsRequestInfo> HnsRequestInfo::MakeCTX(
    const network::ResourceRequest& request,
    int render_process_id,
    int frame_tree_node_id,
    uint64_t request_identifier,
    content::BrowserContext* browser_context,
    std::shared_ptr<hns::HnsRequestInfo> old_ctx) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  auto ctx = std::make_shared<hns::HnsRequestInfo>();
  ...
  ctx->allow_hns_shields =
      map ? hns_shields::GetHnsShieldsEnabled(map, ctx->tab_origin) : true;
  ctx->allow_ads =
      map ? hns_shields::GetAdControlType(map, ctx->tab_origin) ==
                hns_shields::ControlType::ALLOW
          : false;
  ...
  ctx->aggressive_blocking =
      map ? hns_shields::GetCosmeticFilteringControlType(
                map, ctx->tab_origin) == hns_shields::ControlType::BLOCK
          : false;
  ctx->allow_http_upgradable_resource =
      map ? !hns_shields::GetHTTPSEverywhereEnabled(map, ctx->tab_origin)
          : false;

  // HACK: after we fix multiple creations of HnsRequestInfo we should
  // use only tab_origin. Since we recreate HnsRequestInfo during consequent
  ...
  ctx->allow_referrers =
      map ? hns_shields::AreReferrersAllowed(
  ...
  // hns_proxying_url_loader_factory.cc is refactored. See
  // HnsProxyingURLLoaderFactory::InProgressRequest::UpdateRequestInfo().
  if (old_ctx) {
    ctx->internal_redirect = old_ctx->internal_redirect;
    ctx->redirect_source = old_ctx->redirect_source;
  }
