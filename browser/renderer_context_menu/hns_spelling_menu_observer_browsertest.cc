...
// accesses resources.
class HnsSpellingMenuObserverTest : public InProcessBrowserTest {
 public:
  HnsSpellingMenuObserverTest();
  HnsSpellingMenuObserverTest(const HnsSpellingMenuObserverTest&) = delete;
  HnsSpellingMenuObserverTest& operator=(
      const HnsSpellingMenuObserverTest&) = delete;
  ...
  void Reset(bool incognito = false) {
    observer_.reset();
    menu_ = std::make_unique<HnsMockRenderViewContextMenu>(
  ...
  ~HnsSpellingMenuObserverTest() override;
...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(USE_BROWSER_SPELLCHECKER)
HnsSpellingMenuObserverTest::HnsSpellingMenuObserverTest() {
  feature_list_.InitAndDisableFeature(
      spellcheck::kWinDelaySpellcheckServiceInit);
}
#else
HnsSpellingMenuObserverTest::HnsSpellingMenuObserverTest() = default;
#endif
...
// Tests that right-clicking not add "Ask Hns for suggestions".
IN_PROC_BROWSER_TEST_F(HnsSpellingMenuObserverTest,
                       CheckAskHnsNotShown) {
