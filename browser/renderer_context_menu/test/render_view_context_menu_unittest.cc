...
class HnsRenderViewContextMenuMock : public HnsRenderViewContextMenu {
  using HnsRenderViewContextMenu::HnsRenderViewContextMenu;
  void Show() override {}
};

class HnsRenderViewContextMenuTest : public testing::Test {
 protected:
  HnsRenderViewContextMenuTest()
      : testing_local_state_(TestingBrowserProcess::GetGlobal()) {}
  content::WebContents* GetWebContents() { return web_contents_.get(); }

  // Returns a test context menu.
  std::unique_ptr<HnsRenderViewContextMenuMock> CreateContextMenu(
      content::WebContents* web_contents,
      content::ContextMenuParams params) {
    auto menu = std::make_unique<HnsRenderViewContextMenuMock>(
...
TEST_F(HnsRenderViewContextMenuTest, MenuForPlainText) {
...
TEST_F(HnsRenderViewContextMenuTest, MenuForSelectedUrl) {
  content::ContextMenuParams params = CreateSelectedTextParams(u"hns.com");
  auto context_menu = CreateContextMenu(GetWebContents(), params);
  EXPECT_TRUE(context_menu);
  absl::optional<size_t> clean_link_index =
      context_menu->menu_model().GetIndexOfCommandId(IDC_COPY_CLEAN_LINK);
...
TEST_F(HnsRenderViewContextMenuTest, MenuForLink) {
  content::ContextMenuParams params =
      CreateLinkParams(GURL("https://hns.com"));
  auto context_menu = CreateContextMenu(GetWebContents(), params);
  EXPECT_TRUE(context_menu);
  absl::optional<size_t> clean_link_index =
      context_menu->menu_model().GetIndexOfCommandId(IDC_COPY_CLEAN_LINK);
