...
// accesses resources.
class HnsSpellingOptionsSubMenuObserverTest : public InProcessBrowserTest {
 public:
  HnsSpellingOptionsSubMenuObserverTest() {}
  HnsSpellingOptionsSubMenuObserverTest(
      const HnsSpellingOptionsSubMenuObserverTest&) = delete;
  HnsSpellingOptionsSubMenuObserverTest& operator=(
      const HnsSpellingOptionsSubMenuObserverTest&) = delete;
  ~HnsSpellingOptionsSubMenuObserverTest() override {}
  ...
  void Reset(bool incognito = false,
             HnsSpellingOptionsSubMenuObserver::GtestMode gtest_mode =
                 HnsSpellingOptionsSubMenuObserver::GTEST_MODE_NORMAL) {
    Clear();
    menu_.reset(new HnsMockRenderViewContextMenu(
        incognito ? browser()->profile()->GetPrimaryOTRProfile(
                        /*create_if_needed=*/true)
                  : browser()->profile()));
    std::unique_ptr<HnsSpellingOptionsSubMenuObserver> observer =
        std::make_unique<HnsSpellingOptionsSubMenuObserver>(menu_.get(),
                                                              menu_.get(), 1);
  ...
  void CheckExpected() {
    for (size_t i = 0; i < menu()->GetMenuSize(); ++i) {
      HnsMockRenderViewContextMenu::MockMenuItem item;
  ...
  HnsMockRenderViewContextMenu* menu() { return menu_.get(); }
  SpellingOptionsSubMenuObserver* observer() { return observer_.get(); }

 private:
  std::unique_ptr<HnsMockRenderViewContextMenu> menu_;
  std::unique_ptr<SpellingOptionsSubMenuObserver> observer_;
};

// Tests that "Ask Hns for suggestions" isn't shown in the menu and the menu
// doesn't end with a separator.

IN_PROC_BROWSER_TEST_F(HnsSpellingOptionsSubMenuObserverTest,
                       CheckAskHnsNotShown) {
  ...
  // Test empty submenu.
  Reset(false, HnsSpellingOptionsSubMenuObserver::GTEST_MODE_EMPTY_SUBMENU);
  InitMenu(false, "en-US", std::vector<std::string>());
