...
class HnsThemeServiceTest : public InProcessBrowserTest {
 public:
  HnsThemeServiceTest() { hns::RegisterPathProvider(); }
  ~HnsThemeServiceTest() override = default;
  ...
  base::FilePath GetTestDataDir() {
    ...
    CHECK(base::PathService::Get(hns::DIR_TEST_DATA, &test_data_dir));
...
class HnsThemeServiceTestWithoutSystemTheme : public InProcessBrowserTest {
 public:
  HnsThemeServiceTestWithoutSystemTheme() {
    dark_mode::SetUseSystemDarkModeEnabledForTest(false);
  }
};

IN_PROC_BROWSER_TEST_F(HnsThemeServiceTestWithoutSystemTheme,
                       HnsThemeChangeTest) {
...
// Test whether appropriate native/web theme observer is called when hns theme
// is changed.
IN_PROC_BROWSER_TEST_F(HnsThemeServiceTest, ThemeObserverTest) {
...
IN_PROC_BROWSER_TEST_F(HnsThemeServiceTest, SystemThemeChangeTest) {
  ...
  if (dark_mode::SystemDarkModeEnabled()) {
    dark_mode::SetHnsDarkModeType(
        dark_mode::HnsDarkModeType::HNS_DARK_MODE_TYPE_DEFAULT);
    EXPECT_EQ(initial_mode,
              ui::NativeTheme::GetInstanceForNativeUi()->ShouldUseDarkColors());
  }
}

IN_PROC_BROWSER_TEST_F(HnsThemeServiceTest, OmniboxColorTest) {
...
IN_PROC_BROWSER_TEST_F(HnsThemeServiceTest, ColorProviderTest) {
  auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
...
#endif
IN_PROC_BROWSER_TEST_F(HnsThemeServiceTest, MAYBE_DarkModeChangeByRegTest) {
