...
import {DefaultHnsShieldsBrowserProxyImpl, DefaultHnsShieldsBrowserProxyImpl} from './default_hns_shields_browser_proxy.js';
import {getTemplate} from './default_hns_shields_page.html.js'

const HnsShieldsPageBase = WebUiListenerMixin(I18nMixin(PrefsMixin(RouteObserverMixin(PolymerElement))))

/**
* 'settings-default-hns-shields-page' is the settings page containing hns's
* default shields.
*/
class HnsShieldsPage extends HnsShieldsPageBase {
  static get is () {
    return 'settings-default-hns-shields-page'
  }
  ...
  static get properties () {
    return {
      ...
      isHnsRewardsSupported_: {
        readOnly: true,
        type: Boolean,
        value: function () {
          return loadTimeData.getBoolean('isHnsRewardsSupported')
        }
      },
  ...
  browserProxy_ = DefaultHnsShieldsBrowserProxyImpl.getInstance()
  ...
  ready () {
    ...
    this.addWebUiListener('hns-shields-settings-changed',
      () => { this.onShieldsSettingsChanged_() })
  }
...
customElements.define(
  HnsShieldsPage.is, HnsShieldsPage)
