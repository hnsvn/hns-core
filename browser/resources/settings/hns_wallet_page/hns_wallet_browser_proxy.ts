...
export interface HnsWalletBrowserProxy {
  setHnsWalletEnabled(value: boolean): void
...
export class HnsWalletBrowserProxyImpl implements HnsWalletBrowserProxy {
  resetWallet () {
  ...
  setHnsWalletEnabled (value: boolean) {
    chrome.send('setHnsWalletEnabled', [value])
  }
  ...
  getWeb3ProviderList () {
    return new Promise<string>(resolve => chrome.hnsWallet.getWeb3ProviderList(resolve))
  }

  isNativeWalletEnabled() {
    return new Promise<boolean>(resolve => chrome.hnsWallet.isNativeWalletEnabled(resolve))
  }
  ...
  static getInstance() {
    return instance || (instance = new HnsWalletBrowserProxyImpl())
  }
}

let instance: HnsWalletBrowserProxy|null = null
