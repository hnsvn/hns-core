...
import './hns_sync_code_dialog.js';
import './hns_sync_delete_account_dialog.js';
...
import {HnsDeviceInfo, HnsSyncBrowserProxy} from './hns_sync_browser_proxy.js';
import {getTemplate} from './hns_sync_configure.html.js'

/**
 * @fileoverview
 * 'settings-hns-sync-configure' is the set of controls which fetches, displays
 * and updates the sync configuration.
 */

const SettingsHnsSyncConfigureElementBase =
...
export class SettingsHnsSyncConfigureElement extends SettingsHnsSyncConfigureElementBase {
  static get is() {
    return 'settings-hns-sync-configure'
  }
  ...
  private deviceList_: HnsDeviceInfo[];
  private syncCodeDialogType_: string | null;
  private syncDoingDeleteAccount_: Boolean | false;

  browserProxy_: HnsSyncBrowserProxy = HnsSyncBrowserProxy.getInstance();
  ...
  handleDeviceInfo_(deviceList: HnsDeviceInfo[]) {
    this.deviceList_ = deviceList
  }

  getDeviceDisplayDate(device: HnsDeviceInfo): string {
  ...
  async onResetSyncChain_() {
    const messageText = this.i18n('hnsSyncResetConfirmation')
    ...
    router.navigateTo((router.getRoutes() as {HNS_SYNC: Route}).HNS_SYNC);
  }
  ...
  async onDeleteDevice_(e: DomRepeatEvent<HnsDeviceInfo>) {
    const messageText = this.i18n('hnsSyncDeleteDeviceConfirmation')
    const shouldDeleteDevice = confirm(messageText)
    if (!shouldDeleteDevice) {
      return
    }
    const device: HnsDeviceInfo = e.model.item
    await this.browserProxy_.deleteDevice(device.guid);
  }
}

customElements.define(
  SettingsHnsSyncConfigureElement.is, SettingsHnsSyncConfigureElement)
