...
import './hns_sync_configure.js';
import './hns_sync_setup.js';
...
import {HnsSyncBrowserProxy} from './hns_sync_browser_proxy.js';
import {getTemplate} from './hns_sync_subpage.html.js'

const SettingHnsSyncSubpageBase = I18nMixin(RouteObserverMixin(PolymerElement))

class SettingHnsSyncSubpage extends SettingHnsSyncSubpageBase {
  static get is() {
    return 'settings-hns-sync-subpage'
  }
  ...
  hnsSyncBrowserProxy_ = HnsSyncBrowserProxy.getInstance();
  ...
  */
  ...
  attached() {
    ...
    if (router.getCurrentRoute() == router.getRoutes().HNS_SYNC_SETUP) {
  ...
  detached() {
    const router = Router.getInstance()
    if (router.getRoutes().HNS_SYNC_SETUP.contains(router.getCurrentRoute())) {
  ...
  */
  ...
  onNavigateToPage_() {
    const router = Router.getInstance()
    assert(router.getCurrentRoute() == router.getRoutes().HNS_SYNC_SETUP)
    if (this.beforeunloadCallback_) {
      return
    }
  ...
  */
  onSetupSuccess_() {
    ...
    const router = Router.getInstance()
    if (router.getCurrentRoute() == router.getRoutes().HNS_SYNC_SETUP) {
      router.navigateTo(router.getRoutes().HNS_SYNC)
    }
  }
  ...
  */
  onRejoin_ = async function (e) {
    ...
    await this.hnsSyncBrowserProxy_.resetSyncChain();
...
customElements.define(
  SettingHnsSyncSubpage.is, SettingHnsSyncSubpage)
