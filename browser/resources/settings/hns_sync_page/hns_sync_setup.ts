...
import {getTemplate} from './hns_sync_setup.html.js'

/**
 * @fileoverview
 * 'hns-sync-setup' is the UI for starting or joining a sync chain
 * settings.
 */

const SettingsHnsSyncSetupElementBase =
  I18nMixin(BaseMixin(PolymerElement)) as {
    new(): PolymerElement & I18nMixinInterface
  }

export class SettingsHnsSyncSetupElement extends SettingsHnsSyncSetupElementBase {
  static get is() {
    return 'settings-hns-sync-setup'
  }
  ...
  syncBrowserProxy_: HnsSyncBrowserProxy = HnsSyncBrowserProxy.getInstance();
  ...
  handleSyncCodeDialogDone_() {
    if (this.syncCodeDialogType === 'input') {
      const messageText = this.i18n('hnsSyncFinalSecurityWarning')
...
customElements.define(
  SettingsHnsSyncSetupElement.is, SettingsHnsSyncSetupElement)
