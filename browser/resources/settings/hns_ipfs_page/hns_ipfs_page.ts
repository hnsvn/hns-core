...
import {HnsIPFSBrowserProxyImpl} from './hns_ipfs_browser_proxy.js';
import {getTemplate} from './hns_ipfs_page.html.js'

/**
 * Keep it same as in IPFSResolveMethodTypes
 * in hns\components\ipfs\ipfs_constants.h */
...
const SettingHnsIpfsPageElementBase =
 ...
 */
class SettingsHnsIpfsPageElement extends SettingHnsIpfsPageElementBase {
  static get is() {
    return 'settings-hns-ipfs-page'
  }
  ...
  /** @private {?settings.HnsIPFSBrowserProxy} */
  browserProxy_ = HnsIPFSBrowserProxyImpl.getInstance()
  ...
  ready() {
    ...
    this.addWebUiListener('hns-ipfs-node-status-changed', (launched) => {
  ...
  setupOptionsVisibility() {
    const resolve_method = this.getPref('hns.ipfs.resolve_method').value
    ...
    this.$.ipfsStorageMax.value =
      this.getPref('hns.ipfs.storage_max').value
  }
  ...
  onLoad_() {
    ...
    if (this.isKeysEditorRoute() && !this.isLocalNodeEnabled_) {
      ...
      router.navigateTo(router.getRoutes().HNS_WEB3)
    }
  }
  ...
  onIpfsPeersEditorClick_() {
    const router = Router.getInstance()
    router.navigateTo(router.getRoutes().HNS_IPFS_PEERS)
  }

  /** @private **/
  onP2pKeysEditorClick_() {
    const router = Router.getInstance()
    router.navigateTo(router.getRoutes().HNS_IPFS_KEYS)
  }
  ...
  isKeysEditorRoute() {
    const router = Router.getInstance()
    return (router.getCurrentRoute() == router.getRoutes().HNS_IPFS_KEYS)
  }

  /** @private **/
  isPeersEditorRoute() {
    const router = Router.getInstance()
    return (router.getCurrentRoute() == router.getRoutes().HNS_IPFS_PEERS)
  }
...
customElements.define(
  SettingsHnsIpfsPageElement.is, SettingsHnsIpfsPageElement)
