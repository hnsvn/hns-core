...
const SettingsHnsP2pKeysSubpageElementBase =
...
export interface SettingsHnsP2pKeysSubpageElement {
  $: {
...
export class SettingsHnsP2pKeysSubpageElement extends SettingsHnsP2pKeysSubpageElementBase {
  static get is() {
  ...
  browserProxy_: HnsIPFSBrowserProxyImpl = HnsIPFSBrowserProxyImpl.getInstance();

  override ready() {
    super.ready()
    this.addWebUiListener(
      'hns-ipfs-node-status-changed', (launched: boolean) => {
        this.onServiceLaunched(launched)
    })
    this.addWebUiListener('hns-ipfs-keys-loaded', () => {
      this.updateKeys()
    })
    this.addWebUiListener(
      'hns-ipfs-key-exported', (key: string, success: boolean) => {
    ...
    this.addWebUiListener(
      'hns-ipfs-key-imported', (key: string, success: boolean) => {
        this.actionKeysError_ = !success
        if (this.actionKeysError_ ) {
          const errorLabel = (this.$.keyError);
...
customElements.define(
  SettingsHnsP2pKeysSubpageElement.is, SettingsHnsP2pKeysSubpageElement)
