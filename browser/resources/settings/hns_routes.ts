...
import {pageVisibility} from './hns_overrides/page_visibility.js'
import {loadTimeData} from './i18n_setup.js'
import {SettingsRoutes} from 'chrome://settings/settings.js';

export default function addHnsRoutes(r: Partial<SettingsRoutes>) {
  const isGuest = loadTimeData.getBoolean('isGuest')
  if (!r.BASIC) {
    console.error('[Hns Settings Overrides] Routes: could not find BASIC page')
  }
  if (pageVisibility.getStarted) {
    ...
    // We re-section people page into getStarted section (see people_page Hns
    // override), so we need to adjust the route accordingly in order for the
    // direct navigation to hns://settings/importData to work.
    if (r.IMPORT_DATA) {
  ...
  if (loadTimeData.getBoolean('isHnsRewardsSupported')) {
  ...
  if (loadTimeData.getBoolean('areShortcutsSupported')) {
    if (r.SYSTEM) {
      ...
      console.error('[Hns Settings Overrides] Routes: could not find SYSTEM page')
    }
  }
  ...
  if (pageVisibility.hnsSync) {
    r.HNS_SYNC = r.BASIC.createSection('/hnsSync', 'hnsSync')
    r.HNS_SYNC_SETUP = r.HNS_SYNC.createChild('/hnsSync/setup');
  }
  if (pageVisibility.hnsWeb3) {
    r.HNS_WEB3 = r.BASIC.createSection('/web3', 'web3')
  }
  if (pageVisibility.hnsIPFS) {
    r.HNS_IPFS = r.HNS_WEB3.createSection('/ipfs', 'ipfs')
    r.HNS_IPFS_KEYS = r.HNS_IPFS.createChild('/ipfs/keys');
    r.HNS_IPFS_PEERS = r.HNS_IPFS.createChild('/ipfs/peers');
  }
  if (pageVisibility.hnsWallet) {
    r.HNS_WALLET = r.HNS_WEB3.createSection('/wallet', 'wallet')
    r.HNS_WALLET_NETWORKS = r.HNS_WALLET.createChild('/wallet/networks');
  }
  if (r.ADVANCED) {
    r.HNS_HELP_TIPS = r.ADVANCED.createSection('/hnsHelpTips', 'hnsHelpTips')
  } else if (!isGuest) {
    console.error('[Hns Settings Overrides] Could not find ADVANCED page', r)
  }
  r.HNS_NEW_TAB = r.BASIC.createSection('/newTab', 'newTab')
  if (r.SITE_SETTINGS_SITE_DETAILS) {
    r.HNS_SITE_SETTINGS_COOKIES_DETAILS =
      r.SITE_SETTINGS_SITE_DETAILS.createChild('/cookies/detail');
  } else {
    console.error('[Hns Settings Overrides] could not find expected route /content/siteDetails')
  }
  ...
  if (r.SITE_SETTINGS) {
    ...
    const isNativeHnsWalletFeatureEnabled = loadTimeData.getBoolean('isNativeHnsWalletFeatureEnabled')
    if (isNativeHnsWalletFeatureEnabled) {
      r.SITE_SETTINGS_ETHEREUM = r.SITE_SETTINGS.createChild('ethereum')
      r.SITE_SETTINGS_SOLANA = r.SITE_SETTINGS.createChild('solana')
    }
    r.SITE_SETTINGS_SHIELDS_STATUS = r.SITE_SETTINGS.createChild('hnsShields')
    if (r.SITE_SETTINGS_ADS) {
      delete r.SITE_SETTINGS_ADS
    } else {
      console.error('[Hns Settings Overrides] could not find expected route site_settings_ads')
    }
  } else if (!isGuest) {
    console.error('[Hns Settings Overrides] Routes: could not find SITE_SETTINGS page')
  }
  ...
  if (r.AUTOFILL && r.ADVANCED) {
    ...
    console.error('[Hns Settings Overrides] Could not move autofill route to advanced route', r)
  }
  ...
  if (r.SAFETY_CHECK && r.ADVANCED) {
    ...
    console.error('[Hns Settings Overrides] Could not move safety check route to advanced route', r)
  }
}
