...
import {HnsSiteDetailsElement} from '../hns_site_details/hns_site_details.js'

import 'chrome://resources/hns/leo.bundle.js'

RegisterPolymerComponentReplacement(
  'site-details',
  HnsSiteDetailsElement
)
  ...
  'site-details': (templateContent) => {
    if (!loadTimeData.getBoolean('isIdleDetectionFeatureEnabled')) {
      ...
      if (!idleDetectionItem) {
        console.error(`[Hns Settings Overrides] Couldn't find idle detection item`)
      } else {
    ...
    if (!adsItem) {
      console.error(`[Hns Settings Overrides] Couldn't find ads item`)
    } else {
      adsItem.hidden = true
    }
    ...
    if (!firstPermissionItem) {
      console.error(`[Hns Settings Overrides] Couldn't find first permission item`)
    } else {
      ...
      if (!autoplaySettings) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find autoplay settings')
      }
      else {
      ...
      if (isGoogleSignInFeatureEnabled) {
        ...
        if (!googleSignInSettings) {
          console.error(
            '[Hns Settings Overrides] Couldn\'t find Google signin settings')
        }
        else {
      ...
      if (isLocalhostAccessFeatureEnabled) {
        ...
        if (!localhostAccessSettings) {
          console.error(
            '[Hns Settings Overrides] Localhost access settings not found')
        } else {
      ...
      if (isNativeHnsWalletEnabled) {
        ...
        if (!ethereumSettings) {
          console.error(
              '[Hns Settings Overrides] Couldn\'t find Ethereum settings')
        } else {
        ...
        if (!solanaSettings) {
          console.error(
            '[Hns Settings Overrides] Couldn\'t find Solana settings')
        } else {
    ...
    if (!usageSection) {
      console.error(`[Hns Settings Overrides] Couldn't find usageSection item`)
    } else {
      ...
      if (!cookiesDetail) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find cookies link')
      } else {
