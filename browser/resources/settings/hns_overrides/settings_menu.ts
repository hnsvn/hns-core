...
import 'chrome://resources/hns/leo.bundle.js'
...
function getMenuElement(templateContent, href) {
  ...
  if (!menuEl) {
    ...
    console.error(`[Hns Settings Overrides] Could not find menu item '${href}'`)
  }
  return menuEl
}
...
RegisterStyleOverride(
  ...
  html`
    <style>
      :host {
        --hns-settings-menu-margin-v: 30px;
        --hns-settings-menu-padding: 30px;
        --settings-nav-item-color: #424242 !important;
        position: sticky;
        top: var(--hns-settings-menu-margin-v);
        margin: 0 var(--hns-settings-menu-margin) !important;
        max-height: calc(100vh - 56px - (var(--hns-settings-menu-margin-v) * 2) - (var(--hns-settings-menu-padding) * 2));
        min-width: 172px;
        border-radius: 6px;
      ...
      a[href].iron-selected::before {
        ...
        position: absolute;
        top: 50%;
        left: calc(-1 * var(--hns-settings-menu-padding));
      ...
      a[href],
      #advancedButton {
        --cr-selectable-focus_-_outline: var(--hns-focus-outline) !important;
      }
...
RegisterPolymerTemplateModifications({
  'settings-menu': (templateContent) => {
    ...
    if (!menuEl) {
      console.error('[Hns Settings Overrides] Could not find menu element to add title after')
    } else {
    ...
    const getStartedEl = createMenuElement(
      loadTimeData.getString('hnsGetStartedTitle'),
      '/getStarted',
      'rocket',
      'getStarted'
    )
    ...
    const newTabEl = createMenuElement(
      loadTimeData.getString('hnsNewTab'),
      '/newTab',
      'window-tab-new',
      'newTab'
    )
    ...
    const shieldsEl = createMenuElement(
      loadTimeData.getString('hnsShieldsTitle'),
      '/shields',
      'shield-done',
      'shields',
    )
    ...
    if (isHnsRewardsSupported) {
      rewardsEl = createMenuElement(
        loadTimeData.getString('hnsRewards'),
        '/rewards',
        'product-bat-outline',
        'rewards',
      )
    ...
    const syncEl = createMenuElement(
      loadTimeData.getString('hnsSync'),
      '/hnsSync',
      'product-sync',
      'hnsSync',
    )
    ...
    const extensionEl = createMenuElement(
      loadTimeData.getString('hnsDefaultExtensions'),
      '/extensions',
      'browser-extensions',
      'extensions',
    )
    ...
    const web3El = createMenuElement(
      loadTimeData.getString('hnsWeb3'),
      '/web3',
      'product-hns-wallet',
      'wallet',
    )
    ...
    const helpTipsEl = createMenuElement(
      loadTimeData.getString('hnsHelpTips'),
      '/hnsHelpTips',
      'help-outline',
      'hnsHelpTips',
    )
    ...
    if (!extensionsLinkEl) {
      console.error('[Hns Settings Overrides] Could not find extensionsLinkEl to remove')
    }
    extensionsLinkEl.remove()
    // Add version number to 'about' link
    const aboutEl = templateContent.querySelector('#about-menu')
    if (!aboutEl) {
      console.error('[Hns Settings Overrides] Could not find about-menu element')
      return
    }
    ...
    graphicsEl.setAttribute('class', 'hns-about-graphic')

    const icon = document.createElement('leo-icon')
    icon.setAttribute('name', 'hns-icon-release-color')

    const metaEl = document.createElement('div')
    metaEl.setAttribute('class', 'hns-about-meta')

    const menuLink = document.createElement('span')
    menuLink.setAttribute('class', 'hns-about-item hns-about-menu-link-text')
    menuLink.textContent = aboutEl.textContent

    const versionEl = document.createElement('span')
    versionEl.setAttribute('class', 'hns-about-item hns-about-menu-version')
    versionEl.textContent = `v ${loadTimeData.getString('hnsProductVersion')}`
