...
import {RegisterPolymerComponentBehaviors, RegisterStyleOverride} from 'chrome://resources/hns/polymer_overriding.js'
  ...
  html`
    <style>
      :host {
        ...
        --hns-settings-content-max-width: 708px;
        --hns-settings-menu-width: var(--settings-menu-width);
        --hns-settings-menu-margin: 12px;
      }

      /** Styling tweaks for the settings-menu when we display it inside the
        * drawer. */
      cr-drawer settings-menu {
        --hns-settings-menu-margin-v: 0;
        --hns-settings-menu-padding: 24px;
        --hns-settings-menu-margin: 0;
      }
      ...
      #left {
        /* fixed size menu */
        flex: 0 0 calc(var(--settings-menu-width) + var(--hns-settings-menu-margin) * 2)  !important;
      }
      #main {
        /* Take up rest of container up to a max */
        flex: 0 1 var(--hns-settings-content-max-width) !important;
      }
      #right {
...
const HnsClearSettingsMenuHighlightBehavior = {
  ready: function() {
...
RegisterPolymerComponentBehaviors({
  'settings-ui': [
    HnsClearSettingsMenuHighlightBehavior
  ]
})
