...
RegisterPolymerTemplateModifications({
  'settings-site-settings-page': (templateContent) => {
    ...
    if (!allSites) {
      console.error('[Hns Settings Overrides] Could not find all sites list')
      return
    }
    ...
    if (!siteSettingsShieldsTitle) {
      console.error('[Hns Settings Overrides] Couldn\'t find shields title')
    } else {
...
RegisterPolymerComponentReplacement(
  ...
  class HnsComponent extends SettingsSiteSettingsPageElement {
    static get properties() {
      const properties = SettingsSiteSettingsPageElement.properties
      if (!properties || !properties.lists_ || !properties.lists_.value) {
        console.error('[Hns Settings Overrides] Could not find polymer lists_ property')
        return
      }
      ...
      properties.lists_.value = function () {
        ...
        if (!lists_) {
          console.error('[Hns Settings Overrides] did not get lists_ data')
          return
        }
        if (!lists_.permissionsBasic) {
          console.error('[Hns Settings Overrides] did not get lists_.permissionsBasic data')
        } else {
        ...
        if (!lists_.contentAdvanced) {
          console.error('[Hns Settings Overrides] did not get lists_.contentAdvanced data')
        } else {
        ...
        if (!lists_.permissionsAdvanced) {
          console.error('[Hns Settings Overrides] did not get lists_.permissionsAdvanced data')
        } else {
          ...
          if (!loadTimeData.getBoolean('isHnsWebSerialApiEnabled')) {
