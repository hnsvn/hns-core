...
} from 'chrome://resources/hns/polymer_overriding.js'
import { loadTimeData } from '../i18n_setup.js'
import '../hns_system_page/hns_performance_page.js'
// <if expr="enable_hns_vpn_wireguard">
import '../hns_system_page/hns_vpn_page.js'
// </if>
...
RegisterPolymerTemplateModifications({
  'settings-system-page': (templateContent) => {
        ...
        <settings-animated-pages id="pages" section="system">
          <div route-path="default">
            <cr-link-row
              ...
              id="shortcutsButton"
              label=${loadTimeData.getString('hnsShortcutsPage')}
          ...
          <template is="dom-if" route-path="/system/shortcuts">
            <settings-subpage
              ...
              page-title=${loadTimeData.getString('hnsShortcutsPage')}>
    ...
    templateContent.appendChild(
      html`
        <settings-toggle-button
          class="cr-row"
          pref="{{prefs.hns.enable_closing_last_tab}}"
          label="${loadTimeData.getString('hnsHelpTipsClosingLastTab')}"
        >
        </settings-toggle-button>
      `
    )
    ...
    templateContent.appendChild(
      html`
        <settings-hns-vpn-page prefs="{{prefs}}">
        </settings-hns-vpn-page>
      `
    )
    // </if>
    ...
    templateContent.appendChild(
      html`
        <settings-hns-performance-page prefs="{{prefs}}">
        </settings-hns-performance-page>
      `
    )
  }
})
