  ...
  'settings-cookies-page': (templateContent) => {
    ...
    if (!privacySandboxSettings4Template) {
      ...
      if (!clearOnExitToggle) {
        console.error(
          '[Hns Settings Overrides] Could not find clearOnExit id ' +
          'on cookies page.')
      } else {
      ...
      if (!blockThirdPartyIncognitoRadioButton) {
        console.error(
          '[Hns Settings Overrides] Could not find ' +
    ...
    if (!preloadingLinkRowTemplate) {
      console.error(
          '[Hns Settings Overrides] Could not find preloading template')
      return
    }
    ...
    if (!networkPredictionLinkRow) {
      console.error(
          '[Hns Settings Overrides] Could not find preloadingLinkRow id ' +
          'on cookies page.')
    }
    else {
