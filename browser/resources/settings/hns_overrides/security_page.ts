...
RegisterPolymerTemplateModifications({
  'settings-security-page': (templateContent) => {
    ...
    if (!safeBrowsingReportingToggle) {
      console.error('[Hns Settings Overrides] Could not find safeBrowsingReportingToggle id on security page.')
    } else {
    ...
    if (!safeBrowsingEnhanced) {
      console.error('[Hns Settings Overrides] Could not find safeBrowsingEnhanced id on security page.')
    } else {
    ...
    if (!passwordsLeakToggle) {
      console.error('[Hns Settings Overrides] Could not find passwordsLeakToggle id on security page.')
    } else {
    ...
    if (loadTimeData.getBoolean("isHttpsByDefaultEnabled")) {
      ...
      if (!httpsOnlyModeToggle) {
        console.error('[Hns Settings Overrides] Could not find' +
    ...
    if (!link) {
      console.error('[Hns Settings Overrides] Could not find advanced-protection-program-link id on security page.')
    } else {
    ...
    if (!certsTemplate) {
      console.error('[Hns Settings Overrides] Could not find certs template')
      return
    }
    ...
    if (!chromeCertificates) {
      console.error(
        '[Hns Settings Overrides] Could not find chromeCertificates id ' +
        'on security page.')
    } else {
