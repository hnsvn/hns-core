...
import {RegisterPolymerTemplateModifications, RegisterPolymerComponentReplacement} from 'chrome://resources/hns/polymer_overriding.js'
import {getTrustedHTML} from 'chrome://resources/js/static_types.js'
import {HnsSettingsPrivacyPageElement} from '../hns_privacy_page/hns_privacy_page.js'
  ...
  pages: Element)
{
  ...
  if (!googleSignInTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find Google signin template')
  } else {
    const googleSignInSubpage =
      googleSignInTemplate.content.querySelector('settings-subpage')
    if (!googleSignInSubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find Google signin subpage')
    } else {
      const pageTitle =
        googleSignInTemplate.content.getElementById('page-title')
      if (!pageTitle) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Google sigin page-title')
      } else {
      ...
      if (!googleSignInDefault) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Google signin default')
      } else {
      ...
      if (!googleSignInExceptions) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Google signin exceptions')
      } else {
  ...
  pages: Element)
{
  ...
  if (!localhostAccessTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find localhost access template')
  } else {
    const localhostAccessSubpage =
      localhostAccessTemplate.content.querySelector('settings-subpage')
    if (!localhostAccessSubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find localhost access subpage')
    } else {
      const pageTitle =
        localhostAccessTemplate.content.getElementById('page-title')
      if (!pageTitle) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find localhost access ' +
          'page-title')
      } else {
        let pageTitleNode = document.createTextNode(
      ...
      if (!localhostAccessDefault) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find localhost access default')
      } else {
      ...
      if (!localhostExceptions) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find localhost exceptions')
      } else {
  ...
  pages: Element)
{
  ...
  if (!autoplayTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find autoplay template')
  } else {
    const autoplaySubpage =
      autoplayTemplate.content.querySelector('settings-subpage')
    if (!autoplaySubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find autoplay subpage')
    } else {
      const pageTitle =
        autoplayTemplate.content.getElementById('page-title')
      if (!pageTitle) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find autoplay page-title')
      } else {
        let pageTitleNode = document.createTextNode(
      ...
      if (!autoplayDefault) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find autoplay default')
      } else {
      ...
      if (!autoplayExceptions) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find autoplay exceptions')
      } else {
  ...
  pages: Element)
{
  ...
  if (!ethereumTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find Ethereum template')
  } else {
    const ethereumSubpage =
      ethereumTemplate.content.querySelector('settings-subpage')
    if (!ethereumSubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find Ethereum subpage')
    } else {
      const pageTitle =
        ethereumTemplate.content.getElementById('page-title')
      if (!pageTitle) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Ethereum page-title')
      } else {
        let pageTitleNode = document.createTextNode(
      ...
      if (!ethereumDefault) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Ethereum default')
      } else {
      ...
      if (!ethereumExceptions) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Ethereum exceptions')
      } else {
  ...
  pages: Element)
{
  ...
  if (!solanaTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find Solana template')
  } else {
    const solanaSubpage =
      solanaTemplate.content.querySelector('settings-subpage')
    if (!solanaSubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find Solana subpage')
    } else {
      const pageTitle =
        solanaTemplate.content.getElementById('page-title')
      if (!pageTitle) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Solana page-title')
      } else {
        let pageTitleNode = document.createTextNode(
      ...
      if (!solanaDefault) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Solana default')
      } else {
      ...
      if (!solanaExceptions) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Solana exceptions')
      } else {
  ...
  pages: Element)
{
  pages.insertAdjacentHTML(
    'beforeend',
    getTrustedHTML`
      <template is="dom-if" route-path="/content/hnsShields" no-search>
        <settings-subpage>
          <category-setting-exceptions
            id="shieldsExceptions"
            category="[[contentSettingsTypesEnum_.HNS_SHIELDS]]">
          </category-setting-exceptions>
        </settings-subpage>
      </template>`)
  const shieldsTemplate = templateContent.
    querySelector('[route-path="/content/hnsShields"]')
  if (!shieldsTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find Shields template')
  } else {
    const shieldsSubpage =
      shieldsTemplate.content.querySelector('settings-subpage')
    if (!shieldsSubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find Shields subpage')
    } else {
      ...
      if (!shieldsExceptions) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Shields exceptions')
      } else {
  ...
  pages: Element)
{
    ...
    getTrustedHTML`
      <template is="dom-if" route-path="/cookies/detail" no-search>
        <settings-subpage page-title="[[pageTitle]]">
          ...
          <hns-site-data-details-subpage page-title="{{pageTitle}}">
          </hns-site-data-details-subpage>
        </settings-subpage>
      </template>`)
  ...
  if (!cookiesTemplate) {
    console.error(
      '[Hns Settings Overrides] Couldn\'t find Cookies template')
  } else {
    const cookiesSubpage =
      cookiesTemplate.content.querySelector('settings-subpage')
    if (!cookiesSubpage) {
      console.error(
        '[Hns Settings Overrides] Couldn\'t find Cookies subpage')
    } else {
      const removeAllButton =
        cookiesTemplate.content.getElementById('remove-all-button')
      if (!removeAllButton) {
        console.error(
          '[Hns Settings Overrides] Couldn\'t find Cookies remove all button')
      } else {
...
RegisterPolymerTemplateModifications({
  'settings-privacy-page': (templateContent) => {
    ...
    if (!pages) {
      console.error(`[Hns Settings Overrides] Couldn't find privacy_page #pages`)
    } else {
      if (!loadTimeData.getBoolean('isIdleDetectionFeatureEnabled')) {
        ...
        if (!idleDetection) {
          console.error(`[Hns Settings Overrides] Couldn't find idle detection template`)
        } else {
      ...
      const isNativeHnsWalletEnabled = loadTimeData.getBoolean('isNativeHnsWalletFeatureEnabled')
      if (isNativeHnsWalletEnabled) {
    ...
    if (!loadTimeData.getBoolean('isPrivacySandboxRestricted')) {
      ...
      if (!privacySandboxSettings3Template) {
        console.error(
          '[Hns Settings Overrides] Could not find template with ' +
        ...
        if (!privacySandboxLinkRow) {
          console.error('[Hns Settings Overrides] Could not find privacySandboxLinkRow id on privacy page.')
        } else {
        ...
        if (!privacySandboxLink) {
          console.error('[Hns Settings Overrides] Could not find privacySandboxLink id on privacy page.')
        } else {
      ...
      if (!privacySandboxSettings4Template) {
        ...
        if (!privacySandboxLinkRow) {
          console.error(
            '[Hns Settings Overrides] Could not find privacySandboxLinkRow ' +
            'id on privacy page.')
        } else {
          privacySandboxLinkRow.setAttribute('hidden', 'true')
        }
    ...
    if (!showPrivacyGuideEntryPointTemplate) {
      console.error(
          '[Hns Settings Overrides] Could not find template with if*=isPrivacyGuideAvailable on privacy page.')
    } else {
      ...
      if (!privacyGuideLinkRow) {
        console.error('[Hns Settings Overrides] Could not find privacyGuideLinkRow id on privacy page.')
      } else {
