...
class EphemeralStorage1pDomainBlockBrowserTest
 ...
 public:
  ...
  void WaitForAdBlockServiceThreads() {
    scoped_refptr<base::ThreadTestHelper> tr_helper(new base::ThreadTestHelper(
        g_hns_browser_process->local_data_files_service()->GetTaskRunner()));
  ...
  WebContents* BlockAndNavigateToBlockedDomain(const GURL& url,
    ...
    if (is_aggressive) {
      hns_shields::SetCosmeticFilteringControlType(
          content_settings(), hns_shields::ControlType::BLOCK, url);
    }
 ...
 protected:
  std::unique_ptr<hns_shields::TestFiltersProvider> source_provider_;
