...
#if !BUILDFLAG(IS_ANDROID)
class HnsIPFSInfoBarDelegateObserverImpl
    : public HnsIPFSInfoBarDelegateObserver {
 public:
  explicit HnsIPFSInfoBarDelegateObserverImpl(
  ...
  ~HnsIPFSInfoBarDelegateObserverImpl() override = default;
...
void IPFSTabHelper::UpdateLocationBar() {
  ...
  if (content_infobar_manager && ipfs_resolved_url_.is_valid() &&
      !pref_service_->GetBoolean(kIPFSAutoRedirectToConfiguredGateway)) {
    HnsIPFSInfoBarDelegate::Create(
        content_infobar_manager,
        std::make_unique<HnsIPFSInfoBarDelegateObserverImpl>(
