...
#include "hns/browser/ipfs/ipfs_host_resolver.h"
...
IN_PROC_BROWSER_TEST_F(IpfsTabHelperBrowserTest, ResolvedIPFSLinkLocal) {
  ...
  resolver_raw->ResetResolveCalled();
  SetXIpfsPathHeader("/ipns/hns.eth/empty.html");
  test_url = https_server_.GetURL("/?query#ref");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), test_url));
  ASSERT_TRUE(WaitForLoadStop(active_contents()));
  ASSERT_TRUE(resolver_raw->resolve_called());
  resolved_url = helper->GetIPFSResolvedURL();
  EXPECT_EQ(resolved_url, GURL("ipns://hns.eth/empty.html?query#ref"));
...
IN_PROC_BROWSER_TEST_F(IpfsTabHelperBrowserTest, IPFSPromoInfobar) {
  ...
  auto find_infobar =
    ...
    for (size_t i = 0; i < content_infobar_manager->infobar_count(); i++) {
      ...
      if (infobar->delegate()->GetIdentifier() ==
          HnsConfirmInfoBarDelegate::HNS_IPFS_INFOBAR_DELEGATE) {
        return infobar;
      }
    }
    return nullptr;
  ...
  // Press cancel
  {
    ...
    ASSERT_TRUE(infobar);
    static_cast<HnsConfirmInfoBar*>(infobar)->GetDelegate()->Cancel();
  ...
  for (int i = 0; i < 2; i++) {
    {
      ...
      auto* infobar = find_infobar(
          infobars::ContentInfoBarManager::FromWebContents(active_contents()));
      ASSERT_TRUE(infobar);
      static_cast<HnsConfirmInfoBar*>(infobar)
          ->GetDelegate()
          ->ExtraButtonPressed();
  ...
  // Test accept button, which enables the feature
  {
    ...
    auto* infobar = find_infobar(
        infobars::ContentInfoBarManager::FromWebContents(active_contents()));
    ASSERT_TRUE(infobar);
    static_cast<HnsConfirmInfoBar*>(infobar)->GetDelegate()->Accept();
