...
class HnsStatsUpdaterBrowserTest : public PlatformBrowserTest {
 public:
  void SetUp() override {
    auto referral_initialized_callback = base::BindRepeating(
        &HnsStatsUpdaterBrowserTest::OnReferralInitialized,
        base::Unretained(this));
    hns::HnsReferralsService::SetReferralInitializedCallbackForTesting(
        &referral_initialized_callback);

    auto stats_updated_callback = base::BindRepeating(
        &HnsStatsUpdaterBrowserTest::OnStandardStatsUpdated,
        base::Unretained(this));
    hns_stats::HnsStatsUpdater::SetStatsUpdatedCallbackForTesting(
        &stats_updated_callback);

    auto stats_threshold_callback = base::BindRepeating(
        &HnsStatsUpdaterBrowserTest::OnThresholdStatsUpdated,
        base::Unretained(this));
    hns_stats::HnsStatsUpdater::SetStatsThresholdCallbackForTesting(
  ...
  void TearDown() override {
    hns::HnsReferralsService::SetReferralInitializedCallbackForTesting(
        nullptr);
    hns_stats::HnsStatsUpdater::SetStatsUpdatedCallbackForTesting(nullptr);
    hns_stats::HnsStatsUpdater::SetStatsThresholdCallbackForTesting(
        nullptr);
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    command_line->AppendSwitchASCII(
        hns_stats::switches::kHnsStatsUpdaterServer,
  ...
  void SetBaseUpdateURLForTest() {
    ...
    env->SetVar("HNS_REFERRALS_SERVER",
                embedded_test_server()->host_port_pair().ToString());
    env->SetVar("HNS_REFERRALS_LOCAL", "1");  // use http for local testing
  }
...
IN_PROC_BROWSER_TEST_F(HnsStatsUpdaterBrowserTest,
                       StatsUpdaterSetsFirstCheckPreference) {
  WaitForReferralInitializeCallback();
  WaitForStandardStatsUpdatedCallback();

  // We get //1/usage/hns-core here, so ignore the first slash.
  EXPECT_STREQ(GetUpdateURL().path().c_str() + 1, "/1/usage/hns-core");
...
IN_PROC_BROWSER_TEST_F(HnsStatsUpdaterBrowserTest,
  ...
  EXPECT_TRUE(
      g_hns_browser_process->hns_stats_updater()->MaybeDoThresholdPing(3));

  WaitForReferralInitializeCallback();
  WaitForThresholdStatsUpdatedCallback();

  // We get //1/usage/hns-core-threshold here, so ignore the first slash.
  EXPECT_STREQ(GetUpdateURL().path().c_str() + 1,
               "/1/usage/hns-core-threshold");
...
class HnsStatsUpdaterReferralCodeBrowserTest
    : public HnsStatsUpdaterBrowserTest {
 public:
  void SetUp() override {
    ...
    hns::HnsReferralsService::SetPromoFilePathForTesting(promo_code_file);
    HnsStatsUpdaterBrowserTest::SetUp();
  }
...
IN_PROC_BROWSER_TEST_F(HnsStatsUpdaterReferralCodeBrowserTest,
