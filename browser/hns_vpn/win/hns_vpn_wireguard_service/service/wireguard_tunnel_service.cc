...
#include "hns/components/hns_vpn/common/wireguard/win/service_commands.h"
...
constexpr wchar_t kHnsWireguardConfig[] = L"wireguard";
constexpr wchar_t kHnsWireguardConfigExtension[] = L".hns.conf";
...
std::wstring GetWireguardConfigName(const std::wstring& prefix) {
  return prefix + std::wstring(kHnsWireguardConfigExtension);
}
...
absl::optional<base::FilePath> WriteConfigToFile(const std::string& config) {
  ...
  if (!scoped_temp_dir.Set(temp_dir_path.Append(base::FilePath(L"HnsVpn")))) {
    return absl::nullopt;
  }

  base::FilePath temp_file_path(scoped_temp_dir.GetPath().Append(
      GetWireguardConfigName(kHnsWireguardConfig)));
...
bool LaunchWireguardService(const std::wstring& config) {
  ...
  if (!RemoveExistingWireguardService()) {
    VLOG(1) << "Failed to remove existing hns wireguard service";
    return false;
  }
  return CreateAndRunHnsWireguardService(config);
}
...
bool RemoveExistingWireguardService() {
  ...
  if (!scm.IsValid()) {
    VLOG(1) << "::OpenSCManager failed. service_name: "
            << GetHnsVpnWireguardTunnelServiceName()
            << ", error: " << std::hex << HRESULTFromLastError();
    return false;
  }
  ScopedScHandle service(
      ::OpenService(scm.Get(), GetHnsVpnWireguardTunnelServiceName().c_str(),
                    SERVICE_ALL_ACCESS));
  ...
  if (service.IsValid()) {
    if (IsServiceRunning(service.Get())) {
      ...
      hns_vpn::RunWireGuardCommandForUsers(
...
bool CreateAndRunHnsWireguardService(const std::wstring& encoded_config) {
