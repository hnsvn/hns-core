...
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/notifications/notification_utils.h"
...
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/service/wireguard_service_runner.h"
...
absl::optional<int> ProcessUserLevelCommands(
  ...
  if (command_line.HasSwitch(
    ...
    return hns_vpn::StatusTrayRunner::GetInstance()->Run();
  }

  // User level command line. Publishes notification to system notification
  // center when vpn connected.
  if (command_line.HasSwitch(
          hns_vpn::kHnsVpnWireguardServiceNotifyConnectedSwitchName)) {
    hns_vpn::ShowDesktopNotification(
        base::UTF16ToWide(l10n_util::GetStringUTF16(
            IDS_HNS_VPN_WIREGUARD_TRAY_NOTIFICATION_CONNECTED)));
    return 0;
  }
  ...
  if (command_line.HasSwitch(
          hns_vpn::kHnsVpnWireguardServiceNotifyDisconnectedSwitchName)) {
    hns_vpn::ShowDesktopNotification(
        base::UTF16ToWide(l10n_util::GetStringUTF16(
            IDS_HNS_VPN_WIREGUARD_TRAY_NOTIFICATION_DISCONNECTED)));
    return 0;
  }
  return absl::nullopt;
}
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE prev, wchar_t*, int) {
  ...
  if (!process_type.empty()) {
    HnsWireguardCrashReporterClient::InitializeCrashReportingForProcess(
        process_type);
  }
  ...
  if (command_line->HasSwitch(
          hns_vpn::kHnsVpnWireguardServiceConnectSwitchName)) {
    return hns_vpn::wireguard::RunWireguardTunnelService(
        command_line->GetSwitchValuePath(
            hns_vpn::kHnsVpnWireguardServiceConnectSwitchName));
  }

  // System level command line. Makes registeration and configuration for
  // HnsVPNWireguardService windows service. Used by the installer.
  if (command_line->HasSwitch(
          hns_vpn::kHnsVpnWireguardServiceInstallSwitchName)) {
    auto success = hns_vpn::InstallHnsWireguardService();
    return success ? 0 : 1;
  }

  // System level command line. Unregisters HnsVPNWireguardService
  // windows service and removes stored data. Used by the uninstaller.
  if (command_line->HasSwitch(
          hns_vpn::kHnsVpnWireguardServiceUnnstallSwitchName)) {
    auto success = hns_vpn::UninstallHnsWireguardService();
    return success ? 0 : 1;
  }
  ...
  // Runs HnsVpnWireguardService, called by system SCM.
  return hns_vpn::WireguardServiceRunner::GetInstance()->RunAsService();
}
