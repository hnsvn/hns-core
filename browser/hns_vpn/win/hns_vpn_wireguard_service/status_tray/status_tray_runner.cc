...
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/status_tray_runner.h"
...
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/hns_vpn_tray_command_ids.h"
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/ras/ras_utils.h"
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/resources/resource.h"
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/status_icon/icon_utils.h"
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/status_icon/status_icon.h"
#include "hns/browser/hns_vpn/win/hns_vpn_wireguard_service/status_tray/status_icon/status_tray.h"
...
std::u16string GetVpnStatusLabel(bool active) {
  return l10n_util::GetStringUTF16(
      active ? IDS_HNS_VPN_WIREGUARD_TRAY_STATUS_ITEM_ACTIVE
             : IDS_HNS_VPN_WIREGUARD_TRAY_STATUS_ITEM_INACTIVE);
}

int GetStatusIconTooltip(hns_vpn::mojom::ConnectionState state) {
  switch (state) {
    case hns_vpn::mojom::ConnectionState::CONNECTING:
      return IDS_HNS_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_CONNECTING;
    case hns_vpn::mojom::ConnectionState::CONNECTED:
      return IDS_HNS_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_CONNECTED;
    case hns_vpn::mojom::ConnectionState::DISCONNECTING:
      return IDS_HNS_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_DISCONNECTING;
    case hns_vpn::mojom::ConnectionState::DISCONNECTED:
      return IDS_HNS_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_DISCONNECTED;
    case hns_vpn::mojom::ConnectionState::CONNECT_FAILED:
    case hns_vpn::mojom::ConnectionState::CONNECT_NOT_ALLOWED:
      return IDS_HNS_VPN_WIREGUARD_TRAY_ICON_TOOLTIP_ERROR;
  }

  NOTREACHED_NORETURN();
}

int GetStatusTrayIcon(hns_vpn::mojom::ConnectionState state) {
  bool dark_theme =
      ui::NativeTheme::GetInstanceForNativeUi()->ShouldUseDarkColors();
  switch (state) {
    case hns_vpn::mojom::ConnectionState::CONNECTED:
      return dark_theme ? IDR_HNS_VPN_TRAY_LIGHT_CONNECTED
                        : IDR_HNS_VPN_TRAY_DARK_CONNECTED;
    case hns_vpn::mojom::ConnectionState::DISCONNECTING:
    case hns_vpn::mojom::ConnectionState::DISCONNECTED:
      return dark_theme ? IDR_HNS_VPN_TRAY_LIGHT : IDR_HNS_VPN_TRAY_DARK;
    case hns_vpn::mojom::ConnectionState::CONNECTING:
      return dark_theme ? IDR_HNS_VPN_TRAY_LIGHT_CONNECTING
                        : IDR_HNS_VPN_TRAY_DARK_CONNECTING;
    case hns_vpn::mojom::ConnectionState::CONNECT_FAILED:
    case hns_vpn::mojom::ConnectionState::CONNECT_NOT_ALLOWED:
      return dark_theme ? IDR_HNS_VPN_TRAY_LIGHT_ERROR
                        : IDR_HNS_VPN_TRAY_DARK_ERROR;
  }
...
bool StatusTrayRunner::IsVPNConnected() const {
  ...
  return IsWireguardActive()
             ? wireguard::IsHnsVPNWireguardTunnelServiceRunning()
             : ras::IsRasConnected();
}

void StatusTrayRunner::ConnectVPN() {
  if (IsWireguardActive()) {
    wireguard::EnableHnsVpnWireguardService(
...
void StatusTrayRunner::DisconnectVPN() {
  if (IsWireguardActive()) {
    wireguard::DisableHnsVpnWireguardService(base::BindOnce(
...
void StatusTrayRunner::SetupStatusIcon() {
  ...
  current_state_ = IsVPNConnected()
                       ...
                       : hns_vpn::mojom::ConnectionState::DISCONNECTED;
...
void StatusTrayRunner::ExecuteCommand(int command_id, int event_flags) {
  switch (command_id) {
    case IDC_HNS_VPN_TRAY_EXIT_ICON:
      EnableVPNTrayIcon(false);
      SignalExit();
      break;
    case IDC_HNS_VPN_TRAY_CONNECT_VPN_ITEM:
      ConnectVPN();
      break;
    case IDC_HNS_VPN_TRAY_DISCONNECT_VPN_ITEM:
      DisconnectVPN();
      break;
    case IDC_HNS_VPN_TRAY_MANAGE_ACCOUNT_ITEM:
      OpenURLInBrowser(kManageUrlProd);
      break;
    case IDC_HNS_VPN_TRAY_ABOUT_ITEM:
...
void StatusTrayRunner::OnMenuWillShow(ui::SimpleMenuModel* source) {
  ...
  source->Clear();
  source->AddItem(IDC_HNS_VPN_TRAY_STATUS_ITEM, GetVpnStatusLabel(connected));
  source->SetEnabledAt(0, false);
  if (connected) {
    source->AddItem(IDC_HNS_VPN_TRAY_DISCONNECT_VPN_ITEM,
                    l10n_util::GetStringUTF16(
                        IDS_HNS_VPN_WIREGUARD_TRAY_DISCONNECT_ITEM));
  } else {
    source->AddItem(
        IDC_HNS_VPN_TRAY_CONNECT_VPN_ITEM,
        l10n_util::GetStringUTF16(IDS_HNS_VPN_WIREGUARD_TRAY_CONNECT_ITEM));
  }
  source->AddSeparator(ui::NORMAL_SEPARATOR);
  source->AddItem(IDC_HNS_VPN_TRAY_MANAGE_ACCOUNT_ITEM,
                  l10n_util::GetStringUTF16(
                      IDS_HNS_VPN_WIREGUARD_TRAY_MANAGE_ACCOUNT_ITEM));
  source->AddItem(
      IDC_HNS_VPN_TRAY_ABOUT_ITEM,
      l10n_util::GetStringUTF16(IDS_HNS_VPN_WIREGUARD_TRAY_ABOUT_ITEM));
  source->AddSeparator(ui::NORMAL_SEPARATOR);
  source->AddItem(
      IDC_HNS_VPN_TRAY_EXIT_ICON,
      l10n_util::GetStringUTF16(IDS_HNS_VPN_WIREGUARD_TRAY_REMOVE_ICON_ITEM));
}
...
hns_vpn::mojom::ConnectionState StatusTrayRunner::GetConnectionState() {
  if (IsVPNConnected()) {
    return hns_vpn::mojom::ConnectionState::CONNECTED;
  }

  auto state_from_storage = hns_vpn::GetConnectionState();
  if (state_from_storage.has_value()) {
    return static_cast<hns_vpn::mojom::ConnectionState>(
        state_from_storage.value());
  }

  return hns_vpn::mojom::ConnectionState::DISCONNECTED;
}
...
void StatusTrayRunner::OnWireguardServiceStateChanged(int mask) {
  UpdateConnectionState();
  auto service_name = IsVPNConnected() ? GetHnsVpnWireguardTunnelServiceName()
                                       : GetHnsVpnWireguardServiceName();
  if (!hns_vpn::IsWindowsServiceRunning(service_name)) {
...
void StatusTrayRunner::OnStorageUpdated() {
  // Checking if tray icon enabled from Hns-> App menu -> Hns VPN -> Show
  // VPN tray icon.
...
void StatusTrayRunner::SetupConnectionObserver() {
  if (IsWireguardActive()) {
    ...
    SubscribeForWireguardNotifications(
        IsVPNConnected() ? GetHnsVpnWireguardTunnelServiceName()
                         : GetHnsVpnWireguardServiceName());
    return;
  }
...
HRESULT StatusTrayRunner::Run() {
  ...
  base::ThreadPoolInstance::CreateAndStartWithDefaultParams(
      "Hns VPN Wireguard status tray process");
