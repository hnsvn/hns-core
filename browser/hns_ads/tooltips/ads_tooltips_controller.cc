...
void AdsTooltipsController::ShowCaptchaTooltip(
  ...
  const std::u16string title = hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_ADS_SCHEDULED_CAPTCHA_NOTIFICATION_TITLE);
  const std::u16string body = hns_l10n::GetLocalizedResourceUTF16String(
      IDS_HNS_ADS_SCHEDULED_CAPTCHA_NOTIFICATION_BODY);
  const std::u16string ok_button_text =
      hns_l10n::GetLocalizedResourceUTF16String(
          IDS_HNS_ADS_SCHEDULED_CAPTCHA_NOTIFICATION_OK_BUTTON_TEXT);
  std::u16string cancel_button_text =
      hns_l10n::GetLocalizedResourceUTF16String(
          IDS_HNS_ADS_SCHEDULED_CAPTCHA_NOTIFICATION_CANCEL_BUTTON_TEXT);

  hns_tooltips::HnsTooltipAttributes tooltip_attributes(
  ...
  hns_tooltips::HnsTooltipPopupHandler::Show(profile_,
                                                 std::move(captcha_tooltip));
}

void AdsTooltipsController::CloseCaptchaTooltip() {
  hns_tooltips::HnsTooltipPopupHandler::Close(kScheduledCaptchaTooltipId);
}

void AdsTooltipsController::OnTooltipWidgetDestroyed(
    const std::string& tooltip_id) {
  hns_tooltips::HnsTooltipPopupHandler::Destroy(kScheduledCaptchaTooltipId);
}
