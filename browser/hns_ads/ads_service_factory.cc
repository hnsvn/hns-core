...
#include "hns/browser/hns_ads/device_id/device_id_impl.h"
#include "hns/browser/hns_ads/services/bat_ads_service_factory_impl.h"
...
#include "hns/components/hns_federated/notification_ad_task_constants.h"
...
AdsService* AdsServiceFactory::GetForProfile(Profile* profile) {
  if (!hns_rewards::IsSupportedForProfile(profile)) {
    return nullptr;
  }
...
AdsServiceFactory::AdsServiceFactory()
  ...
  DependsOn(hns_rewards::RewardsServiceFactory::GetInstance());
  DependsOn(HistoryServiceFactory::GetInstance());
  DependsOn(hns_federated::HnsFederatedServiceFactory::GetInstance());
  DependsOn(hns_adaptive_captcha::HnsAdaptiveCaptchaServiceFactory::
                GetInstance());
}
...
KeyedService* AdsServiceFactory::BuildServiceInstanceFor(
  ...
  hns_federated::AsyncDataStore* notification_ad_async_data_store = nullptr;
  auto* federated_service =
      hns_federated::HnsFederatedServiceFactory::GetForBrowserContext(
          profile);
  if (federated_service) {
    notification_ad_async_data_store =
        federated_service->GetDataStoreService()->GetDataStore(
            hns_federated::kNotificationAdTaskName);
  }
  ...
  auto* rewards_service =
      hns_rewards::RewardsServiceFactory::GetInstance()->GetForProfile(
          profile);

  std::unique_ptr<AdsServiceImpl> ads_service =
      std::make_unique<AdsServiceImpl>(
          profile, hns_adaptive_captcha_service,
