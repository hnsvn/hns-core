...
// npm run test -- hns_browser_tests --filter=SearchResultAdTest*
...
constexpr char kAllowedDomain[] = "search.hns.com";
constexpr char kNotAllowedDomain[] = "hns.com";
...
constexpr char kSearchResultUrlPath[] =
    "/hns_ads/search_result_ad_sample.html";
...
IN_PROC_BROWSER_TEST_F(SearchResultAdTest, UserHasNotJoinedHnsRewards) {
  browser()->profile()->GetPrefs()->SetBoolean(hns_rewards::prefs::kEnabled,
                                               false);
...
IN_PROC_BROWSER_TEST_F(SearchResultAdTest, BrokenSearchAdMetadata) {
  ...
  GURL url = GetURL(kAllowedDomain, "/hns_ads/search_result_ad_broken.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  content::WebContents* web_contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  EXPECT_EQ(url, web_contents->GetVisibleURL());
