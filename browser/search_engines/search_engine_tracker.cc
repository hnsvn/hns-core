...
constexpr char kSwitchSearchEngineP3AStorage[] =
    "hns.search.p3a_default_switch";
constexpr char kHnsDomain[] = "hns.com";
...
// Not all engines added by Hns are present in |SearchEngineType| enumeration.
void RecordSearchEngineP3A(const GURL& search_engine_url,
  ...
  if (type == SEARCH_ENGINE_GOOGLE) {
    ...
    } else if (base::EndsWith(search_engine_url.host(), "hns.com",
                              base::CompareCase::INSENSITIVE_ASCII)) {
      answer = SearchEngineP3A::kHns;
    }
  }
...
SearchEngineSwitchP3A SearchEngineSwitchP3AMapAnswer(const GURL& to,
  ...
  if (from.DomainIs(kHnsDomain)) {
    // Switching away from Hns Search.
    if (to.DomainIs(kGoogleDomain)) {
      answer = SearchEngineSwitchP3A::kHnsToGoogle;
    } else if (to.DomainIs(kDDGDomain)) {
      answer = SearchEngineSwitchP3A::kHnsToDDG;
    } else {
      answer = SearchEngineSwitchP3A::kHnsToOther;
    }
  } else if (to.DomainIs(kHnsDomain)) {
    // Switching to Hns Search.
    if (from.DomainIs(kGoogleDomain)) {
      answer = SearchEngineSwitchP3A::kGoogleToHns;
    } else if (from.DomainIs(kDDGDomain)) {
      answer = SearchEngineSwitchP3A::kDDGToHns;
    } else {
      answer = SearchEngineSwitchP3A::kOtherToHns;
    }
  } else {
...
void SearchEngineTracker::RecordSwitchP3A(const GURL& url) {
  ...
  if (url.is_valid() && url != previous_search_url_) {
    ...
    if (url.DomainIs(kHnsDomain)) {
      hns_search_conversion::p3a::RecordDefaultEngineChange(local_state_);
    }
  }

  if (hns_search_conversion::IsHnsSearchConversionFeatureEnabled() ||
      base::FeatureList::IsEnabled(hns_search_conversion::features::kNTP)) {
