...
std::string GetHnsSearchProviderSyncGUID(PrefService* prefs) {
  auto data = TemplateURLPrepopulateData::GetPrepopulatedEngine(
      prefs, TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_HNS);
  DCHECK(data);
  return data->sync_guid;
}

bool PrepopulatedDataHasDDG(PrefService* prefs) {
  static constexpr TemplateURLPrepopulateData::HnsPrepopulatedEngineID
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  // Check Hns Search is default provider for private window.
  ...
  HnsProfileResetter resetter(profile);
  ...
  const int default_provider_id =
      TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_HNS_TOR;
...
IN_PROC_BROWSER_TEST_F(ExtensionBrowserTest,
  ...
  // Check Hns Search is back to as a default provider for private window
