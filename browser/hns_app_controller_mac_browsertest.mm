...
#include "hns/browser/hns_app_controller_mac.h"
...
class HnsAppControllerBrowserTest : public InProcessBrowserTest {
 public:
  HnsAppControllerBrowserTest() {
...
class HnsAppControllerCleanLinkFeatureDisabledBrowserTest
    : public InProcessBrowserTest {
 public:
  HnsAppControllerCleanLinkFeatureDisabledBrowserTest() {
    features_.InitWithFeatureState(features::kHnsCopyCleanLinkByDefault,
                                   false);
  }
...
IN_PROC_BROWSER_TEST_F(HnsAppControllerCleanLinkFeatureDisabledBrowserTest,
                       CopyLinkItemVisible) {
  ...
  HnsBrowserView* browser_view = static_cast<HnsBrowserView*>(
  ...
  EXPECT_TRUE(HnsBrowserWindow::From(browser()->window())->HasSelectedURL());

  HnsAppController* ac = base::apple::ObjCCastStrict<HnsAppController>(
...
IN_PROC_BROWSER_TEST_F(HnsAppControllerBrowserTest, CopyLinkItemVisible) {
  ASSERT_TRUE(embedded_test_server()->Start());
  GURL url = embedded_test_server()->GetURL(kTestingPage);
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_EQ(1u, chrome::GetTotalBrowserCount());
...
IN_PROC_BROWSER_TEST_F(HnsAppControllerBrowserTest, CopyLinkItemNotVisible) {
  EXPECT_EQ(1u, chrome::GetTotalBrowserCount());
...
IN_PROC_BROWSER_TEST_F(HnsAppControllerBrowserTest,
  ...
  EXPECT_FALSE(HnsBrowserWindow::From(browser()->window())->HasSelectedURL());
