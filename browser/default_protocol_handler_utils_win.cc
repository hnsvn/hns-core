...
std::wstring GetBrowserProgId() {
  base::FilePath hns_exe;
  if (!base::PathService::Get(base::FILE_EXE, &hns_exe)) {
    LOG(ERROR) << "Error getting app exe path";
    return std::wstring();
  }

  const auto suffix = ShellUtil::GetCurrentInstallationSuffix(hns_exe);
  std::wstring hns_html =
  ...
  if (ShellUtil::GetUserSpecificRegistrySuffix(&new_style_suffix) &&
      suffix == new_style_suffix && hns_html.length() > 39) {
    NOTREACHED();
    hns_html.erase(39);
  }
  return hns_html;
