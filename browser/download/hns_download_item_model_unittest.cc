...
class HnsDownloadItemModelTest : public testing::Test {
 public:
  HnsDownloadItemModelTest() : model_(&item_), hns_model_(&model_) {}
  ~HnsDownloadItemModelTest() override = default;
 ...
 protected:
  ...
  HnsDownloadItemModel& model() { return hns_model_; }
...
TEST_F(HnsDownloadItemModelTest, GetOriginUrlText) {
...
TEST_F(HnsDownloadItemModelTest, GetTooltipText) {
...
TEST_F(HnsDownloadItemModelTest, NoDownloadItem) {
  ...
  HnsDownloadItemModel hns_model(&model);
  bool is_secure = false;
  EXPECT_STREQ(
      "", base::UTF16ToUTF8(hns_model.GetOriginURLText(&is_secure)).c_str());
}
