...
const char* TypeToString(NTPBackgroundPrefs::Type type) {
  ...
  switch (type) {
    case NTPBackgroundPrefs::Type::kHns:
      return "hns";
...
NTPBackgroundPrefs::Type StringToType(const std::string& type_string) {
  // See class description for details.
  if (type_string == "hns")
    return NTPBackgroundPrefs::Type::kHns;
...
void NTPBackgroundPrefs::RegisterPref(
  ...
  dict.Set(kTypeKey, TypeToString(Type::kHns));
...
bool NTPBackgroundPrefs::IsHnsType() const {
  return GetType() == Type::kHns;
}
...
bool NTPBackgroundPrefs::ShouldUseRandomValue() const {
  ...
  return optional_bool.value_or(IsHnsType());
}
