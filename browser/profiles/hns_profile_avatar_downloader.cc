...
HnsProfileAvatarDownloader::HnsProfileAvatarDownloader(
    size_t icon_index,
    FetchCompleteCallback callback)
    : ProfileAvatarDownloader(icon_index, base::BindOnce(DummyCallback)),
      callback_(std::move(callback)) {}

void HnsProfileAvatarDownloader::Start() {
  std::move(callback_).Run(gfx::Image(), "", base::FilePath());
}

HnsProfileAvatarDownloader::~HnsProfileAvatarDownloader() = default;
