...
class HnsProfileManagerTest : public PlatformBrowserTest {
 public:
  HnsProfileManagerTest() {
#if BUILDFLAG(ENABLE_IPFS)
...
// to hns profile names (Profile X).
IN_PROC_BROWSER_TEST_F(HnsProfileManagerTest,
...
// are off on x86 builds
  ...
  EXPECT_EQ(hns_rewards::RewardsServiceFactory::GetForProfile(guest_profile),
            nullptr);
  EXPECT_EQ(hns_ads::AdsServiceFactory::GetForProfile(guest_profile),
            nullptr);

  ASSERT_TRUE(otr_profile->IsOffTheRecord());

  EXPECT_NE(
      hns_rewards::RewardsServiceFactory::GetForProfile(profile), nullptr);
  EXPECT_EQ(
      hns_rewards::RewardsServiceFactory::GetForProfile(otr_profile),
      nullptr);

  EXPECT_NE(hns_ads::AdsServiceFactory::GetForProfile(profile), nullptr);
  EXPECT_EQ(hns_ads::AdsServiceFactory::GetForProfile(otr_profile),
            nullptr);
#endif
...
IN_PROC_BROWSER_TEST_F(HnsProfileManagerTest, MediaRouterEnabledRestartTest) {
  Profile* profile = g_browser_process->profile_manager()->GetLastUsedProfile();
  {
    EXPECT_TRUE(profile->GetPrefs()->GetBoolean(::prefs::kEnableMediaRouter));
    EXPECT_TRUE(profile->GetPrefs()->GetBoolean(kEnableMediaRouterOnRestart));
