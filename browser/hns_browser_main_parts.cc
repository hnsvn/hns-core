...
#include "hns/components/constants/hns_constants.h"
...
#include "hns/components/speedreader/speedreader_extended_info_handler.h"
...
void HnsBrowserMainParts::PreBrowserStart() {
#if BUILDFLAG(ENABLE_SPEEDREADER)
...
void HnsBrowserMainParts::PostBrowserStart() {
  ...
  if (browser) {
    ...
    if (active_web_contents) {
      ...
      if (profile && infobar_manager) {
        HnsConfirmP3AInfoBarDelegate::Create(
...
void HnsBrowserMainParts::PreShutdown() {
  content::HnsClearBrowsingData::ClearOnExit();
}

void HnsBrowserMainParts::PreProfileInit() {
  ChromeBrowserMainParts::PreProfileInit();
#if !BUILDFLAG(IS_ANDROID)
  auto* command_line = base::CommandLine::ForCurrentProcess();
  if (!base::FeatureList::IsEnabled(hns_sync::features::kHnsSync)) {
...
void HnsBrowserMainParts::PostProfileInit(Profile* profile,
