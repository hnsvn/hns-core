...
#include "hns/browser/browsing_data/hns_browsing_data_remover_delegate.h"
...
HnsBrowsingDataRemoverDelegate::HnsBrowsingDataRemoverDelegate(
    content::BrowserContext* browser_context)
    : ChromeBrowsingDataRemoverDelegate(browser_context),
      profile_(Profile::FromBrowserContext(browser_context)) {}

HnsBrowsingDataRemoverDelegate::~HnsBrowsingDataRemoverDelegate() = default;

void HnsBrowsingDataRemoverDelegate::RemoveEmbedderData(
  ...
  if (remove_mask & chrome_browsing_data_remover::DATA_TYPE_HISTORY) {
    hns_news::HnsNewsControllerFactory::GetForContext(profile_)
        ->ClearHistory();
  }
}

void HnsBrowsingDataRemoverDelegate::ClearShieldsSettings(
  ...
  auto* provider =
      static_cast<content_settings::HnsPrefProvider*>(map->GetPrefProvider());
...
#if BUILDFLAG(ENABLE_IPFS)
void HnsBrowsingDataRemoverDelegate::WaitForIPFSRepoGC(
...

  {
    ...
    // Because we set maximum IPFS storage size as 1GB in Hns, ipfs repo gc
...
// data.
void HnsBrowsingDataRemoverDelegate::ClearIPFSCache() {
  auto* service =
  ...
  base::ThreadPool::PostTaskAndReply(
      ...
      base::BindOnce(&HnsBrowsingDataRemoverDelegate::WaitForIPFSRepoGC,
