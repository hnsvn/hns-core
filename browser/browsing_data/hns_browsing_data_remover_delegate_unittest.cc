...
class HnsBrowsingDataRemoverDelegateTest : public testing::Test {
 public:
  void SetUp() override {
  ...
  HnsBrowsingDataRemoverDelegate* delegate() {
    return static_cast<HnsBrowsingDataRemoverDelegate*>(
...
TEST_F(HnsBrowsingDataRemoverDelegateTest, ShieldsSettingsClearTest) {
  const GURL kHnsURL("https://www.hns.com");
  ...
  map()->SetContentSettingDefaultScope(
      kHnsURL, GURL(), ContentSettingsType::HNS_HTTP_UPGRADABLE_RESOURCES,
      CONTENT_SETTING_ALLOW);
  map()->SetContentSettingDefaultScope(
      kBatURL, GURL(), ContentSettingsType::HNS_FINGERPRINTING_V2,
      CONTENT_SETTING_ALLOW);
  map()->SetContentSettingCustomScope(
      hns_shields::GetPatternFromURL(kGoogleURL),
