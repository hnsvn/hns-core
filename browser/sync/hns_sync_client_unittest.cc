...
class HnsSyncClientTest : public testing::Test {
 public:
  HnsSyncClientTest() = default;
  ~HnsSyncClientTest() override = default;
 ...
 protected:
  void SetUp() override {
    ...
    SetupAdblockServiceForHnsBrowserProcess();
  }

  Profile* profile() { return profile_.get(); }

 private:
  void SetupAdblockServiceForHnsBrowserProcess();
...
void HnsSyncClientTest::SetupAdblockServiceForHnsBrowserProcess() {
  ...
  auto adblock_service = std::make_unique<hns_shields::AdBlockService>(
      ...
      std::make_unique<hns_shields::AdBlockSubscriptionServiceManager>(
          local_state_->Get(),
          base::SingleThreadTaskRunner::GetCurrentDefault(),
...
TEST_F(HnsSyncClientTest, CreateDataTypeControllersSearchEngines) {
  auto sync_client =
...
TEST_F(HnsSyncClientTest, PrefSyncedDefaultSearchProviderGUIDIsSyncable) {
