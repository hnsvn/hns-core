...
class HnsTranslateBrowserTest : public InProcessBrowserTest {
 public:
  HnsTranslateBrowserTest() {
    ...
    https_server_->RegisterRequestHandler(base::BindRepeating(
        &HnsTranslateBrowserTest::HandleRequest, base::Unretained(this)));
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // Remap translate.hns.com requests to the https test server.
    const std::string host_port = https_server_->host_port_pair().ToString();
    command_line->AppendSwitchASCII(network::switches::kHostResolverRules,
                                    "MAP translate.hns.com:443 " + host_port +
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    if (request.GetURL().path() == "/translate") {
      ...
      EXPECT_NE(query.find(base::StringPrintf("&key=%s",
                ...
                std::string::npos)
          << "bad hns api key for request " << request.GetURL();
    }
...
IN_PROC_BROWSER_TEST_F(HnsTranslateBrowserTest, InternalTranslation) {
  ResetObserver();
...
IN_PROC_BROWSER_TEST_F(HnsTranslateBrowserTest, NoAutoTranslate) {
...
class HnsTranslateBrowserGoogleRedirectTest
    : public HnsTranslateBrowserTest {
 public:
  void SetUpCommandLine(base::CommandLine* command_line) override {
    HnsTranslateBrowserTest::SetUpCommandLine(command_line);
    const std::string host_port = https_server_->host_port_pair().ToString();
...
IN_PROC_BROWSER_TEST_F(HnsTranslateBrowserGoogleRedirectTest,
  ...
  EXPECT_EQ("https://translate.hns.com/something.svg",
