...
#include "hns/components/tor/mock_tor_launcher_factory.h"
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest,
  ...
  EXPECT_EQ(hns_rewards::RewardsServiceFactory::GetForProfile(tor_profile),
            nullptr);
  EXPECT_EQ(hns_ads::AdsServiceFactory::GetForProfile(tor_profile), nullptr);
#if BUILDFLAG(ENABLE_IPFS)
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest,
  ...
  const GURL hns_url("https://www.hns.com");
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ASSERT_TRUE(profile_manager);
  ...
  ContentSetting setting = GetScriptSetting(tor_content_settings, hns_url);
  ...
  setting = GetScriptSetting(tor_content_settings, hns_url);
  ...
  SetScriptSetting(tor_content_settings,
                   ContentSettingsPattern::FromURL(hns_url),
                   CONTENT_SETTING_BLOCK);
  setting = GetScriptSetting(parent_content_settings, hns_url);
  EXPECT_EQ(setting, CONTENT_SETTING_ALLOW);
  setting = GetScriptSetting(tor_content_settings, hns_url);
  EXPECT_EQ(setting, CONTENT_SETTING_BLOCK);
}
...
class TorProfileManagerExtensionTest : public extensions::ExtensionBrowserTest {
 public:
  void SetUpOnMainThread() override {
    ...
    base::PathService::Get(hns::DIR_TEST_DATA, &test_data_dir_);
