...
class HnsTorTest : public InProcessBrowserTest {
 public:
  struct TorInfo {
  ...
  HnsTorTest() {
    ...
    HnsSettingsUI::ShouldDisableCSPForTesting() = true;
    HnsSettingsUI::ShouldExposeElementsForTesting() = true;
  }

  ~HnsTorTest() override {
    HnsSettingsUI::ShouldDisableCSPForTesting() = false;
    HnsSettingsUI::ShouldExposeElementsForTesting() = false;
  }
  ...
  void WaitForUpdaterThread(hns_component_updater::HnsComponent* updater) {
...
IN_PROC_BROWSER_TEST_F(HnsTorTest, OpenCloseDisableTorWindow) {
  ...
  // Disable tor, expect executables are removed.
  {
    ...
    WaitForUpdaterThread(g_hns_browser_process->tor_client_updater());
...
class HnsTorTestWithCustomProfile : public HnsTorTest {
 private:
...
IN_PROC_BROWSER_TEST_F(HnsTorTestWithCustomProfile, PRE_SetupBridges) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
  DownloadTorClient();
  ...
  EXPECT_TRUE(
      g_hns_browser_process->tor_pluggable_transport_updater()->IsReady());
  ...
  WaitForUpdaterThread(
      g_hns_browser_process->tor_pluggable_transport_updater());
}

IN_PROC_BROWSER_TEST_F(HnsTorTestWithCustomProfile, SetupBridges) {
...
IN_PROC_BROWSER_TEST_F(HnsTorTestWithCustomProfile, Incognito) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
...
IN_PROC_BROWSER_TEST_F(HnsTorTestWithCustomProfile, Autofill) {
  GURL fake_url("http://hns.com/");
  // Disable autofill in private windows.
  browser()->profile()->GetPrefs()->SetBoolean(kHnsAutofillPrivateWindows,
                                               false);
  auto* tor_profile = OpenTorWindow();
  EXPECT_NE(nullptr, tor_profile);
  EXPECT_TRUE(tor_profile->IsTor());
...
IN_PROC_BROWSER_TEST_F(HnsTorTest, PRE_ResetBridges) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
  DownloadTorClient();
  DownloadTorPluggableTransports();
...
IN_PROC_BROWSER_TEST_F(HnsTorTest, ResetBridges) {
...
IN_PROC_BROWSER_TEST_F(HnsTorTest, HttpAllowlistIsolation) {
  // Normal window
...
class HnsTorTest_EnableTorHttpsOnlyFlag
    : public HnsTorTest,
      public ::testing::WithParamInterface<bool> {
 public:
  HnsTorTest_EnableTorHttpsOnlyFlag() {
    if (IsHnsHttpsByDefaultEnabled()) {
      std::vector<base::test::FeatureRef> enabled_features{
          net::features::kHnsTorWindowsHttpsOnly};
      std::vector<base::test::FeatureRef> disabled_features;
      if (IsHnsHttpsByDefaultEnabled()) {
        enabled_features.push_back(net::features::kHnsHttpsByDefault);
      } else {
        disabled_features.push_back(net::features::kHnsHttpsByDefault);
      }
  ...
  ~HnsTorTest_EnableTorHttpsOnlyFlag() override = default;

  bool IsHnsHttpsByDefaultEnabled() { return GetParam(); }

 protected:
  base::test::ScopedFeatureList scoped_feature_list_;
};

IN_PROC_BROWSER_TEST_P(HnsTorTest_EnableTorHttpsOnlyFlag,
                       TorWindowHttpsOnly) {
  EXPECT_FALSE(TorProfileServiceFactory::IsTorDisabled(browser()->profile()));
  DownloadTorClient();
...
INSTANTIATE_TEST_SUITE_P(HnsTorTest_EnableTorHttpsOnlyFlag,
                         HnsTorTest_EnableTorHttpsOnlyFlag,
                         ::testing::Bool());
