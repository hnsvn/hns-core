...
#include "hns/browser/hns_shields/ad_block_service_browsertest.h"
...
#include "hns/components/hns_shields/browser/ad_block_engine.h"
...
using hns_shields::features::kHnsAdblockCnameUncloaking;
using hns_shields::features::kHnsAdblockCollapseBlockedElements;
using hns_shields::features::kHnsAdblockCosmeticFiltering;
using hns_shields::features::kHnsAdblockDefault1pBlocking;
using hns_shields::features::kHnsAdblockScriptletDebugLogs;
using hns_shields::features::kCosmeticFilteringJsPerformance;
...
AdBlockServiceTest::AdBlockServiceTest()
  ...
  hns_shields::SetDefaultAdBlockComponentIdAndBase64PublicKeyForTest(
...
void AdBlockServiceTest::SetUpOnMainThread() {
  ...
  hns_shields::SetCosmeticFilteringControlType(
      content_settings(), hns_shields::ControlType::BLOCK, GURL());
}
...
void AdBlockServiceTest::TearDownOnMainThread() {
  ...
  hns::SetAdblockCnameHostResolverForTesting(nullptr);
...
void AdBlockServiceTest::UpdateAdBlockInstanceWithRules(
  ...
  auto source_provider =
      std::make_unique<hns_shields::TestFiltersProvider>(rules, resources);
...
void AdBlockServiceTest::UpdateAdBlockInstanceWithDAT(
  ...
  auto source_provider = std::make_unique<hns_shields::TestFiltersProvider>(
...
void AdBlockServiceTest::AssertTagExists(const std::string& tag,
  ...
  g_hns_browser_process->ad_block_service()->TagExistsForTest(
      tag, base::BindOnce(
...
class EngineTestObserver : public hns_shields::AdBlockEngine::TestObserver {
 public:
  // Constructs an EngineTestObserver which will observe the given adblock
  // engine for filter data updates.
  explicit EngineTestObserver(hns_shields::AdBlockEngine* engine)
      : engine_(engine) {
 ...
 private:
  ...
  raw_ptr<hns_shields::AdBlockEngine> engine_ = nullptr;
};
...
bool AdBlockServiceTest::InstallRegionalAdBlockExtension(
  ...
  auto* default_engine =
      g_hns_browser_process->ad_block_service()->default_engine_.get();
  EngineTestObserver default_engine_observer(default_engine);
  default_engine_observer.Wait();

  base::FilePath test_data_dir;
  GetTestDataDir(&test_data_dir);
  std::vector<hns_shields::FilterListCatalogEntry> filter_list_catalog;
  filter_list_catalog.push_back(hns_shields::FilterListCatalogEntry(
  ...
  if (enable_list) {
    ...
    auto* regional_engine = g_hns_browser_process->ad_block_service()
...
void AdBlockServiceTest::WaitForAdBlockServiceThreads() {
  scoped_refptr<base::ThreadTestHelper> tr_helper(new base::ThreadTestHelper(
      g_hns_browser_process->ad_block_service()->GetTaskRunner()));
...
void AdBlockServiceTest::DisableAggressiveMode() {
  hns_shields::SetCosmeticFilteringControlType(
      content_settings(), hns_shields::ControlType::BLOCK_THIRD_PARTY,
      GURL());
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceEngineUpdateCountTest,
  ...
  histogram_tester_.ExpectTotalCount(
      "Hns.Adblock.MakeEngineWithRules.Default", 1);
  histogram_tester_.ExpectTotalCount(
      "Hns.Adblock.MakeEngineWithRules.Additional", 1);

  // Loads the default list first, then the additional cookie list.
  ASSERT_TRUE(
      InstallRegionalAdBlockExtension(hns_shields::kCookieListUuid, true));
  ...
  histogram_tester_.ExpectTotalCount(
      "Hns.Adblock.MakeEngineWithRules.Default", 2);
  histogram_tester_.ExpectTotalCount(
      "Hns.Adblock.MakeEngineWithRules.Additional", 2);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SubFrameShieldsOff) {
  ...
  content::RunAllTasksUntilIdle();
  EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  hns_shields::ResetHnsShieldsEnabled(content_settings(), url);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ServiceWorkerRequest) {
  ...
  // https://github.com/hnsvn/hns-browser/issues/14087
  // EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 1ULL);
}
...
// https://github.com/hnsvn/hns-browser/issues/15912
#if BUILDFLAG(IS_MAC)
...
class TestAdBlockSubscriptionServiceManagerObserver
 ...
 public:
  ...
  explicit TestAdBlockSubscriptionServiceManagerObserver(
      hns_shields::AdBlockSubscriptionServiceManager* sub_service_manager)
 ...
 private:
  ...
  base::RunLoop run_loop_;
  raw_ptr<hns_shields::AdBlockSubscriptionServiceManager>
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SubscribeToCustomSubscription) {
  ...
  auto* sub_service_manager = g_hns_browser_process->ad_block_service()
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  hns::SetAdblockCnameHostResolverForTesting(&resolver);
...
class CnameUncloakingFlagDisabledTest : public AdBlockServiceTest {
 public:
  CnameUncloakingFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kHnsAdblockCnameUncloaking);
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBlockTagTest) {
  ...
  UpdateAdBlockInstanceWithRules(
      base::StringPrintf("||example.com^$tag=%s",
                         hns_shields::kFacebookEmbeds)
          .c_str());
  EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  GURL tab_url = embedded_test_server()->GetURL("b.com", kAdBlockTestPage);
  g_hns_browser_process->ad_block_service()->EnableTag(
      hns_shields::kFacebookEmbeds, true);
  WaitForAdBlockServiceThreads();
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ResetPreservesTags) {
  ...
  WaitForAdBlockServiceThreads();
  UpdateAdBlockInstanceWithRules("");
  AssertTagExists(hns_shields::kFacebookEmbeds, true);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(hns_shields::kFacebookEmbeds, true);
  AssertTagExists(hns_shields::kTwitterEmbeds, true);
  AssertTagExists(hns_shields::kLinkedInEmbeds, false);

  // Toggling prefs once is reflected in the adblock client.
  prefs->SetBoolean(hns_shields::prefs::kLinkedInEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(hns_shields::kFacebookEmbeds, true);
  AssertTagExists(hns_shields::kTwitterEmbeds, true);
  AssertTagExists(hns_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(hns_shields::prefs::kFBEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(hns_shields::kFacebookEmbeds, false);
  AssertTagExists(hns_shields::kTwitterEmbeds, true);
  AssertTagExists(hns_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(hns_shields::prefs::kTwitterEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(hns_shields::kFacebookEmbeds, false);
  AssertTagExists(hns_shields::kTwitterEmbeds, false);
  AssertTagExists(hns_shields::kLinkedInEmbeds, true);

  // Toggling prefs back is reflected in the adblock client.
  prefs->SetBoolean(hns_shields::prefs::kLinkedInEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(hns_shields::kFacebookEmbeds, false);
  AssertTagExists(hns_shields::kTwitterEmbeds, false);
  AssertTagExists(hns_shields::kLinkedInEmbeds, false);

  prefs->SetBoolean(hns_shields::prefs::kFBEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(hns_shields::kFacebookEmbeds, true);
  AssertTagExists(hns_shields::kTwitterEmbeds, false);
  AssertTagExists(hns_shields::kLinkedInEmbeds, false);

  prefs->SetBoolean(hns_shields::prefs::kTwitterEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(hns_shields::kFacebookEmbeds, true);
  AssertTagExists(hns_shields::kTwitterEmbeds, true);
  AssertTagExists(hns_shields::kLinkedInEmbeds, false);
...
class CollapseBlockedElementsFlagDisabledTest : public AdBlockServiceTest {
 public:
  CollapseBlockedElementsFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kHnsAdblockCollapseBlockedElements);
  }
...
class Default1pBlockingFlagDisabledTest : public AdBlockServiceTest {
 public:
  Default1pBlockingFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kHnsAdblockDefault1pBlocking);
  }
...
class CosmeticFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  CosmeticFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kHnsAdblockCosmeticFiltering);
  }
...
IN_PROC_BROWSER_TEST_F(CosmeticFilteringPlaylistFlagEnabledTest,
  ...
  hns_shields::SetCosmeticFilteringControlType(
      content_settings(), hns_shields::ControlType::ALLOW, url);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDisabled) {
  ASSERT_TRUE(InstallDefaultAdBlockExtension());
  hns_shields::SetCosmeticFilteringControlType(
      content_settings(), hns_shields::ControlType::ALLOW, GURL());
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDynamicCustom) {
  ASSERT_TRUE(InstallDefaultAdBlockExtension());
  ASSERT_TRUE(g_hns_browser_process->ad_block_service()
...
class ScriptletDebugLogsFlagEnabledTest : public AdBlockServiceTest {
 public:
  ScriptletDebugLogsFlagEnabledTest() {
    feature_list_.InitAndEnableFeature(kHnsAdblockScriptletDebugLogs);
  }
 ...
 public:
  explicit CookieListPrefObserver(PrefService* local_state) {
    ...
    pref_change_registrar_.Add(
        hns_shields::prefs::kAdBlockRegionalFilters,
...
// Test that the `hns-adblock-default-1p-blocking` flag forces the Cookie
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ListEnabled) {
  ASSERT_TRUE(
      InstallRegionalAdBlockExtension(hns_shields::kCookieListUuid, false));

  {
    const auto lists = g_hns_browser_process->ad_block_service()
                           ->regional_service_manager()
                           ->GetRegionalLists();
  ...
  // Disable the filter list.
  {
    ...
    g_hns_browser_process->ad_block_service()
        ->regional_service_manager()
        ->EnableFilterList(hns_shields::kCookieListUuid, false);
    pref_observer.Wait();
  }
