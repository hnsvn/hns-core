...
using blink::features::kHnsRoundTimeStamps;

class HnsTimeStampRoundingRenderViewTest
    : public content::RenderViewTest,
      public ::testing::WithParamInterface<bool> {
 public:
  HnsTimeStampRoundingRenderViewTest() {}

  ~HnsTimeStampRoundingRenderViewTest() override = default;

  bool IsHnsRoundTimeStampsEnabled() { return GetParam(); }

  void SetUp() override {
    if (IsHnsRoundTimeStampsEnabled()) {
      scoped_feature_list_.InitAndEnableFeature(kHnsRoundTimeStamps);
    } else {
      scoped_feature_list_.InitAndDisableFeature(kHnsRoundTimeStamps);
    }
...
TEST_P(HnsTimeStampRoundingRenderViewTest, SynchronousApisRounded) {
  bool expect_rounded = IsHnsRoundTimeStampsEnabled();
...
INSTANTIATE_TEST_SUITE_P(HnsTimeStampRoundingRenderViewTest,
                         HnsTimeStampRoundingRenderViewTest,
                         ::testing::Bool());
