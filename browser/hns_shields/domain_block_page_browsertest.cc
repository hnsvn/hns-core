...
using hns_shields::ResetHnsShieldsEnabled;
using hns_shields::SetHnsShieldsEnabled;
using hns_shields::SetCosmeticFilteringControlType;
...
class DomainBlockTest : public DomainBlockTestBase {
 public:
  DomainBlockTest() { feature_list_.InitAndEnableFeature(kHnsDomainBlock); }
...
IN_PROC_BROWSER_TEST_F(DomainBlockTest, NoInterstitialUnlessAggressive) {
  ...
  BlockDomainByURL(url);
  SetCosmeticFilteringControlType(content_settings(), ControlType::BLOCK, url);
  SetHnsShieldsEnabled(content_settings(), false /* enable */, url);
  NavigateTo(url);
  ASSERT_FALSE(IsShowingInterstitial());
}
