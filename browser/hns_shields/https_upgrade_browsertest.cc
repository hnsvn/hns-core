...
using net::features::kHnsHttpsByDefault;
...
class HttpsUpgradeBrowserTest : public PlatformBrowserTest {
 public:
  ...
  void SetUp() override {
    feature_list_.InitWithFeatures({kHnsHttpsByDefault}, {});
  ...
  GURL RunTestCaseNavigation(bool shields_enabled,
    ...
    hns_shields::SetHnsShieldsEnabled(ContentSettings(), shields_enabled,
                                          initial_url, nullptr);
 ...
 private:
  ...
  HnsContentBrowserClient client_;
};
...
class HttpsUpgradeBrowserTest_FlagDisabled : public HttpsUpgradeBrowserTest {
 public:
  ...
  void SetUp() override {
    feature_list_.InitAndDisableFeature(kHnsHttpsByDefault);
    PlatformBrowserTest::SetUp();
  }
};
...
IN_PROC_BROWSER_TEST_F(HttpsUpgradeBrowserTest, IsolateSettings) {
  ...
  hns_shields::SetHttpsUpgradeControlType(normal_map, ControlType::ALLOW,
                                            host1);
  hns_shields::SetHttpsUpgradeControlType(incognito_map, ControlType::ALLOW,
                                            host2);
  ...
  EXPECT_EQ(ControlType::ALLOW,
            hns_shields::GetHttpsUpgradeControlType(normal_map, host1));
  EXPECT_EQ(ControlType::BLOCK_THIRD_PARTY,
            hns_shields::GetHttpsUpgradeControlType(incognito_map, host1));
  EXPECT_EQ(ControlType::BLOCK_THIRD_PARTY,
            hns_shields::GetHttpsUpgradeControlType(normal_map, host2));
  EXPECT_EQ(ControlType::ALLOW,
            hns_shields::GetHttpsUpgradeControlType(incognito_map, host2));

  // Set strict per-site settings.
  hns_shields::SetHttpsUpgradeControlType(normal_map, ControlType::BLOCK,
                                            host1);
  hns_shields::SetHttpsUpgradeControlType(incognito_map, ControlType::BLOCK,
                                            host2);
  ...
  EXPECT_EQ(ControlType::BLOCK,
            hns_shields::GetHttpsUpgradeControlType(normal_map, GURL()));
  EXPECT_EQ(ControlType::BLOCK,
            hns_shields::GetHttpsUpgradeControlType(incognito_map, GURL()));
