...
#include "hns/browser/ui/speedreader/speedreader_bubble_view.h"
...
std::u16string GetSpeedreaderData(
  ...
  for (const auto& r : resources) {
    auto text = hns_l10n::GetLocalizedResourceUTF16String(r.second);
...
void SpeedreaderTabHelper::ShowSpeedreaderBubble(
  ...
  speedreader_bubble_ = static_cast<HnsBrowserWindow*>(browser->window())
...
void SpeedreaderTabHelper::UpdateUI() {
  ...
  if (const auto* browser =
    ...
    if (!DistillStates::IsDistilled(PageDistillState())) {
      static_cast<HnsBrowserWindow*>(browser->window())
...
void SpeedreaderTabHelper::DOMContentLoaded(
  ...
  render_frame_host->ExecuteJavaScriptInIsolatedWorld(
      *kLoadScript, base::DoNothing(), ISOLATED_WORLD_ID_HNS_INTERNAL);
}
...
void SpeedreaderTabHelper::SetDocumentAttribute(const std::string& attribute,
  ...
  web_contents()->GetPrimaryMainFrame()->ExecuteJavaScriptInIsolatedWorld(
      script, base::DoNothing(), ISOLATED_WORLD_ID_HNS_INTERNAL);
}
