...
using hns_rewards::RewardsServiceImpl;
using hns_rewards::test_util::RewardsBrowserTestResponse;
...
class GreaselionServiceTest : public BaseLocalDataFilesBrowserTest {
 public:
  ...
  LocalDataFilesObserver* service() override {
    return g_hns_browser_process->greaselion_download_service();
  }
  ...
  void WaitForService() override {
    ...
    greaselion::GreaselionDownloadService* download_service =
        g_hns_browser_process->greaselion_download_service();
  ...
  void ClearRules() {
    g_hns_browser_process->greaselion_download_service()->rules()->clear();
  }
  ...
  void StartRewards() {
    ...
    https_server_.RegisterRequestHandler(
        base::BindRepeating(&hns_rewards::test_util::HandleRequest));
    ASSERT_TRUE(https_server_.Start());

    // Rewards service
    rewards_service_ = static_cast<RewardsServiceImpl*>(
        RewardsServiceFactory::GetForProfile(profile()));
    hns_rewards::test_util::StartProcess(rewards_service_);
  ...
  void WaitForAutoContributeEnabled() {
    ...
    if (prefs->GetBoolean(hns_rewards::prefs::kAutoContributeEnabled)) {
      return;
    }
...
// the parent component is updated while Hns is running.)
...
IN_PROC_BROWSER_TEST_F(GreaselionServiceTest,
  ...
  auto* prefs = browser()->profile()->GetPrefs();
  EXPECT_FALSE(prefs->GetBoolean(hns_rewards::prefs::kAutoContributeEnabled));

  // Enable auto-contribute and wait for it
  rewards_service_->SetAutoContributeEnabled(true);
  WaitForAutoContributeEnabled();

  ASSERT_TRUE(prefs->GetBoolean(hns_rewards::prefs::kAutoContributeEnabled));
}
