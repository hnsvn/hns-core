...
#include "hns/browser/web_discovery/web_discovery_cta_util.h"
...
class WebDiscoveryCTATest : public testing::Test {
 public:
  ...
  void SetHnsSearchAsDefaultProvider() {
    // Set hns search as a default provider.
    std::unique_ptr<TemplateURL> hns = CreateTestTemplateURL(
        u"hns", "https://search.hns.com/", std::string(),
        base::Time::FromTimeT(100), false, false,
        TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_HNS);
    service()->SetUserSelectedDefaultSearchProvider(hns.get());
    ASSERT_TRUE(IsHnsSearchDefault());
  }

  void SetNonHnsSearchAsDefaultProvider() {
    // Set hns search as a default provider.
    ...
    ASSERT_FALSE(IsHnsSearchDefault());
  }

  bool IsHnsSearchDefault() {
...
TEST_F(WebDiscoveryCTATest, ShouldShowInfoBarTest) {
  ...
  SetNonHnsSearchAsDefaultProvider();
  EXPECT_FALSE(ShouldShowWebDiscoveryInfoBar());

  // Set Hns as a default provider.
  SetHnsSearchAsDefaultProvider();
