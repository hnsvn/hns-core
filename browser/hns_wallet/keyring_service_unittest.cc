...
#include "hns/components/hns_wallet/browser/filecoin_keyring.h"
...
const char kPasswordHns[] = "hns";
const char kPasswordHns123[] = "hns123";
...
class KeyringServiceUnitTest : public testing::Test {
 ...
 protected:
  ...
  static absl::optional<std::string> EncodePrivateKeyForExport(
      KeyringService* service,
      mojom::AccountIdPtr account_id,
      const std::string& password = kPasswordHns) {
  ...
  static bool RestoreWallet(KeyringService* service,
                            ...
                            const std::string& password,
                            bool is_legacy_hns_wallet) {
    bool success = false;
    base::RunLoop run_loop;
    service->RestoreWallet(mnemonic, password, is_legacy_hns_wallet,
                           base::BindLambdaForTesting([&](bool v) {
                             success = v;
                             run_loop.Quit();
                           }));
...
TEST_F(KeyringServiceUnitTest, HasAndGetPrefForKeyring) {
  ...
  GetPrefs()->SetDict(kHnsWalletKeyrings, std::move(dict));
...
TEST_F(KeyringServiceUnitTest, SetPrefForKeyring) {
  KeyringService::SetPrefForKeyring(GetPrefs(), "pref1", base::Value("123"),
                                    mojom::kDefaultKeyringId);
  const auto& keyrings_pref = GetPrefs()->GetDict(kHnsWalletKeyrings);
...
TEST_F(KeyringServiceUnitTest, GetPrefInBytesForKeyring) {
  ...
  GetPrefs()->ClearPref(kHnsWalletKeyrings);
  EXPECT_FALSE(KeyringService::GetPrefInBytesForKeyring(
      *GetPrefs(), kEncryptedMnemonic, mojom::kDefaultKeyringId));

  // non-existing pref
  EXPECT_FALSE(KeyringService::GetPrefInBytesForKeyring(
      *GetPrefs(), "hns.nothinghere", mojom::kDefaultKeyringId));
...
TEST_F(KeyringServiceUnitTest, CreateDefaultKeyringInternal) {
  ...
  EXPECT_TRUE(
      service.CreateEncryptorForKeyring("hns", mojom::kDefaultKeyringId));
...
TEST_F(KeyringServiceUnitTest, RestoreDefaultKeyring) {
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  absl::optional<std::string> mnemonic = CreateWallet(&service, "hns");
  ...
  EXPECT_NE(service.RestoreKeyring(mojom::kDefaultKeyringId, *mnemonic, "hns",
                                   false),
            nullptr);
  EXPECT_EQ(
      GetStringPrefForKeyring(kEncryptedMnemonic, mojom::kDefaultKeyringId),
  ...
  EXPECT_EQ(
      service.RestoreKeyring(mojom::kDefaultKeyringId, "", "hns", false),
      nullptr);
  ...
  EXPECT_NE(service.RestoreKeyring(mojom::kDefaultKeyringId,
                                   kMnemonicDivideCruise, "hns", false),
            nullptr);
  EXPECT_NE(
      GetStringPrefForKeyring(kEncryptedMnemonic, mojom::kDefaultKeyringId),
      encrypted_mnemonic);
...
TEST_F(KeyringServiceUnitTest, UnlockResumesDefaultKeyring) {
  ...
  {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    ASSERT_TRUE(CreateWallet(&service, "hns"));
    ASSERT_TRUE(AddAccount(&service, mojom::CoinType::ETH,
  ...
  }
  {
    ...
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    EXPECT_TRUE(Unlock(&service, "hns"));
  ...
  }
  {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    // wrong password
    EXPECT_FALSE(Unlock(&service, "hns123"));
...
TEST_F(KeyringServiceUnitTest, GetMnemonicForDefaultKeyring) {
  ...
  GetPrefs()->SetBoolean(kHnsWalletKeyringEncryptionKeysMigrated, true);
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  ASSERT_TRUE(service.CreateEncryptorForKeyring(kPasswordHns,
                                                mojom::kDefaultKeyringId));

  // no pref exists yet
  EXPECT_TRUE(GetMnemonicForDefaultKeyring(kPasswordHns, &service).empty());

  ASSERT_TRUE(service.CreateKeyringInternal(mojom::kDefaultKeyringId,
                                            kMnemonicDivideCruise, false));
  EXPECT_EQ(GetMnemonicForDefaultKeyring(kPasswordHns, &service),
  ...
  ASSERT_FALSE(Unlock(&service, kPasswordHns123));
  ASSERT_TRUE(service.IsLocked(mojom::kDefaultKeyringId));

  EXPECT_TRUE(GetMnemonicForDefaultKeyring(kPasswordHns, &service).empty());

  ASSERT_TRUE(Unlock(&service, kPasswordHns));
  ASSERT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));

  // Can only get mnemonic when password is correct.
  EXPECT_TRUE(
      GetMnemonicForDefaultKeyring(kPasswordHns123, &service).empty());
  EXPECT_EQ(GetMnemonicForDefaultKeyring(kPasswordHns, &service),
            kMnemonicDivideCruise);
}
...
TEST_F(KeyringServiceUnitTest, ValidatePassword) {
  ...
  EXPECT_TRUE(ValidatePassword(&service, "hns"));
  EXPECT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));
  EXPECT_FALSE(ValidatePassword(&service, "hns123"));
  EXPECT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));
...
TEST_F(KeyringServiceUnitTest, LockAndUnlock) {
  ...
  feature_list.InitWithFeatures({features::kHnsWalletFilecoinFeature,
                                 features::kHnsWalletSolanaFeature},
                                {});
...
TEST_F(KeyringServiceUnitTest, Reset) {
  ...
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletKeyrings));
  EXPECT_TRUE(
      HasPrefForKeyring(kPasswordEncryptorSalt, mojom::kDefaultKeyringId));
  EXPECT_TRUE(
      HasPrefForKeyring(kPasswordEncryptorNonce, mojom::kDefaultKeyringId));
  ...
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletKeyrings));
  EXPECT_FALSE(service.GetHDKeyringById(mojom::kDefaultKeyringId));
  ASSERT_TRUE(service.encryptors_.empty());
...
TEST_F(KeyringServiceUnitTest, AccountMetasForKeyring) {
  base::test::ScopedFeatureList feature_list{
      features::kHnsWalletFilecoinFeature};

  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());

  ASSERT_TRUE(RestoreWallet(&service, kMnemonicDivideCruise, "hns", false));
...
TEST_F(KeyringServiceUnitTest, CreateAndRestoreWallet) {
  ...
  enabled_features.emplace_back(features::kHnsWalletSolanaFeature,
                                parameters);
  ...
  auto verify_restore_wallet = base::BindLambdaForTesting(
      [&mnemonic_to_be_restored, &service, &address0]() {
        EXPECT_TRUE(
            RestoreWallet(&service, *mnemonic_to_be_restored, "hns1", false));
        {
          std::vector<mojom::AccountInfoPtr> account_infos =
              service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
          EXPECT_EQ(account_infos.size(), 1u);
...
TEST_F(KeyringServiceUnitTest, DefaultSolanaAccountCreated) {
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  ASSERT_NE(CreateWallet(&service, "hns"), absl::nullopt);
...
TEST_F(KeyringServiceUnitTest, MigrationPrefs) {
  GetPrefs()->SetDict(kHnsWalletKeyrings,
                      GetHardwareKeyringValueForTesting());
  EXPECT_EQ(*GetPrefs()
                 ->GetDict(kHnsWalletKeyrings)
...
TEST_F(KeyringServiceUnitTest, SetDefaultKeyringImportedAccountName) {
  ...
  ASSERT_TRUE(CreateWallet(&service, kPasswordHns));
...
TEST_F(KeyringServiceUnitTest, RestoreLegacyHnsWallet) {
  ...
  auto verify_restore_wallet = base::BindLambdaForTesting(
      [&service](const char* mnemonic, const char* address, bool is_legacy,
                 bool expect_result) {
        if (expect_result) {
          EXPECT_TRUE(RestoreWallet(&service, mnemonic, "hns1", is_legacy));
          std::vector<mojom::AccountInfoPtr> account_infos =
              service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
          ...
          // legacy_hns_wallet pref so it will use the right seed
          service.Lock();
          EXPECT_TRUE(Unlock(&service, "hns1"));
          account_infos.clear();
          account_infos =
              service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
          ASSERT_EQ(account_infos.size(), 1u);
  ...
  // hns legacy menmonic can only be 24 words
...
TEST_F(KeyringServiceUnitTest, HardwareAccounts) {
  ...
  for (const auto& account : accounts) {
    ...
    ASSERT_TRUE(
        GetPrefs()->GetDict(kHnsWalletKeyrings).FindByDottedPath(path));
  }
  {
...
TEST_F(KeyringServiceUnitTest, AutoLock) {
  ...
  service.Reset();
  ASSERT_TRUE(RestoreWallet(&service, *mnemonic, "hns", false));
  ASSERT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));
  ...
  ASSERT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));
  task_environment_.FastForwardBy(base::Minutes(4));
  GetPrefs()->SetInteger(kHnsWalletAutoLockMinutes, 3);
  ...
  task_environment_.FastForwardBy(base::Minutes(2));
  EXPECT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));
  GetPrefs()->SetInteger(kHnsWalletAutoLockMinutes, 10);
  task_environment_.FastForwardBy(base::Minutes(9));
  EXPECT_FALSE(service.IsLocked(mojom::kDefaultKeyringId));
  task_environment_.FastForwardBy(base::Minutes(1));
  EXPECT_TRUE(service.IsLocked(mojom::kDefaultKeyringId));
...
TEST_F(KeyringServiceUnitTest, SetSelectedAccount) {
  ...
  EXPECT_CALL(observer, SelectedDappAccountChanged(mojom::CoinType::SOL, _))
      .Times(0);
  EXPECT_TRUE(
      RemoveAccount(&service, imported_account->account_id, kPasswordHns));
  EXPECT_TRUE(Lock(&service));
  EXPECT_EQ(first_account, service.GetSelectedWalletAccount());
  EXPECT_EQ(first_account, service.GetSelectedEthereumDappAccount());
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, SignMessageByDefaultKeyring) {
  ...
  EXPECT_EQ(
      ...
      l10n_util::GetStringFUTF8(IDS_HNS_WALLET_SIGN_MESSAGE_INVALID_ADDRESS,
  ...
  EXPECT_EQ(
      sig_with_err.error_message,
      l10n_util::GetStringUTF8(IDS_HNS_WALLET_SIGN_MESSAGE_UNLOCK_FIRST));
}
...
TEST_F(KeyringServiceUnitTest, ImportFilecoinAccounts) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndEnableFeature(features::kHnsWalletFilecoinFeature);
...
TEST_F(KeyringServiceUnitTest, PreCreateEncryptors) {
  ...
  }
  {
    ...
    local_feature_list.InitWithFeatures({features::kHnsWalletFilecoinFeature,
                                         features::kHnsWalletSolanaFeature},
                                        {});
  ...
  }
  {
    ...
    ASSERT_TRUE(Unlock(&service, "hns"));
    EXPECT_NE(service.encryptors_.at(mojom::kDefaultKeyringId), nullptr);
    EXPECT_NE(service.encryptors_.at(mojom::kFilecoinKeyringId), nullptr);
    EXPECT_NE(service.encryptors_.at(mojom::kSolanaKeyringId), nullptr);
    service.Reset();
  }
  {
    ...
    service.Reset();
    ASSERT_TRUE(
        RestoreWallet(&service, *mnemonic_to_be_restored, "hns", false));
    EXPECT_NE(service.encryptors_.at(mojom::kDefaultKeyringId), nullptr);
    EXPECT_NE(service.encryptors_.at(mojom::kSolanaKeyringId), nullptr);
    EXPECT_NE(service.encryptors_.at(mojom::kFilecoinKeyringId), nullptr);
    ...
    local_enabled_features.emplace_back(features::kHnsWalletSolanaFeature,
                                        local_parameters);
    local_enabled_features.emplace_back(features::kHnsWalletFilecoinFeature,
...
class KeyringServiceAccountDiscoveryUnitTest : public KeyringServiceUnitTest {
 public:
  ...
  void PrepareAccounts(mojom::CoinType coin_type, mojom::KeyringId keyring_id) {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    saved_mnemonic_ = CreateWallet(&service, "hns").value_or("");
...
TEST_F(KeyringServiceAccountDiscoveryUnitTest, AccountDiscovery) {
  ...
  EXPECT_TRUE(RestoreWallet(&service, saved_mnemonic(), "hns1", false));
  observer.WaitAndVerify();
  std::vector<mojom::AccountInfoPtr> account_infos =
      service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
...
class KeyringServiceEncryptionKeysMigrationUnitTest
 ...
 public:
  KeyringServiceEncryptionKeysMigrationUnitTest() {
    feature_list_.InitWithFeatures({features::kHnsWalletFilecoinFeature,
                                    features::kHnsWalletSolanaFeature},
                                   {});
  }
  ...
  void SetupKeyring() {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    saved_mnemonic_ = *CreateWallet(&service, "hns");
...
TEST_F(KeyringServiceEncryptionKeysMigrationUnitTest, NoMigration) {
  EXPECT_FALSE(
      GetPrefs()->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated));
  SetupKeyring();
  EXPECT_TRUE(
      GetPrefs()->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated));
...
TEST_F(KeyringServiceEncryptionKeysMigrationUnitTest, MigrateWithUnlock) {
  ...
  EXPECT_TRUE(
      GetPrefs()->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated));
  GetPrefs()->ClearPref(kHnsWalletKeyringEncryptionKeysMigrated);
  EXPECT_FALSE(
      GetPrefs()->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated));
  ...
  EXPECT_FALSE(Unlock(&service, "hns"));
  KeyringService::GetPbkdf2IterationsForTesting() = absl::nullopt;
...
TEST_F(KeyringServiceEncryptionKeysMigrationUnitTest,
  ...
  // Restore wallet.
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  EXPECT_TRUE(RestoreWallet(&service, saved_mnemonic(), "hns", false));
  EXPECT_TRUE(
      GetPrefs()->GetBoolean(kHnsWalletKeyringEncryptionKeysMigrated));
...
TEST_F(KeyringServiceEncryptionKeysMigrationUnitTest,
  ...
  // Restore wallet.
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  EXPECT_TRUE(RestoreWallet(&service, saved_mnemonic(), "hns123", false));
...
TEST_F(KeyringServiceEncryptionKeysMigrationUnitTest, NoMigrationAfterResote) {
  ...
  // Restore wallet.
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  EXPECT_TRUE(RestoreWallet(&service, kMnemonicDivideCruise, "hns", false));
...
TEST_F(KeyringServiceUnitTest, AccountsAdded) {
  ...
  RestoreWallet(&service, kMnemonicDivideCruise, kPasswordHns, false);
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, BitcoinReceiveChangeAddresses) {
  ...
  base::test::ScopedFeatureList feature_list{
      features::kHnsWalletBitcoinFeature};
...
TEST_F(KeyringServiceUnitTest, MigrateSelectedAccount) {
  ...
  ASSERT_TRUE(
      RestoreWallet(service.get(), kMnemonicDivideCruise, "hns", false));
  ...
  GetPrefs()->ClearPref(kHnsWalletSelectedWalletAccount);
  GetPrefs()->ClearPref(kHnsWalletSelectedSolDappAccount);
  GetPrefs()->ClearPref(kHnsWalletSelectedEthDappAccount);
  GetPrefs()->SetInteger(kHnsWalletSelectedCoinDeprecated,
                         static_cast<int>(mojom::CoinType::FIL));
  auto keyrings = GetPrefs()->GetDict(kHnsWalletKeyrings).Clone();
  ...
  GetPrefs()->SetDict(kHnsWalletKeyrings, std::move(keyrings));
  ...
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletSelectedCoinDeprecated));
