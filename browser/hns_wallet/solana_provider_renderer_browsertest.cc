...
#include "hns/components/hns_wallet/renderer/resource_helper.h"
...
// IDR_HNS_WALLET_SOLANA_WEB3_JS_FOR_TEST is excluded from Android build to
// save space. Ensure this test is not build on Android. If it will be required
// to run these tests on Android, include again
// IDR_HNS_WALLET_SOLANA_WEB3_JS_FOR_TEST
static_assert(!BUILDFLAG(IS_ANDROID));

using hns_wallet::mojom::SolanaProviderError;
...
constexpr char OnAccountChangedScript[] =
    R"(async function disconnect() {await window.hnsSolana.disconnect()}
       new Promise(resolve => {
...
constexpr char CheckSolanaProviderScript[] = "!!window.hnsSolana";
...
std::string ConnectScript(const std::string& args) {
  return base::StringPrintf(
      R"(async function connect() {
          try {
            const result = await window.hnsSolana.connect(%s);
...
std::string SignTransactionScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function signTransaction() {
          try {
            const result = await window.hnsSolana.signTransaction%s
...
std::string SignAllTransactionsScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function signAllTransactions() {
          try {
            const result = await window.hnsSolana.signAllTransactions%s
            const isSameTx =
...
std::string SignAndSendTransactionScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function signAndSendTransaction() {
          try {
            const result = await window.hnsSolana.signAndSendTransaction%s
...
std::string SignMessageScript(const std::string& args) {
  std::vector<uint8_t> signature(hns_wallet::kSolanaSignatureSize);
  EXPECT_TRUE(hns_wallet::Base58Decode(std::string(kTestSignature),
  ...
  return base::StringPrintf(
      R"(async function signMessage() {
          try {
            const result = await window.hnsSolana.signMessage%s
            if (JSON.stringify(result) === JSON.stringify(%s))
              return true;
            else
              return false;
...
std ::string RequestScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function request() {
          try {
            const result = await window.hnsSolana.request%s
            if (JSON.stringify(result) === JSON.stringify(%s))
              return true;
...
class TestSolanaProvider final : public hns_wallet::mojom::SolanaProvider {
 public:
  ...
  void Init(mojo::PendingRemote<hns_wallet::mojom::SolanaEventsListener>
  ...
  void SignTransaction(hns_wallet::mojom::SolanaSignTransactionParamPtr param,
                       SignTransactionCallback callback) override {
    EXPECT_EQ(param->encoded_serialized_msg,
              hns_wallet::Base58Encode(kSerializedMessage));
    if (error_ == SolanaProviderError::kSuccess) {
  ...
  void SignAllTransactions(
      std::vector<hns_wallet::mojom::SolanaSignTransactionParamPtr> params,
    ...
    if (error_ == SolanaProviderError::kSuccess) {
      ...
      std::move(callback).Run(
          ...
          std::vector<hns_wallet::mojom::SolanaMessageVersion>());
      ClearError();
    }
  }
  void SignAndSendTransaction(
      hns_wallet::mojom::SolanaSignTransactionParamPtr param,
 ...
 private:
  ...
  mojo::Remote<hns_wallet::mojom::SolanaEventsListener> events_listener_;
};

class TestHnsContentBrowserClient : public HnsContentBrowserClient {
 public:
  TestHnsContentBrowserClient() = default;
  ~TestHnsContentBrowserClient() override = default;
  TestHnsContentBrowserClient(const TestHnsContentBrowserClient&) = delete;
  TestHnsContentBrowserClient& operator=(
      const TestHnsContentBrowserClient&) = delete;

  void RegisterBrowserInterfaceBindersForFrame(
      content::RenderFrameHost* render_frame_host,
      mojo::BinderMapWithContext<content::RenderFrameHost*>* map) override {
    HnsContentBrowserClient::RegisterBrowserInterfaceBindersForFrame(
        render_frame_host, map);
    // override binding for SolanaProvider
    map->Add<hns_wallet::mojom::SolanaProvider>(
        base::BindRepeating(&TestHnsContentBrowserClient::BindSolanaProvider,
 ...
 private:
  void BindSolanaProvider(
    ...
    provider->set_connection_error_handler(
        base::BindOnce(&TestHnsContentBrowserClient::OnDisconnect,
  ...
  base::flat_map<
      ...
      mojo::SelfOwnedReceiverRef<hns_wallet::mojom::SolanaProvider>>
      provider_map_;
  base::WeakPtrFactory<TestHnsContentBrowserClient> weak_ptr_factory_{this};
};
...
class SolanaProviderRendererTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    // TestHnsContentBrowserClient::RegisterBrowserInterfaceBindersForFrame
    ASSERT_TRUE(
    ...
    if (g_provider_solana_web3_script->empty()) {
      *g_provider_solana_web3_script = hns_wallet::LoadDataResource(
          IDR_HNS_WALLET_SOLANA_WEB3_JS_FOR_TEST);
    }
  }
 ...
 protected:
  ...
  TestHnsContentBrowserClient test_content_browser_client_;
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest,
  ...
  std::string command = "window.solana.isHnsWallet";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, AttachIfWalletCreated) {
  ...
  constexpr char kEvalIsHnsWallet[] = "window.solana.isHnsWallet";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, NonWritable) {
  for (const std::string& provider : {"hnsSolana", "solana"}) {
    // window.hnsSolana.* and window.solana.* (methods)
    ...
    // window.hnsSolana.* and window.solana.* (properties)
    for (const std::string& property :
         {"isPhantom", "isHnsWallet", "isConnected", "publicKey"}) {
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, IsPhantomAndIsHnsWallet) {
  ASSERT_TRUE(ExecJs(web_contents(browser()),
                     "window.hnsSolana.isPhantom = 123; "
                     "window.hnsSolana.isHnsWallet = 456"));
  // Both are non-writable
  auto result1 =
      EvalJs(web_contents(browser()), "window.hnsSolana.isPhantom");
  EXPECT_EQ(base::Value(true), result1.value);
  auto result2 =
      EvalJs(web_contents(browser()), "window.hnsSolana.isHnsWallet");
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, OnConnect) {
  auto result =
      EvalJs(web_contents(browser()),
             R"(async function connect() {await window.hnsSolana.connect()}
                new Promise(resolve => {
                  window.hnsSolana.on(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, IsConnected) {
  auto result =
      EvalJs(web_contents(browser()), "window.hnsSolana.isConnected");
  EXPECT_EQ(base::Value(true), result.value);
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, GetPublicKey) {
  auto result = EvalJs(web_contents(browser()),
                       "window.hnsSolana.publicKey.toString()");
  EXPECT_EQ(base::Value(kTestPublicKey), result.value);
}
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, NonConfigurable) {
  ...
  EXPECT_TRUE(content::EvalJs(web_contents(browser()),
                              NonConfigurableScript("hnsSolana"))
                  .ExtractBool());
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, Iframe3P) {
  constexpr char kEvalSolanaUndefined[] =
      R"(typeof window.hnsSolana === 'undefined')";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, SecureContextOnly) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  constexpr char kEvalSolana[] = "typeof window.hnsSolana !== 'undefined'";
  content::RenderFrameHost* main_frame =
      web_contents(browser())->GetPrimaryMainFrame();
