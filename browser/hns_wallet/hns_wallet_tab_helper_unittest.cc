...
class HnsWalletTabHelperUnitTest : public BrowserWithTestWindowTest {
 public:
  HnsWalletTabHelperUnitTest() = default;
  ~HnsWalletTabHelperUnitTest() override = default;
...
TEST_F(HnsWalletTabHelperUnitTest, GetApproveBubbleURL) {
  auto* helper = hns_wallet_tab_helper();
  ASSERT_TRUE(helper);
  ASSERT_EQ(helper->GetApproveBubbleURL(),
            GURL("chrome://wallet-panel.top-chrome/#approveTransaction"));
}

TEST_F(HnsWalletTabHelperUnitTest, ChooserTitle) {
  auto wallet_label = HnsCreateTitleLabel();
  EXPECT_EQ(GetHIDTitle(web_contents(), GURL(kHnsUIWalletPanelURL)),
            wallet_label);
  EXPECT_EQ(GetHIDTitle(web_contents(), GURL(kHnsUIWalletPageURL)),
            wallet_label);
  EXPECT_NE(GetHIDTitle(web_contents(), GURL("a.com")), wallet_label);
}

TEST_F(HnsWalletTabHelperUnitTest, SolanaConnectedAccount) {
  auto* helper = hns_wallet_tab_helper();
  ASSERT_TRUE(helper);
