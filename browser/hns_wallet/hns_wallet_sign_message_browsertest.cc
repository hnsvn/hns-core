...
class HnsWalletSignMessageBrowserTest : public InProcessBrowserTest {
 public:
  HnsWalletSignMessageBrowserTest()
      : https_server_(net::EmbeddedTestServer::TYPE_HTTPS) {
    scoped_feature_list_.InitAndEnableFeature(
        hns_wallet::features::kNativeHnsWalletFeature);
  }

  ~HnsWalletSignMessageBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(HnsWalletSignMessageBrowserTest, UserApprovedRequest) {
  RestoreWallet();
...
IN_PROC_BROWSER_TEST_F(HnsWalletSignMessageBrowserTest, UserRejectedRequest) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(HnsWalletSignMessageBrowserTest, UnknownAddress) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(HnsWalletSignMessageBrowserTest, InvalidAddressParam) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(HnsWalletSignMessageBrowserTest, NoEthPermission) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
