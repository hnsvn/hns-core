...
namespace {
int GetStatusTitle(hns_wallet::mojom::TransactionStatus status) {
  switch (status) {
    case hns_wallet::mojom::TransactionStatus::Confirmed:
      return IDS_WALLET_TRANSACTION_STATUS_UPDATE_MESSAGE_TITLE_CONFIRMED;
    case hns_wallet::mojom::TransactionStatus::Error:
      return IDS_WALLET_TRANSACTION_STATUS_UPDATE_MESSAGE_TITLE_ERROR;
    case hns_wallet::mojom::TransactionStatus::Dropped:
...
void PushNotification(content::BrowserContext* context,
  ...
  auto notification = CreateMessageCenterNotification(
      title, body, uuid,
      GURL("hns://wallet/crypto/accounts/" + from + "#" + uuid));
