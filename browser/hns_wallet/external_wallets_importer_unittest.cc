...
TEST_F(ExternalWalletsImporterUnitTest, OnGetImportInfo) {
  ...
  ImportInfo info;
  ImportError error;
  SimulateGetImportInfo("hns4ever", valid_data, &result, &info, &error);
  EXPECT_TRUE(result);
...
TEST_F(ExternalWalletsImporterUnitTest, OnGetImportInfo_UTF8Mnemonic) {
  ...
  ImportInfo info;
  ImportError error;
  SimulateGetImportInfo("hns4ever", valid_data_with_utf8_mnemonic, &result,
                        &info, &error);
  EXPECT_TRUE(result);
  EXPECT_EQ(error, ImportError::kNone);
  EXPECT_EQ(info.mnemonic, valid_mnemonic);
...
TEST_F(ExternalWalletsImporterUnitTest, ImportLegacyWallet) {
  ...
  ImportInfo info;
  ImportError error;
  SimulateGetImportInfo("bbhnsy", valid_legacy_data, &result, &info, &error);
  EXPECT_TRUE(result);
  EXPECT_EQ(error, ImportError::kNone);
