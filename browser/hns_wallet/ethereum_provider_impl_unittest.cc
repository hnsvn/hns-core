...
#include "hns/components/hns_wallet/browser/eth_nonce_tracker.h"
#include "hns/components/hns_wallet/browser/eth_pending_tx_tracker.h"
#include "hns/components/hns_wallet/browser/eth_tx_manager.h"
...
class TestEventsListener : public hns_wallet::mojom::EventsListener {
 public:
  ...
  mojo::PendingRemote<hns_wallet::mojom::EventsListener> GetReceiver() {
 ...
 private:
  mojo::Receiver<hns_wallet::mojom::EventsListener> observer_receiver_{this};
};
...
class EthereumProviderImplUnitTest : public testing::Test {
 public:
  ...
  void TearDown() override {
    ...
    HnsWalletServiceDelegateImpl::SetActiveWebContentsForTesting(nullptr);
  }
  ...
  void SetUp() override {
    ...
    HnsWalletServiceDelegateImpl::SetActiveWebContentsForTesting(
        web_contents_.get());
    ...
    profile_.SetPermissionControllerDelegate(
        base::WrapUnique(static_cast<permissions::HnsPermissionManager*>(
    ...
    provider_ = std::make_unique<EthereumProviderImpl>(
        ...
        keyring_service(), hns_wallet_service_,
  ...
  void RestoreWallet(const std::string& mnemonic,
    ...
    keyring_service_->RestoreWallet(
        mnemonic, password, is_legacy_hns_wallet,
  ...
  void Unlock() {
    ...
    keyring_service_->Unlock(
        "hns", base::BindLambdaForTesting([&run_loop](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
    run_loop.Run();
  ...
  void CreateHnsWalletTabHelper() {
    hns_wallet::HnsWalletTabHelper::CreateForWebContents(
        web_contents_.get());
  }
  hns_wallet::HnsWalletTabHelper* hns_wallet_tab_helper() {
  ...
  void AddEthereumPermission(const mojom::AccountIdPtr& account_id) {
    base::RunLoop run_loop;
    hns_wallet_service_->AddPermission(
        account_id.Clone(), base::BindLambdaForTesting([&](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
  ...
  void ResetEthereumPermission(const mojom::AccountIdPtr& account_id) {
    base::RunLoop run_loop;
    hns_wallet_service_->ResetPermission(
        account_id.Clone(), base::BindLambdaForTesting([&](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
  ...
  void SignMessageHardware(bool user_approved,
    ...
    browser_task_environment_.RunUntilIdle();
    hns_wallet_service_->NotifySignMessageRequestProcessed(
        user_approved, hns_wallet_service_->sign_message_id_ - 1,
  ...
  void SignMessage(absl::optional<bool> user_approved,
    ...
    if (user_approved) {
      hns_wallet_service_->NotifySignMessageRequestProcessed(
          *user_approved, hns_wallet_service_->sign_message_id_ - 1, nullptr,
          absl::nullopt);
    }
    run_loop.Run();
  }
  ...
  GetPendingGetEncryptionPublicKeyRequests() const {
    ...
    hns_wallet_service_->GetPendingGetEncryptionPublicKeyRequests(
        base::BindLambdaForTesting(
  ...
  std::vector<mojom::DecryptRequestPtr> GetPendingDecryptRequests() const {
    ...
    hns_wallet_service_->GetPendingDecryptRequests(base::BindLambdaForTesting(
  ...
  void GetEncryptionPublicKey(const std::string& address,
    ...
    if (requests.size() > 0) {
      ...
      EXPECT_TRUE(hns_wallet_tab_helper()->IsShowingBubble());
      hns_wallet_service_->NotifyGetPublicKeyRequestProcessed(
    ...
    *unsafe_message = "";
    ...
    if (requests.size() > 0) {
      ...
      EXPECT_EQ(requests[0]->account_id,
                GetAccountUtils().FindAccountIdByAddress(address));
      EXPECT_TRUE(hns_wallet_tab_helper()->IsShowingBubble());
      hns_wallet_service_->NotifyDecryptRequestProcessed(
          requests[0]->request_id, approved);
    }
    run_loop.Run();
  }
  ...
  void AddSuggestToken(mojom::BlockchainTokenPtr token,
    ...
    if (!token) {
      ...
      EXPECT_TRUE(hns_wallet_tab_helper()->IsShowingBubble());
      hns_wallet_service_->NotifyAddSuggestTokenRequestsProcessed(
  ...
  GetPendingAddSuggestTokenRequests() const {
    ...
    hns_wallet_service_->GetPendingAddSuggestTokenRequests(
        base::BindLambdaForTesting(
 ...
 protected:
  ...
  raw_ptr<HnsWalletService> hns_wallet_service_ = nullptr;
...
TEST_F(EthereumProviderImplUnitTest, EmptyDelegate) {
  EthereumProviderImpl provider_impl(
      host_content_settings_map(), json_rpc_service(), tx_service(),
      keyring_service(), hns_wallet_service_, nullptr, prefs());
...
TEST_F(EthereumProviderImplUnitTest, RequestEthereumPermissionsWithAccounts) {
  ...
  host_content_settings_map()->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::HNS_ETHEREUM, CONTENT_SETTING_BLOCK);
  base::RunLoop run_loop;
  provider()->RequestEthereumPermissions(
      base::BindLambdaForTesting([&](base::Value id,
  ...
  // again.
  host_content_settings_map()->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::HNS_ETHEREUM, CONTENT_SETTING_DEFAULT);
  EXPECT_THAT(RequestEthereumPermissions(), ElementsAre(address_0, address_1));
}
...
TEST_F(EthereumProviderImplUnitTest, SignTypedMessage) {
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringFUTF8(
                IDS_HNS_WALLET_SIGN_TYPED_MESSAGE_CHAIN_ID_MISMATCH,
  ...
  SignTypedMessage(absl::nullopt, address_0, "{...}", DecodeHexHash("hns"),
                   primary_hash, domain.Clone(), &signature, &error,
                   &error_message);
  EXPECT_TRUE(signature.empty());
  EXPECT_EQ(error, mojom::ProviderError::kInvalidParams);
...
TEST_F(EthereumProviderImplUnitTest, SignMessageRequestQueue) {
  ...
  // wrong order
  hns_wallet_service_->NotifySignMessageRequestProcessed(true, id2, nullptr,
                                                           absl::nullopt);
  EXPECT_EQ(GetSignMessageQueueSize(), 3u);
  EXPECT_EQ(GetSignMessageQueueFront()->id, id1);
  EXPECT_EQ(GetSignMessageQueueFront()->message, message1_in_queue);

  hns_wallet_service_->NotifySignMessageRequestProcessed(true, id3, nullptr,
                                                           absl::nullopt);
  EXPECT_EQ(GetSignMessageQueueSize(), 3u);
  EXPECT_EQ(GetSignMessageQueueFront()->id, id1);
  EXPECT_EQ(GetSignMessageQueueFront()->message, message1_in_queue);

  hns_wallet_service_->NotifySignMessageRequestProcessed(true, id1, nullptr,
                                                           absl::nullopt);
...
TEST_F(EthereumProviderImplUnitTest, AccountsChangedEvent) {
  ...
  Navigate(GURL("https://hnssoftware.com"));
  AddEthereumPermission(account_1->account_id);
  SetSelectedAccount(account_0->account_id);
  EXPECT_FALSE(observer_->AccountsChangedFired());
}
...
TEST_F(EthereumProviderImplUnitTest, Web3ClientVersion) {
  std::string expected_version = base::StringPrintf(
      "HnsWallet/v%s", version_info::GetHnsChromiumVersionNumber().c_str());
  std::string version;
  mojom::ProviderError error = mojom::ProviderError::kUnknown;
  std::string error_message;
...
TEST_F(EthereumProviderImplUnitTest, SwitchEthereumChain) {
  // CreateWallet();
  CreateHnsWalletTabHelper();
  Navigate(GURL("https://hnssoftware.com"));
  hns_wallet_tab_helper()->SetSkipDelegateForTesting(true);
  mojom::ProviderError error = mojom::ProviderError::kUnknown;
  std::string error_message;
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringFUTF8(IDS_WALLET_UNKNOWN_CHAIN,
                                      base::ASCIIToUTF16(chain_id)));
  EXPECT_FALSE(hns_wallet_tab_helper()->IsShowingBubble());
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
  EXPECT_TRUE(hns_wallet_tab_helper()->IsShowingBubble());
  hns_wallet_tab_helper()->CloseBubble();
  EXPECT_FALSE(hns_wallet_tab_helper()->IsShowingBubble());
...
TEST_F(EthereumProviderImplUnitTest, AddEthereumChainSwitchesForInnactive) {
  ...
  std::string params = R"({"params": [{
        "chainId": "0x5",
        "chainName": "Goerli",
        "rpcUrls": ["https://goerli-infura.hns.com/"]
      }]})";
  base::RunLoop run_loop;
  provider()->AddEthereumChain(
      params,
...
TEST_F(EthereumProviderImplUnitTest, GetEncryptionPublicKey) {
  RestoreWallet(kMnemonicDivideCruise, "hns", false);
  auto account_0 = GetAccountUtils().EnsureEthAccount(0);
