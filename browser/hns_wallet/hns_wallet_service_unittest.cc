...
#include "hns/components/hns_wallet/browser/blockchain_list_parser.h"
...
class HnsWalletServiceUnitTest : public testing::Test {
 public:
  HnsWalletServiceUnitTest()
      : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME),
        shared_url_loader_factory_(
            base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(
                &url_loader_factory_)) {}

  ~HnsWalletServiceUnitTest() override = default;
 ...
 protected:
  void SetUp() override {
    ...
    service_ = std::make_unique<HnsWalletService>(
        shared_url_loader_factory_,
        HnsWalletServiceDelegate::Create(profile_.get()), keyring_service_,
        json_rpc_service_, tx_service_, GetPrefs(), local_state_->Get());
    observer_ = std::make_unique<TestHnsWalletServiceObserver>();
  ...
  GURL GetNetwork(const std::string& chain_id, mojom::CoinType coin) {
    return hns_wallet::GetNetworkURL(GetPrefs(), chain_id, coin);
  }
  ...
  void AddSuggestToken(mojom::BlockchainTokenPtr suggested_token,
    ...
    mojom::AddSuggestTokenRequestPtr request =
        mojom::AddSuggestTokenRequest::New(
            MakeOriginInfo(url::Origin::Create(GURL("https://hns.com"))),
  ...
  std::unique_ptr<HnsWalletService> service_;
  raw_ptr<KeyringService> keyring_service_ = nullptr;
  raw_ptr<JsonRpcService> json_rpc_service_;
  raw_ptr<TxService> tx_service_;
  std::unique_ptr<TestHnsWalletServiceObserver> observer_;
...
TEST_F(HnsWalletServiceUnitTest, GetUserAssets) {
  bool success = false;
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(HnsWalletServiceUnitTest, DefaultAssets) {
...
TEST_F(HnsWalletServiceUnitTest, AddUserAsset) {
  bool success = false;
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(HnsWalletServiceUnitTest, AddUserAssetNfts) {
  bool success = false;
...
TEST_F(HnsWalletServiceUnitTest, RemoveUserAsset) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
  mojom::BlockchainTokenPtr token2 = GetToken2();
  mojom::BlockchainTokenPtr token1_0x1 = GetToken1();
  token1_0x1->chain_id = "0x1";
...
TEST_F(HnsWalletServiceUnitTest, SetUserAssetVisible) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
  mojom::BlockchainTokenPtr token2 = GetToken2();
  mojom::BlockchainTokenPtr token1_0x1 = GetToken1();
  token1_0x1->chain_id = "0x1";
...
TEST_F(HnsWalletServiceUnitTest, SetAssetSpamStatus) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
...
TEST_F(HnsWalletServiceUnitTest, GetChecksumAddress) {
...
TEST_F(HnsWalletServiceUnitTest, GetAndSetDefaultEthereumWallet) {
  SetDefaultEthereumWallet(mojom::DefaultWallet::HnsWallet);
  EXPECT_EQ(GetDefaultEthereumWallet(), mojom::DefaultWallet::HnsWallet);
  ...
  SetDefaultEthereumWallet(mojom::DefaultWallet::HnsWalletPreferExtension);
...
TEST_F(HnsWalletServiceUnitTest, GetAndSetDefaultSolanaWallet) {
  SetDefaultSolanaWallet(mojom::DefaultWallet::HnsWallet);
  EXPECT_EQ(GetDefaultSolanaWallet(), mojom::DefaultWallet::HnsWallet);

  SetDefaultSolanaWallet(mojom::DefaultWallet::None);
  EXPECT_EQ(GetDefaultSolanaWallet(), mojom::DefaultWallet::None);

  SetDefaultSolanaWallet(mojom::DefaultWallet::HnsWalletPreferExtension);
...
TEST_F(HnsWalletServiceUnitTest, GetAndSetDefaultBaseCurrency) {
...
TEST_F(HnsWalletServiceUnitTest, GetAndSetDefaultBaseCryptocurrency) {
...
TEST_F(HnsWalletServiceUnitTest, EthAddRemoveSetUserAssetVisible) {
  mojom::BlockchainTokenPtr eth_0xaa36a7_token = GetEthToken();
  eth_0xaa36a7_token->chain_id = "0xaa36a7";
  bool success = false;
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(HnsWalletServiceUnitTest, NetworkListChangedEvent) {
  ...
  observer_->Reset();
  {
    ScopedDictPrefUpdate update(GetPrefs(), kHnsWalletCustomNetworks);
...
TEST_F(HnsWalletServiceUnitTest,
...
TEST_F(HnsWalletServiceUnitTest, ERC721TokenAddRemoveSetUserAssetVisible) {
  bool success = false;
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(HnsWalletServiceUnitTest, SolanaTokenUserAssetsAPI) {
  bool success = false;
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(HnsWalletServiceUnitTest, MigrateUserAssetsDefaultPrefs) {
  EXPECT_FALSE(
      GetPrefs()->GetBoolean(kHnsWalletUserAssetEthContractAddressMigrated));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletUserAssetsDeprecated));
  HnsWalletService::MigrateUserAssetEthContractAddress(GetPrefs());
  HnsWalletService::MigrateMultichainUserAssets(GetPrefs());
  EXPECT_TRUE(
      GetPrefs()->GetBoolean(kHnsWalletUserAssetEthContractAddressMigrated));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletUserAssetsDeprecated));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletUserAssets));
}

TEST_F(HnsWalletServiceUnitTest, MigrateUserAssetEthContractAddress) {
  EXPECT_FALSE(
      GetPrefs()->GetBoolean(kHnsWalletUserAssetEthContractAddressMigrated));

  {
    ScopedDictPrefUpdate update(GetPrefs(), kHnsWalletUserAssetsDeprecated);
  ...
  const auto& pref = GetPrefs()->GetDict(kHnsWalletUserAssetsDeprecated);
...
TEST_F(HnsWalletServiceUnitTest, MigrateMultichainUserAssets) {
  ASSERT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletUserAssetsDeprecated));
  ...
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletUserAssetsDeprecated));
  HnsWalletService::MigrateMultichainUserAssets(GetPrefs());

  const auto& assets = GetPrefs()->GetDict(kHnsWalletUserAssets);
  ...
  EXPECT_EQ(*solana_dict, HnsWalletService::GetDefaultSolanaAssets());

  const auto* filecoin_dict = assets.FindDict("filecoin");
  ASSERT_TRUE(filecoin_dict);
  EXPECT_EQ(*filecoin_dict, HnsWalletService::GetDefaultFilecoinAssets());

  EXPECT_FALSE(GetPrefs()->HasPrefPath(kHnsWalletUserAssetsDeprecated));
}

TEST_F(HnsWalletServiceUnitTest, MigrateUserAssetsAddPreloadingNetworks) {
  ASSERT_FALSE(GetPrefs()->GetBoolean(
      kHnsWalletUserAssetsAddPreloadingNetworksMigrated));
  ...
  GetPrefs()->Set(kHnsWalletUserAssets, *user_assets_value);

  ASSERT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletUserAssets));
  HnsWalletService::MigrateUserAssetsAddPreloadingNetworks(GetPrefs());

  auto wtrtl = mojom::BlockchainToken::New(
      "0x6a31Aca4d2f7398F04d9B6ffae2D898d9A8e7938", "WTRTL",
      "https://hns.com/logo.jpg", true, false, false, false, false, "WTRTL",
...
TEST_F(HnsWalletServiceUnitTest, MigrateUserAssetsAddIsNFT) {
  ASSERT_FALSE(GetPrefs()->GetBoolean(kHnsWalletUserAssetsAddIsNFTMigrated));
  ...
  GetPrefs()->Set(kHnsWalletUserAssets, *user_assets_value);
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletUserAssets));
  HnsWalletService::MigrateUserAssetsAddIsNFT(GetPrefs());
  ...
  ASSERT_TRUE(user_assets_value);
  EXPECT_EQ(GetPrefs()->GetValue(kHnsWalletUserAssets), *user_assets_value);

  EXPECT_TRUE(GetPrefs()->GetBoolean(kHnsWalletUserAssetsAddIsNFTMigrated));
}

TEST_F(HnsWalletServiceUnitTest, MigrateUserAssetsAddIsSpam) {
  ASSERT_FALSE(GetPrefs()->GetBoolean(kHnsWalletUserAssetsAddIsSpamMigrated));
  ...
  GetPrefs()->Set(kHnsWalletUserAssets, user_assets_value);
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletUserAssets));
  HnsWalletService::MigrateUserAssetsAddIsSpam(GetPrefs());
  ...
  EXPECT_TRUE(GetPrefs()->GetBoolean(kHnsWalletUserAssetsAddIsSpamMigrated));
}

TEST_F(HnsWalletServiceUnitTest, MigradeDefaultHiddenNetworks) {
  ASSERT_EQ(GetPrefs()->GetInteger(kHnsWalletDefaultHiddenNetworksVersion),
            0);
  HnsWalletService::MigrateHiddenNetworks(GetPrefs());
  {
    auto* list =
        GetPrefs()->GetDict(kHnsWalletHiddenNetworks).FindList("ethereum");
...
TEST_F(HnsWalletServiceUnitTest, MigradeDefaultHiddenNetworks_NoList) {
  ASSERT_EQ(GetPrefs()->GetInteger(kHnsWalletDefaultHiddenNetworksVersion),
            0);
  {
    ScopedDictPrefUpdate update(GetPrefs(), kHnsWalletHiddenNetworks);
...
TEST_F(HnsWalletServiceUnitTest, MigrateUserAssetsAddIsERC1155) {
  ASSERT_FALSE(
      GetPrefs()->GetBoolean(kHnsWalletUserAssetsAddIsERC1155Migrated));
  ...
  GetPrefs()->Set(kHnsWalletUserAssets, *user_assets_value);
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletUserAssets));
  HnsWalletService::MigrateUserAssetsAddIsERC1155(GetPrefs());

  // Add `"is_erc1155": false` key/values to the expected kHnsWalletUserAssets
...
TEST_F(HnsWalletServiceUnitTest, MigrateFantomMainnetAsCustomNetwork) {
  // CASE 1: Fantom is the selected network of some origin
  ASSERT_FALSE(
      GetPrefs()->GetBoolean(kHnsWalletCustomNetworksFantomMainnetMigrated));
  ...
  GetPrefs()->Set(kHnsWalletSelectedNetworksPerOrigin, *selected_networks);

  EXPECT_FALSE(CustomChainExists(GetPrefs(), "0xfa", mojom::CoinType::ETH));

  HnsWalletService::MigrateFantomMainnetAsCustomNetwork(GetPrefs());
  ...
  GetPrefs()->SetBoolean(kHnsWalletCustomNetworksFantomMainnetMigrated,
                         false);
  GetPrefs()->ClearPref(kHnsWalletCustomNetworks);
  GetPrefs()->ClearPref(kHnsWalletSelectedNetworksPerOrigin);

  auto default_networks = base::JSONReader::Read(R"({
    "ethereum": "0xfa"
  })");
  GetPrefs()->Set(kHnsWalletSelectedNetworks, *default_networks);
  ...
  EXPECT_EQ(
      *GetPrefs()->GetDict(kHnsWalletSelectedNetworks).FindString("ethereum"),
      "0xfa");
  ...
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletCustomNetworks));
  auto* custom_networks =
      GetPrefs()->GetDict(kHnsWalletCustomNetworks).FindList("ethereum");
...
TEST_F(HnsWalletServiceUnitTest, OnGetImportInfo) {
  const char* new_password = "hns1234!";
  bool success;
  std::string error_message;
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringUTF8(IDS_HNS_WALLET_IMPORT_JSON_ERROR));
...
TEST_F(HnsWalletServiceUnitTest, SignMessageHardware) {
  SetupWallet();
...
TEST_F(HnsWalletServiceUnitTest, SignMessage) {
  SetupWallet();
...
TEST_F(HnsWalletServiceUnitTest, AddSuggestToken) {
...
TEST_F(HnsWalletServiceUnitTest, GetUserAsset) {
...
TEST_F(HnsWalletServiceUnitTest, Reset) {
  SetupWallet();
  ...
  AddUserAsset(token1.Clone(), &success);
  EXPECT_TRUE(success);
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kHnsWalletUserAssets));
  ...
  const auto page_storage_key_callback_valid =
      [&](StoragePartition::StorageKeyMatcherFunction callback) {
        return callback.Run(blink::StorageKey::CreateFirstParty(
            url::Origin::Create(GURL(kHnsUIWalletURL))));
      };
  const auto panel_storage_key_callback_valid =
      [&](StoragePartition::StorageKeyMatcherFunction callback) {
        return callback.Run(blink::StorageKey::CreateFirstParty(
            url::Origin::Create(GURL(kHnsUIWalletPanelURL))));
      };
...
TEST_F(HnsWalletServiceUnitTest, GetUserAssetAddress) {
  // Native asset
  EXPECT_EQ(
      *HnsWalletService::GetUserAssetAddress("", mojom::CoinType::ETH, "0x1"),
      "");
  EXPECT_EQ(*HnsWalletService::GetUserAssetAddress("", mojom::CoinType::SOL,
                                                     mojom::kSolanaMainnet),
            "");
  EXPECT_EQ(
      *HnsWalletService::GetUserAssetAddress("", mojom::CoinType::FIL, "f"),
      "");

  // ETH
  EXPECT_EQ(*HnsWalletService::GetUserAssetAddress(
                "0x6b175474e89094c44da98b954eedeac495271d0f",
  ...
  EXPECT_EQ(*HnsWalletService::GetUserAssetAddress(
            ...
            "AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM");
  EXPECT_EQ(HnsWalletService::GetUserAssetAddress("not_base58_encoded_string",
                                                    mojom::CoinType::SOL,
                                                    mojom::kSolanaMainnet),
            absl::nullopt);
...
TEST_F(HnsWalletServiceUnitTest, NewUserReturningMetric) {
  histogram_tester_->ExpectBucketCount(
      kHnsWalletNewUserReturningHistogramName, 0, 1);
  GetLocalState()->SetTime(kHnsWalletLastUnlockTime, base::Time::Now());

  task_environment_.FastForwardBy(base::Days(1));
  histogram_tester_->ExpectBucketCount(
      kHnsWalletNewUserReturningHistogramName, 2, 2);

  GetLocalState()->SetTime(kHnsWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectBucketCount(
      kHnsWalletNewUserReturningHistogramName, 3, 1);

  task_environment_.FastForwardBy(base::Days(6));
  histogram_tester_->ExpectBucketCount(
      kHnsWalletNewUserReturningHistogramName, 1, 1);
}

TEST_F(HnsWalletServiceUnitTest, NewUserReturningMetricMigration) {
  GetLocalState()->SetTime(kHnsWalletLastUnlockTime, base::Time::Now());

  task_environment_.RunUntilIdle();
  GetLocalState()->SetTime(kHnsWalletP3AFirstUnlockTime, base::Time());
  GetLocalState()->SetTime(kHnsWalletP3ALastUnlockTime, base::Time());
  ...
  task_environment_.FastForwardBy(base::Hours(30));
  histogram_tester_->ExpectBucketCount(
      kHnsWalletNewUserReturningHistogramName, 1, 2);
}

TEST_F(HnsWalletServiceUnitTest, LastUsageTimeMetric) {
  histogram_tester_->ExpectTotalCount(kHnsWalletLastUsageTimeHistogramName,
                                      0);

  GetLocalState()->SetTime(kHnsWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectUniqueSample(kHnsWalletLastUsageTimeHistogramName,
                                        1, 1);

  task_environment_.FastForwardBy(base::Days(7));

  histogram_tester_->ExpectBucketCount(kHnsWalletLastUsageTimeHistogramName,
                                       2, 1);
...
TEST_F(HnsWalletServiceUnitTest, GetNftDiscoveryEnabled) {
...
TEST_F(HnsWalletServiceUnitTest, SetNftDiscoveryEnabled) {
  // Default should be off
  EXPECT_FALSE(GetPrefs()->GetBoolean(kHnsWalletNftDiscoveryEnabled));
  ...
  EXPECT_TRUE(GetPrefs()->GetBoolean(kHnsWalletNftDiscoveryEnabled));
...
TEST_F(HnsWalletServiceUnitTest, RecordGeneralUsageMetrics) {
  histogram_tester_->ExpectTotalCount(kHnsWalletMonthlyHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kHnsWalletWeeklyHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kHnsWalletDailyHistogramName, 0);

  GetLocalState()->SetTime(kHnsWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectUniqueSample(kHnsWalletMonthlyHistogramName, 1, 1);
  histogram_tester_->ExpectUniqueSample(kHnsWalletWeeklyHistogramName, 1, 1);
  histogram_tester_->ExpectUniqueSample(kHnsWalletDailyHistogramName, 1, 1);
  ...
  histogram_tester_->ExpectUniqueSample(kHnsWalletMonthlyHistogramName, 1, 2);
  histogram_tester_->ExpectUniqueSample(kHnsWalletWeeklyHistogramName, 1, 2);
  histogram_tester_->ExpectUniqueSample(kHnsWalletDailyHistogramName, 1, 2);
}

TEST_F(HnsWalletServiceUnitTest, GetBalanceScannerSupportedChains) {
...
TEST_F(HnsWalletServiceUnitTest, GetSimpleHashSpamNFTs) {
  ...
  GURL url = GURL(
      "https://simplehash.wallet.hns.com/api/v0/nfts/"
...
TEST_F(HnsWalletServiceUnitTest, EnsureSelectedAccountForChain) {
  const char* new_password = "hns1234!";
  bool success;
  std::string error_message;
  const char* valid_mnemonic =
...
TEST_F(HnsWalletServiceUnitTest, ConvertFEVMToFVMAddress) {
  {
