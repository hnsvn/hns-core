...
class BlockchainImagesSourceTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    base::PathService::Get(hns::DIR_TEST_DATA, &test_dir);
    base::FilePath wallet_dir = test_dir.AppendASCII("wallet");
    // hns/test/data/wallet/1.0.1
    hns_wallet::SetLastInstalledWalletVersionForTest(base::Version("1.0.1"));
...
TEST_F(BlockchainImagesSourceTest, GetMimeType) {
  EXPECT_EQ(source()->GetMimeType(GURL("hns://test/img1.png")), "image/png");
  EXPECT_EQ(source()->GetMimeType(GURL("hns://test/img1.gif")), "image/gif");
  EXPECT_EQ(source()->GetMimeType(GURL("hns://test/img1.jpg")), "image/jpg");
  EXPECT_EQ(source()->GetMimeType(GURL("hns://test/img1.svg")),
            "image/svg+xml");
}
