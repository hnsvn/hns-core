...
class TestHnsWalletServiceObserverForAssetDiscoveryManager
    : public hns_wallet::HnsWalletServiceObserverBase {
 public:
  TestHnsWalletServiceObserverForAssetDiscoveryManager() = default;
...
class AssetDiscoveryManagerUnitTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    wallet_service_ = std::make_unique<HnsWalletService>(
        shared_url_loader_factory_,
    ...
    wallet_service_observer_ = std::make_unique<
        TestHnsWalletServiceObserverForAssetDiscoveryManager>();
  ...
  std::unique_ptr<TestHnsWalletServiceObserverForAssetDiscoveryManager>
      wallet_service_observer_;
  content::BrowserTaskEnvironment task_environment_;
  std::unique_ptr<ScopedTestingLocalState> local_state_;
  std::unique_ptr<TestingProfile> profile_;
  std::unique_ptr<HnsWalletService> wallet_service_;
...
TEST_F(AssetDiscoveryManagerUnitTest, DiscoverAssetsOnAllSupportedChains) {
  ...
  // kHnsWalletLastDiscoveredAssetsAt does not update if accounts_added set
  base::Time current_assets_last_discovered_at =
      GetPrefs()->GetTime(kHnsWalletLastDiscoveredAssetsAt);
  ...
  // Verify OnDiscoverAssetsStarted and OnDiscoverAssetsCompleted both fire and
  // kHnsWalletLastDiscoveredAssetsAt updates if accounts_added not set
...
TEST_F(AssetDiscoveryManagerUnitTest, AccountsAdded) {
  ...
  base::Time current_assets_last_discovered_at =
      GetPrefs()->GetTime(kHnsWalletLastDiscoveredAssetsAt);
  ASSERT_EQ(current_assets_last_discovered_at, base::Time());
  keyring_service_->RestoreWallet(kMnemonicDivideCruise, kPasswordHns, false,
                                  base::DoNothing());
  wallet_service_observer_->WaitForOnDiscoverAssetsCompleted({});
  base::Time previous_assets_last_discovered_at =
      current_assets_last_discovered_at;
