...
// signMessage
constexpr char kMessage[] = "hnsy baby!";
...
class SolanaProviderTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    test_data_dir = test_data_dir.AppendASCII("hns-wallet");
    ...
    if (g_provider_solana_web3_script->empty()) {
      *g_provider_solana_web3_script =
          LoadDataResource(IDR_HNS_WALLET_SOLANA_WEB3_JS_FOR_TEST);
    }
  }
  ...
  void UserGrantPermission(bool granted,
    ...
    if (granted) {
      ...
      permissions::HnsWalletPermissionContext::Cancel(web_contents());
    }
  ...
  bool GetIsHnsWalletViaProxy() {
    return EvalJs(web_contents(), "getIsHnsWalletViaProxy()").ExtractBool();
  }
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectRequestInProgress) {
  ...
  (async function() {
    try {
      return await window.hnsSolana.connect()
    } catch (err) {
      return err
    }
  })()
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectedStatusAndPermission) {
  ...
  host_content_settings_map()->ClearSettingsForOneTypeWithPredicate(
      ContentSettingsType::HNS_SOLANA, base::Time(), base::Time::Max(),
      base::BindLambdaForTesting(
          [&url, &account_0](const ContentSettingsPattern& primary_pattern,
                             const ContentSettingsPattern& secondary_pattern) {
            url::Origin new_origin;
            if (GetSubRequestOrigin(permissions::RequestType::kHnsSolana,
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, GetPublicKey) {
  ...
  constexpr char get_public_key_script[] =
      "window.hnsSolana.publicKey ? window.hnsSolana.publicKey.toString() "
      ": ''";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, SignAllTransactions) {
  ...
  EXPECT_TRUE(WaitForWalletBubble(web_contents()));
  // user rejected request
  hns_wallet_service_->NotifySignAllTransactionsRequestProcessed(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, Request) {
  ...
  hns_wallet_service_->NotifySignMessageRequestProcessed(true, 0, nullptr,
                                                           absl::nullopt);
  WaitForResultReady();
  ...
  EXPECT_TRUE(WaitForWalletBubble(web_contents()));
  hns_wallet_service_->NotifySignTransactionRequestProcessed(true, 0, nullptr,
                                                               absl::nullopt);
  WaitForResultReady();
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, CallViaProxy) {
  ...
  EXPECT_TRUE(GetIsHnsWalletViaProxy());
