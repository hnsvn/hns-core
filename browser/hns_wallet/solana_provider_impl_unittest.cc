...
#include "hns/components/hns_wallet/browser/solana_instruction.h"
#include "hns/components/hns_wallet/browser/solana_message.h"
...
class SolanaProviderImplUnitTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    hns_wallet_service_ =
        hns_wallet::HnsWalletServiceFactory::GetServiceForContext(
            browser_context());
    tx_service_ =
        hns_wallet::TxServiceFactory::GetServiceForContext(browser_context());
    ...
    provider_ = std::make_unique<SolanaProviderImpl>(
        *host_content_settings_map, keyring_service_, hns_wallet_service_,
  ...
  std::string SignMessage(
    ...
    if (run_notify) {
      hns_wallet_service_->NotifySignMessageRequestProcessed(
          approve, hns_wallet_service_->sign_message_id_ - 1,
  ...
  std::vector<uint8_t> SignTransaction(
    ...
    if (run_notify) {
      hns_wallet_service_->SetSignTransactionRequestAddedCallbackForTesting(
          base::BindLambdaForTesting([&]() {
            hns_wallet_service_->NotifySignTransactionRequestProcessed(
                approve, hns_wallet_service_->sign_transaction_id_ - 1,
                std::move(hw_sig), err_in);
          }));
    }
              ...
              base::BindLambdaForTesting([&]() {
                hns_wallet_service_
                    ->NotifySignAllTransactionsRequestProcessed(
                        approve,
                        hns_wallet_service_->sign_all_transactions_id_ - 1,
...
TEST_F(SolanaProviderImplUnitTest, Connect) {
  ...
  // permissions::HnsWalletPermissionContext::RequestPermissions failed
  ...
  map->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::HNS_SOLANA, CONTENT_SETTING_BLOCK);
  account = Connect(absl::nullopt, &error, &error_message);
  EXPECT_TRUE(account.empty());
  EXPECT_EQ(error, mojom::SolanaProviderError::kUserRejectedRequest);
  EXPECT_EQ(error_message,
  ...
  // again.
  map->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::HNS_SOLANA, CONTENT_SETTING_DEFAULT);
  account = Connect(absl::nullopt, &error, &error_message);
  EXPECT_EQ(account, added_account->address);
  EXPECT_EQ(error, mojom::SolanaProviderError::kSuccess);
  EXPECT_TRUE(error_message.empty());
...
TEST_F(SolanaProviderImplUnitTest, SignMessage) {
  ...
  EXPECT_EQ(requests[0]->message, "HNS");
  EXPECT_EQ(requests[1]->message, "HNS");
  EXPECT_EQ(requests[2]->message, "0x4252415645");
  EXPECT_EQ(requests[3]->message, "HNS");
}
