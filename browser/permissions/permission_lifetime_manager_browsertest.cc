...
#include "hns/browser/permissions/mock_permission_lifetime_prompt_factory.h"
...
#include "hns/components/ephemeral_storage/ephemeral_storage_service.h"
...
#define TYPE_CASE(TYPE)  \
  case TYPE:             \
    ...
    TYPE_CASE(ContentSettingsType::HNS_ETHEREUM)
    TYPE_CASE(ContentSettingsType::HNS_SOLANA)
...
class PermissionLifetimeManagerBrowserTest : public InProcessBrowserTest {
 public:
  ...
  HnsPermissionManager* permission_manager() {
    return static_cast<permissions::HnsPermissionManager*>(
  ...
  GURL RequestPermission(const TestCase& entry, const GURL& url) {
    if (entry.address && entry.permission) {
      ...
      url::Origin origin;
      EXPECT_TRUE(hns_wallet::GetConcatOriginFromWalletAddresses(
