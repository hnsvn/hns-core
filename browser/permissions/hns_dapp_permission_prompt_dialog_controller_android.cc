...
#include "hns/build/android/jni_headers/HnsDappPermissionPromptDialog_jni.h"
...
void HnsDappPermissionPromptDialogController::ShowDialog() {
  if (!GetOrCreateJavaObject())
    return;

  JNIEnv* env = base::android::AttachCurrentThread();
  Java_HnsDappPermissionPromptDialog_show(env, GetOrCreateJavaObject());
}

void HnsDappPermissionPromptDialogController::OnPrimaryButtonClicked(
    JNIEnv* env,
...
void HnsDappPermissionPromptDialogController::OnNegativeButtonClicked(
    JNIEnv* env) {
  delegate_->CancelConnectToSite();
}

void HnsDappPermissionPromptDialogController::OnDialogDismissed(JNIEnv* env) {
  java_object_.Reset();
  delegate_->OnDialogDismissed();
}

void HnsDappPermissionPromptDialogController::DismissDialog() {
  if (java_object_) {
    Java_HnsDappPermissionPromptDialog_dismissDialog(
        base::android::AttachCurrentThread(), java_object_);
  }
}

base::android::ScopedJavaGlobalRef<jobject>
HnsDappPermissionPromptDialogController::GetOrCreateJavaObject() {
  if (java_object_)
    return java_object_;
  ...
  return java_object_ = Java_HnsDappPermissionPromptDialog_create(
