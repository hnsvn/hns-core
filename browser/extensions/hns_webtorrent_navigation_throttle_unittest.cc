...
class MockBrowserClient : public content::ContentBrowserClient {
 public:
  ...
  // Only construct an HnsWebTorrentNavigationThrottle so that we can test it
  // in isolation.
  std::vector<std::unique_ptr<NavigationThrottle>> CreateThrottlesForNavigation(
      content::NavigationHandle* handle) override {
    std::vector<std::unique_ptr<NavigationThrottle>> throttles;
    throttles.push_back(
        std::make_unique<HnsWebTorrentNavigationThrottle>(handle));
    return throttles;
  }
};
...
class HnsWebTorrentNavigationThrottleUnitTest
    : public content::RenderViewHostTestHarness {
 public:
  HnsWebTorrentNavigationThrottleUnitTest()
      : local_state_(TestingBrowserProcess::GetGlobal()) {
  }

  HnsWebTorrentNavigationThrottleUnitTest(
      const HnsWebTorrentNavigationThrottleUnitTest&) = delete;

  HnsWebTorrentNavigationThrottleUnitTest& operator=(
      const HnsWebTorrentNavigationThrottleUnitTest&) = delete;
  ...
  void AddExtension() {
    extension_ = ExtensionBuilder()
                     ...
                     .SetID(hns_webtorrent_extension_id)
                     .Build();
...
TEST_F(HnsWebTorrentNavigationThrottleUnitTest, ExternalWebPage) {
  ...
  auto throttle =
      std::make_unique<HnsWebTorrentNavigationThrottle>(&test_handle);
...
TEST_F(HnsWebTorrentNavigationThrottleUnitTest,
...
TEST_F(HnsWebTorrentNavigationThrottleUnitTest, WebTorrentUrlInstalled) {
  AddExtension();
  web_contents_tester()->NavigateAndCommit(GURL("http://example.com"));
  content::RenderFrameHost* host =
      render_frame_host_tester(main_rfh())->AppendChild("child");
...
// disabled.
TEST_F(HnsWebTorrentNavigationThrottleUnitTest, WebTorrentDisabledByPref) {
