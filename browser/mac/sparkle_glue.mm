...
#import "hns/browser/mac/su_updater.h"
...
std::string GetUpdateChannel() {
  std::string channel_name = hns::GetChannelName();
...
- (void)registerWithSparkle {
  // This can be called by HnsBrowserMainPartsMac::PreMainMessageLoopStart()
  // again when browser is relaunched.
  if (_registered) {
    return;
  }

  DCHECK(hns::UpdateEnabled());
  DCHECK(_su_updater);
  ...
  constexpr NSTimeInterval kHnsUpdateCheckIntervalInSec = 3 * 60 * 60;
  [_su_updater setUpdateCheckInterval:kHnsUpdateCheckIntervalInSec];
...
- (void)updater:(id)updater didFinishLoadingAppcast:(id)appcast {
  VLOG(0) << "hns update: did finish loading appcast";

  [self updateStatus:kAutoupdateChecking version:nil error:nil];
}

- (void)updater:(id)updater didFindValidUpdate:(id)item {
  VLOG(0) << "hns update: did find valid update with " +
...
- (void)updaterDidNotFindUpdate:(id)updater {
  VLOG(0) << "hns update: did not find update";
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "hns update: willDownloadUpdate with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater
    failedToDownloadUpdate:(id)item
                     error:(NSError *)error {
  VLOG(0) << "hns update: failed to download update with " +
...
- (void)userDidCancelDownload:(id)updater {
  VLOG(0) << "hns update: user did cancel download";
  [self updateStatus:kAutoupdateInstallFailed
               version:nil
                 error:nil];
}
...
- (void)updater:(id)updater willInstallUpdate:(id)item {
  VLOG(0) << "hns update: will install update with " +
             GetDescriptionFromAppcastItem(item);
  [self updateStatus:kAutoupdateInstalling
             version:nil
               error:nil];
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "hns update: will install update on quit with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater didAbortWithError:(NSError *)error {
  VLOG(0) << "hns update: did abort with error: " +
...
- (NSString*)feedURLStringForUpdater:(id)__unused updater {
  ...
  return [NSString stringWithFormat:@"https://updates.hnssoftware.com/"
                                    @"sparkle/Hns-Browser/%s/appcast.xml",
