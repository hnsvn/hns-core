...
def ReportToDashboardImpl(
  ...
  build_properties['builder_group'] = 'hns.perf'
...
class RunableConfiguration:
  ...
  def RebaseProfile(self) -> bool:
    ...
    rebase_runner_config.extra_browser_args.extend(
        ['--update-source-profile', '--enable-hns-features-for-perf-testing'])

    rebase_benchmark = BenchmarkConfig()
    rebase_benchmark.name = 'loading.desktop.hns'
    rebase_benchmark.stories = ['HnsSearch_cold']
