...
def DownloadFromGoogleStorage(sha1: str, output_path: str) -> None:
  """Download a file from hns perf bucket.

  To upload a file call:
  upload_to_google_storage.py some_file.zip -b hns-telemetry
  """
  gsutil = download_from_google_storage.Gsutil(
      download_from_google_storage.GSUTIL_DEFAULT_PATH)
  gs_path = 'gs://' + path_util.GetHnsPerfBucket() + '/' + sha1
...
def GetProfilePath(profile: str, work_directory: str) -> str:
  ...
  if os.path.isdir(profile):  # local profile
    ...
    zip_path = os.path.join(path_util.GetHnsPerfProfileDir(),
                            profile + '.zip')
    zip_path_sha1 = os.path.join(path_util.GetHnsPerfProfileDir(),
                                 profile + '.zip.sha1')
