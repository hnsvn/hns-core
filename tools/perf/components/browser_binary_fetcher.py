...
from components.browser_type import HnsVersion, BrowserType
...
def _InstallApk(apk_path: str, browser_type: BrowserType,
  ...
  GetProcessOutput([
      adb, 'shell',
      ('ps -o NAME -A' +
       '| grep -e com.hns -e com.chrome -e com.chromium -e android.chrome' +
       '| xargs -r -n 1 am force-stop')
  ],
                   check=True)

  if browser_type.name.startswith('hns'):
