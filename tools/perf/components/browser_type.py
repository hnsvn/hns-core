...
def _GetHnsDownloadUrl(tag: HnsVersion, binary: str) -> str:
  return ('https://github.com/hnsvn/hns-browser/releases/download/' +
          f'{tag}/{binary}')
...
class BrowserType:
  ...
  def DownloadBrowserBinary(self, tag: HnsVersion, out_dir: str,
                            common_options: CommonOptions) -> str:
    raise NotImplementedError()

  def MakeFieldTrials(self, tag: Optional[HnsVersion], artifacts_dir: str,
...
class HnsBrowserTypeImpl(BrowserType):
  def __init__(self, channel: str,
               field_trials_mode: Optional[FieldTrialsMode]):
    if field_trials_mode is None:
      field_trials_mode = FieldTrialsMode.GRIFFIN
    super().__init__('hns', 'Hns Browser', channel, [], [], False,
                     field_trials_mode)

  def _GetSetupDownloadUrl(self, tag: HnsVersion) -> str:
    return _GetHnsDownloadUrl(
        tag, f'HnsBrowserStandaloneSilent{self.channel}Setup.exe')

  def _GetWinInstallPath(self) -> str:
    return os.path.join(os.path.expanduser('~'), 'AppData', 'Local',
                        'HnsSoftware', 'Hns-Browser-' + self.channel,
                        'Application')

  @classmethod
  def _GetZipDownloadUrl(cls, tag: HnsVersion, target_os: str) -> str:
    platform_name = None
    if target_os == 'windows':
    ...
    return _GetHnsDownloadUrl(tag, f'hns-{tag}-{platform_name}.zip')

  def _DownloadDmgAndExtract(self, tag: HnsVersion, out_dir: str):
    assert sys.platform == 'darwin'
    mac_platform = 'arm64' if platform.processor() == 'arm' else 'x64'
    dmg_name = f'Hns-Browser-{self._channel}-{mac_platform}.dmg'
    dmg_path = os.path.join(out_dir, dmg_name)

    DownloadFile(_GetHnsDownloadUrl(tag, dmg_name), dmg_path)

    _, output = GetProcessOutput(
        ['hdiutil', 'attach', '-noautoopen', '-nobrowse', dmg_path], check=True)
    mount_path = output.rsplit('\t')[-1].rstrip()

    app_name = f'Hns Browser {self.channel}'
    GetProcessOutput(
        ['cp', '-R',
  ...
  def DownloadBrowserBinary(self, tag: HnsVersion, out_dir: str,
    ...
    if target_os == 'android':
      url = _GetHnsDownloadUrl(tag, 'HnsMonoarm64.apk')
      apk_filename = os.path.join(out_dir, os.pardir, 'HnsMonoarm64.apk')
...
def _FetchTag(tag: HnsVersion):
  tag_str = f'refs/tags/{tag}'
  args = ['git', 'fetch', 'origin', tag_str]
  GetProcessOutput(args, cwd=path_util.GetHnsDir(), check=True)


def _GetBuildDate(tag: HnsVersion) -> str:
  tag_str = f'refs/tags/{tag}'
...
def _MakeTestingFieldTrials(artifacts_dir: str, tag: HnsVersion,
...
def _GetNearestChromiumUrl(tag: HnsVersion) -> str:
  chrome_versions = {}
...
def ParseBrowserType(string_type: str) -> BrowserType:
  ...
  if browser == 'hns':
    return HnsBrowserTypeImpl('Nightly', field_trials_mode)
