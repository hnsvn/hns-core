...
from components.browser_type import BrowserType, HnsVersion, ParseBrowserType


class RunnerConfig:
  """A description of a browser configuration that is able to run tests."""
  tag: Optional[HnsVersion] = None
...
def ParseTarget(target: str) -> Tuple[Optional[HnsVersion], str]:
  m = re.match(r'^(v\d+\.\d+\.\d+)(?::(.+)|$)', target)
  if not m:
    return None, target
  tag = HnsVersion(m.group(1))
  location = m.group(2)
