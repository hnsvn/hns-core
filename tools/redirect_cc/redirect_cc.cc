...
#include "hns/tools/redirect_cc/gomacc_buildflags.h"
...
const base::FilePath::StringPieceType kHnsChromiumSrc =
    FILE_PATH_LITERAL("hns/chromium_src");
...
class RedirectCC {
 public:
  ...
  int Run() {
    ...
    // Path to `src/hns/chromium_src`.
    base::FilePath::StringType hns_chromium_src_dir;
    // Path to `src/`.
    ...
    for (int arg_idx = first_compiler_arg_idx; arg_idx < argc_; ++arg_idx) {
      ...
      if (base::StartsWith(arg_piece, kIncludeFlag) &&
          base::EndsWith(arg_piece, kHnsChromiumSrc)) {
        arg_piece.remove_prefix(kIncludeFlag.size());
        hns_chromium_src_dir = base::FilePath::StringType(arg_piece);
        arg_piece.remove_suffix(kHnsChromiumSrc.size());
    ...
    base::FilePath::StringType hns_path;
    ...
    for (int arg_idx = first_compiler_arg_idx; arg_idx < argc_; ++arg_idx) {
      ...
      if (!compile_file_found && base::Contains(kCompileFileFlags, arg_piece)) {
        ...
        if (!path_cc.empty()) {
          hns_path = base::StrCat(
              {hns_chromium_src_dir, FILE_PATH_LITERAL("/"), path_cc});
          if (base::PathExists(base::FilePath(hns_path))) {
            launch_argv.push_back(base::FilePath::StringType(arg_piece));
            launch_argv.push_back(hns_path);
            ++arg_idx;
            continue;
          } else {
            hns_path.clear();
    ...
    if (exit_code == 0 && !hns_path.empty()) {
      // This is a specially crafted string that ninja will look for to create
      // deps.
      std::wcerr << L"Note: including file: " << hns_path << L"\n";
    }
#endif  // BUILDFLAG(IS_WIN)
    return exit_code;
  }
