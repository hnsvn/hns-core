...
// Global overrides
import 'chrome://resources/hns/br_elements/br_toolbar/br_toolbar.js'
import 'chrome://resources/hns/br_elements/br_shared_style.css.js'
...
const debug = false || document.location.href.includes('hns-overriding-debug')
...
function addHnsBehaviorsLegacy(moduleName: string, component: { behaviors: Function[] }) {
...
function addHnsProperties(moduleName: string, component: { properties: any }) {
...
const allHnsTemplateModificationsMap = {}

function addHnsTemplateModifications(moduleName: string, component: PolymerElement, modifyFn: (content: DocumentFragment) => unknown) {
...
const styleOverridePrefix = 'hns-override-style-'

function addHnsStyleOverride(moduleName: string, styleOverrideModuleNames: string[], component: PolymerElement, template = component._template || component.template) {
  ...
  if (debug) {
    console.log(`Hns Style Override added of "${styleOverrideModuleNames.join(' ')}" for ${moduleName}`, styleElement)
...
export function RegisterPolymerComponentProperties(propertiesMap) {
  ...
  for (const componentName in propertiesMap) {
    if (componentPropertyModifications[componentName]) {
      addHnsProperties(componentName, componentPropertyModifications[componentName])
    }
  }
}
...
export function RegisterPolymerTemplateModifications(modificationsMap: ModificationsMap) {
  ...
  Object.assign(allHnsTemplateModificationsMap, awaitingComponentModifications)
}
...
export function OverrideIronIcons(iconSetName: string, overridingIconSetName: string, iconOverrides: IconOverridesMap) {
  ...
  for (const chromiumIconName in iconOverrides) {
    ...
    if (!chromiumIcon) {
      console.error(`[hns overrides] Could not find chromium icon '${chromiumIconName}' in iconset '${iconSetName}' for replacement!`)
      continue
    }
    const hnsIconName = iconOverrides[chromiumIconName]
    const hnsIcon = overrideIconSet.querySelector(`#${hnsIconName}`)
    if (!hnsIcon) {
      console.error(`[hns overrides] Could not find hns icon '${hnsIconName}' in iconset '${overridingIconSetName}' for replacement!`)
      continue
    }
    // replace
    ...
    while (hnsIcon.firstChild) {
      chromiumIcon.appendChild(hnsIcon.firstChild)
    }
  }
...
Polymer.Class = function (info, mixin) {
  ...
  addHnsBehaviorsLegacy(name, info)
  return oldClass(info, mixin)
}
...
PolymerElement._prepareTemplate = function HnsPolymer_PrepareTemplate() {
  ...
  addHnsProperties(name, this.prototype)

  // This allows us to add new functions to the prototype and/or replace
  // existing ones.
  applyPrototypeModifications(name, this.prototype)

  const templateModifyFn = allHnsTemplateModificationsMap[name]
  if (templateModifyFn) {
    addHnsTemplateModifications(name, this.prototype, templateModifyFn)
    // TODO(petemill): delete allHnsTemplateModificationsMap entry when done so that the
  ...
  if (styleOverrideModules && styleOverrideModules) {
    addHnsStyleOverride(name, styleOverrideModules, this.prototype)
  }
}
 ...
 * @returns
 */
function HnsDefineCustomElements(name, component, options, useIgnoreList = true) {
  if (component.polymerElementVersion) {
    if (debug) {
      console.log('HnsDefineCustomElements PolymerElement defined', name, component, options)
    }
    // Global ignore (likely due to manual replacement)
    if (useIgnoreList && ignoredComponents.includes(name)) {
      if (debug) {
        console.log(`HnsDefineCustomElements ignored ${name}`)
      }
      return
    }
    // Inject behaviors
    if (allBehaviorsMap[name]) {
      if (debug) {
        console.log('HnsDefineCustomElements added behavior', allBehaviorsMap[name])
      }
...
window.customElements.define = HnsDefineCustomElements

export function define(name, component, options?) {
  // We still want style and template overrides
  HnsDefineCustomElements.call(window.customElements, name, component, options, false)
}
