...
# Merges 2 allowlists (repack_allowlist + hns_allowlist = output file) to
...
def main():
    ...
    parser.add_argument('--repack_allowlist',
                        ...
                        required=True,
                        metavar='FILE')
    parser.add_argument('--hns_allowlist',
                        help='Path to the hns repack allowist.',
                        required=True,
                        metavar='FILE')
    parser.add_argument('--output',
                        help='Path to the hns output allowist.',
                        required=True,
                        metavar='FILE')
    ...
    if not os.path.exists(args.hns_allowlist):
        print('Hns allowlist not found: {}'.format(args.hns_allowlist))
        return 1
    ...
    with open(args.hns_allowlist, 'r') as f:
        for line in f.readlines():
            unique = unique | set(line.split()[:2])
