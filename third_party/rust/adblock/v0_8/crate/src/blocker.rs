...
pub struct BlockerResult {
    ...
    ///
    /// Hns Browser keeps multiple instances of [`Blocker`], so `important`
...
mod blocker_tests {
    ...
    /// Tests ported from the previous query parameter stripping logic in hns-core
    #[test]
    fn removeparam_hns_core_tests() {
        let testcases = [
...
fn test_removeparam_same_tokens() {
    ...
    fn filter_add_tagged() {
        ...
        blocker.add_filter(NetworkFilter::parse("||hns.com$tag=brian", true, Default::default()).unwrap()).unwrap();
...
mod legacy_rule_parsing_tests {
    ...
    const HNS_UNBREAK: ListCounts = ListCounts { filters: 32, cosmetic_filters: 0, exceptions: 4, duplicates: 0 };
    ...
    #[test]
    fn parse_hns_unbreak() {
        check_list_counts(["./data/test/hns-unbreak.txt"], FilterFormat::Standard, HNS_UNBREAK);
    }

    #[test]
    fn parse_hns_disconnect_simple_malware() {
    ...
    #[test]
    fn parse_multilist() {
        let expectation = EASY_LIST + EASY_PRIVACY + UBLOCK_UNBREAK + HNS_UNBREAK;
        check_list_counts(
            [
                "./data/test/easylist.txt",
                "./data/test/easyprivacy.txt",
                "./data/test/ublock-unbreak.txt",
                "./data/test/hns-unbreak.txt",
            ],
