...
HnsAudioFarblingHelper::HnsAudioFarblingHelper(double fudge_factor,
                                                   uint64_t seed,
                                                   bool max)
    : fudge_factor_(fudge_factor), seed_(seed), max_(max) {}

HnsAudioFarblingHelper::~HnsAudioFarblingHelper() = default;

void HnsAudioFarblingHelper::FarbleAudioChannel(float* dst,
                                                  size_t count) const {
  if (max_) {
    uint64_t v = seed_;
...
void HnsAudioFarblingHelper::FarbleFloatTimeDomainData(
...
void HnsAudioFarblingHelper::FarbleByteTimeDomainData(
    const float* input_buffer,
...
void HnsAudioFarblingHelper::FarbleConvertToByteData(
    const float* source,
    unsigned char* destination,
    size_t len,
    double min_decibels,
...
void HnsAudioFarblingHelper::FarbleConvertFloatToDb(const float* source,
                                                      float* destination,
                                                      size_t len) const {
  if (max_) {
    uint64_t v = seed_;
