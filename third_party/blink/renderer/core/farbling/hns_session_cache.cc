...
const char kHnsSessionToken[] = "hns_session_token";
const char HnsSessionCache::kSupplementName[] = "HnsSessionCache";
...
HnsFarblingLevel GetHnsFarblingLevelFor(ExecutionContext* context,
                                            HnsFarblingLevel default_value) {
  HnsFarblingLevel value = default_value;
  if (context)
    value = hns::HnsSessionCache::From(*context).GetHnsFarblingLevel();
  return value;
}

bool AllowFingerprinting(ExecutionContext* context) {
  return (GetHnsFarblingLevelFor(context, HnsFarblingLevel::OFF) !=
...
bool AllowFontFamily(ExecutionContext* context,
  ...
  auto* settings = hns::GetContentSettingsClientFor(context, true);
  if (!settings)
    return true;

  if (!hns::HnsSessionCache::From(*context).AllowFontFamily(settings,
                                                                family_name))
    return false;
...
int FarbleInteger(ExecutionContext* context,
                  ...
                  int min_value,
                  int max_value) {
  HnsSessionCache& cache = HnsSessionCache::From(*context);
...
bool BlockScreenFingerprinting(ExecutionContext* context) {
  ...
  HnsFarblingLevel level =
      GetHnsFarblingLevelFor(context, HnsFarblingLevel::OFF);
  return level != HnsFarblingLevel::OFF;
}
...
HnsSessionCache::HnsSessionCache(ExecutionContext& context)
    : Supplement<ExecutionContext>(context) {
  farbling_enabled_ = false;
  farbling_level_ = HnsFarblingLevel::OFF;
  ...
  base::StringToUint64(
      cmd_line->HasSwitch(kHnsSessionToken)
          ? cmd_line->GetSwitchValueASCII(kHnsSessionToken)
          // https://github.com/hnsvn/hns-browser/issues/22021
  ...
  if (blink::WebContentSettingsClient* settings =
    ...
    farbling_level_ = settings->GetHnsFarblingLevel();
  }
  if (farbling_level_ != HnsFarblingLevel::OFF) {
    audio_farbling_helper_.emplace(
        fudge_factor, seed, farbling_level_ == HnsFarblingLevel::MAXIMUM);
  }
  farbling_enabled_ = true;
}

HnsSessionCache& HnsSessionCache::From(ExecutionContext& context) {
  HnsSessionCache* cache =
      Supplement<ExecutionContext>::From<HnsSessionCache>(context);
  if (!cache) {
    cache = MakeGarbageCollected<HnsSessionCache>(context);
...
// static
void HnsSessionCache::Init() {
  RegisterAllowFontFamilyCallback(base::BindRepeating(&hns::AllowFontFamily));
}

void HnsSessionCache::FarbleAudioChannel(float* dst, size_t count) {
  if (audio_farbling_helper_)
    audio_farbling_helper_->FarbleAudioChannel(dst, count);
}

void HnsSessionCache::PerturbPixels(const unsigned char* data, size_t size) {
  if (!farbling_enabled_ || farbling_level_ == HnsFarblingLevel::OFF)
    return;
  PerturbPixelsInternal(data, size);
}

void HnsSessionCache::PerturbPixelsInternal(const unsigned char* data,
                                              size_t size) {
  if (!data || size == 0)
    return;
...
WTF::String HnsSessionCache::GenerateRandomString(std::string seed,
                                                    wtf_size_t length) {
  uint8_t key[32];
  crypto::HMAC h(crypto::HMAC::SHA256);
...
WTF::String HnsSessionCache::FarbledUserAgent(WTF::String real_user_agent) {
...
int HnsSessionCache::FarbledInteger(FarbleKey key,
                                      int spoof_value,
...
bool HnsSessionCache::AllowFontFamily(
...
FarblingPRNG HnsSessionCache::MakePseudoRandomGenerator(FarbleKey key) {
  uint64_t seed =
