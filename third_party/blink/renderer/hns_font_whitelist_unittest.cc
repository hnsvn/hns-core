...
class HnsFontWhitelistTest : public testing::Test {
 public:
  HnsFontWhitelistTest() = default;
  HnsFontWhitelistTest(const HnsFontWhitelistTest&) = delete;
  HnsFontWhitelistTest& operator=(const HnsFontWhitelistTest&) = delete;
  ~HnsFontWhitelistTest() override = default;

  void SetUp() override {}
};

TEST(HnsFontWhitelistTest, Platforms) {
  base::flat_set<base::StringPiece> allowed(
      hns::get_font_whitelist_for_testing());

#if BUILDFLAG(IS_MAC)
  EXPECT_EQ(hns::get_can_restrict_fonts_for_testing(), true);
...
#else
  EXPECT_EQ(hns::get_can_restrict_fonts_for_testing(), false);
  EXPECT_EQ(allowed.size(), 0UL);
#endif
}

TEST(HnsFontWhitelistTest, Locales) {
  ...
  for (const auto& c : test_cases) {
    base::flat_set<base::StringPiece> allowed(
        hns::GetAdditionalFontWhitelistByLocale(std::get<0>(c)));
    EXPECT_EQ(allowed.size(), std::get<1>(c));
  }
}

TEST(HnsFontWhitelistTest, KnownFonts) {
  ...
  for (const auto& c : test_cases) {
    EXPECT_EQ(hns::AllowFontByFamilyName(std::get<0>(c), ""), std::get<1>(c));
  }
}

TEST(HnsFontWhitelistTest, CaseInsensitivity) {
...
TEST(HnsFontWhitelistTest, API) {
  hns::set_font_whitelist_for_testing(true /* can_restrict_fonts */,
                                        kTestFontWhitelist);
  EXPECT_EQ(hns::get_can_restrict_fonts_for_testing(), true);
  base::flat_set<base::StringPiece> allowed(
      hns::get_font_whitelist_for_testing());
  ...
  hns::set_font_whitelist_for_testing(false /* can_restrict_fonts */,
                                        kEmptyFontSet);
  EXPECT_EQ(hns::get_can_restrict_fonts_for_testing(), false);
