...
def _OverrideVersionKey(plist, hns_version):
    """ `minor.build` version string is used for update.
    When we begin to use the Major version component, Hns version string will
    ...
    version_values = hns_version.split('.')
...
def Main():
    ...
    parser.add_argument('--hns_channel', dest='hns_channel', action='store',
        default=None, help='Channel (beta, dev, nightly)')
    parser.add_argument('--hns_product_dir_name', dest='hns_product_dir_name',
        action='store', default=None,
        help='Product directory name')
    parser.add_argument('--hns_eddsa_key', dest='hns_eddsa_key', action='store',
        default=None, help='Public EdDSA key for update')
    parser.add_argument('--hns_version', dest='hns_version', action='store',
        default=None, help='hns version string')
    ...
    if args.skip_signing and args.hns_channel != "":
        plist['KSChannelID'] = args.hns_channel
    elif 'KSChannelID' in plist:
        # 'KSChannelID' is set at _modify_plists() of modification.py only
        # during signing
        del plist['KSChannelID']

    plist['CrProductDirName'] = args.hns_product_dir_name

    if args.hns_eddsa_key:
        plist['SUPublicEDKey'] = args.hns_eddsa_key

    _OverrideVersionKey(plist, args.hns_version)
