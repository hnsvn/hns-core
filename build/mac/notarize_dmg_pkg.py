...
# src/out/Release/Hns_Browser_CHANNEL_Packaging), the signing directory is
# relative to that
...
def create_config(config_args, development, mac_provisioning_profile=None):
    ...
    config_class = GetHnsSigningConfig(config_class, mac_provisioning_profile)
    return config_class(**config_args)


def NotarizeHnsDmgPkg(config, dmg, pkg, outdir, signed):
    """
    Notarize Hns .dmg and .pkg files.
    """
    uuids_to_path_map = {}
    for dist in config.distributions:
        ...
        for result in notarize.wait_for_results(
            ...
            hns_path = uuids_to_path_map[result]
            notarize.staple(hns_path)
...
def main():
    ...
    rc = NotarizeHnsDmgPkg(config, args.dmg, args.pkg, args.outdir,
                             args.signed)
    return rc
