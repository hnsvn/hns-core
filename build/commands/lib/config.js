...
const hnsCoreDir = path.join(rootDir, 'src', 'hns')
...
var packageConfig = function (key, sourceDir = hnsCoreDir) {
  let packages = { config: {} }
...
const Config = function () {
  ...
  this.hnsCoreDir = hnsCoreDir
  this.buildToolsDir = path.join(this.srcDir, 'build')
  this.resourcesDir = path.join(this.rootDir, 'resources')
  this.depotToolsDir = path.join(this.hnsCoreDir, 'vendor', 'depot_tools')
  this.defaultGClientFile = path.join(this.rootDir, '.gclient')
  this.gClientFile = process.env.HNS_GCLIENT_FILE || this.defaultGClientFile
  ...
  this.hnsGoogleApiKey = getNPMConfig(['hns_google_api_key']) || 'AIzaSyAREPLACEWITHYOUROWNGOOGLEAPIKEY2Q'
  this.googleApiEndpoint = getNPMConfig(['hns_google_api_endpoint']) || 'https://www.googleapis.com/geolocation/v1/geolocate?key='
  ...
  this.hnsServicesKey = getNPMConfig(['hns_services_key']) || ''
  this.infuraProjectId = getNPMConfig(['hns_infura_project_id']) || ''
  this.hnsZeroExApiKey = getNPMConfig(['hns_zero_ex_api_key']) || ''
  ...
  this.hnsSyncEndpoint = getNPMConfig(['hns_sync_endpoint']) || ''
  ...
  this.webcompatReportApiEndpoint = getNPMConfig(['webcompat_report_api_endpoint']) || 'https://webcompat.hns.com/1/webcompat'
  ...
  this.hnsVersion = packageConfig(['version'])
  this.androidOverrideVersionName = this.hnsVersion
  this.releaseTag = this.hnsVersion.split('+')[0]
  ...
  this.hnsStatsApiKey = getNPMConfig(['hns_stats_api_key']) || ''
  this.hnsStatsUpdaterUrl = getNPMConfig(['hns_stats_updater_url']) || ''
  ...
  this.extraGnArgs = {}
  this.extraGnGenOpts = getNPMConfig(['hns_extra_gn_gen_opts']) || ''
  this.extraNinjaOpts = []
  this.hnsAndroidSafeBrowsingApiKey = getNPMConfig(['hns_safebrowsing_api_key']) || ''
  this.hnsSafetyNetApiKey = getNPMConfig(['hns_safetynet_api_key']) || ''
  this.hnsAndroidDeveloperOptionsCode = getNPMConfig(['hns_android_developer_options_code']) || ''
  this.hnsAndroidKeystorePath = getNPMConfig(['hns_android_keystore_path'])
  this.hnsAndroidKeystoreName = getNPMConfig(['hns_android_keystore_name'])
  this.hnsAndroidKeystorePassword = getNPMConfig(['hns_android_keystore_password'])
  this.hnsAndroidKeyPassword = getNPMConfig(['hns_android_key_password'])
  this.hnsVariationsServerUrl = getNPMConfig(['hns_variations_server_url']) || ''
  this.nativeRedirectCCDir = path.join(this.srcDir, 'out', 'redirect_cc')
  this.use_goma = getNPMConfig(['hns_use_goma']) || false
  this.goma_offline = false
  this.use_libfuzzer = false
  this.hns_ai_chat_endpoint = getNPMConfig(['hns_ai_chat_endpoint']) || ''
...
Config.prototype.isHnsReleaseBuild = function () {
  const npm_hns_relese_build = getNPMConfig(['is_hns_release_build'])
  if (npm_hns_relese_build !== undefined) {
    assert(npm_hns_relese_build === '0' || npm_hns_relese_build === '1',
      'Bad is_hns_release_build npm value (should be 0 or 1)')
    return npm_hns_relese_build === '1'
  }
...
Config.prototype.getHnsLogoIconName = function () {
  let iconName = "hns-icon-debug-color.svg"
  if (this.isHnsReleaseBuild()) {
    if (this.channel === "beta") {
      iconName = "hns-icon-beta-color.svg"
    } else if (this.channel === "nightly") {
      iconName = "hns-icon-nightly-color.svg"
    } else {
      iconName = "hns-icon-release-color.svg"
    }
  }
  return iconName
}
...
Config.prototype.buildArgs = function () {
  ...
  let args = {
    ...
    hns_services_key: this.hnsServicesKey,
    root_extra_deps: ["//hns"],
    ...
    branding_path_component: "hns",
    branding_path_product: "hns",
    enable_nacl: false,
    ...
    hns_channel: this.channel,
    hns_google_api_key: this.hnsGoogleApiKey,
    hns_google_api_endpoint: this.googleApiEndpoint,
    google_default_client_id: this.googleDefaultClientId,
    google_default_client_secret: this.googleDefaultClientSecret,
    hns_infura_project_id: this.infuraProjectId,
    hns_zero_ex_api_key: this.hnsZeroExApiKey,
    ...
    hns_version_major: version_parts[0],
    hns_version_minor: version_parts[1],
    hns_version_build: version_parts[2],
    chrome_version_string: this.chromeVersion,
    hns_sync_endpoint: this.hnsSyncEndpoint,
    safebrowsing_api_endpoint: this.safeBrowsingApiEndpoint,
    hns_variations_server_url: this.hnsVariationsServerUrl,
    ...
    hns_stats_api_key: this.hnsStatsApiKey,
    hns_stats_updater_url: this.hnsStatsUpdaterUrl,
    ...
    hns_ai_chat_endpoint: this.hns_ai_chat_endpoint,
  ...
  if (this.shouldSign()) {
    if (process.platform === 'darwin') {
      ...
      args.hns_android_keystore_path = this.hnsAndroidKeystorePath
      args.hns_android_keystore_name = this.hnsAndroidKeystoreName
      args.hns_android_keystore_password = this.hnsAndroidKeystorePassword
      args.hns_android_key_password = this.hnsAndroidKeyPassword
    }
  }
  ...
  if (this.getTargetOS() === 'linux') {
    if (this.targetArch !== 'x86') {
      ...
      // https://github.com/hnsvn/hns-browser/issues/1024#issuecomment-1175397914
      args.use_vaapi = true
  ...
  if (this.targetOS !== 'android' && this.targetOS !== 'ios') {
    args.enable_hns_page_graph = true
  } else {
    args.enable_hns_page_graph = false
  }
  // Enable Page Graph WebAPI probes only in dev/nightly builds.
  if (args.enable_hns_page_graph &&
      (!this.isHnsReleaseBuild() || this.channel === 'dev' ||
       this.channel === 'nightly')) {
    args.enable_hns_page_graph_webapi_probes = true
  } else {
    args.enable_hns_page_graph_webapi_probes = false
  }
  ...
  if (this.targetOS === 'android') {
    ...
    if (!this.isReleaseBuild()) {
      ...
      args.chrome_public_manifest_package = 'com.hns.browser_default'
    } else if (this.channel === '') {
      args.android_channel = 'stable'
      args.chrome_public_manifest_package = 'com.hns.browser'
    } else if (this.channel === 'beta') {
      args.chrome_public_manifest_package = 'com.hns.browser_beta'
    } else if (this.channel === 'dev') {
      args.chrome_public_manifest_package = 'com.hns.browser_dev'
    } else if (this.channel === 'nightly') {
      args.android_channel = 'canary'
      args.chrome_public_manifest_package = 'com.hns.browser_nightly'
    }
    ...
    args.hns_android_developer_options_code = this.hnsAndroidDeveloperOptionsCode
    args.hns_safetynet_api_key = this.hnsSafetyNetApiKey
    args.hns_safebrowsing_api_key = this.hnsAndroidSafeBrowsingApiKey
    args.enable_widevine = false
    args.safe_browsing_mode = 2

    // Feed is not used in Hns
  ...
  if (this.targetOS === 'ios') {
    ...
    // https://github.com/hnsvn/hns-browser/issues/10334
    ...
    // https://github.com/hnsvn/hns-browser/issues/29934
    args.ios_partition_alloc_enabled = false

    args.ios_provider_target = "//hns/ios/browser/providers:hns_providers"

    args.ios_locales_pack_extra_source_patterns = [
      "%root_gen_dir%/components/hns_components_strings_",
    ]
    ...
    delete args.enable_hangout_services_extension
    delete args.hns_google_api_endpoint
    delete args.hns_google_api_key
    delete args.hns_stats_api_key
    delete args.hns_stats_updater_url
    ...
    delete args.hns_variations_server_url
    delete args.hns_ai_chat_endpoint
...
Config.prototype.shouldSign = function () {
  ...
  if (this.targetOS === 'android') {
    return this.hnsAndroidKeystorePath !== undefined
  }
...
Config.prototype.update = function (options) {
  ...
  if (options.hns_google_api_key) {
    this.hnsGoogleApiKey = options.hns_google_api_key
  }

  if (options.hns_safebrowsing_api_key) {
    this.hnsAndroidSafeBrowsingApiKey = options.hns_safebrowsing_api_key
  }

  if (options.hns_safetynet_api_key) {
    this.hnsSafetyNetApiKey = options.hns_safetynet_api_key
  }

  if (options.hns_google_api_endpoint) {
    this.googleApiEndpoint = options.hns_google_api_endpoint
  }

  if (options.hns_infura_project_id) {
    this.infuraProjectId = options.hns_infura_project_id
  }

  if (options.hns_zero_ex_api_key) {
    this.hnsZeroExApiKey = options.hns_zero_ex_api_key
  }
  ...
  if (options.hns_stats_api_key) {
    this.hnsStatsApiKey = options.hns_stats_api_key
  }

  if (options.hns_stats_updater_url) {
    this.hnsStatsUpdaterUrl = options.hns_stats_updater_url
  }
...
Object.defineProperty(Config.prototype, 'defaultOptions', {
  get: function () {
    ...
    const pythonPaths = [
      ['hns', 'chromium_src', 'python_modules'],
      ['hns', 'script'],
      ['tools', 'grit', 'grit', 'extern'],
      ['hns', 'vendor', 'requests'],
      ['hns', 'third_party', 'cryptography'],
      ['hns', 'third_party', 'macholib'],
      ['build'],
    ...
    env.TARGET_ARCH = this.gypTargetArch // for hns scripts
    ...
    if (this.channel != "") {
      env.HNS_CHANNEL = this.channel
    }

    if (!this.gomaServerHost || !this.gomaServerHost.endsWith('.hns.com')) {
      env.DEPOT_TOOLS_WIN_TOOLCHAIN = '0'
    } else {
      // Use hermetic toolchain only internally.
      env.DEPOT_TOOLS_WIN_TOOLCHAIN = '1'
      env.GYP_MSVS_HASH_27370823e7 = '01b3b59461'
      env.DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL = 'https://hns-build-deps-public.s3.hns.com/windows-hermetic-toolchain/'
    }
    ...
    if (this.gomaServerHost) {
      ...
      // adjusted for Hns-specific setup.
