...
const getTestsToRun = (config, suite) => {
  ...
  if (suite === 'hns_unit_tests') {
    if (config.targetOS !== 'android') {
      testsToRun.push('hns_installer_unittests')
    } else {
      testsToRun.push('bin/run_hns_public_test_apk')
    }
  }
  return testsToRun
}
...
const getApplicableFilters = (suite) => {
  ...
  possibleFilters.forEach(filterName => {
    let filterFilePath = path.join(config.hnsCoreDir, 'test', 'filters',  `${filterName}.filter`)
...
const buildTests = (suite, buildConfig = config.defaultBuildConfig, options = {}) => {
  ...
  let testSuites = [
    'hns_unit_tests',
    'hns_browser_tests',
    'hns_network_audit_tests',
  ]
  if (testSuites.includes(suite)) {
    config.buildTarget = 'hns/test:' + suite
  } else {
...
const runTests = (passthroughArgs, suite, buildConfig, options) => {
  ...
  if (options.filter) {
    hnsArgs.push('--gtest_filter=' + options.filter)
  }

  if (options.run_disabled_tests) {
    hnsArgs.push('--gtest_also_run_disabled_tests')
  }

  if (options.output) {
    hnsArgs.push('--gtest_output=xml:' + options.output)
  }
  ...
  if (options.single_process) {
    hnsArgs.push('--single_process')
  }

  if (options.test_launcher_jobs) {
    hnsArgs.push('--test-launcher-jobs=' + options.test_launcher_jobs)
  }

  hnsArgs = hnsArgs.concat(passthroughArgs)

  // Filter out upstream tests that are known to fail for Hns
  ...
  if (upstreamTestSuites.includes(suite)) {
    ...
    if (filterFilePaths.length > 0)
      hnsArgs.push(`--test-launcher-filter-file="${filterFilePaths.join(';')}"`)
  }
  ...
  if (config.targetOS === 'ios') {
    ...
    getTestsToRun(config, suite).every((testSuite) => {
      if (options.output) {
        hnsArgs.splice(hnsArgs.indexOf('--gtest_output=xml:' + options.output), 1)
        hnsArgs.push(`--gtest_output=xml:${testSuite}.xml`)
      }
      if (config.targetOS === 'android') {
        assert(
      ...
      let prog = util.run(path.join(config.outputDir, getTestBinary(testSuite)), hnsArgs, runOptions)
