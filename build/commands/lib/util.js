// Copyright (c) 2016 The Hns Authors. All rights reserved.
...
async function applyPatches() {
  ...
  const coreRepoPath = config.hnsCoreDir
...
const util = {
  ...
  updateBranding: () => {
    ...
    const hnsComponentsDir = path.join(config.hnsCoreDir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const hnsAppDir = path.join(config.hnsCoreDir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const hnsBrowserResourcesDir = path.join(config.hnsCoreDir, 'browser', 'resources')
    const hnsAppVectorIconsDir = path.join(config.hnsCoreDir, 'components')
    const chromeAndroidJavaStringsTranslationsDir = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'strings', 'translations')
    const hnsAndroidJavaStringsTranslationsDir = path.join(config.hnsCoreDir, 'browser', 'ui', 'android', 'strings', 'translations')
    ...
    const hnsAndroidTabUiJavaStringsTranslationsDir = path.join(config.hnsCoreDir, 'android', 'features', 'tab_ui', 'java', 'strings', 'translations')

    let fileMap = new Set();
    const autoGeneratedHnsToChromiumMapping = Object.assign({}, l10nUtil.getAutoGeneratedHnsToChromiumMapping())
    // The following 3 entries we map to the same name, not the chromium equivalent name for copying back
    autoGeneratedHnsToChromiumMapping[path.join(hnsAppDir, 'hns_strings.grd')] = path.join(chromeAppDir, 'hns_strings.grd')
    autoGeneratedHnsToChromiumMapping[path.join(hnsAppDir, 'settings_hns_strings.grdp')] = path.join(chromeAppDir, 'settings_hns_strings.grdp')
    autoGeneratedHnsToChromiumMapping[path.join(hnsComponentsDir, 'components_hns_strings.grd')] = path.join(chromeComponentsDir, 'components_hns_strings.grd')

    Object.entries(autoGeneratedHnsToChromiumMapping).forEach(mapping => fileMap.add(mapping))

    // Copy xtb files for:
    // hns/app/resources/chromium_strings*.xtb
    // hns/app/resources/generated_resoruces*.xtb
    // hns/components/strings/components_chromium_strings*.xtb
    // hns/browser/ui/android/strings/translations/android_chrome_strings*.xtb
    // hns/android/features/tab_ui/java/strings/translations/android_chrome_tab_ui_strings*.xtb
    fileMap.add([path.join(hnsAppDir, 'resources'), path.join(chromeAppDir, 'resources')])
    fileMap.add([path.join(hnsComponentsDir, 'strings'), path.join(chromeComponentsDir, 'strings')])
    fileMap.add([hnsAndroidJavaStringsTranslationsDir, chromeAndroidJavaStringsTranslationsDir])
    fileMap.add([hnsAndroidTabUiJavaStringsTranslationsDir, chromeAndroidTabUiJavaStringsTranslationsDir])
    // By overwriting, we don't need to modify some grd files.
    fileMap.add([path.join(hnsAppDir, 'theme', 'hns'), path.join(chromeAppDir, 'theme', 'hns')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'hns'), path.join(chromeAppDir, 'theme', 'chromium')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'default_100_percent', 'hns'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'hns')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'default_200_percent', 'hns'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'hns')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'default_100_percent', 'hns'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'default_200_percent', 'hns'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'default_100_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'common')])
    fileMap.add([path.join(hnsAppDir, 'theme', 'default_200_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'common')])
    fileMap.add([path.join(hnsComponentsDir, 'resources', 'default_100_percent'), path.join(chromeComponentsDir, 'resources', 'default_100_percent')])
    fileMap.add([path.join(hnsComponentsDir, 'resources', 'default_100_percent', 'hns'), path.join(chromeComponentsDir, 'resources', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(hnsComponentsDir, 'resources', 'default_200_percent'), path.join(chromeComponentsDir, 'resources', 'default_200_percent')])
    fileMap.add([path.join(hnsComponentsDir, 'resources', 'default_200_percent', 'hns'), path.join(chromeComponentsDir, 'resources', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(hnsAppVectorIconsDir, 'vector_icons', 'hns'), path.join(chromeComponentsDir, 'vector_icons', 'hns')])
    // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with hns's on Win8.
    fileMap.add([path.join(hnsBrowserResourcesDir, 'chrome-logo-faded.png'), path.join(chromeBrowserResourcesDir, 'chrome-logo-faded.png')])
    fileMap.add([path.join(hnsBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg'), path.join(chromeBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg')])
    fileMap.add([path.join(hnsBrowserResourcesDir, 'settings', 'images'), path.join(chromeBrowserResourcesDir, 'settings', 'images')])
    fileMap.add([path.join(hnsBrowserResourcesDir, 'signin', 'images'), path.join(chromeBrowserResourcesDir, 'signin', 'images')])
    fileMap.add([
      path.join(
          hnsBrowserResourcesDir, 'signin', 'profile_customization',
          'images'),
      path.join(
          chromeBrowserResourcesDir, 'signin', 'profile_customization',
          'images')
    ])
    fileMap.add([path.join(hnsBrowserResourcesDir, 'signin', 'profile_picker', 'images'), path.join(chromeBrowserResourcesDir, 'signin', 'profile_picker', 'images')])
    fileMap.add([path.join(hnsBrowserResourcesDir, 'side_panel', 'reading_list', 'images'), path.join(chromeBrowserResourcesDir, 'side_panel', 'reading_list', 'images')])

    // Copy to make our ${branding_path_product}_behaviors.cc
    fileMap.add([path.join(config.hnsCoreDir, 'chromium_src', 'chrome', 'installer', 'setup', 'hns_behaviors.cc'),
                 path.join(config.srcDir, 'chrome', 'installer', 'setup',
                           'hns_behaviors.cc')])
    // Replace webui CSS to use our fonts.
    fileMap.add([path.join(config.hnsCoreDir, 'ui', 'webui', 'resources', 'css', 'text_defaults_md.css'),
                 path.join(config.srcDir, 'ui', 'webui', 'resources', 'css', 'text_defaults_md.css')])
    // Replace chrome dark logo with channel specific hns logo.
    fileMap.add([
      path.join(config.hnsCoreDir, 'node_modules', '@hns', 'leo', 'icons',
          config.getHnsLogoIconName()),
    ...
    if (config.getTargetOS() === 'mac') {
      ...
      // Each channel's app icons are stored in hns/app/theme/$channel/app.icns.
      // With this copying, we don't need to modify chrome/BUILD.gn for this.
      const iconSource = path.join(hnsAppDir, 'theme', 'hns', 'mac', config.channel, 'app.icns')
      const iconDest = path.join(chromeAppDir, 'theme', 'hns', 'mac', 'app.icns')
      ...
      const brandingSource = path.join(hnsAppDir, 'theme', 'hns', branding_file_name)
      const brandingDest = path.join(chromeAppDir, 'theme', 'hns', 'BRANDING')
    ...
    if (config.targetOS === 'android') {
      ...
      let hnsOverwrittenFiles = new Set();
      const removeUnlistedAndroidResources = (hnsOverwrittenFiles) => {
        ...
        for (const untrackedChromiumPath of untrackedChromiumPaths) {
          if (isChildOf(untrackedChromiumPath, suspectedDir) && !hnsOverwrittenFiles.has(untrackedChromiumPath)) {
      ...
      if (config.channel === 'development') {
        androidIconSet = 'res_hns_default'
      }
      else if (config.channel === '') {
        androidIconSet = 'res_hns'
      } else if (config.channel === 'beta') {
        androidIconSet = 'res_hns_beta'
      } else if (config.channel === 'dev') {
        androidIconSet = 'res_hns_dev'
      } else if (config.channel === 'nightly') {
        androidIconSet = 'res_hns_nightly'
      }

      const androidTranslateResSource = path.join(config.hnsCoreDir, 'components', 'translate','content' , 'android', 'java', 'res')
      const androidTranslateResDest = path.join(config.srcDir, 'components', 'translate','content' , 'android', 'java', 'res')
      const androidIconSource = path.join(hnsAppDir, 'theme', 'hns', 'android', androidIconSet)
      const androidIconDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_chromium')
      const androidIconBaseSource = path.join(hnsAppDir, 'theme', 'hns', 'android', androidIconSet + '_base')
      const androidIconBaseDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_chromium_base')
      const androidResSource = path.join(config.hnsCoreDir, 'android', 'java', 'res')
      const androidResDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res')
      const androidResTemplateSource = path.join(config.hnsCoreDir, 'android', 'java', 'res_template')
      const androidResTemplateDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_template')
      const androidContentPublicResSource = path.join(config.hnsCoreDir, 'content', 'public', 'android', 'java', 'res')
      const androidContentPublicResDest = path.join(config.srcDir, 'content', 'public', 'android', 'java', 'res')
      const androidTouchtoFillResSource = path.join(config.hnsCoreDir, 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidTouchtoFillResDest = path.join(config.srcDir, 'chrome', 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidToolbarResSource = path.join(config.hnsCoreDir, 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidToolbarResDest = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidComponentsWidgetResSource = path.join(config.hnsCoreDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidComponentsWidgetResDest = path.join(config.srcDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidComponentsStylesResSource = path.join(config.hnsCoreDir, 'components', 'browser_ui', 'styles', 'android', 'java', 'res')
      const androidComponentsStylesResDest = path.join(config.srcDir, 'components', 'browser_ui', 'styles', 'android', 'java', 'res')
      const androidSafeBrowsingResSource = path.join(config.hnsCoreDir, 'browser', 'safe_browsing', 'android', 'java', 'res')
      const androidSafeBrowsingResDest = path.join(config.srcDir, 'chrome', 'browser', 'safe_browsing', 'android', 'java', 'res')
      const androidDownloadInternalResSource = path.join(config.hnsCoreDir, 'browser', 'download', 'internal', 'android', 'java', 'res')
      const androidDownloadInternalResDest = path.join(config.srcDir, 'chrome', 'browser', 'download', 'internal', 'android', 'java', 'res')
      const androidFeaturesTabUiResSource = path.join(config.hnsCoreDir, 'android', 'features', 'tab_ui', 'java', 'res')
      const androidFeaturesTabUiDest = path.join(config.srcDir, 'chrome', 'android', 'features', 'tab_ui', 'java', 'res')
      const androidComponentsOmniboxResSource = path.join(config.hnsCoreDir, 'components', 'omnibox', 'browser', 'android', 'java', 'res')
      const androidComponentsOmniboxResDest = path.join(config.srcDir, 'components', 'omnibox', 'browser', 'android', 'java', 'res')
      const androidBrowserUiOmniboxResSource = path.join(config.hnsCoreDir, 'browser', 'ui', 'android', 'omnibox', 'java', 'hns_res')
      const androidBrowserUiOmniboxResDest = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'omnibox', 'java', 'res')

      // Mapping for copying Hns's Android resource into chromium folder.
      ...
      Object.entries(copyAndroidResourceMapping).map(([sourcePath, destPaths]) => {
        ...
        for (const destPath of destPaths) {
          for (const androidSourceFile of androidSourceFiles) {
            ...
            hnsOverwrittenFiles.add(destinationFile);
          }
        }
      })
      removeUnlistedAndroidResources(hnsOverwrittenFiles)
    }
  ...
  touchOverriddenChromiumSrcFiles: () => {
    ...
    const chromiumSrcDir = path.join(config.srcDir, 'hns', 'chromium_src')
  ...
  touchOverriddenVectorIconFiles: () => {
    Log.progressStart('touch original vector icon files overridden by hns/vector_icons')
    ...
    const hnsVectorIconsDir = path.join(config.srcDir, 'hns', 'vector_icons')
    var hnsVectorIconFiles = util.walkSync(hnsVectorIconsDir, applyFileFilter)

    // Touch original files by updating mtime.
    const hnsVectorIconsDirLen = hnsVectorIconsDir.length
    hnsVectorIconFiles.forEach(hnsVectorIconFile => {
      var overriddenFile = path.join(config.srcDir, hnsVectorIconFile.slice(hnsVectorIconsDirLen))
      if (fs.existsSync(overriddenFile)) {
        // If overriddenFile is older than file in vector_icons, touch it to trigger rebuild.
        updateFileUTimesIfOverrideIsNewer(overriddenFile, hnsVectorIconFile)
      }
    })
    Log.progressFinish('touch original vector icon files overridden by hns/vector_icons')
  },
  ...
  touchGsutilChangeLogFile: () => {
    ...
    // https://github.com/hnsvn/hns-browser/issues/31641
  ...
  updateMidlFiles: () => {
    Log.progressScope('update midl files', () => {
      for (const source of ["google_update", "hns"]) {
        fs.copySync(
          path.join(config.hnsCoreDir, 'win_build_output', 'midl', source),
  ...
  buildNativeRedirectCC: (options = config.defaultOptions) => {
    ...
    if (!config.isCI &&
        ...
        fs.statSync(path.join(config.hnsCoreDir, 'tools', 'redirect_cc', 'redirect_cc.cc')).mtime) {
      return
    }

    Log.progressStart('build redirect_cc')
    gnArgs = {
      'import("//hns/tools/redirect_cc/args.gni")': null,
    ...
    util.buildTarget('hns/tools/redirect_cc', mergeWithDefault({outputDir: config.nativeRedirectCCDir}))
    Log.progressFinish('build redirect_cc')
  },

  runGnGen: (options) => {
    const buildArgsStr = util.buildArgsToString(config.buildArgs())
    const buildArgsFile = path.join(config.outputDir, 'hns_build_args.txt')
    ...
    const extraGnGenOptsFile = path.join(config.outputDir, 'hns_extra_gn_gen_opts.txt')
    ...
    if (shouldRunGnGen) {
      ...
      // hns_build_args.txt are in sync.
      if (prevBuildArgs)
  ...
  generateXcodeWorkspace: (options = config.defaultOptions) => {
    ...
    const genScript = path.join(config.hnsCoreDir, 'vendor', 'gn-project-generators', 'xcode.py')
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.hnsCoreDir
    ...
    util.run(
        ...
        [
          ...
          path.join(
              config.hnsCoreDir, 'build', 'commands', 'scripts', 'lint.py'),
  ...
  presubmit: (options = {}) => {
    ...
    // 'gerrit.host' from their hns checkout.
    util.runGit(
        config.hnsCoreDir, ['config', '--unset-all', 'gerrit.host'], true)
    let cmd_options = config.defaultOptions
    cmd_options.cwd = config.hnsCoreDir
    cmd_options = mergeWithDefault(cmd_options)
    cmd = 'git'
