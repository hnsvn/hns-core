 ...
 * - All mappings for auto-generated Hns files from the associated Chromium files.
 * - Top level global string replacements, such as replacing Chromium with Hns
 */
...
// These map to hns/app/resources/chromium_strings*.xtb
const chromiumStringsPath = path.resolve(path.join(srcDir, 'chrome', 'app', 'chromium_strings.grd'))
const hnsStringsPath = path.resolve(path.join(srcDir, 'hns', 'app', 'hns_strings.grd'))
const chromiumSettingsPartPath = path.resolve(path.join(srcDir, 'chrome', 'app', 'settings_chromium_strings.grdp'))
const hnsSettingsPartPath = path.resolve(path.join(srcDir, 'hns', 'app', 'settings_hns_strings.grdp'))
...
const hnsAndroidChromeStringsPath = path.resolve(path.join(srcDir, 'hns', 'browser', 'ui', 'android', 'strings', 'android_chrome_strings.grd'))
...
const hnsAndroidTabUiStringsPath = path.resolve(path.join(srcDir, 'hns', 'android', 'features', 'tab_ui', 'java', 'strings', 'android_chrome_tab_ui_strings.grd'))


// component_chromium_strings.grd and any of its parts files that we track localization for in transifex
// These map to hns/app/strings/components_chromium_strings*.xtb
const chromiumComponentsChromiumStringsPath = path.resolve(path.join(srcDir, 'components', 'components_chromium_strings.grd'))
const hnsComponentsHnsStringsPath = path.resolve(path.join(srcDir, 'hns', 'components', 'components_hns_strings.grd'))

// components/component_strings.grd and any of its parts files that we track localization for in transifex
// These map to hns/components/component_strings*.xtb
const chromiumComponentsStringsPath = path.resolve(path.join(srcDir, 'components', 'components_strings.grd'))
const hnsComponentsStringsPath = path.resolve(path.join(srcDir, 'hns', 'components', 'components_strings.grd'))
...
// These map to hns/app/resources/generated_resoruces*.xtb
const chromiumGeneratedResourcesPath = path.resolve(path.join(srcDir, 'chrome', 'app', 'generated_resources.grd'))
const hnsGeneratedResourcesPath = path.resolve(path.join(srcDir, 'hns', 'app', 'generated_resources.grd'))
...
// hns_generated_resources.grd maps to hns/app/resources/hns_generated_resources*.xtb,
// hns_components_strings.grd maps to hns/components/resources/strings/hns_components_resources*.xtb
// messages.json localization is handled inside of hns-extension.
const hnsSpecificGeneratedResourcesPath = path.resolve(path.join(srcDir, 'hns', 'app', 'hns_generated_resources.grd'))
const hnsResourcesComponentsStringsPath = path.resolve(path.join(srcDir, 'hns', 'components', 'resources', 'hns_components_strings.grd'))
const hnsExtensionMessagesPath = path.resolve(path.join(srcDir, 'hns', 'components', 'hns_extension', 'extension', 'hns_extension', '_locales', 'en_US', 'messages.json'))
const hnsAndroidHnsStringsPath = path.resolve(path.join(srcDir, 'hns', 'browser', 'ui', 'android', 'strings', 'android_hns_strings.grd'))
...
function addGrd(chromiumPath, hnsPath, exclude = new Set()) {
  ...
  if (grdps.length) {
    ...
    const hnsDir = path.dirname(hnsPath)
    for (const grdp of grdps) {
      if (exclude.has(grdp)) {
        continue
      }
      const chromiumGrdpPath = path.resolve(path.join(chromiumDir, grdp))
      const hnsGrdpPath = path.resolve(path.join(hnsDir, grdp))
      // grdp files can have their own grdp parts too
      mapping = { ...mapping, ...addGrd(chromiumGrdpPath, hnsGrdpPath, exclude) }
    }
    if (verboseLogFindGrd)
      console.log("  - Added " + (Object.keys(mapping).length - 1) + " GRDP.")
  }
  mapping[chromiumPath] = hnsPath
  return mapping
}

// Helper functions that's, for a given pair of chromium to hns GRD mapping
...
function getRemovedGRDParts(mapping) {
  ...
  for (const [sourcePath, destPath] of Object.entries(mapping)) {
    if (path.extname(destPath) === ".grd") {
      const hnsGRDPs = getGrdPartsFromGrd(destPath)
      const chromiumGRDPs = getGrdPartsFromGrd(sourcePath)
      let removed = new Set()
      for (let i = 0; i < hnsGRDPs.length; i++) {
        if (!chromiumGRDPs.includes(hnsGRDPs[i])) {
          removed.add(hnsGRDPs[i])
        }
      }
      if (removed.size) {
...
function getAutoGeneratedGrdMappings() {
  if (typeof(getAutoGeneratedGrdMappings.mappings) === 'undefined') {
    ...
    // Hns specific only grd and grdp files should NOT be added.
    // Using AddGrd will add GRD and all of its GRDPs.
    getAutoGeneratedGrdMappings.mappings = {
      ...addGrd(chromiumComponentsStringsPath, hnsComponentsStringsPath),
      ...addGrd(chromiumGeneratedResourcesPath, hnsGeneratedResourcesPath, chromiumGeneratedResourcesExcludes),
      ...addGrd(androidChromeStringsPath, hnsAndroidChromeStringsPath),
      ...addGrd(androidTabUiStringsPath, hnsAndroidTabUiStringsPath)
    }
...
function getChromiumToAutoGeneratedHnsMapping() {
  if (typeof(getChromiumToAutoGeneratedHnsMapping.mapping) === 'undefined') {
    ...
    getChromiumToAutoGeneratedHnsMapping.mapping = {
      [chromiumSettingsPartPath]: hnsSettingsPartPath,
      [chromiumStringsPath]: hnsStringsPath,

      [chromiumComponentsChromiumStringsPath]: hnsComponentsHnsStringsPath,

      ...getAutoGeneratedGrdMappings()
    }
  }
  return getChromiumToAutoGeneratedHnsMapping.mapping
}

const l10nUtil = {
  // Same as with chromiumToAutoGeneratedHnsMapping but maps in the opposite direction
  getAutoGeneratedHnsToChromiumMapping: () => {
    if (typeof(l10nUtil.getAutoGeneratedHnsToChromiumMapping.mapping) === 'undefined') {
      const chromiumToAutoGeneratedHnsMapping = getChromiumToAutoGeneratedHnsMapping()
      l10nUtil.getAutoGeneratedHnsToChromiumMapping.mapping = Object.keys(
        chromiumToAutoGeneratedHnsMapping).reduce((obj, key) => (
          { ...obj, [chromiumToAutoGeneratedHnsMapping[key]]: key }), {})
    }
    return l10nUtil.getAutoGeneratedHnsToChromiumMapping.mapping
  },

  // All paths which are generated
  getHnsAutoGeneratedPaths: () => {
    return Object.values(getChromiumToAutoGeneratedHnsMapping())
  },

  // All paths which are not generated
  getHnsNonGeneratedPaths: () => {
    if (typeof(l10nUtil.getHnsNonGeneratedPaths.paths) === 'undefined') {
      l10nUtil.getHnsNonGeneratedPaths.paths = [
        hnsSpecificGeneratedResourcesPath,
        hnsResourcesComponentsStringsPath,
        hnsExtensionMessagesPath,
        hnsAndroidHnsStringsPath
      ]
    }
    return l10nUtil.getHnsNonGeneratedPaths.paths
  },

  // Hns specific strings and Chromium mapped Hns strings will be here.
  // But you only need to add the Hns specific strings manually here.
  getAllHnsPaths: () => {
    return l10nUtil.getHnsNonGeneratedPaths().concat(l10nUtil.getHnsAutoGeneratedPaths())
  },
  ...
  getHnsTopLevelPaths: () => {
    return l10nUtil.getAllHnsPaths().filter((x) => ['grd', 'json'].includes(x.split('.').pop()))
  },
...
// to Transifex.
  getGreaselionScriptPaths: (extensionPath) => {
    ...
    if (!basePath) {
      basePath = '../../../hns-site-specific-scripts'
    }
  ...
  // with Hns specific branding strings.
  ...
  rebaseHnsStringFilesOnChromiumL10nFiles: async (path) => {
    const removedMap = getRemovedGRDParts(getAutoGeneratedGrdMappings())
    const ops = Object.entries(getChromiumToAutoGeneratedHnsMapping()).map(async ([sourcePath, destPath]) => {
