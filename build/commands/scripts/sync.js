  ...
  let out = toGClientConfigItem('solutions', [
    {
      ...
      custom_vars: {
        'checkout_rust': '%True%',
        'checkout_pgo_profiles': config.isHnsReleaseBuild() ? '%True%' :
                                                                '%False%'
      }
    },
    {
      managed: '%False%',
      name: 'src/hns',
      // We do not use gclient to manage hns-core, so this should not
      // actually get used.
      url: 'https://github.com/hnsvn/hns-core.git'
    }
  ])
...
function syncChromium(program) {
  ...
  const latestSyncInfoFilePath =
      path.join(config.rootDir, '.hns_latest_successful_sync.json')
  ...
  if (program.delete_unused_deps) {
    if (util.isGitExclusionExists(config.srcDir, 'hns/')) {
      args.push('-D')
    } else if (!config.isCI) {
      Log.warn(
          '--delete_unused_deps is ignored because sync has not yet added ' +
          'the exclusion for the src/hns/ directory, likely because sync ' +
  ...
  util.addGitExclusion(config.srcDir, 'hns/')
...
function syncHns(program) {
  ...
  util.runGClient(
      args, {cwd: config.hnsCoreDir},
      path.join(config.hnsCoreDir, '.hns_gclient'))
}
...
async function RunCommand() {
  ...
  Log.progressScope('gclient sync', () => {
    ...
    if (!didSyncChromium || program.delete_unused_deps) {
      // If no Chromium sync was done, run sync inside `hns` to sync Hns DEPS.
      syncHns(program)
    }
  })
  ...
  if (!program.nohooks) {
    // Run hooks for the root .gclient, this will include Chromium and Hns
...
RunCommand()
.catch((err) => {
  Log.error('Hns Browser Sync ERROR:')
  console.error(err)
  process.exit(1)
})
