...
public class HnsFeedSurfaceMediator extends FeedSurfaceMediator {
    // To delete in bytecode, members from parent class will be used instead.
    private FeedSurfaceCoordinator mCoordinator;
    private SnapScrollHelper mSnapScrollHelper;

    HnsFeedSurfaceMediator(FeedSurfaceCoordinator coordinator, Context context,
    ...
    @Override
    void updateContent() {
        assert !FeedFeatures.isFeedEnabled() : "Feed should be disabled in Hns!";
        assert mCoordinator
                instanceof HnsFeedSurfaceCoordinator : "Wrong feed surface coordinator!";

        if (FeedFeatures.isFeedEnabled()
                || !(mCoordinator instanceof HnsFeedSurfaceCoordinator)) {
            super.updateContent();
            return;
        }

        FrameLayout view = ((HnsFeedSurfaceCoordinator) mCoordinator).getFrameLayoutForPolicy();
        if (view != null) {
            return;
        }
        destroyPropertiesForStream();
        ((HnsFeedSurfaceCoordinator) mCoordinator).createFrameLayoutForPolicy();
        view = ((HnsFeedSurfaceCoordinator) mCoordinator).getFrameLayoutForPolicy();
