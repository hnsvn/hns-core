...
import org.chromium.chrome.browser.app.HnsActivity.HnsActivityNotFoundException;
...
public class OnboardingPrefManager {
    ...
    public static final String HNS = "Hns";
    ...
    public boolean isHnsStatsEnabled() {
        return mSharedPreferences.getBoolean(PREF_HNS_STATS, false);
    }

    public void setHnsStatsEnabled(boolean enabled) {
        SharedPreferences.Editor sharedPreferencesEditor = mSharedPreferences.edit();
        sharedPreferencesEditor.putBoolean(PREF_HNS_STATS, enabled);
        sharedPreferencesEditor.apply();
        try {
            HnsActivity activity = HnsActivity.getHnsActivity();
            activity.getPrivacyHubMetrics().recordEnabledStatus(enabled);
        } catch (HnsActivityNotFoundException e) {
            Log.e(TAG, "Could not report privacy hub enabled change to P3A: " + e);
        }
    }

    public boolean isHnsStatsNotificationEnabled() {
        return mSharedPreferences.getBoolean(PREF_HNS_STATS_NOTIFICATION, true);
    }

    public void setHnsStatsNotificationEnabled(boolean enabled) {
        SharedPreferences.Editor sharedPreferencesEditor = mSharedPreferences.edit();
        sharedPreferencesEditor.putBoolean(PREF_HNS_STATS_NOTIFICATION, enabled);
        sharedPreferencesEditor.apply();
    }
    ...
    public boolean isAdsAvailable() {
        return HnsAdsNativeHelper.nativeIsSupportedRegion(Profile.getLastUsedRegularProfile());
    }
    ...
    public void onboardingNotification() {
        if (!isOnboardingNotificationShown()) {
            HnsOnboardingNotification.showOnboardingNotification();
