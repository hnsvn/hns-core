...
public class HnsRewardsUserWalletActivity
    ...
    private void setUIControls() {
        ...
        final int status =
                intent.getIntExtra(HnsRewardsExternalWallet.STATUS, UNDEFINED_WALLET_STATUS);
        ...
        switch (status) {
            case WalletStatus.CONNECTED:
                txtUserStatus.setText(HnsRewardsExternalWallet.WalletStatusToString(status));
                break;
        ...
        String userId = intent.getStringExtra(HnsRewardsExternalWallet.USER_NAME);
        ...
        if (view.getId() == R.id.provider_action) {
            if (getIntent() != null) {
                int walletStatus = getIntent().getIntExtra(
                        HnsRewardsExternalWallet.STATUS, UNDEFINED_WALLET_STATUS);
                if (walletStatus == WalletStatus.CONNECTED) {
                    if (getIntent().getStringExtra(HnsRewardsExternalWallet.ACCOUNT_URL)
                            != null) {
                        Intent intent = new Intent();
                        intent.putExtra(HnsActivity.OPEN_URL,
                                getIntent().getStringExtra(HnsRewardsExternalWallet.ACCOUNT_URL));
                        setResult(RESULT_OK, intent);
                    }
                } else if (walletStatus == WalletStatus.LOGGED_OUT) {
                    Intent intent = new Intent();
                    intent.putExtra(HnsActivity.OPEN_URL,
                            getIntent().getStringExtra(HnsRewardsExternalWallet.LOGIN_URL));
                    setResult(RESULT_OK, intent);
                }
                finish();
            }
