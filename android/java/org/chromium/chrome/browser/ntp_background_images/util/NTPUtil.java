...
public class NTPUtil {
    ...
    public static int checkForNonDisruptiveBanner(NTPImage ntpImage, SponsoredTab sponsoredTab) {
        ...
        if(sponsoredTab.shouldShowBanner()) {
            ...
            } else if (ntpImage instanceof Wallpaper && HnsRewardsHelper.isRewardsEnabled()) {
    ...
    public static void showBREBottomBanner(View view) {
        Context context = ContextUtils.getApplicationContext();
        if (!PackageUtils.isFirstInstall(context) && HnsRewardsHelper.isRewardsEnabled()
    ...
    private static SpannableString getBannerText(ChromeActivity chromeActivity, int ntpType,
        ...
        Spanned learnMoreSpanned = HnsRewardsHelper.spannedFromHtmlString(bannerText);
        SpannableString learnMoreTextSS = new SpannableString(learnMoreSpanned.toString());

        ForegroundColorSpan brOffForegroundSpan = new ForegroundColorSpan(chromeActivity.getResources().getColor(R.color.hns_theme_color));
    ...
    public static boolean isReferralEnabled() {
        ...
        boolean isReferralEnabled = UserPrefs.get(Profile.getLastUsedRegularProfile()).getInteger(HnsPref.NEW_TAB_PAGE_SUPER_REFERRAL_THEMES_OPTION) == 1 ? true : false;
