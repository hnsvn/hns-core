...
import org.chromium.chrome.browser.toolbar.bottom.HnsBottomControlsCoordinator;
import org.chromium.chrome.browser.toolbar.bottom.HnsScrollingBottomViewResourceFrameLayout;
...
import org.chromium.chrome.browser.toolbar.top.HnsTopToolbarCoordinator;
...
public class HnsToolbarManager extends ToolbarManager {
    private static final String TAG = "HnsToolbarManager";
    ...
    private HomepageManager.HomepageStateListener mHnsHomepageStateListener;
    private AppCompatActivity mActivity;
    private WindowAndroid mWindowAndroid;
    private CompositorViewHolder mCompositorViewHolder;
    private final Object mLock = new Object();
    private boolean mBottomControlsEnabled;
    private HnsScrollingBottomViewResourceFrameLayout mBottomControls;

    public HnsToolbarManager(AppCompatActivity activity, BrowserControlsSizer controlsSizer,
        ...
        mHnsHomepageStateListener = () -> {
            if (mBottomControlsCoordinatorSupplier != null
                    && mBottomControlsCoordinatorSupplier.get()
                                    instanceof HnsBottomControlsCoordinator) {
                ((HnsBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
                        .updateHomeButtonState();
            }
        };
        HomepageManager.getInstance().addListener(mHnsHomepageStateListener);
    }
    ...
    public void enableBottomControls() {
        ...
        synchronized (mLock) {
            ...
            mBottomControls =
                    (HnsScrollingBottomViewResourceFrameLayout) bottomControlsStub.inflate();
            ...
            mBottomControlsCoordinatorSupplier.set(new HnsBottomControlsCoordinator(
    ...
    public void destroy() {
        super.destroy();
        HomepageManager.getInstance().removeListener(mHnsHomepageStateListener);
    ...
    protected void onOrientationChange(int newOrientation) {
        ...
        assert mActivity instanceof HnsActivity;

        if (mActivity instanceof HnsActivity) {
            ((HnsActivity) mActivity).updateBottomSheetPosition(newOrientation);
        }
    }
    ...
    protected void updateBookmarkButtonStatus() {
        ...
        if (mBottomControlsCoordinatorSupplier.get() instanceof HnsBottomControlsCoordinator) {
            ((HnsBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
    ...
    private void setBottomToolbarVisible(boolean visible) {
        ...
        HnsMenuButtonCoordinator.setMenuFromBottom(isMenuFromBottom);
        if (mToolbar instanceof HnsTopToolbarCoordinator) {
            ((HnsTopToolbarCoordinator) mToolbar).onBottomToolbarVisibilityChanged(visible);
        }
        if (mBottomControlsCoordinatorSupplier.get() instanceof HnsBottomControlsCoordinator) {
            ((HnsBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
    ...
    private boolean isToolbarPhone() {
        assert (mToolbar instanceof HnsTopToolbarCoordinator);
        return mToolbar instanceof HnsTopToolbarCoordinator
                && ((HnsTopToolbarCoordinator) mToolbar).isToolbarPhone();
    }

    private ObservableSupplier<Integer> getConstraintsProxy() {
        if (mToolbar instanceof HnsTopToolbarCoordinator) {
            return ((HnsTopToolbarCoordinator) mToolbar).getConstraintsProxy();
        }
