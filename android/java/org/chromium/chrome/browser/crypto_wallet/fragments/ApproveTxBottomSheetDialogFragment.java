...
import org.chromium.hns_wallet.mojom.ProviderErrorUnion;
...
public class ApproveTxBottomSheetDialogFragment extends WalletBottomSheetDialogFragment {
    ...
    private AssetRatioService getAssetRatioService() {
        ...
        if (activity instanceof HnsWalletBaseActivity) {
            return ((HnsWalletBaseActivity) activity).getAssetRatioService();
        }
        return null;
    }
    ...
    private TxService getTxService() {
        Activity activity = getActivity();
        if (activity instanceof HnsWalletBaseActivity) {
            return ((HnsWalletBaseActivity) activity).getTxService();
        }
        return null;
    }
    ...
    private BlockchainRegistry getBlockchainRegistry() {
        Activity activity = getActivity();
        if (activity instanceof HnsWalletBaseActivity) {
            return ((HnsWalletBaseActivity) activity).getBlockchainRegistry();
        }
        return null;
    }

    private HnsWalletService getHnsWalletService() {
        Activity activity = getActivity();
        if (activity instanceof HnsWalletBaseActivity) {
    ...
    private HnsWalletP3a getHnsWalletP3A() {
        Activity activity = getActivity();
        if (activity instanceof HnsWalletBaseActivity) {
            return ((HnsWalletBaseActivity) activity).getHnsWalletP3A();
        }
    ...
    public void setupDialog(@NonNull Dialog dialog, int style) {
        ...
        if (mTxInfo.txType
            ...
            Spanned associatedSPLTokenAccountInfo =
                    Utils.createSpanForSurroundedPhrase(getContext(),
                            R.string.hns_wallet_confirm_transaction_account_creation_fee, (v) -> {
        ...
        keyringService.getAllAccounts(
                allAccounts -> {
                    ...
                    TokenUtils.getAllTokensFiltered(getHnsWalletService(),
                            ...
                            tokenList -> {
                                SolanaTransactionsGasHelper solanaTransactionsGasHelper =
                                        new SolanaTransactionsGasHelper(
                                                (HnsWalletBaseActivity) getActivity(),
        ...
        LiveDataUtil.observeOnce(
                mWalletModel.getCryptoModel().getNetworkModel().mCryptoNetworks, allNetworks -> {
                    Utils.getTxExtraInfo(
                            new WeakReference<>((HnsWalletBaseActivity) getActivity()),
    ...
    private void refreshListContentUi() {
        mRejectAllTx.setText(getString(
                R.string.hns_wallet_queue_reject_all, String.valueOf(mTransactionInfos.size())));
    }
    ...
    private ParsedTransaction fillAssetDependentControls(View view, NetworkInfo txNetwork,
        ...
        if (parsedTx.getType() == TransactionType.ERC20_APPROVE) {
            ...
            txType.setText(R.string.hns_wallet_approve_transaction);
        } else {
    ...
    private void reportTransactionForP3A() {
        ...
        if (countTestNetworks
            ...
            hnsWalletP3A.reportTransactionSent(mCoinType, true);
        }
    }
