...
import org.chromium.hns_wallet.mojom.AddSuggestTokenRequest;
...
public class AddTokenFragment extends BaseDAppsFragment {
    ...
    private void notifyAddSuggestTokenRequestProcessed(boolean approved) {
        ...
        getHnsWalletService().notifyAddSuggestTokenRequestsProcessed(approved, addresses);
        initComponents(false);
    }

    private void fillAddSuggestTokenRequest(boolean init) {
        if (mWalletModel == null) return;
        getHnsWalletService().getPendingAddSuggestTokenRequests(requests -> {
            ...
            if (init) {
                ...
                mTokenAddress.setOnClickListener(v -> {
                    ...
                    if (activity instanceof HnsWalletBaseActivity && selectedNetwork != null) {
                        Utils.openAddress(
                                "/token/" + mCurrentAddSuggestTokenRequest.token.contractAddress,
                                (HnsWalletBaseActivity) activity,
