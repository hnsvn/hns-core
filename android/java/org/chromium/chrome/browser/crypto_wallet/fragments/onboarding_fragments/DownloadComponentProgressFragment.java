...
import org.chromium.chrome.browser.component_updater.HnsComponentUpdater;
import org.chromium.chrome.browser.crypto_wallet.util.WalletDataFilesInstaller;

/**
 * A fragment to display progress of Hns Wallet data files component
 * download. Used on all Hns Wallet Onboarding fragments.
 */
public class DownloadComponentProgressFragment extends Fragment {
    ...
    private HnsComponentUpdater.ComponentUpdaterListener mComponentUpdaterListener;
    ...
    public void onDestroyView() {
        if (mComponentUpdaterListener != null) {
            HnsComponentUpdater.get().removeComponentUpdateEventListener(
    ...
    private void setupDownloadProgress() {
        HnsComponentUpdater.CrxUpdateItem updateItem = HnsComponentUpdater.get().getUpdateState(
                WalletDataFilesInstaller.getWalletDataFilesComponentId());
        ...
        mComponentUpdaterListener = (event, id) -> {
            ...
            HnsComponentUpdater.CrxUpdateItem crxUpdateItem =
                    HnsComponentUpdater.get().getUpdateState(
                            WalletDataFilesInstaller.getWalletDataFilesComponentId());
            showHideProgressByItem(crxUpdateItem);
        };

        HnsComponentUpdater.get().addComponentUpdateEventListener(mComponentUpdaterListener);
    }

    private void showHideProgressByItem(HnsComponentUpdater.CrxUpdateItem updateItem) {
