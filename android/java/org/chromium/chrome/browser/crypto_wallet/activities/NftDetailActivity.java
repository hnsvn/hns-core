...
public class NftDetailActivity extends HnsWalletBaseActivity {
    ...
    protected void triggerLayoutInflation() {
        ...
        // GitHub issue: https://github.com/hnsvn/hns-browser/issues/27802.
        ...
        if (mIsErc721) {
            mTokenStandardView.setText(R.string.hns_wallet_nft_erc_721);
            mTokenAddressLabelView.setText(R.string.hns_wallet_nft_token_id);
        } else if (mCoinType == CoinType.SOL) {
            mTokenStandardView.setText(R.string.hns_wallet_nft_sol_spl);
            mTokenAddressLabelView.setText(R.string.hns_wallet_nft_mint_address);
        } else {
        ...
        HnsActivity hnsActivity = null;
        try {
            hnsActivity = HnsActivity.getHnsActivity();
        } catch (HnsActivity.HnsActivityNotFoundException e) {
            Log.e(TAG, "triggerLayoutInflation " + e);
        }
        assert hnsActivity != null;

        LiveDataUtil.observeOnce(
                hnsActivity.getWalletModel().getCryptoModel().getNetworkModel().mDefaultNetwork,
                defaultNetwork -> {
    ...
    private void createClickableLink(
        ...
        NoUnderlineClickableSpan linkSpan = new NoUnderlineClickableSpan(
                this, R.color.hns_link, (textView) -> { TabUtils.openLinkWithFocus(this, url); });
