...
public class BalanceHelper {
    ...
    public static void getP3ABalances(WeakReference<HnsWalletBaseActivity> activityRef,
        ...
        assert hnsWalletService != null && blockchainRegistry != null && keyringService != null
                && jsonRpcService != null;
        if (JavaUtils.anyNull(
                    hnsWalletService, blockchainRegistry, keyringService, jsonRpcService))
            return;

        boolean P3AEnabled =
                HnsConfig.P3A_ENABLED && HnsLocalState.get().getBoolean(HnsPref.P3A_ENABLED);
            ...
            Utils.getP3ANetworks(allNetworks, relevantNetworks -> {
                ...
                for (int coinType : Utils.P3ACoinTypes) {
                    processP3ACoinNetworks(coinType, sortedNetworks.get(coinType), keyringService,
                            jsonRpcService, hnsWalletService, blockchainRegistry, multiResponse,
    ...
    private static void processP3ACoinNetworks(@CoinType.EnumType int coinType,
            ...
            JsonRpcService jsonRpcService, HnsWalletService hnsWalletService,
            BlockchainRegistry blockchainRegistry, MultiResponseHandler multiResponse,
            ArrayList<GetNativeAssetsBalancesResponseContext> nativeAssetsBalancesResponses,
            ArrayList<GetBlockchainTokensBalancesResponseContext>
                    blockchainTokensBalancesResponses) {
        if (JavaUtils.anyNull(hnsWalletService, blockchainRegistry)) return;
        ...
        keyringService.getAllAccounts(allAccounts -> {
            for (NetworkInfo network : networks) {
                ...
                TokenUtils.getVisibleUserAssetsFiltered(
                        hnsWalletService, network, coinType, TokenUtils.TokenType.ALL, tokens -> {
                            // Assets balances.
