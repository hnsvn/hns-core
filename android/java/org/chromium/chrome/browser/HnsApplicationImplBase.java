...
public class HnsApplicationImplBase extends SplitCompatApplication.Impl {
    @Override
        ...
        if (SplitCompatApplication.isBrowserProcess()) {
            GoBackend.setAlwaysOnCallback(new GoBackend.AlwaysOnCallback() {
                ...
                public void alwaysOnTriggered() {
                    HnsVpnProfileUtils.getInstance().startVpn(getApplication());
                }
            });
            // Set a handler for SafeBrowsing. It has to be done only once for a process lifetime.
            SafeBrowsingApiBridge.setSafetyNetApiHandler(HnsSafeBrowsingApiHandler.getInstance());
        }
    }
}
