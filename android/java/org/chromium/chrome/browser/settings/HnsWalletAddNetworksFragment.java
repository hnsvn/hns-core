...
public class HnsWalletAddNetworksFragment extends Fragment implements ConnectionErrorHandler {
    ...
    /**
     * A host to launch HnsWalletAddNetworksFragment and receive the result.
     */
    interface Launcher {
        /**
         * Launches HnsWalletAddNetworksFragment.
         */
    ...
    public View onCreateView(
        ...
        final View view = inflater.inflate(R.layout.hns_wallet_add_network, container, false);
        ...
        if (!mChainId.isEmpty()) {
            btAdd.setText(R.string.hns_wallet_add_network_submit);
    ...
    private boolean validateInputsAddChain(View view) {
        ...
        try {
            ...
            if (iChainId <= 0) {
                mChainIdEditText.setError(
                        getString(R.string.hns_wallet_add_network_chain_id_error));

                return false;
            }
            strChainId = "0x" + Integer.toHexString(iChainId);
        } catch (NumberFormatException exc) {
            mChainIdEditText.setError(getString(R.string.hns_wallet_add_network_chain_id_error));
        ...
        if (strChainName.isEmpty()) {
            chainName.setError(getString(R.string.hns_wallet_add_network_chain_empty_error));
        ...
        if (strChainCurrencyName.isEmpty()) {
            chainCurrencyName.setError(
                    getString(R.string.hns_wallet_add_network_chain_empty_error));
        ...
        try {
            ...
            if (iChainCurrencyDecimals <= 0) {
                chainCurrencyDecimals.setError(
                        getString(R.string.hns_wallet_add_network_chain_currency_decimals_error));
        ...
        try {
            ...
            if (!url.getProtocol().equals("http") && !url.getProtocol().equals("https")) {
                rpcUrls.setError(getString(R.string.hns_wallet_add_network_urls_error));
        ...
        if (!strIconUrls.isEmpty()) {
            try {
                ...
                if (!url.getProtocol().equals("http") && !url.getProtocol().equals("https")) {
                    iconUrls.setError(getString(R.string.hns_wallet_add_network_urls_error));
            ...
            try {
                ...
                if (!url.getProtocol().equals("http") && !url.getProtocol().equals("https")) {
                    blockExplorerUrls.setError(
                            getString(R.string.hns_wallet_add_network_urls_error));

                    return false;
                }
                chain.blockExplorerUrls = new String[] {strBlockExplorerUrls};
            } catch (MalformedURLException exc) {
                blockExplorerUrls.setError(getString(R.string.hns_wallet_add_network_urls_error));
