 ...
 */
public class HnsQAPreferences extends HnsPreferenceFragment
    ...
    private ChromeSwitchPreference mHnsVpnFeature;
    private ChromeSwitchPreference mVpnLinkSubscriptionOnDev;
    private ChromeSwitchPreference mHnsDormantFeatureEngagement;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mHnsVpnFeature =
                (ChromeSwitchPreference) findPreference(HnsVpnPrefUtils.PREF_HNS_VPN_FEATURE);
        if (mHnsVpnFeature != null) {
            mHnsVpnFeature.setOnPreferenceChangeListener(this);
        }

        mVpnLinkSubscriptionOnDev = (ChromeSwitchPreference) findPreference(
                HnsVpnPrefUtils.PREF_HNS_VPN_LINK_SUBSCRIPTION_ON_STAGING);
        if (mVpnLinkSubscriptionOnDev != null) {
            mVpnLinkSubscriptionOnDev.setOnPreferenceChangeListener(this);
        }

        mHnsDormantFeatureEngagement = (ChromeSwitchPreference) findPreference(
                OnboardingPrefManager.PREF_DORMANT_USERS_ENGAGEMENT);
        if (mHnsDormantFeatureEngagement != null) {
            mHnsDormantFeatureEngagement.setOnPreferenceChangeListener(this);
        }
        ...
        mIsStagingServer.setChecked(UserPrefs.get(Profile.getLastUsedRegularProfile())
                                            .getBoolean(HnsPref.USE_REWARDS_STAGING_SERVER));
    ...
    @Override
    public void onStart() {
        HnsRewardsNativeWorker.getInstance().AddObserver(this);
        super.onStart();
    }

    @Override
    public void onStop() {
        HnsRewardsNativeWorker.getInstance().RemoveObserver(this);
        super.onStop();
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (PREF_USE_REWARDS_STAGING_SERVER.equals(preference.getKey())) {
            ...
            enableMaximumAdsNumber((boolean) newValue);
                ...
                || HnsVpnPrefUtils.PREF_HNS_VPN_FEATURE.equals(preference.getKey())
                || HnsVpnPrefUtils.PREF_HNS_VPN_LINK_SUBSCRIPTION_ON_STAGING.equals(
                        preference.getKey())
    ...
    private void checkQACode() {
        ...
        DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int button) {
                if (button != AlertDialog.BUTTON_POSITIVE
                        || !input.getText().toString().equals(HnsConfig.DEVELOPER_OPTIONS_CODE)) {
                    getActivity().finish();
                }
            }
        };
    ...
    private void enableMaximumAdsNumber(boolean enable) {
        if (enable) {
            // Save current values
            int adsPerHour = HnsRewardsNativeWorker.getInstance().GetAdsPerHour();
            SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
            SharedPreferences.Editor sharedPreferencesEditor = sharedPreferences.edit();
            sharedPreferencesEditor.putInt(QA_ADS_PER_HOUR, adsPerHour);
            sharedPreferencesEditor.apply();
            // Set max value
            HnsRewardsNativeWorker.getInstance().SetAdsPerHour(MAX_ADS);
            return;
        }
        // Set saved values
        int adsPerHour = ContextUtils.getAppSharedPreferences().getInt(
                             QA_ADS_PER_HOUR, DEFAULT_ADS_PER_HOUR);
        HnsRewardsNativeWorker.getInstance().SetAdsPerHour(adsPerHour);
    }
    ...
    public void OnResetTheWholeState(boolean success) {
        if (success) {
            ...
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.USE_REWARDS_STAGING_SERVER, mUseRewardsStagingServer);
            HnsRewardsHelper.setRewardsEnvChange(true);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CHOOSE_FILE_FOR_IMPORT_REQUEST_CODE && resultCode == Activity.RESULT_OK
            ...
            try {
                ...
                Log.e(HnsDbUtil.getTag(), "Error on preparing database file: " + e);
                return;
            }
            requestRestart(true);
        }
