...
import org.chromium.chrome.browser.sync.HnsSyncDevices;
import org.chromium.chrome.browser.sync.SyncServiceFactory;
import org.chromium.chrome.browser.sync.settings.HnsManageSyncSettings;
...
public class HnsSyncScreensPreference extends HnsPreferenceFragment
    ...
    // Hns Sync message text view
    private TextView mHnsSyncTextViewInitial;
    private TextView mHnsSyncTextViewSyncChainCode;
    private TextView mHnsSyncTextViewAddMobileDevice;
    private TextView mHnsSyncTextViewAddLaptop;
    private TextView mHnsSyncWarningTextViewAddMobileDevice;
    private TextView mHnsSyncWarningTextViewAddLaptop;
    private TextView mHnsSyncTextDevicesTitle;
    private TextView mHnsSyncWordCountTitle;
    private TextView mHnsSyncAddDeviceCodeWords;
    private LinearLayout mHnsSyncBtnAndroidSyncSettings;
    ...
    HnsSyncWorker getHnsSyncWorker() {
        return HnsSyncWorker.get();
    }
    ...
    public View onCreateView(
        ...
        return mInflater.inflate(R.layout.hns_sync_layout, container, false);
    }
        ...
        try {
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                        ...
                        if (null != listItemView && null != separator && null != insertPoint) {
                            ...
                            AppCompatImageView deleteButton =
                                    (AppCompatImageView) listItemView.findViewById(
                                            R.id.hns_sync_remove_device);
                            if (device.mSupportsSelfDelete || device.mIsCurrentDevice) {
                                deleteButton.setTag(device);
                                deleteButton.setOnClickListener(v -> {
                                    HnsSyncDevices.SyncDeviceInfo deviceToDelete =
                                            (HnsSyncDevices.SyncDeviceInfo) v.getTag();
                    ...
                    if (index > 0) {
                        mHnsSyncTextDevicesTitle.setText(
                                getResources().getString(R.string.hns_sync_devices_title));
                        View separator = (View) mInflater.inflate(R.layout.menu_separator, null);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mScanChainCodeButton = getView().findViewById(R.id.hns_sync_btn_scan_chain_code);
        if (mScanChainCodeButton != null) {
            mScanChainCodeButton.setOnClickListener(this);
        }

        mStartNewChainButton = getView().findViewById(R.id.hns_sync_btn_start_new_chain);
        if (mStartNewChainButton != null) {
            mStartNewChainButton.setOnClickListener(this);
        }

        mEnterCodeWordsButton = getView().findViewById(R.id.hns_sync_btn_enter_code_words);
        if (mEnterCodeWordsButton != null) {
            mEnterCodeWordsButton.setOnClickListener(this);
        }

        mQRCodeImage = getView().findViewById(R.id.hns_sync_qr_code_image);

        mDoneButton = getView().findViewById(R.id.hns_sync_btn_done);
        if (mDoneButton != null) {
            mDoneButton.setOnClickListener(this);
        }

        mDoneLaptopButton = getView().findViewById(R.id.hns_sync_btn_add_laptop_done);
        if (mDoneLaptopButton != null) {
            mDoneLaptopButton.setOnClickListener(this);
        }

        mUseCameraButton = getView().findViewById(R.id.hns_sync_btn_use_camera);
        if (mUseCameraButton != null) {
            mUseCameraButton.setOnClickListener(this);
        }

        mConfirmCodeWordsButton = getView().findViewById(R.id.hns_sync_confirm_code_words);
        if (mConfirmCodeWordsButton != null) {
            mConfirmCodeWordsButton.setOnClickListener(this);
        }

        mMobileButton = getView().findViewById(R.id.hns_sync_btn_mobile);
        if (mMobileButton != null) {
            mMobileButton.setOnClickListener(this);
        }

        mLaptopButton = getView().findViewById(R.id.hns_sync_btn_laptop);
        if (mLaptopButton != null) {
            mLaptopButton.setOnClickListener(this);
        }

        mPasteButton = getView().findViewById(R.id.hns_sync_paste_button);
        if (mPasteButton != null) {
            mPasteButton.setOnClickListener(this);
        }

        mCopyButton = getView().findViewById(R.id.hns_sync_copy_button);
        if (mCopyButton != null) {
            mCopyButton.setOnClickListener(this);
        }

        mHnsSyncTextViewInitial = getView().findViewById(R.id.hns_sync_text_initial);
        mHnsSyncTextViewSyncChainCode =
                getView().findViewById(R.id.hns_sync_text_sync_chain_code);
        mHnsSyncTextViewAddMobileDevice =
                getView().findViewById(R.id.hns_sync_text_add_mobile_device);
        mHnsSyncTextViewAddLaptop = getView().findViewById(R.id.hns_sync_text_add_laptop);
        mHnsSyncWarningTextViewAddMobileDevice =
                getView().findViewById(R.id.hns_sync_warning_text_add_mobile_device);
        mHnsSyncWarningTextViewAddLaptop =
                getView().findViewById(R.id.hns_sync_warning_text_add_laptop);
        mHnsSyncTextDevicesTitle = getView().findViewById(R.id.hns_sync_devices_title);
        mHnsSyncWordCountTitle = getView().findViewById(R.id.hns_sync_text_word_count);
        mHnsSyncWordCountTitle.setText(getString(R.string.hns_sync_word_count_text, 0));
        mHnsSyncAddDeviceCodeWords =
                getView().findViewById(R.id.hns_sync_add_device_code_words);

        mHnsSyncBtnAndroidSyncSettings =
                getView().findViewById(R.id.hns_sync_btn_android_sync_settings);
        if (null != mHnsSyncBtnAndroidSyncSettings) {
            mHnsSyncBtnAndroidSyncSettings.setOnClickListener(this);
        }

        setMainSyncText();
        mCameraSourcePreview = getView().findViewById(R.id.preview);

        mAddDeviceButton = getView().findViewById(R.id.hns_sync_btn_add_device);
        if (null != mAddDeviceButton) {
            mAddDeviceButton.setOnClickListener(this);
        }

        mDeleteAccountButton = getView().findViewById(R.id.hns_sync_btn_delete_account);
        if (null != mDeleteAccountButton) {
            mDeleteAccountButton.setOnClickListener(this);
        }

        mShowCategoriesButton = getView().findViewById(R.id.hns_sync_btn_show_categories);
        if (null != mShowCategoriesButton) {
            mShowCategoriesButton.setOnClickListener(this);
        }

        mQRCodeButton = getView().findViewById(R.id.hns_sync_qr_code_off_btn);
        if (null != mQRCodeButton) {
            mQRCodeButton.setOnClickListener(this);
        }

        mCodeWordsButton = getView().findViewById(R.id.hns_sync_code_words_off_btn);
        ...
        mLayoutMobile = getView().findViewById(R.id.hns_sync_frame_mobile);
        mLayoutLaptop = getView().findViewById(R.id.hns_sync_frame_laptop);
    ...
    private void setAppropriateView() {
        ...
        boolean firstSetupComplete = getHnsSyncWorker().IsInitialSyncFeatureSetupComplete();
    ...
    private void setMainSyncText() {
        setSyncText(getResources().getString(R.string.hns_sync_official),
                getResources().getString(R.string.hns_sync_description_page_1_part_1) + "\n\n"
                        + getResources().getString(R.string.hns_sync_description_page_1_part_2),
                mHnsSyncTextViewInitial);
    }

    private void setQRCodeText() {
        setSyncText("", getResources().getString(R.string.hns_sync_qrcode_message_v2),
                mHnsSyncTextViewSyncChainCode);
    }
    ...
    private String getWordsValidationString(String words) {
        int validationResult = getHnsSyncWorker().GetWordsValidationResult(words);
        ...
        switch (validationResult) {
            ...
            case 2:
                // kVersionDeprecated
                return getResources().getString(R.string.hns_sync_code_from_deprecated_version);
            case 3:
                // kExpired
                return getResources().getString(R.string.hns_sync_code_expired);
            case 4:
                // kValidForTooLong
                return getResources().getString(R.string.hns_sync_code_valid_for_too_long);

            default:
                // These three different types of errors have the same message
                // kWrongWordsNumber
                // kNotValidPureWords
                return getResources().getString(R.string.hns_sync_wrong_code_error);
        }
    }
    ...
    public void onClick(View v) {
        if ((getActivity() == null)
                || (v != mScanChainCodeButton && v != mStartNewChainButton
                        ...
                        && v != mHnsSyncBtnAndroidSyncSettings))
            return;

        if (mHnsSyncBtnAndroidSyncSettings == v) {
        ...
        if (mScanChainCodeButton == v) {
            ...
            if (null != mHnsSyncAddDeviceCodeWords) {
                ClipboardManager clipboard = (ClipboardManager) getActivity().getSystemService(
                        Context.CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("", mHnsSyncAddDeviceCodeWords.getText());
                clipboard.setPrimaryClip(clip);
                Toast.makeText(getActivity().getApplicationContext(),
                             getResources().getString(R.string.hns_sync_copied_text),
                             Toast.LENGTH_LONG)
                        .show();
            }
            ...
            String codephraseCandidate =
                    getHnsSyncWorker().GetPureWordsFromTimeLimited(trimmedWords);
            ...
            getActivity().setTitle(R.string.hns_sync_code_words_title);
            if (null != mCodeWords && null != mHnsSyncWordCountTitle) {
                mCodeWords.addTextChangedListener(new TextWatcher() {
                    ...
                    public void onTextChanged(CharSequence s, int start, int before, int count) {
                        ...
                        mHnsSyncWordCountTitle.setText(
                                getString(R.string.hns_sync_word_count_text, wordCount));
                        mHnsSyncWordCountTitle.invalidate();
                    }
                });
            }
        } else if (mShowCategoriesButton == v) {
            SettingsLauncher settingsLauncher = new SettingsLauncherImpl();
            settingsLauncher.launchSettingsActivity(getContext(), HnsManageSyncSettings.class);
    ...
    private void seedHexReceived(String seedHex) {
        ...
        getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                String seedWords = getHnsSyncWorker().GetWordsFromSeedHex(seedHex);
    ...
    private void seedWordsReceivedImpl(String seedWords, SyncInputType syncInputType) {
        ...
        getHnsSyncWorker().setJoinSyncChainCallback((Boolean result) -> {
            if (result) {
                ...
                onSyncError(getResources().getString(R.string.hns_sync_joining_deleted_account));
                AllowNewQrScan();
            }
        });

        getHnsSyncWorker().RequestSync();
        getHnsSyncWorker().SaveCodephrase(seedWords);
        getHnsSyncWorker().FinalizeSyncSetup();
    }
    ...
    public void onDestroy() {
        ...
        if (deviceInfoObserverSet) {
            HnsSyncDevices.get().removeDeviceInfoChangedListener(this);
    ...
    private boolean isSeedHexValid(String barcode) {
        ...
        String seedWords = getHnsSyncWorker().GetWordsFromSeedHex(barcode);
    ...
    private String getQrCodeValidationString(String jsonQr) {
        ...
        int validationResult = getHnsSyncWorker().GetQrCodeValidationResult(jsonQr);
        Log.v(TAG, "validationResult is " + validationResult);
        switch (validationResult) {
            case 0:
                // kValid, empty string indicates there is no error
            ...
            default:
                ...
                return getResources().getString(R.string.hns_sync_wrong_qrcode_error);
        }
    }
    ...
    public void onDetectedQrCode(Barcode barcode) {
        if (barcode != null) {
            ...
            String seedHex = getHnsSyncWorker().GetSeedHexFromQrJson(jsonQr);
    ...
    private void showEndDialog(String message, Runnable runWhenDismissed) {
        ...
        AlertDialog alertDialog =
                alertBuilder.setTitle(getResources().getString(R.string.hns_sync_device))
                        .setMessage(message)
    ...
    private void deleteDeviceDialog(HnsSyncDevices.SyncDeviceInfo device) {
        AlertDialog.Builder alertBuilder =
                new AlertDialog.Builder(getActivity(), R.style.ThemeOverlay_BrowserUI_AlertDialog);
        DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int button) {
                if (button == AlertDialog.BUTTON_POSITIVE) {
                    if (device.mIsCurrentDevice) {
                        ...
                        getHnsSyncWorker().ResetSync();
                        startLeaveSyncChainOperations();
                    } else {
                        HnsSyncDevices.get().DeleteDevice(device.mGuid);
                    }
                }
            }
        };
        ...
        if (device.mIsCurrentDevice) {
            deviceNameToDisplay = deviceNameToDisplay + " "
                    + getResources().getString(R.string.hns_sync_this_device_text);
        }
        AlertDialog alertDialog =
                alertBuilder
                        .setTitle(getResources().getString(R.string.hns_sync_remove_device_text))
                        .setMessage(
                                getString(R.string.hns_sync_delete_device, deviceNameToDisplay))
                        .setPositiveButton(R.string.ok, onClickListener)
    ...
    private void permanentlyDeleteAccount() {
        ...
        AlertDialog alertDialog =
                alertBuilder
                        .setTitle(getResources().getString(R.string.hns_sync_delete_account_text))
                        .setMessage(getString(R.string.hns_sync_delete_account_message))
                        .setPositiveButton(
                                R.string.hns_sync_delete_account_button, onClickListener)
                        .setNegativeButton(R.string.cancel, onClickListener)
                        .create();
        alertDialog.getDelegate().setHandleNativeActionModesEnabled(false);
        alertDialog.show();
    ...
    private void permanentlyDeleteAccountImpl() {
        getHnsSyncWorker().permanentlyDeleteAccount((String result) -> {
            if (result == null || result.isEmpty()) {
                Toast.makeText(getActivity().getApplicationContext(),
                             getResources().getString(
                                     R.string.hns_sync_delete_account_succeeded_toast),
                             Toast.LENGTH_LONG)
                        .show();
            } else {
                Toast.makeText(getActivity().getApplicationContext(),
                             getResources().getString(
                                     R.string.hns_sync_delete_account_failed_toast)
                                     + " " + result,
                             Toast.LENGTH_LONG)
                        .show();
            }
    ...
    private void startLeaveSyncChainOperations() {
        ...
        mHnsSyncTextDevicesTitle.setText(
                getResources().getString(R.string.hns_sync_leaving_sync_chain_title));

        mShowCategoriesButton.setVisibility(View.GONE);
        mAddDeviceButton.setVisibility(View.GONE);

        ViewGroup devicesUiGroup = (ViewGroup) getView().findViewById(R.id.hns_sync_devices);
  ...
  private void setJoinExistingChainLayout() {
      ...
      setQRCodeText();
      getActivity().setTitle(R.string.hns_sync_scan_chain_code);
      if (null != mScrollViewSyncChainCode) {
  ...
  private void setNewChainLayout() {
      getActivity().setTitle(R.string.hns_sync_start_new_chain);
      if (null != mScrollViewSyncInitial) {
          mScrollViewSyncInitial.setVisibility(View.GONE);
      }
      if (null != mScrollViewEnterCodeWords) {
  ...
  public String GetPureWords() {
      if (mCodephrase == null || mCodephrase.isEmpty()) {
          mCodephrase = getHnsSyncWorker().GetPureWords();
      }
      return mCodephrase;
  }
  ...
  private void setAddMobileDeviceLayout() {
      getActivity().setTitle(R.string.hns_sync_btn_mobile);
      if (null != mHnsSyncTextViewAddMobileDevice) {
          setSyncText(getResources().getString(R.string.hns_sync_scan_sync_code),
                  getResources().getString(R.string.hns_sync_add_mobile_device_text_part_1)
                          + "\n\n"
                          + getResources().getString(
                                  R.string.hns_sync_add_mobile_device_text_part_2)
                          + "\n",
                  mHnsSyncTextViewAddMobileDevice);
      }
      if (null != mHnsSyncWarningTextViewAddMobileDevice) {
        String hnsSyncCodeWarning = getResources().getString(R.string.hns_sync_code_warning);
        SpannableString hnsSyncCodeWarningSpanned = new SpannableString(hnsSyncCodeWarning);

        ForegroundColorSpan foregroundSpan = new ForegroundColorSpan(getResources().getColor(R.color.default_red));
        hnsSyncCodeWarningSpanned.setSpan(foregroundSpan, 0, hnsSyncCodeWarningSpanned.length()-1, 0);
        mHnsSyncWarningTextViewAddMobileDevice.setText(hnsSyncCodeWarningSpanned);
      }
      if (null != mScrollViewSyncInitial) {
          mScrollViewSyncInitial.setVisibility(View.GONE);
      }
      ...
      getActivity().runOnUiThread(new Runnable() {
          @Override
          public void run() {
              String seedHex = getHnsSyncWorker().GetSeedHexFromWords(GetPureWords());
              if (null == seedHex || seedHex.isEmpty()) {
                  ...
                  if (!isSeedHexValid(seedHex)) {
                      ...
                      String qrCodeString = getHnsSyncWorker().GetQrDataJson(seedHex);
  ...
  private void setAddLaptopLayout() {
      getActivity().setTitle(R.string.hns_sync_btn_laptop);
      if (null != mHnsSyncTextViewAddLaptop) {
          setSyncText(getResources().getString(R.string.hns_sync_add_laptop_text_title),
                        getResources().getString(R.string.hns_sync_add_laptop_text_part_1) + "\n\n" +
                        getResources().getString(R.string.hns_sync_add_laptop_text_part_2_new) + "\n", mHnsSyncTextViewAddLaptop);
      }

      if (null != mHnsSyncWarningTextViewAddLaptop) {
        String hnsSyncCodeWarning = getResources().getString(R.string.hns_sync_code_warning);
        SpannableString hnsSyncCodeWarningSpanned = new SpannableString(hnsSyncCodeWarning);

        ForegroundColorSpan foregroundSpan = new ForegroundColorSpan(getResources().getColor(R.color.default_red));
        hnsSyncCodeWarningSpanned.setSpan(foregroundSpan, 0, hnsSyncCodeWarningSpanned.length()-1, 0);
        mHnsSyncWarningTextViewAddLaptop.setText(hnsSyncCodeWarningSpanned);
      }
      ...
      getActivity().runOnUiThread(new Runnable() {
          ...
          public void run() {
              ...
              String timeLimitedWords =
                      getHnsSyncWorker().GetTimeLimitedWordsFromPure(codePhrase);
              assert timeLimitedWords != null && !timeLimitedWords.isEmpty();
              mHnsSyncAddDeviceCodeWords.setText(timeLimitedWords);
          }
      });
  }
  ...
  private void setSyncDoneLayout() {
      ...
      mHnsSyncBtnAndroidSyncSettings.setVisibility(View.GONE);

      if (!deviceInfoObserverSet) {
          HnsSyncDevices.get().addDeviceInfoChangedListener(this);
      ...
      try {
          HnsActivity mainActivity = HnsActivity.getHnsActivity();
          mHnsSyncTextDevicesTitle.setText(
                  getResources().getString(R.string.hns_sync_loading_devices_title));
