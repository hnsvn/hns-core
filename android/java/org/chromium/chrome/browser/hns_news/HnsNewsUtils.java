...
import org.chromium.hns_news.mojom.LocaleInfo;
...
import org.chromium.chrome.browser.settings.HnsNewsPreferencesDataListener;
...
public class HnsNewsUtils {
    public static final int HNS_NEWS_VIEWD_CARD_TIME = 1000; // milliseconds
    ...
    public static boolean shouldDisplayNews() {
        return HnsPrefServiceBridge.getInstance().getShowNews()
                && HnsPrefServiceBridge.getInstance().getNewsOptIn();
    }

    public static void setChannelIcons() {
        mChannelIcons.put("Hns", R.drawable.ic_channel_hns);
    ...
    public static void getHnsNewsSettingsData(HnsNewsController hnsNewsController,
            HnsNewsPreferencesDataListener hnsNewsPreferencesDataListener) {
        PostTask.postTask(TaskTraits.BEST_EFFORT, () -> {
            if (hnsNewsController != null) {
                hnsNewsController.getLocale((locale) -> {
                    setLocale(locale);
                    getChannels(hnsNewsController, hnsNewsPreferencesDataListener);
                    getPublishers(hnsNewsController, hnsNewsPreferencesDataListener);
                    getSuggestionsSources(hnsNewsController, hnsNewsPreferencesDataListener);
                });
            }
        });
    }

    private static void getChannels(HnsNewsController hnsNewsController,
            HnsNewsPreferencesDataListener hnsNewsPreferencesDataListener) {
        hnsNewsController.getChannels((channels) -> {
            List<Channel> channelList = new ArrayList<>();
            ...
            if (hnsNewsPreferencesDataListener != null) {
                hnsNewsPreferencesDataListener.onChannelReceived();
            }
        });
    }

    private static void getPublishers(HnsNewsController hnsNewsController,
            HnsNewsPreferencesDataListener hnsNewsPreferencesDataListener) {
        hnsNewsController.getPublishers((publishers) -> {
            setPublishers(publishers);
            if (hnsNewsPreferencesDataListener != null) {
                hnsNewsPreferencesDataListener.onPublisherReceived();
            }
        });
    }
    ...
    public static void getSuggestionsSources(HnsNewsController hnsNewsController,
            HnsNewsPreferencesDataListener hnsNewsPreferencesDataListener) {
        hnsNewsController.getSuggestedPublisherIds((publisherIds) -> {
            setSuggestionsIds(Arrays.asList(publisherIds));
            if (hnsNewsPreferencesDataListener != null) {
                hnsNewsPreferencesDataListener.onSuggestionsReceived();
            }
        });
    }
}
