...
public class HnsSearchEngineAdapter extends SearchEngineAdapter {
    private static final String TAG = "HnsSearchEngineAdapter";
    ...
    public HnsSearchEngineAdapter(Context context, Profile profile) {
    ...
    static public String getDSEShortName(Profile profile, boolean readJavaPrefOnly) {
        ...
        if (!readJavaPrefOnly) {
            ...
            // overwrite.
            if (HnsSearchEnginePrefHelper.getInstance().getFetchSEFromNative()) {
                // Set it for normal tab only
                setDSEPrefs(dseTemplateUrl, Profile.getLastUsedRegularProfile());
                HnsSearchEnginePrefHelper.getInstance().setFetchSEFromNative(false);
            }
        }
