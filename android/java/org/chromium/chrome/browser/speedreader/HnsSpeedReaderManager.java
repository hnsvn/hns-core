 ...
 * message UI.
 */
public class HnsSpeedReaderManager extends EmptyTabObserver implements UserData {
    /** The key to access this object from a {@Tab}. */
    public static final Class<HnsSpeedReaderManager> USER_DATA_KEY =
            HnsSpeedReaderManager.class;

    private static final String TAG = "HnsSpeedreaderMgr";
    ...
    HnsSpeedReaderManager(Tab tab, Supplier<MessageDispatcher> messageDispatcherSupplier) {
        super();
        mTab = tab;
    ...
    void tryShowingPrompt() {
        ...
        if (mIsDismissed || !HnsSpeedReaderUtils.tabSupportsDistillation(mTab)) {
            return;
        }
    ...
    private void showReaderModeMessage(MessageDispatcher messageDispatcher) {
        ...
        mMessageModel = new PropertyModel.Builder(MessageBannerProperties.ALL_KEYS)
                                ...
                                .with(MessageBannerProperties.ON_PRIMARY_ACTION,
                                        () -> {
                                            HnsSpeedReaderUtils.enableSpeedreaderMode(mTab);
    ...
    public static boolean isEnabled() {
        boolean isFeatureEnabled = ChromeFeatureList.isEnabled(HnsFeatureList.HNS_SPEEDREADER);
