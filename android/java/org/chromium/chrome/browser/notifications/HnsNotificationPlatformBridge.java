...
public class HnsNotificationPlatformBridge extends NotificationPlatformBridge {
    private static final int[] EMPTY_VIBRATION_PATTERN = new int[0];

    private @NotificationType int mNotificationType;

    @CalledByNative
    private static HnsNotificationPlatformBridge create(long nativeNotificationPlatformBridge) {
        if (sInstance != null) {
            throw new IllegalStateException(
                "There must only be a single NotificationPlatformBridge.");
        }

        sInstance = new HnsNotificationPlatformBridge(nativeNotificationPlatformBridge);
        return (HnsNotificationPlatformBridge) sInstance;
    }

    private HnsNotificationPlatformBridge(long nativeNotificationPlatformBridge) {
    ...
    static boolean dispatchNotificationEvent(Intent intent) {
        if (NotificationPlatformBridge.dispatchNotificationEvent(intent)) {
            ...
            if (notificationType == NotificationType.HNS_ADS
    ...
    protected NotificationBuilderBase prepareNotificationBuilder(String notificationId,
        ...
        assert result
                instanceof HnsNotificationBuilder
            : "Bytecode changes for HnsNotificationBuilder were not applied!";
        if (result instanceof HnsNotificationBuilder) {
            ((HnsNotificationBuilder) result)
                    .setIsHnsNotification(mNotificationType == NotificationType.HNS_ADS);
        }
