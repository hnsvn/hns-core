...
import org.chromium.hns_shields.mojom.FilterListAndroidHandler;
import org.chromium.hns_shields.mojom.FilterListConstants;
import org.chromium.chrome.R;
import org.chromium.chrome.browser.HnsConfig;
import org.chromium.chrome.browser.HnsLocalState;
...
import org.chromium.chrome.browser.settings.HnsDialogPreference;
import org.chromium.chrome.browser.settings.HnsPreferenceDialogFragment;
import org.chromium.chrome.browser.settings.HnsWebrtcPolicyPreference;
...
/**
 * Fragment to keep track of the all the hns privacy related preferences.
 */
public class HnsPrivacySettings extends PrivacySettings implements ConnectionErrorHandler {
    // Chromium Prefs
    ...
    // hns Prefs
    private static final String PREF_HNS_SHIELDS_GLOBALS_SECTION =
            "hns_shields_globals_section";
    private static final String PREF_CLEAR_DATA_SECTION = "clear_data_section";
    private static final String PREF_HNS_SOCIAL_BLOCKING_SECTION =
            "hns_social_blocking_section";
    ...
    private static final String PREF_HNS_STATS_USAGE_PING = "hns_stats_usage_ping";
    ...
    private static final String[] NEW_PRIVACY_PREFERENCE_ORDER = {
            PREF_HNS_SHIELDS_GLOBALS_SECTION, //  shields globals  section
            ...
            PREF_HNS_SOCIAL_BLOCKING_SECTION, // social blocking section
            ...
            PREF_HNS_STATS_USAGE_PING, PREF_SHOW_AUTOCOMPLETE_IN_ADDRESS_BAR,
    ...
    private HnsDialogPreference mAdsTrakersBlockPref;
    private HnsDialogPreference mBlockCrosssiteCookies;
    ...
    private HnsDialogPreference mHttpsUpgradePref;
    private HnsDialogPreference mFingerprintingProtectionPref;
    private HnsDialogPreference mRequestOtrPref;
    ...
    private ChromeSwitchPreference mHnsStatsUsagePing;
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        // override title
        getActivity().setTitle(R.string.hns_shields_and_privacy);

        SettingsUtils.addPreferencesFromResource(this, R.xml.hns_privacy_preferences);
        ...
        if (ChromeFeatureList.isEnabled(HnsFeatureList.DEBOUNCE)) {
        ...
        mHttpsUpgradePref = (HnsDialogPreference) findPreference(PREF_HTTPS_UPGRADE);
        mHttpsUpgradePref.setOnPreferenceChangeListener(this);

        boolean httpsByDefaultIsEnabled =
                ChromeFeatureList.isEnabled(HnsFeatureList.HTTPS_BY_DEFAULT);
        ...
        mAdsTrakersBlockPref = (HnsDialogPreference) findPreference(PREF_BLOCK_TRACKERS_ADS);
        ...
        mBlockCrosssiteCookies =
                (HnsDialogPreference) findPreference(PREF_BLOCK_CROSS_SITE_COOKIES);
        ...
        mFingerprintingProtectionPref =
                (HnsDialogPreference) findPreference(PREF_FINGERPRINTING_PROTECTION);
        mFingerprintingProtectionPref.setOnPreferenceChangeListener(this);

        mRequestOtrPref = (HnsDialogPreference) findPreference(PREF_REQUEST_OTR);
        ...
        boolean forgetFirstPartyStorageIsEnabled =
                ChromeFeatureList.isEnabled(HnsFeatureList.HNS_FORGET_FIRST_PARTY_STORAGE);
        ...
        mHnsStatsUsagePing = (ChromeSwitchPreference) findPreference(PREF_HNS_STATS_USAGE_PING);
        mHnsStatsUsagePing.setOnPreferenceChangeListener(this);
        ...
        mSocialBlockingCategory =
                (PreferenceCategory) findPreference(PREF_HNS_SOCIAL_BLOCKING_SECTION);
        ...
        if (!ChromeFeatureList.isEnabled(HnsFeatureList.HNS_GOOGLE_SIGN_IN_PERMISSION)) {
        ...
        if (!ChromeFeatureList.isEnabled(HnsFeatureList.HNS_ANDROID_SAFE_BROWSING)) {
        ...
        updateHnsPreferences();
    }

    // used for displaying HnsDialogPreference
    @Override
    public void onDisplayPreferenceDialog(Preference preference) {
        if (preference instanceof HnsDialogPreference) {
            HnsPreferenceDialogFragment dialogFragment =
                    HnsPreferenceDialogFragment.newInstance((HnsDialogPreference) preference);
            dialogFragment.setTargetFragment(this, 0);
            if (getFragmentManager() != null) {
                dialogFragment.show(getFragmentManager(), HnsPreferenceDialogFragment.TAG);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (PREF_HTTPSE.equals(key)) {
            ...
            HnsShieldsContentSettings.setHTTPSEverywherePref(newValueBool);
            ...
            switch ((int) newValue) {
                case STRICT:
                    HnsShieldsContentSettings.setHttpsUpgradePref(
                            HnsShieldsContentSettings.BLOCK_RESOURCE);
                ...
                default:
                    HnsShieldsContentSettings.setHttpsUpgradePref(
                            HnsShieldsContentSettings.ALLOW_RESOURCE);
                    mHttpsUpgradePref.setSummary(getActivity().getResources().getString(
            ...
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.DE_AMP_PREF_ENABLED, (boolean) newValue);
        } else if (PREF_DEBOUNCE.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.DEBOUNCE_ENABLED, (boolean) newValue);
        } else if (PREF_IPFS_GATEWAY.equals(key)) {
            HnsPrivacySettingsIPFSUtils.setIPFSGatewayPref((boolean) newValue);
            ...
            if (newValue instanceof String) {
                ...
                switch (newStringValue) {
                    case HnsShieldsContentSettings.BLOCK_RESOURCE:
                        HnsShieldsContentSettings.setFingerprintingPref(newStringValue);
                        ...
                        break;
                    case HnsShieldsContentSettings.DEFAULT:
                        HnsShieldsContentSettings.setFingerprintingPref(newStringValue);
                        mFingerprintingProtectionPref.setSummary(
                                getActivity().getResources().getString(
                        ...
                        break;
                    case HnsShieldsContentSettings.ALLOW_RESOURCE:
                    default:
                        HnsShieldsContentSettings.setFingerprintingPref(
                                HnsShieldsContentSettings.ALLOW_RESOURCE);
                        mFingerprintingProtectionPref.setSummary(
                                getActivity().getResources().getString(
            ...
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setInteger(HnsPref.REQUEST_OTR_ACTION_OPTION, (int) newValue);
            updateRequestOtrPref();
        } else if (PREF_FINGERPRINT_LANGUAGE.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.REDUCE_LANGUAGE_ENABLED, (boolean) newValue);
        } else if (PREF_BLOCK_CROSS_SITE_COOKIES.equals(key)) {
            switch ((int) newValue) {
                case STRICT:
                    HnsShieldsContentSettings.setCookiesPref(
                            HnsShieldsContentSettings.BLOCK_RESOURCE);
            ...
            HnsShieldsContentSettings.setJavascriptPref((boolean) newValue);
        } else if (PREF_FORGET_FIRST_PARTY_STORAGE.equals(key)) {
            HnsShieldsContentSettings.setForgetFirstPartyStoragePref((boolean) newValue);
        } else if (PREF_CLOSE_TABS_ON_EXIT.equals(key)) {
            sharedPreferencesEditor.putBoolean(PREF_CLOSE_TABS_ON_EXIT, (boolean) newValue);
        } else if (PREF_SEND_P3A.equals(key)) {
            HnsLocalState.get().setBoolean(HnsPref.P3A_ENABLED, (boolean) newValue);
            HnsLocalState.commitPendingWrite();
        } else if (PREF_SEND_CRASH_REPORTS.equals(key)) {
            UmaSessionStats.changeMetricsReportingConsent(
                    (boolean) newValue, ChangeMetricsReportingStateCalledFrom.UI_SETTINGS);
        } else if (PREF_HNS_STATS_USAGE_PING.equals(key)) {
            HnsLocalState.get().setBoolean(HnsPref.STATS_REPORTING_ENABLED, (boolean) newValue);
            ...
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.AUTOCOMPLETE_ENABLED, autocompleteEnabled);
        } else if (PREF_AUTOCOMPLETE_TOP_SITES.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.TOP_SITE_SUGGESTIONS_ENABLED, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_GOOGLE.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.GOOGLE_LOGIN_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_FACEBOOK.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.FB_EMBED_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_TWITTER.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.TWITTER_EMBED_CONTROL_TYPE, (boolean) newValue);
        } else if (PREF_SOCIAL_BLOCKING_LINKEDIN.equals(key)) {
            UserPrefs.get(Profile.getLastUsedRegularProfile())
                    .setBoolean(HnsPref.LINKED_IN_EMBED_CONTROL_TYPE, (boolean) newValue);
            ...
            if (newValue instanceof String) {
                ...
                switch (newStringValue) {
                    case HnsShieldsContentSettings.BLOCK_RESOURCE:
                        HnsShieldsContentSettings.setTrackersPref(newStringValue);
                    ...
                    case HnsShieldsContentSettings.DEFAULT:
                        ...
                        break;
                    default:
                        HnsShieldsContentSettings.setTrackersPref(
                                HnsShieldsContentSettings.ALLOW_RESOURCE);
                        mAdsTrakersBlockPref.setSummary(getActivity().getResources().getString(
    ...
    private void updateHnsPreferences() {
        ...
        String blockAdTrackersPref = HnsShieldsContentSettings.getTrackersPref();
        int cookiesBlockPref = sharedPreferences.getInt(PREF_BLOCK_CROSS_SITE_COOKIES, 1);
        String fingerprintingPref = HnsShieldsContentSettings.getFingerprintingPref();
        String httpsUpgradePref = HnsShieldsContentSettings.getHttpsUpgradePref();

        mBlockScriptsPref.setChecked(HnsShieldsContentSettings.getJavascriptPref());

        // HTTPSE and HTTPS only mode
        final boolean httpseEnabled = HnsShieldsContentSettings.getHTTPSEverywherePref();
        ...
        // IPFS Gateway
        mIpfsGatewayPref.setChecked(HnsPrivacySettingsIPFSUtils.getIPFSGatewayPref());

        if (blockAdTrackersPref.equals(HnsShieldsContentSettings.BLOCK_RESOURCE)) {
            mAdsTrakersBlockPref.setCheckedIndex(0);
            mAdsTrakersBlockPref.setSummary(
                    getActivity().getResources().getString(R.string.block_trackers_ads_option_1));
        } else if (blockAdTrackersPref.equals(HnsShieldsContentSettings.DEFAULT)) {
            mAdsTrakersBlockPref.setCheckedIndex(1);
            mAdsTrakersBlockPref.setSummary(
                    getActivity().getResources().getString(R.string.block_trackers_ads_option_2));
        } else if (blockAdTrackersPref.equals(HnsShieldsContentSettings.ALLOW_RESOURCE)) {
            mAdsTrakersBlockPref.setCheckedIndex(2);
            mAdsTrakersBlockPref.setSummary(
        ...
        if (fingerprintingPref.equals(HnsShieldsContentSettings.BLOCK_RESOURCE)) {
            mFingerprintingProtectionPref.setCheckedIndex(0);
            mFingerprintingProtectionPref.setSummary(
                    getActivity().getResources().getString(R.string.block_fingerprinting_option_1));
        } else if (fingerprintingPref.equals(HnsShieldsContentSettings.DEFAULT)) {
            mFingerprintingProtectionPref.setCheckedIndex(1);
            mFingerprintingProtectionPref.setSummary(
                    getActivity().getResources().getString(R.string.block_fingerprinting_option_2));
        } else if (fingerprintingPref.equals(HnsShieldsContentSettings.ALLOW_RESOURCE)) {
            mFingerprintingProtectionPref.setCheckedIndex(2);
            mFingerprintingProtectionPref.setSummary(
                    getActivity().getResources().getString(R.string.block_fingerprinting_option_3));
        }

        if (httpsUpgradePref.equals(HnsShieldsContentSettings.BLOCK_RESOURCE)) {
            mHttpsUpgradePref.setCheckedIndex(0);
            mHttpsUpgradePref.setSummary(
                    getActivity().getResources().getString(R.string.https_upgrade_option_1));
        } else if (httpsUpgradePref.equals(HnsShieldsContentSettings.BLOCK_THIRDPARTY_RESOURCE)) {
            mHttpsUpgradePref.setCheckedIndex(1);
            mHttpsUpgradePref.setSummary(
                    getActivity().getResources().getString(R.string.https_upgrade_option_2));
        } else if (httpsUpgradePref.equals(HnsShieldsContentSettings.ALLOW_RESOURCE)) {
            mHttpsUpgradePref.setCheckedIndex(2);
            mHttpsUpgradePref.setSummary(
        ...
        mForgetFirstPartyStoragePref.setChecked(
                HnsShieldsContentSettings.getForgetFirstPartyStoragePref());
        ...
        if (HnsConfig.P3A_ENABLED) {
            mSendP3A.setChecked(HnsLocalState.get().getBoolean(HnsPref.P3A_ENABLED));
        } else {
        ...
        mHnsStatsUsagePing.setChecked(
                HnsLocalState.get().getBoolean(HnsPref.STATS_REPORTING_ENABLED));

        mWebrtcPolicy.setSummary(
                webrtcPolicyToString(HnsPrefServiceBridge.getInstance().getWebrtcPolicy()));

        mAutocompleteTopSites.setChecked(
                UserPrefs.get(Profile.getLastUsedRegularProfile())
                        .getBoolean(HnsPref.TOP_SITE_SUGGESTIONS_ENABLED));
        ...
        boolean autocompleteEnabled = UserPrefs.get(Profile.getLastUsedRegularProfile())
                                              .getBoolean(HnsPref.AUTOCOMPLETE_ENABLED);
        ...
        mFingerprntLanguagePref.setChecked(UserPrefs.get(Profile.getLastUsedRegularProfile())
                                                   .getBoolean(HnsPref.REDUCE_LANGUAGE_ENABLED));
        if (mFilterListAndroidHandler != null) {
        ...
        if (mDebouncePref != null) {
            mDebouncePref.setChecked(UserPrefs.get(Profile.getLastUsedRegularProfile())
                                             .getBoolean(HnsPref.DEBOUNCE_ENABLED));
        }
        ...
        if (mSocialBlockingGoogle != null) {
            mSocialBlockingGoogle.setChecked(
                    UserPrefs.get(Profile.getLastUsedRegularProfile())
                            .getBoolean(HnsPref.GOOGLE_LOGIN_CONTROL_TYPE));
        }
        if (mSocialBlockingFacebook != null) {
            mSocialBlockingFacebook.setChecked(
                    UserPrefs.get(Profile.getLastUsedRegularProfile())
                            .getBoolean(HnsPref.FB_EMBED_CONTROL_TYPE));
        }
        if (mSocialBlockingTwitter != null) {
            mSocialBlockingTwitter.setChecked(
                    UserPrefs.get(Profile.getLastUsedRegularProfile())
                            .getBoolean(HnsPref.TWITTER_EMBED_CONTROL_TYPE));
        }
        if (mSocialBlockingLinkedin != null) {
            mSocialBlockingLinkedin.setChecked(
                    UserPrefs.get(Profile.getLastUsedRegularProfile())
                            .getBoolean(HnsPref.LINKED_IN_EMBED_CONTROL_TYPE));
        }

        if (mDeAmpPref != null) {
            mDeAmpPref.setChecked(UserPrefs.get(Profile.getLastUsedRegularProfile())
                                          .getBoolean(HnsPref.DE_AMP_PREF_ENABLED));
        }

        if (!ChromeFeatureList.isEnabled(HnsFeatureList.HNS_WALLET_SNS)) {
            removePreferenceIfPresent(PREF_SNS);
        }

        if (!ChromeFeatureList.isEnabled(HnsFeatureList.HNS_REQUEST_OTR_TAB)) {
    ...
    private void updateRequestOtrPref() {
        int requestOtrPrefValue = UserPrefs.get(Profile.getLastUsedRegularProfile())
                                          .getInteger(HnsPref.REQUEST_OTR_ACTION_OPTION);
        if (requestOtrPrefValue == HnsShieldsContentSettings.ALWAYS) {
        ...
        } else if (requestOtrPrefValue == HnsShieldsContentSettings.ASK) {
            mRequestOtrPref.setCheckedIndex(1);
            mRequestOtrPref.setSummary(
                    getActivity().getResources().getString(R.string.request_otr_option_2));
        } else if (requestOtrPrefValue == HnsShieldsContentSettings.NEVER) {
    ...
    private String webrtcPolicyToString(@HnsWebrtcPolicyPreference.WebrtcPolicy int policy) {
        switch (policy) {
            case HnsWebrtcPolicyPreference.WebrtcPolicy.DEFAULT:
                return getActivity().getResources().getString(
                        R.string.settings_webrtc_policy_default);
            case HnsWebrtcPolicyPreference.WebrtcPolicy.DEFAULT_PUBLIC_AND_PRIVATE_INTERFACES:
                return getActivity().getResources().getString(
                        R.string.settings_webrtc_policy_default_public_and_private_interfaces);
            case HnsWebrtcPolicyPreference.WebrtcPolicy.DEFAULT_PUBLIC_INTERFACE_ONLY:
                return getActivity().getResources().getString(
                        R.string.settings_webrtc_policy_default_public_interface_only);
            case HnsWebrtcPolicyPreference.WebrtcPolicy.DISABLE_NON_PROXIED_UDP:
                return getActivity().getResources().getString(
