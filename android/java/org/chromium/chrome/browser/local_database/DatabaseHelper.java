...
import org.chromium.hns_news.mojom.Image;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // Database Name
    private static final String DATABASE_NAME = "hns_db";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(HnsStatsTable.CREATE_TABLE);
    ...
    public DisplayAdsTable getDisplayAd(int position, int tabId) {
        ...
        DisplayAdsTable hnsAd = new DisplayAdsTable(
    ...
    public long insertStats(HnsStatsTable hnsStat) {
        // if (!isAdsTrackerAlreadyAdded(hnsStat)) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(HnsStatsTable.COLUMN_URL, hnsStat.getUrl());
        values.put(HnsStatsTable.COLUMN_DOMAIN, hnsStat.getDomain());
        values.put(HnsStatsTable.COLUMN_STAT_TYPE, hnsStat.getStatType());
        values.put(HnsStatsTable.COLUMN_STAT_SITE, hnsStat.getStatSite());
        values.put(HnsStatsTable.COLUMN_STAT_SITE_DOMAIN, hnsStat.getStatSiteDomain());
        values.put(HnsStatsTable.COLUMN_TIMESTAMP, hnsStat.getTimestamp());

        return  db.insert(HnsStatsTable.TABLE_NAME, null, values);
        // }
        // return -1;
    }

    private boolean isAdsTrackerAlreadyAdded(HnsStatsTable hnsStat) {

        String sql = "SELECT * FROM "
                     + HnsStatsTable.TABLE_NAME
                     + " WHERE " + HnsStatsTable.COLUMN_STAT_SITE + " = '" + hnsStat.getStatSite() + "'"
                     + " AND " + HnsStatsTable.COLUMN_URL + " = '" + hnsStat.getUrl() + "'";
    ...
    @SuppressLint("Range")
    public List<HnsStatsTable> getAllStats() {
        List<HnsStatsTable> hnsStats = new ArrayList<>();
        ...
        if (cursor.moveToFirst()) {
            do {
                HnsStatsTable hnsStat = new HnsStatsTable(
                    cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_URL)),
                    cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_DOMAIN)),
                    cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_STAT_TYPE)),
                    cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_STAT_SITE)),
                    cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_STAT_SITE_DOMAIN)),
                    cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_TIMESTAMP)));

                hnsStats.add(hnsStat);
            } while (cursor.moveToNext());
        }
    ...
    @SuppressLint("Range")
    public List<HnsStatsTable> getAllStatsWithDate(String thresholdTime, String currentTime) {
        List<HnsStatsTable> hnsStats = new ArrayList<>();
        // Select All Query
        String selectQuery = "SELECT  * FROM "
                             + HnsStatsTable.TABLE_NAME
                             + " WHERE " + HnsStatsTable.COLUMN_TIMESTAMP
    ...
    public List<Pair<String, Integer>> getStatsWithDate(String thresholdTime, String currentTime) {
        List<Pair<String, Integer>> hnsStats = new ArrayList<>();

        String selectQuery = "SELECT  " + HnsStatsTable.COLUMN_DOMAIN + ", " + HnsStatsTable.COLUMN_TIMESTAMP + " , COUNT(*) as stat_count FROM "
                             + HnsStatsTable.TABLE_NAME
                             + " WHERE " + HnsStatsTable.COLUMN_TIMESTAMP
                             + " BETWEEN date('" + thresholdTime + "') AND date('" + currentTime + "')"
                             + " GROUP BY " + HnsStatsTable.COLUMN_DOMAIN
        ...
        if (cursor.moveToFirst()) {
            do {
                Pair<String, Integer> statPair = new Pair<>(cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_DOMAIN)), cursor.getInt(cursor.getColumnIndex("stat_count")));
                hnsStats.add(statPair);
            } while (cursor.moveToNext());
        }
    ...
    public List<Pair<String, Integer>> getSitesWithDate(String thresholdTime, String currentTime) {
        List<Pair<String, Integer>> hnsStats = new ArrayList<>();
        // Select All Query
        String selectQuery = "SELECT  " + HnsStatsTable.COLUMN_STAT_SITE_DOMAIN + ", COUNT(*) as site_count FROM "
                             + HnsStatsTable.TABLE_NAME
                             + " WHERE " + HnsStatsTable.COLUMN_TIMESTAMP
                             + " BETWEEN date('" + thresholdTime + "') AND date('" + currentTime + "')"
                             + " GROUP BY " + HnsStatsTable.COLUMN_STAT_SITE_DOMAIN
        ...
        if (cursor.moveToFirst()) {
            do {
                Pair<String, Integer> statPair = new Pair<>(cursor.getString(cursor.getColumnIndex(HnsStatsTable.COLUMN_STAT_SITE_DOMAIN)), cursor.getInt(cursor.getColumnIndex("site_count")));
                hnsStats.add(statPair);
            } while (cursor.moveToNext());
        }
    ...
    public void clearStatsTable() {
        String selectQuery = "DELETE FROM " + HnsStatsTable.TABLE_NAME;
        SQLiteDatabase db = this.getWritableDatabase();
