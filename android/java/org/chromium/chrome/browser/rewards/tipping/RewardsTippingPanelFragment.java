...
public class RewardsTippingPanelFragment
        extends BottomSheetDialogFragment implements HnsRewardsObserver {
    ...
    private String mWalletType = HnsRewardsNativeWorker.getInstance().getExternalWalletType();
    ...
    public Dialog onCreateDialog(Bundle savedInstance) {
        ...
        final View view = LayoutInflater.from(getContext())
                                  .inflate(R.layout.hns_rewards_tippingpanel_fragment, null);
    ...
    private void updateTermsOfServicePlaceHolder(View view) {
        ...
        String termsOfServiceText = String.format(res.getString(R.string.hns_rewards_tos_text),
                res.getString(R.string.terms_of_service), res.getString(R.string.privacy_policy));

        SpannableString spannableString = HnsRewardsHelper.tosSpannableString(
            ...
            try {
                ...
                if (walletStatus != WalletStatus.NOT_CONNECTED) {
                    if (walletStatus == WalletStatus.LOGGED_OUT) {
                        ...
                        int pubStatus = mHnsRewardsNativeWorker.GetPublisherStatus(mCurrentTabId);
    ...
    private void setPublisherNoteText(int pubStatus, int walletStatus) {
        if ((pubStatus == PublisherStatus.UPHOLD_VERIFIED
                    && !mWalletType.equals(HnsWalletProvider.UPHOLD))
                || (pubStatus == PublisherStatus.BITFLYER_VERIFIED
                        && !mWalletType.equals(HnsWalletProvider.BITFLYER))
                || (pubStatus == PublisherStatus.GEMINI_VERIFIED
                        && !mWalletType.equals(HnsWalletProvider.GEMINI))) {
            String notePart1 =
    ...
    private void setAlreadyMonthlyContributionSetMessage() {
        ...
        boolean isPreviouslyMonthlyContributionExist =
                mHnsRewardsNativeWorker.IsCurrentPublisherInRecurrentDonations(pubId);
    ...
    private SpannableString stringMonthlyToSpannableString(String text) {
        Spanned textSpanned = HnsRewardsHelper.spannedFromHtmlString(text);
        ...
        NoUnderlineClickableSpan monthlyContributionClickableSpan = new NoUnderlineClickableSpan(
                getActivity(), R.color.monthly_contributions_text_color, (textView) -> {
                    TabUtils.openUrlInNewTab(
                            false, HnsActivity.HNS_REWARDS_SETTINGS_MONTHLY_URL);
                    getActivity().setResult(Activity.RESULT_OK);
                    getActivity().finish();
                });

        HnsRewardsHelper.setSpan(getActivity(), text, textSpannableString,
        ...
        mSendButton.setOnClickListener(v -> {
            if (mIsLogoutState) {
                ...
                if (mSendButton.isEnabled()) {
                    mHnsRewardsNativeWorker.Donate(
                            mHnsRewardsNativeWorker.GetPublisherId(mCurrentTabId),
    ...
    private void init(View view) {
        ...
        mRate = mHnsRewardsNativeWorker.GetWalletRate();
    ...
    void setBalanceText(View view) {
        ...
        HnsRewardsBalance rewards_balance = mHnsRewardsNativeWorker.GetWalletBalance();
        ...
        String walletAmount = df.format(balance) + " " + HnsRewardsHelper.BAT_TEXT;
    ...
    private void setCustodianIconAndName(View view) {
        ...
        if (mWalletType.equals(HnsWalletProvider.UPHOLD)) {
            custodianIcon = R.drawable.upload_icon;
            custodianName = R.string.uphold;
        } else if (mWalletType.equals(HnsWalletProvider.GEMINI)) {
            custodianIcon = R.drawable.ic_gemini_logo_cyan;
            custodianName = R.string.gemini;
        } else if (mWalletType.equals(HnsWalletProvider.ZEBPAY)) {
    ...
    private void initTipChoice(boolean isBat) {
        mTipChoices = mHnsRewardsNativeWorker.GetTipChoices();
    ...
    public void onDestroyView() {
        super.onDestroyView();
        if (null != mHnsRewardsNativeWorker) {
