...
public class AdaptiveCaptchaHelper {
    ...
    public static void startAttestation(String captchaId, String paymentId) {
        PostTask.postTask(TaskTraits.BEST_EFFORT_MAY_BLOCK, () -> {
            ...
            String startAttestationUrl = HnsRewardsNativeWorker.getInstance().getAttestationURL();
            if (HnsQAPreferences.shouldVlogRewards()) {
                Log.e(TAG, startAttestationUrl);
            }
            NetworkTrafficAnnotationTag annotation =
                    NetworkTrafficAnnotationTag.createComplete("Hns attestation api android",
                            "semantics {"
                                    + "  sender: 'Hns Android app'"
                                    + "  description: "
                                    + "    'This api gets unique value for payment ID'"
                                    + "  trigger: 'When payment id as captcha scheduled'"
                                    + "  data:"
                                    + "    'payment id'"
                                    + "  destination: Hns grant endpoint"
                                    + "}"
                                    + "policy {"
                                    + "  cookies_allowed: NO"
    ...
    private static void solveCaptcha(String captchaId, String paymentId) {
        PostTask.postTask(TaskTraits.BEST_EFFORT_MAY_BLOCK, () -> {
            HttpURLConnection urlConnection = null;
            String solveCaptchaUrl = HnsRewardsNativeWorker.getInstance().getCaptchaSolutionURL(
                    paymentId, captchaId);
            if (HnsQAPreferences.shouldVlogRewards()) {
    ...
    private static void clearCaptchaPrefs() {
        UserPrefs.get(Profile.getLastUsedRegularProfile())
                .setInteger(HnsPref.SCHEDULED_CAPTCHA_FAILED_ATTEMPTS, 0);
        UserPrefs.get(Profile.getLastUsedRegularProfile())
                .setString(HnsPref.SCHEDULED_CAPTCHA_ID, "");
        UserPrefs.get(Profile.getLastUsedRegularProfile())
                .setString(HnsPref.SCHEDULED_CAPTCHA_PAYMENT_ID, "");
        UserPrefs.get(Profile.getLastUsedRegularProfile())
                .setBoolean(HnsPref.SCHEDULED_CAPTCHA_PAUSED, false);
    }

    private static void recordFailureAttempt() {
        UserPrefs.get(Profile.getLastUsedRegularProfile())
                .setInteger(HnsPref.SCHEDULED_CAPTCHA_FAILED_ATTEMPTS,
                        UserPrefs.get(Profile.getLastUsedRegularProfile())
