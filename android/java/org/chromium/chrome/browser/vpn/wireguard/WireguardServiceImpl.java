...
public class WireguardServiceImpl
    ...
    private static final int HNS_VPN_NOTIFICATION_ID = 801;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        getService().startForeground(HNS_VPN_NOTIFICATION_ID, getHnsVpnNotification(""));
    ...
    private Notification getHnsVpnNotification(String notificationText) {
        ...
        NotificationCompat.Builder notificationBuilder =
                new NotificationCompat.Builder(mContext, HnsActivity.CHANNEL_ID);
        notificationBuilder.setSmallIcon(R.drawable.ic_vpn)
                ...
                .setContentTitle(
                        String.format(mContext.getResources().getString(R.string.connected_to_host),
                                HnsVpnPrefUtils.getHostnameDisplay()))
    ...
    private void updateVpnNotification(String notificationText) {
        Notification notification = getHnsVpnNotification(notificationText);
        NotificationManager mNotificationManager =
                (NotificationManager) mContext.getSystemService(Context.NOTIFICATION_SERVICE);
        mNotificationManager.notify(HNS_VPN_NOTIFICATION_ID, notification);
    }

    private void recordSessionTimes() {
        long sessionStartTimeMs = HnsVpnPrefUtils.getSessionStartTimeMs();
        long sessionEndTimeMs = HnsVpnPrefUtils.getSessionEndTimeMs();
        ...
        if (sessionStartTimeMs < 0) {
            HnsVpnPrefUtils.setSessionStartTimeMs(currTimeMs);
        }
        ...
        if (currDate.get(Calendar.YEAR) != sessionEndTimeCal.get(Calendar.YEAR)
            ...
            HnsVpnPrefUtils.setSessionEndTimeMs(currTimeMs);
        }
    }
