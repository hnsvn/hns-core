...
public class HnsWalletEthereumConnectedSitesPreference
    ...
    public HnsWalletEthereumConnectedSitesPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    private void updateWebSitestList() {
        mHnsWalletService.getWebSitesWithPermission(CoinType.ETH, webSites -> {
            if (mAdapter == null) {
    ...
    public void removePermission(String webSite) {
        mHnsWalletService.resetWebSitePermission(CoinType.ETH, webSite, success -> {
            if (success) {
                updateWebSitestList();
            }
        });
