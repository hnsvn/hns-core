...
public class HnsAppMenuPropertiesDelegateImpl extends AppMenuPropertiesDelegateImpl {
    public HnsAppMenuPropertiesDelegateImpl(Context context,
    ...
    public ModelList getMenuItems(
        ...
        for (int i = 0; i < modelList.size(); ++i) {
            ...
            if (id == R.id.request_hns_vpn_row_menu_id) {
            ...
            for (int i = 0; i < menu.size(); ++i) {
                ...
                if (item.hasSubMenu()) {
                    ...
                    item.setIcon(AppCompatResources.getDrawable(
                            mContext, R.drawable.hns_menu_new_private_tab));
                } else if (item.getItemId() == R.id.all_bookmarks_menu_id) {
                    item.setIcon(AppCompatResources.getDrawable(
                            mContext, R.drawable.hns_menu_bookmarks));
                } else if (item.getItemId() == R.id.recent_tabs_menu_id) {
                    item.setIcon(AppCompatResources.getDrawable(
                            mContext, R.drawable.hns_menu_recent_tabs));
                } else if (item.getItemId() == R.id.open_history_menu_id) {
                    item.setIcon(AppCompatResources.getDrawable(
                            mContext, R.drawable.hns_menu_history));
                } else if (item.getItemId() == R.id.downloads_menu_id) {
                    item.setIcon(AppCompatResources.getDrawable(
                            mContext, R.drawable.hns_menu_downloads));
                } else if (item.getItemId() == R.id.preferences_id) {
                    item.setIcon(AppCompatResources.getDrawable(
                            mContext, R.drawable.hns_menu_settings));
                }
            }
        }
    }
