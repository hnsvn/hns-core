...
public class DappsModel implements KeyringServiceObserver {
    ...
    private final MutableLiveData<HnsWalletDAppsActivity.ActivityType> _mProcessNextDAppsRequest =
            new MutableLiveData<>();
    ...
    public final LiveData<HnsWalletDAppsActivity.ActivityType> mProcessNextDAppsRequest =
            _mProcessNextDAppsRequest;

    public DappsModel(JsonRpcService jsonRpcService, HnsWalletService hnsWalletService,
    ...
    public void signTxRequest(boolean isApproved, SignTransactionRequest signTransactionRequest) {
        mHnsWalletService.notifySignTransactionRequestProcessed(
                isApproved, signTransactionRequest.id, null, null);
        mHnsWalletService.getPendingSignTransactionRequests(requests -> {
            if (requests.length == 0) {
                _mProcessNextDAppsRequest.postValue(HnsWalletDAppsActivity.ActivityType.FINISH);
            } else {
                _mSignTxRequests.postValue(new ArrayList<>(Arrays.asList(requests)));
            }
        });
    ...
    public void signAllTxRequest(boolean isApproved, SignAllTransactionsRequest request) {
        mHnsWalletService.notifySignAllTransactionsRequestProcessed(
    ...
    public void resetServices(JsonRpcService jsonRpcService,
            HnsWalletService hnsWalletService, PendingTxHelper pendingTxHelper) {
        synchronized (mLock) {
            mHnsWalletService = hnsWalletService;
            mJsonRpcService = jsonRpcService;
            mPendingTxHelper = pendingTxHelper;
    ...
    public void processPublicEncryptionKey(String requestId, boolean isApproved) {
        synchronized (mLock) {
            ...
            mHnsWalletService.notifyGetPublicKeyRequestProcessed(requestId, isApproved);
            mHnsWalletService.getPendingGetEncryptionPublicKeyRequests(requests -> {
                if (requests != null && requests.length > 0) {
                    _mProcessNextDAppsRequest.postValue(HnsWalletDAppsActivity.ActivityType
                                                                .GET_ENCRYPTION_PUBLIC_KEY_REQUEST);
                } else {
                    _mProcessNextDAppsRequest.postValue(
                            HnsWalletDAppsActivity.ActivityType.FINISH);
                }
            });
        }
    }
    ...
    public void processDecryptRequest(String requestId, boolean isApproved) {
        synchronized (mLock) {
            ...
            mHnsWalletService.notifyDecryptRequestProcessed(requestId, isApproved);
            mHnsWalletService.getPendingDecryptRequests(requests -> {
                if (requests != null && requests.length > 0) {
                    _mProcessNextDAppsRequest.postValue(
                            HnsWalletDAppsActivity.ActivityType.DECRYPT_REQUEST);
                } else {
                    _mProcessNextDAppsRequest.postValue(
                            HnsWalletDAppsActivity.ActivityType.FINISH);
                }
    ...
    private void updateWalletBadgeVisibilityInternal() {
        if (mHnsWalletService == null || mJsonRpcService == null || mPendingTxHelper == null) {
            return;
        }
