        ...
        stage('build') {
            ...
            steps {
                script {
                    ...
                    PIPELINE_NAME = 'pr-hns-browser-' + CHANGE_BRANCH.replace('/', '-') + '-' + PLATFORM

                    withCredentials([usernamePassword(credentialsId: 'hns-builds-github-token-for-pr-builder', usernameVariable: 'PR_BUILDER_USER', passwordVariable: 'PR_BUILDER_TOKEN')]) {
                        GITHUB_API = 'https://api.github.com/repos/hns'
                        ...
                        def prDetails = readJSON(text: httpRequest(url: GITHUB_API + '/hns-core/pulls?head=hns:' + CHANGE_BRANCH_ENCODED, customHeaders: GITHUB_AUTH_HEADERS, quiet: true).content)[0]
                            ...
                            definition {
                                cpsScm {
                                    scm {
                                        git {
                                            remote {
                                                credentials('hns-builds-github-token-for-pr-builder')
                                                github('hns/devops', 'https')
                                            }
                                            branch('${params.DEVOPS_BRANCH}')
                                        }
                                    }
                                    scriptPath("jenkins/jobs/browser/pr-hns-browser-${PLATFORM}.Jenkinsfile")
                                }
                            }
                        }
                    """)
