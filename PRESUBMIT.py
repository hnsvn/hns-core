...
def CheckLeoVariables(input_api, output_api):
    ...
    try:
        parts = [
            hns_node.PathInNodeModules('@hns', 'leo', 'src', 'scripts',
                                         'audit-tokens.js')
        ]
        hns_node.RunNode(parts)
        return []
...
def CheckPatchFormatted(input_api, output_api):
    ...
    if not is_format_required or input_api.PRESUBMIT_FIX:
        ...
        node_args = [
            hns_node.PathInNodeModules('prettier', 'bin-prettier'),
...
def CheckLicense(input_api, output_api):
    """Verifies the Hns license header."""

    files_to_check = input_api.DEFAULT_FILES_TO_CHECK + (r'.+\.gni?$', )
    files_to_skip = input_api.DEFAULT_FILES_TO_SKIP + (
        r"\.storybook/",
        r"ios/browser/api/hns_rewards/legacy_database/core_data_models/",
        r'win_build_output/',
    )
    ...
    new_file_license_re = input_api.re.compile((
        r'.*? Copyright \(c\) %(year)s The Hns Authors\. All rights reserved\.\n'
    ...
    expected_license_template = (
        '%(comment)s Copyright (c) %(year)s The Hns Authors. All rights reserved.\n'
...
# DON'T ADD NEW HNS CHECKS AFTER THIS LINE.
#
...
# Extend BanRule exclude lists with Hns-specific paths.
...
def CheckForIncludeGuards(original_check, input_api, output_api, **kwargs):
    # Add 'hns/' prefix for header guard checks to properly validate guards.
    def AffectedSourceFiles(self, original_method, source_file):
        def PrependHns(affected_file):
            affected_file = copy.copy(affected_file)
            affected_file._path = f'hns/{affected_file._path}'
...
def CheckJavaStyle(_original_check, input_api, output_api):
    ...
    hns/tools/android/checkstyle/hns-style-5.0.xml style file where all
    ...
    local_path = os.path.join(input_api.PresubmitLocalPath(), 'hns')
    style_file = os.path.join(input_api.PresubmitLocalPath(), 'hns', 'tools',
                              'android', 'checkstyle', 'hns-style-5.0.xml')
