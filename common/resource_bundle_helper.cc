...
#if BUILDFLAG(IS_MAC)
  return base::apple::PathForFrameworkBundleResource("hns_resources.pak");
#else
  base::FilePath pak_path;
  base::PathService::Get(base::DIR_ASSETS, &pak_path);
  pak_path = pak_path.AppendASCII("hns_resources.pak");
  return pak_path;
#endif  // OS_MAC
}
#endif  // OS_ANDROID
...
#if BUILDFLAG(IS_ANDROID)
  ui::HnsLoadMainAndroidPackFile("assets/hns_resources.pak",
                                   base::FilePath());
  // hns_100_percent.pak is excluded now from the Android build because
