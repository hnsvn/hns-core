...
HnsContentClient::HnsContentClient() = default;

HnsContentClient::~HnsContentClient() = default;

base::RefCountedMemory* HnsContentClient::GetDataResourceBytes(
    int resource_id) {
  if (resource_id == IDR_FLAGS_UI_FLAGS_JS) {
    ...
    const std::string flags_js =
        ...
        resource_bundle.LoadDataResourceString(
            IDR_FLAGS_UI_HNS_FLAGS_OVERRIDES_JS);
...
void HnsContentClient::AddAdditionalSchemes(Schemes* schemes) {
  ChromeContentClient::AddAdditionalSchemes(schemes);
  schemes->standard_schemes.push_back(content::kHnsUIScheme);
  schemes->secure_schemes.push_back(content::kHnsUIScheme);
  schemes->cors_enabled_schemes.push_back(content::kHnsUIScheme);
  schemes->savable_schemes.push_back(content::kHnsUIScheme);
}

void HnsContentClient::AddContentDecryptionModules(
