...
#import "hns/ios/browser/api/net/certificate_utility.h"
...
// AmazonRootCA1
// From:
// https://github.com/hnsvn/hns-core/blob/master/chromium_src/net/tools/transport_security_state_generator/input_file_parsers.cc
static const char* hns_test_cert = R"(
...
+ (nullable SecCertificateRef)getCertificate {
  NSString* cert_string = [NSString stringWithFormat:@"%s", hns_test_cert];
...
- (void)testAcceptableCertificates {
  std::size_t acceptable_certs_count =
      sizeof(net::kHnsAcceptableCerts) /
          sizeof(net::kHnsAcceptableCerts[0]) -
      1;

  NSArray<NSData*>* certs = [HnsCertificateUtility acceptableSPKIHashes];

  XCTAssertTrue([certs count] > 3);
  XCTAssertTrue([certs count] == acceptable_certs_count);

  std::string test_cert = std::string(net::kHnsAcceptableCerts[0]);
  if (test_cert.size() > 0) {
...
- (void)testHashCertificateSPKI {
  ...
  NSData* spki_hash =
      [HnsCertificateUtility hashCertificateSPKI:[NetTest getCertificate]];
