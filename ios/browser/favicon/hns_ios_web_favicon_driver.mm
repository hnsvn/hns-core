...
#import "hns/ios/browser/favicon/hns_ios_web_favicon_driver.h"
...
void HnsIOSWebFaviconDriver::SetMaximumFaviconImageSize(
...
gfx::Image HnsIOSWebFaviconDriver::GetFavicon() const {
  DCHECK(web_state_);
  return web_state_->GetFaviconStatus().image;
}

bool HnsIOSWebFaviconDriver::FaviconIsValid() const {
  DCHECK(web_state_);
  return web_state_->GetFaviconStatus().valid;
}

GURL HnsIOSWebFaviconDriver::GetActiveURL() {
  DCHECK(web_state_);
...
int HnsIOSWebFaviconDriver::DownloadImage(const GURL& url,
                                            int max_image_size,
...
void HnsIOSWebFaviconDriver::DownloadManifest(
    const GURL& url,
    ManifestDownloadCallback callback) {
  NOTREACHED();
}

bool HnsIOSWebFaviconDriver::IsOffTheRecord() {
  DCHECK(web_state_);
  return web_state_->GetBrowserState()->IsOffTheRecord();
}

void HnsIOSWebFaviconDriver::OnFaviconUpdated(
    const GURL& page_url,
...
void HnsIOSWebFaviconDriver::OnFaviconDeleted(
    const GURL& page_url,
...
HnsIOSWebFaviconDriver::HnsIOSWebFaviconDriver(
...
void HnsIOSWebFaviconDriver::FaviconUrlUpdated(
    web::WebState* web_state,
...
void HnsIOSWebFaviconDriver::WebStateDestroyed(web::WebState* web_state) {
  DCHECK_EQ(web_state_, web_state);
  web_state_->RemoveObserver(this);
  web_state_ = nullptr;
}

void HnsIOSWebFaviconDriver::SetFaviconStatus(
    const GURL& page_url,
...
WEB_STATE_USER_DATA_KEY_IMPL(HnsIOSWebFaviconDriver)
