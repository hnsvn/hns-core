...
#include "hns/ios/browser/api/history/hns_history_observer.h"
#include "hns/ios/browser/api/history/history_service_listener_ios.h"
...
#pragma mark - HnsHistoryAPI

@interface HnsHistoryAPI () {
...
- (void)removeAllWithCompletion:(void (^)())completion {
  ...
  __weak HnsHistoryAPI* weak_history_api = self;
  auto delete_history = ^(void (^callback)()) {
    HnsHistoryAPI* historyAPI = weak_history_api;
    if (!historyAPI) {
      callback();
      return;
    }
