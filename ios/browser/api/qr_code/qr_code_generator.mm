...
#include "hns/ios/browser/api/qr_code/qr_code_generator.h"
#include "base/strings/sys_string_conversions.h"
#include "hns/ios/browser/qr_code_generator/qrcode_generator_service.h"
#include "hns/ios/browser/qr_code_generator/qrcode_models.h"
...
// MARK: - HnsQRCodeGeneratorOptions
@interface HnsQRCodeGeneratorOptions () {
  std::unique_ptr<qrcode_generator::GenerateQRCodeRequest> request_;
}
@end

@implementation HnsQRCodeGeneratorOptions
...
// MARK: - HnsQRCodeGeneratorResult

@implementation HnsQRCodeGeneratorResult
- (instancetype)initWithResponse:
    (qrcode_generator::GenerateQRCodeResponse*)response {
  if ((self = [super init])) {
    _errorCode = static_cast<HnsQRCodeGeneratorError>(response->error_code);
...
// MARK: - HnsQRCodeGenerator

@implementation HnsQRCodeGenerator
- (HnsQRCodeGeneratorResult*)generateQRCode:
  ...
  return [[HnsQRCodeGeneratorResult alloc] initWithResponse:response.get()];
}
@end
