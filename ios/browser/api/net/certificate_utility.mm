...
@implementation HnsCertificateUtility
+ (NSArray<NSData*>*)acceptableSPKIHashes {
  NSMutableArray* result = [[NSMutableArray alloc] init];
  for (std::size_t i = 0; i < sizeof(net::kHnsAcceptableCerts) /
                                  sizeof(net::kHnsAcceptableCerts[0]);
       ++i) {
    if (net::kHnsAcceptableCerts[i]) {
      std::string data = std::string(net::kHnsAcceptableCerts[i]);
      if (data.size() > 0) {
