...
@implementation HnsCertificateRDNSequence
- (instancetype)initWithBERName:(const net::der::Input&)berName
  ...
  if ((self = [super init])) {
    ...
    _organization = hns::vector_to_ns(rdns.organization_names);
    _organizationalUnit = hns::vector_to_ns(rdns.organization_unit_names);
