...
@implementation HnsCertificatePublicKeyInfo
...
- (instancetype)initWithCertificate:(net::ParsedCertificate*)certificate
  ...
  if ((self = [super init])) {
    _type = HnsPublicKeyType_UNKNOWN;
    _keyUsage = HnsPublicKeyUsage_INVALID;
    ...
    if (attributes) {
      ...
      if ([attributes[(NSString*)kSecAttrCanEncrypt] boolValue]) {
        _keyUsage |= HnsPublicKeyUsage_ENCRYPT;
      }

      if ([attributes[(NSString*)kSecAttrCanDecrypt] boolValue]) {
        _keyUsage |= HnsPublicKeyUsage_DECRYPT;
      }

      if ([attributes[(NSString*)kSecAttrCanSign] boolValue]) {
        _keyUsage |= HnsPublicKeyUsage_SIGN;
      }

      if ([attributes[(NSString*)kSecAttrCanVerify] boolValue]) {
        _keyUsage |= HnsPublicKeyUsage_VERIFY;
      }

      if ([attributes[(NSString*)kSecAttrCanWrap] boolValue]) {
        _keyUsage |= HnsPublicKeyUsage_WRAP;
      }

      if ([attributes[(NSString*)kSecAttrCanDerive] boolValue]) {
        _keyUsage |= HnsPublicKeyUsage_DERIVE;
      }

      HnsPublicKeyUsage any_usage = static_cast<HnsPublicKeyUsage>(0);
      any_usage &= HnsPublicKeyUsage_ENCRYPT;
      any_usage &= HnsPublicKeyUsage_SIGN;
      any_usage &= HnsPublicKeyUsage_VERIFY;
      any_usage &= HnsPublicKeyUsage_WRAP;
      any_usage &= HnsPublicKeyUsage_DERIVE;

      if (_keyUsage == any_usage) {
        _keyUsage = HnsPublicKeyUsage_ANY;
      }

      if ([attributes[(NSString*)kSecAttrType]
              isEqualToString:(NSString*)kSecAttrKeyTypeRSA]) {
        _type = HnsPublicKeyType_RSA;
        _algorithm = @"RSA";
