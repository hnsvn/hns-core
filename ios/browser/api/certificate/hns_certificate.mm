...
#include "hns/ios/browser/api/certificate/hns_certificate.h"
...
#include "hns/ios/browser/api/certificate/models/hns_certificate_model+private.h"
...
@implementation HnsCertificateModel
...
- (void)parseCertificate {
  ...
  _subjectName = [[HnsCertificateRDNSequence alloc]
      initWithBERName:extended_cert_->tbs().subject_tlv
             uniqueId:subject_unique_id];
  _issuerName = [[HnsCertificateRDNSequence alloc]
  ...
  _version = static_cast<std::int32_t>(extended_cert_->tbs().version) +
             1;  // version + 1
  _signature = [[HnsCertificateSignature alloc]
  ...
  _notValidAfter = certificate::x509_utils::GeneralizedTimeToTime(
                       ...
                       .ToNSDate();
  _publicKeyInfo = [[HnsCertificatePublicKeyInfo alloc]
      initWithCertificate:extended_cert_.get()
                  withKey:public_key_.get()];
  //  _extensions = nullptr;
  _sha1Fingerprint = [[HnsCertificateFingerprint alloc]
      initWithCertificate:cert_data_.get()
                 withType:HnsFingerprintType_SHA1];
  _sha256Fingerprint = [[HnsCertificateFingerprint alloc]
      initWithCertificate:cert_data_.get()
                 withType:HnsFingerprintType_SHA256];
