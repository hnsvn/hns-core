...
#include "hns/ios/browser/api/bookmarks/importer/hns_bookmarks_importer.h"
...
#include "hns/ios/browser/api/bookmarks/importer/bookmark_html_reader.h"
#include "hns/ios/browser/api/bookmarks/importer/bookmarks_importer.h"
#include "hns/ios/browser/api/bookmarks/importer/imported_bookmark_entry.h"
...
@implementation HnsImportedBookmark
...
- (void)importFromFile:(NSString*)filePath
  ...
  auto start_import = [](HnsBookmarksImporter* weak_importer,
                         const base::FilePath& source_file_path,
                         const std::u16string& top_level_folder_name,
                         bool automaticImport,
                         std::function<void(HnsBookmarksImporterState,
                                            NSArray<HnsImportedBookmark*>*)>
                             listener) {
    // Import cancelled as the importer has been deallocated
    __strong HnsBookmarksImporter* importer = weak_importer;
    if (!importer) {
      listener(HnsBookmarksImporterStateStarted, nullptr);
      listener(HnsBookmarksImporterStateCancelled, nullptr);
      return;
    }
    ...
    bookmark_html_reader::ImportBookmarksFile(
        base::BindRepeating(
            [](HnsBookmarksImporter* importer) -> bool {
              return [importer isImporterCancelled];
            },
            base::Unretained(importer)),
        base::BindRepeating(
            [](HnsBookmarksImporter* importer, const GURL& url) -> bool {
      ...
      if (automaticImport) {
        auto complete_import =
            [](std::vector<ImportedBookmarkEntry> bookmarks,
               const std::u16string& top_level_folder_name,
               std::function<void(HnsBookmarksImporterState,
                                  NSArray<HnsImportedBookmark*>*)> listener) {
              BookmarksImporter::AddBookmarks(top_level_folder_name, bookmarks);
              listener(HnsBookmarksImporterStateAutoCompleted, nullptr);
            };
        ...
        listener(HnsBookmarksImporterStateCompleted,
  ...
  __weak HnsBookmarksImporter* weakSelf = self;
...
- (void)importFromArray:(NSArray<HnsImportedBookmark*>*)bookmarks
  ...
  auto start_import =
      [](HnsBookmarksImporter* weak_importer,
         NSArray<HnsImportedBookmark*>* bookmarks,
         const std::u16string& top_level_folder_name,
         std::function<void(HnsBookmarksImporterState)> listener) {
        // Import cancelled as the importer has been deallocated
        __strong HnsBookmarksImporter* importer = weak_importer;
        if (!importer) {
          listener(HnsBookmarksImporterStateStarted);
          listener(HnsBookmarksImporterStateCancelled);
          return;
        }
...
- (NSArray<HnsImportedBookmark*>*)convertToIOSImportedBookmarks:
    (const std::vector<ImportedBookmarkEntry>&)bookmarks {
  NSMutableArray<HnsImportedBookmark*>* results =
      [[NSMutableArray alloc] init];
  for (const auto& bookmark : bookmarks) {
    HnsImportedBookmark* imported_bookmark = [[HnsImportedBookmark alloc]
...
- (std::vector<ImportedBookmarkEntry>)convertToChromiumImportedBookmarks:
  ...
  for (HnsImportedBookmark* bookmark in bookmarks) {
