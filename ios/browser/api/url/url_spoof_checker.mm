...
#include "hns/ios/browser/api/url/url_spoof_checker.h"
...
HnsSpoofCheckerSkeletonType const HnsSpoofCheckerSkeletonTypeFull =
    static_cast<NSInteger>(url_formatter::SkeletonType::kFull);
HnsSpoofCheckerSkeletonType const
...
HnsSpoofCheckerLookalikeURLMatchType const
...
HnsSpoofCheckerLookalikeURLMatchType const
    HnsSpoofCheckerLookalikeURLMatchTypeEditDistance = static_cast<NSInteger>(
...
- (instancetype)initWithEntry:(url_formatter::TopDomainEntry)entry {
  if ((self = [super init])) {
    ...
    _skeletonType = static_cast<HnsSpoofCheckerLookalikeURLMatchType>(
        entry.skeleton_type);
  }
  return self;
}
...
@implementation HnsURLSpoofChecker
...
+ (HnsURLSpoofCheckerResult*)isLookalikeURL:(NSString*)url {
  ...
  if (!response_url.SchemeIsHTTPOrHTTPS()) {
    return [[HnsURLSpoofCheckerResult alloc]
