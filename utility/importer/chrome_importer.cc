...
#include "hns/common/importer/scoped_copy_file.h"
...
void ChromeImporter::ImportPayments() {
  ...
  sql::Statement s(db.GetUniqueStatement(query));
  auto* hns_bridge =
      static_cast<HnsExternalProcessImporterBridge*>(bridge_.get());
  while (s.Step()) {
    ...
    if (card_number.empty())
      continue;
    hns_bridge->SetCreditCard(s.ColumnString16(0), s.ColumnString16(1),
