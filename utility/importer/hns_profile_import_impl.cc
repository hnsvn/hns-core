...
#include "hns/utility/importer/chrome_importer.h"
...
HnsProfileImportImpl::HnsProfileImportImpl(
    mojo::PendingReceiver<hns::mojom::ProfileImport> receiver)
    : receiver_(this, std::move(receiver)) {}

HnsProfileImportImpl::~HnsProfileImportImpl() = default;

void HnsProfileImportImpl::StartImport(
  ...
  bridge_ = new HnsExternalProcessImporterBridge(
      localized_strings,
      mojo::SharedRemote<chrome::mojom::ProfileImportObserver>(
          std::move(observer)),
      mojo::SharedRemote<hns::mojom::ProfileImportObserver>(
          std::move(hns_observer)));
...
void HnsProfileImportImpl::CancelImport() {
  ImporterCleanup();
}

void HnsProfileImportImpl::ReportImportItemFinished(
...
void HnsProfileImportImpl::ImporterCleanup() {
  importer_->Cancel();
  importer_.reset();
  bridge_.reset();
