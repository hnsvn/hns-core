...
// same structure as ~/Library/Application Support/Google/Chrome in the Hns
...
TEST_F(ChromeImporterTest, ImportHistory) {
  ...
  EXPECT_EQ("https://hns.com/", history[0].url.spec());
  EXPECT_EQ("https://github.com/hns", history[1].url.spec());
...
TEST_F(ChromeImporterTest, ImportBookmarks) {
  ...
  EXPECT_EQ("https://hns.com/", bookmarks[2].url.spec());
...
TEST_F(ChromeImporterTest, ImportFavicons) {
  ...
  EXPECT_EQ("https://hns.com/images/cropped-hns_appicon_release-32x32.png",
