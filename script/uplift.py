...
from lib.config import get_env_var, HNS_CORE_ROOT
...
def get_remote_version(branch_to_compare):
    global config
    decoded_file = get_file_contents(config.github_token, HNS_CORE_REPO,
...
def parse_issues_fixed(body):
    try:
        regex = r'((Resolves|Fixes|Fix|Closes|Close|resolves|fixes|fix|closes|close) https:\/\/github\.com\/hns\/hns-browser\/issues\/(\d*))'  # nopep8
...
def main():
    ...
    result = fetch_origin_check_staged(HNS_CORE_ROOT)
    if result != 0:
        return result

    # get all channel branches (starting at master)
    hns_core_version = get_remote_version('master')
    remote_branches = get_remote_channel_branches(hns_core_version)
    ...
    if args.uplift_using_pr:
        try:
            ...
            repo = GitHub(config.github_token).repos(HNS_CORE_REPO)
    ...
    local_branch = get_local_branch_name(HNS_CORE_ROOT)
    if not config.title and not args.uplift_using_pr:
        config.title = get_title_from_first_commit(HNS_CORE_ROOT,
                                                   top_level_base)
    ...
    push_branches_to_remote(HNS_CORE_ROOT,
...
def get_milestone_for_branch(channel_branch):
    ...
    if not config.milestones:
        config.milestones = get_milestones(config.github_token, HNS_CORE_REPO)
