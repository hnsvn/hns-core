...
# Map of google_chrome_strings.grd resources ids to migrate to hns_strings.grd
# The resources and all translations will be migrated to grd and xtb files.
# key - id in google_chrome_strings.
# value - new id in hns_stirngs.
...
def hnsify_grd_text(text, is_main_text, branding_replacements_only):
    """Replaces text string to Hns wording"""
...
def generate_hnsified_node(elem, is_comment, branding_replacements_only):
    """Replaces a node and attributes to Hns wording"""
    if elem.text:
        elem.text = hnsify_grd_text(
            elem.text, not is_comment, branding_replacements_only)

    if elem.tail:
        elem.tail = hnsify_grd_text(
            elem.tail, not is_comment, branding_replacements_only)

    if 'desc' in elem.keys():
        elem.attrib['desc'] = hnsify_grd_text(
            elem.attrib['desc'], False, branding_replacements_only)
    for child in elem:
        generate_hnsified_node(child, is_comment, branding_replacements_only)
...
def update_hnsified_grd_tree_override(source_xml_tree,
                                        branding_replacements_only):
    """Takes in a grd(p) tree and replaces all messages and comments with Hns
       wording"""
    for elem in source_xml_tree.xpath('//message'):
        generate_hnsified_node(elem, False, branding_replacements_only)
    for elem in source_xml_tree.xpath('//comment()'):
        generate_hnsified_node(elem, True, branding_replacements_only)


def write_hnsified_grd_override(source_string_path):
    """Takes in a grd file and replaces all messages and comments with Hns
       wording"""
    source_xml_tree = lxml.etree.parse(source_string_path)
    update_hnsified_grd_tree_override(source_xml_tree, False)
    write_xml_file_from_tree(source_string_path, source_xml_tree)


def get_override_file_path(source_string_path):
    """Obtain src/hns source string override path for local grd strings with
       replacements"""
...
def update_xtbs_locally(grd_file_path, hns_source_root):
    ...
    # Special treatment for hns_strings.grd
    hns_strings_string_ids = []
    if os.path.basename(grd_file_path) == 'hns_strings.grd':
        assert len(grd_strings) == len(chromium_grd_strings) + \
            len(GOOGLE_CHROME_STRINGS_MIGRATION_MAP)
        hns_strings_string_ids = remove_google_chrome_strings(
    ...
    for idx, xtb_file in enumerate(xtb_file_paths):
        ...
        for node in xml_tree.xpath('//translation'):
            generate_hnsified_node(node, False, True)
...
def get_chromium_grd_src_with_fallback(grd_file_path, hns_source_root):
    source_root = os.path.dirname(hns_source_root)
    chromium_grd_file_path = get_original_grd(source_root, grd_file_path)
    if not chromium_grd_file_path:
        rel_path = os.path.relpath(grd_file_path, hns_source_root)
...
def remove_google_chrome_strings(hns_grd_strings, google_chrome_strings_map):
    string_ids = []
    string_names = [
        string_name[4:].lower()
    ...
    to_remove = []
    for string_tuple in hns_grd_strings:
    ...
    for string_tuple in to_remove:
        hns_grd_strings.remove(string_tuple)

    return string_ids


def add_google_chrome_translations(hns_strings_xtb_file, xml_tree,
                                   string_ids):
    hns_xtb_tree = lxml.etree.parse(hns_strings_xtb_file)
    translationbundle = xml_tree.xpath('//translationbundle')[0]
    for string_id in string_ids:
        translation = hns_xtb_tree.xpath(
...
def is_translateable_string(grd_file_path, message_tag):
    ...
    # to be translateable in Hns. These can be found in the main function in
    # hns/script/chromium-rebase-l10n.py
    grd_file_name = os.path.basename(grd_file_path)
