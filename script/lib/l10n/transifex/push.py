...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
    """Checks the Hns GRD file vs the Chromium GRD file for extra
       languages."""
    chromium_grd_file_path = get_original_grd(src_root, grd_file_path)
    if not chromium_grd_file_path:
        return
    hns_langs = get_grd_languages(grd_file_path)
    chromium_langs = get_grd_languages(chromium_grd_file_path)
    x_hns_extra_langs = hns_langs - chromium_langs
    assert len(x_hns_extra_langs) == 0, \
        f'Hns GRD {grd_file_path} has extra languages ' \
            f'{list(x_hns_extra_langs)} over Chromium GRD ' \
            f'{chromium_grd_file_path}'
    x_chromium_extra_langs = chromium_langs - hns_langs
    assert len(x_chromium_extra_langs) == 0, \
        f'Chromium GRD {chromium_grd_file_path} has extra languages ' \
            f'{list(x_chromium_extra_langs)} over Hns GRD {grd_file_path}'
...
def upload_translation_to_transifex(source_string_path, lang_code, filename,
    ...
    resource_name = transifex_name_from_filename(source_string_path, filename)
    string_hash = get_transifex_string_hash(string_name)
    translated_value = hnsify(translated_value)
    get_api_wrapper().transifex_upload_string_l10n(
        resource_name, string_name, string_hash, lang_code, translated_value,
        missing_only)


def hnsify(string_value):
    """Replace Chromium branded strings with Hns branded strings."""
    return (string_value.replace('Chrome', 'Hns')
            .replace('Chromium', 'Hns')
            .replace('Google', 'Hns')
            .replace('Hns Docs', 'Google Docs')
            .replace('Hns Drive', 'Google Drive')
            .replace('Hns Play', 'Google Play')
            .replace('Hns Safe', 'Google Safe')
            .replace('Sends URLs of some pages you visit to Hns',
                     'Sends URLs of some pages you visit to Google')
            .replace('Hns Account', 'Hns sync chain')
            .replace('Hns Lens', 'Google Lens')
            .replace('Hnsbook', 'Chromebook')
            .replace('Hnscast', 'Chromecast')
            .replace('Hns Cloud', 'Google Cloud')
            .replace('Hns Pay', 'Google Pay')
            .replace('Hns Photos', 'Google Photos')
            .replace('Hns Projects', 'Chromium Projects')
            .replace('Hns Widevine', 'Google Widevine'))
