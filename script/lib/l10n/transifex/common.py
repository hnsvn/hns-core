...
transifex_handled_slugs = [
    'android_hns_strings',
    'hns_generated_resources',
    'hns_components_resources',
    'hns_extension',
    'rewards_extension',
...
def transifex_name_from_filename(source_file_path, filename):
    ...
    if 'hns_components_strings' in source_file_path:
        return 'hns_components_resources'
    if ext == '.grd':
        return filename
    if 'hns-site-specific-scripts/scripts/' in source_file_path:
        return transifex_name_from_greaselion_script_name(source_file_path)
    if 'hns_extension' in source_file_path:
        return 'hns_extension'
    if 'hns_rewards' in source_file_path:
...
def transifex_name_from_greaselion_script_name(script_name):
    match = re.search(('hns-site-specific-scripts/scripts/(.*)/_locales/' +
...
def get_acceptable_json_lang_codes(langs_dir_path):
    ...
    # Source language for Hns locales
...
def get_api_wrapper():
    if get_api_wrapper.wrapper is None:
        get_api_wrapper.wrapper = APIWrapper(project_name=hns_project_name)
