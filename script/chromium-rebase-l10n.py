...
from lib.l10n.grd_utils import (GOOGLE_CHROME_STRINGS_MIGRATION_MAP,
                                ...
                                update_hnsified_grd_tree_override,
                                write_xml_file_from_tree,
                                write_hnsified_grd_override)
from lxml import etree  # pylint: disable=import-error

HNS_SOURCE_ROOT = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
SRC_SOURCE_ROOT = os.path.abspath(os.path.dirname(HNS_SOURCE_ROOT))
...
def write_new_translation_to_xtb(xtb_path, message_id, text):
    ...
    hns_xtb_xml_tree = etree.parse(xtb_path, parser)
    google_elem = hns_xtb_xml_tree.xpath(
        '//translation[@id="{}"]'.format(message_id))
    bundle_element = hns_xtb_xml_tree.xpath('//translationbundle')[0]
    element = google_elem[0] if google_elem else etree.SubElement(
        bundle_element, 'translation')
    element.set('id', message_id)
    element.text = text
    write_xtb_content(hns_xtb_xml_tree, xtb_path)
        ...
        if not lang:
            print(
                'Skipping file {} because unable to determine language'.format(
                    google_xtb_path))
            continue
        hns_xtb_path = os.path.join(
            HNS_SOURCE_ROOT,
            'app/resources/chromium_strings_{}.xtb'.format(lang))
        if not os.path.exists(hns_xtb_path):
            print('Unable to find hns translation file {}'.format(
                hns_xtb_path))
            return False
        write_new_translation_to_xtb(hns_xtb_path, message_id,
                                     google_elem.text)

    return True


def migrate_google_chrome_strings(hns_strings_xml_tree,
    ...
    for item in google_chrome_strings_map.keys():
        ...
        messages_element = hns_strings_xml_tree.xpath('//messages')[0]
...
def generate_overrides_and_replace_strings(source_string_path):
    ...
    write_hnsified_grd_override(source_string_path)
    ...
    for part in parts:
        ...
        # Check for the special case of hns_stings.grd:
        if (os.path.basename(source_string_path) == 'hns_strings.grd'
                and override_file == 'settings_chromium_strings_override.grdp'):
            override_file = 'settings_hns_strings_override.grdp'
...
def main():
    ...
    args = parse_args()
    # This file path is a string path inside hns/ but just recently copied
    # in from chromium files which need replacements.
    source_string_path = os.path.join(HNS_SOURCE_ROOT,
    ...
    # is_translateable_string function in hns/script/lib/transifex_common.py.
    xml_tree = etree.parse(source_string_path)
    (basename, _) = filename.split('.')
    if basename == 'hns_strings':
        ...
        elem1.text = 'Inbound rule for Hns Beta to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_CANARY"]')[0]
        elem1.text = 'Inbound rule for Hns Nightly to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_DEV"]')[0]
        elem1.text = 'Inbound rule for Hns Dev to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//part[@file="settings_chromium_strings.grdp"]')[0]
        elem1.set('file', 'settings_hns_strings.grdp')
        elem1 = xml_tree.xpath(
    ...
    # Fix output filenames to generate "hns" files instead of "chromium".
    if basename in ('hns_strings', 'components_hns_strings'):
        for pak_filename in xml_tree.xpath(
            ...
            pak_filename.attrib['filename'] = pak_filename.attrib[
                'filename'].replace('chromium_strings', 'hns_strings')
