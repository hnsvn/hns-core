...
hns_channel = os.environ.get('HNS_CHANNEL')


def GenerateHnsWidevineSigFile(paths, config, part):
...
def HnsModifyPartsForSigning(parts, config):
    """ Inserts Hns specific parts that need to be signed """
    ...
    parts['privileged-helper'].path = re.sub(
        r'com.hns.Browser(.*).UpdaterPrivilegedHelper',
...
def GetHnsSigningConfig(config_class, mac_provisioning_profile=None):
    """ Creates Hns specific config used for signing """
    class ConfigNonChromeBranded(config_class):  # pylint: disable=too-few-public-methods
        ...
        def distributions(self):
            """ Hns distribution """
            return [model.Distribution(channel=hns_channel)]
