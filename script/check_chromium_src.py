...
HNS_SRC = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
HNS_CHROMIUM_SRC = os.path.join(HNS_SRC, 'chromium_src')
CHROMIUM_SRC = os.path.abspath(os.path.dirname(HNS_SRC))
...
EXCLUDES = [
    ...
    'chrome/installer/linux/common/hns-browser/chromium-browser.appdata.xml',
    'chrome/installer/linux/common/hns-browser/chromium-browser.info',
    'chrome/installer/setup/hns_behaviors.cc',
    'content/browser/tld_ephemeral_lifetime.cc',
    'content/public/browser/tld_ephemeral_lifetime.h',
    'third_party/blink/renderer/modules/storage/hns_dom_window_storage.h',
]
...
def do_check_includes(override_filepath):
    ...
    with open(override_filepath, mode='r', encoding='utf-8') as override_file:
        ...
        # +3 to get to chromium_src->hns->src.
...
def filter_chromium_src_filepaths(include_regexp=None, exclude_regexp=None):
    """
    Return a list of paths pointing to the files in |HNS_CHROMIUM_SRC|
    ...
    for dir_path, _dirnames, filenames in os.walk(HNS_CHROMIUM_SRC):
        for filename in filenames:
            full_path = os.path.join(dir_path, filename)

            # Strip the HNS_CHROMIUM_SRC prefix to match against the
            # list of relative paths to be included and/or excluded
            relative_path = full_path.replace(HNS_CHROMIUM_SRC, '')[1:]
